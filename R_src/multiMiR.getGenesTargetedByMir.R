...
multiMiR.getGenesTargetedByMir <- function(mirID,organism,outputDir,outputPrefix,tbl='all',cutoffType='p',cutoff=20) {
  ...
  output=get_multimir(org=organism,mirna=mirID, table=tbl,predicted.cutoff.type=cutoffType,predicted.cutoff=cutoff,summary=TRUE,add.link=TRUE,legacy.out = TRUE)
