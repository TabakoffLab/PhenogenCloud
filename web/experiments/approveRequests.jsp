...
<%
	...
	if (!hybridIDs.equals("()")) {
		mySessionHandler.createSessionActivity(session.getId(), 
			"Looked at array requests.  There are " + myResults.getNumRows() + " arrays to be approved.", pool);
...
//					thisArray.setArray_users(new int[] {thisUser_id});
		...
		mySessionHandler.createSessionActivity(session.getId(), 
			"Looked at array requests, but there are none pending.", pool);
	}
