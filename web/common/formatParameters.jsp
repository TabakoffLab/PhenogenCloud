...
<%
    ...
    if (parameterType.equals("dataset") || parameterType.equals("datasetVersion")) {
        ...
        //
        hybridIDs = thisDataset.getDatasetHybridIDs(pool);
        if (!hybridIDs.equals("()")) {
            myArrays = myArray.getArraysForDataset(hybridIDs, pool);
            //log.debug("here myArrays length = "+myArrays.length);
            thisDataset.setArray_type(myArray.getDatasetArrayType(hybridIDs, pool));
        }
        if (parameterType.equals("dataset")) {
            ...
            normalizationParameters =
                    myParameterValue.getNormalizationParameters(
                            ...
                            thisDatasetVersion.getVersion(), pool);
            chipAssignments =
                    thisDataset.new Group().getChipAssignments(thisDatasetVersion.getGrouping_id(), pool);
...
<%
        ...
        myParameterValues = myParameterValue.getFiltersUsed(parameterGroupID, pool);
        mySessionHandler.createActivity("Looked at details for filter.  PGID = " + parameterGroupID,pool);
    } else if (parameterType.equals("phenotype")) {
        printPhenotypeParameters = true;
        mySessionHandler.createActivity("Looked at details for phenotype ", pool);
    } else {
        mySessionHandler.createActivity("Looked at details of something.  parameterType = " + parameterType + " and PGID = " + parameterGroupID,
                pool);
        myParameterValues = myParameterValue.getParameterValues(parameterGroupID, pool);
    }
...
<center>
    <div>
        <p style="font-size:12px; margin:10px 0px;">
            (Click the <img src="<%=imagesDir%>icons/add.png" alt="expand"> and <img src="<%=imagesDir%>icons/min.png"
                                                                                     alt="contract">
    ...
    <div id="datasetStuff">
        <table class="list_base" cellpadding="0" cellspacing="3" style="margin:0px 10px">
            <tr>
                ...
                <td><%=thisDataset.getName()%>
                </td>
            </tr>
            <tr>
                <td><b>Description:</b></td>
                <td><%=thisDataset.getDescription()%>
                </td>
            </tr>
            <tr>
                <td><b>Organism:</b></td>
                <td><%=thisDataset.getOrganism()%>
                </td>
            </tr>
            <tr>
                <td><b>Date Created:</b></td>
                <td><%=thisDataset.getCreate_date_as_string()%>
                </td>
            </tr>
            <tr>
                <td><b>Platform:</b></td>
                <td><%=thisDataset.getPlatform()%>
                </td>
            </tr>
            <tr>
                <td><b># of Arrays:</b></td>
                <td><%=thisDataset.getNumber_of_arrays()%>
                </td>
            </tr>
            <tr>
                <td><b>Array Used:</b></td>
                <td><%=thisDataset.getArray_type()%>
                </td>
            </tr>
            <tr>
                <td><b>Quality Control Status:</b></td>
                <td><%=qcStatus%>
                </td>
            </tr>
        </table>
    ...
    <div id="normalizationParametersStuff">
        <table id="normalization_parameters" class="list_base tablesorter" cellpadding="0" cellspacing="3"
            ...
            <tr>
                <td><b><%=paramsToDisplay[i].getParameter()%>
                </b></td>
                <td><%=paramsToDisplay[i].getValue()%>
                </td>
            </tr>
            <% } %>
            </tbody>
    ...
    <div id="<%=parameterType%>_parameters_Stuff" style="display:none">
        <table id="<%=parameterType%>_parameters" class="list_base tablesorter" cellpadding="0" cellspacing="3">
            ...
            <table class="list_base">
                <tr>
                    <td><b>Phenotype Name:<%=twoSpaces%>
                    </b></td>
                    <td><%=thisPhenotype.getName()%>
                    </td>
                </tr>
                <tr>
                    <td><b>Description:<%=twoSpaces%>
                    </b></td>
                    <td><%=thisPhenotype.getDescription()%>
                    </td>
                </tr>
                <%
    ...
    <div id="groupAssignmentStuff" style="display:none">
        ...
        <table class="list_base" id="groupAssignments" cellpadding="0" cellspacing="3">
            ...
            <% for (int i = 0; i < chipAssignments.length; i++) { %>
            <tr>
                <td><b><%=chipAssignments[i].getHybrid_name()%>
                </b></td>
                <td>Group #: <%=chipAssignments[i].getGroup().getGroup_number()%>
                    :<%=twoSpaces%><%=chipAssignments[i].getGroup().getGroup_name()%>
                </td>
            </tr>
            <% } %>
            </tbody>
    ...
    <div id="normalizedVersionStuff">
        <table class="list_base" id="normalizedVersions" cellpadding="0" cellspacing="3">
            ...
            %>
            <tr>
                <td class="center"><%=thisVersion.getVersion()%>
                </td>
                <td><%=thisVersion.getVersion_name()%>
                </td>
                <td class="center"><%=thisVersion.getNumber_of_non_exclude_groups()%>
                </td>
                <td class="center"><%=thisVersion.getNormalization_method()%>
                </td>
            </tr>
            <% } %>
            </tbody>
    ...
    <div id="arrayStuff" style="display:none">
        <table class="list_base" id="arrayList" cellpadding="0" cellspacing="3">
            ...
            <tr>
                <td><b><%=myArrays[i].getHybrid_name()%>
                </b></td>
                <td><%=myArrays[i].getFile_name()%>
                </td>
            </tr>
            <% } %>
            </tbody>
    ...
    <div id="geneLists" style="display:none">
        <table class="list_base" id="geneList" cellpadding="0" cellspacing="3">
            ...
            <tr>
                <td>
                    <a href="<%=geneListsDir%>chooseGeneList.jsp?geneListID=<%=geneList2.getGene_list_id()%>"><%=geneList2.getGene_list_name()%>
                    </a>
                </td>
            </tr>
            <% } %>
    ...
    <% if (printGeneListDetails) {
        log.debug("print geneListDetails"); %>
    <div class="title">
    ...
    <div id="geneListStuff">
        <table class="list_base " cellpadding="0" cellspacing="3" style="margin:0px 10px;">
            <tr>
                <td><b>Gene List Name:</b></td>
                <td><%=selectedGeneList.getGene_list_name()%>
                </td>
            </tr>
            <tr>
                <td><b>Description:</b></td>
                <td><%=selectedGeneList.getDescription()%>
                </td>
            </tr>
            <tr>
                <td><b>Organism:</b></td>
                <td><%=selectedGeneList.getOrganism()%>
                </td>
            </tr>
            <tr>
                <td><b>Date Created:</b></td>
                <td><%=selectedGeneList.getCreate_date_as_string()%>
                </td>
            </tr>
            <tr>
                <td><b>Source:</b></td>
                <td><%=selectedGeneList.getGene_list_source()%>
                </td>
            </tr>
            <tr>
                <td><b>Owner:</b></td>
                <td><%=selectedGeneList.getGene_list_owner()%>
                </td>
            </tr>
        </table>
    </div>
    <div class="brClear"></div>
    <% } %>
    <% if (printGeneListParameters) {
        log.debug("print geneListParameters");
    ...
    <% } %>
    <% if (printPhenotypeParameters) {
        log.debug("print PhenotypeParameters");
    ...
    <div id="phenotypeStuff">
        <table id="phenotype_parameters" class="list_base" cellpadding="0" cellspacing="3" width="60%">
            ...
            %>
            <tr>
                <td><b><%=group.getGroup_name()%>
                </b></td>
                <% if (thisDataset.CALC_QTL_DATASETS.contains(thisDataset.getName())) { %>
                <td class="center"><%=(group.getHas_expression_data().equals("Y") ? checkMark : "-")%>
                </td>
                <td class="center"><%=(group.getHas_genotype_data().equals("Y") ? checkMark : "-")%>
                </td>
                <% } %>
                <td class="right"><%=mean%>
                </td>
                <% if (!variance.equals("")) { %>
                <td class="right"><%=variance%>
                </td>
                <% } %>
            </tr>
            <% } %>
