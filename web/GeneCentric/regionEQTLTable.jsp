...
<div id="eQTLListFromRegion" style="width:100%;">
    ...
    <div id="qtlTableDiv" style="display:inline-block;">
        ...
        <div style=" float:right; ">
            <%if (idListCount <= 300) {%>
            <a href="http://david.abcc.ncifcrf.gov/api.jsp?type=AFFYMETRIX_EXON_GENE_ID&ids=<%=idList%>&tool=summary"
               target="_blank">View DAVID Functional Annotation</a>
            <div class="inpageHelp" style="display:inline-block;"><img id="HelpDAVID" class="helpImage"
                                                                       src="../web/images/icons/help.png"/></div>
            <%} else {%>
            Too many genes to submit to DAVID automatically. Filter or copy and submit on your own <a
                href="http://david.abcc.ncifcrf.gov/" target="_blank">here</a>.<span class="eQTLListToolTip"
                                                                                     title=""><img
        ...
        <TABLE name="items" id="tblFrom" class="list_base" cellpadding="0" cellspacing="0" border="0" width="100%">
            ...
            <TR class="col_title">
                ...
                <TH>Description <span class="eQTLListToolTip" title="The description from Ensembl if available."><img
                        src="<%=imagesDir%>icons/info.gif"></span></TH>
                <TH>Transcript Cluster ID <span class="eQTLListToolTip"
                                                title="Transcript Cluster ID- The unique ID assigned by Affymetrix.  <a href='<%=commonDir%>definitions.jsp#eQTLs' target='_blank'>eQTLs</a> are calculated for this annotation at the gene level by combining probe set data across the gene."><img
                        src="<%=imagesDir%>icons/info.gif"></span></TH>
                <TH>Annotation Level <span class="eQTLListToolTip"
                                           title="Annotation level from Affymetrix is related to the confidence of the annotation of the gene.  Core is the highest confidence and tends to correspond very closely with the Ensembl gene annotation. Extended is lower confidence and may include additional regions outside of the Ensembl annotated exons.  Full is even lower.  Ambiguous is the lowest confidence."><img
                        src="<%=imagesDir%>icons/info.gif"></span></TH>
            ...
            <TR>
                <TD>
                    ...
                    <a href="<%=lg.getGeneLink(tc.getGeneID(),myOrganism,true,true,false)%>" target="_blank"
                ...
                <TD>
                    <%if (!tc.getGeneID().equals("")) {%>
                    <a href="<%=LinkGenerator.getEnsemblLinkEnsemblID(tc.getGeneID(),fullOrg)%>" target="_blank"
                       title="View Ensembl Gene Details"><%=tc.getGeneID()%>
                    </a><BR/>
                ...
                <TD><%=tc.getTranscriptClusterID()%>
                </TD>
                <TD><%=tc.getLevel()%>
                </TD>

                <TD>chr<%=tc.getChromosome() + ":" + dfC.format(tc.getStart()) + "-" + dfC.format(tc.getEnd())%>
                </TD>
                <TD>
                    <a href="web/GeneCentric/setupLocusSpecificEQTL.jsp?geneSym=<%=tc.getGeneSymbol()%>&ensID=<%=tc.getGeneID()%>&chr=<%=tc.getChromosome()%>&start=<%=tc.getStart()%>&stop=<%=tc.getEnd()%>&level=<%=tc.getLevel()%>&tcID=<%=tc.getTranscriptClusterID()%>&curDir=<%=folderName%>"
    ...
    <%}%>
    <%} else {%>
    <strong><%=session.getAttribute("getTransControllingEQTL")%>
    </strong>
    <%}%>
    <div class="regionSubHeader" style="font-size:18px; font-weight:bold; text-align:center; width:100%;">
        <span class="trigger less triggerEC" name="eQTLRegionNote">EQTL Region</span>
        <span class="eQTLListToolTip"
              title="This section lists the regions being reported as begin associated with control of the genes following.  It is important to note that the region entered may only be close to or overlap with a defined SNP or Region between SNPs and may larger than the region selected or may only include part of the region selected."><img
                src="<%=imagesDir%>icons/info.gif"></span>
    </div>
    <div id="eQTLRegionNote" style="width:100%;">
        Genes controlled from and P-values reported for eQTLs from this region are not specific to the region you
        entered. The "P-value from region" columns correspond to the following region(s):<BR/>
        <%for (int i = 0; i < eQTLRegions.size(); i++) {%>
        <a href="<%=request.getContextPath()%>/gene.jsp?geneTxt=<%=eQTLRegions.get(i)%>&speciesCB=<%=myOrganism%>&auto=Y&newWindow=Y"
           target="_blank"><%=eQTLRegions.get(i)%>
        </a><BR/>
        <%}%>
...
<div id="filterdivEQTL" class="filterdivEQTL"
     style="background-color:#F8F8F8;display:none;position:absolute;z-index:999; border:solid;border-color:#000000;border-width:1px;">
                ...
                <table style="width:100%;">
                    ...
                    <tr>
                        <td style="text-align:center;">
                            ...
                            <span class="eQTLListToolTip"
                                  title="Filters the genes displayed in both the image(Circos Plot) and table to only those annotated with the included annotations.  Annotations are related to confidence in the Affymetrix annotation and range from High to Low in the following order: <BR> <ul><li>-Core</li><li>-Extended</li><li>-Full</li><li>-Ambiguous</li></ul>"><img
                                    src="<%=imagesDir%>icons/info.gif"></span>
                        </td>
                    </tr>
                    <TR>
                    ...
                    <tr>
                        <td>
                            ...
                            <select name="trxAnnotMS" id="trxAnnotMS" class="multiselect" size="4" multiple="true">
                                ...
                                <option value="extended" <%if(levelString.contains("extended")){%>selected<%}%>>
                                    Extended
                                </option>
                                <option value="full" <%if(levelString.contains("full")){%>selected<%}%>>Full</option>
                                <option value="ambiguous" <%if(levelString.contains("ambiguous")){%>selected<%}%>>
                                    Ambiguous
                                </option>
                            </select>
