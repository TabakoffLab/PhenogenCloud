    	...
    	<%for(int i=0;i<lines.length;i++){
            ...
            if(col.length>4){
                ...
                <TR id="<%=col[0]+":"+col[1]+"-"+col[2]+":::"+targetSpecies.substring(0,2)%>">
                    ...
                    <TD><a href="/gene.jsp?geneTxt=<%=col[0]+":"+df0.format(tmpStart)+"-"+df0.format(tmpStop)%>&speciesCB=<%=targetSpecies.substring(0,2)%>&auto=Y&newWindow=Y&genomeVer=<%=targetSpecies.toLowerCase()%>" target="_blank">View Region in New Window</a></TD>
                </TR>
            <%}%>
        <%}%>
    </tbody>
