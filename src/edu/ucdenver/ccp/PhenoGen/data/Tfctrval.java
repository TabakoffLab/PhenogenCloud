...
public class Tfctrval {
     ...
     */
    public Tfctrval[] getAllTfctrval(DataSource pool) throws SQLException {
        ...
        Tfctrval[] myTfctrval = new Tfctrval[0];
        try (Connection conn = pool.getConnection()) {
            Results myResults = new Results(query, conn);
            myTfctrval = setupTfctrvalValues(myResults);
            myResults.close();
     ...
     */
    public Tfctrval getTfctrval(int tfctrval_sysuid, DataSource pool) throws SQLException {
        ...
        //log.debug("query =  " + query);
        Tfctrval myTfctrval = null;
        try (Connection conn = pool.getConnection()) {
            Results myResults = new Results(query, tfctrval_sysuid, conn);
            myTfctrval = setupTfctrvalValues(myResults)[0];
            myResults.close();
        } catch (SQLException e) {
            log.debug("SQL Exception:", e);
            throw e;
    ...
    public void update(DataSource pool) throws SQLException {
        String query =
                "update Tfctrval " +
                        "set tfctrval_expfctrid = ?, tfctrval_sampleid = ?, tfctrval_freetext = ?, tfctrval_freetextunit = ?, " +
                        "tfctrval_subid = ?, tfctrval_del_status = ?, tfctrval_user = ?, tfctrval_last_change = ? " +
                        "where tfctrval_sysuid = ?";
        log.debug("query =  " + query);
        java.sql.Timestamp now = new java.sql.Timestamp(System.currentTimeMillis());
        try (Connection conn = pool.getConnection()) {
            PreparedStatement pstmt = conn.prepareStatement(query,
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_UPDATABLE);
            pstmt.setInt(1, tfctrval_expfctrid);
            pstmt.setInt(2, tfctrval_sampleid);
            pstmt.setString(3, tfctrval_freetext);
            pstmt.setString(4, tfctrval_freetextunit);
            pstmt.setInt(5, tfctrval_subid);
            pstmt.setString(6, tfctrval_del_status);
            pstmt.setString(7, tfctrval_user);
            pstmt.setTimestamp(8, now);
            pstmt.setInt(9, tfctrval_sysuid);
            pstmt.executeUpdate();
            pstmt.close();
        } catch (SQLException e) {
            log.debug("SQL Exception:", e);
    ...
    public void deleteTfctrval(DataSource pool) throws SQLException {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public void deleteAllTfctrvalForTsample(int tsample_sysuid, DataSource pool) throws SQLException {
        ...
        try(Connection conn=pool.getConnection()){
            ...
            while ((dataRow = myResults.getNextRow()) != null) {
                new Tfctrval(Integer.parseInt(dataRow[0])).deleteTfctrval(pool);
            }
            myResults.close();
        }catch(SQLException e){
            log.debug("SQL Exception:",e);
     ...
     */
    public int checkRecordExists(Tfctrval myTfctrval, DataSource pool) throws SQLException {
