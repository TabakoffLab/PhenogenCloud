...
public class Protocol {
    ...
    private void isRequiredAndValid(String value, String type, String messageWords, DataSource pool) throws DataException, SQLException {
        isRequired(value, messageWords);
        isValid(value, type, messageWords, pool);
    }
    ...
    private void isValid(String value, String type, String messageWords, DataSource pool) throws DataException, SQLException {
        if (!myObjectHandler.isEmpty(value) &&
                !value.equals(NONE) &&
                getProtocol_id(value, type, pool) == -99) {
    ...
    public void validateTreatment_protocol(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateTreatment_protocol");
        isValid(value, TREATMENT_TYPE, "Sample Treatment Protocol", pool);
    }

    public void validateGrowth_protocol(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateGrowth_protocol");
        isValid(value, GROWTH_TYPE, "Sample Growth Protocol", pool);
    }

    public void validateExtract_protocol(String value,DataSource pool) throws DataException, SQLException {
        log.debug("in validateExtract_protocol");
        isRequiredAndValid(value, EXTRACT_TYPE, "Extract Protocol", pool);
    }

    public void validateLabel_protocol(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateLabel_protocol");
        isRequiredAndValid(value, LABEL_TYPE, "Labeled Extract Protocol", pool);
    }

    public void validateHybrid_protocol(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateHybrid_protocol");
        isRequiredAndValid(value, HYBRID_TYPE, "Hybridization Protocol", pool);
    }

    public void validateScan_protocol(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateScan_protocol");
        isRequiredAndValid(value, SCAN_TYPE, "Scanning Protocol", pool);
    }
     ...
     */
    public Protocol[] getPrivateProtocols(DataSource pool) throws SQLException {
        ...
        Protocol[] protocolArray = new Protocol[0];
        try(Connection conn=pool.getConnection()){
            Results myResults = new Results(query, conn);
            protocolArray = setupProtocolValues(myResults);
            myResults.close();
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public Protocol getProtocol(int protocol_id, DataSource pool) throws SQLException {
        ...
        try(Connection conn=pool.getConnection()){
            ...
            myProtocol = (myResults != null && myResults.getNumRows() > 0 ?
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public int getProtocol_id(String protocol_name, String type, DataSource pool) throws SQLException {

        log.debug("in getProtocol_id.  name = " + protocol_name + ", and type = " + type);
        Protocol myProtocol = getProtocolByNameAndType(protocol_name, type, pool);
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public Protocol[] getProtocolByUserAndType(String user, String type, DataSource pool) throws SQLException {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public Protocol getProtocolByUserAndGlobid(User userLoggedIn, int globid, String type, DataSource pool) throws SQLException {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public Protocol getProtocolByNameAndType(String protocol_name, String type, DataSource pool) throws SQLException {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public Protocol getPublicProtocolByGlobid(int globid, DataSource pool) throws SQLException {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public Protocol[] getPublicProtocols(DataSource pool) throws SQLException, MalformedURLException {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public int createForPublicProtocol(User userLoggedIn, int globid, String protocolType, DataSource pool) throws SQLException {

        log.debug("in Protocol createForPublicProtocol with globid. globid = " + globid);

        Protocol myProtocol = new Protocol();

        int protocolCode = new ValidTerm().getValidTermByValueAndID(protocolType, "PROTOCOL_HEADER_TYPE", pool).getTerm_id();
        log.debug("protocolType = " + protocolType + ", code = " + protocolCode);

        int protocol_id = getProtocolByUserAndGlobid(userLoggedIn, globid, protocolType, pool).getProtocol_id();
        ...
        if (protocol_id == -99) {
            ...
            return myProtocol.createProtocol(pool);
        } else {
            return protocol_id;
        }
    }
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public int createProtocol(User userLoggedIn, Hashtable<String, String> fieldValues, DataSource pool) throws SQLException {
        ...
        int protocolCode = new ValidTerm().getValidTermByValueAndID((String) fieldValues.get("protocolType"),
                "PROTOCOL_HEADER_TYPE", pool).getTerm_id();
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public int createProtocol(DataSource pool) throws SQLException {
        ...
        String query =
                "insert into protocols " +
                        "(protocol_type, protocol_name, protocol_description, globid, " +
        ...
        try(Connection conn=pool.getConnection()) {
            ...
            pstmt.setInt(1, protocol_type);
            pstmt.setString(2, protocol_name);
            pstmt.setString(3, protocol_description);
            log.debug("going to set to null if zero.  it is = " + globid);
            myDbUtils.setToNullIfZero(pstmt, 4, globid);
    ...
    public void update(DataSource pool) throws SQLException {
        ...
        String query =
                "update protocols " +
                        "set protocol_type = ?, protocol_name = ?, protocol_description = ?, globid = ?, " +
                        "created_by_login = ?, protocol_create_date = ? " +
                        "where protocol_id = ?";
        ...
        try(Connection conn=pool.getConnection()) {
            ...
            pstmt.setString(2, protocol_name);
            pstmt.setString(3, protocol_description);
            pstmt.setInt(4, globid);
            pstmt.setString(5, created_by_login);
            pstmt.setTimestamp(6, protocol_create_date);
            pstmt.setInt(7, protocol_id);
            pstmt.executeUpdate();
            pstmt.close();
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public void deleteProtocol(DataSource pool) throws SQLException {
        ...
        try(Connection conn=pool.getConnection()) {
            ...
            new Hybridization().deleteAllHybridizationForProtocol(protocol_id, pool);
            new Tlabel().deleteAllTlabelForProtocol(protocol_id, pool);
            new Textract().deleteAllTextractForProtocol(protocol_id, pool);
            new Tsample().deleteAllTsampleForProtocol(protocol_id, pool);
            new Protocol_value().deleteAllProtocol_valuesForProtocol(protocol_id, pool);
            new Experiment_protocol().deleteAllExperiment_protocolsForProtocol(protocol_id, pool);
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public int checkRecordExists(Protocol myProtocol, DataSource pool) throws SQLException {
