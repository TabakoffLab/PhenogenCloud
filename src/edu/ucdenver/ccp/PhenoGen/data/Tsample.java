...
public class Tsample {
    ...
    public void validateSample_name(String value, int expID, DataSource pool) throws DataException, SQLException {
    ...
    public void validateOrganism(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateOrganism.  = " + value);
        if (myObjectHandler.isEmpty(value)) {
            throw new DataException("ERROR: Organism must be entered.");
        } else if (!myObjectHandler.getAsSet(myArray.getArrayOrganisms("All", "Single", pool), "CountName").contains(value)) {
            throw new DataException("ERROR: Value for Organism is invalid.");
        }
    }

    public void validateSex(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateSex.  = " + value);
        if (myObjectHandler.isEmpty(value)) {
            throw new DataException("ERROR: Sex must be entered.");
        } else if (!myObjectHandler.getAsSet(myValidTerm.getFromValidTerm("SEX", pool), "Value").contains(value)) {
            throw new DataException("ERROR: Value for Sex is invalid.");
        }
    }

    public void validateOrganism_part(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateOrganism_part.  = " + value);
        if (myObjectHandler.isEmpty(value)) {
            throw new DataException("ERROR: Organism Part must be entered.");
        } else if (!myObjectHandler.getAsSet(myValidTerm.getFromValidTerm("ORGANISM_PART", pool), "Value").contains(value)) {
    ...
    public void validateSample_type(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateSample_type.  = " + value);
        if (myObjectHandler.isEmpty(value)) {
            throw new DataException("ERROR: Sample Type must be entered.");
        } else if (!myObjectHandler.getAsSet(myValidTerm.getFromValidTerm("SAMPLE_TYPE", pool), "Value").contains(value)) {
            throw new DataException("WARNING: Value for Sample Type was not one of the suggested values, so it was set to 'other'.  " +
                    "The value you entered was saved as well.");
        }
    }

    public void validateDev_stage(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateDev_stage.  = " + value);
        if (myObjectHandler.isEmpty(value)) {
            throw new DataException("ERROR: Development Stage must be entered.");
        } else if (!myObjectHandler.getAsSet(myValidTerm.getFromValidTerm("DEVELOPMENTAL_STAGE", pool), "Value").contains(value)) {
            throw new DataException("WARNING: Value for Development Stage was not one of the suggested values, so it was set to 'other'.  " +
                    "The value you entered was saved as well.");
        }
    }

    public void validateAge(String value) throws DataException, SQLException {
    ...
    public void validateAge_time_unit(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateAge_time_unit.  = " + value);
        if (!myObjectHandler.isEmpty(value) && !myObjectHandler.getAsSet(myValidTerm.getFromValidTerm("TIME_UNIT", pool), "Value").contains(value)) {
            throw new DataException("ERROR: Value for Age Time Unit is invalid.");
        }
    }

    public void validateGenetic_variation(String value,DataSource pool) throws DataException, SQLException {
        log.debug("in validateGenetic_variation.  = " + value);
        if (!myObjectHandler.isEmpty(value) && !myObjectHandler.getAsSet(myValidTerm.getFromValidTerm("GENETIC_VARIATION", pool), "Value").contains(value)) {
            throw new DataException("ERROR: Value for Genetic Modification is invalid.");
        }
    }

    public void validateIndividual_genotype(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateIndividual_genotype.  = " + value);
        if (!myObjectHandler.isEmpty(value) && !myObjectHandler.getAsSet(myArray.getGenotypes("All", "Single", pool), "CountName").contains(value)) {
    ...
    public void validateCell_line(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateCell_line.  = " + value);
        if (!myObjectHandler.isEmpty(value) && !myObjectHandler.getAsSet(myArray.getCellLines("All", "Single", pool), "CountName").contains(value)) {
            throw new DataException("WARNING: Value for Selected Line was not one of the suggested values.  " +
                    "The value you entered was saved as a new value.");
        }
    }

    public void validateStrain(String value, DataSource pool) throws DataException, SQLException {
        log.debug("in validateStrain.  = " + value);
        if (!myObjectHandler.isEmpty(value) && !myObjectHandler.getAsSet(myArray.getStrains("All", "Single", pool), "CountName").contains(value)) {
            throw new DataException("WARNING: Value for Strain was not one of the suggested values.  " +
                    "The value you entered was saved as a new value.");
        }
    }

    public void validateCompound(String value, boolean isCompoundDesign, DataSource pool) throws DataException, SQLException {
        ...
        if (!isCompoundDesign && !myObjectHandler.isEmpty(value) && !value.equals(NOT_APPLICABLE)) {
            ...
            if (myObjectHandler.isEmpty(value) || value.equals(NOT_APPLICABLE)) {
                throw new DataException("ERROR: Compound must be entered for a 'compound treatment design' experiment.");
                    ...
                    !myObjectHandler.getAsSet(myArray.getCompounds("All", "Single", pool), "CountName").contains(value)) {
    ...
    public void validateDose(String value, boolean isCompoundDesign) throws DataException, SQLException {
    ...
    public void validateDose_unit(String value, boolean isCompoundDesign) throws DataException, SQLException {
    ...
    public void validateTreatment(String value, boolean isCompoundDesign, DataSource pool) throws DataException, SQLException {
        ...
        if (!isCompoundDesign && !myObjectHandler.isEmpty(value) && !value.equals(NOT_APPLICABLE)) {
            ...
            if (myObjectHandler.isEmpty(value) || value.equals(NOT_APPLICABLE)) {
                throw new DataException("ERROR: Treatment must be entered for a 'compound treatment design' experiment.");
                    ...
                    !myObjectHandler.getAsSet(myArray.getTreatments("All", "Single",pool), "CountName").contains(value)) {
    ...
    public void validateDuration(String value, boolean isCompoundDesign) throws DataException, SQLException {
    ...
    public boolean isUnique(String sample_name, int expID, DataSource pool) throws DataException, SQLException {
     ...
     */
    public Tsample[] getAllTsample(int experimentID, DataSource pool) throws SQLException {
        ...
        Tsample[] myTsample = new Tsample[0];
        try(Connection conn=pool.getConnection()){
            Results myResults = new Results(query, experimentID, conn);
            myTsample = setupTsampleValues(myResults);
            myResults.close();
        }catch(SQLException e){
            log.debug("SQL Exception:",e);
            throw e;
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public Tsample[] getAllTsampleForProtocol(int protocol_id, DataSource pool) throws SQLException {
     ...
     */
    public Tsample getTsample(int tsample_sysuid, DataSource pool) throws SQLException {
        ...
        //log.debug("query =  " + query);
        Tsample myTsample = null;
        try(Connection conn=pool.getConnection()){
            Results myResults = new Results(query, tsample_sysuid, conn);
            myTsample = setupTsampleValues(myResults)[0];
            myResults.close();
        }catch(SQLException e){
            log.debug("SQL Exception:",e);
            throw e;
    ...
    public void deleteTsample(DataSource pool) throws SQLException {
        ...
        String query =
                "delete from Tsample " +
                        "where tsample_sysuid = ?";
        new Tfctrval().deleteAllTfctrvalForTsample(tsample_sysuid, pool);
        new Tpooled().deleteAllTpooledForTsample(tsample_sysuid, pool);
        try(Connection conn=pool.getConnection()) {
            try {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public void deleteAllTsampleForTntxsyn(int tntxsyn_tax_id, DataSource pool) throws SQLException {
        ...
        try(Connection conn=pool.getConnection()){
            ...
            while ((dataRow = myResults.getNextRow()) != null) {
                new Tsample(Integer.parseInt(dataRow[0])).deleteTsample(pool);
            }
            myResults.close();
        }catch(SQLException e){
            log.debug("SQL Exception:",e);
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public void deleteAllTsampleForExperiment(int exp_id, DataSource pool) throws SQLException {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public void deleteAllTsampleForProtocol(int protocol_id, DataSource pool) throws SQLException {
     ...
     */
    public int checkRecordExists(Tsample myTsample, DataSource pool) throws SQLException {
