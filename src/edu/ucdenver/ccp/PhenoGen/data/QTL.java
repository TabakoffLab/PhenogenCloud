...
public class QTL {
     ...
     */
    public int createQTLList(QTL myQTL, DataSource pool) throws SQLException {
        ...
        String query =
                "insert into qtl_lists " +
                        "( qtl_list_name, created_by_user_id, organism) values " +
                        "( ?, ?, ?)";
        ...
        try(Connection conn=pool.getConnection()) {
            ...
            try {
                ...
                pstmt.setString(1, myQTL.getQtl_list_name());
                pstmt.setInt(2, myQTL.getCreated_by_user_id());
                pstmt.setString(3, myQTL.getOrganism());

                pstmt.executeUpdate();
                ResultSet rs = pstmt.getGeneratedKeys();
                if (rs.next()) {
                    qtl_list_id = rs.getInt(1);
                }


                myQTL.setQtl_list_id(qtl_list_id);
                createLoci(myQTL, pool);
                conn.commit();
            }catch(SQLException e){
    ...
    public void createLoci(QTL myQTL, DataSource pool) throws SQLException {
        ...
        try(Connection conn=pool.getConnection()){
            ...
            try {
                ...
                log.error("in exception of createLoci", e);
                conn.rollback();
                throw e;
            }
    ...
    public void deleteQtlList(int qtlListID, DataSource pool) throws SQLException {
...
*/
	...
	*/
    ...
    public class EQTL implements Comparable {
         ...
         * @throws SQLException if a database error occurs
         */
        public QTL.EQTL[] getExpressionQTLsInList(String organism, int qtlListID, String tissue, Double pValue, DataSource pool) throws SQLException {
            ...
            String query =
                    "select distinct p.identifier, " +
                            ...
                            "and ql.qtl_list_id = ? " +
                            "and cast(if(eq.p_value='<0.000001', '1.E-100', eq.p_value) as DECIMAL(10,9)) < ? ";
