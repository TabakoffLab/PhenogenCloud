...
public class Tlabel {
    ...
    public void validateLabel_name(String value, int expID, DataSource pool) throws DataException, SQLException {
    ...
    private boolean isUnique(String label_name, int expID, DataSource pool) throws DataException, SQLException {
     ...
     */
    public Tlabel[] getAllTlabel(DataSource pool) throws SQLException {
        ...
        //log.debug("query =  " + query);
        Tlabel[] myTlabel = new Tlabel[0];
        try (Connection conn = pool.getConnection()) {
            Results myResults = new Results(query, conn);
            myTlabel = setupTlabelValues(myResults);
            myResults.close();
        } catch (SQLException e) {
            log.debug("SQL Exception:", e);
            throw e;
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public Tlabel[] getAllTlabelForProtocol(int protocol_id, DataSource pool) throws SQLException {
     ...
     */
    public Tlabel getTlabel(int tlabel_sysuid, DataSource pool) throws SQLException {
        ...
        //log.debug("query =  " + query);
        Tlabel myTlabel = null;
        try(Connection conn=pool.getConnection()){
            Results myResults = new Results(query, tlabel_sysuid, conn);
            myTlabel = setupTlabelValues(myResults)[0];
            myResults.close();
        }catch(SQLException e){
            log.debug("SQL Exception:",e);
            throw e;
    ...
    public void update(DataSource pool) throws SQLException {
        ...
        String query =
                ...
                "update Tlabel " +
                        "set tlabel_id = ?, tlabel_desc = ?, tlabel_protocolid = ?, tlabel_extractid = ?, " +
                        "tlabel_del_status = ?, tlabel_user = ?, tlabel_last_change = ? " +
                        "where tlabel_sysuid = ?";
        ...
        try(Connection conn=pool.getConnection()){
            ...
            //pstmt.setInt(1, tlabel_sysuid);
            pstmt.setString(1, tlabel_id);
            pstmt.setString(2, tlabel_desc);
            myDbUtils.setToNullIfZero(pstmt, 3, tlabel_protocolid);
            pstmt.setInt(4, tlabel_extractid);
            pstmt.setString(5, tlabel_del_status);
            pstmt.setString(6, tlabel_user);
            pstmt.setTimestamp(7, now);
            pstmt.setInt(8, tlabel_sysuid);
            pstmt.executeUpdate();
            pstmt.close();
        }catch(SQLException e){
            log.debug("SQL Exception:",e);
    ...
    public void deleteTlabel(DataSource pool) throws SQLException {

        log.info("in deleteTlabel");
        String query =
                "delete from Tlabel " +
                        "where tlabel_sysuid = ?";
        new Tlabhyb().deleteAllTlabhybForTlabel(tlabel_sysuid, pool);
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public void deleteAllTlabelForProtocol(int protocol_id, DataSource pool) throws SQLException {
        ...
        try(Connection conn=pool.getConnection()){
            ...
            while ((dataRow = myResults.getNextRow()) != null) {
                new Tlabel(Integer.parseInt(dataRow[0])).deleteTlabel(pool);
            }
            myResults.close();
        }catch(SQLException e){
            log.debug("SQL Exception:",e);
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public void deleteAllTlabelForTextract(int textract_sysuid, DataSource pool) throws SQLException {
     ...
     */
    public int checkRecordExists(Tlabel myTlabel, DataSource pool) throws SQLException {
