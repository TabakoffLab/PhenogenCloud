...
public class Hybridization {
    ...
    public void validateHybrid_id(String hybrid_id, int expID, DataSource pool) throws DataException, SQLException {
        ...
        } else if (!isUnique(hybrid_id, expID, pool)) {
            throw new DataException("ERROR: Hybrid Names must be unique within this experiment.");
        }
    }

    private boolean isUnique(String hybrid_id, int expID, DataSource pool) throws DataException, SQLException {
     ...
     */
    public Hybridization[] getAllHybridization(DataSource pool) throws SQLException {
        ...
        Hybridization[] myHybridization = new Hybridization[0];
        try (Connection conn = pool.getConnection()) {
            Results myResults = new Results(query, conn);
            myHybridization = setupHybridizationValues(myResults);
            myResults.close();
        } catch (SQLException e) {
            log.debug("SQL Exception:", e);
            throw e;
     ...
     * @return an array of Hybridization objects
     */
    public Hybridization[] getAllHybridizationForProtocol(int protocol_id, DataSource pool) throws SQLException {
     ...
     */
    public Hybridization getHybridization(int hybrid_id, DataSource pool) throws SQLException {
        ...
        //log.debug("query =  " + query);
        Hybridization myHybridization = null;
        try (Connection conn = pool.getConnection()) {
            Results myResults = new Results(query, hybrid_id, conn);
            myHybridization = setupHybridizationValues(myResults)[0];
    ...
    public void update(DataSource pool) throws SQLException {
        ...
        String query =
                ...
                "update hybridizations " +
                        "set  name = ?, description = ?, protocol_id = ?, scan_protocol_id = ?, " +
                        "norm_protocol_id = ?, array_id = ?, created_by_login = ?, create_date = ? " +
                        "where hybrid_id = ?";
        ...
        try (Connection conn = pool.getConnection()) {
            ...
            myDbUtils.setToNullIfZero(pstmt, 3, protocol_id);
            myDbUtils.setToNullIfZero(pstmt, 4, scan_protocol_id);
            myDbUtils.setToNullIfZero(pstmt, 5, norm_protocol_id);
            pstmt.setInt(6, array_id);
            pstmt.setString(7, created_by_login);
            pstmt.setTimestamp(8, now);
            pstmt.setInt(9, hybrid_id);
    ...
    public void deleteHybridization(DataSource pool) throws SQLException {
        ...
        new Data_file().deleteAllData_filesForHybridization(hybrid_id, pool);
        log.debug("calling deleteAllTlabhybForHybridization with this hybridID: " + hybrid_id);
        new Tlabhyb().deleteAllTlabhybForHybridization(hybrid_id, pool);
        try (Connection conn = pool.getConnection()) {
     ...
     * @throws SQLException if an error occurs while accessing the database
     */
    public void deleteAllHybridizationForProtocol(int protocol_id, DataSource pool) throws SQLException {
     ...
     */
    public int checkRecordExists(Hybridization myHybridization, DataSource pool) throws SQLException {
