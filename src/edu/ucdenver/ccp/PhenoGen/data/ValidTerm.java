...
public class ValidTerm {
     ...
     */
    public String getValue(int term_id, DataSource pool) throws SQLException {
        log.debug("in getValue for uid = " + term_id);
        return getValidTerm(term_id, pool).getValue();
    }
     ...
     */
    public int getSysuid(String value, String type, DataSource pool) throws SQLException {
        log.debug("in getSysuid for value = " + value + ", and type = " + type);
        return getValidTermByValueAndID(value, type, pool).getTerm_id();
    }
     ...
     */
    public ValidTerm[] getFromValidTerm(String type, DataSource pool) throws SQLException {
        String query =
                "select " +
        ...
        ValidTerm[] myValidTerm = new ValidTerm[0];
        try (Connection conn = pool.getConnection()) {
            Results myResults = new Results(query, type, conn);
            myValidTerm = setupValidTermValues(myResults);
            myResults.close();
     ...
     */
    public LinkedHashMap<String, String> getValuesAsSelectOptions(String type, DataSource pool) throws SQLException {
        //log.debug("in getValuesAsSelectOptions");

        ValidTerm[] thisArray = getFromValidTerm(type, pool);
     ...
     * @throws SQLException if a database error occurs
     */
    public int getDefaultPublicationStatus(DataSource pool) throws SQLException {
        //log.debug("in getDefaultPublicationStatus");

        ValidTerm[] thisArray = getFromValidTerm("PUBLICATION_STATUS", pool);
        for (int i = 0; i < thisArray.length; i++) {
     ...
     * @return an array of ValidTerm objects
     */
    public ValidTerm[] getAllValidTerm(DataSource pool) throws SQLException {
     ...
     */
    public ValidTerm getValidTermByValueAndID(String value, String category, DataSource pool) throws SQLException {
     ...
     * @return a ValidTerm object
     */
    public ValidTerm getValidTerm(int term_id, DataSource pool) throws SQLException {
    ...
    public void deleteValidTerm(DataSource pool) throws SQLException {
     ...
     */
    public int checkRecordExists(ValidTerm myValidTerm, DataSource pool) throws SQLException {
