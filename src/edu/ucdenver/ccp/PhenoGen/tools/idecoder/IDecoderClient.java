...
public class IDecoderClient {
     ...
     * @throws Exception from any other method calls
     */
    private HashMap<Identifier, Set<Identifier>> doSearchIDStringLC(String idString, String organism, DataSource pool) throws SQLException {
    ...
    /*private HashMap<Identifier, Set<Identifier>> doSearchCaseInsensitive(int geneListID, DataSource pool) throws SQLException {
        ...
        String query =
                "insert into geneListGraph " +
                        ...
                        "and g.gene_list_id = gl.gene_list_id " +
                        "and g.gene_list_id = ?";
        HashMap<Identifier, Set<Identifier>> ret = new HashMap<Identifier, Set<Identifier>>();
        try (Connection conn = pool.getConnection()) {
            conn.setAutoCommit(false);
            PreparedStatement ct = conn.prepareStatement(createTempTable);
            ct.executeUpdate();
    ...
    /*private HashMap<Identifier, Set<Identifier>> doSearch(String gene, String organism, DataSource pool) throws SQLException {
        ...
        String query =
                "insert into geneListGraph " +
                        ...
                        "and id.ident_type_id = type.ident_type_id  " +
                        "and id.organism = ?";
        HashMap<Identifier, Set<Identifier>> results = new HashMap<Identifier, Set<Identifier>>();
        try (Connection conn = pool.getConnection()) {
            conn.setAutoCommit(false);
            PreparedStatement ct = conn.prepareStatement(createTempTable);
            ct.executeUpdate();
            ...
            pstmt.executeUpdate();
            pstmt.close();
            results = doSearchAll(organism, conn);
            //Results myResults = new Results(countQuery2, conn);
            //log.debug("count of geneListGraph at point 1 = "+ myResults.getIntValueFromFirstRow());
    ...
    /*private HashMap<Identifier, Set<Identifier>> doSearchAll(String geneIDString,String organism, DataSource pool) throws SQLException {
        ...
        String query = "insert into geneListGraph " +
                ...
                "glg.id_number, " +
                "glg.identifier, " +
                " (CASE  when type.name='CodeLink ID' then 'CodeLink' " +
                "when type.name='Affymetrix ID' then 'Affymetrix' " +
                "else '' END ) " +
                "from identifiers id, " +
