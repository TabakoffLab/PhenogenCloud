...
public class Statistic {
    ...
    public void callFilterGenes(String platform,
        ...
        if (new edu.ucdenver.ccp.PhenoGen.data.Array().EXON_ARRAY_TYPES.contains(ds.getArray_type())) {
            ...
            try(Connection conn=pool.getConnection()) {
                ...
                cs.executeUpdate();
                cs.close();
                cs = conn.prepareCall(resetQ);
                cs.setInt(1, 6);
                cs.executeUpdate();
                cs.close();
                PreparedStatement ps = conn.prepareStatement(countQ);
    ...
    public int callPreviousFilter(Dataset dataset, Dataset.DatasetVersion version, int userID, String abnormalFilePath) {
        ...
        try(Connection conn=pool.getConnection()) {
            ...
            cs.executeUpdate();
            cs.close();
            PreparedStatement ps = conn.prepareCall(countQ);
            ResultSet rs = ps.executeQuery();
            rs.next();
            count = rs.getInt(1);
            ps.close();
    ...
    public void callOutputGeneList(String platform,
        ...
        if ((rErrorMsg =
            ...
            log.debug("rawOutputFunctionArgs = ");
            myDebugger.print(rawOutputFunctionArgs);
     ...
     */
    public void createMTParameters(Hashtable mtParameters, ParameterValue myParameterValue, DataSource pool) throws SQLException {
    ...
    public int doNormalization(String normalize_method,
        ...
        if (!alreadyExists.equals("")) {
            ...
            nextVersionNumber = selectedDataset.getNextVersion(pool);
        }
        ...
        Dataset.Group[] myGroups = selectedDataset.getGroupsInGrouping(grouping_id, pool);
        ...
        newDatasetVersion.createDatasetVersion(pool);
        ...
        int parameterGroupID = myParameterValue.createParameterGroup(
                selectedDataset.getDataset_id(), nextVersionNumber, 1, pool);
        ...
        User.UserChip[] myChipAssignments = selectedDataset.new Group().getChipAssignments(grouping_id, pool);
