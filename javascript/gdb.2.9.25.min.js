...
ratOnly.heartTotal=1;ratOnly.kidneyTotal=1;ratOnly.heartilluminaTotalPlus=1;ratOnly.heartilluminaTotalMinus=1;ratOnly.probe=1;ratOnly.mergedTotal=1;ratOnly.brainIso=1;ratOnly.liverIso=1;"rn5"===genomeVer&&(mouseOnly.brainTotal=1);mouseOnly.brainilluminaTotalPlus=1;mouseOnly.brainilluminaTotalMinus=1;mouseOnly.brainspliceJnct=1;mouseOnly.probeMouse=1;
var mmVer='Mouse(<span id="verSelect"></span>) Strain:C57BL/6J',rnVer='Rat(<span id="verSelect"></span>) HRDP version:<span id="hrdpSelect"></span> Strain:BN',siteVer="PhenoGen v3.9.6(8/23/2024)",trackBinCutoff=1E4,customTrackLevel=-1,mouseTTOver=0,ttHideHandle=0,tt=d3.select("body").append("div").attr("class","testToolTip").style("z-index",1001).style("opacity",0);
...
genomeVer,dataVer},dataType:"json",success:function(b){p.prevMinCoord=m;p.prevMaxCoord=r;m<p.dataMinCoord&&(p.dataMinCoord=m);r>p.dataMaxCoord&&(p.dataMaxCoord=r);p.folderName=b.folderName;p.updateData();processAjax=0;gtag("event","outsideLoadedRegion",{event_category:"updatePage"})},error:function(b,a,d){console.log(d)}})}}
...
function zoomOut(p,m){var r={};r.chr=chr;r.start=svgList[p].xScale.domain()[0];r.stop=svgList[p].xScale.domain()[1];m=Math.floor((r.stop-r.start)*m);1>m&&(m=1);r.start-=m;r.stop+=m;1E7>r.stop-r.start?(svgList[p].xScale.domain([r.start,r.stop]),history[p]||(history[p]=[]),history[p].push(r),svgList[p].scaleSVG.select(".x.axis").call(svgList[p].xAxis),svgList[p].redraw(),isNaN(zoomUpdateHandle)||(clearTimeout(zoomUpdateHandle),zoomUpdateHandle=Math.NaN),zoomUpdateHandle=setTimeout(function(){0===p&&
updatePage(svgList[p]);svgList[p].updateFullData();0===p&&setTimeout(function(){DisplayRegionReport()},100)},300),0===p&&$("#geneTxt").val(chr+":"+r.start+"-"+r.stop),gtag("event",p,{event_category:"calledZoomOut"})):alert("maximum viewable area is 10Mb.")}
...
function mmove(){var p;for(p=0;p<svgList.length;p++)if(svgList[p]&&(!svgList[p].overSettings||0===svgList[p].overSettings))if(!isNaN(svgList[p].downx)){var m=d3.mouse(svgList[p].vis.node());rupx=m[0];if(0!==rupx){var r=Math.round(svgList[p].downscalex.domain()[0]);var t=Math.round(svgList[p].mw*(svgList[p].downx-svgList[p].downscalex.domain()[0])/rupx+svgList[p].downscalex.domain()[0]);1E7>t-r?t<=svgList[p].xMax&&r>=svgList[p].xMin&&(0===p&&$("#geneTxt").val(chr+":"+r+"-"+t),svgList[p].xScale.domain([r,
t]),svgList[p].scaleSVG.select(".x.axis").call(svgList[p].xAxis),svgList[p].redraw()):alert("Maximum view is 10Megabases.")}}else if(isNaN(svgList[p].downPanx))isNaN(svgList[p].downZoomx)||(f=svgList[p].downZoomx,m=d3.mouse(svgList[p].vis.node()),svgList[p].downZoomxEnd=m[0],m[0]<f?(f=m[0],m=svgList[p].downZoomx-f):m=m[0]-f,r=Math.round(svgList[p].xScale.invert(f)),t=Math.round(svgList[p].xScale.invert(f+m)),1E7>t-r?(d3.select("#Level"+svgList[p].levelNumber).selectAll("svg rect.zoomRect").attr("x",
f).attr("width",m),d3.select("#Level"+svgList[p].levelNumber).selectAll("svg text#zoomTextStart").attr("x",f).attr("y",15).text(numberWithCommas(r)),d3.select("#Level"+svgList[p].levelNumber).selectAll("svg text#zoomTextEnd").attr("x",f+m).attr("y",50).text(numberWithCommas(t))):alert("Maximum view is 10Megabases."));else if(m=d3.mouse(svgList[p].vis.node()),rupx=m[0],0!==rupx){t=svgList[p].downPanx-rupx;var f=(svgList[p].downscalex.domain()[1]-svgList[p].downscalex.domain()[0])/svgList[p].mw;r=Math.round(svgList[p].downscalex.domain()[0]+
t*f);t=Math.round(t*f+svgList[p].downscalex.domain()[1]);1E7>t-r?t<=svgList[p].xMax&&r>=svgList[p].xMin&&(0===p&&$("#geneTxt").val(chr+":"+r+"-"+t),svgList[p].xScale.domain([r,t]),svgList[p].scaleSVG.select(".x.axis").call(svgList[p].xAxis),svgList[p].redraw(),svgList[p].downPanx=m[0]):alert("Maximum view is 10Megabases.")}}d3.select("html").on("mousemove",mmove).on("mouseup",mup);$(window).resize(function(){for(var p=0;p<svgList.length;p++)svgList[p]&&svgList[p].resize($(window).width()-25)});
...
function GenomeSVG(p,m,r,t,f,b,a,d){var c={};console.log("SVG dataVer:"+dataVer);c.isToolTip=0;c.folderName="";c.selectedTrackSetting="";c.trackListHash={};c.overSettings=0;c.zoomFactor=.05;c.strainSpecificCountColors=d3.scaleOrdinal(d3.schemeCategory20);0==f&&(c.folderName=regionfolderName);var g={};g.chr=chr;g.start=r;g.stop=t;c.allowSelectGenomeVer="undefined"===typeof d?!0:d;if(20>f){history[f]||(history[f]=[]);try{history[f].push(g)}catch(e){Rollbar.error("history["+f+"] not defined")}}c.get=
function(e){return c[e]};c.getTrackData=function(e){for(var h=[],l=0;l<c.trackList.length;l++)"undefined"!==typeof c.trackList[l]&&c.trackList[l].trackClass==e&&(h=c.trackList[l].data);return h};c.getTrack=function(e){return c.trackListHash[e]};c.addTrackErrorRemove=function(e,h){if("undefined"!==typeof e){var l=h.substr(7);e.select(".infoIcon").style("opacity",0);e.select(".settings").style("opacity",0);e.append("g").attr("class","removeErrorTrack").attr("id","remove"+l+c.levelNumber).attr("transform",
"translate("+(c.width-40)+",0)").append("image").attr("width","16px").attr("height","16px").attr("xlink:href","data:image/png;base64,").attr("pointer-events",
...
panel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer},dataType:"json",success:function(A){},error:function(A,D,z){console.log(z)}});q=null;q=d3.select("#Level"+c.levelNumber+e).nodes();if(0===q.length||null==q){q=c.topLevel.append("li").attr("class","draggable"+c.levelNumber).attr("id","li"+e).style("margin-bottom","-3px").append("svg").attr("width",c.width).attr("height",30).attr("class","track").attr("id","Level"+c.levelNumber+e).style("cursor","move").on("mouseover",function(){0===overSelectable&&
...
"all").style("cursor","pointer").on("click",function(A){e!=c.selectedTrackSetting?(c.getTrack(e).generateSettingsDiv("div#trackSettingContent"),c.selectedTrackSetting=e,A=$(this).position(),$("#trackSettingDialog").css("top",A.top).css("left",$(window).width()-380),$("#trackSettingDialog").fadeIn("fast")):(c.selectedTrackSetting="",$("#trackSettingDialog").fadeOut("fast"));gtag("event","click",{event_category:"trackSettings"});return!1}).on("mouseover",function(){c.overSettings=1;$("#mouseHelp").html("Track Settings: Click to access track settings or quickly remove the track from the current view.")}).on("mouseout",
function(){c.overSettings=0;$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")})}console.log("Add Track:"+e+":"+dataVer);if("genomeSeq"==e)q=SequenceTrack(c,e,"Reference Genomic Sequence",l),c.addTrackList(q);else if(-1<e.indexOf("noncoding"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",function(A,D){if(A)if(60>n){var z=1500;1==n&&(z=2500);setTimeout(function(){c.addTrack(e,h,l,n+1)},z)}else d3.select("#Level"+
c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==D)11<=n?(A=[],z=GeneTrack(c,A,e,"Long Non-Coding / Non-PolyA+ Genes",l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,10)},5E3);else{A=D.documentElement.getElementsByTagName("Gene");var F=0;D=D.documentElement.getElementsByTagName("GeneList");0<D.length&&"undefined"!==
typeof D[0].getAttribute("ver")&&(F=D[0].getAttribute("ver"));try{z=GeneTrack(c,A,e,"Long Non-Coding / Non-PolyA+ Genes",l),z.dataVer=F,c.addTrackList(z)}catch(H){}}});else if(-1<e.indexOf("coding"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",function(A,D){if(A)if(60>n){var z=1500;1==n&&(z=2500);setTimeout(function(){c.addTrack(e,h,l,n+1)},z)}else d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+
c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==D)11<=n?(A=[],z=GeneTrack(c,A,e,"Protein Coding / PolyA+",l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,11)},5E3);else{A=D.documentElement.getElementsByTagName("Gene");var F=0;D=D.documentElement.getElementsByTagName("GeneList");0<D.length&&"undefined"!==typeof D[0].getAttribute("ver")&&(F=D[0].getAttribute("ver"));try{z=GeneTrack(c,A,e,"Protein Coding / PolyA+",
l),z.dataVer=F,c.addTrackList(z)}catch(H){console.log(H)}}});else if(-1<e.indexOf("smallnc"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",function(A,D){if(A){if(0===n){var z=c.xScale.domain()[0];A=c.xScale.domain()[1];D="/"+pathPrefix;pathPrefix||(D="");$.ajax({url:D+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:z,maxCoord:A,panel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:e,folder:c.folderName},
dataType:"json",success:function(F){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(F,H,I){}})}70>n?(z=1500,10<n?z=5E3:2<n&&(z=2500),setTimeout(function(){c.addTrack(e,h,l,n+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==D)11<=n?(A=[],z=GeneTrack(c,A,e,"Small RNA (<200 bp) Genes",
l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,11)},5E3);else{A="rn5"===genomeVer?D.documentElement.getElementsByTagName("smnc"):D.documentElement.getElementsByTagName("Gene");try{z=GeneTrack(c,A,e,"Small RNA (<200 bp) Genes",l),c.addTrackList(z),""!=selectGene&&z.setSelected(selectGene)}catch(F){}}});else if(0===e.indexOf("liverTotal")||"heartTotal"===e||0===e.indexOf("brainTotal")||"mergedTotal"===e||0===e.indexOf("kidneyTotal")||0===e.indexOf("brainIso")||0===e.indexOf("liverIso")){var v=
"Liver Reconstructed";"heartTotal"===e?v="Heart Reconstructed":"brainTotal"===e?v="Whole Brain Reconstructed":"mergedTotal"===e?v="Merged (Brain,Heart,Liver,Kidney) Reconstructed":"kidneyTotal"===e?v="Kidney Reconstructed":"liverIso"===e?v="Liver IsoSeq":"brainIso"===e&&(v="Whole Brain IsoSeq");if((0===e.indexOf("liverTotal")||0===e.indexOf("brainTotal"))&&0<e.indexOf("_")){var y=e.substr(e.indexOf("_")+1);v="LEStm"===y?"LE-Stm "+v:"F344Stm"===y?"F344-Stm "+v:v+" v"+y}d3.xml(dataPrefix+"tmpData/browserCache/"+
genomeVer+"/regionData/"+c.folderName+"/"+dataVer+"_"+e+".xml",function(A,D){if(A){if(0===n&&("mergedTotal"===e||0===e.indexOf("liverTotal")||0===e.indexOf("brainTotal")||0===e.indexOf("kidneyTotal")||0===e.indexOf("brainIso")||0===e.indexOf("liverIso"))){var z=c.xScale.domain()[0];D=c.xScale.domain()[1];A="/"+pathPrefix;pathPrefix||(A="");var F=panel;if((0===e.indexOf("liverTotal")||0===e.indexOf("brainTotal"))&&0<e.indexOf("_"))F=e.substr(e.indexOf("_")+1),"LEStm"===F&&(F="LE-Stm"),"F344Stm"===
F&&(F="F344-Stm");else if(0===e.indexOf("brainIso")||0===e.indexOf("liverIso"))F="IsoSeq";$.ajax({url:A+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:z,maxCoord:D,panel:F,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:e,folder:c.folderName},dataType:"json",success:function(H){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(H,I,J){}})}70>n?(z=1500,10<n?z=5E3:2<n&&(z=2500),setTimeout(function(){c.addTrack(e,h,l,n+1)},
z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An error occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==D)11<=n?(A=[],z=GeneTrack(c,A,e,v,l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,11)},5E3);else{A=D.documentElement.getElementsByTagName("Gene");F=0;D=D.documentElement.getElementsByTagName("GeneList");0<D.length&&"undefined"!==typeof D[0].getAttribute("ver")&&
(F=D[0].getAttribute("ver"));try{z=GeneTrack(c,A,e,v,l),z.dataVer=F,c.addTrackList(z),""!=selectGene&&z.setSelected(selectGene)}catch(H){}}})}else if(0===e.indexOf("refSeq")){$("#"+e+c.levelNumber+"Select").val();var w=c.xScale.domain()[0],C=c.xScale.domain()[1];q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml";d3.xml(q,function(A,D){if(A){console.log(A);if(0===n){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,
async:!0,data:{chromosome:chr,minCoord:w,maxCoord:C,panel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:e,folder:c.folderName},dataType:"json",success:function(F){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(F,H,I){}})}70>n?(z=1500,10<n?z=5E3:2<n&&(z=2500),setTimeout(function(){c.addTrack(e,h,l,n+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",
15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==D)11<=n?(A=[],z=RefSeqTrack(c,A,e,"Ref Seq Genes",l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,11)},5E3);else{A=D.documentElement.getElementsByTagName("Gene");try{z=RefSeqTrack(c,A,e,"Ref Seq Genes",l),c.addTrackList(z)}catch(F){console.log(F)}}})}else if(0==e.indexOf("snp"))w=c.xScale.domain()[0],C=c.xScale.domain()[1],q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+
c.folderName+"/"+e+".xml",d3.xml(q,function(A,D){if(A){console.log(A);if(0===n){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:w,maxCoord:C,panel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:e,folder:c.folderName},dataType:"json",success:function(F){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(F,H,I){}})}70>n?(z=1500,10<n?z=5E3:2<n&&(z=2500),setTimeout(function(){c.addTrack(e,
h,l,n+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==D)11<=n?(A=[],z=SNPTrack(c,A,e,h,l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,11)},5E3);else{A=D.documentElement.getElementsByTagName("Snp");try{z=SNPTrack(c,A,e,h,l),c.addTrackList(z)}catch(F){}}});else if("qtl"===e)d3.xml(dataPrefix+
"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/qtl.xml",function(A,D){if(A)if(60>n){var z=1500;1===n&&(z=2500);setTimeout(function(){c.addTrack(e,h,l,n+1)},z)}else d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==D)11<=n?(A=[],z=QTLTrack(c,A,e,h),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,
h,l,11)},5E3);else{A=D.documentElement.getElementsByTagName("QTL");try{z=QTLTrack(c,A,e,h),c.addTrackList(z)}catch(F){}}});else if("trx"===e)y=getAllChildrenByName(getFirstChildByName(c.selectedData,"TranscriptList"),"Transcript"),q=TranscriptTrack(c,y,e,h),c.addTrackList(q);else if("probe"===e||"probeMouse"==e)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/probe.xml",function(A,D){if(A)if(60>n){var z=1500;1===n&&(z=2500);setTimeout(function(){c.addTrack(e,h,l,n+
1)},z)}else d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==D)11<=n?(A=[],z=ProbeTrack(c,A,e,"Affy Exon 1.0 ST Probe Sets",h+","+l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,11)},5E3);else{A=D.documentElement.getElementsByTagName("probe");try{z=ProbeTrack(c,A,e,"Affy Exon 1.0 ST Probe Sets",
h+","+l),c.addTrackList(z)}catch(F){}}});else if("helicos"===e||-1<e.indexOf("illuminaTotal")||-1<e.indexOf("illuminaSmall")||"illuminaPolyA"===e){w=c.xScale.domain()[0];C=c.xScale.domain()[1];var B=calculateBin(C-w,c.width);q=[];l&&(0<l.indexOf(",")?q=l.split(","):q[0]=l);y="Total";var E=1;2<q.length&&(g=1*q[1],2===g&&(y="Norm",E=2));q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/tmp/"+dataVer+"_"+w+"_"+C+".count."+e+"."+y+".xml";0<B&&(w-=w%B,C+=B-C%B,q=dataPrefix+"tmpData/browserCache/"+
genomeVer+"/regionData/"+c.folderName+"/tmp/"+dataVer+"_"+w+"_"+C+".bincount."+B+"."+e+"."+y+".xml");d3.xml(q,function(A,D){if(A){if(0===n){var z="/"+pathPrefix;pathPrefix||(z="");tmpPanel=panel;-1<e.indexOf("-")&&(tmpPanel=e.substr(e.indexOf("-")+1));$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:w,maxCoord:C,panel:tmpPanel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:e,folder:c.folderName,binSize:B,countType:E},dataType:"json",
success:function(F){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(F,H,I){}});z=1E4}70>n?(z=1E3,10<n?z=5E3:2<n&&(z=2500),setTimeout(function(){c.addTrack(e,h,l,n+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==D)A=[],"helicos"==e?z=HelicosTrack(c,A,e,h):"illuminaTotal"==
e?z=IlluminaTotalTrack(c,A,e,h):"illuminaSmall"==e?z=IlluminaSmallTrack(c,A,e,h):"illuminaPolyA"==e?z=IlluminaPolyATrack(c,A,e,h):"liverilluminaTotalPlus"==e?z=LiverIlluminaTotalPlusTrack(c,A,e,h):"liverilluminaTotalMinus"==e?z=LiverIlluminaTotalMinusTrack(c,A,e,h):"heartilluminaTotalPlus"==e?z=HeartIlluminaTotalPlusTrack(c,A,e,h):"heartilluminaTotalMinus"==e?z=HeartIlluminaTotalMinusTrack(c,A,e,h):"brainilluminaTotalPlus"==e?z=BrainIlluminaTotalPlusTrack(c,A,e,h):"brainilluminaTotalMinus"==e?z=BrainIlluminaTotalMinusTrack(c,
A,e,h):"liverilluminaSmall"==e?z=LiverIlluminaSmallTrack(c,A,e,h):"heartilluminaSmall"==e?z=HeartIlluminaSmallTrack(c,A,e,h):-1<e.indexOf("illuminaTotal")&&(z=StrainSpecificIlluminaTotalTrack(c,A,e,h,l)),c.addTrackList(z);else{A=D.documentElement.getElementsByTagName("Count");try{"helicos"===e?z=HelicosTrack(c,A,e,h):"illuminaTotal"===e?z=IlluminaTotalTrack(c,A,e,h):"illuminaSmall"===e?z=IlluminaSmallTrack(c,A,e,h):"illuminaPolyA"===e?z=IlluminaPolyATrack(c,A,e,h):"liverilluminaTotalPlus"===e?z=LiverIlluminaTotalPlusTrack(c,
A,e,h):"liverilluminaTotalMinus"===e?z=LiverIlluminaTotalMinusTrack(c,A,e,h):"heartilluminaTotalPlus"===e?z=HeartIlluminaTotalPlusTrack(c,A,e,h):"heartilluminaTotalMinus"===e?z=HeartIlluminaTotalMinusTrack(c,A,e,h):"brainilluminaTotalPlus"===e?z=BrainIlluminaTotalPlusTrack(c,A,e,h):"brainilluminaTotalMinus"===e?z=BrainIlluminaTotalMinusTrack(c,A,e,h):"liverilluminaSmall"===e?z=LiverIlluminaSmallTrack(c,A,e,h):"heartilluminaSmall"===e?z=HeartIlluminaSmallTrack(c,A,e,h):-1<e.indexOf("illuminaTotal")&&
(z=StrainSpecificIlluminaTotalTrack(c,A,e,h,l)),c.addTrackList(z)}catch(F){}}})}else if(-1<e.indexOf("spliceJnct")){w=c.xScale.domain()[0];C=c.xScale.domain()[1];q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+dataVer+"_"+e+".xml";var G="Brain ";"liverspliceJnct"==e?G="Liver ":"heartspliceJnct"==e&&(G="Heart ");d3.xml(q,function(A,D){if(A){console.log(A);if(0===n){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,
data:{chromosome:chr,minCoord:w,maxCoord:C,panel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:e,folder:c.folderName},dataType:"json",success:function(F){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(F,H,I){}})}70>n?(z=1500,10<n?z=5E3:2<n&&(z=2500),setTimeout(function(){c.addTrack(e,h,l,n+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),
c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==D)11<=n?(A=[],z=SpliceJunctionTrack(c,A,e,G+"Splice Junctions",1,""),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,11)},5E3);else{A=D.documentElement.getElementsByTagName("Feature");try{z=SpliceJunctionTrack(c,A,e,G+"Splice Junctions",3,""),c.addTrackList(z)}catch(F){console.log(F)}}})}else if("polyASite"==e)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+
"/polyASite.xml",function(A,D){if(A)if(60>n){var z=1E3;1===n&&(z=2500);setTimeout(function(){c.addTrack(e,h,l,n+1)},z)}else d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==D)4<=n?(A=[],z=PolyATrack(c,A,e,"Predicted PolyA Sites",l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,4)},5E3);else{A=
D.documentElement.getElementsByTagName("Feature");try{z=PolyATrack(c,A,e,"Predicted PolyA Sites",l),c.addTrackList(z)}catch(F){}}});else if(0===e.indexOf("repeatMask"))$("#"+e+c.levelNumber+"Select").val(),w=c.xScale.domain()[0],C=c.xScale.domain()[1],q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",d3.xml(q,function(A,D){if(A){console.log(A);if(0===n){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,
minCoord:w,maxCoord:C,panel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:e,folder:c.folderName},dataType:"json",success:function(F){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(F,H,I){}})}70>n?(z=1500,10<n?z=5E3:2<n&&(z=2500),setTimeout(function(){c.addTrack(e,h,l,n+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+
c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==D)11<=n?(A=[],z=RepeatMaskTrack(c,A,e,"Repeat Masker",h,l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,h,l,11)},5E3);else{A=D.documentElement.getElementsByTagName("Feature");try{z=RepeatMaskTrack(c,A,e,"Repeat Masker",h,l),c.addTrackList(z)}catch(F){console.log(F)}}});else if(0==e.indexOf("custom"))if(q=trackInfo[e],l=l+",Name="+q.Name,l="DataFile="+q.Location+","+l,"bed"==q.Type||"bb"==q.Type)y=[],q=CustomTranscriptTrack(c,y,e,q.Name,
3,l),c.addTrackList(q),q.updateFullData(0,0);else{if("bg"==q.Type||"bw"==q.Type)y=[],q=CustomCountTrack(c,y,e,3,l),c.addTrackList(q)}else 0<=e.indexOf("cirRNA")&&(q=trackInfo[e],q=CircRNATrack(c,y,e,q.Name,3,l),c.addTrackList(q));$(".sortable"+c.levelNumber).sortable("refresh");gtag("event",e,{event_category:"trackAdded"})};c.addTrackList=function(e){null!=e&&(c.trackList[c.trackCount]=e,c.trackCount++,c.trackListHash[e.trackClass]=e,setTimeout(function(){DisplayRegionReport()},300))};c.updateLinks=
function(){1===c.levelNumber&&d3.select("#probeSetDetailLink1").each(function(){var e=new String(d3.select(this).attr("href"));e=e.substr(0,e.lastIndexOf("=")+1);e+=c.currentView.ViewID;d3.select(this).attr("href",e)})};c.changeTrackHeight=function(e,h){0<h?d3.select("#"+e+"Scroll").style("max-height",h+"px").style("overflow","auto"):d3.select("#"+e+"Scroll").style("max-height","none").style("overflow","hidden")};c.removeAllTracks=function(){d3.selectAll("li.draggable"+c.levelNumber).remove();c.trackList=
[];setTimeout(function(){DisplayRegionReport()},300);gtag("event",c.levelNumber,{event_category:"SVGremoveAllTracks"})};c.removeTrack=function(e){d3.select("#Level"+c.levelNumber+e).remove();for(var h=0;h<c.trackList.length;h++)"undefined"!==typeof c.trackList[h]&&c.trackList[h].trackClass==e&&(c.trackList.splice(h,1),c.trackCount--);c.trackListHash[e]=void 0;setTimeout(function(){DisplayRegionReport()},300);gtag("event",c.levelNumber,{event_category:"removeTrack",event_label:e})};c.redrawTrack=function(e){for(var h=
0;h<c.trackList.length;h++)"undefined"!==typeof c.trackList[h]&&c.trackList[h].trackClass==e&&c.trackList[h].redraw();setTimeout(function(){DisplayRegionReport()},300)};c.redraw=function(){for(var e=0;e<c.trackList.length;e++)"undefined"!==typeof c.trackList[e]&&"undefined"!==typeof c.trackList[e].redraw&&c.trackList[e].redraw();c.selectSvg.redraw()};c.update=function(){for(var e=0;e<c.trackList.length;e++)"undefined"!==typeof c.trackList[e].update&&c.trackList[e].update();setTimeout(function(){DisplayRegionReport()},
300);gtag("event","genomeSVG.update",{event_category:"gsvgUpdate"})};c.updateData=function(){for(var e=0;e<c.trackList.length;e++)"undefined"!==typeof c.trackList[e]&&"undefined"!==typeof c.trackList[e].updateData&&c.trackList[e].updateData(0);c.updateFullData();setTimeout(function(){DisplayRegionReport()},300);gtag("event","genomeSVG.updateData",{event_category:"gsvgUpdateData"})};c.updateFullData=function(){-1<(new String(c.folderName)).indexOf("img")&&(c.folderName="/"+chr+"/"+c.xScale.domain()[0]+
"_"+c.xScale.domain()[1],0==c.levelNumber&&regionfolderName!=c.folderName&&(regionfolderName=c.folderName),setTimeout(function(){DisplayRegionReport()},300));for(var e=0;e<c.trackList.length;e++)"undefined"!==typeof c.trackList[e]&&"undefined"!==typeof c.trackList[e].updateFullData&&c.trackList[e].updateFullData(0,1);gtag("event","genomeSVG.updateFullData",{event_category:"gsvgUpdateFullData"})};c.setLoading=function(){for(var e=0;e<c.trackList.length;e++)"undefined"===typeof c.trackList[e]||"undefined"===
typeof c.trackList[e].updateData&&"undefined"===typeof c.trackList[e].updateFullData||c.trackList[e].showLoading()};c.clearSelection=function(){c.selectionStart=-1;c.selectionEnd=-1;c.scaleSVG.selectAll("rect.selectedArea").remove();c.selectSvg.setVis(!1);for(var e=0;e<c.trackList.length;e++)"undefined"!==typeof c.trackList[e]&&"undefined"!==typeof c.trackList[e].clearSelection&&c.trackList[e].clearSelection()};c.mdown=function(){if(0==c.overSettings)if("dragzoom"!==c.defaultMouseFunct&&d3.event.altKey||
"dragzoom"===c.defaultMouseFunct&&!d3.event.altKey){console.log("zoom++");var e=d3.mouse(c.vis.node());c.downZoomx=e[0];c.scaleSVG.append("rect").attr("class","zoomRect").attr("x",e[0]).attr("y",0).attr("height",c.scaleSVG.attr("height")).attr("width",1).attr("fill","#CECECE").attr("opacity",.3);c.scaleSVG.append("text").attr("id","zoomTextStart").attr("x",c.downZoomx).attr("y",15).text(numberWithCommas(Math.round(c.xScale.invert(c.downZoomx))));c.scaleSVG.append("text").attr("id","zoomTextEnd").attr("x",
c.downZoomx).attr("y",50).text(numberWithCommas(Math.round(c.xScale.invert(c.downZoomx))))}else 0==processAjax&&(c.prevMinCoord=c.xScale.domain()[0],c.prevMaxCoord=c.xScale.domain()[1],e=d3.mouse(c.vis.node()),c.downx=c.xScale.invert(e[0]),c.downscalex=c.xScale)};c.forceDrawAs=function(e){for(var h=0;h<c.trackList.length;h++)"undefined"!==typeof c.trackList[h]&&"undefined"!==typeof c.trackList[h].drawAs&&(c.trackList[h].drawAs=e,c.trackList[h].draw(c.trackList[h].data));c.forceDrawAsValue=e};c.resize=
function(e){c.width=e;c.xScale.range([0,c.width]);c.xAxis=d3.axisTop(c.xScale).ticks(6).tickSize(8).tickPadding(10);c.scaleSVG.attr("width",c.width);c.scaleSVG.select(".x.axis").call(c.xAxis);d3.select("#Level"+c.levelNumber).select(".axisLbl").attr("x",(c.width-2*c.margin)/2);c.topLevel.style("width",c.width+18+"px");for(var h=0;h<c.trackList.length;h++)"undefined"!==typeof c.trackList[h]&&"undefined"!==typeof c.trackList[h].redraw&&c.trackList[h].resize();c.selectSvg.width=e;c.selectSvg.draw()};
...
d3.select("span#verSelect").append("select");if(c.allowSelectGenomeVer)if(e.on("change",function(){displayWorking();!0===isLocalStorage()?localStorage.setItem(organism+"DefGenomeVer",$(this).val()):$.cookie(organism+"DefGenomeVer",$(this).val());changeGenome($(this).val(),c.currentView);setTimeout(function(){$("form#geneCentricForm").submit()},1500)}),"Rn"===organism){var h=e.append("option").attr("value","rn7").html("rn7");"rn7"===genomeVer&&h.attr("selected","selected");h=e.append("option").attr("value",
"rn6").html("rn6");"rn6"===genomeVer&&h.attr("selected","selected");e=e.append("option").attr("value","rn5").html("rn5");"rn5"===genomeVer&&e.attr("selected","selected")}else"Mm"===organism&&(e=e.append("option").attr("value","mm10").html("mm10"),"mm10"===genomeVer&&e.attr("selected","selected"));else e.append("option").attr("value",genomeVer).html(genomeVer)}0<$("span#hrdpSelect").length&&(e=d3.select("span#hrdpSelect").append("select"),e.on("change",function(){displayWorking();!0===isLocalStorage()?
localStorage.setItem(organism+"DefHRDPVer",$(this).val()):$.cookie(organism+"DefHRDPVer",$(this).val());changeHRDPVer($(this).val());setTimeout(function(){$("form#geneCentricForm").submit()},1500)}),"Rn"===organism?($("span#hrdpSelect").css("display","inline-block"),"rn7"===genomeVer?(h=e.append("option").attr("value","hrdp6").html("HRDP v6"),"hrdp6"===dataVer&&h.attr("selected","selected"),h=e.append("option").attr("value","hrdp7.1").html("HRDP v7.1"),"hrdp7.1"===dataVer&&h.attr("selected","selected")):
"rn6"===genomeVer?(e=e.append("option").attr("value","hrdp5").html("HRDP v5"),e.attr("selected","selected")):"rn5"===genomeVer?(e=e.append("option").attr("value","hrdp3").html("HRDP v4"),e.attr("selected","selected")):console.error("Unsupported genome version")):"Mm"===organism&&$("span#hrdpSelect").css("display","none"))},500);c.addTrack("genomeSeq",3,"both",0);c.selectSvg=selectionSVG("#Level"+c.levelNumber,c.width,c.levelNumber,c);return c}
...
e=l=l.substr(l.indexOf(",")+1));if("genomeSeq"===c){var q=SequenceTrack(d,c,"Reference Genomic Sequence",e);q.seqRegionSize=10;d.addTrackList(q)}else if(-1<c.indexOf("noncoding"))q=GeneTrack(d,h,c,"Non-Coding/Non-PolyA+ Transcripts",e),d.addTrackList(q);else if(-1<c.indexOf("coding"))q=GeneTrack(d,h,c,"Protein Coding/PolyA+ Transcripts",e),d.addTrackList(q);else if(0===c.indexOf("liverTotal")||"heartTotal"===c||0===c.indexOf("brainTotal")||0===c.indexOf("kidneyTotal")||"mergedTotal"===c||"brainIso"===
c||"liverIso"===c)g="Liver Total RNA Transcripts","heartTotal"===c?g="Heart Total RNA Transcripts":"brainTotal"===c?g="Brain Total RNA Transcripts":"mergedTotal"===c?g="Merged (Brain,Heart,Liver,Kidney) Reconstructed":"kidneyTotal"===c?g="Kidney Total RNA Transcripts":"liverIso"===c?g="Liver IsoSeq":"brainIso"===c&&(g="Whole Brain IsoSeq"),q=GeneTrack(d,h,c,g,e),d.addTrackList(q);else if(-1<c.indexOf("smallnc"))q=GeneTrack(d,h,c,"Small RNA (<200 bp) Genes",e),d.addTrackList(q);else if(0===c.indexOf("refSeq"))99===
d.levelNumber&&(e+="DrawTrx,"),q=RefSeqTrack(d,h,c,"Ref Seq Genes",e),q.density=99===d.levelNumber?2:g,d.addTrackList(q);else if(0===c.indexOf("snp"))q=SNPTrack(d,h,c,1,"4"),d.addTrackList(q);else if("qtl"===c)q=QTLTrack(d,h,c,1),d.addTrackList(q);else if("trx"===c)h=getAllChildrenByName(getFirstChildByName(d.selectedData,"TranscriptList"),"Transcript"),q=TranscriptTrack(d,h,c,g),d.addTrackList(q);else if("probe"===c||"probeMouse"===c)99!==d.levelNumber&&(e=g+","+e),q=ProbeTrack(d,h,c,"Affy Exon 1.0 ST Probe Sets",
e),d.addTrackList(q);else if(-1<c.indexOf("spliceJnct"))e="Brain ","liverspliceJnct"===c?e="Liver ":"heartspliceJnct"===c&&(e="Heart "),q=SpliceJunctionTrack(d,h,c,e+"Splice Junctions",3,""),100===d.levelNumber&&(q.density=g),d.addTrackList(q);else if("polyASite"===c)q=PolyATrack(d,h,c,"Predicted PolyA Sites",e),d.addTrackList(q);else if("helicos"===c||-1<c.indexOf("illuminaTotal")||-1<c.indexOf("illuminaSmall")||"illuminaPolyA"===c){d.updateTimeoutHandle[c]&&(d.updateTimeoutHandle[c]=-1);g=2;l=l.split(",");
1<l.length&&(1===l[1]||2===l[1])&&(g=l[1]);"helicos"===c?q=HelicosTrack(d,h,c,g):"illuminaTotal"===c?q=IlluminaTotalTrack(d,h,c,g):"illuminaSmall"===c?q=IlluminaSmallTrack(d,h,c,g):"illuminaPolyA"===c?q=IlluminaPolyATrack(d,h,c,g):"liverilluminaTotalPlus"===c?q=LiverIlluminaTotalPlusTrack(d,h,c,g):"liverilluminaTotalMinus"===c?q=LiverIlluminaTotalMinusTrack(d,h,c,g):"heartilluminaTotalPlus"===c?q=HeartIlluminaTotalPlusTrack(d,h,c,g):"heartilluminaTotalMinus"===c?q=HeartIlluminaTotalMinusTrack(d,h,
c,g):"brainilluminaTotalPlus"===c?q=BrainIlluminaTotalPlusTrack(d,h,c,g):"brainilluminaTotalMinus"===c?q=BrainIlluminaTotalMinusTrack(d,h,c,g):"liverilluminaSmall"===c?q=LiverIlluminaSmallTrack(d,h,c,g):"heartilluminaSmall"===c?q=HeartIlluminaSmallTrack(d,h,c,g):-1<c.indexOf("illuminaTotal")&&(q=StrainSpecificIlluminaTotalTrack(d,h,c,g,e));if(99===d.levelNumber){if(0!==d.updateTimeoutHandle[c]){clearTimeout(d.updateTimeoutHandle[c]);try{clearTimeout(d.timeoutTrack[c].fullDataTimeOutHandle)}catch(u){}d.updateTimeoutHandle[c]=
0}d.updateTimeoutHandle[c]=setTimeout(function(){q.updateFullData(0,1);d.timeoutTrack[c]=q;d.updateTimeoutHandle[c]=0},300)}d.addTrackList(q)}else if(0===c.indexOf("repeatMask"))q=RepeatMaskTrack(d,h,c,"Repeat Masker",1,e),d.addTrackList(q);else if(-1<c.indexOf("custom"))if(g=trackInfo[c],e="DataFile="+g.Location+","+e,"bed"===g.Type||"bb"===g.Type)h=[],q=CustomTranscriptTrack(d,h,c,g.Name,3,e),d.addTrackList(q);else{if("bg"===g.Type||"bw"===g.Type)h=[],q=CustomCountTrack(d,h,c,3,e),d.addTrackList(q)}else 0<=
...
typeof d.trackList[c]&&"genomeSeq"!==d.trackList[c].trackClass&&d3.select("#Level"+d.levelNumber+d.trackList[c].trackClass).remove();d.trackList=[];setTimeout(function(){DisplayRegionReport()},300)};d.removeTrack=function(c){d3.select("#Level"+d.levelNumber+c).remove();for(var g=0;g<d.trackList.length;g++)"undefined"!==typeof d.trackList[g]&&d.trackList[g].trackClass==c&&(d.trackList.splice(g,1),d.trackCount--);setTimeout(function(){DisplayRegionReport()},300)};d.redrawTrack=function(c){for(var g=
0;g<d.trackList.length;g++)"undefined"!==typeof d.trackList[g]&&d.trackList[g].trackClass==c&&d.trackList[g].redraw();setTimeout(function(){DisplayRegionReport()},300)};d.redraw=function(){for(var c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c]&&"undefined"!==typeof d.trackList[c].redraw&&d.trackList[c].redraw()};d.update=function(){for(var c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c].update&&d.trackList[c].update();setTimeout(function(){DisplayRegionReport()},
300)};d.updateData=function(){for(var c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c]&&"undefined"!==typeof d.trackList[c].updateData&&d.trackList[c].updateData(0);d.updateFullData();setTimeout(function(){DisplayRegionReport()},300)};d.updateFullData=function(){-1<(new String(d.folderName)).indexOf("img")&&(d.folderName="/"+chr+"/"+d.xScale.domain()[0]+"_"+d.xScale.domain()[1]);for(var c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c]&&"undefined"!==typeof d.trackList[c].updateFullData&&
d.trackList[c].updateFullData(0,1)};d.setLoading=function(){for(var c=0;c<d.trackList.length;c++)"undefined"===typeof d.trackList[c]||"undefined"===typeof d.trackList[c].updateData&&"undefined"===typeof d.trackList[c].updateFullData||d.trackList[c].showLoading()};d.clearSelection=function(){for(var c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c]&&"undefined"!==typeof d.trackList[c].clearSelection&&d.trackList[c].clearSelection()};d.mdown=function(){if(0==processAjax){d.prevMinCoord=
d.xScale.domain()[0];d.prevMaxCoord=d.xScale.domain()[1];var c=d3.mouse(d.vis.node());d.downx=d.xScale.invert(c[0]);d.downscalex=d.xScale}};d.forceDrawAs=function(c){for(var g=0;g<d.trackList.length;g++)"undefined"!==typeof d.trackList[g]&&"undefined"!==typeof d.trackList[g].drawAs&&(d.trackList[g].drawAs=c,d.trackList[g].draw(d.trackList[g].data));d.forceDrawAsValue=c};d.generateSettingsString=function(){ret="";for(var c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c]&&"undefined"!==
typeof d.trackList[c].generateTrackSettingString&&(ret+=d.trackList[c].generateTrackSettingString());return ret};d.setCurrentViewModified=function(){"undefined"!==typeof d.currentView&&(d.currentView.modified=1,$("span#viewModifiedCtl"+d.levelNumber).show())};d.clearCurrentView=function(){d.currentView.modified=0;$("span#viewModifiedCtl"+d.levelNumber).hide()};d.type=a;d.div=p;d.margin=0;d.halfWindowWidth=$(window).width()/2;d.mh=400;d.downx=Math.NaN;d.downscalex;d.downPanx=Math.NaN;d.xMax=29E7;d.xMin=
1;d.prevMinCoord=r;d.prevMaxCoord=t;d.dataMinCoord=r;d.dataMaxCoord=t;d.y=0;d.xScale=null;d.xAxis=null;d.vis=null;d.level=null;d.svg=null;d.txType=null;d.txList=null;d.tt=null;d.trackList=[];d.trackCount=0;d.levelNumber=f;d.selectedData=null;d.txType=null;d.width=m;d.mw=d.width-d.margin;d.vis=d3.select(p);d.topDiv=d.vis.append("div").attr("id","Level"+f).style("text-align","left");d.xScale=d3.scaleLinear().domain([r,t]).range([0,d.width]);d.xAxis=d3.axisTop(d.xScale).ticks(3).tickSize(8).tickPadding(10);
d.scaleSVG=d.topDiv.append("svg:svg").attr("width",d.width).attr("height",60).attr("class","scale");d.scaleSVG.append("g").attr("class","x axis").attr("transform","translate(0,55)").attr("shape-rendering","crispEdges").call(d.xAxis);d3.select("#Level"+d.levelNumber).select(".x.axis").append("text").text(b).attr("x",(d.width-2*d.margin)/2).attr("y",-40).attr("class","axisLbl");d.topLevel=d.topDiv.append("div").attr("id","ScrollLevel"+d.levelNumber).style("width",d.width+18+"px").append("ul").attr("id",
"sortable"+d.levelNumber);return d}
...
minCoord:f.tmpseqRegionMin,maxCoord:f.tmpseqRegionMax,panel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:f.trackClass,folder:f.gsvg.folderName},dataType:"json",success:function(g){gtag("event","generateTrackSequence",{event_category:"browser"})},error:function(g,e,h){}})),3>b?(d=1E4,1===b&&(d=15E3),setTimeout(function(){f.updateData(b+1)},d)):(f.seqRegionMin=0,f.seqRegionMax=0,f.hideLoading(),d3.select("#Level"+f.gsvg.levelNumber+f.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+
...
b.ttTrackList.push("kidneyTotal"),b.ttTrackList.push("mergedTotal"),b.ttTrackList.push("brainIso"),b.ttTrackList.push("liverIso"),b.ttTrackList.push("refSeq"),b.ttTrackList.push("ensemblnoncoding"),b.ttTrackList.push("brainnoncoding")):(b.ttTrackList.push("ensemblsmallnc"),b.ttTrackList.push("brainsmallnc"),b.ttTrackList.push("liversmallnc"),b.ttTrackList.push("heartsmallnc"),b.ttTrackList.push("refSeq"));b.ttTrackList.push("repeatMask");b.ttTrackList.push("snpSHRH");b.ttTrackList.push("snpBNLX");
b.ttTrackList.push("snpF344");b.ttTrackList.push("snpSHRJ");"braincoding"===r?(b.ttTrackList.push("spliceJnct"),b.ttTrackList.push("illuminaPolyA")):0===r.indexOf("liverTotal")?(b.ttTrackList.push("liverspliceJnct"),b.ttTrackList.push("liverilluminaTotalPlus"),b.ttTrackList.push("liverilluminaTotalMinus")):"heartTotal"===r?(b.ttTrackList.push("heartspliceJnct"),b.ttTrackList.push("heartilluminaTotalPlus"),b.ttTrackList.push("heartilluminaTotalMinus")):0===r.indexOf("brainTotal")?(b.ttTrackList.push("brainspliceJnct"),
b.ttTrackList.push("brainilluminaTotalPlus"),b.ttTrackList.push("brainilluminaTotalMinus")):"brainsmallnc"===r?b.ttTrackList.push("illuminaSmall"):"heartsmallnc"===r?b.ttTrackList.push("heartilluminaSmall"):"liversmallnc"===r&&b.ttTrackList.push("liverilluminaSmall");b.pieColorPalette=d3.scaleOrdinal(d3.schemeCategory20);b.cleanID=function(a){return a=a.replace(/\./g,"_")};b.color=function(a){var d="#000000";"ensemblcoding"===b.trackClass?d="#DFC184":"braincoding"===b.trackClass?(d="#7EB5D6",null!==
a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&(d=b.strainSpecColor(d,a))):"ensemblnoncoding"===b.trackClass?d="#B58AA5":"brainnoncoding"===b.trackClass?(d="#CECFCE",null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"BNLX"===a.getAttribute("strain").toUpperCase()?d="#3E75FF":null===a.getAttribute("strain")||"undefined"===typeof a.getAttribute("strain")||"SHR"!==a.getAttribute("strain").toUpperCase()&&"SHRH"!==
a.getAttribute("strain").toUpperCase()||(d="#FE7596")):"ensemblsmallnc"===b.trackClass?d="#FFCC00":"brainsmallnc"===b.trackClass?(d="#3E7596",null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&(d=b.strainSpecColor(d,a))):0===b.trackClass.indexOf("liverTotal")?(d="#bbbedd",0<b.trackClass.indexOf("_F344Stm")?d="#abaefd":0<b.trackClass.indexOf("_LEStm")&&(d="#dbaecd"),null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&
"All"!==a.getAttribute("strain")&&(d=b.strainSpecColor(d,a))):"heartTotal"===b.trackClass?(d="#DC7252",null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&(d=b.strainSpecColor(d,a))):"kidneyTotal"===b.trackClass?(d="#fdb462",null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&(d=b.strainSpecColor(d,a))):0===b.trackClass.indexOf("brainTotal")?(d="#7EB5D6",0<b.trackClass.indexOf("_F344Stm")?
d="#6EA5F6":0<b.trackClass.indexOf("_LEStm")&&(d="#9EA5C6"),null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&(d=b.strainSpecColor(d,a))):"liversmallnc"===b.trackClass?(d="#7b7e9d",null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&(d=b.strainSpecColor(d,a))):"heartsmallnc"===b.trackClass?(d="#9C3212",null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&
"All"!==a.getAttribute("strain")&&(d=b.strainSpecColor(d,a))):"mergedTotal"===b.trackClass?d="#9F4F92":"brainIso"===b.trackClass?d="#4E85D6":"liverIso"===b.trackClass&&(d="#9b9ebd");return d=d3.rgb(d)};b.strainSpecColor=function(a,d){var c=parseInt(a.substr(1,2),16),g=parseInt(a.substr(3,2),16);a=parseInt(a.substr(5),16);"BNLX"===d.getAttribute("strain").toUpperCase()?(a+=64,c-=32,g-=32):"SHR"===d.getAttribute("strain").toUpperCase()||"SHRH"===d.getAttribute("strain").toUpperCase()?(c+=64,a-=32,g-=
32):"F344_ST"===d.getAttribute("strain").toUpperCase()?(a+=32,c-=16,g-=16):"LE_ST"===d.getAttribute("strain").toUpperCase()&&(c+=32,a-=16,g-=16);255<c&&(c=255);255<g&&(g=255);255<a&&(a=255);return"#"+c.toString(16)+g.toString(16)+a.toString(16)};b.pieStrainSpecColor=function(a,d){var c=parseInt(a.substr(1,2),16),g=parseInt(a.substr(3,2),16);a=parseInt(a.substr(5),16);if("BNLX"===d.toUpperCase())a+=64,c-=32,g-=32;else if("SHR"===d.toUpperCase()||"SHRH"===d.toUpperCase())c+=64,a-=32,g-=32;255<c&&(c=
255);255<g&&(g=255);255<a&&(a=255);return a="#"+c.toString(16)+g.toString(16)+a.toString(16)};b.pieColor=function(a,d){d="#000000";"ensemblcoding"===b.trackClass?d="#DFC184":"braincoding"===b.trackClass?d=b.pieStrainSpecColor("#7EB5D6",a.data.names):"ensemblnoncoding"===b.trackClass?d="#B58AA5":"brainnoncoding"===b.trackClass?(d="#CECFCE",null!==a.data.names&&"undefined"!==typeof a.data.names&&"BNLX"===a.data.names.toUpperCase()?d="#3E75FF":null===a.data.names||"undefined"===typeof a.data.names||
"SHR"!==a.data.names.toUpperCase()&&"SHRH"!==a.data.names.toUpperCase()||(d="#FE7596")):"ensemblsmallnc"===b.trackClass?d=b.pieColorPalette(a.data.names):"brainsmallnc"===b.trackClass?d=b.pieStrainSpecColor("#99CC99",a.data.names):0===b.trackClass.indexOf("liverTotal")?d=b.pieStrainSpecColor("#bbbedd",a.data.names):"heartTotal"===b.trackClass?d=b.pieStrainSpecColor("#DC7252",a.data.names):"kidneyTotal"===b.trackClass?d="#fdb462":0===b.trackClass.indexOf("brainTotal")?d=b.pieStrainSpecColor("#7EB5D6",
a.data.names):"liversmallnc"===b.trackClass?d=b.pieStrainSpecColor("#8b8ead",a.data.names):"heartsmallnc"===b.trackClass?d=b.pieStrainSpecColor("#BC5232",a.data.names):"mergedTotal"===b.trackClass?d="#9F4F92":"brainIso"===b.trackClass?d="#4E85D6":"liverIso"===b.trackClass&&(d="#9b9ebd");return d=d3.rgb(d)};b.getDisplayID=function(a){-1<b.trackClass.indexOf("smallnc")||-1<a.indexOf("_")&&(a=a.substr(a.indexOf("_")+1));return a};b.createToolTip=function(a){var d="";if(-1<b.trackClass.indexOf("smallnc")){d=
"Known - "+a.getAttribute("biotype");var c=".";1==a.getAttribute("strand")?c="+":-1==a.getAttribute("strand")&&(c="-");if(0===(new String(a.getAttribute("ID"))).indexOf("P")){var g=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");getAllChildrenByName(getFirstChildByName(a,"StrainQuantList"),"Strains")[1].getAttribute("strain");d=g[0].getAttribute("category")+" - "+g[0].getAttribute("source")}g='<BR><div id="ttSVG" style="background:#FFFFFF;"></div>ID: '+a.getAttribute("ID");
-1===b.trackClass.indexOf("ensembl")&&(g=g+"<BR>PhenoGen ID: "+a.getAttribute("intGeneID"));g=g+"<BR>Length: "+(a.getAttribute("stop")-a.getAttribute("start"))+"<BR>Type:"+d+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+c+"<BR>"}else{var e=a.getAttribute("ID");e=b.getDisplayID(e);var h="";a.getAttribute("geneSymbol")?h=a.getAttribute("geneSymbol"):a.parent&&a.parent.getAttribute("geneSymbol")&&
(h=a.parent.getAttribute("geneSymbol"));c=".";1==a.getAttribute("strand")?c="+":-1==a.getAttribute("strand")&&(c="-");if(a.parent){g=getAllChildrenByName(getFirstChildByName(a.parent,"TranscriptList"),"Transcript");for(var l=0;l<g.length;l++){var n=new String(g[l].getAttribute("ID"));e!=n&&(-1<(new String(g[l].getAttribute("ID"))).indexOf("ENS")?d+="<B>"+n+"</B>":(d+="<B>"+n+"</B>",-1==(new String(g[l].getAttribute("ID"))).indexOf("ENS")&&(n=getFirstChildByName(getFirstChildByName(g[l],"annotationList"),
"annotation"),null!=n&&(d+=" - "+n.getAttribute("reason")))),d+="<br>");if(""==h){n=getAllChildrenByName(getFirstChildByName(g[l],"annotationList"),"annotation");for(var q=0;q<n.length&&""==h;q++)"AKA"==n[q].getAttribute("source")&&(h=n[q].getAttribute("annot_value"),h=(new String(h)).split(":"),h=0!=(new String(h[h.length-1])).indexOf("ENS")?h[h.length-1]:h[0])}}g='<BR><div id="ttSVG" style="background:#FFFFFF;overflow:"></div><BR>Transcript ID: '+e+"<BR>Location: "+a.getAttribute("chromosome")+
":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+c;g=g+"<BR><BR>Gene Symbol: "+h+"<BR>Gene ID: "+b.getDisplayID(a.parent.getAttribute("ID"))+"<BR><BR>Other Transcripts:<BR>"+d+"<BR>"}else{g=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(l=0;l<g.length;l++)if(n=new String(g[l].getAttribute("ID")),-1<(new String(g[l].getAttribute("ID"))).indexOf("ENS")?d+="<B>"+n+"</B>":(d+="<B>"+n+"</B>",-1===(new String(g[l].getAttribute("ID"))).indexOf("ENS")&&
(n=getFirstChildByName(getFirstChildByName(g[l],"annotationList"),"annotation"),null!=n&&(d+=" - "+n.getAttribute("reason")))),d+="<br>",0===h.length)for(n=getAllChildrenByName(getFirstChildByName(g[l],"annotationList"),"annotation"),q=0;q<n.length&&0===h.length;q++)"AKA"===n[q].getAttribute("source")&&(h=n[q].getAttribute("annot_value"),h=(new String(h)).split(":"),h=0!==(new String(h[h.length-1])).indexOf("ENS")?h[h.length-1]:h[0]);g='<BR><div id="ttSVG" style="background:#FFFFFF;overflow:scroll;max-height:400px;"></div>Gene ID: '+
e+"<BR>Gene Symbol: "+h+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+c+"<BR><BR>Transcripts:<BR>"+d+"<BR>"}if(("brainTotal"===b.trackClass||"liverTotal"===b.trackClass||"kidenyTotal"===b.trackClass)&&(-1<a.getAttribute("ID").indexOf("PRN6")||-1<a.getAttribute("ID").indexOf("PRN6"))){c="Gene";if(-1<a.getAttribute("ID").indexOf("PRN6T")||-1<a.getAttribute("ID").indexOf("PRN6.5T"))c="Transcript";
g=g+'<div>Heritability: <span id="ttsingleHerit"></span></div>'+c+' Expression:<BR><div style="text-align:center;">Median Estimated Counts Per Million</div><div id="ttChart"></div>';d="Brain";e="Whole Brain";"liverTotal"===b.trackClass?e=d="Liver":"kidneyTotal"===b.trackClass&&(e=d="Kidney");tmpChart=chart({data:"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/"+d+"expr.json",selector:"#ttChart",allowResize:!1,type:"scatter",width:"400",height:"275",displayHerit:!1,displayControls:!1,
...
"Trx"!==b.drawAs||"Gene"===b.drawnAs&&$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.draw(b.data);else if(a<b.trxCutoff&&"Trx"===b.drawnAs&&"Trx"!==b.drawAs&&(d=1),"Gene"===b.drawAs&&0===d||-1<b.trackClass.indexOf("smallnc"))b.svg&&(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").attr("transform",function(c,g){return"translate("+b.xScale(c.getAttribute("start"))+","+b.calcY(parseInt(c.getAttribute("start"),10),parseInt(c.getAttribute("stop"),10),g)+")"}),d3.select("#Level"+
b.gsvg.levelNumber+b.trackClass).selectAll("g.gene rect").attr("width",function(c){var g=1;1<b.xScale(c.getAttribute("stop"))-b.xScale(c.getAttribute("start"))&&(g=b.xScale(c.getAttribute("stop"))-b.xScale(c.getAttribute("start")));return g}).attr("stroke",b.colorStroke),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").each(function(c){var g=d3.select(this),e=parseInt(c.getAttribute("strand"),10);if(-1===e||1===e){var h=">";-1===e&&(h="<");e="";var l=g.select("rect").attr("width");
if(8.3<=l&&15.8>=l)e=h;else for(;8.5<l;)e+=h,l-=8.5;g.select("text#strandTxt").text(e)}if(2===b.density){var n=c.getAttribute("ID");c.getAttribute("geneSymbol")&&0<c.getAttribute("geneSymbol").length&&(n=n+" ("+c.getAttribute("geneSymbol")+")");0===g.select("text#lblTxt").size()?g.append("svg:text").attr("dx",function(){var q=b.xScale(c.getAttribute("start"));return q=q<$(window).width()/2?b.xScale(c.getAttribute("stop"))-b.xScale(c.getAttribute("start"))+5:-9*n.length}).attr("dy",10).attr("id","lblTxt").text(n):
g.select("text#lblTxt").attr("dx",function(){var q=b.xScale(c.getAttribute("start"));return q=q<$(window).width()/2?b.xScale(c.getAttribute("stop"))-b.xScale(c.getAttribute("start"))+5:-9*n.length})}else g.select("text#lblTxt").remove()})),1===b.density?b.svg.attr("height",30):2===b.density?b.svg.attr("height",15*(b.trackYMax+1)):3===b.density&&b.svg.attr("height",15*(b.trackYMax+1));else if(1===d||"Trx"===b.drawAs)b.svg.selectAll(".trx"+b.gsvg.levelNumber).attr("transform",function(c,g){return"translate(0,"+
b.calcY(parseInt(c.getAttribute("start"),10),parseInt(c.getAttribute("stop"),10),g)+")"}).each(function(c,g){g=parseInt(c.getAttribute("start"),10);var e=parseInt(c.getAttribute("stop"),10);c.getAttribute("cdsStart")&&c.getAttribute("cdsStop")&&(g=parseInt(c.getAttribute("cdsStart"),10),e=parseInt(c.getAttribute("cdsStop"),10));exList=getAllChildrenByName(getFirstChildByName(c,"exonList"),"exon");var h="";1===b.gsvg.levelNumber?h="tx":99===b.gsvg.levelNumber&&(h="ttTx");for(var l=0;l<exList.length;l++){var n=
exList[l].getAttribute("start"),q=exList[l].getAttribute("stop");if(n<g&&g<q||q>e&&e>n){var u=n,v=g;q>e?(u=e,v=q,q=e):n=g;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g#"+h+b.cleanID(c.getAttribute("ID"))+b.gsvg.levelNumber+" rect#ExNC"+exList[l].getAttribute("ID")).attr("x",function(D){return b.xScale(u)}).attr("width",function(D){return b.xScale(v)-b.xScale(u)})}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g#"+h+b.cleanID(c.getAttribute("ID"))+b.gsvg.levelNumber+" rect#Ex"+exList[l].getAttribute("ID")).attr("x",
function(D){return b.xScale(n)}).attr("width",function(D){return b.xScale(q)-b.xScale(n)});if(0<l){var y=">";"-1"==c.getAttribute("strand")&&(y="<");var w=y,C=b.xScale(exList[l-1].getAttribute("stop")),B=b.xScale(exList[l].getAttribute("start")),E=B-C,G=0;if(7>E||"0"==c.getAttribute("strand")||"."==c.getAttribute("strand"))w="";else for(E-=7;8<E;)0===G?(w+=" ",G=1):(w+=y,G=0),E-=7;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+h+b.cleanID(c.getAttribute("ID"))+
b.gsvg.levelNumber+" line#Int"+exList[l-1].getAttribute("ID")+"_"+exList[l].getAttribute("ID")).attr("x1",C).attr("x2",B);d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+h+b.cleanID(c.getAttribute("ID"))+b.gsvg.levelNumber+" #IntTxt"+exList[l-1].getAttribute("ID")+"_"+exList[l].getAttribute("ID")).attr("dx",C+1).text(w)}}if(2===b.density){var A=c.getAttribute("ID");c.getAttribute("geneSymbol")&&0<c.getAttribute("geneSymbol").length&&(A=A+" ("+c.getAttribute("geneSymbol")+
")");0===d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+h+b.cleanID(c.getAttribute("ID"))+b.gsvg.levelNumber+" text#lblTxt").size()?d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+h+b.cleanID(c.getAttribute("ID"))+b.gsvg.levelNumber).append("svg:text").attr("dx",function(){var D=b.xScale(c.parent.getAttribute("start"));return D=D<$(window).width()/2?b.xScale(c.parent.getAttribute("stop"))-b.xScale(c.parent.getAttribute("start"))+
5:-9*A.length}).attr("dy",10).attr("id","lblTxt").text(A):d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+h+b.cleanID(c.getAttribute("ID"))+b.gsvg.levelNumber+" text#lblTxt").attr("dx",function(){var D=b.xScale(c.getAttribute("start"));return D=D<$(window).width()/2?b.xScale(c.getAttribute("stop"))-b.xScale(c.getAttribute("start"))+5:-9*A.length})}else d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+h+b.cleanID(c.getAttribute("ID"))+
b.gsvg.levelNumber+" text#lblTxt").remove()}),1===b.density?b.svg.attr("height",30):2===b.density?b.svg.attr("height",15*(b.trackYMax+1)):3===b.density&&b.svg.attr("height",15*(b.trackYMax+1));b.redrawSelectedArea()};b.setSelected=function(a){if(a)if(0<d3.selectAll("g.gene").size()){if(d3.selectAll("rect.selected").each(function(){d3.select(this).attr("class","").style("fill",b.color)}),a=d3.select("g.gene rect#"+a))a.attr("class","selected").style("fill","green"),b.setupDetailedView(a.data()[0]),
selectGene=""}else 0<d3.selectAll("g.trx"+b.gsvg.levelNumber).size()&&(a=(new String(a)).replace(".","_"),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("line").style("stroke",b.color),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("rect").style("fill",b.color),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("text").style("opacity","0.6").style("fill",b.color),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").each(function(){var d=new String($(this).attr("class"));
d=d.replace(" selected","");$(this).attr("class",d)}),b.svg.selectAll("g.gene"+a).each(function(){var d=$(this).attr("class")+" selected";$(this).attr("class",d)}),b.svg.selectAll("g.gene"+a).selectAll("line").style("stroke","green"),b.svg.selectAll("g.gene"+a).selectAll("rect").style("fill","green"),b.svg.selectAll("g.gene"+a).selectAll("text").style("opacity","0.3").style("fill","green"),b.svg.selectAll("g.gene"+a)&&(a=b.svg.selectAll("g.gene"+a).data())&&0<a.length&&b.setupDetailedView(a[0].parent),
selectGene="")};b.clearSelection=function(){b.selectionStart=-1;b.selectionEnd=-1;b.svg.selectAll("rect.selectedArea").remove();0<b.svg.selectAll("g.gene").size()?b.svg.selectAll("rect.selected").each(function(a){d3.select(this).attr("class","").style("fill",b.color)}):0<b.svg.selectAll("g.trx"+b.gsvg.levelNumber).size()&&(b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("line").style("stroke",b.color),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("rect").style("fill",
b.color),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("text").style("opacity","0.6").style("fill",b.color),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").each(function(){var a=new String($(this).attr("class"));a=a.replace(" selected","");$(this).attr("class",a)}))};b.setupDetailedView=function(a){if(a){b.gsvg.clearSelection();jQuery.Event("keyup").which=32;var d=b.gsvg.levelNumber+1;$("div#collapsableReport").is(":hidden")||($("div#collapsableReport").hide(),$("span[name='collapsableReport']").removeClass("less"));
$("div#selectedDetailHeader").is(":hidden")&&$("div#selectedDetailHeader").show();$("div#selectedDetail").is(":hidden")&&$("div#selectedDetail").show();var c="none";0===b.trackClass.indexOf("brainTotal")||0===b.trackClass.indexOf("liverTotal")||"heartTotal"===b.trackClass||"mergedTotal"===b.trackClass?c=b.trackClass:"protein_coding"===a.getAttribute("biotype")&&200<=a.getAttribute("stop")-a.getAttribute("start")?c="protein":"protein_coding"!==a.getAttribute("biotype")&&200<=a.getAttribute("stop")-
a.getAttribute("start")?c="long":200>a.getAttribute("stop")-a.getAttribute("start")&&(c="small");var g=0,e=0;if("protein"===c||"long"===c||0===c.indexOf("liverTotal")||"heartTotal"===c||0===c.indexOf("brainTotal")||"mergedTotal"===c||"kidneyTotal"===c||"small"===c&&"rn5"!==genomeVer||"small"===c&&-1<(new String(a.getAttribute("ID"))).indexOf("ENS")){g=a.getAttribute("ID");var h="";if(a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length)g=g+" ("+a.getAttribute("geneSymbol")+")";else{e=
getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(var l=0;l<e.length&&""===h;l++)if(""===h)for(var n=getAllChildrenByName(getFirstChildByName(e[l],"annotationList"),"annotation"),q=0;q<n.length&&0===h.length;q++)if("AKA"===n[q].getAttribute("source")){var u=n[q].getAttribute("annot_value");u=(new String(u)).split(":");1<=u.length?(h=u[0],g=g+" ("+h+")"):2<=u.length&&(h=u[0],u=u[1],g=g+" ("+u+")")}}e=parseInt(a.getAttribute("start"),10);n=parseInt(a.getAttribute("stop"),
10);a.getAttribute("extStart")&&a.getAttribute("extStop")&&(e=parseInt(a.getAttribute("extStart"),10),n=parseInt(a.getAttribute("extStop"),10));svgViewIDList[d]||(svgViewIDList[d]=b.gsvg.currentView?b.gsvg.currentView.ViewID:defaultView);l=GenomeSVG("div#selectedImage",b.gsvg.width,e,n,d,g,"transcript");l.xMin=e-.05*(n-e);l.xMax=n+.05*(n-e);g=l.xMin;e=l.xMax;svgList[d]=l;svgList[d].txType=c;svgList[d].selectedData=a;selectedGeneSymbol=a.getAttribute("geneSymbol");b.gsvg.selectedGeneSymbol=a.getAttribute("geneSymbol");
selectedID=new String(a.getAttribute("ID"));-1===selectedID.indexOf("ENS")&&0<h.length&&(selectedID=h);$("div#selectedImage").show();-1<(new String(selectedID)).indexOf("ENS")&&-1<b.trackClass.indexOf("ensembl")?($("div#selectedReport").show(),d=pathPrefix+"geneReport.jsp",a={id:selectedID,geneSymbol:selectedGeneSymbol,chromosome:chr,species:organism,genomeVer,dataVer},DisplaySelectedDetailReport(d,a)):"small"!==c?$("div#selectedReport").html("<BR><BR>Detailed Gene Reports are not currently provided for RNA-Seq(Cufflinks) generated genes that are novel and have not been matched to an existing annotated gene.  In the future overlapping probesets and SNPs will be summarized in the same manner.  This feature should be added by December 2014.  Please keep checking back or let us know you'd like to see it sooner."):
...
"Ensembl","undefined"!==typeof h.getAttribute("strain")&&(l=h.getAttribute("strain")),"undefined"===typeof b.counts[l]?(b.counts[l]={},b.counts[g]=b.counts[l],g++,b.counts[l].value=1,b.counts[l].names=l):b.counts[l].value++,a[d]=h,d++,e++})}return a};b.updateData=function(a){var d=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/"+b.trackClass+".xml";if(0===b.trackClass.indexOf("heartTotal")||0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal")||
0===b.trackClass.indexOf("kidneyTotal")||"mergedTotal"===b.trackClass||0===b.trackClass.indexOf("brainIso")||0===b.trackClass.indexOf("liverIso"))d=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/"+dataVer+"_"+b.trackClass+".xml";d3.xml(d,function(c,g){if(c){if(0===a&&(0===b.trackClass.indexOf("heartTotal")||0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal")||0===b.trackClass.indexOf("kidneyTotal")||"mergedTotal"===b.trackClass||-1<b.trackClass.indexOf("smallnc")||
0===b.trackClass.indexOf("brainIso")||0===b.trackClass.indexOf("liverIso"))){c="/"+pathPrefix;pathPrefix||(c="");g=b.trackClass;var e=panel;if((0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal"))&&0<b.trackClass.indexOf("_"))e=track.substr(track.indexOf("_")+1),"LEStm"===e&&(e="LE-Stm"),"F344Stm"===e&&(e="F344-Stm");else if(0===b.trackClass.indexOf("brainIso")||0===b.trackClass.indexOf("liverIso"))e="IsoSeq";$.ajax({url:c+"generateTrackXML.jsp",type:"GET",cache:!1,asyn:!0,
data:{chromosome:chr,minCoord:b.gsvg.xScale.domain()[0],maxCoord:b.gsvg.xScale.domain()[1],panel:e,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:g,folder:b.gsvg.folderName},dataType:"json",success:function(n){gtag("event","generateTrackGene",{event_category:"browser"})},error:function(n,q,u){}})}10>a?(c=2E3,1===a&&(c=5E3),setTimeout(function(){b.updateData(a+1)},c)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+
b.trackClass),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass))}else if(g){c=g.documentElement.getElementsByTagName("Gene");g=[];e=[];for(var h=0,l=0;l<c.length;l++)c[l]&&(g[h]=c[l],e[c[l].getAttribute("ID")]=1,h++);for(l=0;l<b.data.length;l++)b.data[l]&&!e[b.data[l].getAttribute("ID")]&&(g[h]=b.data[l],h++);b.draw(g);b.hideLoading();setTimeout(function(){DisplayRegionReport()},300)}else b.hideLoading()})};
b.updateDataVersion=function(a,d){var c=b.trackClass+"_"+a;if(0===b.trackClass.indexOf("heartTotal")||0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal")||0===b.trackClass.indexOf("kidneyTotal")||"mergedTotal"===b.trackClass||0===b.trackClass.indexOf("brainIso")||0===b.trackClass.indexOf("liverIso"))c=dataVer+"_"+b.trackClass+".xml";d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/"+c+".xml",function(g,e){g?(0===d&&(g="/"+pathPrefix,
pathPrefix||(g=""),e=panel,(0===track.indexOf("liverTotal")||0===track.indexOf("brainTotal"))&&0<track.indexOf("_")&&(e=track.substr(track.indexOf("_")+1),"LEStm"===e&&(e="LE-Stm"),"F344Stm"===e&&(e="F344-Stm")),$.ajax({url:g+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:b.gsvg.xScale.domain()[0],maxCoord:b.gsvg.xScale.domain()[1],panel:e,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:c,folder:b.gsvg.folderName},dataType:"json",success:function(h){gtag("event",
"generateTrackGene",{event_category:"browser"})},error:function(h,l,n){}})),5>d?(g=1E4,0===d&&(g=5E3),setTimeout(function(){b.updateDataVersion(a,d+1)},g)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass))):(g=e.documentElement.getElementsByTagName("Gene"),e=parseInt(a),b.dataVer=
e,b.draw(g),b.hideLoading(),setTimeout(function(){DisplayRegionReport()},300))})};b.drawTrx=function(a,d){d=parseInt(a.getAttribute("start"),10);var c=parseInt(a.getAttribute("stop"),10);a.getAttribute("cdsStart")&&a.getAttribute("cdsStop")&&(d=parseInt(a.getAttribute("cdsStart"),10),c=parseInt(a.getAttribute("cdsStop"),10));var g="";1===b.gsvg.levelNumber?g="tx":99===b.gsvg.levelNumber&&(g="ttTx");g=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" #"+g+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber);
exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var e=0;e<exList.length;e++){var h=parseInt(exList[e].getAttribute("start"),10),l=parseInt(exList[e].getAttribute("stop"),10);if(h<d&&d<l||l>c&&c>h){var n=0,q=0,u=0,v=0;h<d?(n=b.xScale(h),q=b.xScale(d)-b.xScale(h),u=b.xScale(d),v=b.xScale(l)-b.xScale(d)):l>c&&(u=b.xScale(h),v=b.xScale(c)-b.xScale(h),n=b.xScale(c),q=b.xScale(l)-b.xScale(c));g.append("rect").attr("x",n).attr("y",2.5).attr("height",5).attr("width",q).attr("title",
function(w){return exList[e].getAttribute("ID")}).attr("id",function(w){return"ExNC"+exList[e].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer");g.append("rect").attr("x",u).attr("height",10).attr("width",v).attr("title",function(w){return exList[e].getAttribute("ID")}).attr("id",function(w){return"Ex"+exList[e].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}else{n=10;q=0;if(h<d&&l<d||l>c&&h>c)n=5,q=2.5;g.append("rect").attr("x",function(w){return b.xScale(exList[e].getAttribute("start"))}).attr("y",
q).attr("id",function(w){return"Ex"+exList[e].getAttribute("ID")}).attr("height",n).attr("width",function(w){return b.xScale(exList[e].getAttribute("stop"))-b.xScale(exList[e].getAttribute("start"))}).attr("title",function(w){return exList[e].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}if(0<e&&(g.append("line").attr("x1",function(w){return b.xScale(exList[e-1].getAttribute("stop"))}).attr("x2",function(w){return b.xScale(exList[e].getAttribute("start"))}).attr("y1",5).attr("y2",
5).attr("id",function(w){return"Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")}).attr("stroke",b.color).attr("stroke-width","2"),"1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))){h=">";if("-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand"))h="<";l=h;n=b.xScale(exList[e-1].getAttribute("stop"));q=b.xScale(exList[e].getAttribute("start"))-n;u=0;if(7>q)l="";else for(q-=7;8<q;)0==u?(l+=" ",u=1):(l+=h,u=0),q-=7;g.append("svg:text").attr("id",function(w){return"IntTxt"+
exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")}).attr("dx",n+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(l)}}if(2==b.density){var y=a.getAttribute("ID");"undefined"!==typeof a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length&&(y=y+" ("+a.getAttribute("geneSymbol")+")");g.append("svg:text").attr("dx",function(){var w=b.xScale(a.getAttribute("start"));return w=w<$(window).width()/2?b.xScale(a.getAttribute("stop"))-
b.xScale(a.getAttribute("start"))+5:-9*y.length}).attr("dy",10).attr("id","lblTxt").text(y)}else g.select("text#lblTxt").remove()};b.getSVGID=function(a){return b.getDisplayID(a.getAttribute("ID"))+" ("+a.getAttribute("geneSymbol")+")"};b.setupToolTipSVG=function(a,d){if("undefined"!==typeof a.getAttribute("extStart")&&null!==a.getAttribute("extStart")){var c=parseInt(a.getAttribute("extStart"),10);var g=parseInt(a.getAttribute("extStop"),10)}else c=parseInt(a.getAttribute("start"),10),g=parseInt(a.getAttribute("stop"),
10);d=Math.floor((g-c)*d);10>d&&(d=10);c-=d;g+=d;d=toolTipSVG("div#ttSVG",450,c,g,99,b.getSVGID(a),"transcript");d.xMin=c;d.xMax=g;var e="none";0===b.trackClass.indexOf("brainTotal")||0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("kidneyTotal")||"heartTotal"===b.trackClass||"mergedTotal"==b.trackClass?e=b.trackClass:200>parseInt(a.getAttribute("stop"),10)-parseInt(a.getAttribute("start"),10)?e="small":"protein_coding"==a.getAttribute("biotype")&&200<=parseInt(a.getAttribute("stop"),
10)-parseInt(a.getAttribute("start"),10)?e="protein":"protein_coding"!=a.getAttribute("biotype")&&200<=parseInt(a.getAttribute("stop"),10)-parseInt(a.getAttribute("start"),10)&&(e="long");d.txType=e;d.selectedData=a;e=[];e[0]=a;"undefined"!==typeof a.parent&&(e[0]=a.parent);d.addTrack(b.trackClass,2,"DrawTrx",e);for(a=0;a<b.ttTrackList.length;a++)if(b.trackClass!==b.ttTrackList[a]){e=b.gsvg.getTrackData(b.ttTrackList[a]);var h=[];if("undefined"!==typeof e&&0<e.length){for(var l=0,n=0;n<e.length;n++)if(c<=
parseInt(e[n].getAttribute("start"),10)&&parseInt(e[n].getAttribute("start"),10)<=g||c<=parseInt(e[n].getAttribute("stop"),10)&&parseInt(e[n].getAttribute("stop"),10)<=g||parseInt(e[n].getAttribute("start"),10)<=c&&parseInt(e[n].getAttribute("stop"),10)>=c)h[l]=e[n],l++;0<h.length&&d.addTrack(b.ttTrackList[a],3,"DrawTrx",h)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(var d=0;d<b.gsvg.width;d++)b.yMaxArr[d]=0,b.yArr[0][d]=0;b.svg.selectAll(".gene").remove();
d=b.drawAs;var c=b.xScale.domain()[0];if(b.xScale.domain()[1]-c<b.trxCutoff||$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.drawAs="Trx";c="Genes";"Trx"==b.drawAs&&(c="Transcripts");var g="Protein Coding",e=" / PolyA+";-1<b.trackClass.indexOf("noncoding")?(g="Long Non-Coding",e=" / Non-PolyA+"):-1<b.trackClass.indexOf("smallnc")?(g="Brain",-1<b.trackClass.indexOf("liver")?g="Liver":-1<b.trackClass.indexOf("heart")&&(g="Heart"),g+=" Small RNA",e=""):0===b.trackClass.indexOf("liverTotal")?(g=
"Liver RNA-Seq  ",e="Total RNA"):"heartTotal"==b.trackClass?(g="Heart RNA-Seq Reconstruction ",e="Total RNA"):0===b.trackClass.indexOf("brainTotal")?(g="Whole Brain RNA-Seq Reconstruction ",e="Total RNA"):0===b.trackClass.indexOf("kidneyTotal")?(g="Kidney RNA-Seq Reconstruction ",e="Total RNA"):"mergedTotal"==b.trackClass?(g="Merged (Brain,Heart,Liver,Kidney) Reconstructed",e="Total RNA"):"brainIso"==b.trackClass?(g="Whole Brain",e="Iso-Seq"):"liverIso"==b.trackClass&&(g="Liver",e="Iso-Seq");g=-1<
b.trackClass.indexOf("ensembl")?"Ensembl "+g+" "+c:-1===b.trackClass.indexOf("brainTotal")&&-1===b.trackClass.indexOf("liverTotal")&&"heartTotal"!=b.trackClass&&"mergedTotal"!=b.trackClass&&-1===b.trackClass.indexOf("kidenyTotal")&&-1===b.trackClass.indexOf("Iso")?"Reconstruction "+g+e+" "+c:-1<b.trackClass.indexOf("Iso")?g+" "+e:g+e+" "+c;(0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal"))&&0<b.trackClass.indexOf("_")&&(c=b.trackClass.substr(b.trackClass.indexOf("_")+
1),"LEStm"===c&&(c="LE-Stm"),"F344Stm"===c&&(c="F344-Stm"),g=c+" Filtered (>=1.0TPM) "+g);b.updateLabel(g);b.redrawLegend();c=a;if("Trx"===b.drawAs&&-1===b.trackClass.indexOf("smallnc"))for(c=[],e=g=0;e<a.length;e++)if("undefined"!==typeof a[e])for(var h=getAllChildrenByName(getFirstChildByName(a[e],"TranscriptList"),"Transcript"),l=0;l<h.length;l++)c[g]=h[l],c[g].parent=a[e],g++;0<c.length&&("Gene"===b.drawAs||-1<b.trackClass.indexOf("smallnc")?(b.drawnAs="Gene",b.svg.selectAll(".trx0").each(function(){d3.select(this).remove()}),
...
function RefSeqTrack(p,m,r,t,f){var b=GeneTrack(p,m,r,t);b.counts=[{value:0,names:"Ensembl"},{value:0,names:"RNA-Seq"}];b.drawAs="Gene";b.trxCutoff=1E5;b.density=3;-1<(new String(f)).indexOf("DrawTrx")&&(b.drawAs="Trx");b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(p=0;p<b.gsvg.width;p++)b.yMaxArr[p]=0,b.yArr[0][p]=0;b.ttTrackList=[];b.ttTrackList.push("ensemblcoding");b.ttTrackList.push("braincoding");b.ttTrackList.push("liverTotal");b.ttTrackList.push("heartTotal");b.ttTrackList.push("mergedTotal");b.ttTrackList.push("brainIso");
b.ttTrackList.push("liverIso");b.ttTrackList.push("ensemblnoncoding");b.ttTrackList.push("brainnoncoding");b.ttTrackList.push("repeatMask");b.ttTrackList.push("snpSHRH");b.ttTrackList.push("snpBNLX");b.ttTrackList.push("snpF344");b.ttTrackList.push("snpSHRJ");b.cleanID=function(a){return a=a.replace(/\./g,"_")};b.color=function(a){var d=d3.rgb("#000000");if("Gene"==b.drawnAs){a=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(var c=-1,g=0;g<a.length;g++){var e=new String(a[g].getAttribute("category")),
h=-1;"Reviewed"==e?h=5:"Validated"==e?h=4:"Provisional"==e?h=3:"Inferred"==e?h=2:"Predicted"==e?h=1:"Model"==e?h=1:"Unknown"==e&&(h=0);c<h&&(c=h)}5===c&&(d=d3.rgb("#18814F"));4===c?d=d3.rgb("#38A16F"):3===c?d=d3.rgb("#78E1AF"):2===c?d=d3.rgb("#A8FFDF"):1===c?d=d3.rgb("#A8DFFF"):0===c&&(d=d3.rgb("#C8FFFF"))}else"Trx"==b.drawnAs&&(e=new String(a.getAttribute("category")),"Reviewed"==e?d=d3.rgb("#18814F"):"Validated"==e?d=d3.rgb("#38A16F"):"Provisional"==e?d=d3.rgb("#78E1AF"):"Inferred"==e?d=d3.rgb("#A8FFDF"):
"Predicted"==e?d=d3.rgb("#A8DFFF"):"Model"===e?d=d3.rgb("#A8DFFF"):"Unknown"===e&&(d=d3.rgb("#C8FFFF")));return d};b.pieColor=function(a,d){d=d3.rgb("#000000");a=new String(a.data.names);-1<a.indexOf("Reviewed")?d=d3.rgb("#18814F"):-1<a.indexOf("Validated")?d=d3.rgb("#38A16F"):-1<a.indexOf("Provisional")?d=d3.rgb("#78E1AF"):-1<a.indexOf("Inferred")?d=d3.rgb("#A8FFDF"):-1<a.indexOf("Predicted")?d=d3.rgb("#A8DFFF"):-1<a.indexOf("Model")?d=d3.rgb("#A8DFFF"):-1<a.indexOf("Unknown")&&(d=d3.rgb("#C8FFFF"));
return d};b.getDisplayedData=function(){var a=[],d=0,c=0;if("Gene"==b.drawnAs){var g=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".gene");b.counts=[];var e=0;g.each(function(h){var l=b.xScale(h.getAttribute("start")),n=b.xScale(h.getAttribute("stop"));if(0<=l&&l<=b.gsvg.width||0<=n&&n<=b.gsvg.width||0>=l&&n>=b.gsvg.width){n=getAllChildrenByName(getFirstChildByName(h,"TranscriptList"),"Transcript");l=-1;for(var q=0;q<n.length;q++){var u=new String(n[q].getAttribute("category")),v=
-1;"Reviewed"==u?v=5:"Validated"==u?v=4:"Provisional"==u?v=3:"Inferred"==u?v=2:"Predicted"==u?v=1:"Model"==u?v=1:"Unknown"==u&&(v=1);l<v&&(l=v)}n="Unknown";5==l?n="Reviewed":4==l?n="Validated":3==l?n="Provisional":2==l?n="Inferred":1==l&&(n="Predicted/Model");"undefined"===typeof b.counts[n]?(b.counts[n]={},b.counts[e]=b.counts[n],e++,b.counts[n].value=1,b.counts[n].names=n):b.counts[n].value++;a[d]=h;d++;c++}})}else b.counts=[],e=0,b.svg.selectAll("g.trx"+b.gsvg.levelNumber).each(function(h){var l=
b.xScale(h.getAttribute("start")),n=b.xScale(h.getAttribute("stop"));if(0<=l&&l<=b.gsvg.width||0<=n&&n<=b.gsvg.width||0>=l&&n>=b.gsvg.width)l=new String(h.getAttribute("category")),"undefined"===typeof b.counts[l]?(b.counts[l]={},b.counts[e]=b.counts[l],e++,b.counts[l].value=1,b.counts[l].names=l):b.counts[l].value++,a[d]=h,d++,c++});return a};b.createToolTip=function(a){var d="",c=a.getAttribute("strand");c="1"==c||"+"==c?"+":"-1"==c||"-"==c?"-":".";if("Gene"===b.drawnAs){d="";for(var g=getAllChildrenByName(getFirstChildByName(a,
"TranscriptList"),"Transcript"),e=0;e<g.length;e++){var h=new String(g[e].getAttribute("ID"));d+="<B>"+h+"</B> - "+g[e].getAttribute("category");d+="<br>"}d='<BR><div id="ttSVG" style="background:#FFFFFF;"></div>Gene Symbol: '+a.getAttribute("geneSymbol")+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+c+"<BR>Transcripts:<BR>"+d}else"Trx"===b.drawnAs&&(h=new String(a.getAttribute("ID")),d='<BR><div id="ttSVG" style="background:#FFFFFF;"></div>RefSeq ID:<B>'+
...
" rect#Ex"+exList[n].getAttribute("ID")).attr("x",function(D){return b.xScale(q)}).attr("width",function(D){return b.xScale(u)-b.xScale(q)});if(0<n){var w=b.xScale(exList[n-1].getAttribute("stop")),C=b.xScale(exList[n].getAttribute("start"));if("1"==g.getAttribute("strand")||"-1"==g.getAttribute("strand")||"-"==g.getAttribute("strand")||"+"==g.getAttribute("strand")){var B=">";"-1"==g.getAttribute("strand")&&(B="<");var E=B,G=C-w,A=0;if(7>G)E="";else for(G-=7;8<G;)0==A?(E+=" ",A=1):(E+=B,A=0),G-=
...
myOrganism:organism,genomeVer,dataVer,track:b.trackClass,folder:b.gsvg.folderName,panel},dataType:"json",success:function(q){},error:function(q,u,v){}})),10>a?(g=2500,1==a&&(g=1E4),setTimeout(function(){b.updateData(a+1)},g)):(d3.select("#Level"+b.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass));else if(e){g=
e.documentElement.getElementsByTagName("Gene");e=[];for(var h=[],l=0,n=0;n<g.length;n++)g[n]&&(e[l]=g[n],e[l].setAttribute("ID",l),h[g[n].getAttribute("geneSymbol")]=1,l++);for(n=0;n<b.data.length;n++)b.data[n]&&!h[b.data[n].getAttribute("geneSymbol")]&&(e[l]=b.data[n],e[l].setAttribute("ID",l),l++);b.draw(e);b.hideLoading();b.getDisplayedData();setTimeout(function(){DisplayRegionReport()},300)}else b.updateData(a+1),b.hideLoading()})};b.drawTrx=function(a,d){d=parseInt(a.getAttribute("cdsStart"),
10);var c=parseInt(a.getAttribute("cdsStop"),10),g="";1==b.gsvg.levelNumber&&(g="tx");g=b.svg.select("#"+g+b.cleanID(a.getAttribute("ID")));exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var e=0;e<exList.length;e++){var h=parseInt(exList[e].getAttribute("start"),10),l=parseInt(exList[e].getAttribute("stop"),10);if(h<d&&d<l||l>c&&c>h){var n=0,q=0,u=0,v=0;h<d?(n=b.xScale(h),q=b.xScale(d)-b.xScale(h),u=b.xScale(d),v=b.xScale(l)-b.xScale(d)):l>c&&(u=b.xScale(h),v=b.xScale(c)-
b.xScale(h),n=b.xScale(c),q=b.xScale(l)-b.xScale(c));g.append("rect").attr("x",n).attr("y",2.5).attr("height",5).attr("width",q).attr("title",function(y){return exList[e].getAttribute("ID")}).attr("id",function(y){return"ExNC"+exList[e].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer");g.append("rect").attr("x",u).attr("height",10).attr("width",v).attr("title",function(y){return exList[e].getAttribute("ID")}).attr("id",function(y){return"Ex"+exList[e].getAttribute("ID")}).style("fill",
b.color).style("cursor","pointer")}else{n=10;q=0;if(h<d&&l<d||l>c&&h>c)n=5,q=2.5;g.append("rect").attr("x",function(y){return b.xScale(exList[e].getAttribute("start"))}).attr("y",q).attr("height",n).attr("width",function(y){return b.xScale(exList[e].getAttribute("stop"))-b.xScale(exList[e].getAttribute("start"))}).attr("title",function(y){return exList[e].getAttribute("ID")}).attr("id",function(y){return"Ex"+exList[e].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}if(0<e&&(g.append("line").attr("x1",
function(y){return b.xScale(exList[e-1].getAttribute("stop"))}).attr("x2",function(y){return b.xScale(exList[e].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(y){return"Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")}).attr("stroke",b.color).attr("stroke-width","2"),"1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand"))){h=">";if("-"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))h=
"<";l=h;n=b.xScale(exList[e-1].getAttribute("stop"));q=b.xScale(exList[e].getAttribute("start"))-n;u=0;if(7>q)l="";else for(q-=7;8<q;)0==u?(l+=" ",u=1):(l+=h,u=0),q-=7;g.append("svg:text").attr("id",function(y){return"IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")}).attr("dx",n+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(l)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yArr=[];b.yArr[0]=[];
...
b.displayStrain}];b.strain=p;b.ttTrackList=[];b.ttTrackList.push("ensemblcoding");b.ttTrackList.push("braincoding");b.ttTrackList.push("liverTotal");b.ttTrackList.push("heartTotal");b.ttTrackList.push("mergedTotal");b.ttTrackList.push("brainIso");b.ttTrackList.push("liverIso");b.ttTrackList.push("refSeq");b.ttTrackList.push("ensemblnoncoding");b.ttTrackList.push("brainnoncoding");b.ttTrackList.push("ensemblsmallnc");b.ttTrackList.push("brainsmallnc");b.ttTrackList.push("probe");b.ttTrackList.push("repeatMask");
b.xPadding=1;b.scanBackYLines=50;b.updateSettingsFromUI=function(){0<$("#"+b.trackClass+"Dense"+b.level+"Select").length&&(b.density=$("#"+b.trackClass+"Dense"+b.level+"Select").val());0<$("#"+b.trackClass+b.level+"Select").length&&(b.include=$("#"+b.trackClass+b.level+"Select").val())};b.savePrevious=function(){b.prevSetting={};b.prevSetting.density=b.density;b.prevSetting.include=b.include};b.revertPrevious=function(){b.density=b.prevSetting.density;b.include=b.prevSetting.include};b.generateTrackSettingString=
function(){return b.trackClass+","+b.density+","+b.include+";"};b.generateSettingsDiv=function(d){var c=trackInfo[b.trackClass];b.savePrevious();d3.select(d).select("table").select("tbody").html("");if(0<c.Controls.length&&"null"!=c.Controls){var g=(new String(c.Controls)).split(",");d=d3.select(d).select("table").select("tbody");d.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+c.Name);for(c=0;c<g.length;c++)if("undefined"!==typeof g[c]&&""!=g[c]){var e=g[c].split(";"),
h=d.append("tr").append("td"),l=e[0].substr(5);h.append("text").text(l+": ");3<e.length&&0==e[3].indexOf("Default=")&&e[3].substr(8);if(0==e[1].toLowerCase().indexOf("select")){var n=e[1].split(":");e=e[2].split("}");var q=l="";2<n.length&&(l=n[2]);3<n.length&&(q=n[3]);h=h.append("select").attr("id",b.trackClass+l+b.level+q).attr("name",n[1]);for(n=0;n<e.length;n++){var u=e[n].substr(1).split(":");if(2==u.length){var v=h.append("option").attr("value",u[1]).text(u[0]);("Dense"==l&&u[1]==b.density||
""==l&&u[1]==b.include)&&v.attr("selected","selected")}}d3.select("#"+b.trackClass+l+b.level+q).on("change",function(){b.updateSettingsFromUI();b.updateData()})}}g=d.append("tr").append("td");g.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");b.gsvg.removeTrack(b.trackClass);var y=svgList[b.gsvg.levelNumber].currentView.ViewID,w=viewMenu[b.gsvg.levelNumber].findTrackByClass(b.trackClass,
...
f.counts=[];var a=0;b=b.data();var d=[],c=0,g=0;if("undefined"!==typeof b)for(var e=0;e<b.length;e++){var h=f.xScale(b[e].getAttribute("start")),l=f.xScale(b[e].getAttribute("stop"));if(0<=h&&h<=f.gsvg.width||0<=l&&l<=f.gsvg.width||0>=h&&l>=f.gsvg.width)h=new String(b[e].getAttribute("name")),l=/[0-9]+\s*$/g,l=h.search(l),-1<l&&(h=h.substr(0,l)),l=/QTL\s*$/g,l=h.search(l),-1<l&&(h=h.substr(0,l)),l=/traits+\s*$/g,l=h.search(l),-1<l&&(h=h.substr(0,l+5)),"undefined"===typeof f.counts[h]?(f.counts[h]=
{},f.counts[a]=f.counts[h],a++,f.counts[h].value=1,f.counts[h].names=h):f.counts[h].value++,d[c]=b[e],c++,g++}return d};f.setupDetailedView=function(b){$("div#collapsableReport").is(":hidden")||($("div#collapsableReport").hide(),$("span[name='collapsableReport']").removeClass("less"));$("div#selectedDetailHeader").is(":hidden")&&$("div#selectedDetailHeader").show();$("div#selectedDetail").is(":hidden")&&$("div#selectedDetail").show();$("div#selectedImage").hide();$("div#selectedReport").show();var a=
...
p.ttTrackList.push("mergedTotal");p.ttTrackList.push("brainIso");p.ttTrackList.push("liverIso");p.ttTrackList.push("refSeq");p.ttTrackList.push("repeatMask");"splcJnct"===r?p.ttTrackList.push("illuminaPolyA"):"liverspliceJnct"===r?(p.ttTrackList.push("liverilluminaTotalPlus"),p.ttTrackList.push("liverilluminaTotalMinus")):"heartspliceJnct"===r?(p.ttTrackList.push("heartilluminaTotalPlus"),p.ttTrackList.push("heartilluminaTotalMinus")):"brainspliceJnct"===r&&(p.ttTrackList.push("brainilluminaTotalPlus"),
p.ttTrackList.push("brainilluminaTotalMinus"));p.colorValueField="readCount";p.colorScale=d3.scaleLinear().domain([1,1E3]).range(["#E6E6E6","#000000"]);p.legendLbl="Read Depth";p.density=3;p.getDisplayID=function(a){return a.getAttribute("name")};p.createToolTip=function(a){var d='<BR><div id="ttSVG" style="background:#FFFFFF;"></div><BR>ID: '+a.getAttribute("name");d=d+"<BR>Read Counts="+numberWithCommas(a.getAttribute("readCount"));d=d+"<BR>Splice Junction in "+a.getAttribute("sampleCount")+" of ";
"Rn"===organism?d+="6 samples(3 BN-Lx, 3 SHR)":"Mm"===organism&&(d+="6 samples(3 ILS, 3 ISS)");var c=getAllChildrenByName(getFirstChildByName(a,"blockList"),"block"),g=0,e=1,h="start",l="stop";-1==a.getAttribute("strand")&&(g=1,e=0,h="stop",l="start");d=d+"<BR><BR>Coverage Exon 1: "+numberWithCommas(c[g].getAttribute(h))+"-"+numberWithCommas(c[g].getAttribute(l))+" ("+(parseInt(c[g].getAttribute("stop"),10)-parseInt(c[g].getAttribute("start"),10))+" bp)";d=d+"<BR>Coverage Exon 2: "+numberWithCommas(c[e].getAttribute(h))+
"-"+numberWithCommas(c[e].getAttribute(l))+" ("+(parseInt(c[e].getAttribute("stop"),10)-parseInt(c[e].getAttribute("start"),10))+" bp)";return d=d+"<BR>Intron: "+numberWithCommas(parseInt(c[1].getAttribute("start"),10)-parseInt(c[0].getAttribute("stop"),10))+" bp<BR>"};p.drawScaleLegend("1","1000+","Read Counts","#E6E6E6","#000000",0);p.draw(m);return p}
...
b.ttTrackList.push("liverIso"),b.ttTrackList.push("refSeq"),b.ttTrackList.push("ensemblnoncoding"),b.ttTrackList.push("brainnoncoding"),b.ttTrackList.push("probe"),b.ttTrackList.push("polyASite"),b.ttTrackList.push("spliceJnct"),b.ttTrackList.push("liverspliceJnct"),b.ttTrackList.push("heartspliceJnct"));b.ttTrackList.push("repeatMask");b.calculateBin=function(a){a=new String(Math.floor(a/b.gsvg.width));a=a.substr(0,1)*Math.pow(10,a.length-1);a=Math.floor(a/2);5>a&&(a=0);return a};b.color=function(a){var d=
"#FFFFFF";a.getAttribute("count")>=b.scaleMin&&(d=d3.rgb(b.colorScale(a.getAttribute("count"))));return d};b.redraw=function(){var a=b.gsvg.xScale.domain()[0],d=b.gsvg.xScale.domain()[1];b.xScale(a+b.bin);b.xScale(a);b.prevMinCoord=a;b.prevMaxCoord=d;a=b.xScale.domain()[0];d=b.xScale.domain()[1];for(var c=[],g=0,e=0,h=0;h<b.data.length;h++)if("undefined"!==typeof b.data[h]){var l=parseInt(b.data[h].getAttribute("start"),10),n=parseInt(b.data[h].getAttribute("stop"),10),q=parseInt(b.data[h].getAttribute("count"),
10);if(1!=b.density||1==b.density&&l!=n)if(h+1<b.data.length){if(n=parseInt(b.data[h+1].getAttribute("start"),10),l>=a&&l<=d||n>=a&&n<=d)c[g]=b.data[h],q>e&&(e=q),g++}else l>=a&&l<=d&&(c[g]=b.data[h],q>e&&(e=q),g++)}1==b.density?(b.density!=b.prevDensity?(b.redrawLegend(),d=b.gsvg.xScale.domain()[1],b.prevDensity=b.density,b.svg.selectAll(".area").remove(),b.svg.selectAll("g.y").remove(),b.svg.selectAll(".grid").remove(),b.svg.selectAll(".leftLbl").remove(),a=b.svg.selectAll("."+b.trackClass).data(c,
keyStart),a.each().remove(),a=b.svg.selectAll("."+b.trackClass).data(c,keyStart),a.enter().append("rect").attr("x",function(u){return b.xScale(u.getAttribute("start"))}).attr("y",15).attr("class",b.trackClass).attr("height",10).attr("width",function(u,v){return b.xScale(u.getAttribute("stop"))-b.xScale(u.getAttribute("start"))}).attr("fill",b.color).on("mouseover",function(u){0==b.gsvg.isToolTip&&(d3.select(this).style("fill","green"),tt.transition().duration(200).style("opacity",1),tt.html(b.createToolTip(u)).style("left",
function(){return b.positionTTLeft(d3.event.pageX)}).style("top",function(){return b.positionTTTop(d3.event.pageY)}))}).on("mouseout",function(u){d3.select(this).style("fill",b.color);tt.transition().delay(500).duration(200).style("opacity",0)}),a.exit().remove()):b.svg.selectAll("."+b.trackClass).attr("x",function(u){return b.xScale(u.getAttribute("start"))}).attr("width",function(u,v){return b.xScale(u.getAttribute("stop"))-b.xScale(u.getAttribute("start"))}).attr("fill",b.color),b.svg.attr("height",
...
10,0,0).tickFormat("")),b.svg.select("g.y").selectAll("text").each(function(u){var v=new String(u);d3.select(this).attr("x",function(){return 7.7*v.length+5}).attr("dy","0.05em");b.svg.append("svg:text").attr("class","leftLbl").attr("x",b.gsvg.width-(7.8*v.length+5)).attr("y",function(){return b.yScale(u)}).attr("dy","0.01em").style("font-weight","bold").text(numberWithCommas(u))}),b.svg.append("path").datum(c).attr("class","area").attr("stroke",b.graphColorText).attr("fill",b.graphColorText).attr("d",
...
b.update=function(a){a=b.xScale.domain()[0];var d=b.xScale.domain()[1];b.loadedDataMin<=a&&d<=b.loadedDataMax?b.redraw():b.updateFullData(0,0)};b.updateFullData=function(a,d){var c=b.xScale.domain()[0],g=b.xScale.domain()[1],e=g-c;b.showLoading();if(0===a||1==d)b.bin=b.calculateBin(e);e="Total";2===b.countType&&(e="Norm");var h=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/tmp/"+dataVer+"_"+c+"_"+g+".count."+b.trackClass+"."+e+".xml";0<b.bin&&(c-=2*b.bin,c-=c%(2*
b.bin),g+=2*b.bin,g+=2*b.bin-g%(2*b.bin),h=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/tmp/"+dataVer+"_"+c+"_"+g+".bincount."+b.bin+"."+b.trackClass+"."+e+".xml");d3.xml(h,function(l,n){if(l){console.log("error:"+b.bin+":"+h);if(0==a||1==d)l="/"+pathPrefix,pathPrefix||(l=""),tmpPanel=panel,-1<b.trackClass.indexOf("-")&&(tmpPanel=b.trackClass.substr(b.trackClass.indexOf("-")+1)),$.ajax({url:l+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,
minCoord:c,maxCoord:g,panel:tmpPanel,rnaDatasetID,arrayTypeID,myOrganism:organism,genomeVer,dataVer,track:b.trackClass,folder:b.gsvg.folderName,binSize:b.bin,countType:b.countType},dataType:"json",success:function(q){gtag("event","generateTrackCount",{event_category:"browser"})},error:function(q,u,v){}});8>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},1E3):30>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},2E3):38>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+
1,0)},1E4):(d3.select("#Level"+b.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass),b.hideLoading(),b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,1)},15E3))}else null==n?32<=a?(m=[],b.draw(m)):b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},5E3):(b.fullDataTimeOutHandle=
0,b.loadedDataMin=c,b.loadedDataMax=g,setTimeout(function(){var q=n.documentElement.getElementsByTagName("Count");b.draw(q)},10))})};b.updateCountScale=function(a,d){b.scaleMin=a;b.scaleMax=d;b.colorScale=d3.scaleLinear().domain([b.scaleMin,b.scaleMax]).range(["#EEEEEE","#000000"]);1==b.density&&(b.redrawLegend(),b.redraw())};b.setupToolTipSVG=function(a,d){var c=b.xScale.domain()[0],g=b.xScale.domain()[1],e=parseInt(a.getAttribute("start"),10),h=parseInt(a.getAttribute("stop"),10);c=Math.floor((g-
c)*d);20>c&&(c=20);d=e-c;c=h+c;1>d&&(d=1);"undefined"!==typeof b.ttSVGMinWidth&&c-d<b.ttSVGMinWidth&&(d=e-b.ttSVGMinWidth/2,c=h+b.ttSVGMinWidth/2);a=toolTipSVG("div#ttSVG",450,d,c,99,a.getAttribute("ID"),"transcript");a.addTrack(b.trackClass,3,"",b.data);for(e=0;e<b.ttTrackList.length;e++)if(h=b.gsvg.getTrackData(b.ttTrackList[e]),g=[],"undefined"!==typeof h&&0<h.length){for(var l=0,n=0;n<h.length;n++)if(d<=parseInt(h[n].getAttribute("start"),10)&&parseInt(h[n].getAttribute("start"),10)<=c||parseInt(h[n].getAttribute("start"),
10)<=d&&parseInt(h[n].getAttribute("stop"),10)>=d)g[l]=h[n],l++;0<g.length&&a.addTrack(b.ttTrackList[e],3,"DrawTrx",g)}};b.draw=function(a){b.hideLoading();b.data=a;var d=b.gsvg.xScale.domain()[0],c=b.gsvg.xScale.domain()[1];b.xScale(d+b.bin);b.xScale(d);b.redrawLegend();for(var g=[],e=0,h=0,l=0;l<b.data.length;l++)if("undefined"!==typeof b.data[l]){var n=parseInt(b.data[l].getAttribute("start"),10),q=parseInt(b.data[l].getAttribute("stop"),10),u=parseInt(b.data[l].getAttribute("count"),10);if(1!=
b.density||1==b.density&&n!=q)if(l+1<b.data.length){if(q=parseInt(a[l+1].getAttribute("start"),10),n>=d&&n<=c||q>=d&&q<=c)g[e]=b.data[l],u>h&&(h=u),e++}else n>=d&&n<=c&&(g[e]=b.data[l],u>h&&(h=u),e++)}ndata=g;b.svg.select(".y.axis").remove();b.svg.select("g.grid").remove();b.svg.selectAll(".leftLbl").remove();b.svg.select(".area").remove();b.svg.selectAll(".area").remove();1==b.density?(c=b.gsvg.xScale.domain()[1],b.svg.selectAll("."+b.trackClass).data(ndata,keyStart).enter().append("rect").attr("x",
...
if(d&&d.Controls&&0<d.Controls.length){var c=(new String(d.Controls)).split(",");a=d3.select(a).select("table").select("tbody");a.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+d.Name);for(d=0;d<c.length;d++)if("undefined"!==typeof c[d]&&""!=c[d]){var g=c[d].split(";"),e=a.append("tr").append("td"),h=g[0].substr(5),l="";3<g.length&&0==g[3].indexOf("Default=")&&(l=g[3].substr(8));if(0==g[1].toLowerCase().indexOf("select")){e.append("text").text(h+": ");var n=g[1].split(":");
g=g[2].split("}");h=b.trackClass+"Dense"+b.level+"Select";"colorSelect"==n[1]?h=b.trackClass+b.level+"colorSelect":"countSelect"==n[1]&&(h=b.trackClass+b.level+"countSelect");e=e.append("select").attr("id",h).attr("name",n[1]);for(n=0;n<g.length;n++){var q=g[n].substr(1).split(":");if(2==q.length){var u=e.append("option").attr("value",q[1]).text(q[0]);-1<h.indexOf("Dense")?q[1]==b.density&&u.attr("selected","selected"):q[1]==l&&u.attr("selected","selected")}}d3.select("select#"+h).on("change",function(){$(this).attr("id")==
b.trackClass+"Dense"+b.level+"Select"&&1==$(this).val()?$("#scaleControl"+b.level).show():$("#scaleControl"+b.level).hide();b.updateSettingsFromUI();b.redraw()})}else 0==g[1].toLowerCase().indexOf("slider")&&(l="none",1==b.density&&(l="inline-block"),e=e.append("div").attr("id","scaleControl"+b.level).style("display",l),e.append("text").text(h+": "),e.append("input").attr("type","text").attr("id","amount").attr("value",b.scaleMin+"-"+b.scaleMax).style("border",0).style("color","#f6931f").style("font-weight",
...
function RepeatMaskTrack(p,m,r,t,f,b){var a=GenericTranscriptTrack(p,m,r,t,f,b);a.dataFileName="repeatMask.xml";a.density=1;a.xmlTag="Feature";a.xmlTagBlockElem="Block";a.minFeatureWidth=1;a.ttSVG=1;a.ttTrackList=[];a.ttTrackList.push("ensemblcoding");a.ttTrackList.push("braincoding");a.ttTrackList.push("liverTotal");a.ttTrackList.push("heartTotal");a.ttTrackList.push("mergedTotal");a.ttTrackList.push("brainIso");a.ttTrackList.push("liverIso");a.ttTrackList.push("refSeq");a.ttTrackList.push("spliceJnct");
a.ttTrackList.push("liverspliceJnct");a.ttTrackList.push("heartspliceJnct");a.ttTrackList.push("brainspliceJnct");a.ttTrackList.push("illuminaPolyA");a.ttTrackList.push("liverilluminaTotalPlus");a.ttTrackList.push("liverilluminaTotalMinus");a.ttTrackList.push("heartilluminaTotalPlus");a.ttTrackList.push("heartilluminaTotalMinus");a.ttTrackList.push("brainilluminaTotalPlus");a.ttTrackList.push("brainilluminaTotalMinus");a.ttSVGMinWidth=200;a.legendLbl="Smith-Waterman Alignment Score";a.typeList=[];
a.typeList[0]={name:"SINE",clss:"SINE"};a.typeList[1]={name:"LINE",clss:"LINE"};a.typeList[2]={name:"LTR",clss:"LTR"};a.typeList[3]={name:"DNA",clss:"DNA"};a.typeList[4]={name:"Simple",clss:"Simple_repeat"};a.typeList[5]={name:"Low Comp.",clss:"Low_complexity"};a.typeList[6]={name:"Satellite",clss:"Satellite"};a.typeList[7]={name:"RNA",clss:"RNA"};a.typeList[8]={name:"Other",clss:"Other"};a.typeList[9]={name:"Unknown",clss:"Unknown"};a.yList={};for(p=0;p<a.typeList.length;p++)a.yList[a.typeList[p].clss]=
15+15*p;a.createToolTip=function(d){var c='<BR><div id="ttSVG" style="background:#FFFFFF;"></div><BR>Name: '+d.getAttribute("name")+"&nbsp&nbsp&nbsp&nbsp&nbspClass: "+d.getAttribute("class");c=c+"&nbsp&nbsp&nbsp&nbsp&nbspFamily: "+d.getAttribute("family")+"<BR><BR>Location: "+d.getAttribute("chromosome")+":"+d.getAttribute("start")+"-"+d.getAttribute("stop");c=c+"<BR><BR>Alignment Score: "+d.getAttribute("score")+"<BR>Mismatches(parts/thousand): "+d.getAttribute("mis")+"<BR>Insertions(parts/thousand): "+
d.getAttribute("ins");return c=c+"<BR>Deletions(parts/thousand): "+d.getAttribute("del")};a.pieColor=function(d,c){c=d3.rgb("#000000");d=d.data.names;"LINE"===d?c=d3.rgb("#27e5e5"):"SINE"===d?c=d3.rgb("#e57927"):"LTR"===d?c=d3.rgb("#27e527"):"DNA"===d?c=d3.rgb("#e52727"):"Simple_repeat"===d?c=d3.rgb("#e51ba2"):"Low_complexity"===d?c=d3.rgb("#9029e5"):"RNA"===d?c=d3.rgb("#005be5"):"Satellite"===d?c=d3.rgb("#cccc00"):"Other"===d&&(c=d3.rgb("#007f24"));return c};a.calcY=function(d,c,g,e){d=3===a.density||
"3"===a.density?a.calcYPack(d,c,g):2===a.density||"2"===a.density?a.calcYFull(e):a.calcYDense();a.trackYMax<d/15&&(a.trackYMax=d/15);return d};a.calcYFull=function(d){d=d.getAttribute("class");-1<d.indexOf("RNA")&&(d="RNA");return a.yList[d]};a.redraw=function(){if(a.prevDensity!=a.density)a.draw(a.data);else{a.yMaxArr=[];a.yArr=[];a.yArr[0]=[];for(var d=0;d<a.gsvg.width;d++)a.yMaxArr[d]=0,a.yArr[0][d]=0;a.trackYMax=0;d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+"trx"+
a.gsvg.levelNumber).attr("transform",function(c,g){return"translate("+a.xScale(c.getAttribute("start"))+","+a.calcY(parseInt(c.getAttribute("start"),10),parseInt(c.getAttribute("stop"),10),g,c)+")"}).each(function(c,g){for(var e=getAllChildrenByName(getFirstChildByName(c,a.xmlTagBlockElem+"List"),a.xmlTagBlockElem),h=0;h<e.length;h++){var l=a.idPrefix+"Ex"+e[h].getAttribute("ID");null==e[h].getAttribute("ID")&&(l=a.idPrefix+"Ex"+c.getAttribute("ID")+"_"+h);a.svg.select("g#"+a.idPrefix+"tx"+c.getAttribute("ID")+
" rect#"+l).attr("x",function(y){return a.xScale(e[h].getAttribute("start"))-a.xScale(c.getAttribute("start"))}).attr("width",function(y){return a.xScale(e[h].getAttribute("stop"))-a.xScale(e[h].getAttribute("start"))});if(0<h){l=">";"-1"==c.getAttribute("strand")&&(l="<");g=l;var n=a.xScale(e[h-1].getAttribute("stop"))-a.xScale(c.getAttribute("start")),q=a.xScale(e[h].getAttribute("start"))-a.xScale(c.getAttribute("start")),u=q-n,v=0;if(7>u)g="";else for(u-=7;8<u;)0==v?(g+=" ",v=1):(g+=l,v=0),u-=
7;l=e[h-1].getAttribute("ID")+"_"+e[h].getAttribute("ID");null==e[h].getAttribute("ID")&&(l=c.getAttribute("ID")+"_"+(h-1)+"_"+h);a.svg.select("g#"+a.idPrefix+"tx"+c.getAttribute("ID")+" line#"+a.idPrefix+"Int"+l).attr("x1",n).attr("x2",q);a.svg.select("g#"+a.idPrefix+"tx"+c.getAttribute("ID")+" #"+a.idPrefix+"IntTxt"+l).attr("dx",n+1).text(g)}}});1==a.density?a.svg.attr("height",30):2==a.density?a.svg.attr("height",15*(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+
"trx"+a.gsvg.levelNumber).size()+1)):3==a.density&&a.svg.attr("height",15*(a.trackYMax+1));a.redrawSelectedArea()}};a.drawTrx=function(d,c){c=d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).select("#"+a.idPrefix+"tx"+d.getAttribute("ID"));exList=getAllChildrenByName(getFirstChildByName(d,a.xmlTagBlockElem+"List"),a.xmlTagBlockElem);1===exList.length&&c.append("rect").attr("x",function(g){return a.xScale(exList[0].getAttribute("start"))-a.xScale(g.getAttribute("start"))}).attr("rx",1).attr("ry",
1).attr("height",10).attr("width",function(g){g=a.xScale(exList[0].getAttribute("stop"))-a.xScale(exList[0].getAttribute("start"));0<a.minFeatureWidth&&g<a.minFeatureWidth&&(g=a.minFeatureWidth);return g}).attr("id",function(g,e){e=a.idPrefix+"Ex"+exList[0].getAttribute("ID");null==exList[0].getAttribute("ID")&&(e=a.idPrefix+"Ex"+g.getAttribute("ID")+"_0");return e}).style("fill",a.color).style("cursor","pointer")};a.draw=function(d){a.data=d;a.prevDensity=a.density;a.trackYMax=0;a.yArr=[];a.yArr[0]=
[];for(var c=0;c<a.gsvg.width;c++)a.yMaxArr[c]=0,a.yArr[0][c]=0;a.svg.selectAll(".repeatLine"+a.gsvg.levelNumber).remove();a.svg.selectAll(".repeatLbl"+a.gsvg.levelNumber).remove();if(2==a.density)for(c=0;c<a.typeList.length;c++)a.svg.append("line").attr("class","repeatLine"+a.gsvg.levelNumber).attr("x1",0).attr("x2",a.gsvg.width).attr("y1",12+15*(c+1)).attr("y2",12+15*(c+1)).attr("stroke","#000000").attr("stroke-width","1px"),a.svg.append("text").attr("x",5).attr("class","repeatLbl"+a.gsvg.levelNumber).attr("y",
10+15*(c+1)).style("pointer-events","none").style("opacity","0.5").style("fill","#000000").style("font-size","11px").text(a.typeList[c].name),a.svg.append("text").attr("class","repeatLbl"+a.gsvg.levelNumber).attr("x",a.gsvg.width-5).attr("y",10+15*(c+1)).style("text-anchor","end").style("pointer-events","none").style("opacity","0.5").style("fill","#000000").style("font-size","11px").text(a.typeList[c].name);d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("."+a.idPrefix+"trx"+a.gsvg.levelNumber).remove();
a.redrawLegend();d=d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("."+a.idPrefix+"trx"+a.gsvg.levelNumber).data(d,key).attr("transform",function(g,e){return"translate("+a.xScale(g.getAttribute("start"))+","+a.calcY(parseInt(g.getAttribute("start"),10),parseInt(g.getAttribute("stop"),10),e,g)+")"});d.enter().append("g").attr("class",a.idPrefix+"trx"+a.gsvg.levelNumber).attr("transform",function(g,e){return"translate("+a.xScale(g.getAttribute("start"))+","+a.calcY(parseInt(g.getAttribute("start"),
10),parseInt(g.getAttribute("stop"),10),e,g)+")"}).attr("id",function(g){return a.idPrefix+"tx"+g.getAttribute("ID")}).style("cursor","move").on("mouseover",function(g){0==a.gsvg.isToolTip&&0!=a.trackClass.indexOf("custom")&&(d3.select(this).selectAll("line").style("stroke","green"),d3.select(this).selectAll("rect").style("fill","green"),d3.select(this).selectAll("text").style("opacity","0.3").style("fill","green"),tt.transition().duration(200).style("opacity",1),tt.html(a.createToolTip(g)).style("left",
function(){return a.positionTTLeft(d3.event.pageX)}).style("top",function(){return a.positionTTTop(d3.event.pageY)}),1==a.ttSVG&&a.setupToolTipSVG(g,.05))}).on("mouseout",function(g){d3.select(this).selectAll("line").style("stroke",a.color);d3.select(this).selectAll("rect").style("fill",a.color);d3.select(this).selectAll("text").style("opacity","0.6").style("fill",a.color);tt.transition().delay(100).duration(200).style("opacity",0)}).each(a.drawTrx);d.exit().remove();1==a.density?a.svg.attr("height",
30):2==a.density?a.svg.attr("height",15*a.typeList.length+15):3==a.density&&a.svg.attr("height",15*(a.trackYMax+2));a.redrawSelectedArea()};a.getDisplayedData=function(){var d=d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("."+a.idPrefix+"trx"+a.gsvg.levelNumber);a.counts=[];var c={};d=d[0];var g=[],e=0;if("undefined"!==typeof d){for(var h=0;h<d.length;h++){var l=a.xScale(d[h].__data__.getAttribute("start")),n=a.xScale(d[h].__data__.getAttribute("stop"));if(0<=l&&l<=a.gsvg.width||0<=
n&&n<=a.gsvg.width)"undefined"===typeof c[d[h].__data__.getAttribute("class")]?(c[d[h].__data__.getAttribute("class")]={},c[d[h].__data__.getAttribute("class")].value=1,c[d[h].__data__.getAttribute("class")].names=d[h].__data__.getAttribute("class")):c[d[h].__data__.getAttribute("class")].value=c[d[h].__data__.getAttribute("class")].value+1,g[e]=d[h].__data__,e++}a.counts=[];for(x in c)a.counts.push(c[x])}else a.counts=[];return g};a.draw(m);return a}
...
p.source="brain":-1<reportSelectedTrack.trackClass.indexOf("heart")&&(p.source="heart"),-1<reportSelectedTrack.trackClass.indexOf("liver")&&(p.source="liver"),r="web/GeneCentric/smallGeneTable.jsp"):-1<(new String(reportSelectedTrack.trackClass)).indexOf("snp")||"qtl"===reportSelectedTrack.trackClass&&(r="web/GeneCentric/bqtlTable.jsp"),r&&loadDivWithPage("div#regionTable",r,!1,p,'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>',0));gtag("event",
reportSelectedTrack.trackClass,{event_category:"loadTrackTable"})}
function loadEQTLTable(){var p=svgList[0].xScale.domain()[0],m=svgList[0].xScale.domain()[1];transcriptome=$("#transriptome").val();cisOnly=$("#cisTrans").val();pValueCutoff=$("#pvalueCutoffSelect2").val();p={species:organism,minCoord:p,maxCoord:m,chromosome:chr,rnaDatasetID,arrayTypeID,pValueCutoff,folderName:regionfolderName,genomeVer,transcriptome,cisOnly};"hrdp7.1"!=dataVer?loadDivWithPage("div#regionEQTLTable","web/GeneCentric/regionEQTLTableSeq.jsp",!1,p,'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>',
0):$("div#regionEQTLTable").css("font-size","20pt").css("color","#FF0000").html("Coming soon for HRDP v7.1.  HRDP v6(rn7) does have eQTLs if you would like to switch please change the drop down list at the top of the page.");gtag("event","eQTLTable",{event_category:"loadEQTLTable"})}
function loadRegionWGCNA(){$("div#geneWGCNA").html("");var p=svgList[0].xScale.domain()[0],m=svgList[0].xScale.domain()[1];p={region:chr+":"+p+"-"+m};"hrdp7.1"!=dataVer?loadDivWithPage("div#regionWGCNAEQTL","web/GeneCentric/wgcnaGene.jsp",!0,p,'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>',0):$("div#regionWGCNAEQTL").css("font-size","20pt").css("color","#FF0000").html("Coming soon for HRDP v7.1.  HRDP v6(rn7) does have WGCNA modules if you would like to switch please change the drop down list at the top of the page.");
...
0);gtag("event","eqtlwparams",{event_category:"loadEQTLTableWParams"})}
function loadDivWithPage(p,m,r,t,f,b){$(p).html(f);$.ajax({url:m,type:"GET",cache:!1,data:t,dataType:"html",async:!0,success:function(a){$(p).html(a);r&&setTimeout(function(){$("html, body").animate({scrollTop:$(p).offset().top},200)},300)},error:function(a,d,c){-1==m.indexOf("geneReport.jsp")||0<m.indexOf("geneReport.jsp")&&30>b?($(p).html('<span style="color:#FF0000;">An error occurred generating this page.  This can occur when loading the report the first time. Will automatically continue trying to load the report.</span>'),setTimeout(function(){loadDivWithPage(p,
m,r,t,f,b+1)},15E3)):$(p).html('<span style="color:#FF0000;">An error occurred generating this page.  This can happen initially when loading the report. This error has occurred too many times and has been reported.</span>')}})}function trKey(p){var m="";p&&(m=p.trackClass);return m}
...
"/modules/ds"+that.wDSID+"/"+a+".json",type:"GET",async:!0,data:{},dataType:"json",success:function(c){setTimeout(function(){if(0<that.singleID.length)that.countGeneInstance(that.singleID,c);else{var g=that.moduleGenes[a].split(",");c.GroupedGenes=g.length;that.countGeneInstance(that.moduleGenes[a],c)}that.modules[a]=c;that.requests--;d&&d(a)},20);gtag("event",a,{event_category:"requestModule"})},error:function(c,g,e){console.log(e);that.requests--}});that.moduleRequestList.push(xhr)};that.scanFilter=
function(a,d){var c=[];if(a)for(var g=0;g<a.length;g++)d(a[g])&&c.append(a[g]);return c};that.countGeneInstance=function(a,d){var c=d.TCList,g=0;d.selectGeneTC={};for(var e=0;e<c.length;e++)-1<a.indexOf(c[e].Gene.ID)&&g++;d.GeneCount=g};that.createImageControls=function(){that.imageBar=d3.select("#imageControl").append("span").style("float","left");that.imageBar.append("span").attr("class","saveImage control").style("display","inline-block").attr("id","saveImage").style("cursor","pointer").append("img").attr("src",
...
a=a.replace('width="100%"','width="'+d+'px"');$.ajax({url:pathPrefix+"saveBrowserImage.jsp",type:"POST",contentType:"text/html",data:a+"\n",processData:!1,dataType:"json",success:function(c){var g=new Date;g=g.getMonth()+1+"_"+g.getDate()+"_"+g.getFullYear();var e="http://";"https:"===location.protocol&&(e="https://");var h=e+urlprefix+"/tmpData/download/"+c.imageFile;c="";that.selectedModule&&(c=new String(that.selectedModule.MOD_NAME));c=c+"_"+that.viewType;var l=c+"_"+g+".png",n=new XMLHttpRequest;
n.open("GET",h);n.responseType="blob";n.send();n.onreadystatechange=function(){if(4!=n.readyState||200!=n.status)return!1;var q=document.createElement("a");q.href=window.URL.createObjectURL(n.response);q.download=l;q.style.display="none";document.body.appendChild(q);try{q.click()}catch(u){$("#wgcnaMouseHelp").html("<span style='color:#FF0000;'>Your browser will not save the image directly. Image will open in a popup, in the new window right click to save image.</span>"),window.open(h)}return!0}},
error:function(c,g,e){console.log(e)}});gtag("event","saveWGCNAView",{event_category:"saveWGCNAView"})}}).on("mouseover",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/savePic_white.png");$("#wgcnaMouseHelp").html("Click to download a PNG image of the current view.")}).on("mouseout",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/savePic_dark.png");$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")});that.imageBar.append("span").attr("class",
...
gtag("event","changeDomain",{event_category:"WGCNA_go"})});d.append("option").attr("value","cc").text("Cellular Component");d.append("option").attr("value","bp").text("Biological Process");d.append("option").attr("selected","selected").attr("value","mf").text("Molecular Function");c.append("br");c.append("text").text("Number of Levels to Display:");c.append("input").attr("name","value").attr("id","goSpinner");$("#goSpinner").spinner({min:1,max:100,step:1,change:function(h,l){that.singleImage.displayDepth=
l.value;that.singleImage.selectedNode!==that.singleImage.data.GOList[0]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[1]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[2]?that.singleImage.redraw():that.singleImage.draw();gtag("event","changeDepth",{event_category:"WGCNA_go"})},spin:function(h,l){that.singleImage.displayDepth=l.value;that.singleImage.selectedNode!==that.singleImage.data.GOList[0]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[1]&&that.singleImage.selectedNode!==
...
slide:function(h,l){that.singleImage.CorCutoff_min=l.value;.002>=that.singleImage.CorCutoff_min&&(that.singleImage.CorCutoff_min=0);that.singleImage.CorCutoff_min*=1;$("span#minLabel").html(l.value);setTimeout(function(){that.singleImage.filterLinks();that.singleImage.draw()},10)}});$("#linkSliderMax").slider({min:.01,max:.5,step:.01,value:.5,slide:function(h,l){tmptext=that.singleImage.CorCutoff_max=l.value;.49<=that.singleImage.CorCutoff_max&&(that.singleImage.CorCutoff_max=1,tmptext="0.5+");that.singleImage.CorCutoff_max*=
...
"3").text("0.001");d.append("option").attr("value","4").text("0.0001");d.append("option").attr("value","5").text("0.00001");c.append("br");c.append("br");c.append("br");c.append("br");c.append("input").attr("type","button").attr("value","Apply Selections").on("click",function(){for(var h=$("#eqtlPval").val(),l=$("#eqtlChr").val(),n="",q=0;q<l.length;q++)n=n+l[q]+";";$.ajax({url:pathPrefix+"runCircosModule.jsp",type:"GET",async:!0,data:{cutoffValue:h,organism,module:that.selectedModule.MOD_NAME,modColor:that.selectedModule.ModRGB,
chrList:n,genomeVer,tissue:that.tissue},dataType:"json",beforeSend:function(){$("#waitCircos").show()},success:function(u){$("#waitCircos").hide();that.singleImage.update(u.URL)},error:function(u,v,y){}})})};that.positionTTLeft=function(a){a+=20;a>$(window).width()/2&&(a-=250);return a+"px"};that.positionTTTop=function(a){a+="px";d3.event.clientY>.6*window.innerHeight&&(a=d3.event.pageY-2.2*$(".testToolTip").height()+"px");return a};that.createMultiWGCNAImage=function(){that.multiImage=that.multiWGCNAImageGeneView();
that.multiImage.draw();that.img=that.multiImage};that.multiWGCNAImage=function(){var a={maxWidth:200};a.totalWidth=$(window).width();a.modPerLine=Math.floor($(window).width()/a.maxWidth);a.calcWidth=Math.floor($(window).width()/a.modPerLine);a.data=[];if(0===d3.select("#multiWGCNASVG").size()){var d=d3.select("#wgcnaGeneImage").append("div").attr("id","multiWGCNAScroll").style("overflow","auto").style("max-height","430px");a.svg=d.append("svg").attr("id","multiWGCNASVG").attr("height","430px").attr("width",
"100%");d=a.svg.append("linearGradient").attr("id","lgrad").attr("x1","11%").attr("y1","0%").attr("x2","89%").attr("y2","100%");d.append("stop").attr("offset","0%").attr("style","stop-color:rgb(153,218,255);stop-opacity:1");d.append("stop").attr("offset","100%").attr("style","stop-color:rgb(0,128,128);stop-opacity:1");d=a.svg.append("linearGradient").attr("id","selectGrad").attr("x1","11%").attr("y1","0%").attr("x2","89%").attr("y2","100%");d.append("stop").attr("offset","0%").attr("style","stop-color:rgb(0,163,55);stop-opacity:1");
d.append("stop").attr("offset","100%").attr("style","stop-color:rgb(0,75,20);stop-opacity:1")}else a.svg=d3.select("#multiWGCNASVG");a.modKey=function(c){return c.MOD_NAME};a.calcX=function(c,g){return g%a.modPerLine*a.calcWidth};a.calcY=function(c,g){return Math.floor(g/a.modPerLine)*a.maxWidth};a.sizing=function(c){c=a.size(c.TCList.length);c>a.maxWidth/2&&(c=a.maxWidth/2);return c};a.zoomIn=function(){a.maxWidth<$(window).width()/2&&(a.maxWidth*=2,a.draw())};a.zoomOut=function(){var c=a.maxWidth;
a.maxWidth=Math.floor(a.maxWidth/2);25>a.maxWidth?a.maxWidth=c:a.draw()};a.sortTCLenDesc=function(c,g){return g.TCList.length<c.TCList.length?-1:g.TCList.length>c.TCList.length?1:0};a.sortGeneInstanceDesc=function(c,g){return g.GeneCount<c.GeneCount?-1:g.GeneCount>c.GeneCount?1:a.sortTCLenDesc(c,g)};a.draw=function(){};a.redraw=function(){};return a};that.multiWGCNAImageGeneView=function(){var a=that.multiWGCNAImage();a.createToolTip=function(d){var c="Module Name: "+d.MOD_NAME+"<BR>Transcript Size: "+
d.TCList.length;"undefined"!==typeof d.GroupedGenes?(c=c+"<BR>Genes in Region/List: "+d.GroupedGenes,c=c+"<BR>Transcripts in Region/List: "+d.GeneCount):c=c+"<BR>Selected Gene Transcripts in Module: "+d.GeneCount;return c};a.draw=function(){"undefined"!==typeof that.selectedModule&&(a.maxWidth=85);a.svg.selectAll(".modules").remove();a.svg.selectAll(".moduleGradient").remove();a.totalWidth=$(window).width();a.modPerLine=Math.floor($(window).width()/a.maxWidth);a.calcWidth=Math.floor($(window).width()/
a.modPerLine);var d=[];a.dataList={};for(var c=0,g=0;g<that.moduleList.length;g++)if(("grey"!==that.moduleList[g]||"grey"===that.moduleList[g]&&0===that.skipGrey)&&"undefined"!==typeof that.modules[that.moduleList[g]]){d[c]=that.modules[that.moduleList[g]];var e=d[c].ModRGB.split(","),h="";for(h=0;h<e.length;h++)e[h]-=70,0>e[h]&&(e[h]=0);h=e[0]+","+e[1]+","+e[2];e=a.svg.append("linearGradient").attr("id","grad"+d[c].MOD_NAME).attr("class","moduleGradient").attr("x1","11%").attr("y1","0%").attr("x2",
"89%").attr("y2","100%");e.append("stop").attr("offset","0%").attr("style","stop-color:rgb("+d[c].ModRGB+");stop-opacity:1");e.append("stop").attr("offset","100%").attr("style","stop-color:rgb("+h+");stop-opacity:1");c++}c=a.maxWidth+15;d.length>a.modPerLine&&(g=Math.floor(d.length/a.modPerLine),0<d.length%a.modPerLine&&g++,c*=g);a.svg.attr("height",c+"px");d.sort(a.sortGeneInstanceDesc);a.data=d;for(g=0;g<d.length;g++)a.dataList[d[g].MOD_NAME]=g;c=d3.min(d,function(l){return l.TCList.length});g=
d3.max(d,function(l){return l.TCList.length});a.size=d3.scaleLinear().domain([c,g]).range([10,a.maxWidth/2]);d=a.svg.selectAll(".modules").data(d,a.modKey).attr("transform",function(l,n){return"translate("+a.calcX(l,n)+","+a.calcY(l,n)+")"});d.enter().append("g").attr("class","modules").attr("id",function(l){return l.MOD_NAME}).attr("transform",function(l,n){return"translate("+a.calcX(l,n)+","+a.calcY(l,n)+")"}).merge(d).each(function(l){var n=d3.select("g#"+l.MOD_NAME);n.append("circle").attr("cx",
a.calcWidth/2).attr("cy",a.maxWidth/2).attr("r",a.sizing).attr("stroke",function(q){var u="#FFFFFF";"undefined"!==typeof that.selectedModule&&that.selectedModule===q&&(u="#000000");return u}).attr("stroke-width",function(q){var u="1px";"undefined"!==typeof that.selectedModule&&that.selectedModule===q&&(u="3px");return u}).attr("fill",function(q){var u="url(#grad"+q.MOD_NAME+")";"undefined"!==typeof that.selectedModule&&that.selectedModule===q&&(u="url(#selectGrad)");return u}).on("click",function(){a.maxWidth=
...
0;a.maxCorWidth=15;a.filterLinksGene=function(g){for(var e=[],h=0,l=0;l<that.selectedModule.LinkList.length;l++)if(that.selectedModule.LinkList[l].TC1==g.ID||that.selectedModule.LinkList[l].TC2==g.ID)e[h]=that.selectedModule.LinkList[l],h++;that.selectedModule.filteredLinks=e};a.filterLinks=function(){console.log("filterLinks:"+a.CorCutoff_min+"_"+a.CorCutoff_max);for(var g=[],e=0,h=0;h<that.selectedModule.LinkList.length;h++)if(that.selectedGene[that.selectedModule.LinkList[h].TC1]&&that.selectedGene[that.selectedModule.LinkList[h].TC2]){console.log("found:");
var l=Math.abs(that.selectedModule.LinkList[h].Cor);console.log("cor:"+l);a.CorCutoff_min<=l&&l<=a.CorCutoff_max&&(console.log("found:"+e),g[e]=that.selectedModule.LinkList[h],e++)}that.selectedModule.filteredLinks=g};a.drawLinks=function(g){console.log("draw links");"gene"===that.viewType?a.updateColor("linkColor"):"mir"===that.viewType&&a.updateColor("mirlinkColor");1E4>that.selectedModule.filteredLinks.length&&(console.log("filteredLen:"+that.selectedModule.filteredLinks.length),g=a.topG.selectAll(".link").data(that.selectedModule.filteredLinks,
a.linkKey),g.enter().append("line").attr("class","link").attr("pointer-events","none").attr("id",function(e){return"ln_"+that.removeInvalidIDChars(e.TC1)+that.removeInvalidIDChars(e.TC2)}).attr("x1",function(e){e=d3.select("#tc_"+that.removeInvalidIDChars(e.TC1));var h=0;e&&(h=e.attr("cx"));return h}).attr("y1",function(e){e=d3.select("#tc_"+that.removeInvalidIDChars(e.TC1));var h=0;e&&(h=e.attr("cy"));return h}).attr("x2",function(e){e=d3.select("#tc_"+that.removeInvalidIDChars(e.TC2));var h=0;e&&
(h=e.attr("cx"));return h}).attr("y2",function(e){e=d3.select("#tc_"+that.removeInvalidIDChars(e.TC2));var h=0;e&&(h=e.attr("cy"));return h}).attr("stroke",a.colorLink).attr("stroke-width",function(e){return Math.abs(e.Cor)*a.maxCorWidth}).merge(g),g.exit().remove())};a.drawLinksBetween=function(g){for(var e={},h=0;h<g.length;h++)e[g[h]]=1;"gene"===that.viewType?a.updateColor("linkColor"):"mir"===that.viewType&&a.updateColor("mirlinkColor");a.svg.selectAll(".link").remove();g=that.selectedModule.filteredLinks;
h=[];for(var l=0,n=0;n<g.length;n++)if(0===a.showAllFromTarget&&"undefined"!==typeof e[g[n].TC1]&&"undefined"!==typeof e[g[n].TC2]||1===a.showAllFromTarget&&("undefined"!==typeof e[g[n].TC1]||"undefined"!==typeof e[g[n].TC2]))h[l]=g[n],l++;g=h;1E4>g.length&&(e=a.svg.select("g.svg-pan-zoom_viewport").selectAll(".link").data(g,a.linkKey),e.enter().append("line").attr("class","link").attr("pointer-events","none").attr("id",function(q){return"ln_"+that.removeInvalidIDChars(q.TC1)+that.removeInvalidIDChars(q.TC2)}).attr("x1",
function(q){return d3.select("#tc_"+that.removeInvalidIDChars(q.TC1)).attr("cx")}).attr("y1",function(q){return d3.select("#tc_"+that.removeInvalidIDChars(q.TC1)).attr("cy")}).attr("x2",function(q){return d3.select("#tc_"+that.removeInvalidIDChars(q.TC2)).attr("cx")}).attr("y2",function(q){return d3.select("#tc_"+that.removeInvalidIDChars(q.TC2)).attr("cy")}).attr("stroke",a.colorLink).attr("stroke-width",function(q){return Math.abs(q.Cor)*a.maxCorWidth}).merge(e))};a.nodeX=function(g,e){g=50;var h=
Math.floor(e/a.maxPerLevel)+1;1<h&&(g=60*h);return a.width/2+(a.r-g)*Math.cos(e%a.maxPerLevel*a.angle-1.57079633)};a.nodeY=function(g,e){g=50;e>a.maxPerLevel&&(g=60*(Math.floor(e/a.maxPerLevel)+1));return a.height/2+(a.r-g)*Math.sin(e%a.maxPerLevel*a.angle-1.57079633)};a.nodeXLbl=function(g,e){e=e%a.maxPerLevel*a.angle-1.57079633;var h=1;if(Math.PI/2<e&&e<1.5*Math.PI){var l=0;"undefined"!==typeof g.Gene.geneSymbol&&(l=g.Gene.geneSymbol.length);h+=7.5*l}return a.width/2+(a.r+h)*Math.cos(e)};a.nodeYLbl=
function(g,e){e=e%a.maxPerLevel*a.angle-1.57079633;var h=1;if(Math.PI/2<e&&e<1.5*Math.PI){var l=0;"undefined"!==typeof g.Gene.geneSymbol&&(l=g.Gene.geneSymbol.length);h+=7.5*l}return a.height/2+(a.r+h)*Math.sin(e)};a.drawCorLegend=function(g){a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",g).text("Positive Correlation");a.legendDispG.append("rect").attr("x","0").attr("y",g-10).attr("width","30").attr("height","10").style("fill",a.posColor);a.legendDispG.append("text").style("font-size",
"16").attr("x","35").attr("y",g+20).text("Negative Correlation");a.legendDispG.append("rect").attr("x","0").attr("y",g+10).attr("width","30").attr("height","10").style("fill",a.negColor);a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",g+40).text("Lower Correlation");a.legendDispG.append("line").attr("x1","0").attr("y1",g+30).attr("x2","30").attr("y2",g+30).style("stroke","#000000").style("stroke-width","0");a.legendDispG.append("line").attr("x1","0").attr("y1",g+35).attr("x2",
"30").attr("y2",g+35).style("stroke","#000000").style("stroke-width","0.5");a.legendDispG.append("line").attr("x1","0").attr("y1",g+40).attr("x2","30").attr("y2",g+40).style("stroke","#000000").style("stroke-width","1");a.legendDispG.append("line").attr("x1","0").attr("y1",g+45).attr("x2","30").attr("y2",g+45).style("stroke","#000000").style("stroke-width","2");a.legendDispG.append("line").attr("x1","0").attr("y1",g+55).attr("x2","30").attr("y2",g+55).style("stroke","#000000").style("stroke-width",
"3");a.legendDispG.append("line").attr("x1","0").attr("y1",g+65).attr("x2","30").attr("y2",g+65).style("stroke","#000000").style("stroke-width","5");a.legendDispG.append("line").attr("x1","0").attr("y1",g+75).attr("x2","30").attr("y2",g+75).style("stroke","#000000").style("stroke-width","7");a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",g+80).text("Higher Correlation")};a.drawMirSupportLegend=function(g){a.legendDispG.append("text").style("font-size","16").attr("x",
"35").attr("y",g).text("Validated miRNA Target");a.legendDispG.append("rect").attr("x","0").attr("y",g-10).attr("width","30").attr("height","10").style("fill",a.mirVal);a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",g+20).text("Predicted miRNA Target");a.legendDispG.append("rect").attr("x","0").attr("y",g+10).attr("width","30").attr("height","10").style("fill",a.mirPred);a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",g+40).text("Lower support of targeting");
...
"all").style("cursor","pointer").on("mouseover",function(){$("#wgcnaMouseHelp").html("Warning: Transcripts displayed are truncated to the top 150 most connected transcripts due to the size of the module.")}).on("mouseout",function(){$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")})}else a.maxNodeR=28;a.topG=a.svg.append("g").attr("class","svg-pan-zoom_viewport");a.topG.selectAll(".geneTC").remove();g=a.topG.selectAll(".geneTC").data(a.data,
a.geneKey);g.enter().append("g").append("circle").attr("cx",a.nodeX).attr("cy",a.nodeY).attr("r",a.nodeRadiusSum).attr("stroke","#000000").attr("fill",a.colorCircle).attr("class",function(n){return"geneTC "+n.Gene.ID}).attr("id",function(n){return"tc_"+that.removeInvalidIDChars(n.ID)}).on("click",function(){}).on("mouseover",a.mouseoverGene).on("mouseout",a.mouseleaveGene).merge(g).each(function(n,q){d3.select(this.parentNode).append("g").attr("class","label").attr("id",function(u){return"txt_"+that.removeInvalidIDChars(u.ID)}).attr("transform",
function(){var u=a.nodeXLbl(n,q),v=a.nodeYLbl(n,q);return"translate("+u+","+v+")"}).append("text").attr("transform",function(){var u=-90+q%a.maxPerLevel*a.angleDeg;90<u&&(u-=180);return"rotate("+u+")"}).text(function(){return n.Gene.geneSymbol})});g.exit().remove();g=that.selectedModule.filteredLinks;e=[];for(var h=0,l=0;l<g.length;l++)if(0===a.truncated||"undefined"!==typeof a.dataIndex[g[l].TC1]&&"undefined"!==typeof a.dataIndex[g[l].TC2])e[h]=g[l],h++;console.log("link count:"+h);console.log("filtered:");
...
function(){return that.positionTTTop(d3.event.pageY)});var g=d3.select(this.parentNode).datum();"mir"===that.viewType?(a.mirTargetID=g.Gene.ID.toLowerCase(),that.singleImage.drawMiR(),a.svg.selectAll(".link").remove(),a.showLinks||a.drawLinks(g)):($("[id^=ln_"+that.removeInvalidIDChars(g.ID)+"]").each(function(){var n=$(this).attr("id");d3.select("line#"+n).attr("stroke",a.colorLinkHighlight)}),$("[id$="+that.removeInvalidIDChars(g.ID)+"]").each(function(){var n=$(this).attr("id");d3.select("line#"+
n).attr("stroke",a.colorLinkHighlight)}),a.showLinks||(a.filterLinksGene(g),a.drawLinks()));d3.select("#txt_"+that.removeInvalidIDChars(g.ID)).select("text").attr("fill","#00A347");for(var e=that.selectedGene[g.Gene.ID].tcList,h=0;h<e.length;h++)d3.select("#tc_"+that.removeInvalidIDChars(e[h].ID)).attr("fill","#FEFF49");if("mir"===that.viewType&&a.showMirLinks){var l=g.Gene.ID;g=a.geneMirList[g.Gene.ID];g=a.filterRevMiR(g);g=a.svg.select("g.svg-pan-zoom_viewport").selectAll(".mirlink").data(g,that.mirKey);
g.enter().append("line").attr("class","mirlink").attr("pointer-events","none").attr("id",function(n){return"ln_"+n.ID}).attr("x1",function(n){var q=d3.select("."+l).attr("cx");0<d3.select("#"+that.removeInvalidIDChars(n.ID)).size()&&(q=d3.select("#"+that.removeInvalidIDChars(n.ID))[0][0].__data__.x+a.width/2-a.miRdiameter/2);return q}).attr("y1",function(n){var q=d3.select("."+l).attr("cy");0<d3.select("#"+that.removeInvalidIDChars(n.ID)).size()&&(q=d3.select("#"+that.removeInvalidIDChars(n.ID))[0][0].__data__.y+
a.height/2-a.miRdiameter/2);return q}).attr("x2",function(n){return d3.select("."+l).attr("cx")}).attr("y2",function(n){return d3.select("."+l).attr("cy")}).attr("stroke",function(n){var q=a.mirPred;0<n.V&&(q=a.mirVal);return q}).attr("stroke-width",function(n){return n.P+3*n.V}).merge(g)}gtag("event","mouseOverGene",{event_category:"wgcna"})};a.mouseleaveGene=function(){var g=d3.select(this.parentNode).datum();$("[id^=ln_"+that.removeInvalidIDChars(g.ID)+"]").each(function(){var e=$(this).attr("id");
d3.select("line#"+e).attr("stroke",a.colorLink)});$("[id$="+that.removeInvalidIDChars(g.ID)+"]").each(function(){var e=$(this).attr("id");d3.select("line#"+e).attr("stroke",a.colorLink)});d3.select(this).style("fill",a.colorCircle);d3.select("#txt_"+that.removeInvalidIDChars(g.ID)).select("text").attr("fill","#000000");tt.transition().duration(500).style("opacity",0);a.showLinks||a.svg.selectAll(".link").remove();"mir"===that.viewType&&(a.mirTargetID=$("#mirGeneFilterTxt").val().toLowerCase(),a.svg.selectAll(".mirLink").remove(),
...
d,c,g){a=a.replace(/(<.*?>)*/g,"");a=a.replace(/\s*/g,"");if(3===c||4===c)a=a.replace(/&nbsp;/g," ");return a}},columns:[0,1,2,3,4]}}):($("div#waitModuleTable").hide(),$("div#wgcnaEqtlTable").hide(),$("div#wgcnaModuleTable").hide(),$("div#wgcnaGoTable").hide())};that.singleWGCNATableMirView=function(){var a={orgPref:"mmu-",runMirFilter:function(g){for(var e=[],h=0,l=0;l<g.length;l++)if(0<g[l].filterGLSize||0<g[l].vC)e[h]=g[l],h++;return e},createMirTable:function(){$("div#waitMirTable").hide();$("div#wgcnaMirGeneTable").hide();
$("div#wgcnaMirTable").show();$("span#mirTableName").html(that.selectedModule.MOD_NAME);$.fn.DataTable.isDataTable("table#mirTable")&&$("table#mirTable").DataTable().destroy();d3.select("table#mirTable").select("tbody").selectAll("tr").remove();var g=a.runMirFilter(that.singleImage.mirData.MIRList);d3.select("table#mirTable").select("tbody").selectAll("tr").data(g,that.mirKey).enter().append("tr").attr("id",function(e){return"mirTR"+that.removeInvalidIDChars(e.ID)}).attr("class",function(e,h){e="odd";
0===h%2&&(e="even");return e}).each(function(e,h){for(var l=0,n=[],q=0,u=[],v=e.GeneList,y=0;y<v.length;y++)0<v[y].V?(l++,n.push(v[y])):1<v[y].P&&(q++,u.push(v[y]));d3.select(this).append("td").html(e.ID);d3.select(this).append("td").html(e.ACC);d3.select(this).append("td").html(function(){if(0<q){var w='<span class="triggerpL" name="pl'+h+'">'+q+"</span><BR>";w=w+'<span id="pl'+h+'" style="display:none;" >';for(var C=0;C<u.length;C++){0<C&&(w+=", ");var B=u[C].ID;"undefined"!==typeof that.singleImage.geneList[B]&&
...
h+'">'+w+"</span><BR>";w=w+'<span id="mil'+h+'" style="display:none;" >';if(0<n.length){w+="Validated: ";for(var C=0;C<n.length;C++){0<C&&(w+=", ");var B=n[C].ID;"undefined"!==typeof that.singleImage.geneList[B]&&(B=that.singleImage.geneList[B]);w+=B}}if(0<u.length)for(0<n.length&&(w+="<BR><BR>"),w+="Predicted: ",C=0;C<u.length;C++)0<C&&(w+=", "),B=u[C].ID,"undefined"!==typeof that.singleImage.geneList[B]&&(B=that.singleImage.geneList[B]),w+=B;w+="</span>"}return w})});$("#mirTable .triggerMiL").on("click",
function(e){var h=this.getAttribute("name");$("#"+h).is(":hidden")?($(this).addClass("less"),$("span#"+h).show()):($(this).removeClass("less"),$("span#"+h).hide());e.stopPropagation()});$("#mirTable .triggerpL").on("click",function(e){var h=this.getAttribute("name");$("#"+h).is(":hidden")?($(this).addClass("less"),$("span#"+h).show()):($(this).removeClass("less"),$("span#"+h).hide());e.stopPropagation()});$("#mirTable .triggervL").on("click",function(e){var h=this.getAttribute("name");$("#"+h).is(":hidden")?
($(this).addClass("less"),$("span#"+h).show()):($(this).removeClass("less"),$("span#"+h).hide());e.stopPropagation()});that.datatbl=$("table#mirTable").DataTable({bPaginate:!1,aaSorting:[[4,"desc"]],sDom:'<"leftSearch"B><"centerSearch"f><"rightTable"i><t>',buttons:[$.extend(!0,{},that.buttonCommon,{extend:"copyHtml5"}),$.extend(!0,{},that.buttonCommon,{extend:"csvHtml5"}),$.extend(!0,{},that.buttonCommon,{extend:"excelHtml5"})]});that.PhenogenList=PhenoGenGeneList(that.datatbl,4,that.buttonCommon);
that.buttonCommon={exportOptions:{format:{body:function(e,h,l,n){e=e.replace(/(<.*?>)*/g,"");0!==l&&3!==l?e=e.replace(/\s*/g,""):3===l?e=e.replace(/&nbsp;/g," "):0===l&&(e=e.replace(/PRN/g,";PRN"),e=e.replace(/ENS/g,";ENS"),e=e.substring(1));0===e.indexOf("ENS")&&0<e.indexOf("AllOrganisms:")&&(e=e.substring(0,e.indexOf("AllOrganisms:")));if(7===l||8===l||9===l||11===l||14===l||16===l||19===l||21===l||24===l||26===l)e="\u200c"+e;else if(12===l||13===l||17===l||18===l||22===l||23===l||27===l||28===
...
function(g){return"mirg"+that.removeInvalidIDChars(g.ID)}).attr("class",function(g,e){g="odd";0===e%2&&(g="even");return g}).each(function(g,e){var h=that.singleImage.geneMirList[g.Gene.ID],l=0,n=[],q=0,u=[];if("undefined"!==typeof h)for(var v=0;v<h.length;v++)for(var y=h[v].GeneList,w=0,C=0;C<y.length&&0===w;C++)y[C].ID===g.Gene.ID&&(0<y[C].V?(l++,n.push(h[v])):1<y[C].P&&(q++,u.push(h[v])),w=1);d3.select(this).append("td").html(g.Gene.geneSymbol);d3.select(this).append("td").html(g.Gene.ID);d3.select(this).append("td").html(function(B,
E){if(0<q){B='<span class="triggerpL" name="pl'+e+'">'+q+"</span><BR>";B=B+'<span id="pl'+e+'" style="display:none;" >';for(E=0;E<u.length;E++){0<E&&(B+=", ");var G=u[E].ID.replace(a.orgPref,"");B+=G}B+="</span>"}else B=q;return B});d3.select(this).append("td").html(function(B,E){if(0<l){B='<span class="triggervL" name="vl'+e+'">'+l+"</span><BR>";B=B+'<span id="vl'+e+'" style="display:none;" >';for(E=0;E<n.length;E++){0<E&&(B+=", ");var G=n[E].ID.replace(a.orgPref,"");B+=G}B+="</span>"}else B=l;return B});
d3.select(this).append("td").html(function(B,E){B=l+q;if(0<B){B='<span class="triggerMiL" name="mil'+e+'">'+B+"</span><BR>";B=B+'<span id="mil'+e+'" style="display:none;" >';if(0<n.length)for(B+="Validated: ",E=0;E<n.length;E++){0<E&&(B+=", ");var G=n[E].ID.replace(a.orgPref,"");B+=G}if(0<u.length)for(0<n.length&&(B+="<BR><BR>"),B+="Predicted: ",E=0;E<u.length;E++)0<E&&(B+=", "),G=u[E].ID.replace(a.orgPref,""),B+=G;B+="</span>"}return B})});$("#mirGeneTable .triggerMiL").on("click",function(g){var e=
this.getAttribute("name");$("#"+e).is(":hidden")?($(this).addClass("less"),$("span#"+e).show()):($(this).removeClass("less"),$("span#"+e).hide());g.stopPropagation()});$("#mirGeneTable .triggerpL").on("click",function(g){var e=this.getAttribute("name");$("#"+e).is(":hidden")?($(this).addClass("less"),$("span#"+e).show()):($(this).removeClass("less"),$("span#"+e).hide());g.stopPropagation()});$("#mirGeneTable .triggervL").on("click",function(g){var e=this.getAttribute("name");$("#"+e).is(":hidden")?
($(this).addClass("less"),$("span#"+e).show()):($(this).removeClass("less"),$("span#"+e).hide());g.stopPropagation()});$.fn.DataTable.isDataTable("table#mirGeneTable")&&$("table#mirGeneTable").DataTable().destroy();$("table#mirGeneTable").DataTable({bPaginate:!1,aaSorting:[[4,"desc"]],sDom:'<"rightTable"i><"leftTable"fB><t>',buttons:["copy","excel","pdf"]})}};if("undefined"!==typeof that.selectedModule){$("div#wgcnaEqtlTable").hide();$("div#wgcnaModuleTable").hide();$("div#wgcnaGoTable").hide();d3.select("div#tableExportCtl").selectAll("span").remove();
...
"button").style("margin-right","10px").on("click",function(){$("#eqtlTable").tableExport({type:"csv",escape:"false"})}).html("Export CSV");$.ajax({url:contextRoot+"tmpData/browserCache/"+genomeVer+"/modules/ds"+that.wDSID+"/"+replaceUnderscore(that.selectedModule.MOD_NAME)+".eQTL.json",type:"GET",async:!0,data:{},dataType:"json",beforeSend:function(){$("div#waitEqtlTable").show();setTimeout(function(){$.fn.DataTable.isDataTable("table#eqtlTable")&&$("table#eqtlTable").DataTable().destroy()},10)},
success:function(a){setTimeout(function(){for(var d=[],c=1*$("#eqtlPval").val(),g="",e=0,h=0;h<a.length;h++)a[h].Pval>=c&&(a[h].Pval>e&&(g=a[h].Snp,e=a[h].Pval),d.push(a[h]));d3.select("#eqtlTable").select("thead").selectAll("tr").remove();d3.select("#eqtlTable").select("tbody").selectAll("tr").remove();headerRow=d3.select("#eqtlTable").select("thead").append("tr");headerRow.append("th").html("Chromosome");headerRow.append("th").html("Position(Mbp)");headerRow.append("th").html("SNP ID");headerRow.append("th").html("-log (P-Value)");
d3.select("#eqtlTable").select("tbody").selectAll("tr").data(d,that.eQTLKey).enter().append("tr").attr("id",function(l){return"tblLink_"+l.Snp}).attr("class",function(l,n){l="odd";0===n%2&&(l="even");return l}).each(function(l,n){n=l.Start/1E6;d3.select(this).append("td").html(l.Chr);d3.select(this).append("td").html(n.toFixed(1));d3.select(this).append("td").html(l.Snp);d3.select(this).append("td").html(l.Pval.toFixed(2))});$.fn.DataTable.isDataTable("table#eqtlTable")||$("#eqtlTable").DataTable({bPaginate:!1,
aaSorting:[[3,"desc"]],sDom:"fiB<t>",buttons:["copy","excel","pdf"]});$("div#waitEqtlTable").hide();d3.select("#circosModule").select("g#Link_"+g).select("path").style("stroke","#FFF450");$("table#eqtlTable").DataTable().draw()},50)},error:function(a,d,c){}})};that.singleWGCNAImageMetaView=function(){var a=that.singleWGCNAImage();$(".goCTL").hide();$(".eqtlCtls").hide();$(".linkCTL").show();$(".mirCTL").hide();a.CorCutoff_min=.01;a.CorCutoff_max=1;a.type=that.viewType;a.truncated=0;a.filterOneDB=
1;a.mirGeneCutoff=1;a.mirValid=0;a.mirTargetID="";a.mirID="";a.miRcolor=d3.scaleOrdinal(d3.schemeCategory20b);a.sortMir=0;a.maxCorWidth=5;a.mlinkKey=function(d){return that.removeInvalidIDChars(d.MOD1)+"_"+that.removeInvalidIDChars(d.MOD2)};a.colorCircle=function(d){return d.COLOR};a.colorLink=function(d){var c=a.negColor;0<d.COR&&(c=a.posColor);return c};a.colorLinkHighlight=function(d){var c=a.negDarkColor;0<d.COR&&(c=a.posDarkColor);return c};a.drawLinks=function(d){a.updateColor("linkColor");
a.svg.selectAll(".link").remove();for(var c=a.metaData.LINKS,g=[],e=0,h=0;h<c.length;h++)(c[h].MOD1===d.MODULE||c[h].MOD2===d.MODULE)&&a.CorCutoff_min<=Math.abs(c[h].COR)&&Math.abs(c[h].COR)<=a.CorCutoff_max&&(g[e]=c[h],e++);c=g;1E4>c.length&&(d=a.topG.selectAll(".link").data(c,a.mlinkKey),d.enter().append("line").attr("class","link").attr("pointer-events","none").attr("id",function(l){return"ln_"+that.removeInvalidIDChars(l.MOD1)+that.removeInvalidIDChars(l.MOD2)}).attr("x1",function(l){return d3.select("#mod_"+
that.removeInvalidIDChars(l.MOD1)).attr("cx")}).attr("y1",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD1)).attr("cy")}).attr("x2",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD2)).attr("cx")}).attr("y2",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD2)).attr("cy")}).attr("stroke",a.colorLink).attr("stroke-width",function(l){return Math.abs(l.COR)*a.maxCorWidth}).merge(d))};a.drawLinksBetween=function(d){a.updateColor("linkColor");for(var c=
{},g=0;g<d.length;g++)c[d[g]]=1;a.svg.selectAll(".link").remove();d=a.metaData.LINKS;g=[];for(var e=0,h=0;h<d.length;h++)(0===a.showAllFromTarget&&"undefined"!==typeof c[d[h].MOD1]&&"undefined"!==typeof c[d[h].MOD2]||1===a.showAllFromTarget&&("undefined"!==typeof c[d[h].MOD1]||"undefined"!==typeof c[d[h].MOD2]))&&a.CorCutoff_min<=Math.abs(d[h].COR)&&Math.abs(d[h].COR)<=a.CorCutoff_max&&(g[e]=d[h],e++);d=g;1E4>d.length&&(c=a.topG.selectAll(".link").data(d,a.mlinkKey),c.enter().append("line").attr("class",
"link").attr("pointer-events","none").attr("id",function(l){return"ln_"+that.removeInvalidIDChars(l.MOD1)+that.removeInvalidIDChars(l.MOD2)}).attr("x1",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD1)).attr("cx")}).attr("y1",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD1)).attr("cy")}).attr("x2",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD2)).attr("cx")}).attr("y2",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD2)).attr("cy")}).attr("stroke",
a.colorLink).attr("stroke-width",function(l){return Math.abs(l.COR)*a.maxCorWidth}).merge(c))};a.nodeX=function(d,c){d=50;var g=Math.floor(c/a.maxPerLevel)+1;1<g&&(d=60*g);return a.width/2+(a.r-d)*Math.cos(c%a.maxPerLevel*a.angle-1.57079633)};a.nodeY=function(d,c){d=50;c>a.maxPerLevel&&(d=60*(Math.floor(c/a.maxPerLevel)+1));return a.height/2+(a.r-d)*Math.sin(c%a.maxPerLevel*a.angle-1.57079633)};a.nodeXLbl=function(d,c){c=c%a.maxPerLevel*a.angle-1.57079633;var g=1;if(Math.PI/2<c&&c<1.5*Math.PI){var e=
0;"undefined"!==typeof d.MODULE&&(e=d.MODULE.length);g+=7.5*e}return a.width/2+(a.r+g)*Math.cos(c)};a.nodeYLbl=function(d,c){c=c%a.maxPerLevel*a.angle-1.57079633;var g=1;if(Math.PI/2<c&&c<1.5*Math.PI){var e=0;"undefined"!==typeof d.MODULE&&(e=d.MODULE.length);g+=7.5*e}return a.height/2+(a.r+g)*Math.sin(c)};a.drawCorLegend=function(d){a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",d).text("Positive Correlation");a.legendDispG.append("rect").attr("x","0").attr("y",d-10).attr("width",
"30").attr("height","10").style("fill",a.posColor);a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",d+20).text("Negative Correlation");a.legendDispG.append("rect").attr("x","0").attr("y",d+10).attr("width","30").attr("height","10").style("fill",a.negColor);a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",d+40).text("Lower Correlation");a.legendDispG.append("line").attr("x1","0").attr("y1",d+30).attr("x2","30").attr("y2",d+30).style("stroke",
"#000000").style("stroke-width","0");a.legendDispG.append("line").attr("x1","0").attr("y1",d+35).attr("x2","30").attr("y2",d+35).style("stroke","#000000").style("stroke-width","0.5");a.legendDispG.append("line").attr("x1","0").attr("y1",d+40).attr("x2","30").attr("y2",d+40).style("stroke","#000000").style("stroke-width","1");a.legendDispG.append("line").attr("x1","0").attr("y1",d+45).attr("x2","30").attr("y2",d+45).style("stroke","#000000").style("stroke-width","2");a.legendDispG.append("line").attr("x1",
"0").attr("y1",d+55).attr("x2","30").attr("y2",d+55).style("stroke","#000000").style("stroke-width","3");a.legendDispG.append("line").attr("x1","0").attr("y1",d+65).attr("x2","30").attr("y2",d+65).style("stroke","#000000").style("stroke-width","5");a.legendDispG.append("line").attr("x1","0").attr("y1",d+75).attr("x2","30").attr("y2",d+75).style("stroke","#000000").style("stroke-width","7");a.legendDispG.append("text").style("font-size","16").attr("x","35").attr("y",d+80).text("Higher Correlation")};
a.drawLegend=function(){a.drawCorLegend(20)};a.selectMetaMod=function(d){that.topselectedModule=that.selectedModule;that.selectedModule=that.modules[d];that.topselectedGene=that.selectedGene;that.selectedGene={};for(d=0;d<that.selectedModule.TCList.length;d++)"undefined"===typeof that.selectedGene[that.selectedModule.TCList[d].Gene.ID]&&(that.selectedGene[that.selectedModule.TCList[d].Gene.ID]={},that.selectedGene[that.selectedModule.TCList[d].Gene.ID].geneSymbol=that.selectedModule.TCList[d].Gene.geneSymbol,
that.selectedGene[that.selectedModule.TCList[d].Gene.ID].tcList=[]),that.selectedGene[that.selectedModule.TCList[d].Gene.ID].tcList.push(that.selectedModule.TCList[d]);that.viewType="gene";d3.select("#wgcnaGeneViewRB").property("checked",!0);that.createSingleWGCNAImage()};a.draw=function(){a.updateColor("linkColor");a.width=$(window).width();a.height=Math.floor(.9*window.innerHeight);a.r=a.height>a.width?Math.floor(a.width/2*.95):Math.floor(a.height/2*.95);a.svg.attr("height",a.height+"px");a.svg.selectAll("circle").remove();
