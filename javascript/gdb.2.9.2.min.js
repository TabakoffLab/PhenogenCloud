...
myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(b){f.prevMinCoord=e;f.prevMaxCoord=l;e<f.dataMinCoord&&(f.dataMinCoord=e);l>f.dataMaxCoord&&(f.dataMaxCoord=l);f.folderName=b.folderName;f.updateData();processAjax=0;gtag("event","outsideLoadedRegion",{event_category:"updatePage"})},error:function(b,a,c){console.log(c)}})}}
function back(f){if(history&&history[f]){var e=1<history[f].length?history[f].pop():history[f][0];chr==e.chr&&(e.start==svgList[f].xScale.domain()[0]&&e.stop==svgList[f].xScale.domain()[1]&&(e=1<history[f].length?history[f].pop():history[f][0]),0==f&&$("#geneTxt").val(e.chr+":"+e.start+"-"+e.stop),svgList[f].xScale.domain([e.start,e.stop]),svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis),svgList[f].redraw(),0===f&&updatePage(svgList[f]),svgList[f].updateFullData())}gtag("event","navigateBack",
{event_category:"stepBackSVGNaviation"})}
function zoomIn(f,e){var l={};l.chr=chr;l.start=svgList[f].xScale.domain()[0];l.stop=svgList[f].xScale.domain()[1];var n=Math.floor((l.stop-l.start)*e);1>n&&(n=1);l.start+=n;l.stop-=n;20>l.stop-l.start&&(l.start-=n,l.stop+=n);svgList[f].xScale.domain([l.start,l.stop]);history[f]||(history[f]=[]);history[f].push(l);svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis);svgList[f].redraw();isNaN(zoomUpdateHandle)||(clearTimeout(zoomUpdateHandle),zoomUpdateHandle=Math.NaN);zoomUpdateHandle=setTimeout(function(){0===
f&&updatePage(svgList[f]);svgList[f].updateFullData();0===f&&setTimeout(function(){DisplayRegionReport()},100)},300);0===f&&$("#geneTxt").val(chr+":"+l.start+"-"+l.stop);gtag("event",f,{event_category:"calledZoomIn"})}
function zoomOut(f,e){var l={};l.chr=chr;l.start=svgList[f].xScale.domain()[0];l.stop=svgList[f].xScale.domain()[1];var n=Math.floor((l.stop-l.start)*e);1>n&&(n=1);l.start-=n;l.stop+=n;svgList[f].xScale.domain([l.start,l.stop]);history[f]||(history[f]=[]);history[f].push(l);svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis);svgList[f].redraw();isNaN(zoomUpdateHandle)||(clearTimeout(zoomUpdateHandle),zoomUpdateHandle=Math.NaN);zoomUpdateHandle=setTimeout(function(){0===f&&updatePage(svgList[f]);
svgList[f].updateFullData();0===f&&setTimeout(function(){DisplayRegionReport()},100)},300);0===f&&$("#geneTxt").val(chr+":"+l.start+"-"+l.stop);gtag("event",f,{event_category:"calledZoomOut"})}
function mup(){var f=0;for(f=0;f<svgList.length;f++)if(svgList[f]&&"undefined"!==typeof svgList[f])if(history[f]||0!=f&&1!=f||(history[f]=[]),!(svgList[f].overSettings&&0!=svgList[f].overSettings||isNaN(svgList[f].downx)&&isNaN(svgList[f].downPanx))){if(0===f&&updatePage(svgList[f]),svgList[f].downx=Math.NaN,svgList[f].downPanx=Math.NaN,svgList[f].updateFullData(),0===f&&setTimeout(function(){DisplayRegionReport()},300),0==f||1==f){var e={};e.chr=chr;e.start=svgList[f].xScale.domain()[0];e.stop=svgList[f].xScale.domain()[1];
history[f].push(e)}}else if(!(svgList[f].overSettings&&0!=svgList[f].overSettings||isNaN(svgList[f].downZoomx))){var l=svgList[f].downZoomx;var n=d3.mouse(svgList[f].vis.node());svgList[f].downZoomxEnd=n[0];e=1;n[0]<l?(l=n[0],e=svgList[f].downZoomx-l):e=n[0]-l;n=Math.round(svgList[f].xScale.invert(l));l=Math.round(svgList[f].xScale.invert(l+e));svgList[f].downZoomx=Math.NaN;svgList[f].downZoomxEnd=Math.NaN;0===f&&$("#geneTxt").val(chr+":"+n+"-"+l);d3.select("#Level"+svgList[f].levelNumber).selectAll("svg rect.zoomRect").remove();
d3.select("#Level"+svgList[f].levelNumber).selectAll("svg text#zoomTextStart").remove();d3.select("#Level"+svgList[f].levelNumber).selectAll("svg text#zoomTextEnd").remove();svgList[f].xScale.domain([n,l]);svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis);svgList[f].redraw();svgList[f].updateFullData();0===f&&setTimeout(function(){DisplayRegionReport()},300);if(0==f||1==f)e={},e.chr=chr,e.start=n,e.stop=l,history[f].push(e)}gtag("event","mouseUpFromDrag",{event_category:"dragMouseUp"})}
...
$(document).on("click",".closeBtn",function(){var f=new String($(this).attr("id"));f=f.substr(6);$("."+f).fadeOut("fast");if(-1<f.indexOf("viewsLevel")){var e=f.substr(f.length-1);$("div#nameView"+e).hide();$("div#selection"+e).show();$("span#viewMenuLbl"+e).html("Select/Edit Views");$("div.trackLevel"+e).fadeOut("fast")}gtag("event",f,{event_category:"closeSettings"});return!1});
$(document).on("click",".viewSelect",function(){var f=$(this).attr("id");f=f.substr(f.length-1);if($(".viewsLevel"+f).is(":visible"))$(".viewsLevel"+f).fadeOut("fast");else{var e=$(this).parent().parent().position().top;$(".viewsLevel"+f).css("top",e).css("left",$(window).width()-610);$(".viewsLevel"+f).fadeIn("fast");$("#trackSettingDialog").hide();tt.transition().duration(200).style("opacity",0);gtag("event",f,{event_category:"openViews"})}return!1});
$(document).on("change","input[name='optioncbx']",function(){var f=new String($(this).attr("id")),e=f.indexOf("CBX"),l=new String(f.substr(0,e));e=f.substr(e+3,1);redrawTrack(e,l);gtag("event",f,{event_category:"changeSettingView"})});$(document).on("change","input[name='imgCBX']",function(){var f=new String($(this).attr("id")),e=f.indexOf("CBX");new String(f.substr(0,e));e=f.substr(e+3,1);svgList[e].redraw();setTimeout(function(){DisplayRegionReport()},300);gtag("event",f,{event_category:"clickCbx"})});
$(document).on("click",".reset",function(){var f=new String($(this).attr("id")),e=f.substr(f.length-1);0===f.indexOf("resetImage")?0===e&&($("#geneTxt").val(chr+":"+initMin+"-"+initMax),svgList[0].xScale.domain([initMin,initMax]),svgList[0].scaleSVG.select(".x.axis").call(svgList[0].xAxis),svgList[0].redraw()):0===f.indexOf("resetTracks")&&(svgList[e].removeAllTracks(),setupDefaultView(e));gtag("event",f,{event_category:"resetImage"})});
...
siteVer)}function removeTrack(f,e){"undefined"!==typeof svgList[f]&&svgList[f].removeTrack(e);gtag("event",f,{event_category:"removeTrack",event_label:e})}function redrawTrack(f,e){svgList[f].redrawTrack(e);gtag("event",f,{event_category:"redrawTrack",event_label:e})}function changeTrackHeight(f,e){if(0<e){var l=e+"px";$("#Scroll"+f).css({"max-height":l,overflow:"auto"})}else $("#Scroll"+f).css({"max-height":"",overflow:"hidden"})}
registerKeyboardHandler=function(f){d3.select(window).on("keydown",f)};function getAllChildrenByName(f,e){var l=[],n=0;if("undefined"!==typeof f&&null!==f&&"undefined"!==typeof f.childNodes)for(var c=f.childNodes,b=0;b<c.length;b++)c.item(b).nodeName==e&&(l[n]=c.item(b),n++);return l}
function getFirstChildByName(f,e){var l=null;if(null!==f&&"undefined"!==typeof f&&"undefined"!==typeof f.childNodes)for(var n=f.childNodes,c=!1,b=0;b<n.length&&!c;b++)n.item(b).nodeName==e&&(l=n.item(b),c=!0);return l}
...
"all").style("cursor","pointer").on("click",function(a){d3.select(b).remove();gtag("event",b,{event_category:"removeErrorTrack"})}).on("mouseover",function(){d.overSettings=1;$("#mouseHelp").html("");gtag("event",b,{event_category:"mouseHelp"})}).on("mouseout",function(){d.overSettings=0;$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.");gtag("event",b,{event_category:"hideMouseHelp"})})}};d.addTrack=function(a,b,c,g){"Trx"==d.forceDrawAsValue&&-1==
(new String(c)).indexOf("DrawTrx")&&(c=","===c.substr(c.length-1)?c+"DrawTrx,":c+",DrawTrx,");var m=new String(d.folderName);if(0>m.indexOf(chr)||0>m.indexOf(d.xScale.domain()[0]+"_")||0>m.indexOf("_"+d.xScale.domain()[1]))d.folderName="/"+chr+"/"+d.xScale.domain()[0]+"_"+d.xScale.domain()[1],0==d.levelNumber&&regionfolderName!=d.folderName&&(regionfolderName=d.folderName),$.ajax({url:pathPrefix+"getFullPath.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:d.xScale.domain()[0],maxCoord:d.xScale.domain()[1],
panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(a){},error:function(a,b,d){console.log(d)}});m=null;m=d3.select("#Level"+d.levelNumber+a).nodes();if(0===m.length||null==m){m=d.topLevel.append("li").attr("class","draggable"+d.levelNumber).attr("id","li"+a).style("margin-bottom","-3px").append("svg").attr("width",d.width).attr("height",30).attr("class","track").attr("id","Level"+d.levelNumber+a).style("cursor","move").on("mouseover",
...
d.defaultMouseFunct?d.changeTrackCursor("crosshair"):d3.event.altKey&&"dragzoom"===d.defaultMouseFunct?d.changeTrackCursor("move"):d3.event.altKey||"dragzoom"!==d.defaultMouseFunct?d3.event.altKey||"dragzoom"===d.defaultMouseFunct||d.changeTrackCursor("move"):d.changeTrackCursor("crosshair")});var p=new String("Loading...");m.append("text").text(p).attr("x",d.width/2-p.length/2*7.5).attr("y",12).attr("id","trkLbl");p=m.append("g").attr("class","infoIcon").attr("transform","translate("+(d.width-20)+
",0)").style("cursor","pointer").attr("track",a).attr("title",a).on("mouseover",function(){var b=$(this).attr("track"),d=trackInfo[b];b=$(this).tooltipster({position:"top-right",maxWidth:250,offsetX:24,offsetY:5,contentAsHTML:!0,interactive:!0,interactiveTolerance:350});b.tooltipster("content",function(){var a="";"undefined"!==typeof d&&"undefined"!==typeof d.Description&&(a=d.Description);return a});b.tooltipster("show");gtag("event",a,{event_category:"trackInfo"})}).on("mouseout",function(){$(this).tooltipster("hide")});
p.append("rect").attr("x",0).attr("y",0).attr("rx",3).attr("ry",3).attr("height",14).attr("width",14).attr("fill","#A7C5E2").attr("stroke","#7795B2");p.append("text").attr("x",2.5).attr("y",12).attr("style","font-family:monospace;font-weight:bold;").attr("fill","#FFFFFF").text("i");m.append("g").attr("class","settings").attr("id",a+"_"+d.levelNumber).attr("transform","translate("+(d.width-40)+",0)").append("image").attr("width","16px").attr("height","16px").attr("xlink:href","data:image/png;base64,").attr("pointer-events",
"all").style("cursor","pointer").on("click",function(b){a!=d.selectedTrackSetting?(d.getTrack(a).generateSettingsDiv("div#trackSettingContent"),d.selectedTrackSetting=a,b=$(this).position(),$("#trackSettingDialog").css("top",b.top).css("left",$(window).width()-380),$("#trackSettingDialog").fadeIn("fast")):(d.selectedTrackSetting="",$("#trackSettingDialog").fadeOut("fast"));gtag("event","click",{event_category:"trackSettings"});return!1}).on("mouseover",function(){d.overSettings=1;$("#mouseHelp").html("Track Settings: Click to access track settings or quickly remove the track from the current view.")}).on("mouseout",
function(){d.overSettings=0;$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")})}if("genomeSeq"==a)m=SequenceTrack(d,a,"Reference Genomic Sequence",c),d.addTrackList(m);else if(-1<a.indexOf("noncoding"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",function(m,p){if(m)if(60>g){var h=1500;1==g&&(h=2500);setTimeout(function(){d.addTrack(a,b,c,g+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+
a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==p)if(11<=g){h=[];var q=GeneTrack(d,h,a,"Long Non-Coding / Non-PolyA+ Genes",c);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,c,10)},5E3);else{h=p.documentElement.getElementsByTagName("Gene");var e=0,t=p.documentElement.getElementsByTagName("GeneList");0<t.length&&"undefined"!==typeof t[0].getAttribute("ver")&&(e=t[0].getAttribute("ver"));
try{q=GeneTrack(d,h,a,"Long Non-Coding / Non-PolyA+ Genes",c),q.dataVer=e,d.addTrackList(q)}catch(G){}}});else if(-1<a.indexOf("coding"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",function(m,p){if(m)if(60>g){var h=1500;1==g&&(h=2500);setTimeout(function(){d.addTrack(a,b,c,g+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+
d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==p)if(11<=g){h=[];var q=GeneTrack(d,h,a,"Protein Coding / PolyA+",c);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,c,11)},5E3);else{h=p.documentElement.getElementsByTagName("Gene");var e=0,t=p.documentElement.getElementsByTagName("GeneList");0<t.length&&"undefined"!==typeof t[0].getAttribute("ver")&&(e=t[0].getAttribute("ver"));try{q=GeneTrack(d,h,a,"Protein Coding / PolyA+",c),q.dataVer=e,d.addTrackList(q)}catch(G){console.log(G)}}});
else if(-1<a.indexOf("smallnc"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",function(m,p){if(m){if(0===g){var h=d.xScale.domain()[0],q=d.xScale.domain()[1],e="/"+pathPrefix;pathPrefix||(e="");$.ajax({url:e+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:h,maxCoord:q,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(b){gtag("event",
"generateTrack"+a,{event_category:"browser"})},error:function(a,b,d){}})}60>g?(h=1500,1==g&&(h=2500),setTimeout(function(){d.addTrack(a,b,c,g+1)},h)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==p)11<=g?(h=[],q=GeneTrack(d,h,a,"Small RNA (<200 bp) Genes",c),d.addTrackList(q)):setTimeout(function(){d.addTrack(a,
b,c,11)},5E3);else{h="rn5"===genomeVer?p.documentElement.getElementsByTagName("smnc"):p.documentElement.getElementsByTagName("Gene");try{q=GeneTrack(d,h,a,"Small RNA (<200 bp) Genes",c),d.addTrackList(q),""!=selectGene&&q.setSelected(selectGene)}catch(H){}}});else if(0===a.indexOf("liverTotal")||"heartTotal"===a||0===a.indexOf("brainTotal")||"mergedTotal"===a){var h="Liver Reconstructed";"heartTotal"===a?h="Heart Reconstructed":"brainTotal"===a?h="Whole Brain Reconstructed":"mergedTotal"===a&&(h=
"Merged (Brain,Heart,Liver) Reconstructed");if((0===a.indexOf("liverTotal")||0===a.indexOf("brainTotal"))&&0<a.indexOf("_")){var q=a.substr(a.indexOf("_")+1);h="LEStm"===q?"LE-Stm "+h:"F344Stm"===q?"F344-Stm "+h:h+" v"+q}d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",function(m,p){if(m){if(1===g&&("mergedTotal"===a||0===a.indexOf("liverTotal")||0===a.indexOf("brainTotal"))){var q=d.xScale.domain()[0],e=d.xScale.domain()[1],t="/"+pathPrefix;pathPrefix||
(t="");var f=panel;(0===a.indexOf("liverTotal")||0===a.indexOf("brainTotal"))&&0<a.indexOf("_")&&(f=a.substr(a.indexOf("_")+1),"LEStm"===f&&(f="LE-Stm"),"F344Stm"===f&&(f="F344-Stm"));$.ajax({url:t+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:q,maxCoord:e,panel:f,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(b){gtag("event","generateTrack"+a,{event_category:"browser"})},
error:function(a,b,d){}})}60>g?(q=1500,1==g&&(q=2500),setTimeout(function(){d.addTrack(a,b,c,g+1)},q)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==p)11<=g?(q=[],e=GeneTrack(d,q,a,h,c),d.addTrackList(e)):setTimeout(function(){d.addTrack(a,b,c,11)},5E3);else{q=p.documentElement.getElementsByTagName("Gene");
t=0;f=p.documentElement.getElementsByTagName("GeneList");0<f.length&&"undefined"!==typeof f[0].getAttribute("ver")&&(t=f[0].getAttribute("ver"));try{e=GeneTrack(d,q,a,h,c),e.dataVer=t,d.addTrackList(e),""!=selectGene&&e.setSelected(selectGene)}catch(G){}}})}else if(0===a.indexOf("refSeq")){$("#"+a+d.levelNumber+"Select").val();var e=d.xScale.domain()[0],t=d.xScale.domain()[1];q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml";d3.xml(q,function(m,p){if(m){console.log(m);
if(0===g){var h="/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:e,maxCoord:t,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(b){gtag("event","generateTrack"+a,{event_category:"browser"})},error:function(a,b,d){}})}60>g?(h=1500,1===g&&(h=2500),setTimeout(function(){d.addTrack(a,b,c,g+1)},h)):(d3.select("#Level"+
d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==p)if(11<=g){h=[];var q=RefSeqTrack(d,h,a,"Ref Seq Genes",c);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,c,11)},5E3);else{h=p.documentElement.getElementsByTagName("Gene");try{q=RefSeqTrack(d,h,a,"Ref Seq Genes",c),d.addTrackList(q)}catch(A){console.log(A)}}})}else if(0==
a.indexOf("snp"))e=d.xScale.domain()[0],t=d.xScale.domain()[1],q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",d3.xml(q,function(m,p){if(m){console.log(m);if(0==g){var h="/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:e,maxCoord:t,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(b){gtag("event",
"generateTrack"+a,{event_category:"browser"})},error:function(a,b,d){}})}60>g?(h=1500,1===g&&(h=2500),setTimeout(function(){d.addTrack(a,b,c,g+1)},h)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==p)if(11<=g){h=[];var q=SNPTrack(d,h,a,b,c);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,
b,c,11)},5E3);else{h=p.documentElement.getElementsByTagName("Snp");try{q=SNPTrack(d,h,a,b,c),d.addTrackList(q)}catch(A){}}});else if("qtl"===a)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/qtl.xml",function(m,p){if(m)if(60>g){var h=1500;1===g&&(h=2500);setTimeout(function(){d.addTrack(a,b,c,g+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+
d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==p)if(11<=g){h=[];var q=QTLTrack(d,h,a,b);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,c,11)},5E3);else{h=p.documentElement.getElementsByTagName("QTL");try{q=QTLTrack(d,h,a,b),d.addTrackList(q)}catch(A){}}});else if("trx"===a)q=getAllChildrenByName(getFirstChildByName(d.selectedData,"TranscriptList"),"Transcript"),m=TranscriptTrack(d,q,a,b),d.addTrackList(m);else if("probe"===a||"probeMouse"==a)d3.xml(dataPrefix+"tmpData/browserCache/"+
genomeVer+"/regionData/"+d.folderName+"/probe.xml",function(m,p){if(m)if(60>g){var h=1500;1===g&&(h=2500);setTimeout(function(){d.addTrack(a,b,c,g+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==p)11<=g?(q=[],h=ProbeTrack(d,q,a,"Affy Exon 1.0 ST Probe Sets",b+","+c),d.addTrackList(h)):setTimeout(function(){d.addTrack(a,
b,c,11)},5E3);else{var q=p.documentElement.getElementsByTagName("probe");try{h=ProbeTrack(d,q,a,"Affy Exon 1.0 ST Probe Sets",b+","+c),d.addTrackList(h)}catch(A){}}});else if("helicos"===a||-1<a.indexOf("illuminaTotal")||-1<a.indexOf("illuminaSmall")||"illuminaPolyA"===a){e=d.xScale.domain()[0];t=d.xScale.domain()[1];var f=calculateBin(t-e,d.width);q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/tmp/"+e+"_"+t+".count."+a+".xml";0<f&&(e-=e%f,t+=f-t%f,q=dataPrefix+"tmpData/browserCache/"+
genomeVer+"/regionData/"+d.folderName+"/tmp/"+e+"_"+t+".bincount."+f+"."+a+".xml");d3.xml(q,function(m,p){if(m){if(0===g||3===g){var h="/"+pathPrefix;pathPrefix||(h="");tmpPanel=panel;-1<a.indexOf("-")&&(tmpPanel=a.substr(a.indexOf("-")+1));$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:e,maxCoord:t,panel:tmpPanel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName,binSize:f},dataType:"json",
success:function(b){gtag("event","generateTrack"+a,{event_category:"browser"})},error:function(a,b,d){}});h=1E4}60>g?(h=1E3,0===g&&(h=2500),setTimeout(function(){d.addTrack(a,b,c,g+1)},h)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==p)4<=g?(h=[],"helicos"==a?q=HelicosTrack(d,h,a,b):"illuminaTotal"==
a?q=IlluminaTotalTrack(d,h,a,b):"illuminaSmall"==a?q=IlluminaSmallTrack(d,h,a,b):"illuminaPolyA"==a?q=IlluminaPolyATrack(d,h,a,b):"liverilluminaTotalPlus"==a?q=LiverIlluminaTotalPlusTrack(d,h,a,b):"liverilluminaTotalMinus"==a?q=LiverIlluminaTotalMinusTrack(d,h,a,b):"heartilluminaTotalPlus"==a?q=HeartIlluminaTotalPlusTrack(d,h,a,b):"heartilluminaTotalMinus"==a?q=HeartIlluminaTotalMinusTrack(d,h,a,b):"brainilluminaTotalPlus"==a?q=BrainIlluminaTotalPlusTrack(d,h,a,b):"brainilluminaTotalMinus"==a?q=BrainIlluminaTotalMinusTrack(d,
h,a,b):"liverilluminaSmall"==a?q=LiverIlluminaSmallTrack(d,h,a,b):"heartilluminaSmall"==a?q=HeartIlluminaSmallTrack(d,h,a,b):-1<a.indexOf("illuminaTotal")&&(q=StrainSpecificIlluminaTotalTrack(d,h,a,b)),d.addTrackList(q)):setTimeout(function(){d.addTrack(a,b,c,4)},3E3);else{h=p.documentElement.getElementsByTagName("Count");var q;try{"helicos"===a?q=HelicosTrack(d,h,a,b):"illuminaTotal"===a?q=IlluminaTotalTrack(d,h,a,b):"illuminaSmall"===a?q=IlluminaSmallTrack(d,h,a,b):"illuminaPolyA"===a?q=IlluminaPolyATrack(d,
h,a,b):"liverilluminaTotalPlus"===a?q=LiverIlluminaTotalPlusTrack(d,h,a,b):"liverilluminaTotalMinus"===a?q=LiverIlluminaTotalMinusTrack(d,h,a,b):"heartilluminaTotalPlus"===a?q=HeartIlluminaTotalPlusTrack(d,h,a,b):"heartilluminaTotalMinus"===a?q=HeartIlluminaTotalMinusTrack(d,h,a,b):"brainilluminaTotalPlus"===a?q=BrainIlluminaTotalPlusTrack(d,h,a,b):"brainilluminaTotalMinus"===a?q=BrainIlluminaTotalMinusTrack(d,h,a,b):"liverilluminaSmall"===a?q=LiverIlluminaSmallTrack(d,h,a,b):"heartilluminaSmall"===
a?q=HeartIlluminaSmallTrack(d,h,a,b):-1<a.indexOf("illuminaTotal")&&(q=StrainSpecificIlluminaTotalTrack(d,h,a,b)),d.addTrackList(q)}catch(A){}}})}else if(-1<a.indexOf("spliceJnct")){e=d.xScale.domain()[0];t=d.xScale.domain()[1];q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml";var l="Brain ";"liverspliceJnct"==a?l="Liver ":"heartspliceJnct"==a&&(l="Heart ");d3.xml(q,function(m,h){if(m){console.log(m);if(0==g){var p="/"+pathPrefix;pathPrefix||(p="");$.ajax({url:p+
"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:e,maxCoord:t,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(b){gtag("event","generateTrack"+a,{event_category:"browser"})},error:function(a,b,d){}})}60>g?(p=1500,1===g&&(p=2500),setTimeout(function(){d.addTrack(a,b,c,g+1)},p)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+
a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==h)if(11<=g){p=[];var q=SpliceJunctionTrack(d,p,a,l+"Splice Junctions",1,"");d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,c,11)},5E3);else{p=h.documentElement.getElementsByTagName("Feature");console.log(p);try{q=SpliceJunctionTrack(d,p,a,l+"Splice Junctions",3,""),d.addTrackList(q)}catch(A){console.log(A)}}})}else if("polyASite"==
a)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/polyASite.xml",function(m,h){if(m)if(60>g){var p=1E3;1===g&&(p=2500);setTimeout(function(){d.addTrack(a,b,c,g+1)},p)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==h)if(4<=g){p=[];var q=PolyATrack(d,p,a,"Predicted PolyA Sites",
c);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,c,4)},5E3);else{p=h.documentElement.getElementsByTagName("Feature");try{q=PolyATrack(d,p,a,"Predicted PolyA Sites",c),d.addTrackList(q)}catch(A){}}});else if(0===a.indexOf("repeatMask"))$("#"+a+d.levelNumber+"Select").val(),e=d.xScale.domain()[0],t=d.xScale.domain()[1],q=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",d3.xml(q,function(m,h){if(m){console.log(m);if(0===g){var p="/"+pathPrefix;pathPrefix||
(p="");$.ajax({url:p+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:e,maxCoord:t,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(b){gtag("event","generateTrack"+a,{event_category:"browser"})},error:function(a,b,d){}})}60>g?(p=1500,1===g&&(p=2500),setTimeout(function(){d.addTrack(a,b,c,g+1)},p)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+
a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==h)if(11<=g){p=[];var q=RepeatMaskTrack(d,p,a,"Repeat Masker",b,c);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,c,11)},5E3);else{p=h.documentElement.getElementsByTagName("Feature");try{q=RepeatMaskTrack(d,p,a,"Repeat Masker",b,c),d.addTrackList(q)}catch(A){console.log(A)}}});else if(0==a.indexOf("custom"))if(m=trackInfo[a],c=c+",Name="+
m.Name,c="DataFile="+m.Location+","+c,"bed"==m.Type||"bb"==m.Type)q=[],m=CustomTranscriptTrack(d,q,a,m.Name,3,c),d.addTrackList(m),m.updateFullData(0,0);else{if("bg"==m.Type||"bw"==m.Type)q=[],m=CustomCountTrack(d,q,a,3,c),d.addTrackList(m)}else 0<=a.indexOf("cirRNA")&&(m=trackInfo[a],m=CircRNATrack(d,q,a,m.Name,3,c),d.addTrackList(m));$(".sortable"+d.levelNumber).sortable("refresh");gtag("event",a,{event_category:"trackAdded"})};d.addTrackList=function(a){null!=a&&(d.trackList[d.trackCount]=a,d.trackCount++,
d.trackListHash[a.trackClass]=a,setTimeout(function(){DisplayRegionReport()},300))};d.updateLinks=function(){1===d.levelNumber&&d3.select("#probeSetDetailLink1").each(function(){var a=new String(d3.select(this).attr("href"));a=a.substr(0,a.lastIndexOf("=")+1);a+=d.currentView.ViewID;d3.select(this).attr("href",a)})};d.changeTrackHeight=function(a,b){0<b?d3.select("#"+a+"Scroll").style("max-height",b+"px").style("overflow","auto"):d3.select("#"+a+"Scroll").style("max-height","none").style("overflow",
"hidden")};d.removeAllTracks=function(){d3.selectAll("li.draggable"+d.levelNumber).remove();d.trackList=[];setTimeout(function(){DisplayRegionReport()},300);gtag("event",d.levelNumber,{event_category:"SVGremoveAllTracks"})};d.removeTrack=function(a){d3.select("#Level"+d.levelNumber+a).remove();for(var b=0;b<d.trackList.length;b++)"undefined"!==typeof d.trackList[b]&&d.trackList[b].trackClass==a&&(d.trackList.splice(b,1),d.trackCount--);d.trackListHash[a]=void 0;setTimeout(function(){DisplayRegionReport()},
300);gtag("event",d.levelNumber,{event_category:"removeTrack",event_label:a})};d.redrawTrack=function(a){for(var b=0;b<d.trackList.length;b++)"undefined"!==typeof d.trackList[b]&&d.trackList[b].trackClass==a&&d.trackList[b].redraw();setTimeout(function(){DisplayRegionReport()},300)};d.redraw=function(){for(var a=0;a<d.trackList.length;a++)"undefined"!==typeof d.trackList[a]&&"undefined"!==typeof d.trackList[a].redraw&&d.trackList[a].redraw();d.selectSvg.redraw()};d.update=function(){for(var a=0;a<
d.trackList.length;a++)"undefined"!==typeof d.trackList[a].update&&d.trackList[a].update();setTimeout(function(){DisplayRegionReport()},300);gtag("event","genomeSVG.update",{event_category:"gsvgUpdate"})};d.updateData=function(){for(var a=0;a<d.trackList.length;a++)"undefined"!==typeof d.trackList[a]&&"undefined"!==typeof d.trackList[a].updateData&&d.trackList[a].updateData(0);d.updateFullData();setTimeout(function(){DisplayRegionReport()},300);gtag("event","genomeSVG.updateData",{event_category:"gsvgUpdateData"})};
d.updateFullData=function(){-1<(new String(d.folderName)).indexOf("img")&&(d.folderName="/"+chr+"/"+d.xScale.domain()[0]+"_"+d.xScale.domain()[1],0==d.levelNumber&&regionfolderName!=d.folderName&&(regionfolderName=d.folderName),setTimeout(function(){DisplayRegionReport()},300));for(var a=0;a<d.trackList.length;a++)"undefined"!==typeof d.trackList[a]&&"undefined"!==typeof d.trackList[a].updateFullData&&d.trackList[a].updateFullData(0,1);gtag("event","genomeSVG.updateFullData",{event_category:"gsvgUpdateFullData"})};
d.setLoading=function(){for(var a=0;a<d.trackList.length;a++)"undefined"===typeof d.trackList[a]||"undefined"===typeof d.trackList[a].updateData&&"undefined"===typeof d.trackList[a].updateFullData||d.trackList[a].showLoading()};d.clearSelection=function(){d.selectionStart=-1;d.selectionEnd=-1;d.scaleSVG.selectAll("rect.selectedArea").remove();d.selectSvg.setVis(!1);for(var a=0;a<d.trackList.length;a++)"undefined"!==typeof d.trackList[a]&&"undefined"!==typeof d.trackList[a].clearSelection&&d.trackList[a].clearSelection()};
d.mdown=function(){if(0==d.overSettings)if("dragzoom"!==d.defaultMouseFunct&&d3.event.altKey||"dragzoom"===d.defaultMouseFunct&&!d3.event.altKey){var a=d3.mouse(d.vis.node());d.downZoomx=a[0];d.scaleSVG.append("rect").attr("class","zoomRect").attr("x",a[0]).attr("y",0).attr("height",d.scaleSVG.attr("height")).attr("width",1).attr("fill","#CECECE").attr("opacity",.3);d.scaleSVG.append("text").attr("id","zoomTextStart").attr("x",d.downZoomx).attr("y",15).text(numberWithCommas(Math.round(d.xScale.invert(d.downZoomx))));
d.scaleSVG.append("text").attr("id","zoomTextEnd").attr("x",d.downZoomx).attr("y",50).text(numberWithCommas(Math.round(d.xScale.invert(d.downZoomx))))}else 0==processAjax&&(d.prevMinCoord=d.xScale.domain()[0],d.prevMaxCoord=d.xScale.domain()[1],a=d3.mouse(d.vis.node()),d.downx=d.xScale.invert(a[0]),d.downscalex=d.xScale)};d.forceDrawAs=function(a){for(var b=0;b<d.trackList.length;b++)"undefined"!==typeof d.trackList[b]&&"undefined"!==typeof d.trackList[b].drawAs&&(d.trackList[b].drawAs=a,d.trackList[b].draw(d.trackList[b].data));
d.forceDrawAsValue=a};d.resize=function(a){d.width=a;d.xScale.range([0,d.width]);d.xAxis=d3.axisTop(d.xScale).ticks(6).tickSize(8).tickPadding(10);d.scaleSVG.attr("width",d.width);d.scaleSVG.select(".x.axis").call(d.xAxis);d3.select("#Level"+d.levelNumber).select(".axisLbl").attr("x",(d.width-2*d.margin)/2);d.topLevel.style("width",d.width+18+"px");for(var b=0;b<d.trackList.length;b++)"undefined"!==typeof d.trackList[b]&&"undefined"!==typeof d.trackList[b].redraw&&d.trackList[b].resize();d.selectSvg.width=
a;d.selectSvg.draw()};d.updateCountScales=function(a,b){for(var c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c]&&"undefined"!==typeof d.trackList[c].updateCountScale&&d.trackList[c].updateCountScale(a,b)};d.resetDefaultMouse=function(a){var b="/web/images/icons/"+a+"_dark.png";d3.select("span#"+a+d.levelNumber+" img").attr("src",b);d3.select("span#"+a+d.levelNumber).style("background","#DCDCDC")};d.changeTrackCursor=function(a){for(var b=0;b<d.trackList.length;b++)"undefined"!==typeof d.trackList[b]&&
"undefined"!==typeof d.trackList[b].svg&&d.trackList[b].svg.style("cursor",a)};d.changeScaleCursor=function(a){"undefined"!==typeof d.scaleSVG&&d.scaleSVG.style("cursor",a)};d.updateTrackSelectedArea=function(a,b){d.selectionStart=a;d.selectionEnd=b;var c=d.xScale(a),g=d.xScale(b)-c;d.scaleSVG.selectAll("rect.selectedArea").remove();if((0<c||0<c+g)&&c<d.width)for(1>g&&(g=1),d.scaleSVG.append("rect").attr("class","selectedArea").attr("x",c).attr("y",15).attr("height",d.scaleSVG.attr("height")-15).attr("width",
g).attr("fill","#CECECE").attr("opacity",.3).attr("pointer-events","none"),c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c]&&"undefined"!==typeof d.trackList[c].setSelectedArea&&d.trackList[c].setSelectedArea(a,b)};d.getAddMenus=function(){var a="/"+pathPrefix;""==pathPrefix&&(a="");$.ajax({url:a+"trackMenu.jsp",type:"GET",cache:!1,data:{level:d.levelNumber,organism:organism},dataType:"html",success:function(a){$("#trackMenu"+d.levelNumber).remove();d3.select("div#trackMenu").append("div").attr("id",
"trackMenu"+d.levelNumber);$("#trackMenu"+d.levelNumber).html(a)},error:function(a,b,c){$("#trackMenu"+d.levelNumber).remove();d3.select("div#trackMenu").append("div").attr("id","trackMenu"+d.levelNumber);$("#trackMenu"+d.levelNumber).append('<div class="viewsLevel'+d.levelNumber+'">An error occurred generating this menu.  Please try back later.</div>')},async:!1});$.ajax({url:a+"viewMenu.jsp",type:"GET",cache:!1,data:{level:d.levelNumber,organism:organism},dataType:"html",success:function(a){$("#viewMenu"+
...
"#FFFFFF");d.zoomFactor=parseFloat(d3.select(a).attr("value"));d3.select(a).style("background","#CECECE")};d.setupFunctionBar=function(){d3.selectAll("span.shrt-button").on("click",function(){nameSpan=d3.select(this).attr("name");-1<nameSpan.toLowerCase().indexOf("gene")?(console.log(":"+selectedID+":"),""!==selectedID?($("div#selectedDetail").is(":visible")||0!==selectedID.indexOf("ENS")||d.getTrack("ensemblcoding").setSelected(selectedID),$("span[name='"+nameSpan+"']").click(),$("html, body").animate({scrollTop:$("#selectedReport").offset().top},
1E3)):alert("You must select a gene from an Ensembl Track before clicking on the gene shortcut.")):($("span.closeDetail").click(),$("span#"+nameSpan).click(),$("html, body").animate({scrollTop:$("#regionDiv").offset().top},1E3))});d3.select(f).select("#functLevel"+d.levelNumber).remove();d.functionBar=d.vis.append("div").attr("class","functionBar").attr("id","functLevel"+d.levelNumber).style("float","left");var a=d.functionBar.append("div").attr("class","defaultMouse").attr("id","defaultMouse"+d.levelNumber);
a.append("span").attr("id","dragzoom"+d.levelNumber).style("height","24px").style("display","inline-block").style("cursor","pointer").append("img").attr("class","mouseOpt dragzoom").attr("src","/web/images/icons/dragzoom_dark.png").attr("pointer-events","all").on("click",function(){d.resetDefaultMouse(d.defaultMouseFunct);d.defaultMouseFunct="dragzoom";d3.select(this).attr("src","/web/images/icons/dragzoom_white.png");d3.select("span#dragzoom"+d.levelNumber).style("background","#989898");d.changeTrackCursor("crosshair");
d.changeScaleCursor("crosshair");gtag("event","",{event_category:"clickDragZoom"})}).on("mouseout",function(){"dragzoom"!=d.defaultMouseFunct&&(d3.select(this).attr("src","/web/images/icons/dragzoom_dark.png"),d3.select("span#dragzoom"+d.levelNumber).style("background","#DCDCDC"))}).on("mouseover",function(){d3.select(this).attr("src","/web/images/icons/dragzoom_white.png");d3.select("span#dragzoom"+d.levelNumber).style("background","#989898");$("#mouseHelp").html("Click to set default mouse function to allow click and drag to select a region to zoom in on.")});
...
contentType:"text/html",data:a+"\n",processData:!1,dataType:"json",success:function(a){var d=new Date;d=d.getMonth()+1+"_"+d.getDate()+"_"+d.getFullYear();var c="http://";"https:"===location.protocol&&(c="https://");var g=c+urlprefix+"/tmpData/download/"+a.imageFile;a=new String($("#geneTxt").val());a=a.replace(/:/g,"_");a=a.replace(/-/g,"_");a=a.replace(/,/g,"");"Level1"==b&&(a=svgList[1].selectedData.getAttribute("geneSymbol"));var p=a+"_"+d+".png",h=new XMLHttpRequest;h.open("GET",g);h.responseType=
"blob";h.send();h.onreadystatechange=function(){if(4!=h.readyState||200!=h.status)return!1;var a=document.createElement("a");a.href=window.URL.createObjectURL(h.response);a.download=p;a.style.display="none";document.body.appendChild(a);try{a.click()}catch(E){$("#mouseHelp").html("<span style='color:#FF0000;'>Your browser will not save the image directly. Image will open in a popup, in the new window right click to save image.</span>"),window.open(g)}return!0};gtag("event","saveBrowserImage",{event_category:"browser"})},
error:function(a,b,d){console.log(d)}});gtag("event","",{event_category:"saveImage"})}).on("mouseover",function(){d3.select(this).attr("src","/web/images/icons/savePic_white.png");d3.select("span#savePic"+d.levelNumber).style("background","#DCDCDC");$("#mouseHelp").html("Click to download a PNG image of the current view.")}).on("mouseout",function(){d3.select(this).attr("src","/web/images/icons/savePic_dark.png");d3.select("span#savePic"+d.levelNumber).style("background","#989898");$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")});
...
svgList[0].redraw()):(svgList[a].xScale.domain([svgList[a].initMin,svgList[a].initMax]),svgList[a].scaleSVG.select(".x.axis").call(svgList[a].xAxis),svgList[a].redraw());svgList[0].updateFullData();gtag("event","",{event_category:"clickResetImage"})}).on("mouseover",function(){d3.select(this).attr("src","/web/images/icons/reset_white.png");d3.select("span#reset"+d.levelNumber).style("background","#DCDCDC");$("#mouseHelp").html("Click to reset image zoom to initial region.")}).on("mouseout",function(){d3.select(this).attr("src",
...
testSafari?d3.select("#viewSelect"+d.levelNumber).style("height","2.3em"):testFireFox||testIE&&d3.select("#viewSelect"+d.levelNumber).style("position","relative").style("top","-8px").style("height","2.3em"),e=e.append("ul").attr("id","viewSelectMenu"+d.levelNumber),e.append("li").attr("id","menusaveView"+d.levelNumber).on("click",function(){viewMenu[d.levelNumber].saveView(d.currentView.ViewID,d,!1);$("#viewModifiedCtl"+d.level).hide();gtag("event","",{event_category:"menuSaveView"})}).text("Save"),
e.append("li").attr("id","menusaveAsView"+d.levelNumber).on("click",function(){viewMenu[d.levelNumber].saveAsView(d.currentView,svgList[d.levelNumber]);$(".viewsLevel"+d.levelNumber).css("top",250).css("left",$(window).width()-610);$(".viewsLevel"+d.levelNumber).fadeIn("fast");$("#viewModifiedCtl"+d.level).hide();gtag("event","",{event_category:"menuSaveAsView"})}).text("Save As"),e.append("li").attr("id","menudeleteView"+d.levelNumber).on("click",function(){viewMenu[d.levelNumber].confirmDeleteView(d.currentView);
$(".viewsLevel"+d.levelNumber).css("top",250).css("left",$(window).width()-610);$(".viewsLevel"+d.levelNumber).fadeIn("fast");gtag("event","",{event_category:"menuDeleteView"})}).text("Delete"),$("#viewSelect"+d.levelNumber).button().click(function(){}).next().button({text:!1,icons:{primary:"ui-icon-triangle-1-s"}}).click(function(){var a=$(this).parent().next().show().position({my:"left top",at:"left bottom",of:this});$(document).on("click",function(){a.hide()});return!1}).parent().buttonset().next().hide().menu(),
d.vis.append("span").style("float","right").style("margin-right","5px").append("span").attr("id","mainAddTrack"+d.levelNumber).attr("class","control").style("height","24px").style("display","inline-block").style("cursor","pointer").append("img").attr("class","mainAddTrack").attr("src","/web/images/icons/addTrack_dark_32.png").attr("pointer-events","all").style("height","37px").style("position","relative").style("top","-8px").on("click",function(){if($(".trackLevel"+d.levelNumber).is(":visible"))$(".trackLevel"+
d.levelNumber).fadeOut("fast");else{var a=$(this).parent().parent().position();trackMenu[d.levelNumber].standalone=!0;$(".trackLevel"+d.levelNumber).css("top",a.top).css("left",a.left+-561);$(".trackLevel"+d.levelNumber+" span#selectedViewName").html(d.currentView.Name);$(".trackLevel"+d.levelNumber).fadeIn("fast");$("div#selectTrack"+d.levelNumber).show();$("div#addUsrTrack"+d.levelNumber).hide();$("div#addUsrTrack"+d.levelNumber).hide();$("div#deleteUsrTrack"+d.levelNumber).hide();setTimeout(function(){trackMenu[d.levelNumber].generateTrackTable()},
250);gtag("event","",{event_category:"clickAddTrack"})}}).on("mouseout",function(){d3.select(this).attr("src","/web/images/icons/addTrack_dark_32.png");d3.select("span#mainAddTrack"+d.levelNumber).style("background","#DCDCDC");$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")}).on("mouseover",function(){d3.select(this).attr("src","/web/images/icons/addTrack_light_32.png");d3.select("span#mainAddTrack"+d.levelNumber).style("background","#989898");$("#mouseHelp").html("Click to quickly add other tracks to the current view.")}),
...
100;100>d.scrollSize&&(d.scrollSize=100);changeTrackHeight("Level"+d.levelNumber,d.scrollSize);gtag("event","",{event_category:"changeScrollHeight"})}).on("mouseout",function(){$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")}).on("mouseover",function(){$("#mouseHelp").html("Decrease the vertical length of the scrollable browser image on the page.")}),a.append("span").attr("id","pan"+d.levelNumber).style("height","24px").style("display","inline-block").style("cursor",
...
d3.select("span#reset"+d.levelNumber).style("background","#989898"),$("#scrollSize"+d.levelNumber).show(),changeTrackHeight("Level"+d.levelNumber,d.scrollSize)):(d3.select(this).attr("src","/web/images/icons/no_scroll.png"),d3.select("span#reset"+d.levelNumber).style("background","#DCDCDC"),$("#scrollSize"+d.levelNumber).hide(),changeTrackHeight("Level"+d.levelNumber,0));gtag("event","",{event_category:"toggleImageScroll"})}).on("mouseover",function(){$("#mouseHelp").html("Click to toggle browser image scrolling on/off.  <b>Off</b> the image takes as much space as needed. <b>On</b> you can adjust the maximum length of the image.")}).on("mouseout",
...
h,a,b):"heartilluminaSmall"===a?m=HeartIlluminaSmallTrack(g,h,a,b):-1<a.indexOf("illuminaTotal")&&(m=StrainSpecificIlluminaTotalTrack(g,h,a,b));if(99===g.levelNumber){if(0!==g.updateTimeoutHandle[a]){clearTimeout(g.updateTimeoutHandle[a]);try{clearTimeout(g.timeoutTrack[a].fullDataTimeOutHandle)}catch(r){}g.updateTimeoutHandle[a]=0}g.updateTimeoutHandle[a]=setTimeout(function(){m.updateFullData(0,1);g.timeoutTrack[a]=m;g.updateTimeoutHandle[a]=0},300)}g.addTrackList(m)}else if(0===a.indexOf("repeatMask"))m=
RepeatMaskTrack(g,h,a,"Repeat Masker",1,c),g.addTrackList(m);else if(-1<a.indexOf("custom"))if(b=trackInfo[a],c="DataFile="+b.Location+","+c,"bed"===b.Type||"bb"===b.Type)h=[],m=CustomTranscriptTrack(g,h,a,b.Name,3,c),g.addTrackList(m);else{if("bg"===b.Type||"bw"===b.Type)h=[],m=CustomCountTrack(g,h,a,3,c),g.addTrackList(m)}else 0<=a.indexOf("cirRNA")&&(b=trackInfo[a],h=[],m=CircRNATrack(g,h,a,b.Name,3,c),g.addTrackList(m));$(".sortable"+g.levelNumber).sortable("refresh")};g.addTrackList=function(a){null!=
a&&(g.trackList[g.trackCount]=a,g.trackCount++,setTimeout(function(){DisplayRegionReport()},300))};g.changeTrackHeight=function(a,b){0<b?d3.select("#"+a+"Scroll").style("max-height",b+"px"):d3.select("#"+a+"Scroll").style("max-height","none")};g.removeAllTracks=function(){for(var a=0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a]&&"genomeSeq"!==g.trackList[a].trackClass&&d3.select("#Level"+g.levelNumber+g.trackList[a].trackClass).remove();g.trackList=[];setTimeout(function(){DisplayRegionReport()},
300)};g.removeTrack=function(a){d3.select("#Level"+g.levelNumber+a).remove();for(var b=0;b<g.trackList.length;b++)"undefined"!==typeof g.trackList[b]&&g.trackList[b].trackClass==a&&(g.trackList.splice(b,1),g.trackCount--);setTimeout(function(){DisplayRegionReport()},300)};g.redrawTrack=function(a){for(var b=0;b<g.trackList.length;b++)"undefined"!==typeof g.trackList[b]&&g.trackList[b].trackClass==a&&g.trackList[b].redraw();setTimeout(function(){DisplayRegionReport()},300)};g.redraw=function(){for(var a=
0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a]&&"undefined"!==typeof g.trackList[a].redraw&&g.trackList[a].redraw()};g.update=function(){for(var a=0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a].update&&g.trackList[a].update();setTimeout(function(){DisplayRegionReport()},300)};g.updateData=function(){for(var a=0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a]&&"undefined"!==typeof g.trackList[a].updateData&&g.trackList[a].updateData(0);g.updateFullData();
setTimeout(function(){DisplayRegionReport()},300)};g.updateFullData=function(){-1<(new String(g.folderName)).indexOf("img")&&(g.folderName="/"+chr+"/"+g.xScale.domain()[0]+"_"+g.xScale.domain()[1]);for(var a=0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a]&&"undefined"!==typeof g.trackList[a].updateFullData&&g.trackList[a].updateFullData(0,1)};g.setLoading=function(){for(var a=0;a<g.trackList.length;a++)"undefined"===typeof g.trackList[a]||"undefined"===typeof g.trackList[a].updateData&&
"undefined"===typeof g.trackList[a].updateFullData||g.trackList[a].showLoading()};g.clearSelection=function(){for(var a=0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a]&&"undefined"!==typeof g.trackList[a].clearSelection&&g.trackList[a].clearSelection()};g.mdown=function(){if(0==processAjax){g.prevMinCoord=g.xScale.domain()[0];g.prevMaxCoord=g.xScale.domain()[1];var a=d3.mouse(g.vis.node());g.downx=g.xScale.invert(a[0]);g.downscalex=g.xScale}};g.forceDrawAs=function(a){for(var b=0;b<
g.trackList.length;b++)"undefined"!==typeof g.trackList[b]&&"undefined"!==typeof g.trackList[b].drawAs&&(g.trackList[b].drawAs=a,g.trackList[b].draw(g.trackList[b].data));g.forceDrawAsValue=a};g.generateSettingsString=function(){ret="";for(var a=0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a]&&"undefined"!==typeof g.trackList[a].generateTrackSettingString&&(ret+=g.trackList[a].generateTrackSettingString());return ret};g.setCurrentViewModified=function(){"undefined"!==typeof g.currentView&&
(g.currentView.modified=1,$("span#viewModifiedCtl"+g.levelNumber).show())};g.clearCurrentView=function(){g.currentView.modified=0;$("span#viewModifiedCtl"+g.levelNumber).hide()};g.type=a;g.div=f;g.margin=0;g.halfWindowWidth=$(window).width()/2;g.mh=400;g.downx=Math.NaN;g.downscalex;g.downPanx=Math.NaN;g.xMax=29E7;g.xMin=1;g.prevMinCoord=l;g.prevMaxCoord=n;g.dataMinCoord=l;g.dataMaxCoord=n;g.y=0;g.xScale=null;g.xAxis=null;g.vis=null;g.level=null;g.svg=null;g.txType=null;g.txList=null;g.tt=null;g.trackList=
[];g.trackCount=0;g.levelNumber=c;g.selectedData=null;g.txType=null;g.width=e;g.mw=g.width-g.margin;g.vis=d3.select(f);g.topDiv=g.vis.append("div").attr("id","Level"+c).style("text-align","left");g.xScale=d3.scaleLinear().domain([l,n]).range([0,g.width]);g.xAxis=d3.axisTop(g.xScale).ticks(3).tickSize(8).tickPadding(10);g.scaleSVG=g.topDiv.append("svg:svg").attr("width",g.width).attr("height",60).attr("class","scale");g.scaleSVG.append("g").attr("class","x axis").attr("transform","translate(0,55)").attr("shape-rendering",
"crispEdges").call(g.xAxis);d3.select("#Level"+g.levelNumber).select(".x.axis").append("text").text(b).attr("x",(g.width-2*g.margin)/2).attr("y",-40).attr("class","axisLbl");g.topLevel=g.topDiv.append("div").attr("id","ScrollLevel"+g.levelNumber).style("width",g.width+18+"px").append("ul").attr("id","sortable"+g.levelNumber);return g}
function selectionSVG(f,e,l,n){var c={};c.imageWidth=e+25;c.levelNumber=l;c.parent=n;c.coordSelectStart=0;c.coordSelectStop=0;c.visible=!1;c.changeSelection=function(b,a){c.start=b;c.stop=a;c.setVis(!0);c.draw()};c.setVis=function(b){c.visible=b;c.visible?(c.svg.style("display","inline-block"),$("div#regionDiv").hide()):(c.svg.style("display","none"),$("div#regionDiv").show())};c.draw=function(){console.log("selectionSVG.draw"+c.trackClass);if(c.visible){c.xScale(c.stop);c.xScale(c.start);c.parent.updateTrackSelectedArea(c.start,
c.stop);var b=new String(numberWithCommas(Math.floor(c.start))),a=new String(numberWithCommas(Math.floor(c.stop)));c.svg.selectAll("line").remove();c.svg.selectAll("text").remove();c.svg.append("line").attr("x1",function(){var a=c.xScale(c.start);0>a&&(a=0);return a}).attr("x2",function(){var a=c.xScale(c.start);0>a&&(a=0);return a}).attr("y1",0).attr("y2",15).attr("stroke","#00992D").attr("stroke-width","1");c.svg.append("line").attr("x1",function(){return c.xScale(c.stop)}).attr("x2",function(){return c.xScale(c.stop)}).attr("y1",
...
c.gsvg.redraw(),0==c.gsvg.levelNumber||1==c.gsvg.levelNumber)&&(a={},a.chr=chr,a.start=g,a.stop=b,history[c.gsvg.levelNumber]||(history[c.gsvg.levelNumber]=[]),history[c.gsvg.levelNumber].push(a))};c.clearSelection=function(){c.selectionStart=-1;c.selectionEnd=-1;c.svg.selectAll("rect.selectedArea").remove();c.svg.selectAll(".selected").each(function(){c.svg.select(this).attr("class","").style("fill",c.color)})};c.setSelectedArea=function(b,a){c.selectionStart=b;c.selectionEnd=a;c.redrawSelectedArea()};
c.redrawSelectedArea=function(){d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll(".selectedArea").remove();if(-1<c.selectionStart&&-1<c.selectionEnd){var b=c.xScale(c.selectionStart),a=c.xScale(c.selectionEnd)-b;1>a&&(a=1);d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).append("rect").attr("class","selectedArea").attr("x",b).attr("y",0).attr("height",function(){var a=c.svg.attr("height");15>a&&(a=15);return a}).attr("width",a).attr("fill","#CECECE").attr("opacity",.3).attr("pointer-events",
"none")}};c.colorStroke=function(b){var a="black";3>c.xScale(b.getAttribute("stop"))-c.xScale(b.getAttribute("start"))&&(a=c.color(b));return a};c.calcY=function(b,a,g){b=3===c.density||"3"===c.density?c.calcYPack(b,a,g):2===c.density||"2"===c.density?c.calcYFull(g):c.calcYDense();c.trackYMax<b/15&&(c.trackYMax=b/15);return b};c.calcYPack=function(b,a,g){if(b>=c.xScale.domain()[0]&&b<=c.xScale.domain()[1]||a>=c.xScale.domain()[0]&&a<=c.xScale.domain()[1]||b<=c.xScale.domain()[0]&&a>=c.xScale.domain()[1]){b=
Math.round(c.xScale(b));0>b&&(b=0);a=Math.round(c.xScale(a));a>=c.gsvg.width&&(a=c.gsvg.width-1);g=b-c.xPadding;0>g&&(g=0);var d=a+c.xPadding;d>=c.gsvg.width&&(d=c.gsvg.width-1);for(var p=0,m=g;m<=d;m++)c.yMaxArr[m]>p&&(p=c.yMaxArr[m]);p++;var h=p;if(c.yArr.length<=p)for(c.yArr[h]=[],m=0;m<c.gsvg.width;m++)c.yArr[h][m]=0;m=p-c.scanBackYLines;1>m&&(m=1);h=0;for(var q=m;q<p&&0==h;q++){var e=0;for(m=g;m<=d&&0==e;m++)c.yArr[q][m]>e&&(e=1);0==e&&(p=q,h=1)}p>c.trackYMax&&(c.trackYMax=p);for(m=b;m<=a;m++)c.yMaxArr[m]<
p&&(c.yMaxArr[m]=p),c.yArr[p][m]=1;b=15*p}else b=15;return b};c.calcYDense=function(){return 15};c.calcYFull=function(b){return 15*(b+1)};c.positionTTLeft=function(b){b+=20;b>c.gsvg.halfWindowWidth&&(b-=490);return b+"px"};c.positionTTTop=function(b){b=window.pageYOffset+10;var a=window.innerHeight-1.8*$(".testToolTip").height();20<a&&(b+=a/2);return b+"px"};c.drawLegend=function(b){var a=new String(c.label),g=c.gsvg.width/2+a.length/2*6.5;500>c.gsvg.width&&(g=7*a.length);d3.select("#Level"+c.gsvg.levelNumber+
c.trackClass).selectAll(".legend").remove();for(var d=0;d<b.length;d++)c.svg.append("rect").attr("class","legend").attr("x",g).attr("y",0).attr("rx",3).attr("ry",3).attr("height",12).attr("width",16).attr("fill",b[d].color).attr("stroke",b[d].color),a=new String(b[d].label),c.svg.append("text").text(a).attr("class","legend").attr("x",g+18).attr("y",12),g=g+25+7*a.length};c.drawScaleLegend=function(b,a,g,d,p,m){var h=new String(c.label),q=c.gsvg.width/2+h.length/2*7.5+16;500>c.gsvg.width&&(q=7.5*h.length);
q+=m;d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll(".legend").remove();d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("#def1").remove();h=c.svg.append("defs").attr("id","def1").append("linearGradient").attr("id","grad"+c.gsvg.levelNumber+c.trackClass);h.append("stop").attr("offset","0%").style("stop-color",d);h.append("stop").attr("offset","100%").style("stop-color",p);h=new String(b);b=7.6*h.length;c.svg.append("text").text(h).attr("class","legend").attr("x",q).attr("y",
12);c.svg.append("rect").attr("class","legend").attr("x",q+b+5).attr("y",0).attr("rx",3).attr("ry",3).attr("height",12).attr("width",75).attr("fill","url(#grad"+c.gsvg.levelNumber+c.trackClass+")");h=new String(a);c.svg.append("text").text(h).attr("class","legend").attr("x",q+b+80).attr("y",12);a=8*h.length+5;h=new String(g);c.svg.append("text").text(h).attr("class","legend").attr("x",q+b+80+a).attr("y",12)};c.showLoading=function(){0<d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("g.loading")?
c.loading=d3.select(c.svg).selectAll("g.loading"):(c.loading=c.svg.append("g").attr("class","loading"),c.loading.append("rect").attr("x",0).attr("y",0).attr("height",c.svg.attr("height")).attr("width",c.gsvg.width).attr("fill","#CECECE").attr("opacity",.6),c.loading.append("text").text("Loading...").attr("x",c.gsvg.width/2-37.5).attr("y",c.svg.attr("height")/2))};c.hideLoading=function(){c.loading=d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("g.loading");"undefined"!==typeof c.loading&&
c.loading.remove()};c.displayBreakDown=function(b){$(window).width();1E3<$(window).width()&&$(window).width();if("undefined"!==typeof c.counts&&0<c.counts.length){for(var a=1,g=0;g<c.counts.length&&1===a;g++)0<c.counts[g].value&&(a=0);if(0===a){var d=d3.arc().outerRadius(140).innerRadius(0);a=d3.pie().value(function(a){return a.value});b=d3.select(b).append("svg").attr("width",300).attr("height",300).append("g").attr("transform","translate(150,150)").selectAll(".arc").data(a(c.counts)).enter().append("g").attr("class",
"arc");b.append("path").attr("d",d).attr("fill",c.pieColor).on("mouseover",function(a){d3.select(".testToolTip").transition().duration(200).style("opacity",.95);d3.select(".testToolTip").html("Name: "+a.data.names+"<BR>Count: "+a.data.value).style("left",d3.event.pageX+"px").style("top",d3.event.pageY+20+"px");c.triggerTableFilter(a)}).on("mouseout",function(a){d3.select(".testToolTip").transition().delay(500).duration(200).style("opacity",0);c.clearTableFilter(a)});b.append("text").attr("transform",
function(a){return"translate("+d.centroid(a)+")"}).attr("dy",".35em").attr("fill","#FFFFFF").style("text-anchor","middle").text(function(a){var b=a.value;0==a.value&&(b="");return b})}}};c.updateLabel=function(b){c.label=b;b=new String(c.label);600<c.gsvg.width?d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).select("#trkLbl").attr("x",c.gsvg.width/2-b.length/2*7.5).text(b):d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).select("#trkLbl").attr("x",0).text(b);d3.select("#Level"+c.gsvg.levelNumber+
c.trackClass).select(".infoIcon").attr("transform","translate("+(c.gsvg.width-20)+",0)");d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).select(".settings").attr("transform","translate("+(c.gsvg.width-40)+",0)")};c.resize=function(){d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).attr("width",c.gsvg.width);c.updateLabel(c.label);"undefined"!==typeof c.redrawLegend&&c.redrawLegend();c.redraw()};c.triggerTableFilter=function(b){};c.clearTableFilter=function(b){};c.getDisplayID=function(b){return b.getAttribute("ID")};
c.setupToolTipSVG=function(b,a){var g=parseInt(b.getAttribute("start"),10),d=parseInt(b.getAttribute("stop"),10),p=Math.floor((d-g)*a);20>p&&(p=20);var m=g-p;p=d+p;1>m&&(m=1);"undefined"!==typeof c.ttSVGMinWidth&&p-m<c.ttSVGMinWidth&&(m=g-c.ttSVGMinWidth/2,p=d+c.ttSVGMinWidth/2);g=toolTipSVG("div#ttSVG",450,m,p,99,c.getDisplayID(b),"transcript");g.forLevel=c.gsvg.levelNumber;d=[];d.push(b);g.addTrack(c.trackClass,3,"",d);for(d=0;d<c.ttTrackList.length;d++){var h=c.gsvg.getTrackData(c.ttTrackList[d]),
q=[];if("undefined"!==typeof h&&0<h.length){for(var e=0,f=0;f<h.length;f++)if(m<=parseInt(h[f].getAttribute("start"),10)&&parseInt(h[f].getAttribute("start"),10)<=p||parseInt(h[f].getAttribute("start"),10)<=m&&parseInt(h[f].getAttribute("stop"),10)>=m)q[e]=h[f],e++;0<q.length&&g.addTrack(c.ttTrackList[d],3,"DrawTrx",q)}}};c.generateSettingsDiv=function(b){c.savePrevious();var a=trackInfo[c.trackClass];d3.select(b).select("table").select("tbody").html("");if("undefined"!==typeof a&&0<a.Controls.length&&
"null"!=a.Controls){var g=(new String(a.Controls)).split(",");b=d3.select(b).select("table").select("tbody");b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+a.Name);for(a=0;a<g.length;a++)if("undefined"!==typeof g[a]&&""!=g[a]){var d=g[a].split(";"),p=b.append("tr").append("td"),m=d[0].substr(5);p.append("text").text(m+": ");m="";3<d.length&&0==d[3].indexOf("Default=")&&(m=d[3].substr(8));if(0==d[1].toLowerCase().indexOf("select")){var h=d[1].split(":");d=d[2].split("}");
var q="",e="";2<h.length&&(q=h[2]);3<h.length&&(e=h[3]);p=p.append("select").attr("id",c.trackClass+q+c.level+e).attr("name",h[1]);for(h=0;h<d.length;h++){var f=d[h].substr(1).split(":");if(2==f.length){var l=p.append("option").attr("value",f[1]).text(f[0]);"Dense"===q&&f[1]==c.density?l.attr("selected","selected"):"Version"===q&&f[1]==c.dataVer?l.attr("selected","selected"):f[1]==m&&l.attr("selected","selected")}}d3.select("select#"+c.trackClass+q+c.level+e).on("change",function(){c.updateSettingsFromUI();
c.redraw()})}else console.log("Undefined track settings:  "+g[a])}g=b.append("tr").append("td");g.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");c.gsvg.setCurrentViewModified();c.gsvg.removeTrack(c.trackClass);var a=svgList[c.gsvg.levelNumber].currentView.ViewID,b=viewMenu[c.gsvg.levelNumber].findTrackByClass(c.trackClass,a);b=viewMenu[c.gsvg.levelNumber].findTrackIndexWithViewID(b.TrackID,
a);viewMenu[c.gsvg.levelNumber].removeTrackWithIDIdx(b,a)});g.append("input").attr("type","button").attr("value","Apply").style("float","right").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");c.density!=c.prevSetting.density&&c.gsvg.setCurrentViewModified()});g.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left","5px").on("click",function(){c.revertPrevious();c.draw(c.data);$("#trackSettingDialog").fadeOut("fast")})}else a&&
(b=d3.select(b).select("table").select("tbody"),b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+a.Name),b.append("tr").append("td").html("Sorry no settings for this track."),g=b.append("tr").append("td"),g.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");c.gsvg.setCurrentViewModified();c.gsvg.removeTrack(c.trackClass)}),g.append("input").attr("type",
...
"G"};c.translate=function(b){b=b.toUpperCase();return c.codons[b]};c.translateRegion=function(b,a,g,d){var p=[],m=0,h=0;if(c.data){if(g>d){h=1;var q=g;g=d;d=q}for(q=g;q<=d;q+=3){var e=c.data.substring(q,q+3);1==h&&(e=c.reverseCompliment(e));p[m]=[];p[m].aa=c.translate(e);p[m].pos=a+(q-g);p[m].id=b+":"+q+":"+c.translate(e);m++}}return p};c.reverseCompliment=function(b){for(var a="",g=b.length-1;-1<g;g--)a+=c.complement(b.charAt(g));return a};c.complement=function(b){var a="N";"A"==b?a="T":"T"==b?a=
...
c.updateData(0):c.draw(c.data);else if(b<c.seqRegionMin||a>c.seqRegionMax)c.updateData(0);else{var g=a-b,d="12px";200<g&&(d="10px");if(g<=c.aaDispCutoff&&1==c.includeAA||g<=c.dispCutoff){if(c.seqRegionMin<=b&&b<=c.seqRegionMax&&c.seqRegionMin<=a&&a<=c.seqRegionMax){var p=c.gsvg.width/g,m=p/2;a=b-c.seqRegionMin;var h=g+a,q=27;1==c.includeAA&&"-"!=c.strands&&(q+=30);if(g<c.dispCutoff){d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("g.line").remove();for(var e=[],f=a;f<h;f++)e[f-a]=[],
e[f-a].base=c.data.charAt(f),e[f-a].id=f,e[f-a].pos=f-a;c.textSize(g);var l=c.yPosition(g);f=c.svg.selectAll(".base").data(e,keyID).attr("transform",function(a,b){return"translate("+(a.pos*p-m)+","+q+")"});e=f.enter().append("g").attr("class","base").attr("transform",function(a,b){return"translate("+(a.pos*p-m)+","+q+")"});e.each(function(a){d3.select(this).append("text").text(function(a){return"-"!=c.strands?a.base:c.complement(a.base)});"both"==c.strands&&d3.select(this).append("text").attr("class",
"comp").attr("y",l).text(function(a){return c.complement(a.base)})});f.exit().remove();f.selectAll("text").attr("font-size",function(a){return c.textSize(g)});f.selectAll("text.comp").attr("y",l)}else d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("g.base").remove(),c.svg.append("g").attr("class","line").append("line").attr("stroke","#000000").attr("x1",0).attr("y1",q).attr("x2",c.gsvg.width).attr("y2",q);if(1==c.includeAA){var n=3*p,u=n/2-4,v=25;if("both"==c.strands||"+"==c.strands)for(f=
b%3,e=a-f,h=[],h[0]=c.translateRegion(0,b-f,e,e+g),h[1]=c.translateRegion(1,b-f-1,e-1,e-1+g),h[2]=c.translateRegion(2,b-f-2,e-2,e-2+g),v=25,f=0;3>f;f++){var y=c.svg.selectAll(".aa"+f).data(h[f],keyID).attr("transform",function(a,b){return"translate("+(c.xScale(a.pos)-p)+","+v+")"}).each(function(a){d3.select(this).select("rect").attr("width",n);g<c.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",u).attr("font-size",d).text(function(a){return a.aa}):d3.select(this).select("text").attr("x",
u).attr("font-size",d):d3.select(this).select("text").remove()});e=y.enter().append("g").attr("class","aa"+f).attr("transform",function(a,b){return"translate("+(c.xScale(a.pos)-p)+","+v+")"});e.each(function(a){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",n).attr("stroke",c.colorAA).attr("fill",c.colorAA);g<c.dispCutoff&&d3.select(this).append("text").attr("x",u).attr("font-size",d).text(function(a){return a.aa})});y.exit().remove();v+=11}v="both"==c.strands?80:45;if("both"==
c.strands||"-"==c.strands)for(f=b%3,e=a-f,h=[],h[0]=c.translateRegion(0,b-f,e+g,e),h[1]=c.translateRegion(1,b-f-1,e-1+g,e-1),h[2]=c.translateRegion(2,b-f-2,e-2+g,e-2),f=0;3>f;f++)y=c.svg.selectAll(".aarev"+f).data(h[f],keyID).attr("transform",function(a,b){return"translate("+(c.xScale(a.pos)-p)+","+v+")"}).each(function(a){d3.select(this).select("rect").attr("width",n);g<c.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",u).attr("font-size",d).text(function(a){return a.aa}):
d3.select(this).select("text").attr("x",u).attr("font-size",d):d3.select(this).select("text").remove()}),e=y.enter().append("g").attr("class","aarev"+f).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}),e.each(function(a){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",n).attr("stroke",c.colorAA).attr("fill",c.colorAA);g<c.dispCutoff&&d3.select(this).append("text").attr("x",u).attr("font-size",d).text(function(a){return a.aa})}),y.exit().remove(),
v+=11;"both"==c.strands?c.svg.attr("height",125):c.svg.attr("height",75)}else"both"==c.strands?c.svg.attr("height",45):c.svg.attr("height",30)}else c.updateData(0);$("li.draggable"+c.gsvg.levelNumber+"#li"+c.trackClass).show()}else c.svg.attr("height",0),$("li.draggable"+c.gsvg.levelNumber+"#li"+c.trackClass).hide()}};c.draw=function(b){c.redrawSelectedArea();var a=c.xScale.domain()[0],g=c.xScale.domain()[1]-a,d="";1==c.includeAA&&(d=" and Amino Acids");"both"==c.strands?c.label=c.labelBase+d+" on +/- strands":
...
15).text("<--"),c.svg.append("text").attr("class","dir").attr("x",c.gsvg.width-60).attr("y",15).text("<--"));if(g<c.dispCutoff&&1<b.length){for(var q=[],e=d;e<m;e++)q[e-d]=[],q[e-d].base=b.charAt(e),q[e-d].id=e,q[e-d].pos=e-d;e=c.textSize(g);var f=c.yPosition(g),l=27;1==c.includeAA&&"-"!=c.strands&&(l+=30);q=c.svg.selectAll(".base").data(q,keyID).attr("transform",function(a,b){return"translate("+a.pos*h+","+l+")"});b=q.enter().append("g").attr("class","base").attr("transform",function(a,b){return"translate("+
a.pos*h+","+l+")"});b.each(function(a){d3.select(this).append("text").text(function(b){return"-"!=c.strands?a.base:c.complement(a.base)});"both"==c.strands&&d3.select(this).append("text").attr("class","comp").attr("y",f).text(function(b){return c.complement(a.base)})});q.exit().remove();q.selectAll("text").attr("font-size",e)}else c.svg.append("g").attr("class","base").append("line").attr("stroke","#000000").attr("x1",0).attr("y1",57).attr("x2",c.gsvg.width).attr("y2",57);if(c.data&&1==c.includeAA){var n=
3*h,u=n/2-4,v=25;if("both"==c.strands||"+"==c.strands)for(e=a%3,b=d-e,q=[],q[0]=c.translateRegion(0,a-e,b,b+g),q[1]=c.translateRegion(1,a-e-2,b-2,b-2+g),q[2]=c.translateRegion(2,a-e-1,b-1,b-1+g),v=25,e=0;3>e;e++)m=c.svg.selectAll(".aa"+e).data(q[e],keyID).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}).each(function(a){d3.select(this).select("rect").attr("width",n);g<c.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",u).attr("font-size",
p).text(function(b){return a.aa}):d3.select(this).select("text").attr("x",u).attr("font-size",p):d3.select(this).select("text").remove()}),m.enter().append("g").attr("class","aa"+e).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}).each(function(a){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",n).attr("stroke",c.colorAA(a)).attr("fill",c.colorAA(a));g<c.dispCutoff&&d3.select(this).append("text").attr("x",u).attr("font-size",p).text(function(){return a.aa})}),
m.exit().remove(),v+=11;v="both"==c.strands?80:45;if("both"==c.strands||"-"==c.strands)for(e=a%3,b=d-e,q=[],q[0]=c.translateRegion(0,a-e,b+g,b),q[1]=c.translateRegion(1,a-e-1,b-1+g,b-1),q[2]=c.translateRegion(2,a-e-2,b-2+g,b-2),e=0;3>e;e++)m=c.svg.selectAll(".aarev"+e).data(q[e],keyID).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}),b=m.enter().append("g").attr("class","aarev"+e).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}),b.each(function(a){d3.select(this).append("rect").attr("y",
-10).attr("height",10).attr("width",n).attr("stroke",c.colorAA(a)).attr("fill",c.colorAA(a));g<c.dispCutoff&&d3.select(this).append("text").attr("x",u).attr("font-size",p).text(function(b){return a.aa})}),m.exit().remove(),v+=11;"both"==c.strands?c.svg.attr("height",125):c.svg.attr("height",75)}else"both"==c.strands?c.svg.attr("height",45):c.svg.attr("height",30);c.prevStrand=c.strands;c.prevIncldAA=c.includeAA;$("li.draggable"+c.gsvg.levelNumber+"#li"+c.trackClass).show()}else c.svg.attr("height",
...
data:{chromosome:chr,minCoord:c.tmpseqRegionMin,maxCoord:c.tmpseqRegionMax,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:c.trackClass,folder:c.gsvg.folderName},dataType:"json",success:function(a){gtag("event","generateTrackSequence",{event_category:"browser"})},error:function(a,b,c){}})}3>b?(g=1E4,1===b&&(g=15E3),setTimeout(function(){c.updateData(b+1)},g)):(c.seqRegionMin=0,c.seqRegionMax=0,c.hideLoading(),d3.select("#Level"+c.gsvg.levelNumber+
...
if(0===d[1].toLowerCase().indexOf("select")){p.append("text").text(m+": ");m=d[1].split(":");d=d[2].split("}");for(var h=c.trackClass+c.level+"Select",q=p.append("select").attr("id",h).attr("name",m[1]),e=0;e<d.length;e++){var f=d[e].substr(1).split(":");2==f.length&&(p=q.append("option").attr("value",f[1]).text(f[0]),f[1]==c.strands&&p.attr("selected","selected"))}d3.select("select#"+h).on("change",function(){c.updateSettingsFromUI();c.redraw()})}else if(0===d[1].toLowerCase().indexOf("cbx"))for(p.append("text").text(m),
m=d[1].split(":"),d=d[2].split("}"),e=0;e<d.length;e++)if(f=d[e].substr(1).split(":"),2===f.length){h=p.append("div").style("display","inline-block");var l="",n="";2<m.length&&(n=m[2]);3<m.length&&(l=m[3]);q=h.append("input").attr("type","checkbox").attr("id",c.trackClass+n+"CBX"+c.level+l).attr("name",m[1]).style("margin-left","5px");h.append("text").text(f[0]);1==c.includeAA?$("#"+c.trackClass+n+"CBX"+c.level+l).prop("checked",!0):$("#"+c.trackClass+n+"CBX"+c.level+l).prop("checked",!1);d3.select("input#"+
...
(h=a.parent.getAttribute("geneSymbol"));d=".";1==a.getAttribute("strand")?d="+":-1==a.getAttribute("strand")&&(d="-");if(a.parent){p=getAllChildrenByName(getFirstChildByName(a.parent,"TranscriptList"),"Transcript");for(var q=0;q<p.length;q++){var e=new String(p[q].getAttribute("ID"));m!=e&&(-1<(new String(p[q].getAttribute("ID"))).indexOf("ENS")?c+="<B>"+e+"</B>":(c+="<B>"+e+"</B>",-1==(new String(p[q].getAttribute("ID"))).indexOf("ENS")&&(e=getFirstChildByName(getFirstChildByName(p[q],"annotationList"),
"annotation"),null!=e&&(c+=" - "+e.getAttribute("reason")))),c+="<br>");if(""==h){e=getAllChildrenByName(getFirstChildByName(p[q],"annotationList"),"annotation");for(var f=0;f<e.length&&""==h;f++)"AKA"==e[f].getAttribute("source")&&(h=e[f].getAttribute("annot_value"),h=(new String(h)).split(":"),h=0!=(new String(h[h.length-1])).indexOf("ENS")?h[h.length-1]:h[0])}}p='<BR><div id="ttSVG" style="background:#FFFFFF;overflow:"></div><BR>Transcript ID: '+m+"<BR>Location: "+a.getAttribute("chromosome")+
":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+d;p=p+"<BR><BR>Gene Symbol: "+h+"<BR>Gene ID: "+b.getDisplayID(a.parent.getAttribute("ID"))+"<BR><BR>Other Transcripts:<BR>"+c+"<BR>"}else{p=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(q=0;q<p.length;q++)if(e=new String(p[q].getAttribute("ID")),-1<(new String(p[q].getAttribute("ID"))).indexOf("ENS")?c+="<B>"+e+"</B>":(c+="<B>"+e+"</B>",-1===(new String(p[q].getAttribute("ID"))).indexOf("ENS")&&
(e=getFirstChildByName(getFirstChildByName(p[q],"annotationList"),"annotation"),null!=e&&(c+=" - "+e.getAttribute("reason")))),c+="<br>",0===h.length)for(e=getAllChildrenByName(getFirstChildByName(p[q],"annotationList"),"annotation"),f=0;f<e.length&&0===h.length;f++)"AKA"===e[f].getAttribute("source")&&(h=e[f].getAttribute("annot_value"),h=(new String(h)).split(":"),h=0!==(new String(h[h.length-1])).indexOf("ENS")?h[h.length-1]:h[0]);p='<BR><div id="ttSVG" style="background:#FFFFFF;overflow:scroll;max-height:400px;"></div>Gene ID: '+
...
return c}).attr("stroke",b.colorStroke),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").each(function(a){var c=d3.select(this),d=parseInt(a.getAttribute("strand"),10);if(-1===d||1===d){var g=">";-1===d&&(g="<");d="";var e=c.select("rect").attr("width");if(8.3<=e&&15.8>=e)d=g;else for(;8.5<e;)d+=g,e-=8.5;c.select("text#strandTxt").text(d)}if(2===b.density){var f=a.getAttribute("ID");a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length&&(f=f+" ("+a.getAttribute("geneSymbol")+
")");0===c.select("text#lblTxt").size()?c.append("svg:text").attr("dx",function(){var c=b.xScale(a.getAttribute("start"));return c=c<$(window).width()/2?b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5:-9*f.length}).attr("dy",10).attr("id","lblTxt").text(f):c.select("text#lblTxt").attr("dx",function(){var c=b.xScale(a.getAttribute("start"));return c=c<$(window).width()/2?b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5:-9*f.length})}else c.select("text#lblTxt").remove()})),
...
(d=parseInt(a.getAttribute("cdsStart"),10),g=parseInt(a.getAttribute("cdsStop"),10));exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");var p="";1===b.gsvg.levelNumber?p="tx":99===b.gsvg.levelNumber&&(p="ttTx");for(var e=0;e<exList.length;e++){var f=exList[e].getAttribute("start"),l=exList[e].getAttribute("stop");if(f<d&&d<l||l>g&&g>f){var n=f,u=d;l>g?(n=g,u=l,l=g):f=d;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+
" rect#ExNC"+exList[e].getAttribute("ID")).attr("x",function(a){return b.xScale(n)}).attr("width",function(a){return b.xScale(u)-b.xScale(n)})}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" rect#Ex"+exList[e].getAttribute("ID")).attr("x",function(a){return b.xScale(f)}).attr("width",function(a){return b.xScale(l)-b.xScale(f)});if(0<e){var v=">";"-1"==a.getAttribute("strand")&&(v="<");var y=v,E=b.xScale(exList[e-1].getAttribute("stop")),
F=b.xScale(exList[e].getAttribute("start")),B=F-E,D=0;if(7>B||"0"==a.getAttribute("strand")||"."==a.getAttribute("strand"))y="";else for(B-=7;8<B;)0===D?(y+=" ",D=1):(y+=v,D=0),B-=7;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" line#Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("x1",E).attr("x2",F);d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+
p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" #IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("dx",E+1).text(y)}}if(2===b.density){var C=a.getAttribute("ID");a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length&&(C=C+" ("+a.getAttribute("geneSymbol")+")");0===d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" text#lblTxt").size()?d3.select("#Level"+b.gsvg.levelNumber+
...
b.gsvg.levelNumber+".selected").selectAll("rect").style("fill",b.color),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("text").style("opacity","0.6").style("fill",b.color),b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").each(function(){var a=new String($(this).attr("class"));a=a.replace(" selected","");$(this).attr("class",a)}),b.svg.selectAll("g.gene"+a).each(function(){var a=$(this).attr("class")+" selected";$(this).attr("class",a)}),b.svg.selectAll("g.gene"+a).selectAll("line").style("stroke",
...
a=a.replace(" selected","");$(this).attr("class",a)}))};b.setupDetailedView=function(a){if(a){b.gsvg.clearSelection();jQuery.Event("keyup").which=32;var c=b.gsvg.levelNumber+1;$("div#collapsableReport").is(":hidden")||($("div#collapsableReport").hide(),$("span[name='collapsableReport']").removeClass("less"));$("div#selectedDetailHeader").is(":hidden")&&$("div#selectedDetailHeader").show();$("div#selectedDetail").is(":hidden")&&$("div#selectedDetail").show();var d="none";0===b.trackClass.indexOf("liverTotal")?
d=b.trackClass:"heartTotal"===b.trackClass?d="heartTotal":0===b.trackClass.indexOf("brainTotal")?d=b.trackClass:"mergedTotal"===b.trackClass?d="mergedTotal":"protein_coding"===a.getAttribute("biotype")&&200<=a.getAttribute("stop")-a.getAttribute("start")?d="protein":"protein_coding"!==a.getAttribute("biotype")&&200<=a.getAttribute("stop")-a.getAttribute("start")?d="long":200>a.getAttribute("stop")-a.getAttribute("start")&&(d="small");var p=0,m=0;if("protein"===d||"long"===d||0===d.indexOf("liverTotal")||
"heartTotal"===d||0===d.indexOf("brainTotal")||"mergedTotal"===d||"small"===d&&"rn5"!==genomeVer||"small"===d&&-1<(new String(a.getAttribute("ID"))).indexOf("ENS")){console.log("totalType");p=a.getAttribute("ID");var h="";if(a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length)p=p+" ("+a.getAttribute("geneSymbol")+")";else{m=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(var e=0;e<m.length&&""===h;e++)if(""===h)for(var f=getAllChildrenByName(getFirstChildByName(m[e],
"annotationList"),"annotation"),l=0;l<f.length&&0===h.length;l++)if("AKA"===f[l].getAttribute("source")){var n=f[l].getAttribute("annot_value");n=(new String(n)).split(":");1<=n.length?(h=n[0],p=p+" ("+h+")"):2<=n.length&&(h=n[0],n=n[1],p=p+" ("+n+")")}}m=parseInt(a.getAttribute("start"),10);f=parseInt(a.getAttribute("stop"),10);a.getAttribute("extStart")&&a.getAttribute("extStop")&&(m=parseInt(a.getAttribute("extStart"),10),f=parseInt(a.getAttribute("extStop"),10));svgViewIDList[c]||(svgViewIDList[c]=
b.gsvg.currentView?b.gsvg.currentView.ViewID:defaultView);e=GenomeSVG("div#selectedImage",b.gsvg.width,m,f,c,p,"transcript");e.xMin=m-.05*(f-m);e.xMax=f+.05*(f-m);p=e.xMin;m=e.xMax;svgList[c]=e;svgList[c].txType=d;svgList[c].selectedData=a;selectedGeneSymbol=a.getAttribute("geneSymbol");b.gsvg.selectedGeneSymbol=a.getAttribute("geneSymbol");selectedID=new String(a.getAttribute("ID"));-1===selectedID.indexOf("ENS")&&0<h.length&&(selectedID=h);$("div#selectedImage").show();console.log("here");-1<(new String(selectedID)).indexOf("ENS")&&
-1<b.trackClass.indexOf("ensembl")?(console.log("geneReport"),$("div#selectedReport").show(),c=pathPrefix+"geneReport.jsp",a={id:selectedID,geneSymbol:selectedGeneSymbol,chromosome:chr,species:organism,genomeVer:genomeVer},DisplaySelectedDetailReport(c,a)):"small"!==d?$("div#selectedReport").html("<BR><BR>Detailed Gene Reports are not currently provided for RNA-Seq(Cufflinks) generated genes that are novel and have not been matched to an existing annotated gene.  In the future overlapping probesets and SNPs will be summarized in the same manner.  This feature should be added by December 2014.  Please keep checking back or let us know you'd like to see it sooner."):
...
rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:m,folder:b.gsvg.folderName},dataType:"json",success:function(a){gtag("event","generateTrackGene",{event_category:"browser"})},error:function(a,b,c){}})}10>a?(g=2E3,1===a&&(g=5E3),setTimeout(function(){b.updateData(a+1)},g)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).attr("height",
...
panel:p,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:d,folder:b.gsvg.folderName},dataType:"json",success:function(a){gtag("event","generateTrackGene",{event_category:"browser"})},error:function(a,b,c){}})}5>c?(h=1E4,0===c&&(h=5E3),setTimeout(function(){b.updateDataVersion(a,c+1)},h)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).attr("height",
...
m="tx":99===b.gsvg.levelNumber&&(m="ttTx");m=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" #"+m+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber);exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var h=0;h<exList.length;h++){var e=parseInt(exList[h].getAttribute("start"),10),f=parseInt(exList[h].getAttribute("stop"),10);if(e<d&&d<f||f>g&&g>e){var l=0,n=0,w=0,u=0;e<d?(l=b.xScale(e),n=b.xScale(d)-b.xScale(e),w=b.xScale(d),u=b.xScale(f)-b.xScale(d)):f>g&&(w=b.xScale(e),
u=b.xScale(g)-b.xScale(e),l=b.xScale(g),n=b.xScale(f)-b.xScale(g));m.append("rect").attr("x",l).attr("y",2.5).attr("height",5).attr("width",n).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"ExNC"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer");m.append("rect").attr("x",w).attr("height",10).attr("width",u).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).style("fill",
...
n=b.xScale(exList[h].getAttribute("start"))-l;w=0;if(7>n)f="";else for(n-=7;8<n;)0==w?(f+=" ",w=1):(f+=e,w=0),n-=7;m.append("svg:text").attr("id",function(a){return"IntTxt"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("dx",l+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(f)}}if(2==b.density){var v=a.getAttribute("ID");"undefined"!==typeof a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length&&
(v=v+" ("+a.getAttribute("geneSymbol")+")");m.append("svg:text").attr("dx",function(){var c=b.xScale(a.getAttribute("start"));return c=c<$(window).width()/2?b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5:-9*v.length}).attr("dy",10).attr("id","lblTxt").text(v)}else m.select("text#lblTxt").remove()};b.getSVGID=function(a){return b.getDisplayID(a.getAttribute("ID"))+" ("+a.getAttribute("geneSymbol")+")"};b.setupToolTipSVG=function(a,c){if("undefined"!==typeof a.getAttribute("extStart")&&
...
(h+"</B><BR>Status: ")+a.getAttribute("category")+"<BR>Gene Symbol: "+a.getAttribute("geneSymbol")+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+d);return c};b.getSVGID=function(a){return a.getAttribute("geneSymbol")};b.setupToolTipSVG=function(a,c){var d=parseInt(a.getAttribute("start"),10),g=parseInt(a.getAttribute("stop"),10),m=Math.floor((g-d)*c);20>m&&(m=20);var h=d-m;m=g+m;1>h&&(h=1);"undefined"!==
typeof b.ttSVGMinWidth&&m-h<b.ttSVGMinWidth&&(h=d-b.ttSVGMinWidth/2,m=g+b.ttSVGMinWidth/2);d=toolTipSVG("div#ttSVG",450,h,m,99,b.getSVGID(a),"transcript");d.forLevel=b.gsvg.levelNumber;g=[];g.push(a);d.addTrack(b.trackClass,3,"DrawTrx",g);for(g=0;g<b.ttTrackList.length;g++){var e=b.gsvg.getTrackData(b.ttTrackList[g]),f=[];if("undefined"!==typeof e&&0<e.length){for(var l=0,n=0;n<e.length;n++)if(h<=parseInt(e[n].getAttribute("start"),10)&&parseInt(e[n].getAttribute("start"),10)<=m||parseInt(e[n].getAttribute("start"),
10)<=h&&parseInt(e[n].getAttribute("stop"),10)>=h)f[l]=e[n],l++;0<f.length&&d.addTrack(b.ttTrackList[g],3,"DrawTrx",f)}}};b.calcY=function(a,c,d){if(3==b.density)if(a>=b.xScale.domain()[0]&&a<=b.xScale.domain()[1]||c>=b.xScale.domain()[0]&&c<=b.xScale.domain()[1]||a<=b.xScale.domain()[0]&&c>=b.xScale.domain()[1]){a=Math.floor(b.xScale(a));0>a&&(a=0);c=Math.floor(b.xScale(c))+1;c>=b.gsvg.width&&(c=b.gsvg.width-1);d=a-5;0>d&&(d=0);var g=c+5;g>=b.gsvg.width&&(g=b.gsvg.width-1);for(var m=0,h=d;h<=g;h++)b.yMaxArr[h]>
m&&(m=b.yMaxArr[h]);m++;var e=m;if(b.yArr.length<=m)for(b.yArr[e]=[],h=0;h<b.gsvg.width;h++)b.yArr[e][h]=0;h=m-12;1>h&&(h=1);e=0;for(var f=h;f<m&&0==e;f++){var l=0;for(h=d;h<=g&&0==l;h++)b.yArr[f][h]>l&&(l=1);0==l&&(m=f,e=1)}m>b.trackYMax&&(b.trackYMax=m);for(h=a;h<=c;h++)b.yMaxArr[h]<m&&(b.yMaxArr[h]=m),b.yArr[m][h]=1;a=15*m}else a=15;else a=2==b.density?15*(d+1):15;b.trackYMax<a/15&&(b.trackYMax=a/15);return a};b.redraw=function(){var a=b.xScale.domain()[0];a=b.xScale.domain()[1]-a;var c=0;b.yMaxArr=
[];b.yArr=[];b.yArr[0]=[];for(var d=0;d<b.gsvg.width;d++)b.yMaxArr[d]=0,b.yArr[0][d]=0;b.trackYMax=0;if(a<b.trxCutoff&&"Gene"==b.drawnAs||a>=b.trxCutoff&&"Trx"==b.drawnAs&&"Trx"!=b.drawAs||"Gene"==b.drawnAs&&$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.draw(b.data);else if(a<b.trxCutoff&&"Trx"==b.drawnAs&&"Trx"!=b.drawAs&&(c=1),"Gene"==b.drawAs&&0==c)b.svg.node()&&(b.svg.selectAll(".trx").each(function(){d3.select(this).remove()}),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").attr("transform",
function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+","+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"}),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene rect").attr("width",function(a){var c=1;1<b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))&&(c=b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start")));return c}).attr("stroke",b.colorStroke),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").each(function(a){var b=
d3.select(this);if("1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand")){var c=">";if("-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand"))c="<";a="";var d=b.select("rect").attr("width");if(8.5<=d&&15.8>=d)a=c;else for(;8.7<d;)a+=c,d-=8.5;b.select("text").text(a)}})),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").size()+
1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1));else if(1==c||"Trx"==b.drawAs)b.svg.selectAll("g.trx"+b.gsvg.levelNumber).attr("transform",function(a,c){return"translate(0,"+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"}).each(function(a,c){var d=parseInt(a.getAttribute("cdsStart"),10),g=parseInt(a.getAttribute("cdsStop"),10);exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");var m="";1==b.gsvg.levelNumber&&(m="tx");for(var e=
0;e<exList.length;e++){var p=parseInt(exList[e].getAttribute("start"),10),f=parseInt(exList[e].getAttribute("stop"),10);if(p<d&&d<f||f>g&&g>p){var l=p,n=d;f>g?(l=g,n=f,f=g):p=d;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(a.getAttribute("ID"))+" rect#ExNC"+exList[e].getAttribute("ID")).attr("x",function(a){return b.xScale(l)}).attr("width",function(a){return b.xScale(n)-b.xScale(l)})}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(a.getAttribute("ID"))+
" rect#Ex"+exList[e].getAttribute("ID")).attr("x",function(a){return b.xScale(p)}).attr("width",function(a){return b.xScale(f)-b.xScale(p)});if(0<e){var y=b.xScale(exList[e-1].getAttribute("stop")),E=b.xScale(exList[e].getAttribute("start"));if("1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand")){var F=">";"-1"==a.getAttribute("strand")&&(F="<");var B=F,D=E-y,C=0;if(7>D)B="";else for(D-=7;8<D;)0==C?(B+=" ",C=1):(B+=F,C=0),D-=
7;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(a.getAttribute("ID"))+" #IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("dx",y+1).text(B)}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(a.getAttribute("ID"))+" line#Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("x1",y).attr("x2",E)}}}),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+
b.trackClass).selectAll("g.trx"+b.gsvg.levelNumber).size()+1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1));b.redrawSelectedArea()};b.updateData=function(a){var c=b.xScale.domain()[0],d=b.xScale.domain()[1];d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/refSeq.xml",function(g,m){if(g){if(0==a){var h=contextPath+"/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:c,maxCoord:d,
rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,track:b.trackClass,folder:b.gsvg.folderName,panel:panel},dataType:"json",success:function(a){},error:function(a,b,c){}})}10>a?(h=2500,1==a&&(h=1E4),setTimeout(function(){b.updateData(a+1)},h)):(d3.select("#Level"+b.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+
b.trackClass))}else if(m){h=m.documentElement.getElementsByTagName("Gene");for(var e=[],p=[],f=0,l=0;l<h.length;l++)h[l]&&(e[f]=h[l],e[f].setAttribute("ID",f),p[h[l].getAttribute("geneSymbol")]=1,f++);for(l=0;l<b.data.length;l++)b.data[l]&&!p[b.data[l].getAttribute("geneSymbol")]&&(e[f]=b.data[l],e[f].setAttribute("ID",f),f++);b.draw(e);b.hideLoading();b.getDisplayedData();setTimeout(function(){DisplayRegionReport()},300)}else b.updateData(a+1),b.hideLoading()})};b.drawTrx=function(a,c){var d=parseInt(a.getAttribute("cdsStart"),
10),g=parseInt(a.getAttribute("cdsStop"),10),m="";1==b.gsvg.levelNumber&&(m="tx");m=b.svg.select("#"+m+b.cleanID(a.getAttribute("ID")));exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var h=0;h<exList.length;h++){var e=parseInt(exList[h].getAttribute("start"),10),f=parseInt(exList[h].getAttribute("stop"),10);if(e<d&&d<f||f>g&&g>e){var l=0,n=0,w=0,u=0;e<d?(l=b.xScale(e),n=b.xScale(d)-b.xScale(e),w=b.xScale(d),u=b.xScale(f)-b.xScale(d)):f>g&&(w=b.xScale(e),u=b.xScale(g)-b.xScale(e),
l=b.xScale(g),n=b.xScale(f)-b.xScale(g));m.append("rect").attr("x",l).attr("y",2.5).attr("height",5).attr("width",n).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"ExNC"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer");m.append("rect").attr("x",w).attr("height",10).attr("width",u).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor",
"pointer")}else{l=10;n=0;if(e<d&&f<d||f>g&&e>g)l=5,n=2.5;m.append("rect").attr("x",function(a){return b.xScale(exList[h].getAttribute("start"))}).attr("y",n).attr("height",l).attr("width",function(a){return b.xScale(exList[h].getAttribute("stop"))-b.xScale(exList[h].getAttribute("start"))}).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}if(0<h&&(m.append("line").attr("x1",
function(a){return b.xScale(exList[h-1].getAttribute("stop"))}).attr("x2",function(a){return b.xScale(exList[h].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(a){return"Int"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("stroke",b.color).attr("stroke-width","2"),"1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand"))){e=">";if("-"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))e=
"<";f=e;l=b.xScale(exList[h-1].getAttribute("stop"));n=b.xScale(exList[h].getAttribute("start"))-l;w=0;if(7>n)f="";else for(n-=7;8<n;)0==w?(f+=" ",w=1):(f+=e,w=0),n-=7;m.append("svg:text").attr("id",function(a){return"IntTxt"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("dx",l+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(f)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yArr=[];b.yArr[0]=[];
for(var c=0;c<b.gsvg.width;c++)b.yMaxArr[c]=0,b.yArr[0][c]=0;b.svg.selectAll(".gene").remove();b.svg.selectAll(".trx"+b.gsvg.levelNumber).remove();var d=b.drawAs;c=b.xScale.domain()[0];if(b.xScale.domain()[1]-c<b.trxCutoff||$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.drawAs="Trx";if(0!=a&&"Gene"===b.drawAs)b.drawnAs="Gene",b.label="Ref Seq Genes",b.updateLabel(b.label),b.redrawLegend(),a=b.svg.selectAll(".gene").data(a,key).attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+
","+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"}),a.enter().append("g").attr("class","gene").attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+","+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"}).append("rect").attr("height",10).attr("rx",1).attr("ry",1).attr("width",function(a){return b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))}).attr("title",function(a){return a.getAttribute("ID")}).attr("stroke",
...
b.setupToolTipSVG(a,.05));return!1}).on("mouseout",function(a){overSelectable=0;$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.");"selected"!=d3.select(this).attr("class")&&d3.select(this).style("fill",b.color);tt.transition().delay(500).duration(200).style("opacity",0)}),a.exit().remove(),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").each(function(a){if("undefined"!==typeof a){var b=d3.select(this);if("1"==a.getAttribute("strand")||
"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand")){var c=">";if("-"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))c="<";var d=b.select("rect").attr("width");if(8.5>d)a="";else for(d-=8.5,a=c;8.7<d;)a+=c,d-=8.5;b.append("svg:text").attr("dx","1").attr("dy","10").style("pointer-events","none").text(a)}}}),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").size()+
1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1));else if(0!=a&&"Trx"===b.drawAs){b.drawnAs="Trx";b.label="Ref Seq Transcripts";b.updateLabel(b.label);b.redrawLegend();var e=[],m=0;for(c=0;c<a.length;c++)for(var h=getAllChildrenByName(getFirstChildByName(a[c],"TranscriptList"),"Transcript"),f=0;f<h.length;f++)e[m]=h[f],e[m].parent=a[c],m++;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".trx"+b.gsvg.levelNumber).remove();a=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".trx"+
b.gsvg.levelNumber).data(e,key).attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+","+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"});a.enter().append("g").attr("class","trx"+b.gsvg.levelNumber).attr("transform",function(a,c){return"translate(0,"+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"}).attr("id",function(a){var c="";1==b.gsvg.levelNumber&&(c="tx");return c+b.cleanID(a.getAttribute("ID"))}).on("mouseover",
function(a){0==b.gsvg.isToolTip&&(d3.select(this).selectAll("line").style("stroke","green"),d3.select(this).selectAll("rect").style("fill","green"),d3.select(this).selectAll("text").style("opacity","0.3").style("fill","green"),tt.transition().duration(200).style("opacity",1),tt.html(b.createToolTip(a)).style("left",function(){return b.positionTTLeft(d3.event.pageX)}).style("top",function(){return b.positionTTTop(d3.event.pageY)}),"undefined"!==typeof a&&b.setupToolTipSVG(a.parent,.05))}).on("mouseout",
function(a){d3.select(this).selectAll("line").style("stroke",function(a){return b.color(a)});d3.select(this).selectAll("rect").style("fill",function(a){return b.color(a)});d3.select(this).selectAll("text").style("opacity","0.6").style("fill",function(a){return b.color(a)});tt.transition().delay(500).duration(200).style("opacity",0)}).each(b.drawTrx);a.exit().remove();2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.trx"+b.gsvg.levelNumber).size()+
...
">";"-1"==a.getAttribute("strand")&&(g="<");var m="";if(7.5<=d&&15>=d)m=g;else if(15<d)for(d-=7.5;7.5<d;)m+=g,d-=7.5;c.select("text#strand").text(m);if(2==b.density||3==b.density){var p=a.getAttribute("ID");0===c.select("text#lblTxt").size()?c.append("svg:text").attr("dx",function(){var c=b.xScale(a.getAttribute("stop"))+8*p.length+5,d=b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5;c>b.gsvg.width&&(d=-8*p.length);return d}).attr("dy",10).attr("id","lblTxt").text(p):c.select("text#lblTxt").attr("dx",
function(){var c=b.xScale(a.getAttribute("stop"))+8*p.length+5,d=b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5;c>b.gsvg.width&&(d=-8*p.length);return d})}else c.selectAll("text#lblTxt").remove()});d+=b.trackYMax}b.trackYMax=15*d;b.svg.attr("height",b.trackYMax)}else if("annot"===b.colorSelect){b.drawLegend([{color:"#FF0000",label:"Core"},{color:"#0000FF",label:"Extended"},{color:"#006400",label:"Full"},{color:"#000000",label:"Ambiguous"}]);b.trackYMax=0;b.yMaxArr=[];b.yArr=
...
return c}).attr("fill",function(a){return b.color(a,"")});b.svg.selectAll("g.probe").each(function(a){var c=d3.select(this),d=">";"-1"==a.getAttribute("strand")&&(d="<");var g="",e=c.select("rect").attr("width");if(7.5<=e&&15>=e)g=d;else if(15<e)for(e-=7.5;7.5<e;)g+=d,e-=7.5;c.select("text").text(g);if(2==b.density||3==b.density){var m=a.getAttribute("ID");0===c.select("text#lblTxt").size()?c.append("svg:text").attr("dx",function(){var c=b.xScale(a.getAttribute("stop"))+8*m.length+5,d=b.xScale(a.getAttribute("stop"))-
b.xScale(a.getAttribute("start"))+5;c>b.gsvg.width&&(d=-8*m.length);return d}).attr("dy",10).attr("id","lblTxt").text(m):c.select("text#lblTxt").attr("dx",function(){var c=b.xScale(a.getAttribute("stop"))+8*m.length+5,d=b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5;c>b.gsvg.width&&(d=-8*m.length);return d})}else c.selectAll("text#lblTxt").remove()});1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(b.trackYMax+1)):3==b.density&&b.svg.attr("height",15*
...
typeof d[c]&&""!=d[c]){var e=d[c].split(";"),m=a.append("tr").append("td"),h=e[0].substr(5);3<e.length&&0==e[3].indexOf("Default=")&&e[3].substr(8);if(0==e[1].toLowerCase().indexOf("select")){m.append("text").text(h+": ");h=e[1].split(":");e=e[2].split("}");var f=b.trackClass+"Dense"+b.level+"Select";"colorSelect"==h[1]&&(f=b.trackClass+b.level+"colorSelect");for(var l=m.append("select").attr("id",f).attr("name",h[1]),n=0;n<e.length;n++){var r=e[n].substr(1).split(":");2==r.length&&(m=l.append("option").attr("value",
r[1]).text(r[0]),"colorSelect"==h[1]&&r[1]==b.curColor?m.attr("selected","selected"):r[1]==b.density&&m.attr("selected","selected"))}d3.select("select#"+f).on("change",function(){"dabg"==$(this).val()||"herit"==$(this).val()?$("div#affyTissues"+b.level).show():$("div#affyTissues"+b.level).hide();b.updateSettingsFromUI();b.redraw()})}else if(0==e[1].toLowerCase().indexOf("cbx"))for(m=m.append("div").attr("id","affyTissues"+b.level).style("display","none"),m.append("text").text(h+": "),h=e[1].split(":"),
e=e[2].split("}"),n=0;n<e.length;n++)r=e[n].substr(1).split(":"),2==r.length&&(f=m.append("div").style("display","inline-block"),l=f.append("input").attr("type","checkbox").attr("id",r[1]+"CBX"+b.level).attr("name",h[1]).style("margin-left","5px"),f.append("text").text(r[0]),d3.select("input#"+r[1]+"CBX"+b.level).on("change",function(){b.updateSettingsFromUI();b.redraw()}))}"dabg"==b.curColor||"herit"==b.curColor?$("div#affyTissues"+b.level).show():$("div#affyTissues"+b.level).hide();for(d=0;d<b.tissues.length;d++)$("#"+
...
h.append("text").text(f+": ");3<e.length&&0==e[3].indexOf("Default=")&&e[3].substr(8);if(0==e[1].toLowerCase().indexOf("select")){var l=e[1].split(":");e=e[2].split("}");var n=f="";2<l.length&&(f=l[2]);3<l.length&&(n=l[3]);h=h.append("select").attr("id",b.trackClass+f+b.level+n).attr("name",l[1]);for(l=0;l<e.length;l++){var r=e[l].substr(1).split(":");if(2==r.length){var w=h.append("option").attr("value",r[1]).text(r[0]);("Dense"==f&&r[1]==b.density||""==f&&r[1]==b.include)&&w.attr("selected","selected")}}d3.select("#"+
...
"exonList"),"exon");for(var m=0;m<exList.length;m++){var h=parseInt(exList[m].getAttribute("start"),10),f=parseInt(exList[m].getAttribute("stop"),10);if(h<g&&g<f||f>d&&d>h){var l=0,n=0,r=0,w=0;h<g?(l=c.xScale(exList[m].getAttribute("start")),n=c.xScale(g)-c.xScale(exList[m].getAttribute("start")),r=c.xScale(g),w=c.xScale(exList[m].getAttribute("stop"))-c.xScale(g)):f>d&&(r=c.xScale(exList[m].getAttribute("start")),w=c.xScale(d)-c.xScale(exList[m].getAttribute("start")),l=c.xScale(d),n=c.xScale(exList[m].getAttribute("stop"))-
c.xScale(d));e.append("rect").attr("x",l).attr("y",2.5).attr("height",5).attr("width",n).attr("title",function(a){return exList[m].getAttribute("ID")}).attr("id",function(a){return"ExNC"+exList[m].getAttribute("ID")}).style("fill",c.color).style("cursor","pointer");e.append("rect").attr("x",r).attr("height",10).attr("width",w).attr("title",function(a){return exList[m].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[m].getAttribute("ID")}).style("fill",c.color).style("cursor","pointer")}else{l=
10;n=0;if(h<g&&f<g||f>d&&h>d)l=5,n=2.5;e.append("rect").attr("x",function(a){return c.xScale(exList[m].getAttribute("start"))}).attr("y",n).attr("height",l).attr("width",function(a){return c.xScale(exList[m].getAttribute("stop"))-c.xScale(exList[m].getAttribute("start"))}).attr("title",function(a){return exList[m].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[m].getAttribute("ID")}).style("fill",c.color).style("cursor","pointer")}if(0<m){e.append("line").attr("x1",function(a){return c.xScale(exList[m-
1].getAttribute("stop"))}).attr("x2",function(a){return c.xScale(exList[m].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(a){return"Int"+exList[m-1].getAttribute("ID")+"_"+exList[m].getAttribute("ID")}).attr("stroke",c.color).attr("stroke-width","2");h=">";"-1"==b.getAttribute("strand")&&(h="<");f=h;l=c.xScale(exList[m-1].getAttribute("stop"));n=c.xScale(exList[m].getAttribute("start"))-l;r=0;if(7>n)f="";else for(n-=7;8<n;)0==r?(f+=" ",r=1):(f+=h,r=0),n-=7;e.append("svg:text").attr("id",
function(a){return"IntTxt"+exList[m-1].getAttribute("ID")+"_"+exList[m].getAttribute("ID")}).attr("dx",l+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",c.color).style("font-size","16px").text(f)}}};c.redraw=function(){d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("g.trx"+c.gsvg.levelNumber).each(function(b,a){var g=parseInt(b.getAttribute("start"),10),d=parseInt(b.getAttribute("stop"),10);"undefined"!==typeof b.getAttribute("cdsStart")&&"undefined"!==
typeof b.getAttribute("cdsStop")&&(g=parseInt(b.getAttribute("cdsStart"),10),d=parseInt(b.getAttribute("cdsStop"),10));exList=getAllChildrenByName(getFirstChildByName(b,"exonList"),"exon");for(var e=0;e<exList.length;e++){var m=parseInt(exList[e].getAttribute("start"),10),h=parseInt(exList[e].getAttribute("stop"),10);if(m<g&&g<h||h>d&&d>m){var f=m,l=g;h>d?(f=d,l=h,h=d):m=g;d3.select("#Level"+c.gsvg.levelNumber+c.trackClass+" g#tx"+b.getAttribute("ID")+" rect#ExNC"+exList[e].getAttribute("ID")).attr("x",
function(a){return c.xScale(f)}).attr("width",function(a){return c.xScale(l)-c.xScale(f)})}d3.select("#Level"+c.gsvg.levelNumber+c.trackClass+" g#tx"+b.getAttribute("ID")+" rect#Ex"+exList[e].getAttribute("ID")).attr("x",function(a){return c.xScale(m)}).attr("width",function(a){return c.xScale(h)-c.xScale(m)});if(0<e){var n=">";"-1"==b.getAttribute("strand")&&(n="<");var r=n,w=c.xScale(exList[e-1].getAttribute("stop")),u=c.xScale(exList[e].getAttribute("start")),v=u-w,y=0;if(7>v)r="";else for(v-=
7;8<v;)0==y?(r+=" ",y=1):(r+=n,y=0),v-=7;d3.select("#Level"+c.gsvg.levelNumber+c.trackClass+" g#tx"+b.getAttribute("ID")+" line#Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("x1",w).attr("x2",u);d3.select("#Level"+c.gsvg.levelNumber+c.trackClass+" g#tx"+b.getAttribute("ID")+" #IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("dx",w+1).text(r)}}});c.redrawSelectedArea()};c.update=function(){var b=getAllChildrenByName(getFirstChildByName(c.gsvg.selectedData,
...
var c=getAllChildrenByName(getFirstChildByName(a,"blockList"),"block"),e=0,m=1,h="start",f="stop";-1==a.getAttribute("strand")&&(e=1,m=0,h="stop",f="start");b=b+"<BR><BR>Coverage Exon 1: "+numberWithCommas(c[e].getAttribute(h))+"-"+numberWithCommas(c[e].getAttribute(f))+" ("+(parseInt(c[e].getAttribute("stop"),10)-parseInt(c[e].getAttribute("start"),10))+" bp)";b=b+"<BR>Coverage Exon 2: "+numberWithCommas(c[m].getAttribute(h))+"-"+numberWithCommas(c[m].getAttribute(f))+" ("+(parseInt(c[m].getAttribute("stop"),
10)-parseInt(c[m].getAttribute("start"),10))+" bp)";return b=b+"<BR>Intron: "+numberWithCommas(parseInt(c[1].getAttribute("start"),10)-parseInt(c[0].getAttribute("stop"),10))+" bp<BR>"};f.drawScaleLegend("1","1000+","Read Counts","#E6E6E6","#000000",0);f.draw(e);return f}
...
".count."+b.trackClass+".xml",f=dataPrefix+"tmpData/trackUpload/"+b.trackClass.substr(6),l="bg",n="";0<b.bin&&(d-=2*b.bin,d-=d%(2*b.bin),g+=2*b.bin,g+=2*b.bin-g%(2*b.bin),h=dataPrefix+"tmpData/trackXML/"+b.gsvg.folderName+"/"+d+"_"+g+".bincount."+b.bin+"."+b.trackClass+".xml");-1<b.dataFileName.indexOf("http")&&(n=f=b.dataFileName,l="bw");d3.xml(h,function(e,m){if(e){if(0==a||1==c){var p="/"+pathPrefix;pathPrefix||(p="");$.ajax({url:p+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,
minCoord:d,maxCoord:g,myOrganism:organism,genomeVer:genomeVer,track:b.trackClass,bedFile:f,outFile:h,folder:b.gsvg.folderName,binSize:b.bin,type:l,web:n},dataType:"json",success:function(a){gtag("event","generateTrackCount",{event_category:"browser"})},error:function(a,b,c){}})}8>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1)},500):30>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1)},1E3):32>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+
1)},1E4):(d3.select("#Level"+b.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass),b.hideLoading(),b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},15E3))}else null==m?32<=a?(p=[],b.draw(p)):b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},5E3):(b.fullDataTimeOutHandle=
0,b.loadedDataMin=d,b.loadedDataMax=g,p=m.documentElement.getElementsByTagName("Count"),b.draw(p),b.updateControl=0)})}};b.updateLabel(f);b.redrawLegend();b.redraw();return b}
...
c.bin=c.calculateBin(e);e=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.gsvg.folderName+"/tmp/"+g+"_"+d+".count."+c.trackClass+".xml";0<c.bin&&(g-=2*c.bin,g-=g%(2*c.bin),d+=2*c.bin,d+=2*c.bin-d%(2*c.bin),e=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.gsvg.folderName+"/tmp/"+g+"_"+d+".bincount."+c.bin+"."+c.trackClass+".xml");d3.xml(e,function(e,h){if(e){if(0==b||1==a){var m="/"+pathPrefix;pathPrefix||(m="");tmpPanel=panel;-1<c.trackClass.indexOf("-")&&(tmpPanel=c.trackClass.substr(c.trackClass.indexOf("-")+
1));$.ajax({url:m+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:g,maxCoord:d,panel:tmpPanel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:c.trackClass,folder:c.gsvg.folderName,binSize:c.bin},dataType:"json",success:function(a){gtag("event","generateTrackCount",{event_category:"browser"})},error:function(a,b,c){}})}8>b?c.fullDataTimeOutHandle=setTimeout(function(){c.updateFullData(b+1,0)},500):30>b?c.fullDataTimeOutHandle=
setTimeout(function(){c.updateFullData(b+1,0)},1E3):32>b?c.fullDataTimeOutHandle=setTimeout(function(){c.updateFullData(b+1,0)},1E4):(d3.select("#Level"+c.levelNumber+c.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+c.trackClass),d3.select("#Level"+c.levelNumber+c.trackClass).attr("height",15),c.gsvg.addTrackErrorRemove(c.svg,"#Level"+c.gsvg.levelNumber+c.trackClass),c.hideLoading(),c.fullDataTimeOutHandle=setTimeout(function(){c.updateFullData(b+1,1)},15E3))}else null==h?
32<=b?(m=[],c.draw(m)):c.fullDataTimeOutHandle=setTimeout(function(){c.updateFullData(b+1,0)},5E3):(c.fullDataTimeOutHandle=0,c.loadedDataMin=g,c.loadedDataMax=d,m=h.documentElement.getElementsByTagName("Count"),c.draw(m))})};c.updateCountScale=function(b,a){c.scaleMin=b;c.scaleMax=a;c.colorScale=d3.scaleLinear().domain([c.scaleMin,c.scaleMax]).range(["#EEEEEE","#000000"]);1==c.density&&(c.redrawLegend(),c.redraw())};c.setupToolTipSVG=function(b,a){var g=c.xScale.domain()[0],d=c.xScale.domain()[1],
e=parseInt(b.getAttribute("start"),10),m=parseInt(b.getAttribute("stop"),10);d=Math.floor((d-g)*a);20>d&&(d=20);g=e-d;d=m+d;1>g&&(g=1);"undefined"!==typeof c.ttSVGMinWidth&&d-g<c.ttSVGMinWidth&&(g=e-c.ttSVGMinWidth/2,d=m+c.ttSVGMinWidth/2);e=toolTipSVG("div#ttSVG",450,g,d,99,b.getAttribute("ID"),"transcript");e.addTrack(c.trackClass,3,"",c.data);for(m=0;m<c.ttTrackList.length;m++){var h=c.gsvg.getTrackData(c.ttTrackList[m]),f=[];if("undefined"!==typeof h&&0<h.length){for(var l=0,n=0;n<h.length;n++)if(g<=
parseInt(h[n].getAttribute("start"),10)&&parseInt(h[n].getAttribute("start"),10)<=d||parseInt(h[n].getAttribute("start"),10)<=g&&parseInt(h[n].getAttribute("stop"),10)>=g)f[l]=h[n],l++;0<f.length&&e.addTrack(c.ttTrackList[m],3,"DrawTrx",f)}}};c.draw=function(b){c.hideLoading();c.data=b;var a=c.gsvg.xScale.domain()[0],g=c.gsvg.xScale.domain()[1];c.xScale(a+c.bin);c.xScale(a);c.redrawLegend();for(var d=[],e=0,m=0,h=0;h<b.length;h++)if("undefined"!==typeof b[h]){var f=parseInt(b[h].getAttribute("start"),
10),l=parseInt(b[h].getAttribute("stop"),10),n=parseInt(b[h].getAttribute("count"),10);if(1!=c.density||1==c.density&&f!=l)if(h+1<b.length){if(l=parseInt(b[h+1].getAttribute("start"),10),f>=a&&f<=g||l>=a&&l<=g)d[e]=b[h],n>m&&(m=n),e++}else f>=a&&f<=g&&(d[e]=b[h],n>m&&(m=n),e++)}b=d;c.svg.selectAll("."+c.trackClass).remove();c.svg.select(".y.axis").remove();c.svg.select("g.grid").remove();c.svg.selectAll(".leftLbl").remove();c.svg.select(".area").remove();c.svg.selectAll(".area").remove();1==c.density?
...
if(a&&a.Controls&&0<a.Controls.length){var g=(new String(a.Controls)).split(",");b=d3.select(b).select("table").select("tbody");b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+a.Name);for(a=0;a<g.length;a++)if("undefined"!==typeof g[a]&&""!=g[a]){var d=g[a].split(";"),e=b.append("tr").append("td"),m=d[0].substr(5),h="";3<d.length&&0==d[3].indexOf("Default=")&&(h=d[3].substr(8));if(0==d[1].toLowerCase().indexOf("select")){e.append("text").text(m+": ");var f=d[1].split(":");
d=d[2].split("}");m=c.trackClass+"Dense"+c.level+"Select";"colorSelect"==f[1]&&(m=c.trackClass+c.level+"colorSelect");e=e.append("select").attr("id",m).attr("name",f[1]);for(f=0;f<d.length;f++){var l=d[f].substr(1).split(":");if(2==l.length){var n=e.append("option").attr("value",l[1]).text(l[0]);-1<m.indexOf("Dense")?l[1]==c.density&&n.attr("selected","selected"):l[1]==h&&n.attr("selected","selected")}}d3.select("select#"+m).on("change",function(){$(this).attr("id")==c.trackClass+"Dense"+c.level+
"Select"&&1==$(this).val()?$("#scaleControl"+c.level).show():$("#scaleControl"+c.level).hide();c.updateSettingsFromUI();c.redraw()})}else 0==d[1].toLowerCase().indexOf("slider")&&(h="none",1==c.density&&(h="inline-block"),e=e.append("div").attr("id","scaleControl"+c.level).style("display",h),e.append("text").text(m+": "),e.append("input").attr("type","text").attr("id","amount").attr("value",c.scaleMin+"-"+c.scaleMax).style("border",0).style("color","#f6931f").style("font-weight","bold").style("background-color",
"#EEEEEE"),f=d[1].split(":"),d=d[2].split("}"),e=e.append("div"),e.append("text").text("Min:"),e.append("div").attr("id","min-"+f[1]).style("width","60%").style("display","inline-block").style("float","right"),e.append("br"),e.append("text").text("Max:"),e.append("div").attr("id","max-"+f[1]).style("width","60%").style("display","inline-block").style("float","right"),$("#min-"+f[1]).slider({min:1,max:1E3,step:1,value:c.scaleMin,slide:c.processSlider}),$("#max-"+f[1]).slider({min:1E3,max:2E4,step:100,
...
function CustomTranscriptTrack(f,e,l,n,c,b){var a=GenericTranscriptTrack(f,e,l,n,c,b);f=b.split(",");a.xmlTag="Feature";a.xmlTagBlockElem="block";a.density=c;1!==a.density&&2!==a.density&&3!==a.density&&(a.density=3);0<f.length&&(a.dataFileName=f[0].substr(9));a.colorBy=1<f.length?f[1]:"Score";2<f.length&&(a.minValue=f[2]);3<f.length&&(a.maxValue=f[3]);4<f.length&&(a.minColor=f[4]);5<f.length&&(a.maxColor=f[5]);a.colorValueField="score";a.minFeatureWidth=1;a.updateControl=0;"Score"==a.colorBy&&a.createColorScale();
a.updateFullData=function(b,c){if(a.updateControl==b){a.updateControl=b+1;var d=a.xScale.domain()[0],g=a.xScale.domain()[1],e=dataPrefix+"tmpData/trackXML/"+a.gsvg.folderName+"/"+a.dataFileName+".xml",f=dataPrefix+"tmpData/trackUpload/"+a.dataFileName,l="",n=new Date,r="bed";-1<a.dataFileName.indexOf("http")&&(l=a.dataFileName,f="tmpData/tmpDownload/"+n.getTime()+"_"+a.trackClass,e=dataPrefix+"tmpData/trackXML/"+a.gsvg.folderName+"/"+a.trackClass+".xml",r="bb");d3.xml(e,function(h,m){if(h){console.log(h);
if(0==b||1==c){var p="/"+pathPrefix;pathPrefix||(p="");$.ajax({url:p+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:d,maxCoord:g,folder:a.gsvg.folderName,bedFile:f,outFile:e,track:a.trackClass,web:l,type:r},dataType:"json",success:function(a){gtag("event","generateTrackCustomTranscript",{event_category:"browser"})},error:function(a,b,c){console.log(c)}})}3>b?(p=1E4,1==b&&(p=15E3),setTimeout(function(){a.updateFullData(b+1,0)},p)):(d3.select("#Level"+a.gsvg.levelNumber+
a.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+a.trackClass),d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).attr("height",15),a.gsvg.addTrackErrorRemove(a.svg,"#Level"+a.gsvg.levelNumber+a.trackClass),a.hideLoading())}else p=m.documentElement.getElementsByTagName(a.xmlTag),a.draw(p),a.hideLoading(),a.updateControl=0})}};a.updateSettingsFromUI=function(){0<$("#"+a.trackClass+"Dense"+a.level+"Select").length&&(a.density=$("#"+a.trackClass+"Dense"+a.level+"Select").val());
...
"maxColor"+a.level).val()),a.createColorScale())};a.generateSettingsDiv=function(b){var c=trackInfo[a.trackClass];a.savePrevious();d3.select(b).select("table").select("tbody").html("");if(0<c.Controls.length&&"null"!=c.Controls){var g=(new String(c.Controls)).split(",");b=d3.select(b).select("table").select("tbody");b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+c.Name);for(c=0;c<g.length;c++)if("undefined"!==typeof g[c]&&""!=g[c]){var e=g[c].split(";"),h=b.append("tr").append("td"),
f=e[0].substr(5);3<e.length&&0==e[3].indexOf("Default=")&&e[3].substr(8);if(0==e[1].toLowerCase().indexOf("select")){h.append("text").text(f+": ");f=e[1].split(":");e=e[2].split("}");var l=a.trackClass+"Dense"+a.level+"Select";"colorSelect"==f[1]&&(l=a.trackClass+a.level+"colorSelect");h=h.append("select").attr("id",l).attr("name",f[1]);for(f=0;f<e.length;f++){var n=e[f].substr(1).split(":");if(2==n.length){var r=h.append("option").attr("value",n[1]).text(n[0]);(-1<l.indexOf("Dense")&&n[1]==a.density||
-1<l.indexOf("colorSelect")&&n[1]==a.colorBy)&&r.attr("selected","selected")}}d3.select("select#"+l).on("change",function(){"Score"==$(this).val()?$("div."+a.trackClass+"Scale"+a.level).show():"Color"==$(this).val()&&$("div."+a.trackClass+"Scale"+a.level).hide();a.updateSettingsFromUI();a.draw(a.data)})}else 0==e[1].toLowerCase().indexOf("txt")&&(h=0==$("#colorTrack"+a.level).size()?h.append("div").attr("class",a.trackClass+"Scale"+a.level).style("display","none"):d3.select("#"+a.trackClass+"Scale"+
a.level),h.append("text").text(f+": "),f=e[1].split(":"),e=e[2].split("}"),e="Data",l="text",n=a.minValue,r=a.maxValue,"color"==f[1]&&(l=e="Color",n=a.minColor,r=a.maxColor),h.append("input").attr("type",l).attr("id",a.trackClass+"min"+e+a.level).attr("class",f[1]).style("margin-left","5px").attr("value",n),h.append("text").text(" - "),h.append("input").attr("type",l).attr("id",a.trackClass+"max"+e+a.level).attr("class",f[1]).style("margin-left","5px").attr("value",r),"Color"==e&&(testIE||testSafari)?
($("#"+a.trackClass+"min"+e+a.level).spectrum({change:function(b){a.updateSettingsFromUI();a.draw(a.data)}}),$("#"+a.trackClass+"max"+e+a.level).spectrum({change:function(b){a.updateSettingsFromUI();a.draw(a.data)}})):($("input#"+a.trackClass+"min"+e+a.level).on("change",function(){a.updateSettingsFromUI();a.draw(a.data)}),$("input#"+a.trackClass+"max"+e+a.level).on("change",function(){a.updateSettingsFromUI();a.draw(a.data)})))}"Score"==$("#"+a.trackClass+a.level+"colorSelect").val()?$("div."+a.trackClass+
"Scale"+a.level).show():"Color"==$("#"+a.trackClass+a.level+"colorSelect").val()&&$("div."+a.trackClass+"Scale"+a.level).hide();g=b.append("tr").append("td");g.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");a.gsvg.removeTrack(a.trackClass);var b=svgList[a.gsvg.levelNumber].currentView.ViewID,c=viewMenu[a.gsvg.levelNumber].findTrackByClass(a.trackClass,b);c=viewMenu[a.gsvg.levelNumber].findTrackIndexWithViewID(c.TrackID,
b);viewMenu[a.gsvg.levelNumber].removeTrackWithIDIdx(c,b)});g.append("input").attr("type","button").attr("value","Apply").style("float","right").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")});g.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left","5px").on("click",function(){a.revertPrevious();a.draw(a.data);$("#trackSettingDialog").fadeOut("fast")})}else b=d3.select(b).select("table").select("tbody"),
b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+c.Name),b.append("tr").append("td").html("Sorry no settings for this track."),g=b.append("tr").append("td"),g.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")}),g.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")})};
...
function(b){return a.xScale(d[g].getAttribute("stop"))-a.xScale(d[g].getAttribute("start"))});if(0<g){e=">";"-1"==b.getAttribute("strand")&&(e="<");var f=e,p=a.xScale(d[g-1].getAttribute("stop"))-a.xScale(b.getAttribute("start")),l=a.xScale(d[g].getAttribute("start"))-a.xScale(b.getAttribute("start")),n=l-p,u=0;if(7>n)f="";else for(n-=7;8<n;)0==u?(f+=" ",u=1):(f+=e,u=0),n-=7;e=d[g-1].getAttribute("ID")+"_"+d[g].getAttribute("ID");null==d[g].getAttribute("ID")&&(e=b.getAttribute("ID")+"_"+(g-1)+"_"+
...
a.xmlTagBlockElem+"List"),a.xmlTagBlockElem);for(var g=0;g<exList.length;g++)if(d.append("rect").attr("x",function(b){return a.xScale(exList[g].getAttribute("start"))-a.xScale(b.getAttribute("start"))}).attr("rx",1).attr("ry",1).attr("height",10).attr("width",function(b){b=a.xScale(exList[g].getAttribute("stop"))-a.xScale(exList[g].getAttribute("start"));0<a.minFeatureWidth&&b<a.minFeatureWidth&&(b=a.minFeatureWidth);return b}).attr("id",function(b,c){var d=a.idPrefix+"Ex"+exList[g].getAttribute("ID");
null==exList[g].getAttribute("ID")&&(d=a.idPrefix+"Ex"+b.getAttribute("ID")+"_"+g);return d}).style("fill",a.color).style("cursor","pointer"),0<g){var e=a.xScale(exList[g-1].getAttribute("stop"))-a.xScale(b.getAttribute("start")),f=a.xScale(exList[g].getAttribute("start"))-a.xScale(b.getAttribute("start"));d.append("line").attr("x1",e).attr("x2",f).attr("y1",5).attr("y2",5).attr("stroke",a.color).attr("stroke-width","2").attr("id",function(b,c){var d=a.idPrefix+"Int"+exList[g-1].getAttribute("ID")+
"_"+exList[g].getAttribute("ID");null==exList[g].getAttribute("ID")&&(d=a.idPrefix+"Int"+b.getAttribute("ID")+"_"+(g-1)+"_"+g);return d});var l=">";"-1"==b.getAttribute("strand")&&(l="<");var n=l;f-=e;var r=0;if(7>f)n="";else for(f-=7;8<f;)0==r?(n+=" ",r=1):(n+=l,r=0),f-=7;d.append("svg:text").attr("id",function(b){var c=a.idPrefix+"IntTxt"+exList[g-1].getAttribute("ID")+"_"+exList[g].getAttribute("ID");null==exList[g].getAttribute("ID")&&(c=a.idPrefix+"IntTxt"+b.getAttribute("ID")+"_"+(g-1)+"_"+
g);return c}).attr("dx",e+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",a.color).style("font-size","16px").text(n)}};a.redraw=function(){if(a.prevDensity!=a.density)a.draw(a.data);else{a.yMaxArr=[];a.yArr=[];a.yArr[0]=[];for(var b=0;b<a.gsvg.width;b++)a.yMaxArr[b]=0,a.yArr[0][b]=0;a.trackYMax=0;d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+"trx"+a.gsvg.levelNumber).attr("transform",function(b,c){return"translate("+a.xScale(b.getAttribute("start"))+
...
a.updateFullData=function(b,c){var d=a.xScale.domain()[0],g=a.xScale.domain()[1];a.showLoading();d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+a.gsvg.folderName+"/"+a.dataFileName,function(e,f){if(e){if(0==b||1==c){var h="/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:d,maxCoord:g,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a.trackClass,
folder:a.gsvg.folderName},dataType:"json",success:function(a){gtag("event","generateTrackTranscript",{event_category:"browser"})},error:function(a,b,c){}})}3>b?(h=1E4,1==b&&(h=15E3),setTimeout(function(){a.updateFullData(b+1,0)},h)):(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+a.trackClass),d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).attr("height",15),a.gsvg.addTrackErrorRemove(a.svg,"#Level"+a.gsvg.levelNumber+a.trackClass),
a.hideLoading())}else null==f?4<=b?(h=[],a.draw(h),a.hideLoading()):setTimeout(function(){a.updateFullData(b+1,0)},5E3):(h=f.documentElement.getElementsByTagName(a.xmlTag),a.draw(h),a.hideLoading())})};a.redrawLegend=function(){var b=[];"Score"==a.colorBy?a.drawScaleLegend(a.minValue,a.maxValue+"+",a.legendLbl,a.minColor,a.maxColor,0):"Color"==a.colorBy&&(b[0]={color:"#FFFFFF",label:"User assigned color from track file."},a.drawLegend(b))};a.draw=function(b){a.data=b;a.prevDensity=a.density;a.trackYMax=
...
function CircRNATrack(f,e,l,n,c,b){var a=GenericTranscriptTrack(f,e,l,n,c,b);f=b.split(",");a.density=c;0<f.length&&(a.dataFileName=f[0].substr(9));a.colorBy=1<f.length?f[1]:"Score";2<f.length&&(a.minValue=f[2]);3<f.length&&(a.maxValue=f[3]);4<f.length&&(a.minColor=f[4]);5<f.length&&(a.maxColor=f[5]);a.colorValueField="score";a.minFeatureWidth=1;a.updateControl=0;a.xmlTag="Gene";a.xmlTagBlockElem="Block";a.idPrefix="cirRNA";a.xCirLineWidth=4;a.xPadding=7;a.featureHeight=30;"Score"==a.colorBy&&a.createColorScale();
a.color=function(a){var b=d3.rgb("#000000");"1"===a.getAttribute("Array")&&"1"===a.getAttribute("Ciri")&&"1"===a.getAttribute("Circexplorer")?b=d3.rgb("#DD9C56"):"0"===a.getAttribute("Array")&&"1"===a.getAttribute("Ciri")&&"1"===a.getAttribute("Circexplorer")?b=d3.rgb("#0f4c75"):"0"!==a.getAttribute("Array")||"1"!==a.getAttribute("Ciri")&&"1"!==a.getAttribute("Circexplorer")?"1"===a.getAttribute("Array")&&"0"===a.getAttribute("Ciri")&&"0"===a.getAttribute("Circexplorer")?b=d3.rgb("#5D576B"):"1"!==
a.getAttribute("Array")||"1"!==a.getAttribute("Ciri")&&"1"!==a.getAttribute("Circexplorer")||(b=d3.rgb("#7A9D98")):b=d3.rgb("#3282b8");return b};a.calcY=function(b,c,e){b=3===a.density||"3"===a.density?a.calcYPack(b,c,e):2===a.density||"2"===a.density?a.calcYFull(e):a.calcYDense();a.trackYMax<b/a.featureHeight&&(a.trackYMax=b/a.featureHeight);return b};a.calcYPack=function(b,c,e){if(b>=a.xScale.domain()[0]&&b<=a.xScale.domain()[1]||c>=a.xScale.domain()[0]&&c<=a.xScale.domain()[1]||b<=a.xScale.domain()[0]&&
c>=a.xScale.domain()[1]){b=Math.round(a.xScale(b));0>b&&(b=0);c=Math.round(a.xScale(c));c>=a.gsvg.width&&(c=a.gsvg.width-1);e=b-a.xPadding;0>e&&(e=0);var d=c+a.xPadding;d>=a.gsvg.width&&(d=a.gsvg.width-1);for(var g=0,f=e;f<=d;f++)a.yMaxArr[f]>g&&(g=a.yMaxArr[f]);g++;var p=g;if(a.yArr.length<=g)for(a.yArr[p]=[],f=0;f<a.gsvg.width;f++)a.yArr[p][f]=0;f=g-a.scanBackYLines;1>f&&(f=1);p=0;for(var l=f;l<g&&0==p;l++){var n=0;for(f=e;f<=d&&0==n;f++)a.yArr[l][f]>n&&(n=1);0==n&&(g=l,p=1)}g>a.trackYMax&&(a.trackYMax=
g);for(f=b;f<=c;f++)a.yMaxArr[f]<g&&(a.yMaxArr[f]=g),a.yArr[g][f]=1;b=g*a.featureHeight}else b=a.featureHeight;return b};a.calcYFull=function(b){return(b+1)*a.featureHeight};a.draw=function(b){a.data=b;a.prevDensity=a.density;a.trackYMax=0;a.yArr=[];a.yArr[0]=[];for(var c=0;c<a.gsvg.width;c++)a.yMaxArr[c]=0,a.yArr[0][c]=0;d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("."+a.idPrefix+"trx"+a.gsvg.levelNumber).remove();a.redrawLegend();b&&(b=d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("."+
a.idPrefix+"trx"+a.gsvg.levelNumber).data(b,key),b.enter().append("g").attr("class",a.idPrefix+"trx"+a.gsvg.levelNumber).attr("transform",function(b,c){return"translate("+a.xScale(b.getAttribute("start"))+","+a.calcY(parseInt(b.getAttribute("start"),10),parseInt(b.getAttribute("stop"),10),c)+")"}).attr("id",function(b){return a.idPrefix+"tx"+b.getAttribute("ID")}).style("cursor","move").on("mouseover",function(b){0==a.gsvg.isToolTip&&0!=a.trackClass.indexOf("custom")&&(d3.select(this).selectAll("line").style("stroke",
"green"),d3.select(this).selectAll("rect").style("fill","green"),d3.select(this).selectAll("text").style("opacity","0.3").style("fill","green"),tt.transition().duration(200).style("opacity",1),tt.html(a.createToolTip(b)).style("left",function(){return a.positionTTLeft(d3.event.pageX)}).style("top",function(){return a.positionTTTop(d3.event.pageY)}),1==a.ttSVG&&a.setupToolTipSVG(b,.05))}).on("mouseout",function(b){d3.select(this).selectAll("line").style("stroke",a.color);d3.select(this).selectAll("rect").style("fill",
a.color);d3.select(this).selectAll("text").style("opacity","0.6").style("fill",a.color);tt.transition().delay(100).duration(200).style("opacity",0)}).merge(b).each(a.drawTrx),b.exit().remove());1==a.density?a.svg.attr("height",30):2==a.density?a.svg.attr("height",15*(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+"trx"+a.gsvg.levelNumber).size()+1)):3==a.density&&a.svg.attr("height",30*(a.trackYMax+2));a.redrawSelectedArea()};a.drawTrx=function(b,c){var d=d3.select("#Level"+
a.gsvg.levelNumber+a.trackClass).select("#"+a.idPrefix+"tx"+b.getAttribute("ID"));exList=getAllChildrenByName(getFirstChildByName(b,a.xmlTagBlockElem+"List"),a.xmlTagBlockElem);minStart=a.xScale(b.getAttribute("start"));maxEnd=a.xScale(b.getAttribute("stop"));d.append("line").attr("id",function(){return"l"+b.getAttribute("ID")+"_x1"}).attr("x1",function(){return a.calcXLine(exList,0,-1,"start",minStart)}).attr("x2",function(){return a.calcXLine(exList,0,-1,"start",minStart)}).attr("y1",5).attr("y2",
20).attr("stroke",a.color).attr("stroke-width","1");d.append("line").attr("id",function(){return"l"+b.getAttribute("ID")+"_x2"}).attr("x1",function(){return a.calcXLine(exList,exList.length-1,1,"stop",minStart)}).attr("x2",function(){return a.calcXLine(exList,exList.length-1,1,"stop",minStart)}).attr("y1",5).attr("y2",20).attr("stroke",a.color).attr("stroke-width","1");d.append("line").attr("id",function(){return"l"+b.getAttribute("ID")+"_x3"}).attr("x1",function(){return a.calcXLine(exList,0,-1,
"start",minStart)}).attr("x2",function(){return a.calcXLine(exList,exList.length-1,1,"stop",minStart)}).attr("y1",5).attr("y2",5).attr("stroke",a.color).attr("stroke-width","1");d.append("line").attr("id",function(){return"l"+b.getAttribute("ID")+"_x4"}).attr("x1",function(){return a.calcXLine(exList,0,-1,"start",minStart)}).attr("x2",function(){return a.calcXLine(exList,0,0,"start",minStart)}).attr("y1",20).attr("y2",20).attr("stroke",a.color).attr("stroke-width","1");d.append("line").attr("id",
function(){return"l"+b.getAttribute("ID")+"_x5"}).attr("x1",function(){return a.calcXLine(exList,exList.length-1,0,"stop",minStart)}).attr("x2",function(){return a.calcXLine(exList,exList.length-1,1,"stop",minStart)}).attr("y1",20).attr("y2",20).attr("stroke",a.color).attr("stroke-width","1");for(var e=0;e<exList.length;e++)if(d.append("rect").attr("x",function(b){return a.xScale(exList[e].getAttribute("start"))-a.xScale(b.getAttribute("start"))}).attr("y",15).attr("rx",1).attr("ry",1).attr("height",
10).attr("width",function(b){b=a.xScale(exList[e].getAttribute("stop"))-a.xScale(exList[e].getAttribute("start"));0<a.minFeatureWidth&&b<a.minFeatureWidth&&(b=a.minFeatureWidth);return b}).attr("id",function(b,c){var d=a.idPrefix+"Ex"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(d=a.idPrefix+"Ex"+b.getAttribute("ID")+"_"+e);return d}).style("fill",a.color).style("cursor","pointer"),0<e){var g=a.xScale(exList[e-1].getAttribute("stop"))-a.xScale(b.getAttribute("start")),f=a.xScale(exList[e].getAttribute("start"))-
a.xScale(b.getAttribute("start"));d.append("line").attr("x1",g).attr("x2",f).attr("y1",20).attr("y2",20).attr("stroke",a.color).attr("stroke-width","2").attr("id",function(b,c){var d=a.idPrefix+"Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(d=a.idPrefix+"Int"+b.getAttribute("ID")+"_"+(e-1)+"_"+e);return d});var l=">";"-"===b.getAttribute("strand")&&(l="<");var n=l;f-=g;var r=0;if(7>f)n="";else for(f-=7;8<f;)0==r?(n+=" ",r=1):(n+=l,r=0),f-=
7;d.append("svg:text").attr("id",function(b){var c=a.idPrefix+"IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(c=a.idPrefix+"IntTxt"+b.getAttribute("ID")+"_"+(e-1)+"_"+e);return c}).attr("y",15).attr("dx",g+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",a.color).style("font-size","16px").text(n)}};a.calcXLine=function(b,c,e,f,h){e*=a.xCirLineWidth;return 0<b.length?a.xScale(b[c].getAttribute(f))-h+
e:0+e};a.redraw=function(){if(a.prevDensity!=a.density)a.draw(a.data);else{a.yMaxArr=[];a.yArr=[];a.yArr[0]=[];for(var b=0;b<a.gsvg.width;b++)a.yMaxArr[b]=0,a.yArr[0][b]=0;a.trackYMax=0;d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+"trx"+a.gsvg.levelNumber).attr("transform",function(b,c){return"translate("+a.xScale(b.getAttribute("start"))+","+a.calcY(parseInt(b.getAttribute("start"),10),parseInt(b.getAttribute("stop"),10),c)+")"}).each(function(b,c){minStart=a.xScale(b.getAttribute("start"));
maxEnd=a.xScale(b.getAttribute("stop"));var d=getAllChildrenByName(getFirstChildByName(b,a.xmlTagBlockElem+"List"),a.xmlTagBlockElem);a.svg.select("#l"+b.getAttribute("ID")+"_x1").attr("x1",function(){return a.calcXLine(d,0,-1,"start",minStart)}).attr("x2",function(){return a.calcXLine(d,0,-1,"start",minStart)});a.svg.select("#l"+b.getAttribute("ID")+"_x2").attr("x1",function(){return a.calcXLine(d,d.length-1,1,"stop",minStart)}).attr("x2",function(){return a.calcXLine(d,d.length-1,1,"stop",minStart)});
a.svg.select("#l"+b.getAttribute("ID")+"_x3").attr("x1",function(){return a.calcXLine(d,0,-1,"start",minStart)}).attr("x2",function(){return a.calcXLine(d,d.length-1,1,"stop",minStart)});a.svg.select("#l"+b.getAttribute("ID")+"_x4").attr("x1",function(){return a.calcXLine(d,0,-1,"start",minStart)}).attr("x2",function(){return a.calcXLine(d,0,0,"start",minStart)});a.svg.select("#l"+b.getAttribute("ID")+"_x5").attr("x1",function(){return a.calcXLine(d,d.length-1,0,"stop",minStart)}).attr("x2",function(){return a.calcXLine(d,
d.length-1,1,"stop",minStart)});for(var e=0;e<d.length;e++){var g=a.idPrefix+"Ex"+d[e].getAttribute("ID");null==d[e].getAttribute("ID")&&(g=a.idPrefix+"Ex"+b.getAttribute("ID")+"_"+e);a.svg.select("g#"+a.idPrefix+"tx"+b.getAttribute("ID")+" rect#"+g).attr("x",function(c){return a.xScale(d[e].getAttribute("start"))-a.xScale(b.getAttribute("start"))}).attr("width",function(b){return a.xScale(d[e].getAttribute("stop"))-a.xScale(d[e].getAttribute("start"))});if(0<e){g=">";"-"===b.getAttribute("strand")&&
(g="<");var f=g,p=a.xScale(d[e-1].getAttribute("stop"))-a.xScale(b.getAttribute("start")),l=a.xScale(d[e].getAttribute("start"))-a.xScale(b.getAttribute("start")),n=l-p,u=0;if(7>n)f="";else for(n-=7;8<n;)0==u?(f+=" ",u=1):(f+=g,u=0),n-=7;g=d[e-1].getAttribute("ID")+"_"+d[e].getAttribute("ID");null==d[e].getAttribute("ID")&&(g=b.getAttribute("ID")+"_"+(e-1)+"_"+e);a.svg.select("g#"+a.idPrefix+"tx"+b.getAttribute("ID")+" line#"+a.idPrefix+"Int"+g).attr("x1",p).attr("x2",l);a.svg.select("g#"+a.idPrefix+
"tx"+b.getAttribute("ID")+" #"+a.idPrefix+"IntTxt"+g).attr("dx",p+1).text(f)}}});1==a.density?a.svg.attr("height",40):2==a.density?a.svg.attr("height",25*(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+"trx"+a.gsvg.levelNumber).size()+1)):3==a.density&&a.svg.attr("height",25*(a.trackYMax+1));a.redrawSelectedArea()}};a.updateFullData=function(b,c){if(a.updateControl==b){a.updateControl=b+1;var d=a.xScale.domain()[0],e=a.xScale.domain()[1];d3.xml(dataPrefix+"tmpData/browserCache/"+
genomeVer+"/regionData/"+a.gsvg.folderName+"/"+a.trackClass+".xml",function(g,f){if(g){console.log(g);if(0==b||1==c){var h="/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:d,maxCoord:e,folder:a.gsvg.folderName,track:a.trackClass,myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(a){gtag("event","generateTrackCustomTranscript",{event_category:"browser"})},error:function(a,b,c){console.log(c)}})}3>
b?(h=1E4,1==b&&(h=15E3),setTimeout(function(){a.updateFullData(b+1,0)},h)):(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+a.trackClass),d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).attr("height",15),a.gsvg.addTrackErrorRemove(a.svg,"#Level"+a.gsvg.levelNumber+a.trackClass),a.hideLoading())}else h=f.documentElement.getElementsByTagName(a.xmlTag),a.draw(h),a.hideLoading(),a.updateControl=0})}};a.updateSettingsFromUI=function(){0<
$("#"+a.trackClass+"Dense"+a.level+"Select").length&&(a.density=$("#"+a.trackClass+"Dense"+a.level+"Select").val());0<$("#"+a.trackClass+a.level+"colorSelect").length&&(a.colorBy=$("#"+a.trackClass+a.level+"colorSelect").val())};a.generateSettingsDiv=function(b){var c=trackInfo[a.trackClass];a.savePrevious();d3.select(b).select("table").select("tbody").html("");if(0<c.Controls.length&&"null"!=c.Controls){var e=(new String(c.Controls)).split(",");b=d3.select(b).select("table").select("tbody");b.append("tr").append("td").style("font-weight",
"bold").html("Track Settings: "+c.Name);for(c=0;c<e.length;c++)if("undefined"!==typeof e[c]&&""!=e[c]){var g=e[c].split(";"),f=b.append("tr").append("td"),l=g[0].substr(5);3<g.length&&0==g[3].indexOf("Default=")&&g[3].substr(8);if(0==g[1].toLowerCase().indexOf("select")){f.append("text").text(l+": ");l=g[1].split(":");g=g[2].split("}");var n=a.trackClass+"Dense"+a.level+"Select";"colorSelect"==l[1]&&(n=a.trackClass+a.level+"colorSelect");f=f.append("select").attr("id",n).attr("name",l[1]);for(l=0;l<
g.length;l++){var z=g[l].substr(1).split(":");if(2==z.length){var r=f.append("option").attr("value",z[1]).text(z[0]);(-1<n.indexOf("Dense")&&z[1]==a.density||-1<n.indexOf("colorSelect")&&z[1]==a.colorBy)&&r.attr("selected","selected")}}d3.select("select#"+n).on("change",function(){"Score"==$(this).val()?$("div."+a.trackClass+"Scale"+a.level).show():"Color"==$(this).val()&&$("div."+a.trackClass+"Scale"+a.level).hide();a.updateSettingsFromUI();a.draw(a.data)})}else 0==g[1].toLowerCase().indexOf("txt")&&
(f=0==$("#colorTrack"+a.level).size()?f.append("div").attr("class",a.trackClass+"Scale"+a.level).style("display","none"):d3.select("#"+a.trackClass+"Scale"+a.level),f.append("text").text(l+": "),l=g[1].split(":"),g=g[2].split("}"),g="Data",n="text",z=a.minValue,r=a.maxValue,"color"==l[1]&&(n=g="Color",z=a.minColor,r=a.maxColor),f.append("input").attr("type",n).attr("id",a.trackClass+"min"+g+a.level).attr("class",l[1]).style("margin-left","5px").attr("value",z),f.append("text").text(" - "),f.append("input").attr("type",
n).attr("id",a.trackClass+"max"+g+a.level).attr("class",l[1]).style("margin-left","5px").attr("value",r),"Color"==g&&(testIE||testSafari)?($("#"+a.trackClass+"min"+g+a.level).spectrum({change:function(b){a.updateSettingsFromUI();a.draw(a.data)}}),$("#"+a.trackClass+"max"+g+a.level).spectrum({change:function(b){a.updateSettingsFromUI();a.draw(a.data)}})):($("input#"+a.trackClass+"min"+g+a.level).on("change",function(){a.updateSettingsFromUI();a.draw(a.data)}),$("input#"+a.trackClass+"max"+g+a.level).on("change",
function(){a.updateSettingsFromUI();a.draw(a.data)})))}"Score"==$("#"+a.trackClass+a.level+"colorSelect").val()?$("div."+a.trackClass+"Scale"+a.level).show():"Color"==$("#"+a.trackClass+a.level+"colorSelect").val()&&$("div."+a.trackClass+"Scale"+a.level).hide();e=b.append("tr").append("td");e.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");a.gsvg.removeTrack(a.trackClass);
var b=svgList[a.gsvg.levelNumber].currentView.ViewID,c=viewMenu[a.gsvg.levelNumber].findTrackByClass(a.trackClass,b);c=viewMenu[a.gsvg.levelNumber].findTrackIndexWithViewID(c.TrackID,b);viewMenu[a.gsvg.levelNumber].removeTrackWithIDIdx(c,b)});e.append("input").attr("type","button").attr("value","Apply").style("float","right").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")});e.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left",
"5px").on("click",function(){a.revertPrevious();a.draw(a.data);$("#trackSettingDialog").fadeOut("fast")})}else b=d3.select(b).select("table").select("tbody"),b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+c.Name),b.append("tr").append("td").html("Sorry no settings for this track."),e=b.append("tr").append("td"),e.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")}),
e.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")})};a.generateTrackSettingString=function(){return a.trackClass+","+a.density+","+a.colorBy+","+a.minValue+","+a.maxValue+","+a.minColor+","+a.maxColor+";"};a.updateFullData(0,0);a.redrawLegend=function(){a.drawLegend([{color:"#DD9C56",label:"All"},{color:"#0f4c75",label:"Predicted(Multi)"},{color:"#3282b8",label:"Predicted(Single)"},
{color:"#5D576B",label:"Array"},{color:"#7A9D98",label:"Array/Predicted"}])};return a}window.GenomeSVG=GenomeSVG;var reportSelectedTrack=null,loadedTrackTable=null,regionDetailLoaded={};$(document).on("click","span.detailMenu",function(f){f=$(this).attr("name");var e=$("span.detailMenu.selected").attr("name");$("div#"+e).hide();$("span.detailMenu.selected").removeClass("selected");$("span[name='"+f+"']").addClass("selected");$("div#"+f).show();if("undefined"!==typeof svgList[0]){e=svgList[0].xScale.domain()[0];var l=svgList[0].xScale.domain()[1];loadRegionReport(f,chr,e,l)}});
...
f&&loadRegionWGCNA(),regionDetailLoaded[f]=e);"regionEQTLTable"===f&&tblFrom&&tblFrom.fnAdjustColumnSizing();gtag("event",f,{event_category:"loadRegionReport"})}
...
(f.source="liver"),l="web/GeneCentric/smallGeneTable.jsp"):-1<(new String(reportSelectedTrack.trackClass)).indexOf("snp")||"qtl"===reportSelectedTrack.trackClass&&(l="web/GeneCentric/bqtlTable.jsp"),l&&loadDivWithPage("div#regionTable",l,!1,f,'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>'));gtag("event",reportSelectedTrack.trackClass,{event_category:"loadTrackTable"})}
function loadEQTLTable(){var f=svgList[0].xScale.domain()[0],e=svgList[0].xScale.domain()[1];loadDivWithPage("div#regionEQTLTable","web/GeneCentric/regionEQTLTable.jsp",!1,{species:organism,minCoord:f,maxCoord:e,chromosome:chr,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,pValueCutoff:pValueCutoff,folderName:regionfolderName,genomeVer:genomeVer},'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>');gtag("event","eQTLTable",{event_category:"loadEQTLTable"})}
function loadRegionWGCNA(){$("div#geneWGCNA").html("");var f=svgList[0].xScale.domain()[0],e=svgList[0].xScale.domain()[1];loadDivWithPage("div#regionWGCNAEQTL","web/GeneCentric/wgcnaGene.jsp",!0,{region:chr+":"+f+"-"+e},'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>');gtag("event","wgcna",{event_category:"loadWGCNA"})}
function loadEQTLTableWParams(f,e,l,n){loadDivWithPage("div#regionEQTLTable","web/GeneCentric/regionEQTLTable.jsp",!1,{species:organism,minCoord:minCoord,maxCoord:maxCoord,chromosome:chr,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,pValueCutoff:n,tissues:l,chromosomes:e,levels:f,folderName:regionfolderName},'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>');gtag("event","eqtlwparams",{event_category:"loadEQTLTableWParams"})}
...
"generateTrackList",{event_category:"BrowserViews"})};e.applyView=function(){var f=e.findSelectedView(),n=e.generateSettingStringFromUI();e.setupImage(n,f);$("div.viewsLevel"+e.level).hide();$("div.trackLevel"+e.level).hide();n="/"+pathPrefix;"db"===f.Source&&(pathPrefix||(n=""),$.ajax({url:n+"addBrowserCount.jsp",type:"GET",cache:!1,data:{viewID:f.ViewID},dataType:"json",success:function(c){},error:function(c,b,a){},async:!0}));gtag("event","applyView",{event_category:"BrowserViews"})};e.applySelectedView=
function(f){for(var l=NaN,c=-1,b=0;b<e.filterList.length&&isNaN(l);b++)e.filterList[b].ViewID===f&&(l=e.filterList[b],c=b);f=e.generateSettingStringFromView(l);e.setupImage(f,l);$("#viewSelect"+e.level).prop("selectedIndex",c);e.selectChange();gtag("event","applySelectedView",{event_category:"BrowserViews"})};e.setupImage=function(f,n){svgList[e.level].removeAllTracks();svgList[e.level].currentView=n;svgViewIDList[e.level]=n.ViewID;loadStateFromString(f,n.imgSettings,e.level,svgList[e.level]);$("span#viewLbl"+
e.level).html("View: "+n.Name);0===n.UserID?($("li#menusaveView"+e.level).addClass("ui-state-disabled"),$("li#menudeleteView"+e.level).addClass("ui-state-disabled")):($("li#menusaveView"+e.level).removeClass("ui-state-disabled"),$("li#menudeleteView"+e.level).removeClass("ui-state-disabled"))};e.updateTrackNumber=function(f,n){$("td.ind",$("table#trackListTbl"+e.level)).each(function(b){var a='<img src="'+iconPath+'grab_flat.png" style="cursor: move; cursor: -webkit-grab; cursor: -moz-grab; " class="handle'+
e.level+'">    ';$(this).html(a+(b+1))});d3.select("table#trackListTbl"+e.level).select("tbody").selectAll("tr").attr("class",function(b,a){return 0===a%2?"even":"odd"});var c=e.findSelectedView();e.generatePreview(c)};e.generateSettingsString=function(f){var l="";d3.select("table#trackListTbl"+e.level).select("tbody").selectAll("tr").each(function(c){l=l+c.TrackClass+","+c.Settings+";"});return l};e.generateSettingStringFromUI=function(){return e.previewSVG.generateSettingsString()};e.getViewData=
function(){var f="/"+pathPrefix;pathPrefix||(f="");$.ajax({url:f+"getBrowserViews.jsp",type:"GET",data:{genomeVer:genomeVer},dataType:"json",success:function(f){e.viewList=[];for(var c=0;c<f.length;c++)if("AA"===f[c].Organism||f[c].Organism===e.curOrg){for(var b=0,a=0;a<f[c].TrackList.length;a++){var g=f[c].TrackList[a];"AA"!==g.Organism&&g.Organism!==e.curOrg||b++}f[c].orgCount=b;e.viewList.push(f[c])}trackInfo&&e.readCookieViews();e.generateViewList();0===e.initialized&&0===e.level?setTimeout(function(){e.initialized=
1;e.applySelectedView($("#defaultView").val())},500):1===e.level&&(e.initialized=1,e.applySelectedView(svgViewIDList[e.level]));gtag("event","getViews",{event_category:"BrowserViews"})},error:function(e,c,b){}})};e.readCookieViews=function(){var f="";if(!0===isLocalStorage()){var n=localStorage.getItem("phenogenCustomViews");n&&(f=n)}else $.cookie("phenogenCustomViews")&&(f=$.cookie("phenogenCustomViews"));if(f&&-1<f.indexOf("<///>"))for(f=f.split("<///>"),n=0;n<f.length;n++){var c=f[n].split("</>"),
b={};for(h=0;h<c.length;h++){var a=c[h].split("=");if("TrackSettingList"===a[0]){a=a[1].split(";");b.TrackList=[];for(var g=0;g<a.length;g++)if(0<a[g].length){var d=a[g].substr(0,a[g].indexOf(",")),p=a[g].substr(a[g].indexOf(",")+1),m={};m.TrackClass=d;m.Settings=p;m.Order=g;b.TrackList.push(m)}}else 2>=a.length?b[a[0]]=a[1]:2<a.length&&(a=c[h].substr(0,c[h].indexOf("=")),g=c[h].substr(c[h].indexOf("=")+1),b[a]=g)}b.Source="local";if(3<c.length){for(var h=c=0;h<b.TrackList.length;h++){if(a=trackInfo[b.TrackList[h].TrackClass])b.TrackList[h].TrackID=
...
gtag("event","saveView",{event_category:"BrowserViews"})};e.saveAsView=function(f,n){e.saveAs=f;e.saveAsImg=n;$("div#nameView"+e.level).show();$("div#selection"+e.level).hide();$("span#viewMenuLbl"+e.level).text("Save As...");$("input#createType"+e.level).val("blank");$("input#function"+e.level).val("saveAs");gtag("event","saveAsView",{event_category:"BrowserViews"})};e.confirmDeleteView=function(f){e.findSelectedView().ViewID!==f.ViewID&&e.setSelectedView(f.ViewID);0<f.UserID||-999===Number(f.UserID)?
($("div#selection"+e.level).hide(),$("#deleteViewName"+e.level).html(f.Name+"    ("+f.orgCount+" tracks)"),$("#confirmDeleteView"+e.level).show()):$("#topcontrolInfo"+e.level).html("This is a predefined view, you may not delete it.")};e.cancelDeleteView=function(){$("#confirmDeleteView"+e.level).hide();$("div#selection"+e.level).show()};e.deleteView=function(){var f=e.findSelectedViewIndex(),n=e.viewList[f];if("db"===n.Source){var c="/"+pathPrefix;pathPrefix||(c="");$.ajax({url:c+"deleteView.jsp",
type:"GET",data:{viewID:n.ViewID},dataType:"json",cache:!1,success:function(a){"Deleted Successfully"===a.status&&(e.viewList.splice(f,1),e.generateViewList());$("#topcontrolInfo"+e.level).html(a.status)},error:function(a,b,c){console.log(c);$("#topcontrolInfo"+e.level).html("There was an error deleting this view.")},async:!0})}else{var b="";c="";if(!0===isLocalStorage()){var a=localStorage.getItem("phenogenCustomViews");a&&(b=a)}else $.cookie("phenogenCustomViews")&&(b=$.cookie("phenogenCustomViews"));
b=b.split("<///>");for(a=0;a<b.length;a++)-1<b[a].indexOf("ViewID="+n.ViewID)||10<b[a].length&&(c=c+b[a]+"<///>");!0===isLocalStorage()?localStorage.setItem("phenogenCustomViews",c):$.cookie("phenogenCustomViews",c);e.viewList.splice(f,1);e.generateViewList()}$("#confirmDeleteView"+e.level).hide();$("div#selection"+e.level).show();$("#viewSelect"+e.level).prop("selectedIndex",0);e.selectChange();svgList[e.level].currentView.ViewID===n.ViewID&&e.applyView();getMainViewData(1);gtag("event","deleteView",
{event_category:"BrowserViews"})};e.cancelView=function(){$("div#nameView"+e.level).hide();$("div#selection"+e.level).show();$("#predefinedSaveAs"+e.level).hide();$("span#viewMenuLbl"+e.level).html("Select/Edit Views")};e.getViewData();e.setupControls();return e};function TrackMenu(f){var e={};e.level=f;e.trackList=[];e.previewLevel=101;e.previewSVG=NaN;e.dataInitialized=0;e.trackDataTable;e.generateTrackTable=function(){for(var f=$("#trackTypeSelect"+e.level).val(),n=[],c=0,b=0;b<e.trackList.length;b++)!("all"===f||"allpublic"===f&&0===e.trackList[b].UserID||"custom"===f&&0!==e.trackList[b].UserID||"genome"===f&&"Genome"===e.trackList[b].GenericCategory||"trxome"===f&&"Transcriptome"===e.trackList[b].GenericCategory)||"AA"!==e.trackList[b].Organism.toUpperCase()&&
e.trackList[b].Organism.toUpperCase()!==organism.toUpperCase()&&e.trackList[b].Organism.toUpperCase()!==organism.toUpperCase()&&e.trackList[b].Organism.substring(0,2).toUpperCase()!==organism.toUpperCase()||0!==$("table#trackListTbl"+e.level+" tr#trk"+e.trackList[b].TrackID).length||(n[c]=e.trackList[b],c++);try{$.fn.DataTable.isDataTable("table#trkSelList"+e.level)&&e.trackDataTable.destroy()}catch(a){}"custom"===f&&0===n.length?($("#noCustomTracks").show(),0===uid?$("#notSignedIn1").show():$("#notSignedIn1").hide()):
($("#noCustomTracks").hide(),$("#notSignedIn1").hide());d3.select("table#trkSelList"+e.level).select("tbody").selectAll("tr").remove();d3.select("table#trkSelList"+e.level).select("tbody").selectAll("tr").data(n).enter().append("tr").attr("id",function(a){return"trk"+a.TrackID}).attr("class",function(a,b){var c="odd";0===b%2&&(c="even");return c}).each(function(a,b){var c="",g="";"local"===a.Source&&(g=" (Local)");0!==a.UserID&&(c=new String(a.SetupDate),c=c.substr(0,c.indexOf(":",c.indexOf(":")+
1)),c="<BR><BR>File:"+a.OriginalFile+"<BR><BR>Setup Date:"+c+"<BR><BR>Type:"+a.Type,"local"===a.Source&&(c+="<BR><BR>Local Track only available on this computer."));c='  <span class="trlisttooltip'+e.level+'" title= "'+a.Description+c+'"><img src="'+iconPath+'info.gif"></span>';d3.select(this).append("td").html(a.Name+g+c);g="";"RN"===a.Organism?g="Rat":"MM"===a.Organism&&(g="Mouse");c="Public";0!==a.UserID&&(c="Custom");d3.select(this).append("td").html(g);d3.select(this).append("td").html(a.GenericCategory);
d3.select(this).append("td").html(a.Category);d3.select(this).append("td").html(c)});$.fn.DataTable.isDataTable("table#trkSelList"+e.level)||(f=e.getInitTrackTableHeight(n),e.trackDataTable=$("table#trkSelList"+e.level).DataTable({bPaginate:!1,aaSorting:[[3,"asc"]],sScrollY:f,sDom:'<"rightSearch"fr><t>',columnDefs:[{width:"45%",targets:0}]}));e.trackDataTable.draw();$("td#selectedTrack"+e.level).hide();$("table#trkSelList"+e.level+" tbody").on("click","tr",function(){if(!$(this).hasClass("selected")){$("table#trkSelList"+
e.level+" tbody tr.selected").removeClass("selected");$(this).addClass("selected");$(this).attr("id");var a=e.findSelectedTrack(),b="Selected Track Name: "+a.Name+'  <span class="trInfotooltip'+e.level+'" title= "'+a.Description+'"><img src="'+iconPath+'info.gif"></span>';$("div#trackHeaderOuter"+e.level+" #trackHeaderContent").html(b);b=e.getTrackTableHeight(n);e.trackDataTable.settings()[0].oScroll.sy=b;try{e.trackDataTable.columns.adjust().draw()}catch(d){}$("#trkSelList"+e.level+"_wrapper div.dataTables_scroll div.dataTables_scrollBody").css("height",
...
error:function(e,c,b){console.log(b)}})};e.readCookieTracks=function(){var f="";if(!0===isLocalStorage()){var n=localStorage.getItem("phenogenCustomTracks");n&&(f=n)}else $.cookie("phenogenCustomTracks")&&(f=$.cookie("phenogenCustomTracks"));if(f&&-1<f.indexOf("<;>"))for(f=f.split("<;>"),n=0;n<f.length;n++){var c=f[n].split("<->"),b={};for(k=0;k<c.length;k++){var a=c[k].split("=");if(2>=a.length)b[a[0]]=a[1];else if(2<a.length){a=c[k].substr(0,c[k].indexOf("="));var g=c[k].substr(c[k].indexOf("=")+
1);if("TrackID"===a||"TrackClass"===a)g=g.replace(/\./g,"_");b[a]=g}}b.Source="local";3<c.length&&(e.trackList.push(b),0===e.level&&(trackInfo[b.TrackClass]=b))}};e.findSelectedTrack=function(){var f=NaN;if(0<$("table#trkSelList"+e.level+" tbody tr.selected").length){var n=$("table#trkSelList"+e.level+" tbody tr.selected").attr("id");n=n.substr(3);for(var c=0;c<e.trackList.length&&isNaN(f);c++)e.trackList[c].TrackID===n&&(f=e.trackList[c])}return f};e.generateSettingsString=function(e){return e.TrackClass+
","+e.Settings+";"};e.generatePreview=function(f){$("div#trackPreviewOuter"+e.level+" div#trackPreviewContent").html("");var l=(new String(organism)).toUpperCase();if("AA"===f.Organism.substring(0,2)||f.Organism.substring(0,2)===l){l=svgList[e.level].xScale.domain()[0];var c=svgList[e.level].xScale.domain()[1];e.previewSVG=toolTipSVG("div#trackPreviewOuter"+e.level+" div#trackPreviewContent",565,l,c,e.previewLevel,chr,svgList[e.level].type);e.previewSVG.folderName=svgList[e.level].folderName;f=e.generateSettingsString(f);
loadStateFromString(f,"",e.previewLevel,e.previewSVG);e.previewSVG.updateData();e.previewSVG.updateFullData();$("div#trackPreviewContent div#ScrollLevel"+e.previewLevel).css("max-height","250px").css("overflow","auto")}else $("div#previewOuter"+e.level+" div#previewContent").html("No Preview Available: The current organism doesn't match the tracks included in this view.")};e.removeTrack=function(f){e.generateTrackTable()};e.confirmdeleteCustomTrack=function(){var f=e.findSelectedTrack();$("#deleteUsrTrack"+
e.level).show();$("#selectTrack"+e.level).hide();f="Name: <B>"+f.Name+"</B><BR>Description: <B>"+f.Description+"</B><BR>Original File: <B>"+f.OriginalFile+"</B><BR>Setup Date: <B>"+f.SetupDate+"</b>";$("span#customTrackInfo"+e.level).html(f);gtag("event","deleteCustomTrack",{event_category:"BrowserTracks"})};e.cancelDeleteTrack=function(){$("#deleteUsrTrack"+e.level).hide();$("#selectTrack"+e.level).show()};e.deleteTrack=function(){var f=e.findSelectedTrack();if("local"===f.Source){var n="TrackClass="+
f.TrackClass+"<->",c="";if(!0===isLocalStorage()){var b=localStorage.getItem("phenogenCustomTracks");b&&(c=b)}else $.cookie("phenogenCustomTracks")&&(c=$.cookie("phenogenCustomTracks"));if(c&&-1<c.indexOf("<;>")){var a="";c=c.split("<;>");for(b=0;b<c.length;b++)-1===c[b].indexOf(n)&&(a=a+c[b]+"<;>");n="/"+pathPrefix;pathPrefix||(n="");$.ajax({url:n+"removeCustomTrack.jsp",type:"GET",data:{trackID:f.TrackID},dataType:"json",success:function(b){!0===isLocalStorage()?localStorage.setItem("phenogenCustomTracks",
a):$.cookie("phenogenCustomTracks",a)},error:function(a,b,c){console.log(c)},async:!1})}}else"db"===f.Source&&(n="/"+pathPrefix,pathPrefix||(n=""),$.ajax({url:n+"deleteTrack.jsp",type:"GET",data:{trackID:f.TrackID},dataType:"json",success:function(a){"true"===a.success?$(".uploadStatus").html("<B>Track Setup Successfully</B>"):$(".uploadStatus").html("<B>Track Setup Failed</B>")},error:function(a,b,c){console.log(c)},async:!1}));e.getTrackData();$("#deleteUsrTrack"+e.level).hide();$("#selectTrack"+
e.level).show();e.generateTrackTable();gtag("event","deleteTrack",{event_category:"BrowserTracks"})};e.confirmUpload=function(){$("div#confirmUpload"+e.level).show();$("div#uploadBtn"+e.level).hide()};e.confirmBed=function(){$("input#hasconfirmBed"+e.level).val(1);e.createCustomTrack(e.level)};e.cancelUpload=function(){$("div#confirmUpload"+e.level).hide();$("div#confirmBed"+e.level).hide();$("div#uploadBtn"+e.level).show()};e.createCustomTrack=function(){$("div#confirmUpload"+e.level).hide();customTrackLevel=
e.level;var f=$("#usrtrkFileTypeSelect"+e.level).val();"bed"===f||"bg"===f?e.createUploadedCustomTrack():("bb"===f||"bw"===f)&&e.createRemoteCustomTrack();gtag("event","createCustomTrack",{event_category:"BrowserTracks"})};e.createUploadedCustomTrack=function(){var f=$("input#customUploadFile"+e.level)[0].files[0],n=$("#usrtrkFileTypeSelect"+e.level).val(),c=f.name,b=f.size/1024/1024;$(".uploadStatus").show();var a="";if(0<c.indexOf(".")){for(var g=c;-1<g.indexOf(".");)g=g.substr(g.indexOf(".")+1);
""!==g&&g!==c&&(a=g)}20>b?a!==n?"gz"===a||"tar"===a||"zip"===a||"exe"===a||"bin"===a?(setTimeout(function(){$("div#uploadBtn"+customTrackLevel).show();$(".progressInd").hide()},5E3),$(".uploadStatus").html("Error: Selected File Type is not supported.")):0===$("input#hasconfirmBed"+e.level).val()?$("div#confirmBed"+customTrackLevel).show():1===$("input#hasconfirmBed"+e.level).val()&&($("div#confirmBed"+customTrackLevel).hide(),e.createTrack(f)):e.createTrack(f):($("div#uploadBtn"+customTrackLevel).show(),
$(".progressInd").hide(),$(".uploadStatus").html("File is too large.  20MB is the current limit."))};e.createTrack=function(f){e.readFile(f)};e.createRemoteCustomTrack=function(){$(".uploadStatus").html("Setting up track...");$(".uploadStatus").show();var f=$("#customFileURL"+e.level).val(),n="",c=new Date;$.cookie("JSESSIONID")&&(n=$.cookie("JSESSIONID"),n=n+"_"+c.getTime());e.saveTrack(n,f)};e.saveTrack=function(f,n){var c=$("#usrtrkNameTxt"+e.level).val(),b=$("#usrtrkDescTxt"+e.level).val(),a=
$("#usrtrkOrgSelect"+e.level).val(),g=$("#usrtrkGenCatSelect"+e.level).val(),d=$("#usrtrkCatTxt"+e.level).val(),p="",m=$("#usrtrkFileTypeSelect"+e.level).val(),h="";if("bed"===m||"bg"===m)h=$("input#customUploadFile"+e.level)[0].files[0].name;var l="Color";0<$("#usrtrkColorSelect"+e.level).length&&(l=$("#usrtrkColorSelect"+e.level).val());if("Score"===l){var t="000000",z="FFFFFF",r=1,w=1E3;testIE||testSafari?(t=$("#usrtrkColorMin"+e.level).spectrum("get").toHexString(),z=$("#usrtrkColorMax"+e.level).spectrum("get").toHexString()):
(t=$("#usrtrkColorMin"+e.level).val(),z=$("#usrtrkColorMax"+e.level).val());t=t.substr(1);z=z.substr(1);r=$("#usrtrkScoreMinTxt"+e.level).val();w=$("#usrtrkScoreMaxTxt"+e.level).val()}"bed"===m?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Pack:3}{Full:2};Default=3,Name=Color by;Select:colorSelect::colorSelect;{Feature Defined:Color}{Score based scale:Score};Default="+l+",Name=Scale Range;Txt:tissuecbx;{min:1}{max:1000};Default="+r+":"+w+",Name=Color Range;Txt:color;{min:FFFFFF}{max:000000};Default="+
t+":"+z:"bg"===m?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Full:2};Default=1,Name=Scale Range;Slider:rangeslider;{min:1-1000}{max:1000-20000};Default=1:5000":"bw"===m?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Full:2};Default=1,Name=Scale Range;Slider:rangeslider;{min:1-1000}{max:1000-20000};Default=1:5000":"bb"===m&&(p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Pack:3}{Full:2};Default=3,Name=Color by;Select:colorSelect::colorSelect;{Feature Defined:Color}{Score based scale:Score};Default="+
l+",Name=Scale Range;Txt:tissuecbx;{min:1}{max:1000};Default="+r+":"+w+",Name=Color Range;Txt:color;{min:FFFFFF}{max:000000};Default="+t+":"+z);f="custom"+f;0<uid?(l="/"+pathPrefix,pathPrefix||(l=""),$.ajax({url:l+"addTrack.jsp",type:"GET",data:{trackClass:f,trackName:c,trackDesc:b,trackOrg:a,genericCategory:g,category:d,controls:p,location:n,type:m,file:h},dataType:"json",success:function(a){"true"===a.success?$(".uploadStatus").html("<B>Track Setup Successfully</B>"):$(".uploadStatus").html("<B>Track Setup Failed</B>")},
error:function(a,b,c){console.log(c)},async:!1})):(l=new Date,c="TrackID="+f+"<->UserID=-999<->TrackClass="+f+"<->Name="+c+"<->Description="+b+"<->Organism="+a.toUpperCase(),c=c+"<->Settings=<->Order=0<->GenericCategory="+g+"<->Category="+d+"<->Controls="+p+"<->SetupDate="+l+"<->Type="+m+"<->OriginalFile="+h+"<->Location="+n,c=c+"<->genomeVer="+genomeVer,c+="<;>",!0===isLocalStorage()?((g=localStorage.getItem("phenogenCustomTracks"))&&(c=g+c),localStorage.setItem("phenogenCustomTracks",c)):($.cookie("phenogenCustomTracks")&&
(c=$.cookie("phenogenCustomTracks")+c),$.cookie("phenogenCustomTracks",c)),$(".uploadStatus").html("<B>Track Setup Successfully</B>"));$("div#finished"+e.level).show();$("input#customUploadFile"+e.level).val("");$("input#customURL"+e.level).val("");$("input#usrtrkNameTxt"+e.level).val("");$("#usrtrkDescTxt"+e.level).val("");$("input#usrtrkOrgSelect"+e.level).val("Rn");$("input#usrtrkGenCatSelect"+e.level).val("Genome");$("input#usrtrkCatTxt"+e.level).val("");$("#usrtrkColorSelect"+e.level).val("Color");
$("#customFileURL"+e.level).val("");e.getTrackData();gtag("event","saveTrack",{event_category:"BrowserTracks"})};e.readFile=function(f){var l=new FileReader;l.readAsText(f,"UTF-8");l.onload=e.sendFile};e.sendFile=function(f){$.ajax({url:pathPrefix+"trackUpload.jsp",type:"POST",contentType:"text/plain",xhr:function(){var f=$.ajaxSettings.xhr();f.upload.addEventListener("progress",e.progressHandlingFunction,!1);return f},data:f.target.result,processData:!1,cache:!1,dataType:"json",success:function(f){$(".uploadStatus").html("<B>Upload Completed Successfully</b>");
e.saveTrack(f.trackFile,f.trackFile);gtag("event","customTrackUploaded",{event_category:"BrowserTracks"})},error:function(e,c,b){console.log(b);$(".uploadStatus").html("Error:"+b);$("div#uploadBtn"+customTrackLevel).show()}})};e.progressHandlingFunction=function(e){$(".progressInd").show();$(".uploadStatus").html("Uploading...");e.lengthComputable&&$("progress").attr({value:e.loaded,max:e.total})};e.getTrackData();return e};$(window).resize(function(){"undefined"!==typeof wgcna&&("undefined"!==typeof wgcna.multiImage?wgcna.multiImage.draw():"undefined"!==typeof wgcna.singleImage&&wgcna.singleImage.draw())});function replaceDot(f){return f.replace(/\./g,"_")}function replaceUnderscore(f){return f.replace(/_/g,".")}function removeColon(f){return f.replace(/:/g,"")}
function WGCNABrowser(f,e,l,n,c,b){that={};that.singleID=f;that.region=e;that.dispType=n;that.viewType=c;that.geneList=l;that.moduleList=[];that.moduleGenes={};that.modules={};that.tissue=b;that.panel="";that.requests=0;that.skipGrey=1;that.chrLen=19;that.selSource="seq";that.version="2";that.moduleRequestList=[];"Mm"===organism?(that.panel="ILS/ISS",that.wDSID=1):"Rn"===organism&&("rn5"===genomeVer?(that.panel="BNLx/SHR",that.chrLen=20,that.wDSID=2):(that.panel="BNLx/SHR",that.chrLen=20,"seq"===
that.selSource?"5"===that.version?"Whole Brain"===b?that.wDSID=11:"Liver"===b&&(that.wDSID=12):"2"===that.version?"Whole Brain"===b?that.wDSID=8:"Liver"===b&&(that.wDSID=9):"Whole Brain"===b?that.wDSID=6:"Liver"===b&&(that.wDSID=7):"Whole Brain"===b?that.wDSID=3:"Heart"===b?that.wDSID=4:"Liver"===b&&(that.wDSID=5)));that.eQTLKey=function(a){return"Link_"+a.Snp};that.mirKey=function(a){return a.ID};that.modKey=function(a){return a.MODULE};that.removeInvalidIDChars=function(a){"undefined"!==typeof a&&
(-1<a.indexOf("*")&&(a=a.replace(/\*/g,"star")),-1<a.indexOf(" ")&&(a=a.replace(/\s/g,"_")),-1<a.indexOf(".")&&(a=a.replace(/\./g,"_")));return a};that.setup=function(){that.requestModuleList();that.createImageControls();that.createViewControls();that.createDataControls();$(".wgcnaControltooltip").tooltipster({position:"top-right",maxWidth:250,offsetX:5,offsetY:5,contentAsHTML:!0,interactive:!0,interactiveTolerance:350});gtag("event","setupWGCNA",{event_category:"setupWGCNA"})};that.requestModuleList=
function(){that.moduleGenes={};that.moduleList=[];that.modules={};$.ajax({url:pathPrefix+"getWGCNAModules.jsp",type:"GET",async:!0,data:{source:that.selSource,modFileType:that.viewtype,id:that.singleID,organism:organism,panel:that.panel,tissue:that.tissue,region:that.region,geneList:that.geneList,genomeVer:genomeVer,version:that.version},dataType:"json",beforeSend:function(){$("#waitCircos").show();for(var a=0;a<that.moduleRequestList.length;a++)that.moduleRequestList[a].abort();that.moduleRequestList=
[];console.log("request mod list:"+that.wDSID+":"+that.selSource)},success:function(a){$("#wgcnaGeneImage #message").hide();that.moduleList=[];0<a.length&&(1!==a.length||"grey"!==a[0].ModuleID)?setTimeout(function(){for(var b=0;b<a.length;b++)console.log("Mod:"+a[b].ModuleID),"grey"!==a[b].ModuleID&&(that.moduleList.push(a[b].ModuleID),that.requests++,that.requestModule(a[b].ModuleID),"undefined"!==typeof a[b].GeneList&&(that.moduleGenes[a[b].ModuleID]=a[b].GeneList),console.log("after calling:"+
that.requests+":"+a[b].ModuleID));that.refreshRegion(1E3);setTimeout(function(){$("html, body").animate({scrollTop:$("#multiWGCNAScroll").offset().top},100)},1200)},1):0===a.length?that.displayMessage("There are no transcripts with Affymetrix Probesets that represent this gene."):1===a.length&&"grey"===a[0].ModuleID&&that.displayMessage("Only the Grey module contains transcripts for this gene.  The Grey module contains all transcripts that did not have a high enough correlation with any other transcripts to be included in another module.");
gtag("event","requestModuleList",{event_category:"requestModuleList"})},error:function(a,b,c){}})};that.displayMessage=function(a){$("#waitCircos").hide();$("#wgcnaGeneImage #message").html("<BR><BR><h2>"+a+"</h2><BR><BR>").show()};that.refreshRegion=function(a){setTimeout(function(){that.createMultiWGCNAImage();0<that.requests?that.refreshRegion(2E3):1!==a&&(that.refreshRegion(1),$("#waitCircos").hide())},a)};that.requestModule=function(a,b){xhr=$.ajax({url:contextRoot+"tmpData/browserCache/"+genomeVer+
"/modules/ds"+that.wDSID+"/"+a+".json",type:"GET",async:!0,data:{},dataType:"json",success:function(c){console.log("success:"+contextRoot+"tmpData/browserCache/"+genomeVer+"/modules/ds"+that.wDSID+"/"+a+".json");if(0<that.singleID.length)that.countGeneInstance(that.singleID,c);else{var d=that.moduleGenes[a].split(",");c.GroupedGenes=d.length;that.countGeneInstance(that.moduleGenes[a],c)}that.modules[a]=c;that.requests--;b&&b(a);gtag("event",a,{event_category:"requestModule"})},error:function(a,b,
c){that.requests--}});that.moduleRequestList.push(xhr)};that.scanFilter=function(a,b){var c=[];if(a)for(var e=0;e<a.length;e++)b(a[e])&&c.append(a[e]);return c};that.countGeneInstance=function(a,b){var c=b.TCList,e=0;b.selectGeneTC={};for(var f=0;f<c.length;f++)-1<a.indexOf(c[f].Gene.ID)&&e++;b.GeneCount=e};that.createImageControls=function(){that.imageBar=d3.select("#imageControl").append("span").style("float","left");that.imageBar.append("span").attr("class","saveImage control").style("display",
"inline-block").attr("id","saveImage").style("cursor","pointer").append("img").attr("src",contextRoot+"web/images/icons/savePic_dark.png").attr("pointer-events","all").attr("cursor","pointer").on("click",function(){var a="";if(that.singleImage){"eqtl"===that.viewType&&"eqtl"===that.singleImage.type?a=$("div#circos").html():"gene"===that.viewType&&"gene"===that.singleImage.type?a=$("div#viewGene").html():"mir"===that.viewType&&"mir"===that.singleImage.type?a=$("div#viewGene").html():"go"===that.viewType&&
"go"===that.singleImage.type&&(a=$("div#viewGene").html());var b=$(window).width();a=a.replace('width="100%"','width="'+b+'px"');$.ajax({url:pathPrefix+"saveBrowserImage.jsp",type:"POST",contentType:"text/html",data:a+"\n",processData:!1,dataType:"json",success:function(a){var b=new Date;b=b.getMonth()+1+"_"+b.getDate()+"_"+b.getFullYear();var c="http://";"https:"===location.protocol&&(c="https://");var d=c+urlprefix+"/tmpData/download/"+a.imageFile;a="";that.selectedModule&&(a=new String(that.selectedModule.MOD_NAME));
a=a+"_"+that.viewType;var e=a+"_"+b+".png",f=new XMLHttpRequest;f.open("GET",d);f.responseType="blob";f.send();f.onreadystatechange=function(){if(4!=f.readyState||200!=f.status)return!1;var a=document.createElement("a");a.href=window.URL.createObjectURL(f.response);a.download=e;a.style.display="none";document.body.appendChild(a);try{a.click()}catch(r){$("#wgcnaMouseHelp").html("<span style='color:#FF0000;'>Your browser will not save the image directly. Image will open in a popup, in the new window right click to save image.</span>"),
window.open(d)}return!0}},error:function(a,b,c){console.log(c)}});gtag("event","saveWGCNAView",{event_category:"saveWGCNAView"})}}).on("mouseover",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/savePic_white.png");$("#wgcnaMouseHelp").html("Click to download a PNG image of the current view.")}).on("mouseout",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/savePic_dark.png");$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")});
...
contextRoot+'web/images/icons/help.png"> above for a detailed description of each view.').style("margin-left","5px").append("img").attr("src","/web/images/icons/info.gif");$("#HelpWGCNAView").on("click",function(a){var b=$(this).attr("id");$("#"+b+"Content").dialog("option","position",{my:"right top",at:"left bottom",of:$(this)});$("#"+b+"Content").dialog("open").css({"font-size":12});a.stopPropagation()})};that.createDataControls=function(){that.dataBar=d3.select("#dataControl").append("span").attr("id",
...
["seq","array"],that.dispSources=["RNA-Seq","Arrays"]);for(var c=0;c<that.sources.length;c++)b.append("option").attr("value",that.sources[c]).text(that.dispSources[c]);a.append("br");b=a.append("span").attr("id","spanVersion");a.append("text").text("Tissue:");b=a.append("select").attr("id","wgcnaTissueSelect").on("change",function(){$("#singleWGCNASVG").hide();d3.select("#multiWGCNASVG").remove();$("#circosModule").hide();$("#wgcnaModuleTable").hide();$("#wgcnaMirTable").hide();$("#wgcnaMirGeneTable").hide();
$("#wgcnaGoTable").hide();$("#wgcnaEqtlTable").hide();that.selSource=$("#wgcnaSourceSelect").val();that.tissue=$("#wgcnaTissueSelect").val();"array"===that.selSource?"Whole Brain"===that.tissue?that.wDSID=3:"Heart"===that.tissue?that.wDSID=4:"Liver"===that.tissue&&(that.wDSID=5):"seq"===that.selSource&&("Whole Brain"===that.tissue?(that.wDSID=11,"2"===that.version?that.wDSID=8:"1"===that.version&&(that.wDSID=6)):"Liver"===that.tissue&&(that.wDSID=12,"2"===that.version?that.wDSID=9:"1"===that.version&&
(that.wDSID=7)));that.requestModuleList()});that.tissues=["Brain"];that.dispTissues=["Whole Brain"];"Rn"===organism&&"rn6"===genomeVer&&(that.source&&"arrays"===that.source?(that.tissues=["Whole Brain","Heart","Liver"],that.dispTissues=["Whole Brain","Heart","Liver"]):(that.tissues=["Whole Brain","Liver"],that.dispTissues=["Whole Brain","Liver"]));for(c=0;c<that.tissues.length;c++)b.append("option").attr("value",that.tissues[c]).text(that.dispTissues[c]);a.append("br");b=a.append("span").attr("id",
"spanVersion");b.append("text").text("Version:");b=b.append("select").attr("id","wgcnaVersionSelect").on("change",function(){that.version=$("#wgcnaVersionSelect").val();"Whole Brain"===that.tissue?(that.wDSID=11,"2"===that.version?that.wDSID=8:"1"===that.version&&(that.wDSID=6)):"Liver"===that.tissue&&(that.wDSID=12,"2"===that.version?that.wDSID=9:"1"===that.version&&(that.wDSID=7));that.requestModuleList()});that.ver=["5","2","1"];that.dispVer=["HRDP v5","HRDP v4","HRDP v3"];for(c=0;c<that.ver.length;c++)b.append("option").attr("value",
that.ver[c]).text(that.dispVer[c]);d3.select("#wgcnaVersionSelect").select('option[value="'+that.version+'"]').attr("selected","selected");a=that.dataBar.append("td").append("div").attr("id","goCtls").attr("class","goCTL").style("display","none").style("margin-left","10px");a.append("text").text("GO Domain:");b=a.append("select").attr("id","goDomainSelect").on("change",function(){that.singleImage.goDomain=$("#goDomainSelect").val();that.singleImage.draw();gtag("event","changeDomain",{event_category:"WGCNA_go"})});
b.append("option").attr("value","cc").text("Cellular Component");b.append("option").attr("value","bp").text("Biological Process");b.append("option").attr("selected","selected").attr("value","mf").text("Molecular Function");a.append("br");a.append("text").text("Number of Levels to Display:");a.append("input").attr("name","value").attr("id","goSpinner");$("#goSpinner").spinner({min:1,max:100,step:1,change:function(a,b){console.log("change");that.singleImage.displayDepth=b.value;that.singleImage.selectedNode!==
that.singleImage.data.GOList[0]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[1]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[2]?that.singleImage.redraw():that.singleImage.draw();gtag("event","changeDepth",{event_category:"WGCNA_go"})},spin:function(a,b){that.singleImage.displayDepth=b.value;that.singleImage.selectedNode!==that.singleImage.data.GOList[0]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[1]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[2]?
that.singleImage.redraw():that.singleImage.draw();gtag("event","changeDepth",{event_category:"WGCNA_go"})}}).val(3);a=that.dataBar.append("td").append("div").attr("id","mirCtls").attr("class","mirCTL").style("display","none").style("margin-left","10px");b=a.append("table").append("tr");a.append("text").style("font-weight","bold").text("Filter miRNAs:");a.append("br");a.append("input").attr("type","checkbox").style("margin-right","5px").attr("id","mirValidatedCBX").on("click",function(){$(this).is(":checked")?
that.singleImage.mirValid=1:that.singleImage.mirValid=0;that.singleImage.drawMiR();gtag("event","changeValidated",{event_category:"WGCNA_mir"})});a.append("text").text("only miRNAs with Validated Targets");a.append("br");a.append("text").style("margin-right","5px").text("# of targeted Genes: ");a.append("div").attr("id","minMirGeneSlider").style("display","inline-block").style("width","150px");a.append("span").attr("id","minMirGeneLabel").style("margin-left","5px").append("text").text("1");a.append("br");
a.append("text").style("margin-right","5px").text("miRNAs targeting gene: ");a.append("input").attr("type","text").attr("id","mirGeneFilterTxt").on("keyup",function(a){that.singleImage.mirTargetID=$(this).val().toLowerCase();that.singleImage.drawMiR();gtag("event","geneFilter",{event_category:"WGCNA_mir"})});a.append("br");a.append("text").style("margin-right","5px").text("miRNA(ID or Accession): ");a.append("input").attr("type","text").attr("id","mirIDFilterTxt").on("keyup",function(a){that.singleImage.mirID=
$(this).val().toLowerCase();that.singleImage.drawMiR();gtag("event","geneFilter",{event_category:"WGCNA_mir"})});a=that.dataBar.append("td").append("div").attr("id","mirCtls2").attr("class","mirCTL").style("display","none").style("margin-left","10px");a.append("text").style("font-weight","bold").text("Display Options:");a.append("br");a.append("input").attr("type","checkbox").attr("id","mirSortCBX").style("margin-right","5px").on("click",function(){$(this).prop("checked")?that.singleImage.sortMir=
$("#mirSortSelect").val():that.singleImage.sortMir=0;that.singleImage.drawMiR();gtag("event","mirSort",{event_category:"WGCNA_mir"})});a.append("text").text("Hovering over a node(miR or transcript):");a.append("br");a.append("input").attr("type","checkbox").attr("id","mirLinkCBX").attr("checked","checked").style("margin-left","12px").style("margin-right","3px").on("click",function(){$(this).prop("checked")?that.singleImage.showMirLinks=1:that.singleImage.showMirLinks=0;gtag("event","showLinks",{event_category:"WGCNA_mir"})});
a.append("text").text("Display links between miRNAs and targeted transcripts");a.append("br");a.append("input").attr("type","checkbox").attr("id","mirgeneLinkCBX").style("margin-left","12px").style("margin-right","3px").on("click",function(){$(this).prop("checked")?(that.singleImage.showLinks=0,$("div#mirLinkColorSelect").show()):(that.singleImage.showLinks=1,$("div#mirLinkColorSelect").hide());that.singleImage.setupLegend();gtag("event","linkColor",{event_category:"WGCNA_mir"})});a.append("text").text("Display links between correlated transcripts");
b=a.append("div").attr("id","mirLinkColorSelect").style("display","none");b.append("text").style("margin-left","24px").text("Correlation Link Color Options:");a=b.append("select").attr("id","mirlinkColor").on("change",function(){that.singleImage.updateColor("mirlinkColor");that.singleImage.setupLegend();gtag("event","linkColor",{event_category:"WGCNA_mir"})});a.append("option").attr("selected","selected").attr("value","Green_Red").text("Green(+) / Red(-)");a.append("option").attr("value","Blue_Yellow").text("Blue(+) / Yellow(-)");
a.append("option").attr("value","Red_Green").text("Red(+) / Green(-)");b.append("br");b.append("input").attr("type","checkbox").style("margin-left","24px").style("margin-right","3px").on("click",function(){$(this).prop("checked")?that.singleImage.showAllFromTarget=1:that.singleImage.showAllFromTarget=0;gtag("event","linkColor",{event_category:"WGCNA_mir"})});b.append("text").text("Display all cor. transcript links from targeted transcripts.");$("#minMirGeneSlider").slider({min:1,max:100,step:1,value:1,
slide:function(a,b){that.singleImage.mirGeneCutoff=b.value;$("span#minMirGeneLabel").html(b.value);that.singleImage.drawMiR();gtag("event","minMir",{event_category:"WGCNA_mir"})}});a=that.dataBar.append("td").append("div").attr("id","linkCtls").attr("class","linkCTL").style("display","none").style("margin-left","10px");a.append("text").text("Link Correlation Values");a.append("span").style("cursor","pointer").on("click",function(){$("#linkSliderMin").slider("value",.75);that.singleImage.CorCutoff_min=
.75;$("span#minLabel").html(.75);$("#linkSliderMax").slider("value",1);that.singleImage.CorCutoff_max=1;$("span#maxLabel").html(1);that.singleImage.draw();gtag("event","resetCorrelation",{event_category:"WGCNA_mir"})}).append("img").attr("src",contextRoot+"/web/images/icons/reset_16.png");c=a.append("table");b=c.append("tr");b.append("td").append("text").text("Min:");b.append("td").append("div").attr("id","linkSliderMin").style("width","100px");b.append("td").append("span").attr("id","minLabel").style("margin-left",
"5px").append("text").text("0.75");b=c.append("tr");b.append("td").append("text").text("Max:");b.append("td").append("div").attr("id","linkSliderMax").style("width","100px");b.append("td").append("span").attr("id","maxLabel").style("margin-left","5px").append("text").text("1");a.append("input").attr("type","checkbox").attr("id","hideLinkExceptNode").on("click",function(){that.singleImage.showLinks=!$(this).is(":checked");that.singleImage.draw();that.singleImage.panZoom=svgPanZoom("#singleWGCNASVG",
{panEnabled:!0,controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,minZoom:.1,maxZoom:4,fit:!0,center:!0})});a.append("text").text("Hide Links Except Selected Node");b=that.dataBar.append("td").append("div").attr("id","linkCtl2").attr("class","linkCTL").style("display","none").style("margin-left","10px");b.append("text").text("Correlation Link Color Options:");b.append("br");b=b.append("select").attr("id","linkColor").on("change",function(){that.singleImage.draw();
that.singleImage.panZoom=svgPanZoom("#singleWGCNASVG",{panEnabled:!0,controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,minZoom:.1,maxZoom:4,fit:!0,center:!0})});b.append("option").attr("selected","selected").attr("value","Green_Red").text("Green(+) / Red(-)");b.append("option").attr("value","Blue_Yellow").text("Blue(+) / Yellow(-)");b.append("option").attr("value","Red_Green").text("Red(+) / Green(-)");$("#linkSliderMin").slider({min:.05,max:.95,step:.05,value:.75,
slide:function(a,b){that.singleImage.CorCutoff_min=b.value;$("span#minLabel").html(b.value);that.singleImage.draw()}});$("#linkSliderMax").slider({min:.1,max:1,step:.05,value:1,slide:function(a,b){that.singleImage.CorCutoff_max=b.value;$("span#maxLabel").html(b.value);that.singleImage.draw()}});a=that.dataBar.append("td").append("div").attr("class","eqtlCtls").attr("id","eqtlCtl").style("display","none").style("margin-left","10px");a.append("text").text("Select Chromosomes");a.append("br");b=a.append("select").attr("id",
"eqtlChr").attr("multiple","multiple").attr("size","4").style("width","100px");for(c=1;c<=that.chrLen;c++)b.append("option").attr("selected","selected").attr("value",organism.toLowerCase()+c).text("chr"+c);b.append("option").attr("selected","selected").attr("value",organism.toLowerCase()+"X").text("chrX");a=that.dataBar.append("td").append("div").attr("class","eqtlCtls").attr("id","eqtlCtl2").style("display","none").style("margin-left","10px");a.append("text").text("P-value cutoff:");b=a.append("select").attr("id",
"eqtlPval");b.append("option").attr("value","1").text("0.1");b.append("option").attr("selected","selected").attr("value","2").text("0.01");b.append("option").attr("value","3").text("0.001");b.append("option").attr("value","4").text("0.0001");b.append("option").attr("value","5").text("0.00001");a.append("br");a.append("br");a.append("br");a.append("br");a.append("input").attr("type","button").attr("value","Apply Selections").on("click",function(){for(var a=$("#eqtlPval").val(),b=$("#eqtlChr").val(),
c="",d=0;d<b.length;d++)c=c+b[d]+";";$.ajax({url:pathPrefix+"runCircosModule.jsp",type:"GET",async:!0,data:{cutoffValue:a,organism:organism,module:that.selectedModule.MOD_NAME,modColor:that.selectedModule.ModRGB,chrList:c,genomeVer:genomeVer,tissue:that.tissue},dataType:"json",beforeSend:function(){$("#waitCircos").show()},success:function(a){$("#waitCircos").hide();that.singleImage.update(a.URL)},error:function(a,b,c){}})})};that.positionTTLeft=function(a){a+=20;a>$(window).width()/2&&(a-=250);return a+
"px"};that.positionTTTop=function(a){a+="px";d3.event.clientY>.6*window.innerHeight&&(a=d3.event.pageY-2.2*$(".testToolTip").height()+"px");return a};that.createMultiWGCNAImage=function(){that.multiImage=that.multiWGCNAImageGeneView();that.multiImage.draw();that.img=that.multiImage};that.multiWGCNAImage=function(){var a={maxWidth:200};a.totalWidth=$(window).width();a.modPerLine=Math.floor($(window).width()/a.maxWidth);a.calcWidth=Math.floor($(window).width()/a.modPerLine);a.data=[];if(0===d3.select("#multiWGCNASVG").size()){var b=
d3.select("#wgcnaGeneImage").append("div").attr("id","multiWGCNAScroll").style("overflow","auto").style("max-height","430px");a.svg=b.append("svg").attr("id","multiWGCNASVG").attr("height","430px").attr("width","100%");b=a.svg.append("linearGradient").attr("id","lgrad").attr("x1","11%").attr("y1","0%").attr("x2","89%").attr("y2","100%");b.append("stop").attr("offset","0%").attr("style","stop-color:rgb(153,218,255);stop-opacity:1");b.append("stop").attr("offset","100%").attr("style","stop-color:rgb(0,128,128);stop-opacity:1");
b=a.svg.append("linearGradient").attr("id","selectGrad").attr("x1","11%").attr("y1","0%").attr("x2","89%").attr("y2","100%");b.append("stop").attr("offset","0%").attr("style","stop-color:rgb(0,163,55);stop-opacity:1");b.append("stop").attr("offset","100%").attr("style","stop-color:rgb(0,75,20);stop-opacity:1")}else a.svg=d3.select("#multiWGCNASVG");a.modKey=function(a){return a.MOD_NAME};a.calcX=function(b,c){return c%a.modPerLine*a.calcWidth};a.calcY=function(b,c){return Math.floor(c/a.modPerLine)*
a.maxWidth};a.sizing=function(b){b=a.size(b.TCList.length);b>a.maxWidth/2&&(b=a.maxWidth/2);return b};a.zoomIn=function(){a.maxWidth<$(window).width()/2&&(a.maxWidth*=2,a.draw())};a.zoomOut=function(){var b=a.maxWidth;a.maxWidth=Math.floor(a.maxWidth/2);25>a.maxWidth?a.maxWidth=b:a.draw()};a.sortTCLenDesc=function(a,b){return b.TCList.length<a.TCList.length?-1:b.TCList.length>a.TCList.length?1:0};a.sortGeneInstanceDesc=function(b,c){return c.GeneCount<b.GeneCount?-1:c.GeneCount>b.GeneCount?1:a.sortTCLenDesc(b,
c)};a.draw=function(){};a.redraw=function(){};return a};that.multiWGCNAImageGeneView=function(){var a=that.multiWGCNAImage();a.createToolTip=function(a){var b="Module Name: "+a.MOD_NAME+"<BR>Transcript Size: "+a.TCList.length;"undefined"!==typeof a.GroupedGenes?(b=b+"<BR>Genes in Region/List: "+a.GroupedGenes,b=b+"<BR>Transcripts in Region/List: "+a.GeneCount):b=b+"<BR>Selected Gene Transcripts in Module: "+a.GeneCount;return b};a.draw=function(){"undefined"!==typeof that.selectedModule&&(a.maxWidth=
85);a.svg.selectAll(".modules").remove();a.svg.selectAll(".moduleGradient").remove();a.totalWidth=$(window).width();a.modPerLine=Math.floor($(window).width()/a.maxWidth);a.calcWidth=Math.floor($(window).width()/a.modPerLine);var b=[];a.dataList={};for(var c=0,e=0;e<that.moduleList.length;e++)if(console.log("multiModView:"+that.moduleList.length+":"+that.moduleList[e]),("grey"!==that.moduleList[e]||"grey"===that.moduleList[e]&&0===that.skipGrey)&&"undefined"!==typeof that.modules[that.moduleList[e]]){b[c]=
that.modules[that.moduleList[e]];var f=b[c].ModRGB.split(","),h="";for(h=0;h<f.length;h++)f[h]-=70,0>f[h]&&(f[h]=0);h=f[0]+","+f[1]+","+f[2];f=a.svg.append("linearGradient").attr("id","grad"+b[c].MOD_NAME).attr("class","moduleGradient").attr("x1","11%").attr("y1","0%").attr("x2","89%").attr("y2","100%");f.append("stop").attr("offset","0%").attr("style","stop-color:rgb("+b[c].ModRGB+");stop-opacity:1");f.append("stop").attr("offset","100%").attr("style","stop-color:rgb("+h+");stop-opacity:1");c++}c=
a.maxWidth+15;b.length>a.modPerLine&&(e=Math.floor(b.length/a.modPerLine),0<b.length%a.modPerLine&&e++,c*=e);a.svg.attr("height",c+"px");b.sort(a.sortGeneInstanceDesc);a.data=b;for(e=0;e<b.length;e++)a.dataList[b[e].MOD_NAME]=e;c=d3.min(b,function(a){return a.TCList.length});e=d3.max(b,function(a){return a.TCList.length});a.size=d3.scaleLinear().domain([c,e]).range([10,a.maxWidth/2]);b=a.svg.selectAll(".modules").data(b,a.modKey).attr("transform",function(b,c){return"translate("+a.calcX(b,c)+","+
a.calcY(b,c)+")"});console.log("before append to image");b.enter().append("g").attr("class","modules").attr("id",function(a){return a.MOD_NAME}).attr("transform",function(b,c){return"translate("+a.calcX(b,c)+","+a.calcY(b,c)+")"}).merge(b).each(function(b){var c=d3.select("g#"+b.MOD_NAME);c.append("circle").attr("cx",a.calcWidth/2).attr("cy",a.maxWidth/2).attr("r",a.sizing).attr("stroke",function(a){var b="#FFFFFF";"undefined"!==typeof that.selectedModule&&that.selectedModule===a&&(b="#000000");return b}).attr("stroke-width",
function(a){var b="1px";"undefined"!==typeof that.selectedModule&&that.selectedModule===a&&(b="3px");return b}).attr("fill",function(a){var b="url(#grad"+a.MOD_NAME+")";"undefined"!==typeof that.selectedModule&&that.selectedModule===a&&(b="url(#selectGrad)");return b}).on("click",function(){a.maxWidth=85;that.selectedModule=b;a.draw();d3.select("#multiWGCNAScroll").style("max-height","110px");a.svg.select("g#"+b.MOD_NAME).style("background","#CECECE");a.svg.select("g#"+b.MOD_NAME).select("circle").attr("fill",
"url(#selectGrad)").attr("stroke","#000000").attr("stroke-width","3px");a.svg.select("g#"+b.MOD_NAME).select("text.txtLbl").style("font-weight","bold");that.selectedGene={};for(var c=0;c<b.TCList.length;c++)"undefined"===typeof that.selectedGene[b.TCList[c].Gene.ID]&&(that.selectedGene[b.TCList[c].Gene.ID]={},that.selectedGene[b.TCList[c].Gene.ID].geneSymbol=b.TCList[c].Gene.geneSymbol,that.selectedGene[b.TCList[c].Gene.ID].tcList=[]),that.selectedGene[b.TCList[c].Gene.ID].tcList.push(b.TCList[c]);
setTimeout(that.createSingleWGCNAImage,20)}).on("mouseover",function(){$("#wgcnaMouseHelp").html("<B>Click</B> to select this module and see additional details.");d3.select(this).style("fill","url(#selectGrad)");tt.transition().duration(200).style("opacity",1);tt.html(a.createToolTip(b)).style("left",function(){return that.positionTTLeft(d3.event.pageX)}).style("top",function(){return that.positionTTTop(d3.event.pageY)})}).on("mouseout",function(){var a=d3.select(this).data()[0];"undefined"!==typeof that.selectedModule&&
a.MOD_NAME===that.selectedModule.MOD_NAME||d3.select(this).style("fill","url(#grad"+a.MOD_NAME+")");tt.transition().duration(500).style("opacity",0)});"undefined"!==typeof that.selectedModule&&that.selectedModule===b?c.append("text").attr("class","txtLbl").attr("y",a.maxWidth+12).attr("x",function(b){return a.calcWidth/2-b.MOD_NAME.length/2*6.5}).style("font-weight","bold").text(replaceUnderscore(b.MOD_NAME)):6.5*b.MOD_NAME.length<a.calcWidth&&c.append("text").attr("class","txtLbl").attr("y",a.maxWidth+
12).attr("x",function(b){return a.calcWidth/2-b.MOD_NAME.length/2*6.5}).text(replaceUnderscore(b.MOD_NAME));c.append("text").attr("y",a.maxWidth/2+5).attr("x",a.calcWidth/2-3).attr("pointer-events","none").attr("fill",function(a){var b="#000000";a=a.ModRGB.split(",");360>a[0]+a[1]+a[2]&&(b="#FFFFFF");return b}).text(function(a){var b=a.GeneCount;"undefined"!==typeof a.GroupedGenes&&(b=a.GroupedGenes);return b})});console.log("end of draw")};a.backModule=function(){if(that.selectedModule){var b=a.dataList[that.selectedModule.MOD_NAME];
if(0<b){b=a.data[b-1];var c=a.svg.select("g#"+that.selectedModule.MOD_NAME);c.select("circle").attr("fill","url(#grad"+that.selectedModule.MOD_NAME+")").attr("stroke","#FFFFFF").attr("stroke-width","1px");c.select("text.txtLbl").style("font-weight","normal");that.selectedModule=b;b=a.svg.select("g#"+b.MOD_NAME);b.select("circle").attr("fill","url(#selectGrad)").attr("stroke","#000000").attr("stroke-width","3px");b.select("text.txtLbl").style("font-weight","bold");setTimeout(that.createSingleWGCNAImage,
20)}}};a.forwardModule=function(){if(that.selectedModule){var b=a.dataList[that.selectedModule.MOD_NAME];if(b<a.data.length-1){b=a.data[b+1];var c=a.svg.select("g#"+that.selectedModule.MOD_NAME);c.select("circle").attr("fill","url(#grad"+that.selectedModule.MOD_NAME+")").attr("stroke","#FFFFFF").attr("stroke-width","1px");c.select("text.txtLbl").style("font-weight","normal");that.selectedModule=b;b=a.svg.select("g#"+b.MOD_NAME);b.select("circle").attr("fill","url(#selectGrad)").attr("stroke","#000000").attr("stroke-width",
"3px");b.select("text.txtLbl").style("font-weight","bold");setTimeout(that.createSingleWGCNAImage,20)}}};return a};that.createSingleWGCNAImage=function(){d3.select("#viewGene").remove();d3.select("#circos").remove();$("#wgcnaresetImage").show();$("#wgcnazoomInImage").show();$("#wgcnazoomOutImage").show();"gene"===that.viewType?($("#linkSliderMin").slider("value",.01),$("span#minLabel").html(.01),$("#linkSliderMax").slider("value",1),$("span#maxLabel").html(1),$("#hideLinkExceptNode").prop("checked",
!1),setTimeout(function(){that.singleImage=that.singleWGCNAImageGeneView(that.modules);that.singleImage.draw();that.img=that.singleImage;that.singleWGCNATableGeneView();setTimeout(function(){that.singleImage.panZoom=svgPanZoom("#singleWGCNASVG",{panEnabled:!0,controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,minZoom:.1,maxZoom:4,fit:!0,center:!0})},500)},50)):"go"===that.viewType?($("#wgcnaresetImage").hide(),$("#wgcnazoomInImage").hide(),$("#wgcnazoomOutImage").hide(),
$("#goDomainSelect").val("mf"),$("#goSpinner").val(3),setTimeout(function(){that.singleImage=that.singleWGCNAImageGoView(that.modules);that.singleImage.goDomain=$("#goDomainSelect").val();that.singleImage.draw();that.img=that.singleImage},50)):"eqtl"===that.viewType?($("#eqtlChr option").prop("selected",!0),$("#eqtlPval").val(2),setTimeout(function(){that.singleImage=that.singleWGCNAImageEQTLView(that.modules);that.singleImage.displayDefault();that.img=that.singleImage},50)):"mir"===that.viewType?
($("#mirValidatedCBX").prop("checked",!1),$("#minMirGeneSlider").slider("value",1),$("span#minMirGeneLabel").html(1),$("#mirGeneFilterTxt").val(""),$("#mirIDFilterTxt").val(""),$("#mirSortCBX").prop("checked",!1),$("#mirLinkCBX").prop("checked",!0),$("#mirgeneLinkCBX").prop("checked",!1),$("div#mirLinkColorSelect").hide(),setTimeout(function(){that.singleImage=that.singleWGCNAImageGeneView(that.modules);that.singleImage.draw();that.img=that.singleImage;setTimeout(function(){that.singleImage.panZoom=
svgPanZoom("#singleWGCNASVG",{panEnabled:!0,controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,minZoom:.1,maxZoom:4,fit:!0,center:!0})},500)},50)):"meta"===that.viewType&&setTimeout(function(){that.singleImage=that.singleWGCNAImageMetaView(that.modules);that.singleImage.requestMeta();that.img=that.singleImage;setTimeout(function(){that.singleImage.panZoom=svgPanZoom("#singleWGCNASVG",{panEnabled:!0,controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,
minZoom:.1,maxZoom:4,fit:!0,center:!0})},500)},50);gtag("event",that.viewType,{event_category:"changedWGCNAView"})};that.singleWGCNAImage=function(){var a={};a.width=$(window).width();a.height=Math.floor(.9*window.innerHeight);a.r=a.height>a.width?Math.floor(a.width/2*.95):Math.floor(a.height/2*.95);a.maxPerLevel=150;a.CorCutoff_min=.75;a.CorCutoff_max=1;a.showLinks=!0;a.data=[];a.maxNodeR=28;a.negColor="#DD0000";a.negDarkColor="#770000";a.posColor="#00DD00";a.posDarkColor="#007700";a.mirVal="#FFFF00";
a.mirPred="#0000CC";0===d3.select("#singleWGCNASVG").size()?a.svg=d3.select("#wgcnaGeneImage").append("div").attr("id","viewGene").append("svg").attr("id","singleWGCNASVG").attr("height",a.height+"px").attr("width","100%"):a.svg=d3.select("#singleWGCNASVG");a.geneKey=function(a){return a.ID};a.linkKey=function(a){return that.removeInvalidIDChars(a.TC1)+that.removeInvalidIDChars(a.TC2)};a.calcX=function(b,c){return c%a.modPerLine*a.calcWidth};a.calcY=function(b,c){return Math.floor(c/a.modPerLine)*
a.maxWidth};a.sizing=function(b){b=a.size(b.TCList.length);b>a.maxWidth/2&&(b=a.maxWidth/2);return b};a.colorCircle=function(a){a=a.Gene.ID;var b="#DFC184";if(0==a.indexOf("PRN")||0==a.indexOf("PMM")||0==a.indexOf("Unannotated"))"Whole Brain"===that.tissue?b="#7EB5D6":"Heart"===that.tissue?b="#DC7252":"Liver"===that.tissue&&(b="#bbbedd");return b};a.colorLink=function(b){var c=a.negColor;0<b.Cor&&(c=a.posColor);return c};a.colorLinkHighlight=function(b){var c=a.negDarkColor;0<b.Cor&&(c=a.posDarkColor);
return c};a.updateColor=function(b){b=$("#"+b).val();"Red_Green"===b?(a.posColor="#DD0000",a.posDarkColor="#770000",a.negColor="#00DD00",a.negDarkColor="#007700",a.mirVal="#FFFF00",a.mirPred="#0000CC"):"Blue_Yellow"===b?(a.posColor="#0000DD",a.posDarkColor="#000077",a.negColor="#ffef16",a.negDarkColor="#b7ae2a",a.mirVal="#FF9C2A",a.mirPred="#BA49FF"):(a.negColor="#DD0000",a.negDarkColor="#770000",a.posColor="#00DD00",a.posDarkColor="#007700",a.mirVal="#FFFF00",a.mirPred="#0000CC")};a.nodeRadiusSum=
function(b,c){var d=3+20*b.LinkSum;d>a.maxNodeR&&(d=a.maxNodeR);return d};a.zoomIn=function(){a.maxWidth<$(window).width()/2&&(a.maxWidth*=2,a.draw())};a.zoomOut=function(){var b=a.maxWidth;a.maxWidth=Math.floor(a.maxWidth/2);25>a.maxWidth?a.maxWidth=b:a.draw()};a.draw=function(){};a.redraw=function(){};a.setupLegend=function(){a.svg.select("#legend").remove();a.svg.select("#legendDisp").remove();a.legendG=a.svg.append("g").attr("id","legend").attr("transform","translate(10,60)").style("cursor","pointer").on("mouseover",
function(){a.legendDispG.transition().duration(300).style("opacity",1);gtag("event","viewLegend",{event_category:"wgcna"})}).on("mouseleave",function(){a.legendDispG.transition().duration(300).style("opacity",0)});a.legendG.append("image").attr("x",0).attr("y",-20).attr("width","24px").attr("height","24px").attr("xlink:href","data:image/png;base64,");
...
d3.select(this).append("td").html(a.Gene.ID);d3.select(this).append("td").html(a.ID);a.PSList?d3.select(this).append("td").html(a.PSList.length):d3.select(this).append("td").html("0");d3.select(this).append("td").html(a.LinkSum.toFixed(2))}),$("table#moduleTable").DataTable({bPaginate:!1,aaSorting:[[4,"desc"]],sDom:"fi<t>"})):($("div#waitModuleTable").hide(),$("div#wgcnaEqtlTable").hide(),$("div#wgcnaModuleTable").hide(),$("div#wgcnaGoTable").hide())};that.singleWGCNATableMirView=function(){var a=
{orgPref:"mmu-",runMirFilter:function(a){for(var b=[],c=0,d=0;d<a.length;d++)if(0<a[d].filterGLSize||0<a[d].vC)b[c]=a[d],c++;return b},createMirTable:function(){$("div#waitMirTable").hide();$("div#wgcnaMirGeneTable").hide();$("div#wgcnaMirTable").show();$("span#mirTableName").html(that.selectedModule.MOD_NAME);$.fn.DataTable.isDataTable("table#mirTable")&&$("table#mirTable").DataTable().destroy();d3.select("table#mirTable").select("tbody").selectAll("tr").remove();var b=a.runMirFilter(that.singleImage.mirData.MIRList);
d3.select("table#mirTable").select("tbody").selectAll("tr").data(b,that.mirKey).enter().append("tr").attr("id",function(a){return"mirTR"+that.removeInvalidIDChars(a.ID)}).attr("class",function(a,b){var c="odd";0===b%2&&(c="even");return c}).each(function(a,b){for(var c=0,d=[],e=0,f=[],g=a.GeneList,h=0;h<g.length;h++)0<g[h].V?(c++,d.push(g[h])):1<g[h].P&&(e++,f.push(g[h]));d3.select(this).append("td").html(a.ID);d3.select(this).append("td").html(a.ACC);d3.select(this).append("td").html(function(){if(0<
e){var a='<span class="triggerpL" name="pl'+b+'">'+e+"</span><BR>";a=a+'<span id="pl'+b+'" style="display:none;" >';for(var c=0;c<f.length;c++){0<c&&(a+=", ");var d=f[c].ID;"undefined"!==typeof that.singleImage.geneList[d]&&(d=that.singleImage.geneList[d]);a+=d}a+="</span>"}else a=e;return a});d3.select(this).append("td").html(function(){if(0<c){var a='<span class="triggervL" name="vl'+b+'">'+c+"</span><BR>";a=a+'<span id="vl'+b+'" style="display:none;" >';for(var e=0;e<d.length;e++){0<e&&(a+=", ");
var f=d[e].ID;"undefined"!==typeof that.singleImage.geneList[f]&&(f=that.singleImage.geneList[f]);a+=f}a+="</span>"}else a=c;return a});d3.select(this).append("td").html(function(){var a=e+c;if(0<a){a='<span class="triggerMiL" name="mil'+b+'">'+a+"</span><BR>";a=a+'<span id="mil'+b+'" style="display:none;" >';if(0<d.length){a+="Validated: ";for(var g=0;g<d.length;g++){0<g&&(a+=", ");var h=d[g].ID;"undefined"!==typeof that.singleImage.geneList[h]&&(h=that.singleImage.geneList[h]);a+=h}}if(0<f.length)for(0<
d.length&&(a+="<BR><BR>"),a+="Predicted: ",g=0;g<f.length;g++)0<g&&(a+=", "),h=f[g].ID,"undefined"!==typeof that.singleImage.geneList[h]&&(h=that.singleImage.geneList[h]),a+=h;a+="</span>"}return a})});$("#mirTable .triggerMiL").on("click",function(a){var b=this.getAttribute("name");$("#"+b).is(":hidden")?($(this).addClass("less"),$("span#"+b).show()):($(this).removeClass("less"),$("span#"+b).hide());a.stopPropagation()});$("#mirTable .triggerpL").on("click",function(a){var b=this.getAttribute("name");
$("#"+b).is(":hidden")?($(this).addClass("less"),$("span#"+b).show()):($(this).removeClass("less"),$("span#"+b).hide());a.stopPropagation()});$("#mirTable .triggervL").on("click",function(a){var b=this.getAttribute("name");$("#"+b).is(":hidden")?($(this).addClass("less"),$("span#"+b).show()):($(this).removeClass("less"),$("span#"+b).hide());a.stopPropagation()});$("table#mirTable").DataTable({bPaginate:!1,aaSorting:[[4,"desc"]],sDom:'f<"rightTable"i><t>'})},createMirGeneTable:function(){$("div#waitMirGeneTable").hide();
...
e=0,f=[],g=0,m=[];if("undefined"!==typeof d)for(var l=0;l<d.length;l++)for(var p=d[l].GeneList,n=0,y=0;y<p.length&&0===n;y++)p[y].ID===b.Gene.ID&&(0<p[y].V?(e++,f.push(d[l])):1<p[y].P&&(g++,m.push(d[l])),n=1);d3.select(this).append("td").html(b.Gene.geneSymbol);d3.select(this).append("td").html(b.Gene.ID);d3.select(this).append("td").html(function(b,d){if(0<g){var e='<span class="triggerpL" name="pl'+c+'">'+g+"</span><BR>";e=e+'<span id="pl'+c+'" style="display:none;" >';for(var f=0;f<m.length;f++){0<
f&&(e+=", ");var h=m[f].ID.replace(a.orgPref,"");e+=h}e+="</span>"}else e=g;return e});d3.select(this).append("td").html(function(b,d){if(0<e){var g='<span class="triggervL" name="vl'+c+'">'+e+"</span><BR>";g=g+'<span id="vl'+c+'" style="display:none;" >';for(var h=0;h<f.length;h++){0<h&&(g+=", ");var m=f[h].ID.replace(a.orgPref,"");g+=m}g+="</span>"}else g=e;return g});d3.select(this).append("td").html(function(b,d){var h=e+g;if(0<h){h='<span class="triggerMiL" name="mil'+c+'">'+h+"</span><BR>";
h=h+'<span id="mil'+c+'" style="display:none;" >';if(0<f.length){h+="Validated: ";for(var l=0;l<f.length;l++){0<l&&(h+=", ");var p=f[l].ID.replace(a.orgPref,"");h+=p}}if(0<m.length)for(0<f.length&&(h+="<BR><BR>"),h+="Predicted: ",l=0;l<m.length;l++)0<l&&(h+=", "),p=m[l].ID.replace(a.orgPref,""),h+=p;h+="</span>"}return h})});$("#mirGeneTable .triggerMiL").on("click",function(a){var b=this.getAttribute("name");$("#"+b).is(":hidden")?($(this).addClass("less"),$("span#"+b).show()):($(this).removeClass("less"),
...
"undefined"!==typeof a.geneList[p]&&(p=a.geneList[p],25<f-d&&(p=p+" ("+b.data.name+")"));var n=p.split(" ")[0].split("-")[0];if(n.length<=l||-1===b.data.name.indexOf("ENS")&&n.length/2<=l&&10<f-d){m=[];n=0;if(p.length>l)for(;0<p.length;){var q=p.indexOf(" ");if(-1===q)p.length>l?(m[n]=p.substr(0,l),n++,p=p.substr(l)):(m[n]=p,n++,p="");else{for(var r=0;q<l&&1>r;){var t=p.indexOf(" ",q+1);-1!==t&&t<=l?q=t:r=1}q>l?-1<p.indexOf("-")&&p.indexOf("-")<l?(q=p.indexOf("-"),m[n]=p.substr(0,q+1),n++,p=p.substr(q+
1)):(m[n]=p.substr(0,l),n++,p=p.substr(l)):(m[n]=p.substr(0,q),n++,p=p.substr(q+1))}}else m[n]=p;n=a.topG.append("text").style("fill-opacity",1).style("fill","#000").attr("pointer-events","none").attr("class","txt"+removeColon(b.data.id)).attr("text-anchor",function(a){return"start"}).attr("dy","1em").style("opacity",function(a){return 1}).append("textPath").attr("xlink:href",function(a){return"#ap"+c}).attr("startOffset",function(a){return 0===d&&360===f?90:5});for(b=0;b<m.length&&b<g;b++)b<g&&(b===
...
function(a){return a.id}).attr("class",function(a){return" d"+a.depth}).on("mouseover",function(a){d3.select(this).style("background-color","#FFFFFF");gtag("event","mouseOverGOTbl",{event_category:"wgcna"})}).on("mouseleave",function(a){d3.select(this).style("background-color",that.singleImage.pathColor(a))}).on("click",function(a){that.singleImage.selectedNode=a.data;that.singleImage.rootN=a;$("span[name=d"+a.depth+"]").addClass("less");$("tr.d"+a.depth).show();that.singleImage.redraw();gtag("event",
"wgcna",{event_category:"mouseClickGOTbl"})}).each(function(a,b){var c=d3.select(this);c.append("td").style("padding-left",function(){return 20*a.depth+"px"}).html(function(){var b=a.data.name+" ("+a.data.id+")";-1<a.data.name.indexOf("ENS")&&(b=that.singleImage.geneList[a.data.name]+" ("+a.data.id+")");var c=b;if("undefined"!==typeof a.children&&0<a.children.length){c="";a.depth<=that.singleImage.rootN.depth&&(c=" less");for(var d=0,e=0;e<a.children.length&&0===d;e++)-1===a.children[e].data.name.indexOf("ENS")&&
...
d3.select(this).append("td").html(a.Snp);d3.select(this).append("td").html(a.Pval.toFixed(2))});$("#eqtlTable").DataTable({bPaginate:!1,aaSorting:[[3,"desc"]],sDom:"fi<t>"});$("div#waitEqtlTable").hide();d3.select("#circosModule").select("g#Link_"+e).select("path").style("stroke","#FFF450")},20)},error:function(a,b,c){}})};that.singleWGCNAImageMetaView=function(){var a=that.singleWGCNAImage();$(".goCTL").hide();$(".eqtlCtls").hide();$(".linkCTL").show();$(".mirCTL").hide();a.CorCutoff_min=.75;a.CorCutoff_max=
1;a.type=that.viewType;a.truncated=0;a.filterOneDB=1;a.mirGeneCutoff=1;a.mirValid=0;a.mirTargetID="";a.mirID="";a.miRcolor=d3.scaleOrdinal(d3.schemeCategory20b);a.sortMir=0;a.maxCorWidth=5;a.mlinkKey=function(a){return that.removeInvalidIDChars(a.MOD1)+"_"+that.removeInvalidIDChars(a.MOD2)};a.colorCircle=function(a){return a.COLOR};a.colorLink=function(b){var c=a.negColor;0<b.COR&&(c=a.posColor);return c};a.colorLinkHighlight=function(b){var c=a.negDarkColor;0<b.COR&&(c=a.posDarkColor);return c};
a.drawLinks=function(b){a.updateColor("linkColor");a.svg.selectAll(".link").remove();for(var c=a.metaData.LINKS,e=[],f=0,g=0;g<c.length;g++)(c[g].MOD1===b.MODULE||c[g].MOD2===b.MODULE)&&a.CorCutoff_min<=Math.abs(c[g].COR)&&Math.abs(c[g].COR)<=a.CorCutoff_max&&(e[f]=c[g],f++);c=e;1E4>c.length&&(b=a.topG.selectAll(".link").data(c,a.mlinkKey),b.enter().append("line").attr("class","link").attr("pointer-events","none").attr("id",function(a){return"ln_"+that.removeInvalidIDChars(a.MOD1)+that.removeInvalidIDChars(a.MOD2)}).attr("x1",
