...
e=[];if("undefined"!==typeof h&&0<h.length){for(var q=0,f=0;f<h.length;f++)if(m<=parseInt(h[f].getAttribute("start"),10)&&parseInt(h[f].getAttribute("start"),10)<=p||parseInt(h[f].getAttribute("start"),10)<=m&&parseInt(h[f].getAttribute("stop"),10)>=m)e[q]=h[f],q++;0<e.length&&g.addTrack(c.ttTrackList[d],3,"DrawTrx",e)}}};c.generateSettingsDiv=function(b){c.savePrevious();var a=trackInfo[c.trackClass];d3.select(b).select("table").select("tbody").html("");if("undefined"!==typeof a&&0<a.Controls.length&&
...
1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1));else if(1==c||"Trx"==b.drawAs)b.svg.selectAll("g.trx"+b.gsvg.levelNumber).attr("transform",function(a,c){return"translate(0,"+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"}).each(function(a,c){var d=parseInt(a.getAttribute("cdsStart"),10),g=parseInt(a.getAttribute("cdsStop"),10);exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");var e="";1==b.gsvg.levelNumber&&(e="tx");for(var p=
0;p<exList.length;p++){var m=parseInt(exList[p].getAttribute("start"),10),f=parseInt(exList[p].getAttribute("stop"),10);if(m<d&&d<f||f>g&&g>m){var l=m,n=d;f>g?(l=g,n=f,f=g):m=d;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+e+b.cleanID(a.getAttribute("ID"))+" rect#ExNC"+exList[p].getAttribute("ID")).attr("x",function(a){return b.xScale(l)}).attr("width",function(a){return b.xScale(n)-b.xScale(l)})}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+e+b.cleanID(a.getAttribute("ID"))+
" rect#Ex"+exList[p].getAttribute("ID")).attr("x",function(a){return b.xScale(m)}).attr("width",function(a){return b.xScale(f)-b.xScale(m)});if(0<p){var z=b.xScale(exList[p-1].getAttribute("stop")),E=b.xScale(exList[p].getAttribute("start"));if("1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand")){var F=">";"-1"==a.getAttribute("strand")&&(F="<");var B=F,D=E-z,C=0;if(7>D)B="";else for(D-=7;8<D;)0==C?(B+=" ",C=1):(B+=F,C=0),D-=
7;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+e+b.cleanID(a.getAttribute("ID"))+" #IntTxt"+exList[p-1].getAttribute("ID")+"_"+exList[p].getAttribute("ID")).attr("dx",z+1).text(B)}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+e+b.cleanID(a.getAttribute("ID"))+" line#Int"+exList[p-1].getAttribute("ID")+"_"+exList[p].getAttribute("ID")).attr("x1",z).attr("x2",E)}}}),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+
...
b.trackClass))}else if(e){h=e.documentElement.getElementsByTagName("Gene");for(var p=[],m=[],f=0,l=0;l<h.length;l++)h[l]&&(p[f]=h[l],p[f].setAttribute("ID",f),m[h[l].getAttribute("geneSymbol")]=1,f++);for(l=0;l<b.data.length;l++)b.data[l]&&!m[b.data[l].getAttribute("geneSymbol")]&&(p[f]=b.data[l],p[f].setAttribute("ID",f),f++);b.draw(p);b.hideLoading();b.getDisplayedData();setTimeout(function(){DisplayRegionReport()},300)}else b.updateData(a+1),b.hideLoading()})};b.drawTrx=function(a,c){var d=parseInt(a.getAttribute("cdsStart"),
10),g=parseInt(a.getAttribute("cdsStop"),10),e="";1==b.gsvg.levelNumber&&(e="tx");e=b.svg.select("#"+e+b.cleanID(a.getAttribute("ID")));exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var h=0;h<exList.length;h++){var f=parseInt(exList[h].getAttribute("start"),10),l=parseInt(exList[h].getAttribute("stop"),10);if(f<d&&d<l||l>g&&g>f){var n=0,t=0,y=0,v=0;f<d?(n=b.xScale(f),t=b.xScale(d)-b.xScale(f),y=b.xScale(d),v=b.xScale(l)-b.xScale(d)):l>g&&(y=b.xScale(f),v=b.xScale(g)-b.xScale(f),
n=b.xScale(g),t=b.xScale(l)-b.xScale(g));e.append("rect").attr("x",n).attr("y",2.5).attr("height",5).attr("width",t).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"ExNC"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer");e.append("rect").attr("x",y).attr("height",10).attr("width",v).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor",
"pointer")}else{n=10;t=0;if(f<d&&l<d||l>g&&f>g)n=5,t=2.5;e.append("rect").attr("x",function(a){return b.xScale(exList[h].getAttribute("start"))}).attr("y",t).attr("height",n).attr("width",function(a){return b.xScale(exList[h].getAttribute("stop"))-b.xScale(exList[h].getAttribute("start"))}).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}if(0<h&&(e.append("line").attr("x1",
function(a){return b.xScale(exList[h-1].getAttribute("stop"))}).attr("x2",function(a){return b.xScale(exList[h].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(a){return"Int"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("stroke",b.color).attr("stroke-width","2"),"1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand"))){f=">";if("-"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))f=
"<";l=f;n=b.xScale(exList[h-1].getAttribute("stop"));t=b.xScale(exList[h].getAttribute("start"))-n;y=0;if(7>t)l="";else for(t-=7;8<t;)0==y?(l+=" ",y=1):(l+=f,y=0),t-=7;e.append("svg:text").attr("id",function(a){return"IntTxt"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("dx",n+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(l)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yArr=[];b.yArr[0]=[];
...
c.trackClass),d3.select("#Level"+c.levelNumber+c.trackClass).attr("height",15),c.gsvg.addTrackErrorRemove(c.svg,"#Level"+c.gsvg.levelNumber+c.trackClass);else{if(g){d=g.documentElement.getElementsByTagName("QTL");for(var e=[],f=[],h=0,l=0;l<d.length;l++)"undefined"!==typeof d[l]&&(e[h]=d[l],f[d[l].getAttribute("ID")]=1,h++);for(l=0;l<c.data.length;l++)"undefined"!==typeof c.data[l]&&"undefined"===typeof f[c.data[l].getAttribute("ID")]&&(e[h]=c.data[l],h++);c.draw(e)}c.hideLoading()}})};c.calcY=function(b,
...
(l=$("#usrtrkColorSelect"+e.level).val());if("Score"===l){var q="000000",u="FFFFFF",t=1,y=1E3;testIE||testSafari?(q=$("#usrtrkColorMin"+e.level).spectrum("get").toHexString(),u=$("#usrtrkColorMax"+e.level).spectrum("get").toHexString()):(q=$("#usrtrkColorMin"+e.level).val(),u=$("#usrtrkColorMax"+e.level).val(),console.log(q),console.log(u));q=q.substr(1);u=u.substr(1);t=$("#usrtrkScoreMinTxt"+e.level).val();y=$("#usrtrkScoreMaxTxt"+e.level).val()}"bed"===m?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Pack:3}{Full:2};Default=3,Name=Color by;Select:colorSelect::colorSelect;{Feature Defined:Color}{Score based scale:Score};Default="+
l+",Name=Scale Range;Txt:tissuecbx;{min:1}{max:1000};Default="+t+":"+y+",Name=Color Range;Txt:color;{min:FFFFFF}{max:000000};Default="+q+":"+u:"bg"===m?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Full:2};Default=1,Name=Scale Range;Slider:rangeslider;{min:1-1000}{max:1000-20000};Default=1:5000":"bw"===m?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Full:2};Default=1,Name=Scale Range;Slider:rangeslider;{min:1-1000}{max:1000-20000};Default=1:5000":"bb"===m&&(p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Pack:3}{Full:2};Default=3,Name=Color by;Select:colorSelect::colorSelect;{Feature Defined:Color}{Score based scale:Score};Default="+
l+",Name=Scale Range;Txt:tissuecbx;{min:1}{max:1000};Default="+t+":"+y+",Name=Color Range;Txt:color;{min:FFFFFF}{max:000000};Default="+q+":"+u);f="custom"+f;0<uid?(l="/"+pathPrefix,pathPrefix||(l=""),$.ajax({url:l+"addTrack.jsp",type:"GET",data:{trackClass:f,trackName:c,trackDesc:b,trackOrg:a,genericCategory:g,category:d,controls:p,location:n,type:m,file:h},dataType:"json",success:function(a){"true"===a.success?$(".uploadStatus").html("<B>Track Setup Successfully</B>"):$(".uploadStatus").html("<B>Track Setup Failed</B>")},
...
that.selSource?"2"===that.version?"Whole Brain"===b?that.wDSID=8:"Liver"===b&&(that.wDSID=9):"Whole Brain"===b?that.wDSID=6:"Liver"===b&&(that.wDSID=7):"Whole Brain"===b?that.wDSID=3:"Heart"===b?that.wDSID=4:"Liver"===b&&(that.wDSID=5)));that.eQTLKey=function(a){return"Link_"+a.Snp};that.mirKey=function(a){return a.ID};that.modKey=function(a){return a.MODULE};that.removeInvalidIDChars=function(a){"undefined"!==typeof a&&(-1<a.indexOf("*")&&(a=a.replace(/\*/g,"star")),-1<a.indexOf(" ")&&(a=a.replace(/\s/g,
"_")),-1<a.indexOf(".")&&(a=a.replace(/\./g,"_")));return a};that.setup=function(){that.requestModuleList();that.createImageControls();that.createViewControls();that.createDataControls();$(".wgcnaControltooltip").tooltipster({position:"top-right",maxWidth:250,offsetX:5,offsetY:5,contentAsHTML:!0,interactive:!0,interactiveTolerance:350});ga&&ga("send","event","setupWGCNA","setupWGCNA")};that.requestModuleList=function(){that.moduleGenes={};that.moduleList=[];that.modules={};$.ajax({url:pathPrefix+
"getWGCNAModules.jsp",type:"GET",async:!0,data:{source:that.selSource,modFileType:that.viewtype,id:that.singleID,organism:organism,panel:that.panel,tissue:that.tissue,region:that.region,geneList:that.geneList,genomeVer:genomeVer,version:that.version},dataType:"json",beforeSend:function(){$("#waitCircos").show();for(var a=0;a<that.moduleRequestList.length;a++)that.moduleRequestList[a].abort();that.moduleRequestList=[];console.log("request mod list:"+that.wDSID+":"+that.selSource)},success:function(a){$("#wgcnaGeneImage #message").hide();
that.moduleList=[];0<a.length&&(1!==a.length||"grey"!==a[0].ModuleID)?setTimeout(function(){for(var b=0;b<a.length;b++)"grey"!==a[b].ModuleID&&(that.moduleList.push(a[b].ModuleID),that.requests++,that.requestModule(a[b].ModuleID),"undefined"!==typeof a[b].GeneList&&(that.moduleGenes[a[b].ModuleID]=a[b].GeneList));that.refreshRegion(1E3);setTimeout(function(){$("html, body").animate({scrollTop:$("#multiWGCNAScroll").offset().top},100)},1200)},1):0===a.length?that.displayMessage("There are no transcripts with Affymetrix Probesets that represent this gene."):
1===a.length&&"grey"===a[0].ModuleID&&that.displayMessage("Only the Grey module contains transcripts for this gene.  The Grey module contains all transcripts that did not have a high enough correlation with any other transcripts to be included in another module.");ga&&ga("send","event","requestModuleList","requestModuleList")},error:function(a,b,c){}})};that.displayMessage=function(a){$("#waitCircos").hide();$("#wgcnaGeneImage #message").html("<BR><BR><h2>"+a+"</h2><BR><BR>").show()};that.refreshRegion=
function(a){setTimeout(function(){that.createMultiWGCNAImage();0<that.requests?that.refreshRegion(2E3):1!==a&&(that.refreshRegion(1),$("#waitCircos").hide())},a)};that.requestModule=function(a,b){xhr=$.ajax({url:contextRoot+"tmpData/browserCache/"+genomeVer+"/modules/ds"+that.wDSID+"/"+a+".json",type:"GET",async:!0,data:{},dataType:"json",success:function(c){console.log("success:"+contextRoot+"tmpData/browserCache/"+genomeVer+"/modules/ds"+that.wDSID+"/"+a+".json");if(0<that.singleID.length)that.countGeneInstance(that.singleID,
c);else{var d=that.moduleGenes[a].split(",");c.GroupedGenes=d.length;that.countGeneInstance(that.moduleGenes[a],c)}that.modules[a]=c;that.requests--;b&&b(a);ga&&ga("send","event","requestModule",a)},error:function(a,b,c){that.requests--}});that.moduleRequestList.push(xhr)};that.scanFilter=function(a,b){var c=[];if(a)for(var e=0;e<a.length;e++)b(a[e])&&c.append(a[e]);return c};that.countGeneInstance=function(a,b){var c=b.TCList,e=0;b.selectGeneTC={};for(var f=0;f<c.length;f++)-1<a.indexOf(c[f].Gene.ID)&&
e++;b.GeneCount=e};that.createImageControls=function(){that.imageBar=d3.select("#imageControl").append("span").style("float","left");that.imageBar.append("span").attr("class","saveImage control").style("display","inline-block").attr("id","saveImage").style("cursor","pointer").append("img").attr("src",contextRoot+"web/images/icons/savePic_dark.png").attr("pointer-events","all").attr("cursor","pointer").on("click",function(){var a="";if(that.singleImage){"eqtl"===that.viewType&&"eqtl"===that.singleImage.type?
a=$("div#circos").html():"gene"===that.viewType&&"gene"===that.singleImage.type?a=$("div#viewGene").html():"mir"===that.viewType&&"mir"===that.singleImage.type?a=$("div#viewGene").html():"go"===that.viewType&&"go"===that.singleImage.type&&(a=$("div#viewGene").html());var b=$(window).width();a=a.replace('width="100%"','width="'+b+'px"');$.ajax({url:pathPrefix+"saveBrowserImage.jsp",type:"POST",contentType:"text/html",data:a+"\n",processData:!1,dataType:"json",success:function(a){var b=new Date;b=b.getMonth()+
1+"_"+b.getDate()+"_"+b.getFullYear();var c="http://";"https:"===location.protocol&&(c="https://");var d=c+urlprefix+"/tmpData/download/"+a.imageFile;a="";that.selectedModule&&(a=new String(that.selectedModule.MOD_NAME));a=a+"_"+that.viewType;var e=a+"_"+b+".png",f=new XMLHttpRequest;f.open("GET",d);f.responseType="blob";f.send();f.onreadystatechange=function(){if(4!=f.readyState||200!=f.status)return!1;var a=document.createElement("a");a.href=window.URL.createObjectURL(f.response);a.download=e;a.style.display=
"none";document.body.appendChild(a);try{a.click()}catch(t){$("#wgcnaMouseHelp").html("<span style='color:#FF0000;'>Your browser will not save the image directly. Image will open in a popup, in the new window right click to save image.</span>"),window.open(d)}return!0}},error:function(a,b,c){console.log(c)}});ga&&ga("send","event","saveWGCNAView","saveWGCNAView")}}).on("mouseover",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/savePic_white.png");$("#wgcnaMouseHelp").html("Click to download a PNG image of the current view.")}).on("mouseout",
function(){d3.select(this).attr("src",contextRoot+"web/images/icons/savePic_dark.png");$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")});that.imageBar.append("span").attr("class","reset control").style("display","inline-block").attr("id","wgcnaresetImage").style("cursor","pointer").append("img").attr("src",contextRoot+"web/images/icons/reset_dark.png").attr("pointer-events","all").attr("cursor","pointer").on("click",function(){that.singleImage&&
("eqtl"===that.viewType?that.singleImage.panZoomCircos.reset():"undefined"!==typeof that.singleImage&&that.singleImage.panZoom.reset());ga&&ga("send","event","resetWGCNAView","resetWGCNAView")}).on("mouseover",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/reset_white.png");$("#wgcnaMouseHelp").html("Click to reset image zoom to initial region.")}).on("mouseout",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/reset_dark.png");$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")});
that.imageBar.append("span").attr("class","zoomIn control").style("display","inline-block").style("cursor","pointer").style("position","relative").style("top","7px").attr("id","wgcnazoomInImage").append("img").attr("src",contextRoot+"web/images/icons/magPlus_dark_32.png").attr("pointer-events","all").attr("cursor","pointer").style("position","relative").style("top","-5px").on("click",function(){"eqtl"===that.viewType&&that.singleImage?that.singleImage.panZoomCircos.zoomIn():void 0==that.singleImage&&
that.multiImage?that.multiImage.zoomIn():"undefined"!==typeof that.singleImage&&that.singleImage&&that.singleImage.panZoom.zoomIn();ga&&ga("send","event","zoomInWGCNAView","zoomInWGCNAView")}).on("mouseover",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/magPlus_light_32.png");$("#wgcnaMouseHelp").html("Click to zoom in.")}).on("mouseout",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/magPlus_dark_32.png");$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")});
that.imageBar.append("span").attr("class","zoomOut control").style("display","inline-block").style("cursor","pointer").style("position","relative").style("top","7px").attr("id","wgcnazoomOutImage").append("img").attr("src",contextRoot+"web/images/icons/magMinus_dark_32.png").attr("pointer-events","all").attr("cursor","pointer").style("position","relative").style("top","-5px").on("click",function(){"eqtl"===that.viewType&&that.singleImage?that.singleImage.panZoomCircos.zoomOut():void 0==that.singleImage&&
that.multiImage?that.multiImage.zoomOut():that.singleImage&&"undefined"!==typeof that.singleImage&&"undefined"!==typeof that.singleImage.panZoom&&that.singleImage.panZoom.zoomOut();ga&&ga("send","event","zoomOutWGCNAView","zoomOutWGCNAView")}).on("mouseover",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/magMinus_light_32.png");$("#wgcnaMouseHelp").html("Click to zoom out.")}).on("mouseout",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/magMinus_dark_32.png");
$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")});that.imageBar.append("span").attr("class","back control").style("display","inline-block").style("cursor","pointer").style("position","relative").style("top","7px").attr("id","backModule").append("img").attr("src",contextRoot+"web/images/icons/back_dark_32.png").attr("pointer-events","all").attr("cursor","pointer").style("position","relative").style("top","-5px").on("click",function(){that.multiImage.backModule()}).on("mouseover",
function(){d3.select(this).attr("src",contextRoot+"web/images/icons/back_light_32.png");$("#wgcnaMouseHelp").html("Click to move to the previous module.")}).on("mouseout",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/back_dark_32.png");$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")});that.imageBar.append("span").attr("class","forward control").style("display","inline-block").style("cursor","pointer").style("position",
"relative").style("top","7px").attr("id","forwardModule").append("img").attr("src",contextRoot+"web/images/icons/forward_dark_32.png").attr("pointer-events","all").attr("cursor","pointer").style("position","relative").style("top","-5px").on("click",function(){that.multiImage.forwardModule()}).on("mouseover",function(){d3.select(this).attr("src",contextRoot+"web/images/icons/forward_light_32.png");$("#wgcnaMouseHelp").html("Click to move to the next module.")}).on("mouseout",function(){d3.select(this).attr("src",
contextRoot+"web/images/icons/forward_dark_32.png");$("#wgcnaMouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")})};that.createViewControls=function(){that.viewBar=d3.select("#viewControl").append("div").attr("id","wgcnaViewControls").style("float","right").style("margin-left","10px").style("margin-right","5px").style("text-align","left");that.viewBar.append("text").text("Views:");that.viewBar.append("span").attr("id","HelpWGCNAView").append("img").attr("src",
contextRoot+"web/images/icons/help.png");that.viewBar.append("br");that.viewBar.append("input").attr("type","radio").attr("id","wgcnaGeneViewRB").attr("name","wgcnaViewRB").attr("value","gene").attr("checked","checked").style("margin-left","7px").style("margin-right","3px").on("click",function(){that.viewType="gene";that.createSingleWGCNAImage();ga&&ga("send","event","WGCNA","moduleView")});that.viewBar.append("text").text("Module");that.viewBar.append("span").attr("class","wgcnaControltooltip").attr("title",
'View transcripts within the selected module and their connectivity based on correlation of expression.<BR><BR>Have a question about this view click the <img src="'+contextRoot+'web/images/icons/help.png"> above for a detailed description of each view.').style("margin-left","5px").append("img").attr("src","/web/images/icons/info.gif");that.viewBar.append("br");that.viewBar.append("input").attr("type","radio").attr("id","wgcnaEQTLViewRB").attr("name","wgcnaViewRB").attr("value","eqtl").style("margin-left",
"7px").style("margin-right","3px").on("click",function(){that.viewType="eqtl";that.createSingleWGCNAImage();ga&&ga("send","event","WGCNA","eQTLView")});that.viewBar.append("text").text("Eigengene eQTL");that.viewBar.append("span").attr("class","wgcnaControltooltip").attr("title",'View Circos plot of the module Eigengene eQTLs.<BR><BR>Have a question about this view click the <img src="'+contextRoot+'web/images/icons/help.png"> above for a detailed description of each view.').style("margin-left","5px").append("img").attr("src",
"/web/images/icons/info.gif");that.viewBar.append("br");that.viewBar.append("input").attr("type","radio").attr("id","wgcnaGOViewRB").attr("name","wgcnaViewRB").attr("value","go").style("margin-left","7px").style("margin-right","3px").on("click",function(){that.viewType="go";that.createSingleWGCNAImage();ga&&ga("send","event","WGCNA","goView")});that.viewBar.append("text").text("Gene Ontology");that.viewBar.append("span").attr("class","wgcnaControltooltip").attr("title",'TEMPORARILY DISABLED PLEASE CHECK BACK. View a summary of Gene Ontology terms assigned to transcripts in the selected module.<BR><BR>Have a question about this view click the <img src="'+
...
"meta").style("margin-left","7px").style("margin-right","3px").on("click",function(){that.viewType="meta";that.createSingleWGCNAImage();ga&&ga("send","event","WGCNA","metaView")});that.viewBar.append("text").text("Meta Module (nearest neighbors)");that.viewBar.append("span").attr("class","wgcnaControltooltip").attr("title",'View most closely correlated modules and browse those modules.<BR><BR>Have a question about this view click the <img src="'+contextRoot+'web/images/icons/help.png"> above for a detailed description of each view.').style("margin-left",
"5px").append("img").attr("src","/web/images/icons/info.gif");$("#HelpWGCNAView").on("click",function(a){var b=$(this).attr("id");$("#"+b+"Content").dialog("option","position",{my:"right top",at:"left bottom",of:$(this)});$("#"+b+"Content").dialog("open").css({"font-size":12});a.stopPropagation()})};that.createDataControls=function(){that.dataBar=d3.select("#dataControl").append("span").attr("id","wgcnaDataControls").style("margin-left","10px").style("margin-right","5px").append("table").append("tr");
var a=that.dataBar.append("td");a.append("text").text("Data Source:");var b=a.append("select").attr("id","wgcnaSourceSelect").on("change",function(){$("#singleWGCNASVG").hide();d3.select("#multiWGCNASVG").remove();$("#circosModule").hide();$("#wgcnaModuleTable").hide();$("#wgcnaMirTable").hide();$("#wgcnaMirGeneTable").hide();$("#wgcnaGoTable").hide();$("#wgcnaEqtlTable").hide();that.selSource=$("#wgcnaSourceSelect").val();that.tissue=$("#wgcnaTissueSelect").val();"array"===that.selSource?($("span#spanVersion").hide(),
"Whole Brain"===that.tissue?that.wDSID=3:"Heart"===that.tissue?that.wDSID=4:"Liver"===that.tissue&&(that.wDSID=5),d3.select("#wgcnaTissueSelect").append("option").attr("value","Heart").text("Heart")):"seq"===that.selSource&&($("span#spanVersion").show(),"Whole Brain"===that.tissue?(that.wDSID=8,"1"===that.version&&(that.wDSID=6)):"Liver"===that.tissue?(that.wDSID=9,"1"===that.version&&(that.wDSID=7)):(that.wDSID=8,that.version="2",d3.select("#wgcnaTissueSelect").select('option[value="Brain"]').attr("selected",
"selected"),d3.select("#wgcnaVersionSelect").select('option[value="2"]').attr("selected","selected")),d3.select("#wgcnaTissueSelect").select('option[value="Heart"]').remove());that.requestModuleList()});that.sources=["array"];that.dispSources=["Arrays"];"Rn"===organism&&"rn6"===genomeVer&&(that.sources=["seq","array"],that.dispSources=["RNA-Seq","Arrays"]);for(var c=0;c<that.sources.length;c++)b.append("option").attr("value",that.sources[c]).text(that.dispSources[c]);a.append("br");b=a.append("span").attr("id",
"spanVersion");a.append("text").text("Tissue:");b=a.append("select").attr("id","wgcnaTissueSelect").on("change",function(){$("#singleWGCNASVG").hide();d3.select("#multiWGCNASVG").remove();$("#circosModule").hide();$("#wgcnaModuleTable").hide();$("#wgcnaMirTable").hide();$("#wgcnaMirGeneTable").hide();$("#wgcnaGoTable").hide();$("#wgcnaEqtlTable").hide();that.selSource=$("#wgcnaSourceSelect").val();that.tissue=$("#wgcnaTissueSelect").val();"array"===that.selSource?"Whole Brain"===that.tissue?that.wDSID=
3:"Heart"===that.tissue?that.wDSID=4:"Liver"===that.tissue&&(that.wDSID=5):"seq"===that.selSource&&("Whole Brain"===that.tissue?(that.wDSID=8,"1"===that.version&&(that.wDSID=6)):"Liver"===that.tissue&&(that.wDSID=9,"1"===that.version&&(that.wDSID=7)));that.requestModuleList()});that.tissues=["Brain"];that.dispTissues=["Whole Brain"];"Rn"===organism&&"rn6"===genomeVer&&(that.source&&"arrays"===that.source?(that.tissues=["Whole Brain","Heart","Liver"],that.dispTissues=["Whole Brain","Heart","Liver"]):
(that.tissues=["Whole Brain","Liver"],that.dispTissues=["Whole Brain","Liver"]));for(c=0;c<that.tissues.length;c++)b.append("option").attr("value",that.tissues[c]).text(that.dispTissues[c]);a.append("br");b=a.append("span").attr("id","spanVersion");b.append("text").text("Version:");b=b.append("select").attr("id","wgcnaVersionSelect").on("change",function(){that.version=$("#wgcnaVersionSelect").val();"Whole Brain"===that.tissue?(that.wDSID=8,"1"===that.version&&(that.wDSID=6)):"Liver"===that.tissue&&
(that.wDSID=9,"1"===that.version&&(that.wDSID=7));that.requestModuleList()});that.ver=["2","1"];that.dispVer=["HRDP v4","HRDP v3"];for(c=0;c<that.ver.length;c++)b.append("option").attr("value",that.ver[c]).text(that.dispVer[c]);d3.select("#wgcnaVersionSelect").select('option[value="'+that.version+'"]').attr("selected","selected");a=that.dataBar.append("td").append("div").attr("id","goCtls").attr("class","goCTL").style("display","none").style("margin-left","10px");a.append("text").text("GO Domain:");
b=a.append("select").attr("id","goDomainSelect").on("change",function(){that.singleImage.goDomain=$("#goDomainSelect").val();that.singleImage.draw();ga&&ga("send","event","WGCNA_go","changeDomain")});b.append("option").attr("value","cc").text("Cellular Component");b.append("option").attr("value","bp").text("Biological Process");b.append("option").attr("selected","selected").attr("value","mf").text("Molecular Function");a.append("br");a.append("text").text("Number of Levels to Display:");a.append("input").attr("name",
"value").attr("id","goSpinner");$("#goSpinner").spinner({min:1,max:100,step:1,change:function(a,b){console.log("change");that.singleImage.displayDepth=b.value;that.singleImage.selectedNode!==that.singleImage.data.GOList[0]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[1]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[2]?that.singleImage.redraw():that.singleImage.draw();ga&&ga("send","event","WGCNA_go","changeDepth")},spin:function(a,b){that.singleImage.displayDepth=
b.value;that.singleImage.selectedNode!==that.singleImage.data.GOList[0]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[1]&&that.singleImage.selectedNode!==that.singleImage.data.GOList[2]?that.singleImage.redraw():that.singleImage.draw();ga&&ga("send","event","WGCNA_go","changeDepth")}}).val(3);a=that.dataBar.append("td").append("div").attr("id","mirCtls").attr("class","mirCTL").style("display","none").style("margin-left","10px");b=a.append("table").append("tr");a.append("text").style("font-weight",
"bold").text("Filter miRNAs:");a.append("br");a.append("input").attr("type","checkbox").style("margin-right","5px").attr("id","mirValidatedCBX").on("click",function(){$(this).is(":checked")?that.singleImage.mirValid=1:that.singleImage.mirValid=0;that.singleImage.drawMiR();ga&&ga("send","event","WGCNA_mir","changeValidated")});a.append("text").text("only miRNAs with Validated Targets");a.append("br");a.append("text").style("margin-right","5px").text("# of targeted Genes: ");a.append("div").attr("id",
"minMirGeneSlider").style("display","inline-block").style("width","150px");a.append("span").attr("id","minMirGeneLabel").style("margin-left","5px").append("text").text("1");a.append("br");a.append("text").style("margin-right","5px").text("miRNAs targeting gene: ");a.append("input").attr("type","text").attr("id","mirGeneFilterTxt").on("keyup",function(a){that.singleImage.mirTargetID=$(this).val().toLowerCase();that.singleImage.drawMiR();ga&&ga("send","event","WGCNA_mir","geneFilter")});a.append("br");
a.append("text").style("margin-right","5px").text("miRNA(ID or Accession): ");a.append("input").attr("type","text").attr("id","mirIDFilterTxt").on("keyup",function(a){that.singleImage.mirID=$(this).val().toLowerCase();that.singleImage.drawMiR();ga&&ga("send","event","WGCNA_mir","geneFilter")});a=that.dataBar.append("td").append("div").attr("id","mirCtls2").attr("class","mirCTL").style("display","none").style("margin-left","10px");a.append("text").style("font-weight","bold").text("Display Options:");
a.append("br");a.append("input").attr("type","checkbox").attr("id","mirSortCBX").style("margin-right","5px").on("click",function(){$(this).prop("checked")?that.singleImage.sortMir=$("#mirSortSelect").val():that.singleImage.sortMir=0;that.singleImage.drawMiR();ga&&ga("send","event","WGCNA_mir","mirSort")});a.append("text").text("Hovering over a node(miR or transcript):");a.append("br");a.append("input").attr("type","checkbox").attr("id","mirLinkCBX").attr("checked","checked").style("margin-left","12px").style("margin-right",
"3px").on("click",function(){$(this).prop("checked")?that.singleImage.showMirLinks=1:that.singleImage.showMirLinks=0;ga&&ga("send","event","WGCNA_mir","showLinks")});a.append("text").text("Display links between miRNAs and targeted transcripts");a.append("br");a.append("input").attr("type","checkbox").attr("id","mirgeneLinkCBX").style("margin-left","12px").style("margin-right","3px").on("click",function(){$(this).prop("checked")?(that.singleImage.showLinks=0,$("div#mirLinkColorSelect").show()):(that.singleImage.showLinks=
1,$("div#mirLinkColorSelect").hide());that.singleImage.setupLegend();ga&&ga("send","event","WGCNA_mir","linkColor")});a.append("text").text("Display links between correlated transcripts");b=a.append("div").attr("id","mirLinkColorSelect").style("display","none");b.append("text").style("margin-left","24px").text("Correlation Link Color Options:");a=b.append("select").attr("id","mirlinkColor").on("change",function(){that.singleImage.updateColor("mirlinkColor");that.singleImage.setupLegend();ga&&ga("send",
"event","WGCNA_mir","linkColor")});a.append("option").attr("selected","selected").attr("value","Green_Red").text("Green(+) / Red(-)");a.append("option").attr("value","Blue_Yellow").text("Blue(+) / Yellow(-)");a.append("option").attr("value","Red_Green").text("Red(+) / Green(-)");b.append("br");b.append("input").attr("type","checkbox").style("margin-left","24px").style("margin-right","3px").on("click",function(){$(this).prop("checked")?that.singleImage.showAllFromTarget=1:that.singleImage.showAllFromTarget=
0;ga&&ga("send","event","WGCNA_mir","linkColor")});b.append("text").text("Display all cor. transcript links from targeted transcripts.");$("#minMirGeneSlider").slider({min:1,max:100,step:1,value:1,slide:function(a,b){that.singleImage.mirGeneCutoff=b.value;$("span#minMirGeneLabel").html(b.value);that.singleImage.drawMiR();ga&&ga("send","event","WGCNA_mir","minMir")}});a=that.dataBar.append("td").append("div").attr("id","linkCtls").attr("class","linkCTL").style("display","none").style("margin-left",
"10px");a.append("text").text("Link Correlation Values");a.append("span").style("cursor","pointer").on("click",function(){$("#linkSliderMin").slider("value",.75);that.singleImage.CorCutoff_min=.75;$("span#minLabel").html(.75);$("#linkSliderMax").slider("value",1);that.singleImage.CorCutoff_max=1;$("span#maxLabel").html(1);that.singleImage.draw();ga&&ga("send","event","WGCNA_mir","resetCorrelation")}).append("img").attr("src",contextRoot+"/web/images/icons/reset_16.png");c=a.append("table");b=c.append("tr");
b.append("td").append("text").text("Min:");b.append("td").append("div").attr("id","linkSliderMin").style("width","100px");b.append("td").append("span").attr("id","minLabel").style("margin-left","5px").append("text").text("0.75");b=c.append("tr");b.append("td").append("text").text("Max:");b.append("td").append("div").attr("id","linkSliderMax").style("width","100px");b.append("td").append("span").attr("id","maxLabel").style("margin-left","5px").append("text").text("1");a.append("input").attr("type",
"checkbox").attr("id","hideLinkExceptNode").on("click",function(){that.singleImage.showLinks=!$(this).is(":checked");that.singleImage.draw();that.singleImage.panZoom=svgPanZoom("#singleWGCNASVG",{panEnabled:!0,controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,minZoom:.1,maxZoom:4,fit:!0,center:!0})});a.append("text").text("Hide Links Except Selected Node");b=that.dataBar.append("td").append("div").attr("id","linkCtl2").attr("class","linkCTL").style("display","none").style("margin-left",
"10px");b.append("text").text("Correlation Link Color Options:");b.append("br");b=b.append("select").attr("id","linkColor").on("change",function(){that.singleImage.draw();that.singleImage.panZoom=svgPanZoom("#singleWGCNASVG",{panEnabled:!0,controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,minZoom:.1,maxZoom:4,fit:!0,center:!0})});b.append("option").attr("selected","selected").attr("value","Green_Red").text("Green(+) / Red(-)");b.append("option").attr("value","Blue_Yellow").text("Blue(+) / Yellow(-)");
b.append("option").attr("value","Red_Green").text("Red(+) / Green(-)");$("#linkSliderMin").slider({min:.05,max:.95,step:.05,value:.75,slide:function(a,b){that.singleImage.CorCutoff_min=b.value;$("span#minLabel").html(b.value);that.singleImage.draw()}});$("#linkSliderMax").slider({min:.1,max:1,step:.05,value:1,slide:function(a,b){that.singleImage.CorCutoff_max=b.value;$("span#maxLabel").html(b.value);that.singleImage.draw()}});a=that.dataBar.append("td").append("div").attr("class","eqtlCtls").attr("id",
"eqtlCtl").style("display","none").style("margin-left","10px");a.append("text").text("Select Chromosomes");a.append("br");b=a.append("select").attr("id","eqtlChr").attr("multiple","multiple").attr("size","4").style("width","100px");for(c=1;c<=that.chrLen;c++)b.append("option").attr("selected","selected").attr("value",organism.toLowerCase()+c).text("chr"+c);b.append("option").attr("selected","selected").attr("value",organism.toLowerCase()+"X").text("chrX");a=that.dataBar.append("td").append("div").attr("class",
"eqtlCtls").attr("id","eqtlCtl2").style("display","none").style("margin-left","10px");a.append("text").text("P-value cutoff:");b=a.append("select").attr("id","eqtlPval");b.append("option").attr("value","1").text("0.1");b.append("option").attr("selected","selected").attr("value","2").text("0.01");b.append("option").attr("value","3").text("0.001");b.append("option").attr("value","4").text("0.0001");b.append("option").attr("value","5").text("0.00001");a.append("br");a.append("br");a.append("br");a.append("br");
a.append("input").attr("type","button").attr("value","Apply Selections").on("click",function(){for(var a=$("#eqtlPval").val(),b=$("#eqtlChr").val(),c="",d=0;d<b.length;d++)c=c+b[d]+";";$.ajax({url:pathPrefix+"runCircosModule.jsp",type:"GET",async:!0,data:{cutoffValue:a,organism:organism,module:that.selectedModule.MOD_NAME,modColor:that.selectedModule.ModRGB,chrList:c,genomeVer:genomeVer,tissue:that.tissue},dataType:"json",beforeSend:function(){$("#waitCircos").show()},success:function(a){$("#waitCircos").hide();
that.singleImage.update(a.URL)},error:function(a,b,c){}})})};that.positionTTLeft=function(a){a+=20;a>$(window).width()/2&&(a-=250);return a+"px"};that.positionTTTop=function(a){a+="px";d3.event.clientY>.6*window.innerHeight&&(a=d3.event.pageY-2.2*$(".testToolTip").height()+"px");return a};that.createMultiWGCNAImage=function(){that.multiImage=that.multiWGCNAImageGeneView();that.multiImage.draw();that.img=that.multiImage};that.multiWGCNAImage=function(){var a={maxWidth:200};a.totalWidth=$(window).width();
a.modPerLine=Math.floor($(window).width()/a.maxWidth);a.calcWidth=Math.floor($(window).width()/a.modPerLine);a.data=[];if(0===d3.select("#multiWGCNASVG").size()){var b=d3.select("#wgcnaGeneImage").append("div").attr("id","multiWGCNAScroll").style("overflow","auto").style("max-height","430px");a.svg=b.append("svg").attr("id","multiWGCNASVG").attr("height","430px").attr("width","100%");b=a.svg.append("linearGradient").attr("id","lgrad").attr("x1","11%").attr("y1","0%").attr("x2","89%").attr("y2","100%");
b.append("stop").attr("offset","0%").attr("style","stop-color:rgb(153,218,255);stop-opacity:1");b.append("stop").attr("offset","100%").attr("style","stop-color:rgb(0,128,128);stop-opacity:1");b=a.svg.append("linearGradient").attr("id","selectGrad").attr("x1","11%").attr("y1","0%").attr("x2","89%").attr("y2","100%");b.append("stop").attr("offset","0%").attr("style","stop-color:rgb(0,163,55);stop-opacity:1");b.append("stop").attr("offset","100%").attr("style","stop-color:rgb(0,75,20);stop-opacity:1")}else a.svg=
d3.select("#multiWGCNASVG");a.modKey=function(a){return a.MOD_NAME};a.calcX=function(b,c){return c%a.modPerLine*a.calcWidth};a.calcY=function(b,c){return Math.floor(c/a.modPerLine)*a.maxWidth};a.sizing=function(b){b=a.size(b.TCList.length);b>a.maxWidth/2&&(b=a.maxWidth/2);return b};a.zoomIn=function(){a.maxWidth<$(window).width()/2&&(a.maxWidth*=2,a.draw())};a.zoomOut=function(){var b=a.maxWidth;a.maxWidth=Math.floor(a.maxWidth/2);25>a.maxWidth?a.maxWidth=b:a.draw()};a.sortTCLenDesc=function(a,b){return b.TCList.length<
a.TCList.length?-1:b.TCList.length>a.TCList.length?1:0};a.sortGeneInstanceDesc=function(b,c){return c.GeneCount<b.GeneCount?-1:c.GeneCount>b.GeneCount?1:a.sortTCLenDesc(b,c)};a.draw=function(){};a.redraw=function(){};return a};that.multiWGCNAImageGeneView=function(){var a=that.multiWGCNAImage();a.createToolTip=function(a){var b="Module Name: "+a.MOD_NAME+"<BR>Transcript Size: "+a.TCList.length;"undefined"!==typeof a.GroupedGenes?(b=b+"<BR>Genes in Region/List: "+a.GroupedGenes,b=b+"<BR>Transcripts in Region/List: "+
a.GeneCount):b=b+"<BR>Selected Gene Transcripts in Module: "+a.GeneCount;return b};a.draw=function(){"undefined"!==typeof that.selectedModule&&(a.maxWidth=85);a.svg.selectAll(".modules").remove();a.svg.selectAll(".moduleGradient").remove();a.totalWidth=$(window).width();a.modPerLine=Math.floor($(window).width()/a.maxWidth);a.calcWidth=Math.floor($(window).width()/a.modPerLine);var b=[];a.dataList={};for(var c=0,e=0;e<that.moduleList.length;e++)if(console.log("multiModView:"+that.moduleList[e]),("grey"!==
that.moduleList[e]||"grey"===that.moduleList[e]&&0===that.skipGrey)&&"undefined"!==typeof that.modules[that.moduleList[e]]){b[c]=that.modules[that.moduleList[e]];var f=b[c].ModRGB.split(","),h="";for(h=0;h<f.length;h++)f[h]-=70,0>f[h]&&(f[h]=0);h=f[0]+","+f[1]+","+f[2];f=a.svg.append("linearGradient").attr("id","grad"+b[c].MOD_NAME).attr("class","moduleGradient").attr("x1","11%").attr("y1","0%").attr("x2","89%").attr("y2","100%");f.append("stop").attr("offset","0%").attr("style","stop-color:rgb("+
b[c].ModRGB+");stop-opacity:1");f.append("stop").attr("offset","100%").attr("style","stop-color:rgb("+h+");stop-opacity:1");c++}c=a.maxWidth+15;b.length>a.modPerLine&&(e=Math.floor(b.length/a.modPerLine),0<b.length%a.modPerLine&&e++,c*=e);a.svg.attr("height",c+"px");b.sort(a.sortGeneInstanceDesc);a.data=b;for(e=0;e<b.length;e++)a.dataList[b[e].MOD_NAME]=e;c=d3.min(b,function(a){return a.TCList.length});e=d3.max(b,function(a){return a.TCList.length});a.size=d3.scaleLinear().domain([c,e]).range([10,
a.maxWidth/2]);b=a.svg.selectAll(".modules").data(b,a.modKey).attr("transform",function(b,c){return"translate("+a.calcX(b,c)+","+a.calcY(b,c)+")"});b.enter().append("g").attr("class","modules").attr("id",function(a){return a.MOD_NAME}).attr("transform",function(b,c){return"translate("+a.calcX(b,c)+","+a.calcY(b,c)+")"}).merge(b).each(function(b){var c=d3.select("g#"+b.MOD_NAME);c.append("circle").attr("cx",a.calcWidth/2).attr("cy",a.maxWidth/2).attr("r",a.sizing).attr("stroke",function(a){var b="#FFFFFF";
"undefined"!==typeof that.selectedModule&&that.selectedModule===a&&(b="#000000");return b}).attr("stroke-width",function(a){var b="1px";"undefined"!==typeof that.selectedModule&&that.selectedModule===a&&(b="3px");return b}).attr("fill",function(a){var b="url(#grad"+a.MOD_NAME+")";"undefined"!==typeof that.selectedModule&&that.selectedModule===a&&(b="url(#selectGrad)");return b}).on("click",function(){a.maxWidth=85;that.selectedModule=b;a.draw();d3.select("#multiWGCNAScroll").style("max-height","110px");
a.svg.select("g#"+b.MOD_NAME).style("background","#CECECE");a.svg.select("g#"+b.MOD_NAME).select("circle").attr("fill","url(#selectGrad)").attr("stroke","#000000").attr("stroke-width","3px");a.svg.select("g#"+b.MOD_NAME).select("text.txtLbl").style("font-weight","bold");that.selectedGene={};for(var c=0;c<b.TCList.length;c++)"undefined"===typeof that.selectedGene[b.TCList[c].Gene.ID]&&(that.selectedGene[b.TCList[c].Gene.ID]={},that.selectedGene[b.TCList[c].Gene.ID].geneSymbol=b.TCList[c].Gene.geneSymbol,
that.selectedGene[b.TCList[c].Gene.ID].tcList=[]),that.selectedGene[b.TCList[c].Gene.ID].tcList.push(b.TCList[c]);setTimeout(that.createSingleWGCNAImage,20)}).on("mouseover",function(){$("#wgcnaMouseHelp").html("<B>Click</B> to select this module and see additional details.");d3.select(this).style("fill","url(#selectGrad)");tt.transition().duration(200).style("opacity",1);tt.html(a.createToolTip(b)).style("left",function(){return that.positionTTLeft(d3.event.pageX)}).style("top",function(){return that.positionTTTop(d3.event.pageY)})}).on("mouseout",
function(){var a=d3.select(this).data()[0];"undefined"!==typeof that.selectedModule&&a.MOD_NAME===that.selectedModule.MOD_NAME||d3.select(this).style("fill","url(#grad"+a.MOD_NAME+")");tt.transition().duration(500).style("opacity",0)});"undefined"!==typeof that.selectedModule&&that.selectedModule===b?c.append("text").attr("class","txtLbl").attr("y",a.maxWidth+12).attr("x",function(b){return a.calcWidth/2-b.MOD_NAME.length/2*6.5}).style("font-weight","bold").text(replaceUnderscore(b.MOD_NAME)):6.5*
...
that.img=that.singleImage;setTimeout(function(){that.singleImage.panZoom=svgPanZoom("#singleWGCNASVG",{panEnabled:!0,controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,minZoom:.1,maxZoom:4,fit:!0,center:!0})},500)},50)):"meta"===that.viewType&&setTimeout(function(){that.singleImage=that.singleWGCNAImageMetaView(that.modules);that.singleImage.requestMeta();that.img=that.singleImage;setTimeout(function(){that.singleImage.panZoom=svgPanZoom("#singleWGCNASVG",{panEnabled:!0,
controlIconsEnabled:!1,zoomEnabled:!0,dblClickZoomEnabled:!0,zoomScaleSensitivity:.2,minZoom:.1,maxZoom:4,fit:!0,center:!0})},500)},50);ga&&ga("send","event","changedWGCNAView",that.viewType)};that.singleWGCNAImage=function(){var a={};a.width=$(window).width();a.height=Math.floor(.9*window.innerHeight);a.r=a.height>a.width?Math.floor(a.width/2*.95):Math.floor(a.height/2*.95);a.maxPerLevel=150;a.CorCutoff_min=.75;a.CorCutoff_max=1;a.showLinks=!0;a.data=[];a.maxNodeR=28;a.negColor="#DD0000";a.negDarkColor=
"#770000";a.posColor="#00DD00";a.posDarkColor="#007700";a.mirVal="#FFFF00";a.mirPred="#0000CC";0===d3.select("#singleWGCNASVG").size()?a.svg=d3.select("#wgcnaGeneImage").append("div").attr("id","viewGene").append("svg").attr("id","singleWGCNASVG").attr("height",a.height+"px").attr("width","100%"):a.svg=d3.select("#singleWGCNASVG");a.geneKey=function(a){return a.ID};a.linkKey=function(a){return that.removeInvalidIDChars(a.TC1)+that.removeInvalidIDChars(a.TC2)};a.calcX=function(b,c){return c%a.modPerLine*
