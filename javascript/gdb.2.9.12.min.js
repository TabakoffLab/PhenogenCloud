...
var mmVer='Mouse(<span id="verSelect"></span>) Strain:C57BL/6J',rnVer='Rat(<span id="verSelect"></span>) Strain:BN',siteVer="PhenoGen v3.7.9(8/5/2021)",trackBinCutoff=1E4,customTrackLevel=-1,mouseTTOver=0,ttHideHandle=0,tt=d3.select("body").append("div").attr("class","testToolTip").style("z-index",1001).style("opacity",0);tt.append("span").style("float","right").append("img").attr("src","web/images/icons/close.png").on("click",function(){tt.style("opacity",0)});
...
function updatePage(q){var n=Math.round(q.xScale.domain()[0]),r=Math.round(q.xScale.domain()[1]);if((n<q.prevMinCoord||r>q.prevMaxCoord)&&(n<q.dataMinCoord||r>q.dataMaxCoord)){processAjax=1;var u=n;var f=r;n>=q.dataMinCoord&&r>q.dataMaxCoord?u=q.dataMaxCoord+1:n<q.dataMinCoord&&r<=q.dataMaxCoord&&(f=q.dataMinCoord-1);q.setLoading();$.ajax({url:pathPrefix+"updateRegion.jsp",type:"GET",data:{chromosome:chr,minCoord:u,maxCoord:f,fullminCoord:n,fullmaxCoord:r,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,
myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(b){q.prevMinCoord=n;q.prevMaxCoord=r;n<q.dataMinCoord&&(q.dataMinCoord=n);r>q.dataMaxCoord&&(q.dataMaxCoord=r);q.folderName=b.folderName;q.updateData();processAjax=0;gtag("event","outsideLoadedRegion",{event_category:"updatePage"})},error:function(b,a,d){console.log(d)}})}}
function back(q){if(history&&history[q]){var n=1<history[q].length?history[q].pop():history[q][0];chr==n.chr&&(n.start==svgList[q].xScale.domain()[0]&&n.stop==svgList[q].xScale.domain()[1]&&(n=1<history[q].length?history[q].pop():history[q][0]),0==q&&$("#geneTxt").val(n.chr+":"+n.start+"-"+n.stop),svgList[q].xScale.domain([n.start,n.stop]),svgList[q].scaleSVG.select(".x.axis").call(svgList[q].xAxis),svgList[q].redraw(),0===q&&updatePage(svgList[q]),svgList[q].updateFullData())}gtag("event","navigateBack",
{event_category:"stepBackSVGNaviation"})}
function zoomIn(q,n){var r={};r.chr=chr;r.start=svgList[q].xScale.domain()[0];r.stop=svgList[q].xScale.domain()[1];var u=Math.floor((r.stop-r.start)*n);1>u&&(u=1);r.start+=u;r.stop-=u;20>r.stop-r.start&&(r.start-=u,r.stop+=u);svgList[q].xScale.domain([r.start,r.stop]);history[q]||(history[q]=[]);history[q].push(r);svgList[q].scaleSVG.select(".x.axis").call(svgList[q].xAxis);svgList[q].redraw();isNaN(zoomUpdateHandle)||(clearTimeout(zoomUpdateHandle),zoomUpdateHandle=Math.NaN);zoomUpdateHandle=setTimeout(function(){0===
...
function zoomOut(q,n){var r={};r.chr=chr;r.start=svgList[q].xScale.domain()[0];r.stop=svgList[q].xScale.domain()[1];var u=Math.floor((r.stop-r.start)*n);1>u&&(u=1);r.start-=u;r.stop+=u;svgList[q].xScale.domain([r.start,r.stop]);history[q]||(history[q]=[]);history[q].push(r);svgList[q].scaleSVG.select(".x.axis").call(svgList[q].xAxis);svgList[q].redraw();isNaN(zoomUpdateHandle)||(clearTimeout(zoomUpdateHandle),zoomUpdateHandle=Math.NaN);zoomUpdateHandle=setTimeout(function(){0===q&&updatePage(svgList[q]);
...
function mup(){var q=0;for(q=0;q<svgList.length;q++)if(svgList[q]&&"undefined"!==typeof svgList[q])if(history[q]||0!=q&&1!=q||(history[q]=[]),!(svgList[q].overSettings&&0!=svgList[q].overSettings||isNaN(svgList[q].downx)&&isNaN(svgList[q].downPanx))){if(0===q&&updatePage(svgList[q]),svgList[q].downx=Math.NaN,svgList[q].downPanx=Math.NaN,svgList[q].updateFullData(),0===q&&setTimeout(function(){DisplayRegionReport()},300),0==q||1==q){var n={};n.chr=chr;n.start=svgList[q].xScale.domain()[0];n.stop=svgList[q].xScale.domain()[1];
history[q].push(n)}}else if(!(svgList[q].overSettings&&0!=svgList[q].overSettings||isNaN(svgList[q].downZoomx))){var r=svgList[q].downZoomx;var u=d3.mouse(svgList[q].vis.node());svgList[q].downZoomxEnd=u[0];n=1;u[0]<r?(r=u[0],n=svgList[q].downZoomx-r):n=u[0]-r;u=Math.round(svgList[q].xScale.invert(r));r=Math.round(svgList[q].xScale.invert(r+n));svgList[q].downZoomx=Math.NaN;svgList[q].downZoomxEnd=Math.NaN;0===q&&$("#geneTxt").val(chr+":"+u+"-"+r);d3.select("#Level"+svgList[q].levelNumber).selectAll("svg rect.zoomRect").remove();
d3.select("#Level"+svgList[q].levelNumber).selectAll("svg text#zoomTextStart").remove();d3.select("#Level"+svgList[q].levelNumber).selectAll("svg text#zoomTextEnd").remove();svgList[q].xScale.domain([u,r]);svgList[q].scaleSVG.select(".x.axis").call(svgList[q].xAxis);svgList[q].redraw();svgList[q].updateFullData();0===q&&setTimeout(function(){DisplayRegionReport()},300);if(0==q||1==q)n={},n.chr=chr,n.start=u,n.stop=r,history[q].push(n)}gtag("event","mouseUpFromDrag",{event_category:"dragMouseUp"})}
function mmove(){var q;for(q=0;q<svgList.length;q++)if(svgList[q]&&(!svgList[q].overSettings||0===svgList[q].overSettings))if(!isNaN(svgList[q].downx)){var n=d3.mouse(svgList[q].vis.node());rupx=n[0];if(0!==rupx){var r=Math.round(svgList[q].downscalex.domain()[0]);var u=Math.round(svgList[q].mw*(svgList[q].downx-svgList[q].downscalex.domain()[0])/rupx+svgList[q].downscalex.domain()[0]);u<=svgList[q].xMax&&r>=svgList[q].xMin&&(0===q&&$("#geneTxt").val(chr+":"+r+"-"+u),svgList[q].xScale.domain([r,u]),
svgList[q].scaleSVG.select(".x.axis").call(svgList[q].xAxis),svgList[q].redraw())}}else if(isNaN(svgList[q].downPanx))isNaN(svgList[q].downZoomx)||(f=svgList[q].downZoomx,n=d3.mouse(svgList[q].vis.node()),svgList[q].downZoomxEnd=n[0],n[0]<f?(f=n[0],n=svgList[q].downZoomx-f):n=n[0]-f,r=Math.round(svgList[q].xScale.invert(f)),u=Math.round(svgList[q].xScale.invert(f+n)),d3.select("#Level"+svgList[q].levelNumber).selectAll("svg rect.zoomRect").attr("x",f).attr("width",n),d3.select("#Level"+svgList[q].levelNumber).selectAll("svg text#zoomTextStart").attr("x",
f).attr("y",15).text(numberWithCommas(r)),d3.select("#Level"+svgList[q].levelNumber).selectAll("svg text#zoomTextEnd").attr("x",f+n).attr("y",50).text(numberWithCommas(u)));else if(n=d3.mouse(svgList[q].vis.node()),rupx=n[0],0!==rupx){u=svgList[q].downPanx-rupx;var f=(svgList[q].downscalex.domain()[1]-svgList[q].downscalex.domain()[0])/svgList[q].mw;r=Math.round(svgList[q].downscalex.domain()[0]+u*f);u=Math.round(u*f+svgList[q].downscalex.domain()[1]);u<=svgList[q].xMax&&r>=svgList[q].xMin&&(0===
q&&$("#geneTxt").val(chr+":"+r+"-"+u),svgList[q].xScale.domain([r,u]),svgList[q].scaleSVG.select(".x.axis").call(svgList[q].xAxis),svgList[q].redraw(),svgList[q].downPanx=n[0])}}d3.select("html").on("mousemove",mmove).on("mouseup",mup);$(window).resize(function(){for(var q=0;q<svgList.length;q++)svgList[q]&&svgList[q].resize($(window).width()-25)});
$(document).on("click",".closeBtn",function(){var q=new String($(this).attr("id"));q=q.substr(6);$("."+q).fadeOut("fast");if(-1<q.indexOf("viewsLevel")){var n=q.substr(q.length-1);$("div#nameView"+n).hide();$("div#selection"+n).show();$("span#viewMenuLbl"+n).html("Select/Edit Views");$("div.trackLevel"+n).fadeOut("fast")}gtag("event",q,{event_category:"closeSettings"});return!1});
$(document).on("click",".viewSelect",function(){var q=$(this).attr("id");q=q.substr(q.length-1);if($(".viewsLevel"+q).is(":visible"))$(".viewsLevel"+q).fadeOut("fast");else{var n=$(this).parent().parent().position().top;$(".viewsLevel"+q).css("top",n).css("left",$(window).width()-610);$(".viewsLevel"+q).fadeIn("fast");$("#trackSettingDialog").hide();tt.transition().duration(200).style("opacity",0);gtag("event",q,{event_category:"openViews"})}return!1});
$(document).on("change","input[name='optioncbx']",function(){var q=new String($(this).attr("id")),n=q.indexOf("CBX"),r=new String(q.substr(0,n));n=q.substr(n+3,1);redrawTrack(n,r);gtag("event",q,{event_category:"changeSettingView"})});$(document).on("change","input[name='imgCBX']",function(){var q=new String($(this).attr("id")),n=q.indexOf("CBX");new String(q.substr(0,n));n=q.substr(n+3,1);svgList[n].redraw();setTimeout(function(){DisplayRegionReport()},300);gtag("event",q,{event_category:"clickCbx"})});
$(document).on("click",".reset",function(){var q=new String($(this).attr("id")),n=q.substr(q.length-1);0===q.indexOf("resetImage")?0===n&&($("#geneTxt").val(chr+":"+initMin+"-"+initMax),svgList[0].xScale.domain([initMin,initMax]),svgList[0].scaleSVG.select(".x.axis").call(svgList[0].xAxis),svgList[0].redraw()):0===q.indexOf("resetTracks")&&(svgList[n].removeAllTracks(),setupDefaultView(n));gtag("event",q,{event_category:"resetImage"})});
function numberWithCommas(q){return q.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}
function displayHelpFirstTime(){if(-1===navigator.userAgent.toLowerCase().indexOf("phantomjs"))if($.cookie("genomeBrowserHelp")){var q=$.cookie("genomeBrowserHelp"),n=q.indexOf("v")+1,r=q.indexOf(".",q.indexOf(".")+1);q=q.substr(n,r-n);n=siteVer.indexOf("v")+1;r=siteVer.indexOf(".",siteVer.indexOf(".")+1);n=siteVer.substr(n,r-n);q!==n&&(setTimeout(function(){$("a#fbhelp1").click()},1E3),$.cookie("genomeBrowserHelp",siteVer))}else setTimeout(function(){$("a#fbhelp1").click()},1E3),$.cookie("genomeBrowserHelp",
siteVer)}function removeTrack(q,n){"undefined"!==typeof svgList[q]&&svgList[q].removeTrack(n);gtag("event",q,{event_category:"removeTrack",event_label:n})}function redrawTrack(q,n){svgList[q].redrawTrack(n);gtag("event",q,{event_category:"redrawTrack",event_label:n})}function changeTrackHeight(q,n){if(0<n){var r=n+"px";$("#Scroll"+q).css({"max-height":r,overflow:"auto"})}else $("#Scroll"+q).css({"max-height":"",overflow:"hidden"})}
registerKeyboardHandler=function(q){d3.select(window).on("keydown",q)};function getAllChildrenByName(q,n){var r=[],u=0;if("undefined"!==typeof q&&null!==q&&"undefined"!==typeof q.childNodes)for(var f=q.childNodes,b=0;b<f.length;b++)f.item(b).nodeName==n&&(r[u]=f.item(b),u++);return r}
function getFirstChildByName(q,n){var r=null;if(null!==q&&"undefined"!==typeof q&&"undefined"!==typeof q.childNodes)for(var u=q.childNodes,f=!1,b=0;b<u.length&&!f;b++)u.item(b).nodeName==n&&(r=u.item(b),f=!0);return r}
function getAddMenuDiv(q,n){var r="/"+pathPrefix;""==pathPrefix&&(r="");$.ajax({url:r+"settingsMenu.jsp",type:"GET",cache:!1,data:{level:q,organism:organism,type:n},dataType:"html",success:function(u){$("#imageMenu"+q).remove();d3.select("div#imageMenu").append("div").attr("id","imageMenu"+q);$("#imageMenu"+q).html(u)},error:function(u,f,b){$("#imageMenu"+q).remove();d3.select("div#imageMenu").append("div").attr("id","imageMenu"+q);$("#imageMenu"+q).append('<div class="settingsLevel'+q+'">An error occurred generating this menu.  Please try back later.</div>')},
async:!1})}function loadStateFromString(q,n,r,u){loadSavedConfigTracks(q,r,u);loadImageState(n,r)}
function loadSavedConfigTracks(q,n,r){q=q.split(";");var u=0;r=90>n?svgList[n]:r;for(var f=0;f<q.length;f++){var b=q[f].split(",");if(("Rn"==organism&&"undefined"===typeof mouseOnly[b[0]]||"Mm"==organism&&"undefined"===typeof ratOnly[b[0]])&&""!=b[0]){u++;var a="";if(2<b.length)for(var d=2;d<b.length;d++)a=2==d?b[d]:a+","+b[d];1==n&&(a+=",DrawTrx");r.addTrack(b[0],b[1],a,0)}}0==u&&setupDefaultView(n)}
function loadImageState(q,n){if("undefined"!==typeof q&&""!=q)for(var r=q.split(";"),u=0;u<r.length;u++){var f=r[u].split("=");0==(new String(f[0])).indexOf("displaySelect")&&changeTrackHeight("Level"+n,f[1])}}function calculateBin(q,n){var r=new String(Math.floor(q/n));r=r.substr(0,1)*Math.pow(10,r.length-1);r=Math.floor(r/2);5>r&&(r=0);return r}function key(q){return"undefined"!==typeof q?q.getAttribute("ID"):"unknown"}
function keyName(q){return"undefined"!==typeof q?q.getAttribute("name"):"unknown"}function keyStart(q){return"undefined"!==typeof q?q.getAttribute("start"):"unknown"}function keyTissue(q,n){return"undefined"!==typeof q?q.getAttribute("ID")+n:"unknown"}function keyPos(q){return"undefined"!==typeof q?q.pos:"unknown"}function keyID(q){return"undefined"!==typeof q?q.id:"unknown"}function keySNP(q){return"undefined"!==typeof q?q.getAttribute("strain")+"_"+q.getAttribute("start"):"unknown"}
function GenomeSVG(q,n,r,u,f,b,a,d){var c={isToolTip:0,folderName:"",selectedTrackSetting:"",trackListHash:{},overSettings:0,zoomFactor:.05};c.strainSpecificCountColors=d3.scaleOrdinal(d3.schemeCategory20);0==f&&(c.folderName=regionfolderName);var h={};h.chr=chr;h.start=r;h.stop=u;c.allowSelectGenomeVer="undefined"===typeof d?!0:d;if(20>f){history[f]||(history[f]=[]);try{history[f].push(h)}catch(e){Rollbar.error("history["+f+"] not defined")}}c.get=function(e){return c[e]};c.getTrackData=function(e){for(var g=
...
panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(D){},error:function(D,E,z){console.log(z)}});p=null;p=d3.select("#Level"+c.levelNumber+e).nodes();if(0===p.length||null==p){p=c.topLevel.append("li").attr("class","draggable"+c.levelNumber).attr("id","li"+e).style("margin-bottom","-3px").append("svg").attr("width",c.width).attr("height",30).attr("class","track").attr("id","Level"+c.levelNumber+e).style("cursor","move").on("mouseover",
...
function(){c.overSettings=0;$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")})}if("genomeSeq"==e)p=SequenceTrack(c,e,"Reference Genomic Sequence",l),c.addTrackList(p);else if(-1<e.indexOf("noncoding"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",function(D,E){if(D)if(60>m){var z=1500;1==m&&(z=2500);setTimeout(function(){c.addTrack(e,g,l,m+1)},z)}else d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+
e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==E)if(11<=m){z=[];var C=GeneTrack(c,z,e,"Long Non-Coding / Non-PolyA+ Genes",l);c.addTrackList(C)}else setTimeout(function(){c.addTrack(e,g,l,10)},5E3);else{z=E.documentElement.getElementsByTagName("Gene");var H=0,I=E.documentElement.getElementsByTagName("GeneList");0<I.length&&"undefined"!==typeof I[0].getAttribute("ver")&&(H=I[0].getAttribute("ver"));
try{C=GeneTrack(c,z,e,"Long Non-Coding / Non-PolyA+ Genes",l),C.dataVer=H,c.addTrackList(C)}catch(J){}}});else if(-1<e.indexOf("coding"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",function(D,E){if(D)if(60>m){var z=1500;1==m&&(z=2500);setTimeout(function(){c.addTrack(e,g,l,m+1)},z)}else d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+
c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==E)if(11<=m){z=[];var C=GeneTrack(c,z,e,"Protein Coding / PolyA+",l);c.addTrackList(C)}else setTimeout(function(){c.addTrack(e,g,l,11)},5E3);else{z=E.documentElement.getElementsByTagName("Gene");var H=0,I=E.documentElement.getElementsByTagName("GeneList");0<I.length&&"undefined"!==typeof I[0].getAttribute("ver")&&(H=I[0].getAttribute("ver"));try{C=GeneTrack(c,z,e,"Protein Coding / PolyA+",l),C.dataVer=H,c.addTrackList(C)}catch(J){console.log(J)}}});
else if(-1<e.indexOf("smallnc"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",function(D,E){if(D){if(0===m){var z=c.xScale.domain()[0],C=c.xScale.domain()[1],H="/"+pathPrefix;pathPrefix||(H="");$.ajax({url:H+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:z,maxCoord:C,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:e,folder:c.folderName},dataType:"json",success:function(I){gtag("event",
"generateTrack"+e,{event_category:"browser"})},error:function(I,J,K){}})}60>m?(z=1500,1==m&&(z=2500),setTimeout(function(){c.addTrack(e,g,l,m+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==E)11<=m?(z=[],C=GeneTrack(c,z,e,"Small RNA (<200 bp) Genes",l),c.addTrackList(C)):setTimeout(function(){c.addTrack(e,
...
success:function(J){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(J,K,L){}})}60>m?(z=1500,1==m&&(z=2500),setTimeout(function(){c.addTrack(e,g,l,m+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An error occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==E)11<=m?(z=[],C=GeneTrack(c,z,e,v,l),c.addTrackList(C)):setTimeout(function(){c.addTrack(e,
g,l,11)},5E3);else{z=E.documentElement.getElementsByTagName("Gene");H=0;I=E.documentElement.getElementsByTagName("GeneList");0<I.length&&"undefined"!==typeof I[0].getAttribute("ver")&&(H=I[0].getAttribute("ver"));try{C=GeneTrack(c,z,e,v,l),C.dataVer=H,c.addTrackList(C),""!=selectGene&&C.setSelected(selectGene)}catch(J){}}})}else if(0===e.indexOf("refSeq")){$("#"+e+c.levelNumber+"Select").val();var w=c.xScale.domain()[0],A=c.xScale.domain()[1];p=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+
c.folderName+"/"+e+".xml";d3.xml(p,function(D,E){if(D){console.log(D);if(0===m){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:w,maxCoord:A,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:e,folder:c.folderName},dataType:"json",success:function(H){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(H,I,J){}})}60>m?(z=1500,1===m&&
(z=2500),setTimeout(function(){c.addTrack(e,g,l,m+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==E)if(11<=m){z=[];var C=RefSeqTrack(c,z,e,"Ref Seq Genes",l);c.addTrackList(C)}else setTimeout(function(){c.addTrack(e,g,l,11)},5E3);else{z=E.documentElement.getElementsByTagName("Gene");try{C=
RefSeqTrack(c,z,e,"Ref Seq Genes",l),c.addTrackList(C)}catch(H){console.log(H)}}})}else if(0==e.indexOf("snp"))w=c.xScale.domain()[0],A=c.xScale.domain()[1],p=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",d3.xml(p,function(D,E){if(D){console.log(D);if(0==m){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:w,maxCoord:A,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,
myOrganism:organism,genomeVer:genomeVer,track:e,folder:c.folderName},dataType:"json",success:function(H){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(H,I,J){}})}60>m?(z=1500,1===m&&(z=2500),setTimeout(function(){c.addTrack(e,g,l,m+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+
e))}else if(null==E)if(11<=m){z=[];var C=SNPTrack(c,z,e,g,l);c.addTrackList(C)}else setTimeout(function(){c.addTrack(e,g,l,11)},5E3);else{z=E.documentElement.getElementsByTagName("Snp");try{C=SNPTrack(c,z,e,g,l),c.addTrackList(C)}catch(H){}}});else if("qtl"===e)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/qtl.xml",function(D,E){if(D)if(60>m){var z=1500;1===m&&(z=2500);setTimeout(function(){c.addTrack(e,g,l,m+1)},z)}else d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+
e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==E)if(11<=m){z=[];var C=QTLTrack(c,z,e,g);c.addTrackList(C)}else setTimeout(function(){c.addTrack(e,g,l,11)},5E3);else{z=E.documentElement.getElementsByTagName("QTL");try{C=QTLTrack(c,z,e,g),c.addTrackList(C)}catch(H){}}});else if("trx"===e)y=getAllChildrenByName(getFirstChildByName(c.selectedData,"TranscriptList"),"Transcript"),p=TranscriptTrack(c,
y,e,g),c.addTrackList(p);else if("probe"===e||"probeMouse"==e)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/probe.xml",function(D,E){if(D)if(60>m){var z=1500;1===m&&(z=2500);setTimeout(function(){c.addTrack(e,g,l,m+1)},z)}else d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==
E)11<=m?(C=[],z=ProbeTrack(c,C,e,"Affy Exon 1.0 ST Probe Sets",g+","+l),c.addTrackList(z)):setTimeout(function(){c.addTrack(e,g,l,11)},5E3);else{var C=E.documentElement.getElementsByTagName("probe");try{z=ProbeTrack(c,C,e,"Affy Exon 1.0 ST Probe Sets",g+","+l),c.addTrackList(z)}catch(H){}}});else if("helicos"===e||-1<e.indexOf("illuminaTotal")||-1<e.indexOf("illuminaSmall")||"illuminaPolyA"===e){w=c.xScale.domain()[0];A=c.xScale.domain()[1];var B=calculateBin(A-w,c.width);p=[];l&&(0<l.indexOf(",")?
p=l.split(","):p[0]=l);y="Total";var G=1;2<p.length&&(h=1*p[1],2===h&&(y="Norm",G=2));p=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/tmp/"+w+"_"+A+".count."+e+"."+y+".xml";0<B&&(w-=w%B,A+=B-A%B,p=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/tmp/"+w+"_"+A+".bincount."+B+"."+e+"."+y+".xml");d3.xml(p,function(D,E){if(D){if(0===m||3===m){var z="/"+pathPrefix;pathPrefix||(z="");tmpPanel=panel;-1<e.indexOf("-")&&(tmpPanel=e.substr(e.indexOf("-")+
1));$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:w,maxCoord:A,panel:tmpPanel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:e,folder:c.folderName,binSize:B,countType:G},dataType:"json",success:function(H){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(H,I,J){}});z=1E4}60>m?(z=1E3,0===m&&(z=2500),setTimeout(function(){c.addTrack(e,g,l,m+1)},z)):(d3.select("#Level"+c.levelNumber+
e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==E)4<=m?(z=[],"helicos"==e?C=HelicosTrack(c,z,e,g):"illuminaTotal"==e?C=IlluminaTotalTrack(c,z,e,g):"illuminaSmall"==e?C=IlluminaSmallTrack(c,z,e,g):"illuminaPolyA"==e?C=IlluminaPolyATrack(c,z,e,g):"liverilluminaTotalPlus"==e?C=LiverIlluminaTotalPlusTrack(c,z,e,g):"liverilluminaTotalMinus"==
...
z,e,g,l)),c.addTrackList(C)):setTimeout(function(){c.addTrack(e,g,l,4)},3E3);else{z=E.documentElement.getElementsByTagName("Count");var C;try{"helicos"===e?C=HelicosTrack(c,z,e,g):"illuminaTotal"===e?C=IlluminaTotalTrack(c,z,e,g):"illuminaSmall"===e?C=IlluminaSmallTrack(c,z,e,g):"illuminaPolyA"===e?C=IlluminaPolyATrack(c,z,e,g):"liverilluminaTotalPlus"===e?C=LiverIlluminaTotalPlusTrack(c,z,e,g):"liverilluminaTotalMinus"===e?C=LiverIlluminaTotalMinusTrack(c,z,e,g):"heartilluminaTotalPlus"===e?C=HeartIlluminaTotalPlusTrack(c,
z,e,g):"heartilluminaTotalMinus"===e?C=HeartIlluminaTotalMinusTrack(c,z,e,g):"brainilluminaTotalPlus"===e?C=BrainIlluminaTotalPlusTrack(c,z,e,g):"brainilluminaTotalMinus"===e?C=BrainIlluminaTotalMinusTrack(c,z,e,g):"liverilluminaSmall"===e?C=LiverIlluminaSmallTrack(c,z,e,g):"heartilluminaSmall"===e?C=HeartIlluminaSmallTrack(c,z,e,g):-1<e.indexOf("illuminaTotal")&&(C=StrainSpecificIlluminaTotalTrack(c,z,e,g,l)),c.addTrackList(C)}catch(H){}}})}else if(-1<e.indexOf("spliceJnct")){w=c.xScale.domain()[0];
A=c.xScale.domain()[1];p=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml";var F="Brain ";"liverspliceJnct"==e?F="Liver ":"heartspliceJnct"==e&&(F="Heart ");d3.xml(p,function(D,E){if(D){console.log(D);if(0==m){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:w,maxCoord:A,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:e,
folder:c.folderName},dataType:"json",success:function(H){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(H,I,J){}})}60>m?(z=1500,1===m&&(z=2500),setTimeout(function(){c.addTrack(e,g,l,m+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==E)if(11<=m){z=[];var C=SpliceJunctionTrack(c,
z,e,F+"Splice Junctions",1,"");c.addTrackList(C)}else setTimeout(function(){c.addTrack(e,g,l,11)},5E3);else{z=E.documentElement.getElementsByTagName("Feature");console.log(z);try{C=SpliceJunctionTrack(c,z,e,F+"Splice Junctions",3,""),c.addTrackList(C)}catch(H){console.log(H)}}})}else if("polyASite"==e)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/polyASite.xml",function(D,E){if(D)if(60>m){var z=1E3;1===m&&(z=2500);setTimeout(function(){c.addTrack(e,g,l,m+1)},z)}else d3.select("#Level"+
c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e);else if(null==E)if(4<=m){z=[];var C=PolyATrack(c,z,e,"Predicted PolyA Sites",l);c.addTrackList(C)}else setTimeout(function(){c.addTrack(e,g,l,4)},5E3);else{z=E.documentElement.getElementsByTagName("Feature");try{C=PolyATrack(c,z,e,"Predicted PolyA Sites",l),c.addTrackList(C)}catch(H){}}});
else if(0===e.indexOf("repeatMask"))$("#"+e+c.levelNumber+"Select").val(),w=c.xScale.domain()[0],A=c.xScale.domain()[1],p=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml",d3.xml(p,function(D,E){if(D){console.log(D);if(0===m){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:w,maxCoord:A,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,
track:e,folder:c.folderName},dataType:"json",success:function(H){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(H,I,J){}})}60>m?(z=1500,1===m&&(z=2500),setTimeout(function(){c.addTrack(e,g,l,m+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==E)if(11<=m){z=[];var C=
RepeatMaskTrack(c,z,e,"Repeat Masker",g,l);c.addTrackList(C)}else setTimeout(function(){c.addTrack(e,g,l,11)},5E3);else{z=E.documentElement.getElementsByTagName("Feature");try{C=RepeatMaskTrack(c,z,e,"Repeat Masker",g,l),c.addTrackList(C)}catch(H){console.log(H)}}});else if(0==e.indexOf("custom"))if(p=trackInfo[e],l=l+",Name="+p.Name,l="DataFile="+p.Location+","+l,"bed"==p.Type||"bb"==p.Type)y=[],p=CustomTranscriptTrack(c,y,e,p.Name,3,l),c.addTrackList(p),p.updateFullData(0,0);else{if("bg"==p.Type||
"bw"==p.Type)y=[],p=CustomCountTrack(c,y,e,3,l),c.addTrackList(p)}else 0<=e.indexOf("cirRNA")&&(p=trackInfo[e],p=CircRNATrack(c,y,e,p.Name,3,l),c.addTrackList(p));$(".sortable"+c.levelNumber).sortable("refresh");gtag("event",e,{event_category:"trackAdded"})};c.addTrackList=function(e){null!=e&&(c.trackList[c.trackCount]=e,c.trackCount++,c.trackListHash[e.trackClass]=e,setTimeout(function(){DisplayRegionReport()},300))};c.updateLinks=function(){1===c.levelNumber&&d3.select("#probeSetDetailLink1").each(function(){var e=
...
typeof c.scaleSVG&&c.scaleSVG.style("cursor",e)};c.updateTrackSelectedArea=function(e,g){c.selectionStart=e;c.selectionEnd=g;var l=c.xScale(e),m=c.xScale(g)-l;c.scaleSVG.selectAll("rect.selectedArea").remove();if((0<l||0<l+m)&&l<c.width)for(1>m&&(m=1),c.scaleSVG.append("rect").attr("class","selectedArea").attr("x",l).attr("y",15).attr("height",c.scaleSVG.attr("height")-15).attr("width",m).attr("fill","#CECECE").attr("opacity",.3).attr("pointer-events","none"),l=0;l<c.trackList.length;l++)"undefined"!==
typeof c.trackList[l]&&"undefined"!==typeof c.trackList[l].setSelectedArea&&c.trackList[l].setSelectedArea(e,g)};c.getAddMenus=function(){var e="/"+pathPrefix;""==pathPrefix&&(e="");$.ajax({url:e+"trackMenu.jsp",type:"GET",cache:!1,data:{level:c.levelNumber,organism:organism},dataType:"html",success:function(g){$("#trackMenu"+c.levelNumber).remove();d3.select("div#trackMenu").append("div").attr("id","trackMenu"+c.levelNumber);$("#trackMenu"+c.levelNumber).html(g)},error:function(g,l,m){$("#trackMenu"+
...
($("span.closeDetail").click(),$("span#"+nameSpan).click(),$("html, body").animate({scrollTop:$("#regionDiv").offset().top},1E3))});d3.select(q).select("#functLevel"+c.levelNumber).remove();c.functionBar=c.vis.append("div").attr("class","functionBar").attr("id","functLevel"+c.levelNumber).style("float","left");var e=c.functionBar.append("div").attr("class","defaultMouse").attr("id","defaultMouse"+c.levelNumber);e.append("span").attr("id","dragzoom"+c.levelNumber).style("height","24px").style("display",
...
function(){ret="";$("#ScrollLevel"+c.levelNumber+" li.draggable"+c.levelNumber).each(function(){var e=new String($(this).attr("id"));e=e.substr(2);"undefined"!==typeof c.trackListHash[e]&&(ret+=c.trackListHash[e].generateTrackSettingString())});return ret};c.setCurrentViewModified=function(){c.currentView.modified=1;$("span#viewModifiedCtl"+c.levelNumber).show()};c.clearCurrentView=function(){c.currentView.modified=0;$("span#viewModifiedCtl"+c.levelNumber).hide()};c.type=a;c.div=q;c.margin=0;c.halfWindowWidth=
...
c.width=n;c.mw=c.width-c.margin;d3.select(q).select("#viewsLevel"+f).remove();d3.select(q).select("#Level"+f).remove();c.vis=d3.select(q);c.setupFunctionBar();0==$("#viewSelect"+c.levelNumber).length&&(n=c.vis.append("div").style("float","right").style("display","inline-block").style("margin-right","5px"),a=n.append("div"),a.append("button").attr("id","viewSelect"+c.levelNumber).attr("class","viewSelect").text("Select/Edit Views").on("mouseout",function(){$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")}).on("mouseover",
...
e=l=l.substr(l.indexOf(",")+1));if("genomeSeq"===c){var p=SequenceTrack(d,c,"Reference Genomic Sequence",e);p.seqRegionSize=10;d.addTrackList(p)}else if(-1<c.indexOf("noncoding"))p=GeneTrack(d,g,c,"Non-Coding/Non-PolyA+ Transcripts",e),d.addTrackList(p);else if(-1<c.indexOf("coding"))p=GeneTrack(d,g,c,"Protein Coding/PolyA+ Transcripts",e),d.addTrackList(p);else if(0===c.indexOf("liverTotal")||"heartTotal"===c||0===c.indexOf("brainTotal")||0===c.indexOf("kidneyTotal")||"mergedTotal"===c)h="Liver Total RNA Transcripts",
"heartTotal"===c?h="Heart Total RNA Transcripts":"brainTotal"===c?h="Brain Total RNA Transcripts":"mergedTotal"===c?h="Merged (Brain,Heart,Liver,Kidney) Reconstructed":"kidneyTotal"===c&&(h="Kidney Total RNA Transcripts"),p=GeneTrack(d,g,c,h,e),d.addTrackList(p);else if(-1<c.indexOf("smallnc"))p=GeneTrack(d,g,c,"Small RNA (<200 bp) Genes",e),d.addTrackList(p);else if(0===c.indexOf("refSeq"))99===d.levelNumber&&(e+="DrawTrx,"),p=RefSeqTrack(d,g,c,"Ref Seq Genes",e),p.density=99===d.levelNumber?2:h,
d.addTrackList(p);else if(0===c.indexOf("snp"))p=SNPTrack(d,g,c,1,"4"),d.addTrackList(p);else if("qtl"===c)p=QTLTrack(d,g,c,1),d.addTrackList(p);else if("trx"===c)g=getAllChildrenByName(getFirstChildByName(d.selectedData,"TranscriptList"),"Transcript"),p=TranscriptTrack(d,g,c,h),d.addTrackList(p);else if("probe"===c||"probeMouse"===c)99!==d.levelNumber&&(e=h+","+e),p=ProbeTrack(d,g,c,"Affy Exon 1.0 ST Probe Sets",e),d.addTrackList(p);else if(-1<c.indexOf("spliceJnct"))e="Brain ","liverspliceJnct"===
c?e="Liver ":"heartspliceJnct"===c&&(e="Heart "),p=SpliceJunctionTrack(d,g,c,e+"Splice Junctions",3,""),100===d.levelNumber&&(p.density=h),d.addTrackList(p);else if("polyASite"===c)p=PolyATrack(d,g,c,"Predicted PolyA Sites",e),d.addTrackList(p);else if("helicos"===c||-1<c.indexOf("illuminaTotal")||-1<c.indexOf("illuminaSmall")||"illuminaPolyA"===c){d.updateTimeoutHandle[c]&&(d.updateTimeoutHandle[c]=-1);h=2;l=l.split(",");1<l.length&&(1===l[1]||2===l[1])&&(h=l[1]);"helicos"===c?p=HelicosTrack(d,g,
c,h):"illuminaTotal"===c?p=IlluminaTotalTrack(d,g,c,h):"illuminaSmall"===c?p=IlluminaSmallTrack(d,g,c,h):"illuminaPolyA"===c?p=IlluminaPolyATrack(d,g,c,h):"liverilluminaTotalPlus"===c?p=LiverIlluminaTotalPlusTrack(d,g,c,h):"liverilluminaTotalMinus"===c?p=LiverIlluminaTotalMinusTrack(d,g,c,h):"heartilluminaTotalPlus"===c?p=HeartIlluminaTotalPlusTrack(d,g,c,h):"heartilluminaTotalMinus"===c?p=HeartIlluminaTotalMinusTrack(d,g,c,h):"brainilluminaTotalPlus"===c?p=BrainIlluminaTotalPlusTrack(d,g,c,h):"brainilluminaTotalMinus"===
c?p=BrainIlluminaTotalMinusTrack(d,g,c,h):"liverilluminaSmall"===c?p=LiverIlluminaSmallTrack(d,g,c,h):"heartilluminaSmall"===c?p=HeartIlluminaSmallTrack(d,g,c,h):-1<c.indexOf("illuminaTotal")&&(p=StrainSpecificIlluminaTotalTrack(d,g,c,h,e));if(99===d.levelNumber){if(0!==d.updateTimeoutHandle[c]){clearTimeout(d.updateTimeoutHandle[c]);try{clearTimeout(d.timeoutTrack[c].fullDataTimeOutHandle)}catch(t){}d.updateTimeoutHandle[c]=0}d.updateTimeoutHandle[c]=setTimeout(function(){p.updateFullData(0,1);d.timeoutTrack[c]=
p;d.updateTimeoutHandle[c]=0},300)}d.addTrackList(p)}else if(0===c.indexOf("repeatMask"))p=RepeatMaskTrack(d,g,c,"Repeat Masker",1,e),d.addTrackList(p);else if(-1<c.indexOf("custom"))if(h=trackInfo[c],e="DataFile="+h.Location+","+e,"bed"===h.Type||"bb"===h.Type)g=[],p=CustomTranscriptTrack(d,g,c,h.Name,3,e),d.addTrackList(p);else{if("bg"===h.Type||"bw"===h.Type)g=[],p=CustomCountTrack(d,g,c,3,e),d.addTrackList(p)}else 0<=c.indexOf("cirRNA")&&(h=trackInfo[c],g=[],p=CircRNATrack(d,g,c,h.Name,3,e),d.addTrackList(p));
...
d.setCurrentViewModified=function(){"undefined"!==typeof d.currentView&&(d.currentView.modified=1,$("span#viewModifiedCtl"+d.levelNumber).show())};d.clearCurrentView=function(){d.currentView.modified=0;$("span#viewModifiedCtl"+d.levelNumber).hide()};d.type=a;d.div=q;d.margin=0;d.halfWindowWidth=$(window).width()/2;d.mh=400;d.downx=Math.NaN;d.downscalex;d.downPanx=Math.NaN;d.xMax=29E7;d.xMin=1;d.prevMinCoord=r;d.prevMaxCoord=u;d.dataMinCoord=r;d.dataMaxCoord=u;d.y=0;d.xScale=null;d.xAxis=null;d.vis=
null;d.level=null;d.svg=null;d.txType=null;d.txList=null;d.tt=null;d.trackList=[];d.trackCount=0;d.levelNumber=f;d.selectedData=null;d.txType=null;d.width=n;d.mw=d.width-d.margin;d.vis=d3.select(q);d.topDiv=d.vis.append("div").attr("id","Level"+f).style("text-align","left");d.xScale=d3.scaleLinear().domain([r,u]).range([0,d.width]);d.xAxis=d3.axisTop(d.xScale).ticks(3).tickSize(8).tickPadding(10);d.scaleSVG=d.topDiv.append("svg:svg").attr("width",d.width).attr("height",60).attr("class","scale");d.scaleSVG.append("g").attr("class",
...
f.svg.append("text").attr("x",f.xScale(f.stop)+10).attr("y",15).text(a)}};f.redraw=function(){f.draw()};f.setup=function(){f.svg=d3.select(q).append("svg:svg").attr("width",f.imageWidth).attr("height",60).attr("id","selectionLevel"+f.levelNumber).style("display","none");f.xScale=f.parent.xScale};f.setup();return f}
function Track(q,n,r,u){var f={};f.selectionStart=q.selectionStart;f.selectionEnd=q.selectionEnd;f.ttSVGMinWidth=20;f.panDown=function(){if(!d3.event.shiftKey&&0===f.gsvg.overSettings&&0===overSelectable&&("dragzoom"!==f.gsvg.defaultMouseFunct&&d3.event.altKey||"dragzoom"===f.gsvg.defaultMouseFunct&&!d3.event.altKey)){var b=d3.mouse(f.gsvg.vis.node());f.gsvg.downZoomx=b[0];f.svg.append("rect").attr("class","zoomRect").attr("x",b[0]).attr("y",0).attr("height",f.svg.attr("height")).attr("width",1).attr("fill",
...
l=[];if("undefined"!==typeof g&&0<g.length){for(var m=0,p=0;p<g.length;p++)if(e<=parseInt(g[p].getAttribute("start"),10)&&parseInt(g[p].getAttribute("start"),10)<=h||parseInt(g[p].getAttribute("start"),10)<=e&&parseInt(g[p].getAttribute("stop"),10)>=e)l[m]=g[p],m++;0<l.length&&d.addTrack(f.ttTrackList[c],3,"DrawTrx",l)}}};f.generateSettingsDiv=function(b){f.savePrevious();var a=trackInfo[f.trackClass];d3.select(b).select("table").select("tbody").html("");if("undefined"!==typeof a&&0<a.Controls.length&&
...
var l="",m="";2<g.length&&(l=g[2]);3<g.length&&(m=g[3]);h=h.append("select").attr("id",f.trackClass+l+f.level+m).attr("name",g[1]);for(g=0;g<c.length;g++){var p=c[g].substr(1).split(":");if(2==p.length){var t=h.append("option").attr("value",p[1]).text(p[0]);"Dense"===l&&p[1]==f.density?t.attr("selected","selected"):"Version"===l&&p[1]==f.dataVer?t.attr("selected","selected"):p[1]==e&&t.attr("selected","selected")}}d3.select("select#"+f.trackClass+l+f.level+m).on("change",function(){f.updateSettingsFromUI();
...
f.updateData(0):f.draw(f.data);else if(b<f.seqRegionMin||a>f.seqRegionMax)f.updateData(0);else{var d=a-b,c="12px";200<d&&(c="10px");if(d<=f.aaDispCutoff&&1==f.includeAA||d<=f.dispCutoff){if(f.seqRegionMin<=b&&b<=f.seqRegionMax&&f.seqRegionMin<=a&&a<=f.seqRegionMax){var h=f.gsvg.width/d,e=h/2;a=b-f.seqRegionMin;var g=d+a,l=27;1==f.includeAA&&"-"!=f.strands&&(l+=30);if(d<f.dispCutoff){d3.select("#Level"+f.gsvg.levelNumber+f.trackClass).selectAll("g.line").remove();for(var m=[],p=a;p<g;p++)m[p-a]=[],
m[p-a].base=f.data.charAt(p),m[p-a].id=p,m[p-a].pos=p-a;f.textSize(d);var t=f.yPosition(d);p=f.svg.selectAll(".base").data(m,keyID).attr("transform",function(B,G){return"translate("+(B.pos*h-e)+","+l+")"});m=p.enter().append("g").attr("class","base").attr("transform",function(B,G){return"translate("+(B.pos*h-e)+","+l+")"});m.each(function(B){d3.select(this).append("text").text(function(G){return"-"!=f.strands?G.base:f.complement(G.base)});"both"==f.strands&&d3.select(this).append("text").attr("class",
"comp").attr("y",t).text(function(G){return f.complement(G.base)})});p.exit().remove();p.selectAll("text").attr("font-size",function(B){return f.textSize(d)});p.selectAll("text.comp").attr("y",t)}else d3.select("#Level"+f.gsvg.levelNumber+f.trackClass).selectAll("g.base").remove(),f.svg.append("g").attr("class","line").append("line").attr("stroke","#000000").attr("x1",0).attr("y1",l).attr("x2",f.gsvg.width).attr("y2",l);if(1==f.includeAA){var v=3*h,y=v/2-4,w=25;if("both"==f.strands||"+"==f.strands)for(p=
b%3,m=a-p,g=[],g[0]=f.translateRegion(0,b-p,m,m+d),g[1]=f.translateRegion(1,b-p-1,m-1,m-1+d),g[2]=f.translateRegion(2,b-p-2,m-2,m-2+d),w=25,p=0;3>p;p++){var A=f.svg.selectAll(".aa"+p).data(g[p],keyID).attr("transform",function(B,G){return"translate("+(f.xScale(B.pos)-h)+","+w+")"}).each(function(B){d3.select(this).select("rect").attr("width",v);d<f.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",y).attr("font-size",c).text(function(G){return G.aa}):d3.select(this).select("text").attr("x",
y).attr("font-size",c):d3.select(this).select("text").remove()});m=A.enter().append("g").attr("class","aa"+p).attr("transform",function(B,G){return"translate("+(f.xScale(B.pos)-h)+","+w+")"});m.each(function(B){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",v).attr("stroke",f.colorAA).attr("fill",f.colorAA);d<f.dispCutoff&&d3.select(this).append("text").attr("x",y).attr("font-size",c).text(function(G){return G.aa})});A.exit().remove();w+=11}w="both"==f.strands?80:45;if("both"==
f.strands||"-"==f.strands)for(p=b%3,m=a-p,g=[],g[0]=f.translateRegion(0,b-p,m+d,m),g[1]=f.translateRegion(1,b-p-1,m-1+d,m-1),g[2]=f.translateRegion(2,b-p-2,m-2+d,m-2),p=0;3>p;p++)A=f.svg.selectAll(".aarev"+p).data(g[p],keyID).attr("transform",function(B,G){return"translate("+(f.xScale(B.pos)-h)+","+w+")"}).each(function(B){d3.select(this).select("rect").attr("width",v);d<f.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",y).attr("font-size",c).text(function(G){return G.aa}):
d3.select(this).select("text").attr("x",y).attr("font-size",c):d3.select(this).select("text").remove()}),m=A.enter().append("g").attr("class","aarev"+p).attr("transform",function(B,G){return"translate("+f.xScale(B.pos)+","+w+")"}),m.each(function(B){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",v).attr("stroke",f.colorAA).attr("fill",f.colorAA);d<f.dispCutoff&&d3.select(this).append("text").attr("x",y).attr("font-size",c).text(function(G){return G.aa})}),A.exit().remove(),
...
15).text("<--"),f.svg.append("text").attr("class","dir").attr("x",f.gsvg.width-60).attr("y",15).text("<--"));if(d<f.dispCutoff&&1<b.length){for(var l=[],m=c;m<e;m++)l[m-c]=[],l[m-c].base=b.charAt(m),l[m-c].id=m,l[m-c].pos=m-c;m=f.textSize(d);var p=f.yPosition(d),t=27;1==f.includeAA&&"-"!=f.strands&&(t+=30);l=f.svg.selectAll(".base").data(l,keyID).attr("transform",function(A,B){return"translate("+A.pos*g+","+t+")"});b=l.enter().append("g").attr("class","base").attr("transform",function(A,B){return"translate("+
A.pos*g+","+t+")"});b.each(function(A){d3.select(this).append("text").text(function(B){return"-"!=f.strands?A.base:f.complement(A.base)});"both"==f.strands&&d3.select(this).append("text").attr("class","comp").attr("y",p).text(function(B){return f.complement(A.base)})});l.exit().remove();l.selectAll("text").attr("font-size",m)}else f.svg.append("g").attr("class","base").append("line").attr("stroke","#000000").attr("x1",0).attr("y1",57).attr("x2",f.gsvg.width).attr("y2",57);if(f.data&&1==f.includeAA){var v=
3*g,y=v/2-4,w=25;if("both"==f.strands||"+"==f.strands)for(m=a%3,b=c-m,l=[],l[0]=f.translateRegion(0,a-m,b,b+d),l[1]=f.translateRegion(1,a-m-2,b-2,b-2+d),l[2]=f.translateRegion(2,a-m-1,b-1,b-1+d),w=25,m=0;3>m;m++)e=f.svg.selectAll(".aa"+m).data(l[m],keyID).attr("transform",function(A,B){return"translate("+f.xScale(A.pos)+","+w+")"}).each(function(A){d3.select(this).select("rect").attr("width",v);d<f.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",y).attr("font-size",
h).text(function(B){return A.aa}):d3.select(this).select("text").attr("x",y).attr("font-size",h):d3.select(this).select("text").remove()}),e.enter().append("g").attr("class","aa"+m).attr("transform",function(A,B){return"translate("+f.xScale(A.pos)+","+w+")"}).each(function(A){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",v).attr("stroke",f.colorAA(A)).attr("fill",f.colorAA(A));d<f.dispCutoff&&d3.select(this).append("text").attr("x",y).attr("font-size",h).text(function(){return A.aa})}),
e.exit().remove(),w+=11;w="both"==f.strands?80:45;if("both"==f.strands||"-"==f.strands)for(m=a%3,b=c-m,l=[],l[0]=f.translateRegion(0,a-m,b+d,b),l[1]=f.translateRegion(1,a-m-1,b-1+d,b-1),l[2]=f.translateRegion(2,a-m-2,b-2+d,b-2),m=0;3>m;m++)e=f.svg.selectAll(".aarev"+m).data(l[m],keyID).attr("transform",function(A,B){return"translate("+f.xScale(A.pos)+","+w+")"}),b=e.enter().append("g").attr("class","aarev"+m).attr("transform",function(A,B){return"translate("+f.xScale(A.pos)+","+w+")"}),b.each(function(A){d3.select(this).append("rect").attr("y",
...
if(0===c[1].toLowerCase().indexOf("select")){h.append("text").text(e+": ");e=c[1].split(":");c=c[2].split("}");for(var g=f.trackClass+f.level+"Select",l=h.append("select").attr("id",g).attr("name",e[1]),m=0;m<c.length;m++){var p=c[m].substr(1).split(":");2==p.length&&(h=l.append("option").attr("value",p[1]).text(p[0]),p[1]==f.strands&&h.attr("selected","selected"))}d3.select("select#"+g).on("change",function(){f.updateSettingsFromUI();f.redraw()})}else if(0===c[1].toLowerCase().indexOf("cbx"))for(h.append("text").text(e),
e=c[1].split(":"),c=c[2].split("}"),m=0;m<c.length;m++)if(p=c[m].substr(1).split(":"),2===p.length){g=h.append("div").style("display","inline-block");var t="",v="";2<e.length&&(v=e[2]);3<e.length&&(t=e[3]);l=g.append("input").attr("type","checkbox").attr("id",f.trackClass+v+"CBX"+f.level+t).attr("name",e[1]).style("margin-left","5px");g.append("text").text(p[0]);1==f.includeAA?$("#"+f.trackClass+v+"CBX"+f.level+t).prop("checked",!0):$("#"+f.trackClass+v+"CBX"+f.level+t).prop("checked",!1);d3.select("input#"+
...
function GeneTrack(q,n,r,u,f){var b=Track(q,n,r,u);b.counts=[{value:0,names:"Ensembl"},{value:0,names:"Brain RNA-Seq"}];q=new String(f);b.drawAs="Gene";b.trxCutoff=1E5;q=new String(f);b.xScale.domain();b.xScale.domain();-1<q.indexOf("DrawTrx")&&(b.drawAs="Trx");b.density=3;b.dataVer=0;b.reqDataVer=0;b.ttTrackList=[];-1===b.trackClass.indexOf("smallnc")?(b.ttTrackList.push("ensemblcoding"),b.ttTrackList.push("braincoding"),b.ttTrackList.push("brainTotal"),b.ttTrackList.push("liverTotal"),b.ttTrackList.push("heartTotal"),
...
"<BR>Strand: "+c+"<BR>"}else{var e=a.getAttribute("ID");e=b.getDisplayID(e);var g="";a.getAttribute("geneSymbol")?g=a.getAttribute("geneSymbol"):a.parent&&a.parent.getAttribute("geneSymbol")&&(g=a.parent.getAttribute("geneSymbol"));c=".";1==a.getAttribute("strand")?c="+":-1==a.getAttribute("strand")&&(c="-");if(a.parent){h=getAllChildrenByName(getFirstChildByName(a.parent,"TranscriptList"),"Transcript");for(var l=0;l<h.length;l++){var m=new String(h[l].getAttribute("ID"));e!=m&&(-1<(new String(h[l].getAttribute("ID"))).indexOf("ENS")?
d+="<B>"+m+"</B>":(d+="<B>"+m+"</B>",-1==(new String(h[l].getAttribute("ID"))).indexOf("ENS")&&(m=getFirstChildByName(getFirstChildByName(h[l],"annotationList"),"annotation"),null!=m&&(d+=" - "+m.getAttribute("reason")))),d+="<br>");if(""==g){m=getAllChildrenByName(getFirstChildByName(h[l],"annotationList"),"annotation");for(var p=0;p<m.length&&""==g;p++)"AKA"==m[p].getAttribute("source")&&(g=m[p].getAttribute("annot_value"),g=(new String(g)).split(":"),g=0!=(new String(g[g.length-1])).indexOf("ENS")?
g[g.length-1]:g[0])}}h='<BR><div id="ttSVG" style="background:#FFFFFF;overflow:"></div><BR>Transcript ID: '+e+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+c;h=h+"<BR><BR>Gene Symbol: "+g+"<BR>Gene ID: "+b.getDisplayID(a.parent.getAttribute("ID"))+"<BR><BR>Other Transcripts:<BR>"+d+"<BR>"}else{h=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(l=0;l<h.length;l++)if(m=
new String(h[l].getAttribute("ID")),-1<(new String(h[l].getAttribute("ID"))).indexOf("ENS")?d+="<B>"+m+"</B>":(d+="<B>"+m+"</B>",-1===(new String(h[l].getAttribute("ID"))).indexOf("ENS")&&(m=getFirstChildByName(getFirstChildByName(h[l],"annotationList"),"annotation"),null!=m&&(d+=" - "+m.getAttribute("reason")))),d+="<br>",0===g.length)for(m=getAllChildrenByName(getFirstChildByName(h[l],"annotationList"),"annotation"),p=0;p<m.length&&0===g.length;p++)"AKA"===m[p].getAttribute("source")&&(g=m[p].getAttribute("annot_value"),
...
return p=p<$(window).width()/2?b.xScale(c.getAttribute("stop"))-b.xScale(c.getAttribute("start"))+5:-9*m.length}).attr("dy",10).attr("id","lblTxt").text(m):h.select("text#lblTxt").attr("dx",function(){var p=b.xScale(c.getAttribute("start"));return p=p<$(window).width()/2?b.xScale(c.getAttribute("stop"))-b.xScale(c.getAttribute("start"))+5:-9*m.length})}else h.select("text#lblTxt").remove()})),1===b.density?b.svg.attr("height",30):2===b.density?b.svg.attr("height",15*(b.trackYMax+1)):3===b.density&&
...
genomeVer||"small"===c&&-1<(new String(a.getAttribute("ID"))).indexOf("ENS")){console.log("totalType");h=a.getAttribute("ID");var g="";if(a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length)h=h+" ("+a.getAttribute("geneSymbol")+")";else{e=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(var l=0;l<e.length&&""===g;l++)if(""===g)for(var m=getAllChildrenByName(getFirstChildByName(e[l],"annotationList"),"annotation"),p=0;p<m.length&&0===g.length;p++)if("AKA"===
m[p].getAttribute("source")){var t=m[p].getAttribute("annot_value");t=(new String(t)).split(":");1<=t.length?(g=t[0],h=h+" ("+g+")"):2<=t.length&&(g=t[0],t=t[1],h=h+" ("+t+")")}}e=parseInt(a.getAttribute("start"),10);m=parseInt(a.getAttribute("stop"),10);a.getAttribute("extStart")&&a.getAttribute("extStop")&&(e=parseInt(a.getAttribute("extStart"),10),m=parseInt(a.getAttribute("extStop"),10));svgViewIDList[d]||(svgViewIDList[d]=b.gsvg.currentView?b.gsvg.currentView.ViewID:defaultView);l=GenomeSVG("div#selectedImage",
b.gsvg.width,e,m,d,h,"transcript");l.xMin=e-.05*(m-e);l.xMax=m+.05*(m-e);h=l.xMin;e=l.xMax;svgList[d]=l;svgList[d].txType=c;svgList[d].selectedData=a;selectedGeneSymbol=a.getAttribute("geneSymbol");b.gsvg.selectedGeneSymbol=a.getAttribute("geneSymbol");selectedID=new String(a.getAttribute("ID"));-1===selectedID.indexOf("ENS")&&0<g.length&&(selectedID=g);$("div#selectedImage").show();console.log("here");-1<(new String(selectedID)).indexOf("ENS")&&-1<b.trackClass.indexOf("ensembl")?(console.log("geneReport"),
...
maxCoord:b.gsvg.xScale.domain()[1],panel:g,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:e,folder:b.gsvg.folderName},dataType:"json",success:function(p){gtag("event","generateTrackGene",{event_category:"browser"})},error:function(p,t,v){}})}10>a?(h=2E3,1===a&&(h=5E3),setTimeout(function(){b.updateData(a+1)},h)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+
b.gsvg.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass))}else if(c){h=c.documentElement.getElementsByTagName("Gene");e=[];g=[];for(var l=0,m=0;m<h.length;m++)h[m]&&(e[l]=h[m],g[h[m].getAttribute("ID")]=1,l++);for(m=0;m<b.data.length;m++)b.data[m]&&!g[b.data[m].getAttribute("ID")]&&(e[l]=b.data[m],l++);b.draw(e);b.hideLoading();setTimeout(function(){DisplayRegionReport()},300)}else b.hideLoading()})};b.updateDataVersion=function(a,
...
maxCoord:b.gsvg.xScale.domain()[1],panel:l,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:c,folder:b.gsvg.folderName},dataType:"json",success:function(m){gtag("event","generateTrackGene",{event_category:"browser"})},error:function(m,p,t){}})}5>d?(g=1E4,0===d&&(g=5E3),setTimeout(function(){b.updateDataVersion(a,d+1)},g)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+
...
10));var e="";1===b.gsvg.levelNumber?e="tx":99===b.gsvg.levelNumber&&(e="ttTx");e=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" #"+e+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber);exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var g=0;g<exList.length;g++){var l=parseInt(exList[g].getAttribute("start"),10),m=parseInt(exList[g].getAttribute("stop"),10);if(l<c&&c<m||m>h&&h>l){var p=0,t=0,v=0,y=0;l<c?(p=b.xScale(l),t=b.xScale(c)-b.xScale(l),v=b.xScale(c),y=b.xScale(m)-
b.xScale(c)):m>h&&(v=b.xScale(l),y=b.xScale(h)-b.xScale(l),p=b.xScale(h),t=b.xScale(m)-b.xScale(h));e.append("rect").attr("x",p).attr("y",2.5).attr("height",5).attr("width",t).attr("title",function(A){return exList[g].getAttribute("ID")}).attr("id",function(A){return"ExNC"+exList[g].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer");e.append("rect").attr("x",v).attr("height",10).attr("width",y).attr("title",function(A){return exList[g].getAttribute("ID")}).attr("id",function(A){return"Ex"+
exList[g].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}else{p=10;t=0;if(l<c&&m<c||m>h&&l>h)p=5,t=2.5;e.append("rect").attr("x",function(A){return b.xScale(exList[g].getAttribute("start"))}).attr("y",t).attr("id",function(A){return"Ex"+exList[g].getAttribute("ID")}).attr("height",p).attr("width",function(A){return b.xScale(exList[g].getAttribute("stop"))-b.xScale(exList[g].getAttribute("start"))}).attr("title",function(A){return exList[g].getAttribute("ID")}).style("fill",b.color).style("cursor",
"pointer")}if(0<g&&(e.append("line").attr("x1",function(A){return b.xScale(exList[g-1].getAttribute("stop"))}).attr("x2",function(A){return b.xScale(exList[g].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(A){return"Int"+exList[g-1].getAttribute("ID")+"_"+exList[g].getAttribute("ID")}).attr("stroke",b.color).attr("stroke-width","2"),"1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))){l=">";if("-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand"))l="<";m=
l;p=b.xScale(exList[g-1].getAttribute("stop"));t=b.xScale(exList[g].getAttribute("start"))-p;v=0;if(7>t)m="";else for(t-=7;8<t;)0==v?(m+=" ",v=1):(m+=l,v=0),t-=7;e.append("svg:text").attr("id",function(A){return"IntTxt"+exList[g-1].getAttribute("ID")+"_"+exList[g].getAttribute("ID")}).attr("dx",p+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(m)}}if(2==b.density){var w=a.getAttribute("ID");"undefined"!==typeof a.getAttribute("geneSymbol")&&
...
a.parent);e.addTrack(b.trackClass,2,"DrawTrx",g);for(g=0;g<b.ttTrackList.length;g++)if(b.trackClass!==b.ttTrackList[g]){var l=b.gsvg.getTrackData(b.ttTrackList[g]),m=[];if("undefined"!==typeof l&&0<l.length){for(var p=0,t=0;t<l.length;t++)if(c<=parseInt(l[t].getAttribute("start"),10)&&parseInt(l[t].getAttribute("start"),10)<=h||c<=parseInt(l[t].getAttribute("stop"),10)&&parseInt(l[t].getAttribute("stop"),10)<=h||parseInt(l[t].getAttribute("start"),10)<=c&&parseInt(l[t].getAttribute("stop"),10)>=c)m[p]=
l[t],p++;0<m.length&&e.addTrack(b.ttTrackList[g],3,"DrawTrx",m)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(var d=0;d<b.gsvg.width;d++)b.yMaxArr[d]=0,b.yArr[0][d]=0;b.svg.selectAll(".gene").remove();d=b.drawAs;var c=b.xScale.domain()[0];if(b.xScale.domain()[1]-c<b.trxCutoff||$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.drawAs="Trx";c="Genes";"Trx"==b.drawAs&&(c="Transcripts");var h="Protein Coding",e=" / PolyA+";-1<b.trackClass.indexOf("noncoding")?
(h="Long Non-Coding",e=" / Non-PolyA+"):-1<b.trackClass.indexOf("smallnc")?(h="Brain",-1<b.trackClass.indexOf("liver")?h="Liver":-1<b.trackClass.indexOf("heart")&&(h="Heart"),h+=" Small RNA",e=""):0===b.trackClass.indexOf("liverTotal")?(h="Liver RNA-Seq Reconstruction ",e="Total RNA"):"heartTotal"==b.trackClass?(h="Heart RNA-Seq Reconstruction ",e="Total RNA"):0===b.trackClass.indexOf("brainTotal")?(h="Whole Brain RNA-Seq Reconstruction ",e="Total RNA"):0===b.trackClass.indexOf("kidneyTotal")?(h=
"Kidney RNA-Seq Reconstruction ",e="Total RNA"):"mergedTotal"==b.trackClass&&(h="Merged (Brain,Heart,Liver,Kidney) Reconstructed",e="Total RNA");h=-1<b.trackClass.indexOf("ensembl")?"Ensembl "+h+" "+c:-1===b.trackClass.indexOf("brainTotal")&&-1===b.trackClass.indexOf("liverTotal")&&"heartTotal"!=b.trackClass&&"mergedTotal"!=b.trackClass&&-1===b.trackClass.indexOf("kidenyTotal")?"Reconstruction "+h+e+" "+c:h+e+" "+c;(0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal"))&&
0<b.trackClass.indexOf("_")&&(c=b.trackClass.substr(b.trackClass.indexOf("_")+1),"LEStm"===c&&(c="LE-Stm"),"F344Stm"===c&&(c="F344-Stm"),h=c+" Filtered (>=1.0TPM) "+h);b.updateLabel(h);b.redrawLegend();c=a;if("Trx"===b.drawAs&&-1===b.trackClass.indexOf("smallnc"))for(c=[],e=h=0;e<a.length;e++)if("undefined"!==typeof a[e])for(var g=getAllChildrenByName(getFirstChildByName(a[e],"TranscriptList"),"Transcript"),l=0;l<g.length;l++)c[h]=g[l],c[h].parent=a[e],h++;0<c.length&&("Gene"===b.drawAs||-1<b.trackClass.indexOf("smallnc")?
(b.drawnAs="Gene",b.svg.selectAll(".trx0").each(function(){d3.select(this).remove()}),a=b.svg.selectAll(".gene").data(c,key).attr("transform",function(m,p){return"translate("+b.xScale(m.getAttribute("start"))+","+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),10),p)+")"}),a.enter().append("g").attr("class","gene").attr("transform",function(m,p){return"translate("+b.xScale(m.getAttribute("start"))+","+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),
10),p)+")"}).merge(a).append("rect").attr("height",10).attr("rx",1).attr("ry",1).attr("width",function(m){return b.xScale(m.getAttribute("stop"))-b.xScale(m.getAttribute("start"))}).attr("title",function(m){return m.getAttribute("ID")}).attr("stroke",b.colorStroke).attr("stroke-width","1").attr("id",function(m){return b.cleanID(m.getAttribute("ID"))}).style("fill",b.color).style("cursor","pointer").on("click",function(m){setTimeout(function(){b.setupDetailedView(m);d3.select(this).attr("class","selected").style("fill",
"green")}.bind(this),10)}).on("dblclick",b.zoomToFeature).on("mouseover",function(m){0===b.gsvg.isToolTip&&(overSelectable=1,$("#mouseHelp").html("<B>Click</B> to see additional details. <B>Double Click</B> to zoom in on this feature."),d3.select(this).style("fill","green"),tt.transition().duration(200).style("opacity",1).style("top",function(){return b.positionTTTop(d3.event.pageY)}).style("left",function(){return b.positionTTLeft(d3.event.pageX)}),tt.html(b.createToolTip(m)),m&&(b.triggerTableFilter(m),
-1===b.trackClass.indexOf("smallnc")?"Trx"===b.drawAs?b.setupToolTipSVG(m.parent,.05):b.setupToolTipSVG(m,.05):b.setupToolTipSVG(m,.05),tt.style("top",function(){return b.positionTTTop(d3.event.pageY)})))}).on("mouseout",function(m){$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.");"selected"!==d3.select(this).attr("class")&&d3.select(this).style("fill",b.color);tt.transition().delay(500).duration(200).style("opacity",0);b.clearTableFilter(m)}),a.exit().remove(),
d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").each(function(m){if(m){var p=d3.select(this);if("-1"==m.getAttribute("strand")||"1"==m.getAttribute("strand")){var t=">";"-1"==m.getAttribute("strand")&&(t="<");var v=t,y=p.select("rect").attr("width");if(8.5>y)v="";else for(y-=8.5,v=t;8.7<y;)v+=t,y-=8.5;p.append("svg:text").attr("dx","1").attr("dy","10").attr("id","strandTxt").style("pointer-events","none").text(v)}if(2===b.density){var w=m.getAttribute("ID");m.getAttribute("geneSymbol")&&
0<m.getAttribute("geneSymbol").length&&(w=w+" ("+m.getAttribute("geneSymbol")+")");p.append("svg:text").attr("dx",function(){var A=b.xScale(m.getAttribute("start"));return A=A<$(window).width()/2?b.xScale(m.getAttribute("stop"))-b.xScale(m.getAttribute("start"))+5:-9*w.length}).attr("dy",10).attr("id","lblTxt").text(w)}else p.select("text#lblTxt").remove()}}),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").size()+
1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1))):"Trx"==b.drawAs&&(b.drawnAs="Trx",b.svg.selectAll(".gene").each(function(){d3.select(this).remove()}),b.svg.selectAll(".trx"+b.gsvg.levelNumber).remove(),a=b.svg.selectAll(".trx"+b.gsvg.levelNumber).data(c,key).attr("transform",function(m,p){return"translate("+b.xScale(m.getAttribute("start"))+","+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),10),p)+")"}),a.enter().append("g").attr("class",function(m){m=new String(m.parent.getAttribute("ID"));
return"trx"+b.gsvg.levelNumber+" gene"+m.replace(".","_")}).attr("transform",function(m,p){return"translate(0,"+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),10),p)+")"}).attr("id",function(m){var p="";1==b.gsvg.levelNumber&&(p="tx");99==b.gsvg.levelNumber&&(p="ttTx");return p+b.cleanID(m.getAttribute("ID"))+b.gsvg.levelNumber}).style("cursor","pointer").on("click",function(m){setTimeout(function(){if(0==b.gsvg.levelNumber){var p=(new String(m.parent.getAttribute("ID"))).replace(".",
"_");b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("line").style("stroke",b.color);b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("rect").style("fill",b.color);b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("text").style("opacity","0.6").style("fill",b.color);b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").each(function(){var t=new String($(this).attr("class"));t=t.replace(" selected","");$(this).attr("class",t)});b.svg.selectAll("g.gene"+
p).each(function(){var t=$(this).attr("class")+" selected";$(this).attr("class",t)});b.svg.selectAll("g.gene"+p).selectAll("line").style("stroke","green");b.svg.selectAll("g.gene"+p).selectAll("rect").style("fill","green");b.svg.selectAll("g.gene"+p).selectAll("text").style("opacity","0.3").style("fill","green");b.setupDetailedView(m.parent)}},10)}).on("dblclick",b.zoomToFeature).on("mouseover",function(m){if(0===b.gsvg.isToolTip){if(0===b.gsvg.levelNumber&&"undefined"!==typeof m.parent){var p=(new String(m.parent.getAttribute("ID"))).replace(".",
"_");b.svg.selectAll("g.gene"+p).selectAll("line").style("stroke","green");b.svg.selectAll("g.gene"+p).selectAll("rect").style("fill","green");b.svg.selectAll("g.gene"+p).selectAll("text").style("opacity","0.3").style("fill","green")}else d3.select(this).selectAll("line").style("stroke","green"),d3.select(this).selectAll("rect").style("fill","green"),d3.select(this).selectAll("text").style("opacity","0.3").style("fill","green");tt.transition().duration(200).style("opacity",1);tt.html(b.createToolTip(m)).style("left",
function(){return b.positionTTLeft(d3.event.pageX)}).style("top",function(){return b.positionTTTop(d3.event.pageY)});b.triggerTableFilter(m);if("undefined"!==typeof m){if(-1===b.trackClass.indexOf("smallnc"))b.setupToolTipSVG(m,.05);else{p=toolTipSVG("div#ttSVG",450,parseInt(m.getAttribute("start"),10)-10,parseInt(m.getAttribute("stop"),10)+10,99,b.getDisplayID(m.getAttribute("ID")),"");p.xMin=parseInt(m.getAttribute("start"),10)-20;p.xMax=parseInt(m.getAttribute("stop"),10)+20;var t=[];t[0]=m;p.addTrack(b.trackClass,
2,"",t)}tt.style("top",function(){return b.positionTTTop(d3.event.pageY)})}}}).on("mouseout",function(m){if(-1==(new String(d3.select(this).attr("class"))).indexOf("selected"))if(0==b.gsvg.levelNumber){var p=(new String(m.parent.getAttribute("ID"))).replace(".","_");b.svg.selectAll("g.gene"+p).selectAll("line").style("stroke",b.color);b.svg.selectAll("g.gene"+p).selectAll("rect").style("fill",b.color);b.svg.selectAll("g.gene"+p).selectAll("text").style("opacity","0.6").style("fill",b.color)}else d3.select(this).selectAll("line").style("stroke",
b.color),d3.select(this).selectAll("rect").style("fill",b.color),d3.select(this).selectAll("text").style("opacity","0.6").style("fill",b.color);tt.transition().delay(500).duration(200).style("opacity",0);b.clearTableFilter(m)}).merge(a).each(b.drawTrx),a.exit().remove(),a,1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.trx"+b.gsvg.levelNumber).size()+1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1))));
""!=selectGene&&b.setSelected(selectGene);b.redrawSelectedArea();b.drawAs=d};b.redrawLegend=function(){var a=[],d=0;"Trx"===b.drawAs&&("braincoding"===b.trackClass?"Rn"===organism&&(a[d]={color:"#7EB5D6",label:"All"},d++,a[d]={color:"#5E95FF",label:"BN-Lx"},d++,a[d]={color:"#BE95B6",label:"SHR"},d++):"brainnoncoding"===b.trackClass?"Rn"===organism&&(a[d]={color:"#CECFCE",label:"All"},d++,a[d]={color:"#3E75FF",label:"BN-Lx"},d++,a[d]={color:"#FE7596",label:"SHR"},d++):"liverTotal"===b.trackClass?(a[d]=
{color:"#bbbedd",label:"All"},d++,a[d]={color:"#9b9eFF",label:"BN-Lx"},d++,a[d]={color:"#Fb9eBd",label:"SHR"},d++):"heartTotal"===b.trackClass?(a[d]={color:"#DC7252",label:"All"},d++,a[d]={color:"#BC5292",label:"BN-Lx"},d++,a[d]={color:"#FF5232",label:"SHR"},d++):"mergedTotal"!==b.trackClass&&"brainTotal"===b.trackClass&&(a[d]={color:"#7EB5D6",label:"All"},d++,a[d]={color:"#5E95FF",label:"BN-Lx"},d++,a[d]={color:"#BE95B6",label:"SHR"},d++));"brainsmallnc"===b.trackClass?(a[d]={color:"#3E7596",label:"All"},
d++,a[d]={color:"#1E55D6",label:"BN-Lx"},d++,a[d]={color:"#7E5576",label:"SHR"}):"heartsmallnc"===b.trackClass?(a[d]={color:"#9C3212",label:"All"},d++,a[d]={color:"#7C1252",label:"BN-Lx"},d++,a[d]={color:"#DC1200",label:"SHR"}):"liversmallnc"===b.trackClass&&(a[d]={color:"#7b7e9d",label:"All"},d++,a[d]={color:"#5b5eDd",label:"BN-Lx"},d++,a[d]={color:"#bb5e7d",label:"SHR"});b.drawLegend(a)};b.triggerTableFilter=function(a){var d=jQuery.Event("keyup");d.which=32;var c="";"undefined"!==typeof a.getAttribute&&
"undefined"!==typeof a.getAttribute("ID")&&(c=b.getDisplayID(a.getAttribute("ID")));-1==b.trackClass.indexOf("smallnc")?$("#tblGenes"+b.trackClass+"_filter input").val(c).trigger(d):$("#tblsmGenes_filter input").val(c).trigger(d)};b.clearTableFilter=function(a){a=jQuery.Event("keyup");a.which=32;"smallnc"!=b.trackClass?$("#tblGenes"+b.trackClass+"_filter input").val("").trigger(a):$("#tblsmGenes_filter input").val("").trigger(a)};b.updateSettingsFromUI=function(){0<$("#"+b.trackClass+"Dense"+b.level+
"Select").length&&(b.density=$("#"+b.trackClass+"Dense"+b.level+"Select").val(),"string"===typeof b.density&&(b.density=parseInt(b.density)));0<$("#"+b.trackClass+"Version"+b.level+"Select").length&&(b.reqDataVer=$("#"+b.trackClass+"Version"+b.level+"Select").val())};b.savePrevious=function(){b.prevSetting={};b.prevSetting.density=b.density;b.prevSetting.dataVer=b.dataVer};b.revertPrevious=function(){b.density=b.prevSetting.density;b.dataVer=b.prevSetting.dataVer};b.updateSettings=function(a){};b.generateTrackSettingString=
function(){return b.trackClass+","+b.density+","+b.dataVer+";"};b.redrawLegend();b.draw(n);return b}
function RefSeqTrack(q,n,r,u,f){var b=GeneTrack(q,n,r,u);b.counts=[{value:0,names:"Ensembl"},{value:0,names:"RNA-Seq"}];b.drawAs="Gene";b.trxCutoff=1E5;b.density=3;-1<(new String(f)).indexOf("DrawTrx")&&(b.drawAs="Trx");b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(q=0;q<b.gsvg.width;q++)b.yMaxArr[q]=0,b.yArr[0][q]=0;b.ttTrackList=[];b.ttTrackList.push("ensemblcoding");b.ttTrackList.push("braincoding");b.ttTrackList.push("liverTotal");b.ttTrackList.push("heartTotal");b.ttTrackList.push("mergedTotal");b.ttTrackList.push("ensemblnoncoding");
...
typeof b.ttSVGMinWidth&&e-g<b.ttSVGMinWidth&&(g=c-b.ttSVGMinWidth/2,e=h+b.ttSVGMinWidth/2);c=toolTipSVG("div#ttSVG",450,g,e,99,b.getSVGID(a),"transcript");c.forLevel=b.gsvg.levelNumber;h=[];h.push(a);c.addTrack(b.trackClass,3,"DrawTrx",h);for(h=0;h<b.ttTrackList.length;h++){var l=b.gsvg.getTrackData(b.ttTrackList[h]),m=[];if("undefined"!==typeof l&&0<l.length){for(var p=0,t=0;t<l.length;t++)if(g<=parseInt(l[t].getAttribute("start"),10)&&parseInt(l[t].getAttribute("start"),10)<=e||parseInt(l[t].getAttribute("start"),
10)<=g&&parseInt(l[t].getAttribute("stop"),10)>=g)m[p]=l[t],p++;0<m.length&&c.addTrack(b.ttTrackList[h],3,"DrawTrx",m)}}};b.calcY=function(a,d,c){if(3==b.density)if(a>=b.xScale.domain()[0]&&a<=b.xScale.domain()[1]||d>=b.xScale.domain()[0]&&d<=b.xScale.domain()[1]||a<=b.xScale.domain()[0]&&d>=b.xScale.domain()[1]){a=Math.floor(b.xScale(a));0>a&&(a=0);d=Math.floor(b.xScale(d))+1;d>=b.gsvg.width&&(d=b.gsvg.width-1);c=a-5;0>c&&(c=0);var h=d+5;h>=b.gsvg.width&&(h=b.gsvg.width-1);for(var e=0,g=c;g<=h;g++)b.yMaxArr[g]>
e&&(e=b.yMaxArr[g]);e++;var l=e;if(b.yArr.length<=e)for(b.yArr[l]=[],g=0;g<b.gsvg.width;g++)b.yArr[l][g]=0;g=e-12;1>g&&(g=1);l=0;for(var m=g;m<e&&0==l;m++){var p=0;for(g=c;g<=h&&0==p;g++)b.yArr[m][g]>p&&(p=1);0==p&&(e=m,l=1)}e>b.trackYMax&&(b.trackYMax=e);for(g=a;g<=d;g++)b.yMaxArr[g]<e&&(b.yMaxArr[g]=e),b.yArr[e][g]=1;a=15*e}else a=15;else a=2==b.density?15*(c+1):15;b.trackYMax<a/15&&(b.trackYMax=a/15);return a};b.redraw=function(){var a=b.xScale.domain()[0];a=b.xScale.domain()[1]-a;var d=0;b.yMaxArr=
...
1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1));else if(1==d||"Trx"==b.drawAs)b.svg.selectAll("g.trx"+b.gsvg.levelNumber).attr("transform",function(h,e){return"translate(0,"+b.calcY(parseInt(h.getAttribute("start"),10),parseInt(h.getAttribute("stop"),10),e)+")"}).each(function(h,e){var g=parseInt(h.getAttribute("cdsStart"),10),l=parseInt(h.getAttribute("cdsStop"),10);exList=getAllChildrenByName(getFirstChildByName(h,"exonList"),"exon");var m="";1==b.gsvg.levelNumber&&(m="tx");for(var p=
0;p<exList.length;p++){var t=parseInt(exList[p].getAttribute("start"),10),v=parseInt(exList[p].getAttribute("stop"),10);if(t<g&&g<v||v>l&&l>t){var y=t,w=g;v>l?(y=l,w=v,v=l):t=g;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(h.getAttribute("ID"))+" rect#ExNC"+exList[p].getAttribute("ID")).attr("x",function(z){return b.xScale(y)}).attr("width",function(z){return b.xScale(w)-b.xScale(y)})}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(h.getAttribute("ID"))+
" rect#Ex"+exList[p].getAttribute("ID")).attr("x",function(z){return b.xScale(t)}).attr("width",function(z){return b.xScale(v)-b.xScale(t)});if(0<p){var A=b.xScale(exList[p-1].getAttribute("stop")),B=b.xScale(exList[p].getAttribute("start"));if("1"==h.getAttribute("strand")||"-1"==h.getAttribute("strand")||"-"==h.getAttribute("strand")||"+"==h.getAttribute("strand")){var G=">";"-1"==h.getAttribute("strand")&&(G="<");var F=G,D=B-A,E=0;if(7>D)F="";else for(D-=7;8<D;)0==E?(F+=" ",E=1):(F+=G,E=0),D-=
7;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(h.getAttribute("ID"))+" #IntTxt"+exList[p-1].getAttribute("ID")+"_"+exList[p].getAttribute("ID")).attr("dx",A+1).text(F)}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(h.getAttribute("ID"))+" line#Int"+exList[p-1].getAttribute("ID")+"_"+exList[p].getAttribute("ID")).attr("x1",A).attr("x2",B)}}}),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+
...
e.documentElement.getElementsByTagName("Gene");for(var l=[],m=[],p=0,t=0;t<g.length;t++)g[t]&&(l[p]=g[t],l[p].setAttribute("ID",p),m[g[t].getAttribute("geneSymbol")]=1,p++);for(t=0;t<b.data.length;t++)b.data[t]&&!m[b.data[t].getAttribute("geneSymbol")]&&(l[p]=b.data[t],l[p].setAttribute("ID",p),p++);b.draw(l);b.hideLoading();b.getDisplayedData();setTimeout(function(){DisplayRegionReport()},300)}else b.updateData(a+1),b.hideLoading()})};b.drawTrx=function(a,d){var c=parseInt(a.getAttribute("cdsStart"),
10),h=parseInt(a.getAttribute("cdsStop"),10),e="";1==b.gsvg.levelNumber&&(e="tx");e=b.svg.select("#"+e+b.cleanID(a.getAttribute("ID")));exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var g=0;g<exList.length;g++){var l=parseInt(exList[g].getAttribute("start"),10),m=parseInt(exList[g].getAttribute("stop"),10);if(l<c&&c<m||m>h&&h>l){var p=0,t=0,v=0,y=0;l<c?(p=b.xScale(l),t=b.xScale(c)-b.xScale(l),v=b.xScale(c),y=b.xScale(m)-b.xScale(c)):m>h&&(v=b.xScale(l),y=b.xScale(h)-b.xScale(l),
p=b.xScale(h),t=b.xScale(m)-b.xScale(h));e.append("rect").attr("x",p).attr("y",2.5).attr("height",5).attr("width",t).attr("title",function(w){return exList[g].getAttribute("ID")}).attr("id",function(w){return"ExNC"+exList[g].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer");e.append("rect").attr("x",v).attr("height",10).attr("width",y).attr("title",function(w){return exList[g].getAttribute("ID")}).attr("id",function(w){return"Ex"+exList[g].getAttribute("ID")}).style("fill",b.color).style("cursor",
"pointer")}else{p=10;t=0;if(l<c&&m<c||m>h&&l>h)p=5,t=2.5;e.append("rect").attr("x",function(w){return b.xScale(exList[g].getAttribute("start"))}).attr("y",t).attr("height",p).attr("width",function(w){return b.xScale(exList[g].getAttribute("stop"))-b.xScale(exList[g].getAttribute("start"))}).attr("title",function(w){return exList[g].getAttribute("ID")}).attr("id",function(w){return"Ex"+exList[g].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}if(0<g&&(e.append("line").attr("x1",
...
"<";m=l;p=b.xScale(exList[g-1].getAttribute("stop"));t=b.xScale(exList[g].getAttribute("start"))-p;v=0;if(7>t)m="";else for(t-=7;8<t;)0==v?(m+=" ",v=1):(m+=l,v=0),t-=7;e.append("svg:text").attr("id",function(w){return"IntTxt"+exList[g-1].getAttribute("ID")+"_"+exList[g].getAttribute("ID")}).attr("dx",p+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(m)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yArr=[];b.yArr[0]=[];
for(var d=0;d<b.gsvg.width;d++)b.yMaxArr[d]=0,b.yArr[0][d]=0;b.svg.selectAll(".gene").remove();b.svg.selectAll(".trx"+b.gsvg.levelNumber).remove();var c=b.drawAs;d=b.xScale.domain()[0];if(b.xScale.domain()[1]-d<b.trxCutoff||$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.drawAs="Trx";if(0!=a&&"Gene"===b.drawAs)b.drawnAs="Gene",b.label="Ref Seq Genes",b.updateLabel(b.label),b.redrawLegend(),a=b.svg.selectAll(".gene").data(a,key).attr("transform",function(m,p){return"translate("+b.xScale(m.getAttribute("start"))+
","+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),10),p)+")"}),a.enter().append("g").attr("class","gene").attr("transform",function(m,p){return"translate("+b.xScale(m.getAttribute("start"))+","+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),10),p)+")"}).append("rect").attr("height",10).attr("rx",1).attr("ry",1).attr("width",function(m){return b.xScale(m.getAttribute("stop"))-b.xScale(m.getAttribute("start"))}).attr("title",function(m){return m.getAttribute("ID")}).attr("stroke",
...
b.gsvg.levelNumber).data(h,key).attr("transform",function(m,p){return"translate("+b.xScale(m.getAttribute("start"))+","+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),10),p)+")"});a.enter().append("g").attr("class","trx"+b.gsvg.levelNumber).attr("transform",function(m,p){return"translate(0,"+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),10),p)+")"}).attr("id",function(m){var p="";1==b.gsvg.levelNumber&&(p="tx");return p+b.cleanID(m.getAttribute("ID"))}).on("mouseover",
...
Math.round(b.xScale(a));0>a&&(a=0);d=Math.round(b.xScale(d));d>=b.gsvg.width&&(d=b.gsvg.width-1);c=a-b.xPadding;0>c&&(c=0);h=d+b.xPadding;h>=b.gsvg.width&&(h=b.gsvg.width-1);h+e<b.gsvg.width?(h+=e,d+=e):0<c-e&&(c-=e,a-=e);e=0;for(var g=c;g<=h;g++)b.yMaxArr[g]>e&&(e=b.yMaxArr[g]);e++;var l=e;if(b.yArr.length<=e)for(b.yArr[l]=[],g=0;g<b.gsvg.width;g++)b.yArr[l][g]=0;g=e-b.scanBackYLines;1>g&&(g=1);l=0;for(var m=g;m<e&&0==l;m++){var p=0;for(g=c;g<=h&&0==p;g++)b.yArr[m][g]>p&&(p=1);0==p&&(e=m,l=1)}e>
...
">";"-1"==e.getAttribute("strand")&&(m="<");var p="";if(7.5<=l&&15>=l)p=m;else if(15<l)for(l-=7.5;7.5<l;)p+=m,l-=7.5;g.select("text#strand").text(p);if(2==b.density||3==b.density){var t=e.getAttribute("ID");0===g.select("text#lblTxt").size()?g.append("svg:text").attr("dx",function(){var v=b.xScale(e.getAttribute("stop"))+8*t.length+5,y=b.xScale(e.getAttribute("stop"))-b.xScale(e.getAttribute("start"))+5;v>b.gsvg.width&&(y=-8*t.length);return y}).attr("dy",10).attr("id","lblTxt").text(t):g.select("text#lblTxt").attr("dx",
...
return g}).attr("fill",function(e){return b.color(e,"")});b.svg.selectAll("g.probe").each(function(e){var g=d3.select(this),l=">";"-1"==e.getAttribute("strand")&&(l="<");var m="",p=g.select("rect").attr("width");if(7.5<=p&&15>=p)m=l;else if(15<p)for(p-=7.5;7.5<p;)m+=l,p-=7.5;g.select("text").text(m);if(2==b.density||3==b.density){var t=e.getAttribute("ID");0===g.select("text#lblTxt").size()?g.append("svg:text").attr("dx",function(){var v=b.xScale(e.getAttribute("stop"))+8*t.length+5,y=b.xScale(e.getAttribute("stop"))-
...
0))}),b.svg.selectAll("g.probe."+e).each(function(g){var l=d3.select(this),m=">";"-1"==g.getAttribute("strand")&&(m="<");var p=m,t=l.select("rect").attr("width");if(7.5>t)p="";else for(t-=7.5;8.5<t;)p+=m,t-=7.5;l.append("svg:text").attr("dx","1").attr("id","strand").attr("dy","10").style("pointer-events","none").style("fill","white").text(p);if(2==b.density||3==b.density){var v=g.getAttribute("ID");l.append("svg:text").attr("dx",function(){var y=b.xScale(g.getAttribute("stop"))+8*v.length+5,w=b.xScale(g.getAttribute("stop"))-
...
h=h[2].split("}"),p=0;p<h.length;p++)t=h[p].substr(1).split(":"),2==t.length&&(l=e.append("div").style("display","inline-block"),m=l.append("input").attr("type","checkbox").attr("id",t[1]+"CBX"+b.level).attr("name",g[1]).style("margin-left","5px"),l.append("text").text(t[0]),d3.select("input#"+t[1]+"CBX"+b.level).on("change",function(){b.updateSettingsFromUI();b.redraw()}))}"dabg"==b.curColor||"herit"==b.curColor?$("div#affyTissues"+b.level).show():$("div#affyTissues"+b.level).hide();for(c=0;c<b.tissues.length;c++)$("#"+
...
g.append("text").text(l+": ");3<e.length&&0==e[3].indexOf("Default=")&&e[3].substr(8);if(0==e[1].toLowerCase().indexOf("select")){var m=e[1].split(":");e=e[2].split("}");var p=l="";2<m.length&&(l=m[2]);3<m.length&&(p=m[3]);g=g.append("select").attr("id",b.trackClass+l+b.level+p).attr("name",m[1]);for(m=0;m<e.length;m++){var t=e[m].substr(1).split(":");if(2==t.length){var v=g.append("option").attr("value",t[1]).text(t[0]);("Dense"==l&&t[1]==b.density||""==l&&t[1]==b.include)&&v.attr("selected","selected")}}d3.select("#"+
b.trackClass+l+b.level+p).on("change",function(){b.updateSettingsFromUI();b.updateData()})}else console.log("Undefined track settings:  "+h[c])}h=d.append("tr").append("td");h.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");b.gsvg.removeTrack(b.trackClass);var y=svgList[b.gsvg.levelNumber].currentView.ViewID,w=viewMenu[b.gsvg.levelNumber].findTrackByClass(b.trackClass,
...
e.length*l<h?d3.select(this).append("text").attr("y",m).attr("x",function(p){p=0;7<h&&(p=h/2-e.length/2*l);return p}).attr("font-size",g).attr("stroke",d3.rgb("#FFFFFF")).attr("class","snpLbl").text(e):d3.select(this).selectAll(".snpLbl").attr("x",function(p){p=0;7<h&&(p=h/2-e.length/2*l);return p})}):b.svg.selectAll("text.snpLbl").remove();1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(b.trackYMax+1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1));b.redrawSelectedArea()};
...
"#Level"+b.gsvg.levelNumber+b.trackClass));else{e=h.documentElement.getElementsByTagName("Snp");for(var g=[],l=[],m=0,p=0;p<e.length;p++)"undefined"!==typeof e[p]&&(g[m]=e[p],l[keySNP(e[p])]=1,m++);for(p=0;p<b.data.length;p++)"undefined"!==typeof b.data[p]&&"undefined"===typeof l[keySNP(b.data[p])]&&(g[m]=b.data[p],m++);b.draw(g);b.hideLoading()}})};b.getDisplayedData=function(){var d=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".snp");b.counts=[{value:0,perc:0,names:"SNP"},{value:0,
...
function TranscriptTrack(q,n,r,u){var f=Track(q,n,r,"Selected Gene Transcripts");f.createToolTip=function(b){var a=".";1==b.getAttribute("strand")?a="+":-1==b.getAttribute("strand")&&(a="-");a="ID: "+b.getAttribute("ID")+"<BR>Location: chr"+b.getAttribute("chromosome")+":"+numberWithCommas(b.getAttribute("start"))+"-"+numberWithCommas(b.getAttribute("stop"))+"<BR>Strand: "+a;-1==(new String(b.getAttribute("ID"))).indexOf("ENS")&&(b=getFirstChildByName(getFirstChildByName(b,"annotationList"),"annotation"),
...
"exonList"),"exon");for(var e=0;e<exList.length;e++){var g=parseInt(exList[e].getAttribute("start"),10),l=parseInt(exList[e].getAttribute("stop"),10);if(g<d&&d<l||l>c&&c>g){var m=0,p=0,t=0,v=0;g<d?(m=f.xScale(exList[e].getAttribute("start")),p=f.xScale(d)-f.xScale(exList[e].getAttribute("start")),t=f.xScale(d),v=f.xScale(exList[e].getAttribute("stop"))-f.xScale(d)):l>c&&(t=f.xScale(exList[e].getAttribute("start")),v=f.xScale(c)-f.xScale(exList[e].getAttribute("start")),m=f.xScale(c),p=f.xScale(exList[e].getAttribute("stop"))-
f.xScale(c));h.append("rect").attr("x",m).attr("y",2.5).attr("height",5).attr("width",p).attr("title",function(y){return exList[e].getAttribute("ID")}).attr("id",function(y){return"ExNC"+exList[e].getAttribute("ID")}).style("fill",f.color).style("cursor","pointer");h.append("rect").attr("x",t).attr("height",10).attr("width",v).attr("title",function(y){return exList[e].getAttribute("ID")}).attr("id",function(y){return"Ex"+exList[e].getAttribute("ID")}).style("fill",f.color).style("cursor","pointer")}else{m=
10;p=0;if(g<d&&l<d||l>c&&g>c)m=5,p=2.5;h.append("rect").attr("x",function(y){return f.xScale(exList[e].getAttribute("start"))}).attr("y",p).attr("height",m).attr("width",function(y){return f.xScale(exList[e].getAttribute("stop"))-f.xScale(exList[e].getAttribute("start"))}).attr("title",function(y){return exList[e].getAttribute("ID")}).attr("id",function(y){return"Ex"+exList[e].getAttribute("ID")}).style("fill",f.color).style("cursor","pointer")}if(0<e){h.append("line").attr("x1",function(y){return f.xScale(exList[e-
1].getAttribute("stop"))}).attr("x2",function(y){return f.xScale(exList[e].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(y){return"Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")}).attr("stroke",f.color).attr("stroke-width","2");g=">";"-1"==b.getAttribute("strand")&&(g="<");l=g;m=f.xScale(exList[e-1].getAttribute("stop"));p=f.xScale(exList[e].getAttribute("start"))-m;t=0;if(7>p)l="";else for(p-=7;8<p;)0==t?(l+=" ",t=1):(l+=g,t=0),p-=7;h.append("svg:text").attr("id",
...
function(B){return f.xScale(l)}).attr("width",function(B){return f.xScale(m)-f.xScale(l)})}d3.select("#Level"+f.gsvg.levelNumber+f.trackClass+" g#tx"+b.getAttribute("ID")+" rect#Ex"+exList[h].getAttribute("ID")).attr("x",function(B){return f.xScale(e)}).attr("width",function(B){return f.xScale(g)-f.xScale(e)});if(0<h){var p=">";"-1"==b.getAttribute("strand")&&(p="<");var t=p,v=f.xScale(exList[h-1].getAttribute("stop")),y=f.xScale(exList[h].getAttribute("start")),w=y-v,A=0;if(7>w)t="";else for(w-=
7;8<w;)0==A?(t+=" ",A=1):(t+=p,A=0),w-=7;d3.select("#Level"+f.gsvg.levelNumber+f.trackClass+" g#tx"+b.getAttribute("ID")+" line#Int"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")).attr("x1",v).attr("x2",y);d3.select("#Level"+f.gsvg.levelNumber+f.trackClass+" g#tx"+b.getAttribute("ID")+" #IntTxt"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")).attr("dx",v+1).text(t)}}});f.redrawSelectedArea()};f.update=function(){var b=getAllChildrenByName(getFirstChildByName(f.gsvg.selectedData,
...
f.gsvg.levelNumber+f.trackClass).attr("height",15*(1+n.length));d3.select("#Level"+f.gsvg.levelNumber+f.trackClass).selectAll(".trx"+f.gsvg.levelNumber).remove();q=d3.select("#Level"+f.gsvg.levelNumber+f.trackClass).selectAll(".trx"+f.gsvg.levelNumber).data(n,key).attr("transform",function(b,a){return"translate("+f.xScale(b.getAttribute("start"))+","+f.calcY(b,a)+")"});q.enter().append("g").attr("class","trx"+f.gsvg.levelNumber).attr("transform",function(b,a){return"translate(0,"+f.calcY(b,a)+")"}).attr("id",
...
function(b){d3.select(this).selectAll("line").style("stroke",f.color);d3.select(this).selectAll("rect").style("fill",f.color);d3.select(this).selectAll("text").style("opacity","0.6").style("fill",f.color);tt.transition().delay(500).duration(200).style("opacity",0)}).each(f.drawTrx);q.exit().remove();f.redrawLegend();f.scaleSVG.transition().duration(300).style("opacity",1);f.svg.transition().duration(300).style("opacity",1);f.redraw();return f}
function HelicosTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#DD0000";q.updateLabel("Brain Helicos RNA Read Counts");q.redrawLegend();q.redraw();return q}function IlluminaSmallTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#99CC99";q.updateLabel("Brain Illumina Small RNA Read Counts");q.redrawLegend();q.redraw();q.redraw();return q}
function IlluminaTotalTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.updateLabel("BNLx/SHR Brain Illumina Total-RNA(rRNA depleted) Read Counts");q.redrawLegend();return q}function LiverIlluminaTotalPlusTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#abaecd";q.updateLabel("BNLx/SHR Liver + Strand Total-RNA Read Counts");q.redrawLegend();q.redraw();return q}
function LiverIlluminaTotalMinusTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#dbdefd";q.updateLabel("BNLx/SHR Liver - Strand Total-RNA Read Counts");q.redrawLegend();q.redraw();return q}function LiverIlluminaSmallTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#7b7e9d";q.updateLabel("Liver  Illumina Small RNA Read Counts");q.redrawLegend();q.redraw();return q}
function HeartIlluminaTotalPlusTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#CC6242";q.updateLabel("BNLx/SHR Heart + Strand Total-RNA Read Counts");q.redrawLegend();q.redraw();return q}function HeartIlluminaTotalMinusTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#FC9272";q.updateLabel("BNLx/SHR Heart - Strand Total-RNA Read Counts");q.redrawLegend();q.redraw();return q}
function HeartIlluminaSmallTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#9C3212";q.updateLabel("Heart Illumina Small RNA Read Counts");q.redrawLegend();q.redraw();return q}function BrainIlluminaTotalPlusTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#7EB5D6";q.updateLabel("Brain + Strand Total-RNA Read Counts");q.redrawLegend();q.redraw();return q}
function BrainIlluminaTotalMinusTrack(q,n,r,u){q=CountTrack(q,n,r,u);q.updateLabel("Brain - Strand Total-RNA Read Counts");q.redrawLegend();q.redraw();return q}
function StrainSpecificIlluminaTotalTrack(q,n,r,u,f){n=CountTrack(q,n,r,u,f);u=r.substr(r.indexOf("-")+1);f=".";0<r.indexOf("Plus")?f="+":0<r.indexOf("Minus")&&(f="-");r=r.substr(0,r.indexOf("illumina"));"brain"===r?r="Whole Brain":"liver"===r&&(r="Liver");n.graphColorText=q.strainSpecificCountColors(u);q="";2===n.countType&&(q=" Sampled");n.updateLabel(u+" "+r+" "+f+" Strand Total-RNA"+q+" Read Counts");n.redrawLegend();n.redraw();return n}
function IlluminaPolyATrack(q,n,r,u){q=CountTrack(q,n,r,u);q.graphColorText="#7EB5D6";q.updateLabel("Brain Illumina PolyA+ RNA Read Counts");q.redrawLegend();q.redraw();return q}
function SpliceJunctionTrack(q,n,r,u,f,b){q=GenericTranscriptTrack(q,n,r,u,f,b);q.dataFileName=r+".xml";q.xmlTag="Feature";q.xmlTagBlockElem="block";q.idPrefix="splcJnct";"liverspliceJnct"===r?q.idPrefix="lvrsplcJnct":"heartspliceJnct"===r&&(q.idPrefix="hrtsplcJnct");q.ttSVG=1;q.ttTrackList=[];q.ttTrackList.push(r);q.ttTrackList.push("ensemblcoding");q.ttTrackList.push("braincoding");q.ttTrackList.push("brainTotal");q.ttTrackList.push("liverTotal");q.ttTrackList.push("heartTotal");q.ttTrackList.push("kidneyTotal");
q.ttTrackList.push("mergedTotal");q.ttTrackList.push("refSeq");q.ttTrackList.push("repeatMask");"splcJnct"===r?q.ttTrackList.push("illuminaPolyA"):"liverspliceJnct"===r?(q.ttTrackList.push("liverilluminaTotalPlus"),q.ttTrackList.push("liverilluminaTotalMinus")):"heartspliceJnct"===r?(q.ttTrackList.push("heartilluminaTotalPlus"),q.ttTrackList.push("heartilluminaTotalMinus")):"brainspliceJnct"===r&&(q.ttTrackList.push("brainilluminaTotalPlus"),q.ttTrackList.push("brainilluminaTotalMinus"));q.colorValueField=
"readCount";q.colorScale=d3.scaleLinear().domain([1,1E3]).range(["#E6E6E6","#000000"]);q.legendLbl="Read Depth";q.density=3;q.getDisplayID=function(a){return a.getAttribute("name")};q.createToolTip=function(a){var d='<BR><div id="ttSVG" style="background:#FFFFFF;"></div><BR>ID: '+a.getAttribute("name");d=d+"<BR>Read Counts="+numberWithCommas(a.getAttribute("readCount"));d=d+"<BR>Splice Junction in "+a.getAttribute("sampleCount")+" of ";"Rn"===organism?d+="6 samples(3 BN-Lx, 3 SHR)":"Mm"===organism&&
...
" ("+(parseInt(c[e].getAttribute("stop"),10)-parseInt(c[e].getAttribute("start"),10))+" bp)";return d=d+"<BR>Intron: "+numberWithCommas(parseInt(c[1].getAttribute("start"),10)-parseInt(c[0].getAttribute("stop"),10))+" bp<BR>"};q.drawScaleLegend("1","1000+","Read Counts","#E6E6E6","#000000",0);q.draw(n);return q}
function CustomCountTrack(q,n,r,u,f){var b=CountTrack(q,n,r,u,f);b.graphColorText="#4E85A6";b.updateControl=0;b.updateFullData=function(a,d){if(b.updateControl==a){b.updateControl=a+1;var c=b.xScale.domain()[0],h=b.xScale.domain()[1],e=h-c;b.showLoading();b.bin=b.calculateBin(e);var g=dataPrefix+"tmpData/trackXML/"+b.gsvg.folderName+"/tmp/"+c+"_"+h+".count."+b.trackClass+".xml",l=dataPrefix+"tmpData/trackUpload/"+b.trackClass.substr(6),m="bg",p="";0<b.bin&&(c-=2*b.bin,c-=c%(2*b.bin),h+=2*b.bin,h+=
2*b.bin-h%(2*b.bin),g=dataPrefix+"tmpData/trackXML/"+b.gsvg.folderName+"/"+c+"_"+h+".bincount."+b.bin+"."+b.trackClass+".xml");-1<b.dataFileName.indexOf("http")&&(p=l=b.dataFileName,m="bw");d3.xml(g,function(t,v){if(t){if(0==a||1==d){var y="/"+pathPrefix;pathPrefix||(y="");$.ajax({url:y+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:c,maxCoord:h,myOrganism:organism,genomeVer:genomeVer,track:b.trackClass,bedFile:l,outFile:g,folder:b.gsvg.folderName,binSize:b.bin,
type:m,web:p},dataType:"json",success:function(w){gtag("event","generateTrackCount",{event_category:"browser"})},error:function(w,A,B){}})}8>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1)},500):30>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1)},1E3):32>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1)},1E4):(d3.select("#Level"+b.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+
...
function CountTrack(q,n,r,u,f){var b=Track(q,n,r,"Generic Counts");b.loadedDataMin=b.xScale.domain()[0];b.loadedDataMax=b.xScale.domain()[1];b.dataFileName=b.trackClass;b.scaleMin=1;b.scaleMax=5E3;b.graphColorText="steelblue";b.colorScale=d3.scaleLinear().domain([b.scaleMin,b.scaleMax]).range(["#EEEEEE","#000000"]);b.ttSVG=1;b.data=n;b.density=u;b.prevDensity=u;b.countType=1;b.displayBreakDown=null;q=b.gsvg.xScale.domain()[0];q=b.gsvg.xScale.domain()[1]-q;console.log(b.trackClass+":"+f);u=[];f&&(0<
f.indexOf(",")?u=f.split(","):u[0]=f);1<u.length&&(tmp=u[0].split(":"),b.scaleMin=1*tmp[0],b.scaleMax=1*tmp[1],console.log("scale:"+b.scaleMin+":"+b.scaleMax));2<u.length&&(b.countType=1*u[1],console.log("countType:"+b.countType));b.fullDataTimeOutHandle=0;b.ttTrackList=[];-1<r.indexOf("illuminaSmall")?(b.ttTrackList.push("ensemblsmallnc"),b.ttTrackList.push("brainsmallnc"),b.ttTrackList.push("liversmallnc"),b.ttTrackList.push("heartsmallnc")):(b.ttTrackList.push("ensemblcoding"),b.ttTrackList.push("braincoding"),
...
b.update=function(a){a=b.xScale.domain()[0];var d=b.xScale.domain()[1];b.loadedDataMin<=a&&d<=b.loadedDataMax?b.redraw():b.updateFullData(0,1)};b.updateFullData=function(a,d){var c=b.xScale.domain()[0],h=b.xScale.domain()[1],e=h-c;b.showLoading();b.bin=b.calculateBin(e);e="Total";2===b.countType&&(e="Norm");var g=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/tmp/"+c+"_"+h+".count."+b.trackClass+"."+e+".xml";0<b.bin&&(c-=2*b.bin,c-=c%(2*b.bin),h+=2*b.bin,h+=2*b.bin-
h%(2*b.bin),g=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/tmp/"+c+"_"+h+".bincount."+b.bin+"."+b.trackClass+"."+e+".xml");d3.xml(g,function(l,m){if(l){if(0==a||1==d){var p="/"+pathPrefix;pathPrefix||(p="");tmpPanel=panel;-1<b.trackClass.indexOf("-")&&(tmpPanel=b.trackClass.substr(b.trackClass.indexOf("-")+1));$.ajax({url:p+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:c,maxCoord:h,panel:tmpPanel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,
myOrganism:organism,genomeVer:genomeVer,track:b.trackClass,folder:b.gsvg.folderName,binSize:b.bin,countType:b.countType},dataType:"json",success:function(t){gtag("event","generateTrackCount",{event_category:"browser"})},error:function(t,v,y){}})}8>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},500):30>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},1E3):32>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},1E4):(d3.select("#Level"+
b.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass),b.hideLoading(),b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,1)},15E3))}else null==m?32<=a?(p=[],b.draw(p)):b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},5E3):(b.fullDataTimeOutHandle=0,b.loadedDataMin=c,b.loadedDataMax=
h,p=m.documentElement.getElementsByTagName("Count"),b.draw(p))})};b.updateCountScale=function(a,d){b.scaleMin=a;b.scaleMax=d;b.colorScale=d3.scaleLinear().domain([b.scaleMin,b.scaleMax]).range(["#EEEEEE","#000000"]);1==b.density&&(b.redrawLegend(),b.redraw())};b.setupToolTipSVG=function(a,d){var c=b.xScale.domain()[0],h=b.xScale.domain()[1],e=parseInt(a.getAttribute("start"),10),g=parseInt(a.getAttribute("stop"),10);h=Math.floor((h-c)*d);20>h&&(h=20);c=e-h;h=g+h;1>c&&(c=1);"undefined"!==typeof b.ttSVGMinWidth&&
h-c<b.ttSVGMinWidth&&(c=e-b.ttSVGMinWidth/2,h=g+b.ttSVGMinWidth/2);e=toolTipSVG("div#ttSVG",450,c,h,99,a.getAttribute("ID"),"transcript");e.addTrack(b.trackClass,3,"",b.data);for(g=0;g<b.ttTrackList.length;g++){var l=b.gsvg.getTrackData(b.ttTrackList[g]),m=[];if("undefined"!==typeof l&&0<l.length){for(var p=0,t=0;t<l.length;t++)if(c<=parseInt(l[t].getAttribute("start"),10)&&parseInt(l[t].getAttribute("start"),10)<=h||parseInt(l[t].getAttribute("start"),10)<=c&&parseInt(l[t].getAttribute("stop"),10)>=
c)m[p]=l[t],p++;0<m.length&&e.addTrack(b.ttTrackList[g],3,"DrawTrx",m)}}};b.draw=function(a){b.hideLoading();b.data=a;var d=b.gsvg.xScale.domain()[0],c=b.gsvg.xScale.domain()[1];b.xScale(d+b.bin);b.xScale(d);b.redrawLegend();for(var h=[],e=0,g=0,l=0;l<a.length;l++)if("undefined"!==typeof a[l]){var m=parseInt(a[l].getAttribute("start"),10),p=parseInt(a[l].getAttribute("stop"),10),t=parseInt(a[l].getAttribute("count"),10);if(1!=b.density||1==b.density&&m!=p)if(l+1<a.length){if(p=parseInt(a[l+1].getAttribute("start"),
10),m>=d&&m<=c||p>=d&&p<=c)h[e]=a[l],t>g&&(g=t),e++}else m>=d&&m<=c&&(h[e]=a[l],t>g&&(g=t),e++)}a=h;b.svg.selectAll("."+b.trackClass).remove();b.svg.select(".y.axis").remove();b.svg.select("g.grid").remove();b.svg.selectAll(".leftLbl").remove();b.svg.select(".area").remove();b.svg.selectAll(".area").remove();1==b.density?(c=b.gsvg.xScale.domain()[1],b.svg.selectAll("."+b.trackClass).data(a,keyStart).enter().append("rect").attr("x",function(v){return b.xScale(v.getAttribute("start"))}).attr("y",15).attr("class",
b.trackClass).attr("height",10).attr("width",function(v,y){return b.xScale(v.getAttribute("stop"))-b.xScale(v.getAttribute("start"))}).attr("fill",b.color).on("mouseover",function(v){0==b.gsvg.isToolTip&&(d3.select(this).style("fill","green"),tt.transition().duration(200).style("opacity",1),tt.html(b.createToolTip(v)).style("left",function(){return b.positionTTLeft(d3.event.pageX)}).style("top",function(){return b.positionTTTop(d3.event.pageY)}),1==b.ttSVG&&b.setupToolTipSVG(v,.005))}).on("mouseout",
function(v){d3.select(this).style("fill",b.color);tt.transition().delay(500).duration(200).style("opacity",0)}),b.svg.attr("height",30)):2==b.density&&(b.yScale.domain([0,g]),b.yAxis=d3.axisLeft(b.yScale).ticks(5),b.svg.select("g.grid").remove(),b.svg.select(".y.axis").remove(),b.svg.selectAll(".leftLbl").remove(),b.svg.append("g").attr("class","y axis").call(b.yAxis),b.svg.select("g.y").selectAll("text").each(function(v){var y=new String(v);b.svg.append("svg:text").attr("class","leftLbl").attr("x",
b.gsvg.width-(7.8*y.length+5)).attr("y",function(){return b.yScale(v)}).attr("dy","0.01em").style("font-weight","bold").text(numberWithCommas(v));d3.select(this).attr("x",function(){return 7.7*y.length+5}).attr("dy","0.05em")}),b.svg.append("g").attr("class","grid").call(b.yAxis.tickSize(-b.gsvg.width+10,0,0).tickFormat("")),b.svg.select(".area").remove(),b.area=d3.area().x(function(v){return b.xScale(v.getAttribute("start"))}).y0(140).y1(function(v,y){return b.yScale(v.getAttribute("count"))}),b.svg.append("path").datum(a).attr("class",
"area").attr("stroke",b.graphColorText).attr("fill",b.graphColorText).attr("d",b.area),b.svg.attr("height",140));b.redrawSelectedArea()};b.redrawLegend=function(){if(2==b.density)d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".legend").remove();else if(1==b.density){var a=new String(b.label);a=b.gsvg.width/2+a.length/2*7.5-10;var d=new String("<"+b.scaleMin);b.drawScaleLegend(b.scaleMin,numberWithCommas(b.scaleMax)+"+","Read Counts","#EEEEEE","#00000",15+7.6*d.length);b.svg.append("text").text("<"+
b.scaleMin).attr("class","legend").attr("x",a).attr("y",12);b.svg.append("rect").attr("class","legend").attr("x",a+7.6*d.length+5).attr("y",0).attr("rx",2).attr("ry",2).attr("height",12).attr("width",15).attr("fill","#FFFFFF").attr("stroke","#CECECE")}};b.redrawSelectedArea=function(){if(1<b.density){var a=b.svg.attr("height");d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".selectedArea").remove();if(-1<b.selectionStart&&-1<b.selectionEnd){var d=b.xScale(b.selectionStart),c=b.xScale(b.selectionEnd)-
d;1>c&&(c=1);d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).append("rect").attr("class","selectedArea").attr("x",d).attr("y",0).attr("height",a).attr("width",c).attr("fill","#CECECE").attr("opacity",.3).attr("pointer-events","none")}}else d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".selectedArea").remove()};b.savePrevious=function(){b.prevSetting={};b.prevSetting.density=b.density;b.prevSetting.scaleMin=b.scaleMin;b.prevSetting.scaleMax=b.scaleMax;b.prevSetting.countType=b.countType};
b.revertPrevious=function(){b.density=b.prevSetting.density;b.scaleMin=b.prevSetting.scaleMin;b.scaleMax=b.prevSetting.scaleMax;b.countType=b.prevSetting.countType};b.generateTrackSettingString=function(){return b.trackClass+","+b.density+","+b.scaleMin+":"+b.scaleMax+","+b.countType+";"};b.generateSettingsDiv=function(a){var d=trackInfo[b.trackClass];b.savePrevious();d3.select(a).select("table").select("tbody").html("");if(d&&d.Controls&&0<d.Controls.length){var c=(new String(d.Controls)).split(",");
a=d3.select(a).select("table").select("tbody");a.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+d.Name);for(d=0;d<c.length;d++)if("undefined"!==typeof c[d]&&""!=c[d]){var h=c[d].split(";"),e=a.append("tr").append("td"),g=h[0].substr(5),l="";3<h.length&&0==h[3].indexOf("Default=")&&(l=h[3].substr(8));if(0==h[1].toLowerCase().indexOf("select")){e.append("text").text(g+": ");var m=h[1].split(":");h=h[2].split("}");g=b.trackClass+"Dense"+b.level+"Select";"colorSelect"==
m[1]?g=b.trackClass+b.level+"colorSelect":"countSelect"==m[1]&&(g=b.trackClass+b.level+"countSelect");e=e.append("select").attr("id",g).attr("name",m[1]);for(m=0;m<h.length;m++){var p=h[m].substr(1).split(":");if(2==p.length){var t=e.append("option").attr("value",p[1]).text(p[0]);-1<g.indexOf("Dense")?p[1]==b.density&&t.attr("selected","selected"):p[1]==l&&t.attr("selected","selected")}}d3.select("select#"+g).on("change",function(){$(this).attr("id")==b.trackClass+"Dense"+b.level+"Select"&&1==$(this).val()?
$("#scaleControl"+b.level).show():$("#scaleControl"+b.level).hide();b.updateSettingsFromUI();b.redraw()})}else 0==h[1].toLowerCase().indexOf("slider")&&(l="none",1==b.density&&(l="inline-block"),e=e.append("div").attr("id","scaleControl"+b.level).style("display",l),e.append("text").text(g+": "),e.append("input").attr("type","text").attr("id","amount").attr("value",b.scaleMin+"-"+b.scaleMax).style("border",0).style("color","#f6931f").style("font-weight","bold").style("background-color","#EEEEEE"),
m=h[1].split(":"),h=h[2].split("}"),e=e.append("div"),e.append("text").text("Min:"),e.append("div").attr("id","min-"+m[1]).style("width","60%").style("display","inline-block").style("float","right"),e.append("br"),e.append("text").text("Max:"),e.append("div").attr("id","max-"+m[1]).style("width","60%").style("display","inline-block").style("float","right"),$("#min-"+m[1]).slider({min:1,max:1E3,step:1,value:b.scaleMin,slide:b.processSlider}),$("#max-"+m[1]).slider({min:1E3,max:2E4,step:100,value:b.scaleMax,
slide:b.processSlider}),b.updateSettingsFromUI(),b.redraw())}c=a.append("tr").append("td");c.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");b.gsvg.setCurrentViewModified();b.gsvg.removeTrack(b.trackClass);var v=svgList[b.gsvg.levelNumber].currentView.ViewID,y=viewMenu[b.gsvg.levelNumber].findTrackByClass(b.trackClass,v);y=viewMenu[b.gsvg.levelNumber].findTrackIndexWithViewID(y.TrackID,
...
typeof h[c]&&""!=h[c]){var e=h[c].split(";"),g=d.append("tr").append("td"),l=e[0].substr(5);3<e.length&&0==e[3].indexOf("Default=")&&e[3].substr(8);if(0==e[1].toLowerCase().indexOf("select")){g.append("text").text(l+": ");l=e[1].split(":");e=e[2].split("}");var m=a.trackClass+"Dense"+a.level+"Select";"colorSelect"==l[1]&&(m=a.trackClass+a.level+"colorSelect");g=g.append("select").attr("id",m).attr("name",l[1]);for(l=0;l<e.length;l++){var p=e[l].substr(1).split(":");if(2==p.length){var t=g.append("option").attr("value",
p[1]).text(p[0]);(-1<m.indexOf("Dense")&&p[1]==a.density||-1<m.indexOf("colorSelect")&&p[1]==a.colorBy)&&t.attr("selected","selected")}}d3.select("select#"+m).on("change",function(){"Score"==$(this).val()?$("div."+a.trackClass+"Scale"+a.level).show():"Color"==$(this).val()&&$("div."+a.trackClass+"Scale"+a.level).hide();a.updateSettingsFromUI();a.draw(a.data)})}else 0==e[1].toLowerCase().indexOf("txt")&&(g=0==$("#colorTrack"+a.level).size()?g.append("div").attr("class",a.trackClass+"Scale"+a.level).style("display",
"none"):d3.select("#"+a.trackClass+"Scale"+a.level),g.append("text").text(l+": "),l=e[1].split(":"),e=e[2].split("}"),e="Data",m="text",p=a.minValue,t=a.maxValue,"color"==l[1]&&(m=e="Color",p=a.minColor,t=a.maxColor),g.append("input").attr("type",m).attr("id",a.trackClass+"min"+e+a.level).attr("class",l[1]).style("margin-left","5px").attr("value",p),g.append("text").text(" - "),g.append("input").attr("type",m).attr("id",a.trackClass+"max"+e+a.level).attr("class",l[1]).style("margin-left","5px").attr("value",
...
a.typeList[2]={name:"LTR",clss:"LTR"};a.typeList[3]={name:"DNA",clss:"DNA"};a.typeList[4]={name:"Simple",clss:"Simple_repeat"};a.typeList[5]={name:"Low Comp.",clss:"Low_complexity"};a.typeList[6]={name:"Satellite",clss:"Satellite"};a.typeList[7]={name:"RNA",clss:"RNA"};a.typeList[8]={name:"Other",clss:"Other"};a.typeList[9]={name:"Unknown",clss:"Unknown"};a.yList={};for(q=0;q<a.typeList.length;q++)a.yList[a.typeList[q].clss]=15+15*q;a.createToolTip=function(d){var c='<BR><div id="ttSVG" style="background:#FFFFFF;"></div><BR>Name: '+
...
"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(w=a.idPrefix+"Int"+v.getAttribute("ID")+"_"+(e-1)+"_"+e);return w});var m=">";"-1"==d.getAttribute("strand")&&(m="<");var p=m;l-=g;var t=0;if(7>l)p="";else for(l-=7;8<l;)0==t?(p+=" ",t=1):(p+=m,t=0),l-=7;h.append("svg:text").attr("id",function(v){var y=a.idPrefix+"IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(y=a.idPrefix+"IntTxt"+v.getAttribute("ID")+"_"+(e-1)+"_"+
e);return y}).attr("dx",g+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",a.color).style("font-size","16px").text(p)}};a.redraw=function(){if(a.prevDensity!=a.density)a.draw(a.data);else{a.yMaxArr=[];a.yArr=[];a.yArr[0]=[];for(var d=0;d<a.gsvg.width;d++)a.yMaxArr[d]=0,a.yArr[0][d]=0;a.trackYMax=0;d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+"trx"+a.gsvg.levelNumber).attr("transform",function(c,h){return"translate("+a.xScale(c.getAttribute("start"))+
...
folder:a.gsvg.folderName},dataType:"json",success:function(p){gtag("event","generateTrackTranscript",{event_category:"browser"})},error:function(p,t,v){}})}3>d?(m=1E4,1==d&&(m=15E3),setTimeout(function(){a.updateFullData(d+1,0)},m)):(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+a.trackClass),d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).attr("height",15),a.gsvg.addTrackErrorRemove(a.svg,"#Level"+a.gsvg.levelNumber+a.trackClass),
...
function CircRNATrack(q,n,r,u,f,b){var a=GenericTranscriptTrack(q,n,r,u,f,b);q=[];b&&(0<b.indexOf(",")?q=b.split(","):q[0]=b);a.density=f;0<q.length&&(a.dataFileName=q[0].substr(9));a.colorBy=1<q.length?q[1]:"Score";2<q.length&&(a.minValue=q[2]);3<q.length&&(a.maxValue=q[3]);4<q.length&&(a.minColor=q[4]);5<q.length&&(a.maxColor=q[5]);a.colorValueField="score";a.minFeatureWidth=1;a.updateControl=0;a.xmlTag="Gene";a.xmlTagBlockElem="Block";a.idPrefix="cirRNA";a.xCirLineWidth=4;a.xPadding=7;a.featureHeight=
...
d<=a.xScale.domain()[0]&&c>=a.xScale.domain()[1]){d=Math.round(a.xScale(d));0>d&&(d=0);c=Math.round(a.xScale(c));c>=a.gsvg.width&&(c=a.gsvg.width-1);h=d-a.xPadding;0>h&&(h=0);var e=c+a.xPadding;e>=a.gsvg.width&&(e=a.gsvg.width-1);for(var g=0,l=h;l<=e;l++)a.yMaxArr[l]>g&&(g=a.yMaxArr[l]);g++;var m=g;if(a.yArr.length<=g)for(a.yArr[m]=[],l=0;l<a.gsvg.width;l++)a.yArr[m][l]=0;l=g-a.scanBackYLines;1>l&&(l=1);m=0;for(var p=l;p<g&&0==m;p++){var t=0;for(l=h;l<=e&&0==t;l++)a.yArr[p][l]>t&&(t=1);0==t&&(g=p,
m=1)}g>a.trackYMax&&(a.trackYMax=g);for(l=d;l<=c;l++)a.yMaxArr[l]<g&&(a.yMaxArr[l]=g),a.yArr[g][l]=1;d=g*a.featureHeight}else d=a.featureHeight;return d};a.calcYFull=function(d){return(d+1)*a.featureHeight};a.draw=function(d){a.data=d;a.prevDensity=a.density;a.trackYMax=0;a.yArr=[];a.yArr[0]=[];for(var c=0;c<a.gsvg.width;c++)a.yMaxArr[c]=0,a.yArr[0][c]=0;d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("."+a.idPrefix+"trx"+a.gsvg.levelNumber).remove();a.redrawLegend();d&&(d=d3.select("#Level"+
...
l=a.xScale(exList[e].getAttribute("start"))-a.xScale(d.getAttribute("start"));h.append("line").attr("x1",g).attr("x2",l).attr("y1",20).attr("y2",20).attr("stroke",a.color).attr("stroke-width","2").attr("id",function(v,y){var w=a.idPrefix+"Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(w=a.idPrefix+"Int"+v.getAttribute("ID")+"_"+(e-1)+"_"+e);return w});var m=">";"-"===d.getAttribute("strand")&&(m="<");var p=m;l-=g;var t=0;if(7>l)p="";else for(l-=
7;8<l;)0==t?(p+=" ",t=1):(p+=m,t=0),l-=7;h.append("svg:text").attr("id",function(v){var y=a.idPrefix+"IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(y=a.idPrefix+"IntTxt"+v.getAttribute("ID")+"_"+(e-1)+"_"+e);return y}).attr("y",15).attr("dx",g+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",a.color).style("font-size","16px").text(p)}};a.calcXLine=function(d,c,h,e,g){h*=a.xCirLineWidth;return 0<d.length?
...
label:"Predicted(Multi)"},{color:"#3282b8",label:"Predicted(Single)"},{color:"#5D576B",label:"Array"},{color:"#7A9D98",label:"Array/Predicted"}])};return a}window.GenomeSVG=GenomeSVG;var reportSelectedTrack=null,loadedTrackTable=null,regionDetailLoaded={};$(document).on("click","span.detailMenu",function(q){q=$(this).attr("name");var n=$("span.detailMenu.selected").attr("name");$("div#"+n).hide();$("span.detailMenu.selected").removeClass("selected");$("span[name='"+q+"']").addClass("selected");$("div#"+q).show();if("undefined"!==typeof svgList[0]){n=svgList[0].xScale.domain()[0];var r=svgList[0].xScale.domain()[1];loadRegionReport(q,chr,n,r)}});
function loadRegionReport(q,n,r,u){n=n+":"+r+"-"+u;"regionTable"===q&&(n=n+":"+reportSelectedTrack);regionDetailLoaded[q]&&regionDetailLoaded[q]===n?"regionWGCNAEQTL"===q&&""===$("div#regionWGCNAEQTL").html()&&loadRegionWGCNA():("regionTable"==q&&null!=reportSelectedTrack?loadTrackTable():"regionEQTLTable"==q?($("div#regionEQTLTable").html('<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>'),setTimeout(function(){loadEQTLTable()},750)):"regionWGCNAEQTL"==
q&&loadRegionWGCNA(),regionDetailLoaded[q]=n);gtag("event",q,{event_category:"loadRegionReport"})}
function loadTrackTable(){var q=svgList[0].xScale.domain()[0],n=svgList[0].xScale.domain()[1],r=chr+":"+minCoord+"-"+maxCoord+":"+reportSelectedTrack.trackClass;loadedTrackTable&&loadedTrackTable===r||(loadedTrackTable=r,r="",q={species:organism,minCoord:q,maxCoord:n,chromosome:chr,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,forwardPvalueCutoff:forwardPValueCutoff,folderName:regionfolderName,genomeVer:genomeVer,track:reportSelectedTrack.trackClass},-1<reportSelectedTrack.trackClass.indexOf("noncoding")?
...
function loadRegionWGCNA(){$("div#geneWGCNA").html("");var q=svgList[0].xScale.domain()[0],n=svgList[0].xScale.domain()[1];loadDivWithPage("div#regionWGCNAEQTL","web/GeneCentric/wgcnaGene.jsp",!0,{region:chr+":"+q+"-"+n},'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>');gtag("event","wgcna",{event_category:"loadWGCNA"})}
function loadEQTLTableWParams(q,n,r,u,f){q=svgList[0].xScale.domain()[0];u=svgList[0].xScale.domain()[1];transcriptome=$("#transriptome").val();cisOnly=$("#cisTrans").val();pValueCutoff=$("#pvalueCutoffSelect2").val();loadDivWithPage("div#regionEQTLTable","web/GeneCentric/regionEQTLTableSeq.jsp",!1,{genomeVer:genomeVer,species:organism,minCoord:q,maxCoord:u,chromosome:chr,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,pValueCutoff:pValueCutoff,tissues:r,chromosomes:n,folderName:regionfolderName,
...
function loadDivWithPage(q,n,r,u,f){$(q).html(f);$.ajax({url:n,type:"GET",cache:!1,data:u,dataType:"html",async:!0,success:function(b){$(q).html(b);r&&setTimeout(function(){$("html, body").animate({scrollTop:$(q).offset().top},200)},300)},error:function(b,a,d){$(q).html('<span style="color:#FF0000;">An error occurred generating this page.  Please try back later.</span>')}})}function trKey(q){var n="";q&&(n=q.trackClass);return n}
function DisplayRegionReport(){if(0<d3.select("#collapsableReportList").size()){var q=svgList[0].trackList;if(reportSelectedTrack)for(var n=0;n<q.length&&!reportSelectedTrack;n++)q[n]&&q[n].getDisplayedData&&(reportSelectedTrack=q[n]);q=d3.select("#collapsableReportList").selectAll("li.report").data(q,trKey).html(function(u){var f="";u&&u.getDisplayedData&&(f=u.getDisplayedData(),f=u.label+": "+f.length);return f});q.enter().append("li").attr("class",function(u){return u?"report "+u.trackClass:"report"}).html(function(u){var f=
"";u&&u.getDisplayedData&&(f=u.getDisplayedData(),f=u.label+": "+f.length);return f}).on("click",displayDetailedView);q.exit().remove();!$("div#collapsableReport").is(":hidden")&&reportSelectedTrack&&displayDetailedView(reportSelectedTrack)}q=$("span.detailMenu.selected").attr("name");n=svgList[0].xScale.domain()[0];var r=svgList[0].xScale.domain()[1];loadRegionReport(q,chr,n,r);gtag("event","regionReport",{event_category:"loadRegionReport"})}
function displayDetailedView(q){reportSelectedTrack=q;$("li.report").removeClass("selected");$("li."+q.trackClass).addClass("selected");q.displayBreakDown&&setTimeout(function(){$("div#trackGraph").html("");q.displayBreakDown("div#collapsableReport div#trackGraph")},50);var n=new String(q.trackClass);-1<n.indexOf("coding")||-1<n.indexOf("noncoding")||-1<n.indexOf("smallnc")||-1<n.indexOf("liverTotal")||-1<n.indexOf("heartTotal")||-1<n.indexOf("brainTotal")||-1<n.indexOf("mergedTotal")||-1<n.indexOf("kidneyTotal")||
"qtl"==n?($("#regionTableSubHeader").show(),$("#regionTable").show()):($("#regionTableSubHeader").hide(),$("#regionTable").hide());$("div#regionTable").is(":hidden")||loadTrackTable();gtag("event",q,{event_category:"loaddetailedView"})}function DisplaySelectedDetailReport(q,n){loadDivWithPage("div#selectedReport",q,!1,n,'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>')};function ViewMenu(q){var n={};n.level=q;n.viewList=[];n.selectedTrackSetting=0;n.previewLevel=100;n.previewSVG=NaN;n.curOrg=(new String(organism)).toUpperCase();n.initialized=0;n.generatePreview=function(r){$("div#previewOuter"+n.level+" div#previewContent").html("");var u=(new String(organism)).toUpperCase();if("AA"===r.Organism||r.Organism===u){u=svgList[n.level].xScale.domain()[0];var f=svgList[n.level].xScale.domain()[1];n.previewSVG=toolTipSVG("div#previewOuter"+n.level+" div#previewContent",
...
u){for(var f=n.findViewWithID(u).TrackList,b=NaN,a=0;a<f.length&&isNaN(b);a++)f[a].TrackID===r&&(b=a);return b};n.setupControls=function(){-1<q&&($(".control"+n.level+"#addTrack"+n.level).on("click",function(){if($(".trackLevel"+n.level).is(":visible"))$(".trackLevel"+n.level).fadeOut("fast");else{var r=$("div.viewsLevel"+n.level).position(),u=0;1200<=$(window).width()&&(u=-601);trackMenu[n.level].standalone=!1;$(".trackLevel"+n.level).css("top",r.top).css("left",r.left+u);r=n.findSelectedView();
...
$("#usrtrkOrgSelect"+n.level).val(),d=$("#usrtrkGenCatSelect"+n.level).val(),c=$("#usrtrkCatTxt"+n.level).val(),h="",e=$("#usrtrkFileTypeSelect"+n.level).val(),g="";if("bed"===e||"bg"===e)g=$("input#customUploadFile"+n.level)[0].files[0].name;var l="Color";0<$("#usrtrkColorSelect"+n.level).length&&(l=$("#usrtrkColorSelect"+n.level).val());if("Score"===l){var m="000000",p="FFFFFF",t=1,v=1E3;testIE||testSafari?(m=$("#usrtrkColorMin"+n.level).spectrum("get").toHexString(),p=$("#usrtrkColorMax"+n.level).spectrum("get").toHexString()):
(m=$("#usrtrkColorMin"+n.level).val(),p=$("#usrtrkColorMax"+n.level).val());m=m.substr(1);p=p.substr(1);t=$("#usrtrkScoreMinTxt"+n.level).val();v=$("#usrtrkScoreMaxTxt"+n.level).val()}"bed"===e?h="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Pack:3}{Full:2};Default=3,Name=Color by;Select:colorSelect::colorSelect;{Feature Defined:Color}{Score based scale:Score};Default="+l+",Name=Scale Range;Txt:tissuecbx;{min:1}{max:1000};Default="+t+":"+v+",Name=Color Range;Txt:color;{min:FFFFFF}{max:000000};Default="+
m+":"+p:"bg"===e?h="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Full:2};Default=1,Name=Scale Range;Slider:rangeslider;{min:1-1000}{max:1000-20000};Default=1:5000":"bw"===e?h="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Full:2};Default=1,Name=Scale Range;Slider:rangeslider;{min:1-1000}{max:1000-20000};Default=1:5000":"bb"===e&&(h="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Pack:3}{Full:2};Default=3,Name=Color by;Select:colorSelect::colorSelect;{Feature Defined:Color}{Score based scale:Score};Default="+
l+",Name=Scale Range;Txt:tissuecbx;{min:1}{max:1000};Default="+t+":"+v+",Name=Color Range;Txt:color;{min:FFFFFF}{max:000000};Default="+m+":"+p);r="custom"+r;0<uid?(l="/"+pathPrefix,pathPrefix||(l=""),$.ajax({url:l+"addTrack.jsp",type:"GET",data:{trackClass:r,trackName:f,trackDesc:b,trackOrg:a,genericCategory:d,category:c,controls:h,location:u,type:e,file:g},dataType:"json",success:function(y){"true"===y.success?$(".uploadStatus").html("<B>Track Setup Successfully</B>"):$(".uploadStatus").html("<B>Track Setup Failed</B>")},
...
function(p){return Math.abs(p.Cor)*a.maxCorWidth}).merge(h))};a.drawLinksBetween=function(h){for(var e={},g=0;g<h.length;g++)e[h[g]]=1;"gene"===that.viewType?a.updateColor("linkColor"):"mir"===that.viewType&&a.updateColor("mirlinkColor");a.svg.selectAll(".link").remove();h=that.selectedModule.LinkList;g=[];for(var l=0,m=0;m<h.length;m++)(0===a.showAllFromTarget&&"undefined"!==typeof e[h[m].TC1]&&"undefined"!==typeof e[h[m].TC2]||1===a.showAllFromTarget&&("undefined"!==typeof e[h[m].TC1]||"undefined"!==
...
function(){}).on("mouseover",a.mouseoverGene).on("mouseout",a.mouseleaveGene).merge(h).each(function(m,p){d3.select(this.parentNode).append("g").attr("class","label").attr("id",function(t){return"txt_"+that.removeInvalidIDChars(t.ID)}).attr("transform",function(){var t=a.nodeXLbl(m,p),v=a.nodeYLbl(m,p);return"translate("+t+","+v+")"}).append("text").attr("transform",function(){var t=-90+p%a.maxPerLevel*a.angleDeg;90<t&&(t-=180);return"rotate("+t+")"}).text(function(){return m.Gene.geneSymbol})});
...
return v}).attr("y2",function(t){var v=h.y+a.height/2-a.miRdiameter/2;0<d3.select("."+t.ID).size()&&(v=d3.select("."+t.ID).attr("cy"));return v}).attr("stroke",function(t){var v=a.mirPred;0<t.V&&(v=a.mirVal);return v}).attr("stroke-width",function(t){return t.P+3*t.V}).merge(e);if(!a.showLinks){e=[];for(var l=0;l<g.length;l++)for(var m=d3.selectAll("."+g[l].ID).nodes(),p=0;p<m.length;p++)"undefined"!==typeof m[p]&&e.push(d3.select(m[p].parentNode).datum().ID);a.drawLinksBetween(e)}gtag("event","mouseOverMir",
...
"none").attr("id",function(m){return"ln_"+m.ID}).attr("x1",function(m){var p=d3.select("."+l).attr("cx");0<d3.select("#"+that.removeInvalidIDChars(m.ID)).size()&&(p=d3.select("#"+that.removeInvalidIDChars(m.ID))[0][0].__data__.x+a.width/2-a.miRdiameter/2);return p}).attr("y1",function(m){var p=d3.select("."+l).attr("cy");0<d3.select("#"+that.removeInvalidIDChars(m.ID)).size()&&(p=d3.select("#"+that.removeInvalidIDChars(m.ID))[0][0].__data__.y+a.height/2-a.miRdiameter/2);return p}).attr("x2",function(m){return d3.select("."+
l).attr("cx")}).attr("y2",function(m){return d3.select("."+l).attr("cy")}).attr("stroke",function(m){var p=a.mirPred;0<m.V&&(p=a.mirVal);return p}).attr("stroke-width",function(m){return m.P+3*m.V}).merge(h)}gtag("event","mouseOverGene",{event_category:"wgcna"})};a.mouseleaveGene=function(){var h=d3.select(this.parentNode).datum();$("[id^=ln_"+that.removeInvalidIDChars(h.ID)+"]").each(function(){var e=$(this).attr("id");d3.select("line#"+e).attr("stroke",a.colorLink)});$("[id$="+that.removeInvalidIDChars(h.ID)+
...
$.fn.DataTable.isDataTable("table#mirTable")&&$("table#mirTable").DataTable().destroy();d3.select("table#mirTable").select("tbody").selectAll("tr").remove();var h=a.runMirFilter(that.singleImage.mirData.MIRList);d3.select("table#mirTable").select("tbody").selectAll("tr").data(h,that.mirKey).enter().append("tr").attr("id",function(e){return"mirTR"+that.removeInvalidIDChars(e.ID)}).attr("class",function(e,g){var l="odd";0===g%2&&(l="even");return l}).each(function(e,g){for(var l=0,m=[],p=0,t=[],v=e.GeneList,
y=0;y<v.length;y++)0<v[y].V?(l++,m.push(v[y])):1<v[y].P&&(p++,t.push(v[y]));d3.select(this).append("td").html(e.ID);d3.select(this).append("td").html(e.ACC);d3.select(this).append("td").html(function(){if(0<p){var w='<span class="triggerpL" name="pl'+g+'">'+p+"</span><BR>";w=w+'<span id="pl'+g+'" style="display:none;" >';for(var A=0;A<t.length;A++){0<A&&(w+=", ");var B=t[A].ID;"undefined"!==typeof that.singleImage.geneList[B]&&(B=that.singleImage.geneList[B]);w+=B}w+="</span>"}else w=p;return w});
d3.select(this).append("td").html(function(){if(0<l){var w='<span class="triggervL" name="vl'+g+'">'+l+"</span><BR>";w=w+'<span id="vl'+g+'" style="display:none;" >';for(var A=0;A<m.length;A++){0<A&&(w+=", ");var B=m[A].ID;"undefined"!==typeof that.singleImage.geneList[B]&&(B=that.singleImage.geneList[B]);w+=B}w+="</span>"}else w=l;return w});d3.select(this).append("td").html(function(){var w=p+l;if(0<w){w='<span class="triggerMiL" name="mil'+g+'">'+w+"</span><BR>";w=w+'<span id="mil'+g+'" style="display:none;" >';
if(0<m.length){w+="Validated: ";for(var A=0;A<m.length;A++){0<A&&(w+=", ");var B=m[A].ID;"undefined"!==typeof that.singleImage.geneList[B]&&(B=that.singleImage.geneList[B]);w+=B}}if(0<t.length)for(0<m.length&&(w+="<BR><BR>"),w+="Predicted: ",A=0;A<t.length;A++)0<A&&(w+=", "),B=t[A].ID,"undefined"!==typeof that.singleImage.geneList[B]&&(B=that.singleImage.geneList[B]),w+=B;w+="</span>"}return w})});$("#mirTable .triggerMiL").on("click",function(e){var g=this.getAttribute("name");$("#"+g).is(":hidden")?
...
function(h,e){var g="odd";0===e%2&&(g="even");return g}).each(function(h,e){var g=that.singleImage.geneMirList[h.Gene.ID],l=0,m=[],p=0,t=[];if("undefined"!==typeof g)for(var v=0;v<g.length;v++)for(var y=g[v].GeneList,w=0,A=0;A<y.length&&0===w;A++)y[A].ID===h.Gene.ID&&(0<y[A].V?(l++,m.push(g[v])):1<y[A].P&&(p++,t.push(g[v])),w=1);d3.select(this).append("td").html(h.Gene.geneSymbol);d3.select(this).append("td").html(h.Gene.ID);d3.select(this).append("td").html(function(B,G){if(0<p){var F='<span class="triggerpL" name="pl'+
e+'">'+p+"</span><BR>";F=F+'<span id="pl'+e+'" style="display:none;" >';for(var D=0;D<t.length;D++){0<D&&(F+=", ");var E=t[D].ID.replace(a.orgPref,"");F+=E}F+="</span>"}else F=p;return F});d3.select(this).append("td").html(function(B,G){if(0<l){var F='<span class="triggervL" name="vl'+e+'">'+l+"</span><BR>";F=F+'<span id="vl'+e+'" style="display:none;" >';for(var D=0;D<m.length;D++){0<D&&(F+=", ");var E=m[D].ID.replace(a.orgPref,"");F+=E}F+="</span>"}else F=l;return F});d3.select(this).append("td").html(function(B,
G){var F=l+p;if(0<F){F='<span class="triggerMiL" name="mil'+e+'">'+F+"</span><BR>";F=F+'<span id="mil'+e+'" style="display:none;" >';if(0<m.length){F+="Validated: ";for(var D=0;D<m.length;D++){0<D&&(F+=", ");var E=m[D].ID.replace(a.orgPref,"");F+=E}}if(0<t.length)for(0<m.length&&(F+="<BR><BR>"),F+="Predicted: ",D=0;D<t.length;D++)0<D&&(F+=", "),E=t[D].ID.replace(a.orgPref,""),F+=E;F+="</span>"}return F})});$("#mirGeneTable .triggerMiL").on("click",function(h){var e=this.getAttribute("name");$("#"+
...
a.displayDepth=3;a.geneList={};var d=[];if(that.selectedModule){d=that.selectedModule.TCList;for(var c=0;c<d.length;c++)a.geneList[that.selectedModule.TCList[c].Gene.ID]=that.selectedModule.TCList[c].Gene.geneSymbol}a.pathColor=function(h){return h&&h.data?h.data.name&&0==h.data.name.indexOf("ENS")?"#DFC184":h.data.name?a.colorGO(h.data.name):"#000000":h&&h.name&&0==h.name.indexOf("ENS")?"#DFC184":h&&h.name?a.colorGO(h.name):"#000000"};a.addLabels=function(h,e,g,l,m,p,t){p=a.radius/(t+1)*h.depth+
a.radius/(t+1)-12;var v=a.radius/(t+1)*h.depth,y=(m-g)/360*Math.PI*v*2;t=Math.floor((m-g)/360*Math.PI*p*2/7)-1;p=Math.floor((p-v)/12);var w=h.data.name;0===h.data.name.indexOf("ENS")&&"undefined"!==typeof a.geneList[w]&&(w=a.geneList[w],25<m-g&&(w=w+" ("+h.data.name+")"));var A=w.split(" ")[0].split("-")[0];if(A.length<=t||-1===h.data.name.indexOf("ENS")&&A.length/2<=t&&10<m-g){y=[];A=0;if(w.length>t)for(;0<w.length;){var B=w.indexOf(" ");if(-1===B)w.length>t?(y[A]=w.substr(0,t),A++,w=w.substr(t)):
(y[A]=w,A++,w="");else{for(var G=0;B<t&&1>G;){var F=w.indexOf(" ",B+1);-1!==F&&F<=t?B=F:G=1}B>t?-1<w.indexOf("-")&&w.indexOf("-")<t?(B=w.indexOf("-"),y[A]=w.substr(0,B+1),A++,w=w.substr(B+1)):(y[A]=w.substr(0,t),A++,w=w.substr(t)):(y[A]=w.substr(0,B),A++,w=w.substr(B+1))}}else y[A]=w;A=a.topG.append("text").style("fill-opacity",1).style("fill","#000").attr("pointer-events","none").attr("class","txt"+removeColon(h.data.id)).attr("text-anchor",function(D){return"start"}).attr("dy","1em").style("opacity",
function(D){return 1}).append("textPath").attr("xlink:href",function(D){return"#ap"+e}).attr("startOffset",function(D){return 0===g&&360===m?90:5});for(h=0;h<y.length&&h<p;h++)h<p&&(h===p-1&&y.length>p&&(y[h]+="...",y[h].length>t&&(y[h]=y[h].substr(0,t-4)+"...")),A.append("tspan").attr("x",0).attr("dy","1em").text(y[h]))}else if(12<=y&&0===h.data.name.indexOf("ENS"))for(A=a.topG.append("text").style("fill-opacity",1).style("fill","#000").attr("pointer-events","none").attr("class","txt"+removeColon(h.data.id)).attr("text-anchor",
function(D){D="start";180<=m&&(D="end");return D}).attr("dy",".2em").attr("transform",function(D){D=l-2.5;var E=v;0===g&&360===m&&(E=20);180<=m&&(D=l+2.5);return"rotate("+(D-90)+")translate("+E+")rotate("+(180<D?-180:0)+")"}).style("opacity",function(D){return 1}),w=h.data.name,0===h.data.name.indexOf("ENS")&&"undefined"!==typeof a.geneList[w]&&(w=a.geneList[w]),t=w.split(" "),h=0;h<t.length;h++)A.append("tspan").attr("x",0).attr("dy","1em").text(t[h])};a.redraw=function(){a.svg.selectAll("path").remove();
...
a.b.runW=0;a.b.runH=0;g=g.enter().append("svg:g").merge(g).attr("transform",function(l,m){var p=7.5*l.data.name.length+25,t=a.b.runW;t+p>a.width&&(t=0,a.b.runH=a.b.runH+a.b.h+a.b.s,a.b.runW=0);a.b.runW=a.b.runW+p+a.b.s;return"translate("+t+", "+a.b.runH+")"});g.append("svg:polygon").attr("points",a.breadcrumbPoints).style("fill",a.pathColor);g.append("svg:text").attr("x",function(l){return(7.5*l.data.name.length+25+a.b.t)/2}).attr("y",a.b.h/2).attr("dy","0.35em").attr("text-anchor","middle").text(function(l){l=
...
(that.moduleGenes[g.MODULE]="",that.requestModule(g.MODULE,a.selectMetaMod))}).on("mouseover",a.mouseoverModule).on("mouseout",a.mouseleaveModule).merge(d).each(function(g,l){d3.select(this.parentNode).append("g").attr("class","label").attr("id",function(m){return"txt_"+that.removeInvalidIDChars(m.MODULE)}).attr("transform",function(){var m=a.nodeXLbl(g,l),p=a.nodeYLbl(g,l);return"translate("+m+","+p+")"}).append("text").attr("transform",function(){var m=-90+l%a.maxPerLevel*a.angleDeg;90<m&&(m-=180);
