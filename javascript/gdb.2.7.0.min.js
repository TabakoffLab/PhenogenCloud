...
function updatePage(f){var e=Math.round(f.xScale.domain()[0]),m=Math.round(f.xScale.domain()[1]);if((e<f.prevMinCoord||m>f.prevMaxCoord)&&(e<f.dataMinCoord||m>f.dataMaxCoord)){processAjax=1;var n=e;var c=m;e>=f.dataMinCoord&&m>f.dataMaxCoord?n=f.dataMaxCoord+1:e<f.dataMinCoord&&m<=f.dataMaxCoord&&(c=f.dataMinCoord-1);f.setLoading();$.ajax({url:pathPrefix+"updateRegion.jsp",type:"GET",data:{chromosome:chr,minCoord:n,maxCoord:c,fullminCoord:e,fullmaxCoord:m,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,
myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(b){f.prevMinCoord=e;f.prevMaxCoord=m;e<f.dataMinCoord&&(f.dataMinCoord=e);m>f.dataMaxCoord&&(f.dataMaxCoord=m);f.folderName=b.folderName;f.updateData();processAjax=0;ga&&ga("send","event","updatePage","outsideLoadedRegion")},error:function(b,a,c){console.log(c)}})}}
...
"navigateBack")}
function zoomIn(f,e){var m={};m.chr=chr;m.start=svgList[f].xScale.domain()[0];m.stop=svgList[f].xScale.domain()[1];var n=Math.floor((m.stop-m.start)*e);1>n&&(n=1);m.start+=n;m.stop-=n;20>m.stop-m.start&&(m.start-=n,m.stop+=n);svgList[f].xScale.domain([m.start,m.stop]);history[f].push(m);svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis);svgList[f].redraw();isNaN(zoomUpdateHandle)||(clearTimeout(zoomUpdateHandle),zoomUpdateHandle=Math.NaN);zoomUpdateHandle=setTimeout(function(){0===f&&updatePage(svgList[f]);
svgList[f].updateFullData();0===f&&setTimeout(function(){DisplayRegionReport()},100)},300);0===f&&$("#geneTxt").val(chr+":"+m.start+"-"+m.stop);ga&&ga("send","event","calledZoomIn",f)}
function zoomOut(f,e){var m={};m.chr=chr;m.start=svgList[f].xScale.domain()[0];m.stop=svgList[f].xScale.domain()[1];var n=Math.floor((m.stop-m.start)*e);1>n&&(n=1);m.start-=n;m.stop+=n;svgList[f].xScale.domain([m.start,m.stop]);history[f].push(m);svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis);svgList[f].redraw();isNaN(zoomUpdateHandle)||(clearTimeout(zoomUpdateHandle),zoomUpdateHandle=Math.NaN);zoomUpdateHandle=setTimeout(function(){0===f&&updatePage(svgList[f]);svgList[f].updateFullData();
0===f&&setTimeout(function(){DisplayRegionReport()},100)},300);0===f&&$("#geneTxt").val(chr+":"+m.start+"-"+m.stop);ga&&ga("send","event","calledZoomOut",f)}
...
0!=svgList[f].overSettings||isNaN(svgList[f].downZoomx))){var m=svgList[f].downZoomx;var n=d3.mouse(svgList[f].vis.node());svgList[f].downZoomxEnd=n[0];e=1;n[0]<m?(m=n[0],e=svgList[f].downZoomx-m):e=n[0]-m;n=Math.round(svgList[f].xScale.invert(m));m=Math.round(svgList[f].xScale.invert(m+e));svgList[f].downZoomx=Math.NaN;svgList[f].downZoomxEnd=Math.NaN;0===f&&$("#geneTxt").val(chr+":"+n+"-"+m);d3.select("#Level"+svgList[f].levelNumber).selectAll("svg rect.zoomRect").remove();d3.select("#Level"+svgList[f].levelNumber).selectAll("svg text#zoomTextStart").remove();
d3.select("#Level"+svgList[f].levelNumber).selectAll("svg text#zoomTextEnd").remove();svgList[f].xScale.domain([n,m]);svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis);svgList[f].redraw();svgList[f].updateFullData();0===f&&setTimeout(function(){DisplayRegionReport()},300);if(0==f||1==f)e={},e.chr=chr,e.start=n,e.stop=m,history[f].push(e)}ga&&ga("send","event","dragMouseUp","mouseUpFromDrag")}
function mmove(){var f;for(f=0;f<svgList.length;f++)if(svgList[f]&&(!svgList[f].overSettings||0===svgList[f].overSettings))if(!isNaN(svgList[f].downx)){var e=d3.mouse(svgList[f].vis.node());rupx=e[0];if(0!==rupx){var m=Math.round(svgList[f].downscalex.domain()[0]);var n=Math.round(svgList[f].mw*(svgList[f].downx-svgList[f].downscalex.domain()[0])/rupx+svgList[f].downscalex.domain()[0]);n<=svgList[f].xMax&&m>=svgList[f].xMin&&(0===f&&$("#geneTxt").val(chr+":"+m+"-"+n),svgList[f].xScale.domain([m,n]),
svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis),svgList[f].redraw())}}else if(isNaN(svgList[f].downPanx))isNaN(svgList[f].downZoomx)||(c=svgList[f].downZoomx,e=d3.mouse(svgList[f].vis.node()),svgList[f].downZoomxEnd=e[0],e[0]<c?(c=e[0],e=svgList[f].downZoomx-c):e=e[0]-c,m=Math.round(svgList[f].xScale.invert(c)),n=Math.round(svgList[f].xScale.invert(c+e)),d3.select("#Level"+svgList[f].levelNumber).selectAll("svg rect.zoomRect").attr("x",c).attr("width",e),d3.select("#Level"+svgList[f].levelNumber).selectAll("svg text#zoomTextStart").attr("x",
c).attr("y",15).text(numberWithCommas(m)),d3.select("#Level"+svgList[f].levelNumber).selectAll("svg text#zoomTextEnd").attr("x",c+e).attr("y",50).text(numberWithCommas(n)));else if(e=d3.mouse(svgList[f].vis.node()),rupx=e[0],0!==rupx){n=svgList[f].downPanx-rupx;var c=(svgList[f].downscalex.domain()[1]-svgList[f].downscalex.domain()[0])/svgList[f].mw;m=Math.round(svgList[f].downscalex.domain()[0]+n*c);n=Math.round(n*c+svgList[f].downscalex.domain()[1]);n<=svgList[f].xMax&&m>=svgList[f].xMin&&(0===
f&&$("#geneTxt").val(chr+":"+m+"-"+n),svgList[f].xScale.domain([m,n]),svgList[f].scaleSVG.select(".x.axis").call(svgList[f].xAxis),svgList[f].redraw(),svgList[f].downPanx=e[0])}}d3.select("html").on("mousemove",mmove).on("mouseup",mup);$(window).resize(function(){for(var f=0;f<svgList.length;f++)svgList[f]&&svgList[f].resize($(window).width()-25)});
...
$(document).on("change","input[name='optioncbx']",function(){var f=new String($(this).attr("id")),e=f.indexOf("CBX"),m=new String(f.substr(0,e));e=f.substr(e+3,1);redrawTrack(e,m);ga&&ga("send","event","changeSettingView",f)});$(document).on("change","input[name='imgCBX']",function(){var f=new String($(this).attr("id")),e=f.indexOf("CBX");new String(f.substr(0,e));e=f.substr(e+3,1);svgList[e].redraw();setTimeout(function(){DisplayRegionReport()},300);ga&&ga("send","event","clickCbx",f)});
...
function displayHelpFirstTime(){if(-1===navigator.userAgent.toLowerCase().indexOf("phantomjs"))if($.cookie("genomeBrowserHelp")){var f=$.cookie("genomeBrowserHelp"),e=f.indexOf("v")+1,m=f.indexOf(".",f.indexOf(".")+1);f=f.substr(e,m-e);e=siteVer.indexOf("v")+1;m=siteVer.indexOf(".",siteVer.indexOf(".")+1);e=siteVer.substr(e,m-e);f!==e&&(setTimeout(function(){$("a#fbhelp1").click()},1E3),$.cookie("genomeBrowserHelp",siteVer))}else setTimeout(function(){$("a#fbhelp1").click()},1E3),$.cookie("genomeBrowserHelp",
siteVer)}function removeTrack(f,e){"undefined"!==typeof svgList[f]&&svgList[f].removeTrack(e);ga&&ga("send","event","removeTrack",f,e)}function redrawTrack(f,e){svgList[f].redrawTrack(e);ga&&ga("send","event","redrawTrack",f,e)}function changeTrackHeight(f,e){if(0<e){var m=e+"px";$("#Scroll"+f).css({"max-height":m,overflow:"auto"})}else $("#Scroll"+f).css({"max-height":"",overflow:"hidden"})}registerKeyboardHandler=function(f){d3.select(window).on("keydown",f)};
function getAllChildrenByName(f,e){var m=[],n=0;if("undefined"!==typeof f&&null!==f&&"undefined"!==typeof f.childNodes)for(var c=f.childNodes,b=0;b<c.length;b++)c.item(b).nodeName==e&&(m[n]=c.item(b),n++);return m}function getFirstChildByName(f,e){var m=null;if(null!==f&&"undefined"!==typeof f&&"undefined"!==typeof f.childNodes)for(var n=f.childNodes,c=!1,b=0;b<n.length&&!c;b++)n.item(b).nodeName==e&&(m=n.item(b),c=!0);return m}
function getAddMenuDiv(f,e){var m="/"+pathPrefix;""==pathPrefix&&(m="");$.ajax({url:m+"settingsMenu.jsp",type:"GET",cache:!1,data:{level:f,organism:organism,type:e},dataType:"html",success:function(e){$("#imageMenu"+f).remove();d3.select("div#imageMenu").append("div").attr("id","imageMenu"+f);$("#imageMenu"+f).html(e)},error:function(e,c,b){$("#imageMenu"+f).remove();d3.select("div#imageMenu").append("div").attr("id","imageMenu"+f);$("#imageMenu"+f).append('<div class="settingsLevel'+f+'">An error occurred generating this menu.  Please try back later.</div>')},
async:!1})}function loadStateFromString(f,e,m,n){loadSavedConfigTracks(f,m,n);loadImageState(e,m)}
function loadSavedConfigTracks(f,e,m){f=f.split(";");var n=0;m=90>e?svgList[e]:m;for(var c=0;c<f.length;c++){var b=f[c].split(",");if(("Rn"==organism&&"undefined"===typeof mouseOnly[b[0]]||"Mm"==organism&&"undefined"===typeof ratOnly[b[0]])&&""!=b[0]){n++;var a="";if(2<b.length)for(var g=2;g<b.length;g++)a=2==g?b[g]:a+","+b[g];1==e&&(a+=",DrawTrx");m.addTrack(b[0],b[1],a,0)}}0==n&&setupDefaultView(e)}
function loadImageState(f,e){if("undefined"!==typeof f&&""!=f)for(var m=f.split(";"),n=0;n<m.length;n++){var c=m[n].split("=");0==(new String(c[0])).indexOf("displaySelect")&&changeTrackHeight("Level"+e,c[1])}}function calculateBin(f,e){var m=new String(Math.floor(f/e));m=m.substr(0,1)*Math.pow(10,m.length-1);m=Math.floor(m/2);5>m&&(m=0);return m}function key(f){return"undefined"!==typeof f?f.getAttribute("ID"):"unknown"}
...
function GenomeSVG(f,e,m,n,c,b,a,g){var d={isToolTip:0,folderName:"",selectedTrackSetting:"",trackListHash:{},overSettings:0,zoomFactor:.05};d.strainSpecificCountColors=d3.scaleOrdinal(d3.schemeCategory20);0==c&&(d.folderName=regionfolderName);var p={};p.chr=chr;p.start=m;p.stop=n;d.allowSelectGenomeVer="undefined"===typeof g?!0:g;if(20>c)try{history[c].push(p)}catch(l){Rollbar.error("history["+c+"] not defined")}d.get=function(a){return d[a]};d.getTrackData=function(a){for(var b=[],c=0;c<d.trackList.length;c++)"undefined"!==
...
"all").style("cursor","pointer").on("click",function(a){d3.select(b).remove();ga&&ga("send","event","removeErrorTrack",b)}).on("mouseover",function(){d.overSettings=1;$("#mouseHelp").html("");ga&&ga("send","event","mouseHelp",b)}).on("mouseout",function(){d.overSettings=0;$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.");ga&&ga("send","event","hideMouseHelp",b)})}};d.addTrack=function(a,b,g,p){"Trx"==d.forceDrawAsValue&&-1==(new String(g)).indexOf("DrawTrx")&&
(g=","===g.substr(g.length-1)?g+"DrawTrx,":g+",DrawTrx,");var l=new String(d.folderName);(0>l.indexOf("_"+d.xScale.domain()[0]+"_")||0>l.indexOf("_"+d.xScale.domain()[1]+"_"))&&$.ajax({url:pathPrefix+"getFullPath.jsp",type:"GET",cache:!1,async:!1,data:{chromosome:chr,minCoord:d.xScale.domain()[0],maxCoord:d.xScale.domain()[1],panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(a){d.folderName=a.folderName;0==c&&(regionfolderName=
d.folderName)},error:function(a,b,d){console.log(d)}});l=null;l=d3.select("#Level"+d.levelNumber+a).nodes();if(0===l.length||null==l){l=d.topLevel.append("li").attr("class","draggable"+d.levelNumber).attr("id","li"+a).style("margin-bottom","-3px").append("svg").attr("width",d.width).attr("height",30).attr("class","track").attr("id","Level"+d.levelNumber+a).style("cursor","move").on("mouseover",function(){0===overSelectable&&("dragzoom"===d.defaultMouseFunct?$("#mouseHelp").html("<B>Zoom:</B> Click and drag to select a region to zoom in. <B>Navigate or Reorder Tracks:</B> Select the appropriate function at the top left of the image."):
...
function(){d.overSettings=0;$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")})}if("genomeSeq"==a)l=SequenceTrack(d,a,"Reference Genomic Sequence",g),d.addTrackList(l);else if(-1<a.indexOf("noncoding"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",function(c,l){if(c)if(60>p){var h=1500;1==p&&(h=2500);setTimeout(function(){d.addTrack(a,b,g,p+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+
a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==l)if(11<=p){h=[];var q=GeneTrack(d,h,a,"Long Non-Coding / Non-PolyA+ Genes",g);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,g,10)},5E3);else{h=l.documentElement.getElementsByTagName("Gene");var e=0,r=l.documentElement.getElementsByTagName("GeneList");0<r.length&&"undefined"!==typeof r[0].getAttribute("ver")&&(e=r[0].getAttribute("ver"));
try{q=GeneTrack(d,h,a,"Long Non-Coding / Non-PolyA+ Genes",g),q.dataVer=e,d.addTrackList(q)}catch(G){}}});else if(-1<a.indexOf("coding"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",function(c,l){if(c)if(60>p){var h=1500;1==p&&(h=2500);setTimeout(function(){d.addTrack(a,b,g,p+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+
d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==l)if(11<=p){h=[];var q=GeneTrack(d,h,a,"Protein Coding / PolyA+",g);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,g,11)},5E3);else{h=l.documentElement.getElementsByTagName("Gene");var e=0,r=l.documentElement.getElementsByTagName("GeneList");0<r.length&&"undefined"!==typeof r[0].getAttribute("ver")&&(e=r[0].getAttribute("ver"));try{q=GeneTrack(d,h,a,"Protein Coding / PolyA+",g),q.dataVer=e,d.addTrackList(q)}catch(G){console.log(G)}}});
else if(-1<a.indexOf("smallnc"))d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",function(c,l){if(c){if(0===p){var h=d.xScale.domain()[0],q=d.xScale.domain()[1],e="/"+pathPrefix;pathPrefix||(e="");$.ajax({url:e+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:h,maxCoord:q,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(a){ga&&
ga("send","event","browser","generateTrackRefseq")},error:function(a,b,d){}})}60>p?(h=1500,1==p&&(h=2500),setTimeout(function(){d.addTrack(a,b,g,p+1)},h)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==l)11<=p?(h=[],q=GeneTrack(d,h,a,"Small RNA (<200 bp) Genes",g),d.addTrackList(q)):setTimeout(function(){d.addTrack(a,
b,g,11)},5E3);else{h="rn5"===genomeVer?l.documentElement.getElementsByTagName("smnc"):l.documentElement.getElementsByTagName("Gene");try{q=GeneTrack(d,h,a,"Small RNA (<200 bp) Genes",g),d.addTrackList(q),""!=selectGene&&q.setSelected(selectGene)}catch(H){}}});else if(0===a.indexOf("liverTotal")||"heartTotal"===a||0===a.indexOf("brainTotal")||"mergedTotal"===a){var q="Liver Reconstructed";"heartTotal"===a?q="Heart Reconstructed":"brainTotal"===a?q="Whole Brain Reconstructed":"mergedTotal"===a&&(q=
"Merged (Brain,Heart,Liver) Reconstructed");(0===a.indexOf("liverTotal")||0===a.indexOf("brainTotal"))&&0<a.indexOf("_")&&(l=a.substr(a.indexOf("_")+1),"LEStm"===l&&(l="LE-Stm"),"F344Stm"===l&&(l="F344-Stm"),q=l+" "+q);d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",function(c,l){if(c){if(1===p&&("mergedTotal"===a||0===a.indexOf("liverTotal")||0===a.indexOf("brainTotal"))){var h=d.xScale.domain()[0],e=d.xScale.domain()[1],r="/"+pathPrefix;pathPrefix||(r=
"");var f=panel;(0===a.indexOf("liverTotal")||0===a.indexOf("brainTotal"))&&0<a.indexOf("_")&&(f=a.substr(a.indexOf("_")+1),"LEStm"===f&&(f="LE-Stm"),"F344Stm"===f&&(f="F344-Stm"));$.ajax({url:r+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:h,maxCoord:e,panel:f,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackRefseq")},error:function(a,
b,d){}})}60>p?(h=1500,1==p&&(h=2500),setTimeout(function(){d.addTrack(a,b,g,p+1)},h)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==l)11<=p?(h=[],e=GeneTrack(d,h,a,q,g),d.addTrackList(e)):setTimeout(function(){d.addTrack(a,b,g,11)},5E3);else{h=l.documentElement.getElementsByTagName("Gene");r=0;
f=l.documentElement.getElementsByTagName("GeneList");0<f.length&&"undefined"!==typeof f[0].getAttribute("ver")&&(r=f[0].getAttribute("ver"));try{e=GeneTrack(d,h,a,q,g),e.dataVer=r,d.addTrackList(e),""!=selectGene&&e.setSelected(selectGene)}catch(G){}}})}else if(0===a.indexOf("refSeq")){$("#"+a+d.levelNumber+"Select").val();var e=d.xScale.domain()[0],r=d.xScale.domain()[1];l=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml";d3.xml(l,function(c,l){if(c){console.log(c);
if(0===p){var h="/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:e,maxCoord:r,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackRefseq")},error:function(a,b,d){}})}60>p?(h=1500,1===p&&(h=2500),setTimeout(function(){d.addTrack(a,b,g,p+1)},h)):(d3.select("#Level"+d.levelNumber+
a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==l)if(11<=p){h=[];var q=RefSeqTrack(d,h,a,"Ref Seq Genes",g);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,g,11)},5E3);else{h=l.documentElement.getElementsByTagName("Gene");try{q=RefSeqTrack(d,h,a,"Ref Seq Genes",g),d.addTrackList(q)}catch(z){console.log(z)}}})}else if(0==
a.indexOf("snp"))e=d.xScale.domain()[0],r=d.xScale.domain()[1],l=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",d3.xml(l,function(c,l){if(c){console.log(c);if(0==p){var h="/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:e,maxCoord:r,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(a){ga&&
ga("send","event","browser","generateTrackSNP")},error:function(a,b,d){}})}60>p?(h=1500,1===p&&(h=2500),setTimeout(function(){d.addTrack(a,b,g,p+1)},h)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==l)if(11<=p){h=[];var q=SNPTrack(d,h,a,b,g);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,
b,g,11)},5E3);else{h=l.documentElement.getElementsByTagName("Snp");try{q=SNPTrack(d,h,a,b,g),d.addTrackList(q)}catch(z){}}});else if("qtl"===a)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/qtl.xml",function(c,l){if(c)if(60>p){var h=1500;1===p&&(h=2500);setTimeout(function(){d.addTrack(a,b,g,p+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+
d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==l)if(11<=p){h=[];var q=QTLTrack(d,h,a,b);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,g,11)},5E3);else{h=l.documentElement.getElementsByTagName("QTL");try{q=QTLTrack(d,h,a,b),d.addTrackList(q)}catch(z){}}});else if("trx"===a)l=getAllChildrenByName(getFirstChildByName(d.selectedData,"TranscriptList"),"Transcript"),l=TranscriptTrack(d,l,a,b),d.addTrackList(l);else if("probe"===a||"probeMouse"==a)d3.xml(dataPrefix+"tmpData/browserCache/"+
genomeVer+"/regionData/"+d.folderName+"/probe.xml",function(c,l){if(c)if(60>p){var h=1500;1===p&&(h=2500);setTimeout(function(){d.addTrack(a,b,g,p+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==l)11<=p?(q=[],h=ProbeTrack(d,q,a,"Affy Exon 1.0 ST Probe Sets",b+","+g),d.addTrackList(h)):setTimeout(function(){d.addTrack(a,
b,g,11)},5E3);else{var q=l.documentElement.getElementsByTagName("probe");try{h=ProbeTrack(d,q,a,"Affy Exon 1.0 ST Probe Sets",b+","+g),d.addTrackList(h)}catch(z){}}});else if("helicos"===a||-1<a.indexOf("illuminaTotal")||-1<a.indexOf("illuminaSmall")||"illuminaPolyA"===a){e=d.xScale.domain()[0];r=d.xScale.domain()[1];var f=calculateBin(r-e,d.width);l=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/tmp/"+e+"_"+r+".count."+a+".xml";0<f&&(e-=e%f,r+=f-r%f,l=dataPrefix+"tmpData/browserCache/"+
genomeVer+"/regionData/"+d.folderName+"/tmp/"+e+"_"+r+".bincount."+f+"."+a+".xml");d3.xml(l,function(c,l){if(c){if(0===p||3===p){var h="/"+pathPrefix;pathPrefix||(h="");tmpPanel=panel;-1<a.indexOf("-")&&(tmpPanel=a.substr(a.indexOf("-")+1));$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:e,maxCoord:r,panel:tmpPanel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName,binSize:f},dataType:"json",
success:function(a){ga&&ga("send","event","browser","generateTrackCounts")},error:function(a,b,d){}});h=1E4}60>p?(h=1E3,0===p&&(h=2500),setTimeout(function(){d.addTrack(a,b,g,p+1)},h)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==l)4<=p?(h=[],"helicos"==a?q=HelicosTrack(d,h,a,b):"illuminaTotal"==
...
h,a,b):"liverilluminaSmall"==a?q=LiverIlluminaSmallTrack(d,h,a,b):"heartilluminaSmall"==a?q=HeartIlluminaSmallTrack(d,h,a,b):-1<a.indexOf("illuminaTotal")&&(q=StrainSpecificIlluminaTotalTrack(d,h,a,b)),d.addTrackList(q)):setTimeout(function(){d.addTrack(a,b,g,4)},3E3);else{h=l.documentElement.getElementsByTagName("Count");var q;try{"helicos"===a?q=HelicosTrack(d,h,a,b):"illuminaTotal"===a?q=IlluminaTotalTrack(d,h,a,b):"illuminaSmall"===a?q=IlluminaSmallTrack(d,h,a,b):"illuminaPolyA"===a?q=IlluminaPolyATrack(d,
h,a,b):"liverilluminaTotalPlus"===a?q=LiverIlluminaTotalPlusTrack(d,h,a,b):"liverilluminaTotalMinus"===a?q=LiverIlluminaTotalMinusTrack(d,h,a,b):"heartilluminaTotalPlus"===a?q=HeartIlluminaTotalPlusTrack(d,h,a,b):"heartilluminaTotalMinus"===a?q=HeartIlluminaTotalMinusTrack(d,h,a,b):"brainilluminaTotalPlus"===a?q=BrainIlluminaTotalPlusTrack(d,h,a,b):"brainilluminaTotalMinus"===a?q=BrainIlluminaTotalMinusTrack(d,h,a,b):"liverilluminaSmall"===a?q=LiverIlluminaSmallTrack(d,h,a,b):"heartilluminaSmall"===
a?q=HeartIlluminaSmallTrack(d,h,a,b):-1<a.indexOf("illuminaTotal")&&(q=StrainSpecificIlluminaTotalTrack(d,h,a,b)),d.addTrackList(q)}catch(z){}}})}else if(-1<a.indexOf("spliceJnct")){e=d.xScale.domain()[0];r=d.xScale.domain()[1];l=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml";var m="Brain ";"liverspliceJnct"==a?m="Liver ":"heartspliceJnct"==a&&(m="Heart ");d3.xml(l,function(c,l){if(c){console.log(c);if(0==p){var h="/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+
"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:e,maxCoord:r,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackSpliceJnct")},error:function(a,b,d){}})}60>p?(h=1500,1===p&&(h=2500),setTimeout(function(){d.addTrack(a,b,g,p+1)},h)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+
a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==l)if(11<=p){h=[];var q=SpliceJunctionTrack(d,h,a,m+"Splice Junctions",1,"");d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,g,11)},5E3);else{h=l.documentElement.getElementsByTagName("Feature");console.log(h);try{q=SpliceJunctionTrack(d,h,a,m+"Splice Junctions",3,""),d.addTrackList(q)}catch(z){console.log(z)}}})}else if("polyASite"==
a)d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/polyASite.xml",function(c,l){if(c)if(60>p){var h=1E3;1===p&&(h=2500);setTimeout(function(){d.addTrack(a,b,g,p+1)},h)}else d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a);else if(null==l)if(4<=p){h=[];var q=PolyATrack(d,h,a,"Predicted PolyA Sites",
g);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,g,4)},5E3);else{h=l.documentElement.getElementsByTagName("Feature");try{q=PolyATrack(d,h,a,"Predicted PolyA Sites",g),d.addTrackList(q)}catch(z){}}});else if(0===a.indexOf("repeatMask"))$("#"+a+d.levelNumber+"Select").val(),e=d.xScale.domain()[0],r=d.xScale.domain()[1],l=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+d.folderName+"/"+a+".xml",d3.xml(l,function(c,h){if(c){console.log(c);if(0===p){var l="/"+pathPrefix;pathPrefix||
(l="");$.ajax({url:l+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:e,maxCoord:r,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a,folder:d.folderName},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackRepeatMask")},error:function(a,b,d){}})}60>p?(l=1500,1===p&&(l=2500),setTimeout(function(){d.addTrack(a,b,g,p+1)},l)):(d3.select("#Level"+d.levelNumber+a).select("#trkLbl").text("An errror occurred loading Track:"+
a),d3.select("#Level"+d.levelNumber+a).attr("height",15),d.addTrackErrorRemove(d3.select("#Level"+d.levelNumber+a),"#Level"+d.levelNumber+a))}else if(null==h)if(11<=p){l=[];var q=RepeatMaskTrack(d,l,a,"Repeat Masker",b,g);d.addTrackList(q)}else setTimeout(function(){d.addTrack(a,b,g,11)},5E3);else{l=h.documentElement.getElementsByTagName("Feature");try{q=RepeatMaskTrack(d,l,a,"Repeat Masker",b,g),d.addTrackList(q)}catch(z){console.log(z)}}});else if(0==a.indexOf("custom"))if(l=trackInfo[a],g=g+",Name="+
l.Name,g="DataFile="+l.Location+","+g,"bed"==l.Type||"bb"==l.Type)h=[],l=CustomTranscriptTrack(d,h,a,l.Name,3,g),d.addTrackList(l),l.updateFullData(0,0);else if("bg"==l.Type||"bw"==l.Type)h=[],l=CustomCountTrack(d,h,a,3,g),d.addTrackList(l);$(".sortable"+d.levelNumber).sortable("refresh");ga&&ga("send","event","trackAdded",a)};d.addTrackList=function(a){null!=a&&(d.trackList[d.trackCount]=a,d.trackCount++,d.trackListHash[a.trackClass]=a,setTimeout(function(){DisplayRegionReport()},300))};d.updateLinks=
...
minCoord:d.xScale.domain()[0],maxCoord:d.xScale.domain()[1],panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(a){d.folderName=a.folderName;0==d.levelNumber&&regionfolderName!=d.folderName&&(regionfolderName=d.folderName);setTimeout(function(){DisplayRegionReport()},300)},error:function(a,b,d){console.log(d)}});for(var a=0;a<d.trackList.length;a++)"undefined"!==typeof d.trackList[a]&&"undefined"!==typeof d.trackList[a].updateFullData&&
...
"zoomTextStart").attr("x",d.downZoomx).attr("y",15).text(numberWithCommas(Math.round(d.xScale.invert(d.downZoomx))));d.scaleSVG.append("text").attr("id","zoomTextEnd").attr("x",d.downZoomx).attr("y",50).text(numberWithCommas(Math.round(d.xScale.invert(d.downZoomx))))}else 0==processAjax&&(d.prevMinCoord=d.xScale.domain()[0],d.prevMaxCoord=d.xScale.domain()[1],a=d3.mouse(d.vis.node()),d.downx=d.xScale.invert(a[0]),d.downscalex=d.xScale)};d.forceDrawAs=function(a){for(var b=0;b<d.trackList.length;b++)"undefined"!==
typeof d.trackList[b]&&"undefined"!==typeof d.trackList[b].drawAs&&(d.trackList[b].drawAs=a,d.trackList[b].draw(d.trackList[b].data));d.forceDrawAsValue=a};d.resize=function(a){d.width=a;d.xScale.range([0,d.width]);d.xAxis=d3.axisTop(d.xScale).ticks(6).tickSize(8).tickPadding(10);d.scaleSVG.attr("width",d.width);d.scaleSVG.select(".x.axis").call(d.xAxis);d3.select("#Level"+d.levelNumber).select(".axisLbl").attr("x",(d.width-2*d.margin)/2);d.topLevel.style("width",d.width+18+"px");for(var b=0;b<d.trackList.length;b++)"undefined"!==
...
"#DCDCDC")};d.changeTrackCursor=function(a){for(var b=0;b<d.trackList.length;b++)"undefined"!==typeof d.trackList[b]&&"undefined"!==typeof d.trackList[b].svg&&d.trackList[b].svg.style("cursor",a)};d.changeScaleCursor=function(a){"undefined"!==typeof d.scaleSVG&&d.scaleSVG.style("cursor",a)};d.updateTrackSelectedArea=function(a,b){d.selectionStart=a;d.selectionEnd=b;var c=d.xScale(a),g=d.xScale(b)-c;d.scaleSVG.selectAll("rect.selectedArea").remove();if((0<c||0<c+g)&&c<d.width)for(1>g&&(g=1),d.scaleSVG.append("rect").attr("class",
"selectedArea").attr("x",c).attr("y",15).attr("height",d.scaleSVG.attr("height")-15).attr("width",g).attr("fill","#CECECE").attr("opacity",.3).attr("pointer-events","none"),c=0;c<d.trackList.length;c++)"undefined"!==typeof d.trackList[c]&&"undefined"!==typeof d.trackList[c].setSelectedArea&&d.trackList[c].setSelectedArea(a,b)};d.getAddMenus=function(){var a="/"+pathPrefix;""==pathPrefix&&(a="");$.ajax({url:a+"trackMenu.jsp",type:"GET",cache:!1,data:{level:d.levelNumber,organism:organism},dataType:"html",
success:function(a){$("#trackMenu"+d.levelNumber).remove();d3.select("div#trackMenu").append("div").attr("id","trackMenu"+d.levelNumber);$("#trackMenu"+d.levelNumber).html(a)},error:function(a,b,c){$("#trackMenu"+d.levelNumber).remove();d3.select("div#trackMenu").append("div").attr("id","trackMenu"+d.levelNumber);$("#trackMenu"+d.levelNumber).append('<div class="viewsLevel'+d.levelNumber+'">An error occurred generating this menu.  Please try back later.</div>')},async:!1});$.ajax({url:a+"viewMenu.jsp",
type:"GET",cache:!1,data:{level:d.levelNumber,organism:organism},dataType:"html",success:function(a){$("#viewMenu"+d.levelNumber).remove();d3.select("div#viewMenu").append("div").attr("id","viewMenu"+d.levelNumber);$("#viewMenu"+d.levelNumber).html(a)},error:function(a,b,c){$("#viewMenu"+d.levelNumber).remove();d3.select("div#viewMenu").append("div").attr("id","viewMenu"+d.levelNumber);$("#viewMenu"+d.levelNumber).append('<div class="viewsLevel'+d.levelNumber+'">An error occurred generating this menu.  Please try back later.</div>')},
async:!1})};d.zoomMenuChange=function(a){d3.select("#zoomSelectMenu"+d.levelNumber).selectAll("li").style("background","#FFFFFF");d.zoomFactor=parseFloat(d3.select(a).attr("value"));d3.select(a).style("background","#CECECE")};d.setupFunctionBar=function(){d3.select(f).select("#functLevel"+d.levelNumber).remove();d.functionBar=d.vis.append("div").attr("class","functionBar").attr("id","functLevel"+d.levelNumber).style("float","left");var a=d.functionBar.append("div").attr("class","defaultMouse").attr("id",
...
$(window).width()/2;d.mh=400;d.downx=Math.NaN;d.downscalex;d.downPanx=Math.NaN;d.downZoomx=Math.NaN;d.downZoomxEnd=Math.NaN;d.defaultMouseFunct="pan";d.xMax=29E7;d.xMin=1;d.prevMinCoord=m;d.prevMaxCoord=n;d.initMin=m;d.initMax=n;d.dataMinCoord=m;d.dataMaxCoord=n;d.y=0;d.xScale=null;d.xAxis=null;d.vis=null;d.level=null;d.svg=null;d.txType=null;d.txList=null;d.tt=null;d.trackList=[];d.trackCount=0;d.levelNumber=c;d.selectedData=null;d.txType=null;d.selectionStart=-1;d.selectionEnd=-1;d.scrollSize=350;
...
function(){$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.")}));d.topDiv=d.vis.append("div").attr("id","Level"+c).style("text-align","left");d.xScale=d3.scaleLinear().domain([m,n]).range([0,d.width]);e={};e.chr=chr;e.start=m;e.stop=n;history[d.levelNumber]||(history[d.levelNumber]=[]);history[d.levelNumber].push(e);d.xAxis=d3.axisTop(d.xScale).ticks(6).tickSize(8).tickPadding(10);d.scaleSVG=d.topDiv.append("svg:svg").attr("width",d.width).attr("height",
...
"dragzoom"!=d.defaultMouseFunct?d3.event.altKey||"dragzoom"==d.defaultMouseFunct||d.changeScaleCursor("ew-resize"):d.changeScaleCursor("crosshair")}).style("cursor","ew-resize");d.scaleSVG.append("g").attr("class","x axis").attr("transform","translate(0,55)").attr("shape-rendering","crispEdges").call(d.xAxis);d3.select("#Level"+d.levelNumber).select(".x.axis").append("text").text(b).attr("x",(d.width-2*d.margin)/2).attr("y",-40).attr("class","axisLbl");d.topLevel=d.topDiv.append("div").attr("id",
"ScrollLevel"+c).attr("class","scroll").style("max-height","350px").style("overflow","auto").style("width","100%").append("ul").attr("id","sortable"+c);d.getAddMenus();svgList[c]=d;$("#sortable"+c).sortable({appendTo:"parent",containment:"parent",stop:function(){}}).disableSelection();$(".draggable"+c).draggable({connectToSortable:"#sortable"+c,scroll:!0,revert:"invalid",axis:"y"});m=mmVer;"Rn"==organism&&(m=rnVer);d3.select("div#imageHeader").html("Organism: "+m+"&nbsp&nbsp&nbsp&nbsp"+siteVer);0<
$("span#verSelect").length&&(m=d3.select("span#verSelect").append("select"),d.allowSelectGenomeVer?(m.on("change",function(){displayWorking();!0===isLocalStorage()?localStorage.setItem(organism+"DefGenomeVer",$(this).val()):$.cookie(organism+"DefGenomeVer",$(this).val());changeGenome($(this).val(),d.currentView);setTimeout(function(){$("form#geneCentricForm").submit()},1500)}),"Rn"===organism?(n=m.append("option").attr("value","rn6").html("rn6"),"rn6"===genomeVer&&n.attr("selected","selected"),m=
m.append("option").attr("value","rn5").html("rn5"),"rn5"===genomeVer&&m.attr("selected","selected")):"Mm"===organism&&(m=m.append("option").attr("value","mm10").html("mm10"),"mm10"===genomeVer&&m.attr("selected","selected"))):m.append("option").attr("value",genomeVer).html(genomeVer));d.addTrack("genomeSeq",3,"both",0);d.selectSvg=selectionSVG("#Level"+d.levelNumber,d.width,d.levelNumber,d);return d}
function toolTipSVG(f,e,m,n,c,b,a){var g={isToolTip:1};g.folderName=regionfolderName;g.updateTimeoutHandle={};g.timeoutTrack=-1;g.forLevel=-1;g.strainSpecificCountColors=d3.scaleOrdinal(d3.schemeCategory20);g.get=function(a){return g[a]};g.getTrack=function(a){for(var b,d=0;d<g.trackList.length;d++)"undefined"!==typeof g.trackList[d]&&g.trackList[d].trackClass===a&&(b=g.trackList[d]);return b};g.addTrackErrorRemove=function(a,b){};g.addTrack=function(a,b,c,h){"Trx"==g.forceDrawAsValue&&-1==(new String(c)).indexOf("DrawTrx")&&
...
c=d=d.substr(d.indexOf(",")+1));if("genomeSeq"===a){var l=SequenceTrack(g,a,"Reference Genomic Sequence",c);l.seqRegionSize=10;g.addTrackList(l)}else if(-1<a.indexOf("noncoding"))l=GeneTrack(g,h,a,"Non-Coding/Non-PolyA+ Transcripts",c),g.addTrackList(l);else if(-1<a.indexOf("coding"))l=GeneTrack(g,h,a,"Protein Coding/PolyA+ Transcripts",c),g.addTrackList(l);else if(0===a.indexOf("liverTotal")||"heartTotal"===a||0===a.indexOf("brainTotal")||"mergedTotal"===a)b="Liver Total RNA Transcripts","heartTotal"===
a?b="Heart Total RNA Transcripts":"brainTotal"===a?b="Brain Total RNA Transcripts":"mergedTotal"===a&&(b="Merged (Brain,Heart,Liver) Reconstructed"),l=GeneTrack(g,h,a,b,c),g.addTrackList(l);else if(-1<a.indexOf("smallnc"))l=GeneTrack(g,h,a,"Small RNA (<200 bp) Genes",c),g.addTrackList(l);else if(0===a.indexOf("refSeq"))99===g.levelNumber&&(c+="DrawTrx,"),l=RefSeqTrack(g,h,a,"Ref Seq Genes",c),l.density=99===g.levelNumber?2:b,g.addTrackList(l);else if(0===a.indexOf("snp"))l=SNPTrack(g,h,a,1,"4"),g.addTrackList(l);
else if("qtl"===a)l=QTLTrack(g,h,a,1),g.addTrackList(l);else if("trx"===a)h=getAllChildrenByName(getFirstChildByName(g.selectedData,"TranscriptList"),"Transcript"),l=TranscriptTrack(g,h,a,b),g.addTrackList(l);else if("probe"===a||"probeMouse"===a)99!==g.levelNumber&&(c=b+","+c),l=ProbeTrack(g,h,a,"Affy Exon 1.0 ST Probe Sets",c),g.addTrackList(l);else if(-1<a.indexOf("spliceJnct"))c="Brain ","liverspliceJnct"===a?c="Liver ":"heartspliceJnct"===a&&(c="Heart "),l=SpliceJunctionTrack(g,h,a,c+"Splice Junctions",
3,""),100===g.levelNumber&&(l.density=b),g.addTrackList(l);else if("polyASite"===a)l=PolyATrack(g,h,a,"Predicted PolyA Sites",c),g.addTrackList(l);else if("helicos"===a||-1<a.indexOf("illuminaTotal")||-1<a.indexOf("illuminaSmall")||"illuminaPolyA"===a){g.updateTimeoutHandle[a]&&(g.updateTimeoutHandle[a]=-1);b=2;c=d.split(",");1<c.length&&(1===c[1]||2===c[1])&&(b=c[1]);"helicos"===a?l=HelicosTrack(g,h,a,b):"illuminaTotal"===a?l=IlluminaTotalTrack(g,h,a,b):"illuminaSmall"===a?l=IlluminaSmallTrack(g,
h,a,b):"illuminaPolyA"===a?l=IlluminaPolyATrack(g,h,a,b):"liverilluminaTotalPlus"===a?l=LiverIlluminaTotalPlusTrack(g,h,a,b):"liverilluminaTotalMinus"===a?l=LiverIlluminaTotalMinusTrack(g,h,a,b):"heartilluminaTotalPlus"===a?l=HeartIlluminaTotalPlusTrack(g,h,a,b):"heartilluminaTotalMinus"===a?l=HeartIlluminaTotalMinusTrack(g,h,a,b):"brainilluminaTotalPlus"===a?l=BrainIlluminaTotalPlusTrack(g,h,a,b):"brainilluminaTotalMinus"===a?l=BrainIlluminaTotalMinusTrack(g,h,a,b):"liverilluminaSmall"===a?l=LiverIlluminaSmallTrack(g,
h,a,b):"heartilluminaSmall"===a?l=HeartIlluminaSmallTrack(g,h,a,b):-1<a.indexOf("illuminaTotal")&&(l=StrainSpecificIlluminaTotalTrack(g,h,a,b));if(99===g.levelNumber){if(0!==g.updateTimeoutHandle[a]){clearTimeout(g.updateTimeoutHandle[a]);try{clearTimeout(g.timeoutTrack[a].fullDataTimeOutHandle)}catch(t){}g.updateTimeoutHandle[a]=0}g.updateTimeoutHandle[a]=setTimeout(function(){l.updateFullData(0,1);g.timeoutTrack[a]=l;g.updateTimeoutHandle[a]=0},300)}g.addTrackList(l)}else if(0===a.indexOf("repeatMask"))l=
RepeatMaskTrack(g,h,a,"Repeat Masker",1,c),g.addTrackList(l);else if(-1<a.indexOf("custom"))if(b=trackInfo[a],c="DataFile="+b.Location+","+c,"bed"===b.Type||"bb"===b.Type)h=[],l=CustomTranscriptTrack(g,h,a,b.Name,3,c),g.addTrackList(l);else if("bg"===b.Type||"bw"===b.Type)h=[],l=CustomCountTrack(g,h,a,3,c),g.addTrackList(l);$(".sortable"+g.levelNumber).sortable("refresh")};g.addTrackList=function(a){null!=a&&(g.trackList[g.trackCount]=a,g.trackCount++,setTimeout(function(){DisplayRegionReport()},
...
(new String(g.folderName)).indexOf("img")&&$.ajax({url:pathPrefix+"getFullPath.jsp",type:"GET",async:!1,data:{chromosome:chr,minCoord:g.xScale.domain()[0],maxCoord:g.xScale.domain()[1],panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer},dataType:"json",success:function(a){g.folderName=a.folderName},error:function(a,b,c){}});for(var a=0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a]&&"undefined"!==typeof g.trackList[a].updateFullData&&
...
g.xScale.domain()[0];g.prevMaxCoord=g.xScale.domain()[1];var a=d3.mouse(g.vis.node());g.downx=g.xScale.invert(a[0]);g.downscalex=g.xScale}};g.forceDrawAs=function(a){for(var b=0;b<g.trackList.length;b++)"undefined"!==typeof g.trackList[b]&&"undefined"!==typeof g.trackList[b].drawAs&&(g.trackList[b].drawAs=a,g.trackList[b].draw(g.trackList[b].data));g.forceDrawAsValue=a};g.generateSettingsString=function(){ret="";for(var a=0;a<g.trackList.length;a++)"undefined"!==typeof g.trackList[a]&&"undefined"!==
typeof g.trackList[a].generateTrackSettingString&&(ret+=g.trackList[a].generateTrackSettingString());return ret};g.setCurrentViewModified=function(){"undefined"!==typeof g.currentView&&(g.currentView.modified=1,$("span#viewModifiedCtl"+g.levelNumber).show())};g.clearCurrentView=function(){g.currentView.modified=0;$("span#viewModifiedCtl"+g.levelNumber).hide()};g.type=a;g.div=f;g.margin=0;g.halfWindowWidth=$(window).width()/2;g.mh=400;g.downx=Math.NaN;g.downscalex;g.downPanx=Math.NaN;g.xMax=29E7;g.xMin=
1;g.prevMinCoord=m;g.prevMaxCoord=n;g.dataMinCoord=m;g.dataMaxCoord=n;g.y=0;g.xScale=null;g.xAxis=null;g.vis=null;g.level=null;g.svg=null;g.txType=null;g.txList=null;g.tt=null;g.trackList=[];g.trackCount=0;g.levelNumber=c;g.selectedData=null;g.txType=null;g.width=e;g.mw=g.width-g.margin;g.vis=d3.select(f);g.topDiv=g.vis.append("div").attr("id","Level"+c).style("text-align","left");g.xScale=d3.scaleLinear().domain([m,n]).range([0,g.width]);g.xAxis=d3.axisTop(g.xScale).ticks(3).tickSize(8).tickPadding(10);
g.scaleSVG=g.topDiv.append("svg:svg").attr("width",g.width).attr("height",60).attr("class","scale");g.scaleSVG.append("g").attr("class","x axis").attr("transform","translate(0,55)").attr("shape-rendering","crispEdges").call(g.xAxis);d3.select("#Level"+g.levelNumber).select(".x.axis").append("text").text(b).attr("x",(g.width-2*g.margin)/2).attr("y",-40).attr("class","axisLbl");g.topLevel=g.topDiv.append("div").attr("id","ScrollLevel"+g.levelNumber).style("width",g.width+18+"px").append("ul").attr("id",
"sortable"+g.levelNumber);return g}
function selectionSVG(f,e,m,n){var c={};c.imageWidth=e+25;c.levelNumber=m;c.parent=n;c.coordSelectStart=0;c.coordSelectStop=0;c.visible=!1;c.changeSelection=function(b,a){c.start=b;c.stop=a;c.setVis(!0);c.draw()};c.setVis=function(b){c.visible=b;c.visible?(c.svg.style("display","inline-block"),$("div#regionDiv").hide()):(c.svg.style("display","none"),$("div#regionDiv").show())};c.draw=function(){if(c.visible){c.xScale(c.stop);c.xScale(c.start);c.parent.updateTrackSelectedArea(c.start,c.stop);var b=
...
(a=c.gsvg.width-1);g=b-c.xPadding;0>g&&(g=0);var d=a+c.xPadding;d>=c.gsvg.width&&(d=c.gsvg.width-1);for(var p=0,l=g;l<=d;l++)c.yMaxArr[l]>p&&(p=c.yMaxArr[l]);p++;var h=p;if(c.yArr.length<=p)for(c.yArr[h]=[],l=0;l<c.gsvg.width;l++)c.yArr[h][l]=0;l=p-c.scanBackYLines;1>l&&(l=1);h=0;for(var q=l;q<p&&0==h;q++){var e=0;for(l=g;l<=d&&0==e;l++)c.yArr[q][l]>e&&(e=1);0==e&&(p=q,h=1)}p>c.trackYMax&&(c.trackYMax=p);for(l=b;l<=a;l++)c.yMaxArr[l]<p&&(c.yMaxArr[l]=p),c.yArr[p][l]=1;b=15*p}else b=15;return b};c.calcYDense=
...
b.length;d++)c.svg.append("rect").attr("class","legend").attr("x",g).attr("y",0).attr("rx",3).attr("ry",3).attr("height",12).attr("width",16).attr("fill",b[d].color).attr("stroke",b[d].color),a=new String(b[d].label),c.svg.append("text").text(a).attr("class","legend").attr("x",g+18).attr("y",12),g=g+25+8*a.length};c.drawScaleLegend=function(b,a,g,d,p,l){var h=new String(c.label),q=c.gsvg.width/2+h.length/2*7.5+16;500>c.gsvg.width&&(q=7.5*h.length);q+=l;d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll(".legend").remove();
d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("#def1").remove();h=c.svg.append("defs").attr("id","def1").append("linearGradient").attr("id","grad"+c.gsvg.levelNumber+c.trackClass);h.append("stop").attr("offset","0%").style("stop-color",d);h.append("stop").attr("offset","100%").style("stop-color",p);h=new String(b);b=7.6*h.length;c.svg.append("text").text(h).attr("class","legend").attr("x",q).attr("y",12);c.svg.append("rect").attr("class","legend").attr("x",q+b+5).attr("y",0).attr("rx",
...
10),d=parseInt(b.getAttribute("stop"),10),p=Math.floor((d-g)*a);20>p&&(p=20);var l=g-p;p=d+p;1>l&&(l=1);"undefined"!==typeof c.ttSVGMinWidth&&p-l<c.ttSVGMinWidth&&(l=g-c.ttSVGMinWidth/2,p=d+c.ttSVGMinWidth/2);g=toolTipSVG("div#ttSVG",450,l,p,99,c.getDisplayID(b),"transcript");g.forLevel=c.gsvg.levelNumber;d=[];d.push(b);g.addTrack(c.trackClass,3,"",d);for(d=0;d<c.ttTrackList.length;d++){var h=c.gsvg.getTrackData(c.ttTrackList[d]),q=[];if("undefined"!==typeof h&&0<h.length){for(var e=0,f=0;f<h.length;f++)if(l<=
parseInt(h[f].getAttribute("start"),10)&&parseInt(h[f].getAttribute("start"),10)<=p||parseInt(h[f].getAttribute("start"),10)<=l&&parseInt(h[f].getAttribute("stop"),10)>=l)q[e]=h[f],e++;0<q.length&&g.addTrack(c.ttTrackList[d],3,"DrawTrx",q)}}};c.generateSettingsDiv=function(b){c.savePrevious();var a=trackInfo[c.trackClass];d3.select(b).select("table").select("tbody").html("");if("undefined"!==typeof a&&0<a.Controls.length&&"null"!=a.Controls){var g=(new String(a.Controls)).split(",");b=d3.select(b).select("table").select("tbody");
b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+a.Name);for(a=0;a<g.length;a++)if("undefined"!==typeof g[a]&&""!=g[a]){var d=g[a].split(";"),p=b.append("tr").append("td"),l=d[0].substr(5);p.append("text").text(l+": ");l="";3<d.length&&0==d[3].indexOf("Default=")&&(l=d[3].substr(8));if(0==d[1].toLowerCase().indexOf("select")){var h=d[1].split(":");d=d[2].split("}");var q="",e="";2<h.length&&(q=h[2]);3<h.length&&(e=h[3]);p=p.append("select").attr("id",c.trackClass+q+
c.level+e).attr("name",h[1]);for(h=0;h<d.length;h++){var f=d[h].substr(1).split(":");if(2==f.length){var m=p.append("option").attr("value",f[1]).text(f[0]);"Dense"===q&&f[1]==c.density?m.attr("selected","selected"):"Version"===q&&f[1]==c.dataVer?m.attr("selected","selected"):f[1]==l&&m.attr("selected","selected")}}d3.select("select#"+c.trackClass+q+c.level+e).on("change",function(){c.updateSettingsFromUI();c.redraw()})}else console.log("Undefined track settings:  "+g[a])}g=b.append("tr").append("td");
...
"G"};c.translate=function(b){b=b.toUpperCase();return c.codons[b]};c.translateRegion=function(b,a,g,d){var p=[],l=0,h=0;if(c.data){if(g>d){h=1;var q=g;g=d;d=q}for(q=g;q<=d;q+=3){var e=c.data.substring(q,q+3);1==h&&(e=c.reverseCompliment(e));p[l]=[];p[l].aa=c.translate(e);p[l].pos=a+(q-g);p[l].id=b+":"+q+":"+c.translate(e);l++}}return p};c.reverseCompliment=function(b){for(var a="",g=b.length-1;-1<g;g--)a+=c.complement(b.charAt(g));return a};c.complement=function(b){var a="N";"A"==b?a="T":"T"==b?a=
...
c.updateData(0):c.draw(c.data);else if(b<c.seqRegionMin||a>c.seqRegionMax)c.updateData(0);else{var g=a-b,d="12px";200<g&&(d="10px");if(g<=c.aaDispCutoff&&1==c.includeAA||g<=c.dispCutoff){if(c.seqRegionMin<=b&&b<=c.seqRegionMax&&c.seqRegionMin<=a&&a<=c.seqRegionMax){var p=c.gsvg.width/g,l=p/2;a=b-c.seqRegionMin;var h=g+a,q=27;1==c.includeAA&&"-"!=c.strands&&(q+=30);if(g<c.dispCutoff){d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("g.line").remove();for(var e=[],f=a;f<h;f++)e[f-a]=[],
e[f-a].base=c.data.charAt(f),e[f-a].id=f,e[f-a].pos=f-a;c.textSize(g);var m=c.yPosition(g);f=c.svg.selectAll(".base").data(e,keyID).attr("transform",function(a,b){return"translate("+(a.pos*p-l)+","+q+")"});e=f.enter().append("g").attr("class","base").attr("transform",function(a,b){return"translate("+(a.pos*p-l)+","+q+")"});e.each(function(a){d3.select(this).append("text").text(function(a){return"-"!=c.strands?a.base:c.complement(a.base)});"both"==c.strands&&d3.select(this).append("text").attr("class",
"comp").attr("y",m).text(function(a){return c.complement(a.base)})});f.exit().remove();f.selectAll("text").attr("font-size",function(a){return c.textSize(g)});f.selectAll("text.comp").attr("y",m)}else d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("g.base").remove(),c.svg.append("g").attr("class","line").append("line").attr("stroke","#000000").attr("x1",0).attr("y1",q).attr("x2",c.gsvg.width).attr("y2",q);if(1==c.includeAA){var n=3*p,u=n/2-4,v=25;if("both"==c.strands||"+"==c.strands)for(f=
...
15).text("<--"),c.svg.append("text").attr("class","dir").attr("x",c.gsvg.width-60).attr("y",15).text("<--"));if(g<c.dispCutoff&&1<b.length){for(var e=[],r=d;r<l;r++)e[r-d]=[],e[r-d].base=b.charAt(r),e[r-d].id=r,e[r-d].pos=r-d;r=c.textSize(g);var f=c.yPosition(g),m=27;1==c.includeAA&&"-"!=c.strands&&(m+=30);e=c.svg.selectAll(".base").data(e,keyID).attr("transform",function(a,b){return"translate("+a.pos*h+","+m+")"});b=e.enter().append("g").attr("class","base").attr("transform",function(a,b){return"translate("+
a.pos*h+","+m+")"});b.each(function(a){d3.select(this).append("text").text(function(b){return"-"!=c.strands?a.base:c.complement(a.base)});"both"==c.strands&&d3.select(this).append("text").attr("class","comp").attr("y",f).text(function(b){return c.complement(a.base)})});e.exit().remove();e.selectAll("text").attr("font-size",r)}else c.svg.append("g").attr("class","base").append("line").attr("stroke","#000000").attr("x1",0).attr("y1",57).attr("x2",c.gsvg.width).attr("y2",57);if(c.data&&1==c.includeAA){var n=
3*h,u=n/2-4,v=25;if("both"==c.strands||"+"==c.strands)for(r=a%3,b=d-r,e=[],e[0]=c.translateRegion(0,a-r,b,b+g),e[1]=c.translateRegion(1,a-r-2,b-2,b-2+g),e[2]=c.translateRegion(2,a-r-1,b-1,b-1+g),v=25,r=0;3>r;r++)l=c.svg.selectAll(".aa"+r).data(e[r],keyID).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}).each(function(a){d3.select(this).select("rect").attr("width",n);g<c.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",u).attr("font-size",
p).text(function(b){return a.aa}):d3.select(this).select("text").attr("x",u).attr("font-size",p):d3.select(this).select("text").remove()}),l.enter().append("g").attr("class","aa"+r).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}).each(function(a){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",n).attr("stroke",c.colorAA(a)).attr("fill",c.colorAA(a));g<c.dispCutoff&&d3.select(this).append("text").attr("x",u).attr("font-size",p).text(function(){return a.aa})}),
l.exit().remove(),v+=11;v="both"==c.strands?80:45;if("both"==c.strands||"-"==c.strands)for(r=a%3,b=d-r,e=[],e[0]=c.translateRegion(0,a-r,b+g,b),e[1]=c.translateRegion(1,a-r-1,b-1+g,b-1),e[2]=c.translateRegion(2,a-r-2,b-2+g,b-2),r=0;3>r;r++)l=c.svg.selectAll(".aarev"+r).data(e[r],keyID).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}),b=l.enter().append("g").attr("class","aarev"+r).attr("transform",function(a,b){return"translate("+c.xScale(a.pos)+","+v+")"}),b.each(function(a){d3.select(this).append("rect").attr("y",
-10).attr("height",10).attr("width",n).attr("stroke",c.colorAA(a)).attr("fill",c.colorAA(a));g<c.dispCutoff&&d3.select(this).append("text").attr("x",u).attr("font-size",p).text(function(b){return a.aa})}),l.exit().remove(),v+=11;"both"==c.strands?c.svg.attr("height",125):c.svg.attr("height",75)}else"both"==c.strands?c.svg.attr("height",45):c.svg.attr("height",30);c.prevStrand=c.strands;c.prevIncldAA=c.includeAA;$("li.draggable"+c.gsvg.levelNumber+"#li"+c.trackClass).show()}else c.svg.attr("height",
...
c.xScale.domain()[1]+c.seqRegionSize,0>c.tmpseqRegionMin&&(c.tmpseqRegionMin=0)),c.xScale.domain()[1]-c.xScale.domain()[0]<c.aaDispCutoff&&(c.lastUpdate=a,c.svg&&30>c.svg.attr("height")&&c.svg.attr("height",30),0===b&&c.showLoading(),d3.text(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.gsvg.folderName+"/"+c.tmpseqRegionMin+"_"+c.tmpseqRegionMax+".seq",function(a,d){if(a){if(0===b){var g="/"+pathPrefix;pathPrefix||(g="");$.ajax({url:g+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,
minCoord:c.tmpseqRegionMin,maxCoord:c.tmpseqRegionMax,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:c.trackClass,folder:c.gsvg.folderName},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackSequence")},error:function(a,b,c){}})}3>b?(g=1E4,1===b&&(g=15E3),setTimeout(function(){c.updateData(b+1)},g)):(c.seqRegionMin=0,c.seqRegionMax=0,c.hideLoading(),d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+
c.trackClass),d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).attr("height",15),c.gsvg.addTrackErrorRemove(c.svg,"#Level"+c.gsvg.levelNumber+c.trackClass),c.lastUpdate=0)}else c.seqRegionMin=c.tmpseqRegionMin,c.seqRegionMax=c.tmpseqRegionMax,c.hideLoading(),c.draw(d),c.lastUpdate=0,setTimeout(function(){DisplayRegionReport()},300)}))};c.updateSettingsFromUI=function(){0<$("#"+c.trackClass+c.level+"Select").length&&(c.strands=$("#"+c.trackClass+c.level+"Select").val());0<$("#"+c.trackClass+"CBX"+
c.level+"dispAA").length&&($("#"+c.trackClass+"CBX"+c.level+"dispAA").is(":checked")?c.includeAA=1:c.includeAA=0)};c.savePrevious=function(){c.prevSetting={};c.prevSetting.strands=c.strands;c.prevSetting.includeAA=c.includeAA};c.revertPrevious=function(){c.strands=c.prevSetting.strands;c.includeAA=c.prevSetting.includeAA};c.generateTrackSettingString=function(){return c.trackClass+","+c.strands+","+c.includeAA+";"};c.generateSettingsDiv=function(b){var a=trackInfo[c.trackClass];c.savePrevious();d3.select(b).select("table").select("tbody").html("");
if(0<a.Controls.length&&"null"!=a.Controls){var g=(new String(a.Controls)).split(",");b=d3.select(b).select("table").select("tbody");b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+a.Name);for(a=0;a<g.length;a++)if(g[a]&&""!=g[a]){var d=g[a].split(";"),p=b.append("tr").append("td"),l=d[0].substr(5);3<d.length&&0===d[3].indexOf("Default=")&&d[3].substr(8);if(0===d[1].toLowerCase().indexOf("select")){p.append("text").text(l+": ");l=d[1].split(":");d=d[2].split("}");for(var h=
c.trackClass+c.level+"Select",e=p.append("select").attr("id",h).attr("name",l[1]),r=0;r<d.length;r++){var f=d[r].substr(1).split(":");2==f.length&&(p=e.append("option").attr("value",f[1]).text(f[0]),f[1]==c.strands&&p.attr("selected","selected"))}d3.select("select#"+h).on("change",function(){c.updateSettingsFromUI();c.redraw()})}else if(0===d[1].toLowerCase().indexOf("cbx"))for(p.append("text").text(l),l=d[1].split(":"),d=d[2].split("}"),r=0;r<d.length;r++)if(f=d[r].substr(1).split(":"),2===f.length){h=
p.append("div").style("display","inline-block");var m="",n="";2<l.length&&(n=l[2]);3<l.length&&(m=l[3]);e=h.append("input").attr("type","checkbox").attr("id",c.trackClass+n+"CBX"+c.level+m).attr("name",l[1]).style("margin-left","5px");h.append("text").text(f[0]);1==c.includeAA?$("#"+c.trackClass+n+"CBX"+c.level+m).prop("checked",!0):$("#"+c.trackClass+n+"CBX"+c.level+m).prop("checked",!1);d3.select("input#"+c.trackClass+n+"CBX"+c.level+m).on("change",function(){c.updateSettingsFromUI();c.redraw()})}}g=
b.append("tr").append("td");g.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");c.gsvg.removeTrack(c.trackClass);c.gsvg.setCurrentViewModified();var a=svgList[c.gsvg.levelNumber].currentView.ViewID,b=viewMenu[c.gsvg.levelNumber].findTrackByClass(c.trackClass,a);b=viewMenu[c.gsvg.levelNumber].findTrackIndexWithViewID(b.TrackID,a);viewMenu[c.gsvg.levelNumber].removeTrackWithIDIdx(b,
a)});g.append("input").attr("type","button").attr("value","Apply").style("float","right").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");c.strands==c.prevSetting.strands&&c.includeAA==c.prevSetting.includeAA||c.gsvg.setCurrentViewModified()});g.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left","5px").on("click",function(){c.revertPrevious();c.draw(c.data);$("#trackSettingDialog").fadeOut("fast")})}else b=
d3.select(b).select("table").select("tbody"),b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+a.Name),b.append("tr").append("td").html("Sorry no settings for this track."),g=b.append("tr").append("td"),g.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")}),g.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left",
"5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")})};c.createCodon();c.updateData(0);c.draw(n);return c}
function GeneTrack(f,e,m,n,c){var b=Track(f,e,m,n);b.counts=[{value:0,names:"Ensembl"},{value:0,names:"Brain RNA-Seq"}];f=new String(c);b.drawAs="Gene";b.trxCutoff=1E5;f=new String(c);b.xScale.domain();b.xScale.domain();-1<f.indexOf("DrawTrx")&&(b.drawAs="Trx");b.density=3;b.dataVer=0;b.reqDataVer=0;b.ttTrackList=[];-1===b.trackClass.indexOf("smallnc")?(b.ttTrackList.push("ensemblcoding"),b.ttTrackList.push("braincoding"),b.ttTrackList.push("brainTotal"),b.ttTrackList.push("liverTotal"),b.ttTrackList.push("heartTotal"),
b.ttTrackList.push("mergedTotal"),b.ttTrackList.push("refSeq"),b.ttTrackList.push("ensemblnoncoding"),b.ttTrackList.push("brainnoncoding")):(b.ttTrackList.push("ensemblsmallnc"),b.ttTrackList.push("brainsmallnc"),b.ttTrackList.push("liversmallnc"),b.ttTrackList.push("heartsmallnc"),b.ttTrackList.push("refSeq"));b.ttTrackList.push("repeatMask");b.ttTrackList.push("snpSHRH");b.ttTrackList.push("snpBNLX");b.ttTrackList.push("snpF344");b.ttTrackList.push("snpSHRJ");"braincoding"===m?(b.ttTrackList.push("spliceJnct"),
b.ttTrackList.push("illuminaPolyA")):0===m.indexOf("liverTotal")?(b.ttTrackList.push("liverspliceJnct"),b.ttTrackList.push("liverilluminaTotalPlus"),b.ttTrackList.push("liverilluminaTotalMinus")):"heartTotal"===m?(b.ttTrackList.push("heartspliceJnct"),b.ttTrackList.push("heartilluminaTotalPlus"),b.ttTrackList.push("heartilluminaTotalMinus")):0===m.indexOf("brainTotal")?(b.ttTrackList.push("brainspliceJnct"),b.ttTrackList.push("brainilluminaTotalPlus"),b.ttTrackList.push("brainilluminaTotalMinus")):
"brainsmallnc"===m?b.ttTrackList.push("illuminaSmall"):"heartsmallnc"===m?b.ttTrackList.push("heartilluminaSmall"):"liversmallnc"===m&&b.ttTrackList.push("liverilluminaSmall");b.pieColorPalette=d3.scaleOrdinal(d3.schemeCategory20);b.cleanID=function(a){return a=a.replace(/\./g,"_")};b.color=function(a){var c="#000000";"ensemblcoding"===b.trackClass?c="#DFC184":"braincoding"===b.trackClass?(c="#7EB5D6",null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&
...
typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&(c=b.strainSpecColor(c,a))):"heartsmallnc"===b.trackClass?(c="#9C3212",null!==a.getAttribute("strain")&&"undefined"!==typeof a.getAttribute("strain")&&"All"!==a.getAttribute("strain")&&(c=b.strainSpecColor(c,a))):"mergedTotal"===b.trackClass&&(c="#9F4F92");return c=d3.rgb(c)};b.strainSpecColor=function(a,b){var c=parseInt(a.substr(1,2),16),g=parseInt(a.substr(3,2),16),l=parseInt(a.substr(5),16);"BNLX"===b.getAttribute("strain").toUpperCase()?
(l+=64,c-=32,g-=32):"SHR"===b.getAttribute("strain").toUpperCase()||"SHRH"===b.getAttribute("strain").toUpperCase()?(c+=64,l-=32,g-=32):"F344_ST"===b.getAttribute("strain").toUpperCase()?(l+=32,c-=16,g-=16):"LE_ST"===b.getAttribute("strain").toUpperCase()&&(c+=32,l-=16,g-=16);255<c&&(c=255);255<g&&(g=255);255<l&&(l=255);return"#"+c.toString(16)+g.toString(16)+l.toString(16)};b.pieStrainSpecColor=function(a,b){var c=parseInt(a.substr(1,2),16),g=parseInt(a.substr(3,2),16),l=parseInt(a.substr(5),16);
if("BNLX"===b.toUpperCase())l+=64,c-=32,g-=32;else if("SHR"===b.toUpperCase()||"SHRH"===b.toUpperCase())c+=64,l-=32,g-=32;255<c&&(c=255);255<g&&(g=255);255<l&&(l=255);return a="#"+c.toString(16)+g.toString(16)+l.toString(16)};b.pieColor=function(a,c){var d="#000000";"ensemblcoding"===b.trackClass?d="#DFC184":"braincoding"===b.trackClass?d=b.pieStrainSpecColor("#7EB5D6",a.data.names):"ensemblnoncoding"===b.trackClass?d="#B58AA5":"brainnoncoding"===b.trackClass?(d="#CECFCE",null!==a.data.names&&"undefined"!==
...
-1===b.trackClass.indexOf("ensembl")&&(p=p+"<BR>PhenoGen ID: "+a.getAttribute("intGeneID"));p=p+"<BR>Length: "+(a.getAttribute("stop")-a.getAttribute("start"))+"<BR>Type:"+c+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+d+"<BR>"}else{var l=a.getAttribute("ID");l=b.getDisplayID(l);var h="";a.getAttribute("geneSymbol")?h=a.getAttribute("geneSymbol"):a.parent&&a.parent.getAttribute("geneSymbol")&&
(h=a.parent.getAttribute("geneSymbol"));d=".";1==a.getAttribute("strand")?d="+":-1==a.getAttribute("strand")&&(d="-");if(a.parent){p=getAllChildrenByName(getFirstChildByName(a.parent,"TranscriptList"),"Transcript");for(var e=0;e<p.length;e++){var f=new String(p[e].getAttribute("ID"));l!=f&&(-1<(new String(p[e].getAttribute("ID"))).indexOf("ENS")?c+="<B>"+f+"</B>":(c+="<B>"+f+"</B>",-1==(new String(p[e].getAttribute("ID"))).indexOf("ENS")&&(f=getFirstChildByName(getFirstChildByName(p[e],"annotationList"),
"annotation"),null!=f&&(c+=" - "+f.getAttribute("reason")))),c+="<br>");if(""==h){f=getAllChildrenByName(getFirstChildByName(p[e],"annotationList"),"annotation");for(var m=0;m<f.length&&""==h;m++)"AKA"==f[m].getAttribute("source")&&(h=f[m].getAttribute("annot_value"),h=(new String(h)).split(":"),h=0!=(new String(h[h.length-1])).indexOf("ENS")?h[h.length-1]:h[0])}}p='<BR><div id="ttSVG" style="background:#FFFFFF;overflow:"></div><BR>Transcript ID: '+l+"<BR>Location: "+a.getAttribute("chromosome")+
":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+d;p=p+"<BR><BR>Gene Symbol: "+h+"<BR>Gene ID: "+b.getDisplayID(a.parent.getAttribute("ID"))+"<BR><BR>Other Transcripts:<BR>"+c+"<BR>"}else{p=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(e=0;e<p.length;e++)if(f=new String(p[e].getAttribute("ID")),-1<(new String(p[e].getAttribute("ID"))).indexOf("ENS")?c+="<B>"+f+"</B>":(c+="<B>"+f+"</B>",-1===(new String(p[e].getAttribute("ID"))).indexOf("ENS")&&
(f=getFirstChildByName(getFirstChildByName(p[e],"annotationList"),"annotation"),null!=f&&(c+=" - "+f.getAttribute("reason")))),c+="<br>",0===h.length)for(f=getAllChildrenByName(getFirstChildByName(p[e],"annotationList"),"annotation"),m=0;m<f.length&&0===h.length;m++)"AKA"===f[m].getAttribute("source")&&(h=f[m].getAttribute("annot_value"),h=(new String(h)).split(":"),h=0!==(new String(h[h.length-1])).indexOf("ENS")?h[h.length-1]:h[0]);p='<BR><div id="ttSVG" style="background:#FFFFFF;overflow:scroll;max-height:400px;"></div>Gene ID: '+
l+"<BR>Gene Symbol: "+h+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+d+"<BR><BR>Transcripts:<BR>"+c+"<BR>"}"brainTotal"!==b.trackClass&&"liverTotal"!==b.trackClass||!(-1<a.getAttribute("ID").indexOf("PRN6T")||-1<a.getAttribute("ID").indexOf("PRN6G"))||(d="Gene",-1<a.getAttribute("ID").indexOf("PRN6T")&&(d="Transcript"),p=p+'<div>Heritability: <span id="ttsingleHerit"></span></div>'+d+' Expression:<BR><div style="text-align:center;">Median Estimated Counts Per Million</div><div id="ttChart"></div>',
c="Brain",l="Whole Brain","liverTotal"===b.trackClass&&(l=c="Liver"),tmpChart=chart({data:"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/"+c+"expr.json",selector:"#ttChart",allowResize:!1,type:"scatter",width:"400",height:"275",displayHerit:!1,displayControls:!1,title:d+" Expression",titlePrefix:l,filterID:a.getAttribute("ID")}))}return p};b.redraw=function(){b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(var a=0;a<b.gsvg.width;a++)b.yMaxArr[a]=0,b.yArr[0][a]=0;b.trackYMax=0;a=b.xScale.domain()[0];
...
(d=parseInt(a.getAttribute("cdsStart"),10),g=parseInt(a.getAttribute("cdsStop"),10));exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");var p="";1===b.gsvg.levelNumber?p="tx":99===b.gsvg.levelNumber&&(p="ttTx");for(var e=0;e<exList.length;e++){var f=exList[e].getAttribute("start"),m=exList[e].getAttribute("stop");if(f<d&&d<m||m>g&&g>f){var n=f,u=d;m>g?(n=g,u=m,m=g):f=d;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+
" rect#ExNC"+exList[e].getAttribute("ID")).attr("x",function(a){return b.xScale(n)}).attr("width",function(a){return b.xScale(u)-b.xScale(n)})}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" rect#Ex"+exList[e].getAttribute("ID")).attr("x",function(a){return b.xScale(f)}).attr("width",function(a){return b.xScale(m)-b.xScale(f)});if(0<e){var v=">";"-1"==a.getAttribute("strand")&&(v="<");var y=v,E=b.xScale(exList[e-1].getAttribute("stop")),
F=b.xScale(exList[e].getAttribute("start")),A=F-E,C=0;if(7>A||"0"==a.getAttribute("strand")||"."==a.getAttribute("strand"))y="";else for(A-=7;8<A;)0===C?(y+=" ",C=1):(y+=v,C=0),A-=7;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" line#Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("x1",E).attr("x2",F);d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+
p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" #IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("dx",E+1).text(y)}}if(2===b.density){var B=a.getAttribute("ID");a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length&&(B=B+" ("+a.getAttribute("geneSymbol")+")");0===d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" text#lblTxt").size()?d3.select("#Level"+b.gsvg.levelNumber+
b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber).append("svg:text").attr("dx",function(){var c=b.xScale(a.parent.getAttribute("start"));return c=c<$(window).width()/2?b.xScale(a.parent.getAttribute("stop"))-b.xScale(a.parent.getAttribute("start"))+5:-9*B.length}).attr("dy",10).attr("id","lblTxt").text(B):d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" text#lblTxt").attr("dx",
function(){var c=b.xScale(a.getAttribute("start"));return c=c<$(window).width()/2?b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5:-9*B.length})}else d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" g.trx"+b.gsvg.levelNumber+"#"+p+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber+" text#lblTxt").remove()}),1===b.density?b.svg.attr("height",30):2===b.density?b.svg.attr("height",15*(b.trackYMax+1)):3===b.density&&b.svg.attr("height",15*(b.trackYMax+1));b.redrawSelectedArea()};
...
a.getAttribute("start")&&(d="small");var p=0,l=0;if("protein"===d||"long"===d||0===d.indexOf("liverTotal")||"heartTotal"===d||0===d.indexOf("brainTotal")||"mergedTotal"===d||"small"===d&&"rn5"!==genomeVer||"small"===d&&-1<(new String(a.getAttribute("ID"))).indexOf("ENS")){console.log("totalType");p=a.getAttribute("ID");var h="";if(a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length)p=p+" ("+a.getAttribute("geneSymbol")+")";else{l=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),
"Transcript");for(var e=0;e<l.length&&""===h;e++)if(""===h)for(var f=getAllChildrenByName(getFirstChildByName(l[e],"annotationList"),"annotation"),m=0;m<f.length&&0===h.length;m++)if("AKA"===f[m].getAttribute("source")){var n=f[m].getAttribute("annot_value");n=(new String(n)).split(":");1<=n.length?(h=n[0],p=p+" ("+h+")"):2<=n.length&&(h=n[0],n=n[1],p=p+" ("+n+")")}}l=parseInt(a.getAttribute("start"),10);f=parseInt(a.getAttribute("stop"),10);a.getAttribute("extStart")&&a.getAttribute("extStop")&&
(l=parseInt(a.getAttribute("extStart"),10),f=parseInt(a.getAttribute("extStop"),10));svgViewIDList[c]||(svgViewIDList[c]=b.gsvg.currentView?b.gsvg.currentView.ViewID:defaultView);e=GenomeSVG("div#selectedImage",b.gsvg.width,l,f,c,p,"transcript");e.xMin=l-.05*(f-l);e.xMax=f+.05*(f-l);p=e.xMin;l=e.xMax;svgList[c]=e;svgList[c].txType=d;svgList[c].selectedData=a;selectedGeneSymbol=a.getAttribute("geneSymbol");selectedID=new String(a.getAttribute("ID"));-1===selectedID.indexOf("ENS")&&0<h.length&&(selectedID=
...
"rn6"===genomeVer&&(a=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/"+b.trackClass+".xml",c=pathPrefix+"smallRNAReport.jsp",a={dataPath:a,id:selectedID},DisplaySelectedDetailReport(c,a))}else"small"===d&&-1===(new String(a.getAttribute("ID"))).indexOf("ENS")&&"rn5"===genomeVer&&($("div#selectedImage").hide(),$("div#selectedReport").show(),p=a.getAttribute("start"),l=a.getAttribute("stop"),c=pathPrefix+"viewSmallNonCoding.jsp",a={id:a.getAttribute("ID"),name:"smRNA_"+
a.getAttribute("ID")},DisplaySelectedDetailReport(c,a));b.gsvg&&b.gsvg.selectSvg&&b.gsvg.selectSvg.changeSelection(p,l);$("html, body").animate({scrollTop:$("#selectedDetail").offset().top},200)}};b.getDisplayedData=function(){var a=[],c=0,d=[];if(-1<b.trackClass.indexOf("smallnc")){var p=0;b.counts=[];b.trackClass.indexOf("ensembl");"Gene"===b.drawnAs?d=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene"):"Trx"===b.drawnAs&&(d=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.trx"));
...
-1<b.trackClass.indexOf("liver")?b.counts[2].value++:-1<b.trackClass.indexOf("heart")?b.counts[3].value++:-1<b.trackClass.indexOf("merged")&&b.counts[4].value++,a[c]=d,c++}),0===d.size()&&(b.counts=[]);else if("Trx"===b.drawnAs){d=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.trx"+b.gsvg.levelNumber);b.counts=[];var l=p=0;d.each(function(d){var g=b.xScale(d.getAttribute("start")),h=b.xScale(d.getAttribute("stop"));if(0<=g&&g<=b.gsvg.width||0<=h&&h<=b.gsvg.width||0>=g&&h>=b.gsvg.width)g=
"Ensembl","undefined"!==typeof d.getAttribute("strain")&&(g=d.getAttribute("strain")),"undefined"===typeof b.counts[g]?(b.counts[g]={},b.counts[p]=b.counts[g],p++,b.counts[g].value=1,b.counts[g].names=g):b.counts[g].value++,a[c]=d,c++,l++})}return a};b.updateData=function(a){d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/"+b.trackClass+".xml",function(c,d){if(c){if(0===a&&(0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal")||"mergedTotal"===
b.trackClass||-1<b.trackClass.indexOf("smallnc"))){var g="/"+pathPrefix;pathPrefix||(g="");var l=b.trackClass,h=panel;(0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal"))&&0<b.trackClass.indexOf("_")&&(h=track.substr(track.indexOf("_")+1),"LEStm"===h&&(h="LE-Stm"),"F344Stm"===h&&(h="F344-Stm"));$.ajax({url:g+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:b.gsvg.xScale.domain()[0],maxCoord:b.gsvg.xScale.domain()[1],panel:h,rnaDatasetID:rnaDatasetID,
arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:l,folder:b.gsvg.folderName},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackGene")},error:function(a,b,c){}})}3>a?(g=1E4,1===a&&(g=15E3),setTimeout(function(){b.updateData(a+1)},g)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,
"#Level"+b.gsvg.levelNumber+b.trackClass))}else if(d){g=d.documentElement.getElementsByTagName("Gene");l=[];h=[];for(var e=0,f=0;f<g.length;f++)g[f]&&(l[e]=g[f],h[g[f].getAttribute("ID")]=1,e++);for(f=0;f<b.data.length;f++)b.data[f]&&!h[b.data[f].getAttribute("ID")]&&(l[e]=b.data[f],e++);b.draw(l);b.hideLoading();setTimeout(function(){DisplayRegionReport()},300)}else b.hideLoading()})};b.updateDataVersion=function(a,c){var d=b.trackClass+"_"+a;d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+
b.gsvg.folderName+"/"+d+".xml",function(g,l){if(g){if(0===c){var h="/"+pathPrefix;pathPrefix||(h="");var p=panel;(0===track.indexOf("liverTotal")||0===track.indexOf("brainTotal"))&&0<track.indexOf("_")&&(p=track.substr(track.indexOf("_")+1),"LEStm"===p&&(p="LE-Stm"),"F344Stm"===p&&(p="F344-Stm"));$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:b.gsvg.xScale.domain()[0],maxCoord:b.gsvg.xScale.domain()[1],panel:p,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,
myOrganism:organism,genomeVer:genomeVer,track:d,folder:b.gsvg.folderName},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackGene")},error:function(a,b,c){}})}5>c?(h=1E4,0===c&&(h=5E3),setTimeout(function(){b.updateDataVersion(a,c+1)},h)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+
b.gsvg.levelNumber+b.trackClass))}else h=l.documentElement.getElementsByTagName("Gene"),p=parseInt(a),b.dataVer=p,b.draw(h),b.hideLoading(),setTimeout(function(){DisplayRegionReport()},300)})};b.drawTrx=function(a,c){var d=parseInt(a.getAttribute("start"),10),g=parseInt(a.getAttribute("stop"),10);a.getAttribute("cdsStart")&&a.getAttribute("cdsStop")&&(d=parseInt(a.getAttribute("cdsStart"),10),g=parseInt(a.getAttribute("cdsStop"),10));var l="";1===b.gsvg.levelNumber?l="tx":99===b.gsvg.levelNumber&&
(l="ttTx");l=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass+" #"+l+b.cleanID(a.getAttribute("ID"))+b.gsvg.levelNumber);exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var h=0;h<exList.length;h++){var e=parseInt(exList[h].getAttribute("start"),10),f=parseInt(exList[h].getAttribute("stop"),10);if(e<d&&d<f||f>g&&g>e){var m=0,n=0,w=0,u=0;e<d?(m=b.xScale(e),n=b.xScale(d)-b.xScale(e),w=b.xScale(d),u=b.xScale(f)-b.xScale(d)):f>g&&(w=b.xScale(e),u=b.xScale(g)-b.xScale(e),m=b.xScale(g),
n=b.xScale(f)-b.xScale(g));l.append("rect").attr("x",m).attr("y",2.5).attr("height",5).attr("width",n).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"ExNC"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer");l.append("rect").attr("x",w).attr("height",10).attr("width",u).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor",
"pointer")}else{m=10;n=0;if(e<d&&f<d||f>g&&e>g)m=5,n=2.5;l.append("rect").attr("x",function(a){return b.xScale(exList[h].getAttribute("start"))}).attr("y",n).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).attr("height",m).attr("width",function(a){return b.xScale(exList[h].getAttribute("stop"))-b.xScale(exList[h].getAttribute("start"))}).attr("title",function(a){return exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}if(0<h&&(l.append("line").attr("x1",
function(a){return b.xScale(exList[h-1].getAttribute("stop"))}).attr("x2",function(a){return b.xScale(exList[h].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(a){return"Int"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("stroke",b.color).attr("stroke-width","2"),"1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))){e=">";if("-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand"))e="<";f=e;m=b.xScale(exList[h-1].getAttribute("stop"));
n=b.xScale(exList[h].getAttribute("start"))-m;w=0;if(7>n)f="";else for(n-=7;8<n;)0==w?(f+=" ",w=1):(f+=e,w=0),n-=7;l.append("svg:text").attr("id",function(a){return"IntTxt"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("dx",m+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(f)}}if(2==b.density){var v=a.getAttribute("ID");"undefined"!==typeof a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length&&
(v=v+" ("+a.getAttribute("geneSymbol")+")");l.append("svg:text").attr("dx",function(){var c=b.xScale(a.getAttribute("start"));return c=c<$(window).width()/2?b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5:-9*v.length}).attr("dy",10).attr("id","lblTxt").text(v)}else l.select("text#lblTxt").remove()};b.getSVGID=function(a){return b.getDisplayID(a.getAttribute("ID"))+" ("+a.getAttribute("geneSymbol")+")"};b.setupToolTipSVG=function(a,c){if("undefined"!==typeof a.getAttribute("extStart")&&
null!==a.getAttribute("extStart")){var d=parseInt(a.getAttribute("extStart"),10);var g=parseInt(a.getAttribute("extStop"),10)}else d=parseInt(a.getAttribute("start"),10),g=parseInt(a.getAttribute("stop"),10);var l=Math.floor((g-d)*c);10>l&&(l=10);d-=l;g+=l;l=toolTipSVG("div#ttSVG",450,d,g,99,b.getSVGID(a),"transcript");l.xMin=d;l.xMax=g;var h="none";0===b.trackClass.indexOf("liverTotal")?h=b.trackClass:"heartTotal"==b.trackClass?h="heartTotal":0===b.trackClass.indexOf("brainTotal")?h=b.trackClass:
"mergedTotal"==b.trackClass?h="mergedTotal":200>parseInt(a.getAttribute("stop"),10)-parseInt(a.getAttribute("start"),10)?h="small":"protein_coding"==a.getAttribute("biotype")&&200<=parseInt(a.getAttribute("stop"),10)-parseInt(a.getAttribute("start"),10)?h="protein":"protein_coding"!=a.getAttribute("biotype")&&200<=parseInt(a.getAttribute("stop"),10)-parseInt(a.getAttribute("start"),10)&&(h="long");l.txType=h;l.selectedData=a;h=[];h[0]=a;"undefined"!==typeof a.parent&&(h[0]=a.parent);l.addTrack(b.trackClass,
2,"DrawTrx",h);for(h=0;h<b.ttTrackList.length;h++)if(b.trackClass!==b.ttTrackList[h]){var e=b.gsvg.getTrackData(b.ttTrackList[h]),f=[];if("undefined"!==typeof e&&0<e.length){for(var m=0,n=0;n<e.length;n++)if(d<=parseInt(e[n].getAttribute("start"),10)&&parseInt(e[n].getAttribute("start"),10)<=g||d<=parseInt(e[n].getAttribute("stop"),10)&&parseInt(e[n].getAttribute("stop"),10)<=g||parseInt(e[n].getAttribute("start"),10)<=d&&parseInt(e[n].getAttribute("stop"),10)>=d)f[m]=e[n],m++;0<f.length&&l.addTrack(b.ttTrackList[h],
3,"DrawTrx",f)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(var c=0;c<b.gsvg.width;c++)b.yMaxArr[c]=0,b.yArr[0][c]=0;b.svg.selectAll(".gene").remove();c=b.drawAs;var d=b.xScale.domain()[0];if(b.xScale.domain()[1]-d<b.trxCutoff||$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.drawAs="Trx";d="Genes";"Trx"==b.drawAs&&(d="Transcripts");var p="Protein Coding",l=" / PolyA+";-1<b.trackClass.indexOf("noncoding")?(p="Long Non-Coding",l=" / Non-PolyA+"):-1<b.trackClass.indexOf("smallnc")?
(p="Brain",-1<b.trackClass.indexOf("liver")?p="Liver":-1<b.trackClass.indexOf("heart")&&(p="Heart"),p+=" Small RNA",l=""):0===b.trackClass.indexOf("liverTotal")?(p="Liver RNA-Seq Reconstruction ",l="Total RNA"):"heartTotal"==b.trackClass?(p="Heart RNA-Seq Reconstruction ",l="Total RNA"):0===b.trackClass.indexOf("brainTotal")?(p="Whole Brain RNA-Seq Reconstruction ",l="Total RNA"):"mergedTotal"==b.trackClass&&(p="Merged (Brain,Heart,Liver) Reconstructed",l="Total RNA");p=-1<b.trackClass.indexOf("ensembl")?
"Ensembl "+p+" "+d:-1===b.trackClass.indexOf("brainTotal")&&-1===b.trackClass.indexOf("liverTotal")&&"heartTotal"!=b.trackClass&&"mergedTotal"!=b.trackClass?"Reconstruction "+p+l+" "+d:p+l+" "+d;(0===b.trackClass.indexOf("liverTotal")||0===b.trackClass.indexOf("brainTotal"))&&0<b.trackClass.indexOf("_")&&(d=b.trackClass.substr(b.trackClass.indexOf("_")+1),"LEStm"===d&&(d="LE-Stm"),"F344Stm"===d&&(d="F344-Stm"),p=d+" Filtered (>=1.0TPM) "+p);b.updateLabel(p);b.redrawLegend();d=a;if("Trx"===b.drawAs&&
-1===b.trackClass.indexOf("smallnc"))for(d=[],l=p=0;l<a.length;l++)if("undefined"!==typeof a[l])for(var h=getAllChildrenByName(getFirstChildByName(a[l],"TranscriptList"),"Transcript"),e=0;e<h.length;e++)d[p]=h[e],d[p].parent=a[l],p++;0<d.length&&("Gene"===b.drawAs||-1<b.trackClass.indexOf("smallnc")?(b.drawnAs="Gene",b.svg.selectAll(".trx0").each(function(){d3.select(this).remove()}),a=b.svg.selectAll(".gene").data(d,key).attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+
...
function RefSeqTrack(f,e,m,n,c){var b=GeneTrack(f,e,m,n);b.counts=[{value:0,names:"Ensembl"},{value:0,names:"RNA-Seq"}];b.drawAs="Gene";b.trxCutoff=1E5;b.density=3;-1<(new String(c)).indexOf("DrawTrx")&&(b.drawAs="Trx");b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(f=0;f<b.gsvg.width;f++)b.yMaxArr[f]=0,b.yArr[0][f]=0;b.ttTrackList=[];b.ttTrackList.push("ensemblcoding");b.ttTrackList.push("braincoding");b.ttTrackList.push("liverTotal");b.ttTrackList.push("heartTotal");b.ttTrackList.push("mergedTotal");b.ttTrackList.push("ensemblnoncoding");
b.ttTrackList.push("brainnoncoding");b.ttTrackList.push("repeatMask");b.ttTrackList.push("snpSHRH");b.ttTrackList.push("snpBNLX");b.ttTrackList.push("snpF344");b.ttTrackList.push("snpSHRJ");b.cleanID=function(a){return a=a.replace(/\./g,"_")};b.color=function(a){var c=d3.rgb("#000000");if("Gene"==b.drawnAs){a=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(var d=-1,p=0;p<a.length;p++){var l=new String(a[p].getAttribute("category")),h=-1;"Reviewed"==l?h=5:"Validated"==
l?h=4:"Provisional"==l?h=3:"Inferred"==l?h=2:"Predicted"==l?h=1:"Model"==l?h=1:"Unknown"==l&&(h=0);d<h&&(d=h)}5===d&&(c=d3.rgb("#18814F"));4===d?c=d3.rgb("#38A16F"):3===d?c=d3.rgb("#78E1AF"):2===d?c=d3.rgb("#A8FFDF"):1===d?c=d3.rgb("#A8DFFF"):0===d&&(c=d3.rgb("#C8FFFF"))}else"Trx"==b.drawnAs&&(l=new String(a.getAttribute("category")),"Reviewed"==l?c=d3.rgb("#18814F"):"Validated"==l?c=d3.rgb("#38A16F"):"Provisional"==l?c=d3.rgb("#78E1AF"):"Inferred"==l?c=d3.rgb("#A8FFDF"):"Predicted"==l?c=d3.rgb("#A8DFFF"):
"Model"===l?c=d3.rgb("#A8DFFF"):"Unknown"===l&&(c=d3.rgb("#C8FFFF")));return c};b.pieColor=function(a,b){var c=d3.rgb("#000000"),g=new String(a.data.names);-1<g.indexOf("Reviewed")?c=d3.rgb("#18814F"):-1<g.indexOf("Validated")?c=d3.rgb("#38A16F"):-1<g.indexOf("Provisional")?c=d3.rgb("#78E1AF"):-1<g.indexOf("Inferred")?c=d3.rgb("#A8FFDF"):-1<g.indexOf("Predicted")?c=d3.rgb("#A8DFFF"):-1<g.indexOf("Model")?c=d3.rgb("#A8DFFF"):-1<g.indexOf("Unknown")&&(c=d3.rgb("#C8FFFF"));return c};b.getDisplayedData=
function(){var a=[],c=0,d=0;if("Gene"==b.drawnAs){var p=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".gene");b.counts=[];var l=0;p.each(function(g){var h=b.xScale(g.getAttribute("start")),p=b.xScale(g.getAttribute("stop"));if(0<=h&&h<=b.gsvg.width||0<=p&&p<=b.gsvg.width||0>=h&&p>=b.gsvg.width){p=getAllChildrenByName(getFirstChildByName(g,"TranscriptList"),"Transcript");h=-1;for(var e=0;e<p.length;e++){var f=new String(p[e].getAttribute("category")),m=-1;"Reviewed"==f?m=5:"Validated"==
f?m=4:"Provisional"==f?m=3:"Inferred"==f?m=2:"Predicted"==f?m=1:"Model"==f?m=1:"Unknown"==f&&(m=1);h<m&&(h=m)}p="Unknown";5==h?p="Reviewed":4==h?p="Validated":3==h?p="Provisional":2==h?p="Inferred":1==h&&(p="Predicted/Model");"undefined"===typeof b.counts[p]?(b.counts[p]={},b.counts[l]=b.counts[p],l++,b.counts[p].value=1,b.counts[p].names=p):b.counts[p].value++;a[c]=g;c++;d++}})}else b.counts=[],l=0,b.svg.selectAll("g.trx"+b.gsvg.levelNumber).each(function(g){var h=b.xScale(g.getAttribute("start")),
p=b.xScale(g.getAttribute("stop"));if(0<=h&&h<=b.gsvg.width||0<=p&&p<=b.gsvg.width||0>=h&&p>=b.gsvg.width)h=new String(g.getAttribute("category")),"undefined"===typeof b.counts[h]?(b.counts[h]={},b.counts[l]=b.counts[h],l++,b.counts[h].value=1,b.counts[h].names=h):b.counts[h].value++,a[c]=g,c++,d++});return a};b.createToolTip=function(a){var c="",d=a.getAttribute("strand");d="1"==d||"+"==d?"+":"-1"==d||"-"==d?"-":".";if("Gene"===b.drawnAs){c="";for(var p=getAllChildrenByName(getFirstChildByName(a,
"TranscriptList"),"Transcript"),l=0;l<p.length;l++){var h=new String(p[l].getAttribute("ID"));c+="<B>"+h+"</B> - "+p[l].getAttribute("category");c+="<br>"}c='<BR><div id="ttSVG" style="background:#FFFFFF;"></div>Gene Symbol: '+a.getAttribute("geneSymbol")+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+d+"<BR>Transcripts:<BR>"+c}else"Trx"===b.drawnAs&&(h=new String(a.getAttribute("ID")),c='<BR><div id="ttSVG" style="background:#FFFFFF;"></div>RefSeq ID:'+
("<B>"+h+"</B>")+"<BR>Status: "+a.getAttribute("category")+"<BR>Gene Symbol: "+a.getAttribute("geneSymbol")+"<BR>Location: "+a.getAttribute("chromosome")+":"+numberWithCommas(a.getAttribute("start"))+"-"+numberWithCommas(a.getAttribute("stop"))+"<BR>Strand: "+d);return c};b.getSVGID=function(a){return a.getAttribute("geneSymbol")};b.setupToolTipSVG=function(a,c){var d=parseInt(a.getAttribute("start"),10),g=parseInt(a.getAttribute("stop"),10),l=Math.floor((g-d)*c);20>l&&(l=20);var h=d-l;l=g+l;1>h&&
(h=1);"undefined"!==typeof b.ttSVGMinWidth&&l-h<b.ttSVGMinWidth&&(h=d-b.ttSVGMinWidth/2,l=g+b.ttSVGMinWidth/2);d=toolTipSVG("div#ttSVG",450,h,l,99,b.getSVGID(a),"transcript");d.forLevel=b.gsvg.levelNumber;g=[];g.push(a);d.addTrack(b.trackClass,3,"DrawTrx",g);for(g=0;g<b.ttTrackList.length;g++){var e=b.gsvg.getTrackData(b.ttTrackList[g]),f=[];if("undefined"!==typeof e&&0<e.length){for(var m=0,n=0;n<e.length;n++)if(h<=parseInt(e[n].getAttribute("start"),10)&&parseInt(e[n].getAttribute("start"),10)<=
l||parseInt(e[n].getAttribute("start"),10)<=h&&parseInt(e[n].getAttribute("stop"),10)>=h)f[m]=e[n],m++;0<f.length&&d.addTrack(b.ttTrackList[g],3,"DrawTrx",f)}}};b.calcY=function(a,c,d){if(3==b.density)if(a>=b.xScale.domain()[0]&&a<=b.xScale.domain()[1]||c>=b.xScale.domain()[0]&&c<=b.xScale.domain()[1]||a<=b.xScale.domain()[0]&&c>=b.xScale.domain()[1]){a=Math.floor(b.xScale(a));0>a&&(a=0);c=Math.floor(b.xScale(c))+1;c>=b.gsvg.width&&(c=b.gsvg.width-1);d=a-5;0>d&&(d=0);var g=c+5;g>=b.gsvg.width&&(g=
b.gsvg.width-1);for(var l=0,h=d;h<=g;h++)b.yMaxArr[h]>l&&(l=b.yMaxArr[h]);l++;var e=l;if(b.yArr.length<=l)for(b.yArr[e]=[],h=0;h<b.gsvg.width;h++)b.yArr[e][h]=0;h=l-12;1>h&&(h=1);e=0;for(var f=h;f<l&&0==e;f++){var m=0;for(h=d;h<=g&&0==m;h++)b.yArr[f][h]>m&&(m=1);0==m&&(l=f,e=1)}l>b.trackYMax&&(b.trackYMax=l);for(h=a;h<=c;h++)b.yMaxArr[h]<l&&(b.yMaxArr[h]=l),b.yArr[l][h]=1;a=15*l}else a=15;else a=2==b.density?15*(d+1):15;b.trackYMax<a/15&&(b.trackYMax=a/15);return a};b.redraw=function(){var a=b.xScale.domain()[0];
...
"exonList"),"exon");var l="";1==b.gsvg.levelNumber&&(l="tx");for(var e=0;e<exList.length;e++){var p=parseInt(exList[e].getAttribute("start"),10),f=parseInt(exList[e].getAttribute("stop"),10);if(p<d&&d<f||f>g&&g>p){var m=p,n=d;f>g?(m=g,n=f,f=g):p=d;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+l+b.cleanID(a.getAttribute("ID"))+" rect#ExNC"+exList[e].getAttribute("ID")).attr("x",function(a){return b.xScale(m)}).attr("width",function(a){return b.xScale(n)-b.xScale(m)})}d3.select("#Level"+
b.gsvg.levelNumber+b.trackClass).select("g#"+l+b.cleanID(a.getAttribute("ID"))+" rect#Ex"+exList[e].getAttribute("ID")).attr("x",function(a){return b.xScale(p)}).attr("width",function(a){return b.xScale(f)-b.xScale(p)});if(0<e){var y=b.xScale(exList[e-1].getAttribute("stop")),E=b.xScale(exList[e].getAttribute("start"));if("1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand")){var F=">";"-1"==a.getAttribute("strand")&&(F="<");var A=
F,C=E-y,B=0;if(7>C)A="";else for(C-=7;8<C;)0==B?(A+=" ",B=1):(A+=F,B=0),C-=7;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+l+b.cleanID(a.getAttribute("ID"))+" #IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("dx",y+1).text(A)}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+l+b.cleanID(a.getAttribute("ID"))+" line#Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("x1",y).attr("x2",E)}}}),1==b.density?b.svg.attr("height",
30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.trx"+b.gsvg.levelNumber).size()+1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1));b.redrawSelectedArea()};b.updateData=function(a){b.xScale.domain();b.xScale.domain();d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+b.gsvg.folderName+"/refSeq.xml",function(c,d){if(c)if(10>a){var g=2500;1==a&&(g=1E4);setTimeout(function(){b.updateData(a+1)},g)}else d3.select("#Level"+
b.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass);else if(d){g=d.documentElement.getElementsByTagName("Gene");for(var l=[],h=[],e=0,f=0;f<g.length;f++)g[f]&&(l[e]=g[f],l[e].setAttribute("ID",e),h[g[f].getAttribute("geneSymbol")]=1,e++);for(f=0;f<b.data.length;f++)b.data[f]&&!h[b.data[f].getAttribute("geneSymbol")]&&
(l[e]=b.data[f],l[e].setAttribute("ID",e),e++);b.draw(l);b.hideLoading();b.getDisplayedData();setTimeout(function(){DisplayRegionReport()},300)}else b.updateData(a+1),b.hideLoading()})};b.drawTrx=function(a,c){var d=parseInt(a.getAttribute("cdsStart"),10),g=parseInt(a.getAttribute("cdsStop"),10),l="";1==b.gsvg.levelNumber&&(l="tx");l=b.svg.select("#"+l+b.cleanID(a.getAttribute("ID")));exList=getAllChildrenByName(getFirstChildByName(a,"exonList"),"exon");for(var h=0;h<exList.length;h++){var e=parseInt(exList[h].getAttribute("start"),
10),f=parseInt(exList[h].getAttribute("stop"),10);if(e<d&&d<f||f>g&&g>e){var m=0,n=0,w=0,u=0;e<d?(m=b.xScale(e),n=b.xScale(d)-b.xScale(e),w=b.xScale(d),u=b.xScale(f)-b.xScale(d)):f>g&&(w=b.xScale(e),u=b.xScale(g)-b.xScale(e),m=b.xScale(g),n=b.xScale(f)-b.xScale(g));l.append("rect").attr("x",m).attr("y",2.5).attr("height",5).attr("width",n).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"ExNC"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor",
"pointer");l.append("rect").attr("x",w).attr("height",10).attr("width",u).attr("title",function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}else{m=10;n=0;if(e<d&&f<d||f>g&&e>g)m=5,n=2.5;l.append("rect").attr("x",function(a){return b.xScale(exList[h].getAttribute("start"))}).attr("y",n).attr("height",m).attr("width",function(a){return b.xScale(exList[h].getAttribute("stop"))-b.xScale(exList[h].getAttribute("start"))}).attr("title",
function(a){return exList[h].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[h].getAttribute("ID")}).style("fill",b.color).style("cursor","pointer")}if(0<h&&(l.append("line").attr("x1",function(a){return b.xScale(exList[h-1].getAttribute("stop"))}).attr("x2",function(a){return b.xScale(exList[h].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(a){return"Int"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("stroke",b.color).attr("stroke-width",
"2"),"1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand"))){e=">";if("-"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))e="<";f=e;m=b.xScale(exList[h-1].getAttribute("stop"));n=b.xScale(exList[h].getAttribute("start"))-m;w=0;if(7>n)f="";else for(n-=7;8<n;)0==w?(f+=" ",w=1):(f+=e,w=0),n-=7;l.append("svg:text").attr("id",function(a){return"IntTxt"+exList[h-1].getAttribute("ID")+"_"+exList[h].getAttribute("ID")}).attr("dx",
m+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(f)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yArr=[];b.yArr[0]=[];for(var c=0;c<b.gsvg.width;c++)b.yMaxArr[c]=0,b.yArr[0][c]=0;b.svg.selectAll(".gene").remove();b.svg.selectAll(".trx"+b.gsvg.levelNumber).remove();var d=b.drawAs;c=b.xScale.domain()[0];if(b.xScale.domain()[1]-c<b.trxCutoff||$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.drawAs="Trx";if(0!=a&&"Gene"===
b.drawAs)b.drawnAs="Gene",b.label="Ref Seq Genes",b.updateLabel(b.label),b.redrawLegend(),a=b.svg.selectAll(".gene").data(a,key).attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+","+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"}),a.enter().append("g").attr("class","gene").attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+","+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),
10),c)+")"}).append("rect").attr("height",10).attr("rx",1).attr("ry",1).attr("width",function(a){return b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))}).attr("title",function(a){return a.getAttribute("ID")}).attr("stroke",b.colorStroke).attr("stroke-width","1").attr("id",function(a){return b.cleanID(a.getAttribute("ID"))}).style("fill",b.color).on("dblclick",b.zoomToFeature).on("mouseover",function(a){0==b.gsvg.isToolTip&&(overSelectable=1,$("#mouseHelp").html("<B>Double Click</B> to zoom in on this feature."),
...
tt.transition().delay(500).duration(200).style("opacity",0)}),a.exit().remove(),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").each(function(a){if("undefined"!==typeof a){var b=d3.select(this);if("1"==a.getAttribute("strand")||"-1"==a.getAttribute("strand")||"-"==a.getAttribute("strand")||"+"==a.getAttribute("strand")){var c=">";if("-"==a.getAttribute("strand")||"-1"==a.getAttribute("strand"))c="<";var d=b.select("rect").attr("width");if(8.5>d)a="";else for(d-=8.5,a=c;8.7<
d;)a+=c,d-=8.5;b.append("svg:text").attr("dx","1").attr("dy","10").style("pointer-events","none").text(a)}}}),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").size()+1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1));else if(0!=a&&"Trx"===b.drawAs){b.drawnAs="Trx";b.label="Ref Seq Transcripts";b.updateLabel(b.label);b.redrawLegend();var e=[],l=0;for(c=0;c<a.length;c++)for(var h=getAllChildrenByName(getFirstChildByName(a[c],
"TranscriptList"),"Transcript"),f=0;f<h.length;f++)e[l]=h[f],e[l].parent=a[c],l++;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".trx"+b.gsvg.levelNumber).remove();a=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".trx"+b.gsvg.levelNumber).data(e,key).attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+","+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"});a.enter().append("g").attr("class","trx"+
b.gsvg.levelNumber).attr("transform",function(a,c){return"translate(0,"+b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c)+")"}).attr("id",function(a){var c="";1==b.gsvg.levelNumber&&(c="tx");return c+b.cleanID(a.getAttribute("ID"))}).on("mouseover",function(a){0==b.gsvg.isToolTip&&(d3.select(this).selectAll("line").style("stroke","green"),d3.select(this).selectAll("rect").style("fill","green"),d3.select(this).selectAll("text").style("opacity","0.3").style("fill",
"green"),tt.transition().duration(200).style("opacity",1),tt.html(b.createToolTip(a)).style("left",function(){return b.positionTTLeft(d3.event.pageX)}).style("top",function(){return b.positionTTTop(d3.event.pageY)}),"undefined"!==typeof a&&b.setupToolTipSVG(a.parent,.05))}).on("mouseout",function(a){d3.select(this).selectAll("line").style("stroke",function(a){return b.color(a)});d3.select(this).selectAll("rect").style("fill",function(a){return b.color(a)});d3.select(this).selectAll("text").style("opacity",
...
for(var e={},l=0;l<b.tissues.length;l++){var h=new String(b.tissues[l]);-1<h.indexOf("Affy")&&(h=h.substr(0,h.indexOf("Affy")));e[h]=1;var f=Math.floor(255*d.getAttribute(h));f=d3.rgb(f,0,0);var m=Math.floor(2.55*a.getAttribute(h));m=d3.rgb(0,m,0);c=c+"<TR><TD>"+h+'</TD><TD style="background:'+f+';color:white;">'+d.getAttribute(h)+'</TD><TD style="background:'+m+';color:white;">'+a.getAttribute(h)+"%</TD></TR>"}if(b.tissues.length<b.tissuesAll.length)for(c+='<TR><TD colspan="3">Other Tissues:</TD></TR>',
l=0;l<b.tissuesAll.length;l++)h=new String(b.tissuesAll[l]),-1<h.indexOf("Affy")&&(h=h.substr(0,h.indexOf("Affy"))),1!=e[h]&&(f=Math.floor(255*d.getAttribute(h)),f=d3.rgb(f,0,0),m=Math.floor(2.55*a.getAttribute(h)),m=d3.rgb(0,m,0),c=c+"<TR><TD>"+h+'</TD><TD style="background:'+f+';color:white;">'+d.getAttribute(h)+'</TD><TD style="background:'+m+';color:white;">'+a.getAttribute(h)+"%</TD></TR>");return c+"</table>"};b.updateSettingsFromUI=function(){0<$("#"+b.trackClass+"Dense"+b.level+"Select").length?
...
Math.round(b.xScale(a));0>a&&(a=0);c=Math.round(b.xScale(c));c>=b.gsvg.width&&(c=b.gsvg.width-1);d=a-b.xPadding;0>d&&(d=0);e=c+b.xPadding;e>=b.gsvg.width&&(e=b.gsvg.width-1);e+g<b.gsvg.width?(e+=g,c+=g):0<d-g&&(d-=g,a-=g);g=0;for(var h=d;h<=e;h++)b.yMaxArr[h]>g&&(g=b.yMaxArr[h]);g++;var p=g;if(b.yArr.length<=g)for(b.yArr[p]=[],h=0;h<b.gsvg.width;h++)b.yArr[p][h]=0;h=g-b.scanBackYLines;1>h&&(h=1);p=0;for(var f=h;f<g&&0==p;f++){var m=0;for(h=d;h<=e&&0==m;h++)b.yArr[f][h]>m&&(m=1);0==m&&(g=f,p=1)}g>
...
">";"-1"==a.getAttribute("strand")&&(g="<");var l="";if(7.5<=d&&15>=d)l=g;else if(15<d)for(d-=7.5;7.5<d;)l+=g,d-=7.5;c.select("text#strand").text(l);if(2==b.density||3==b.density){var p=a.getAttribute("ID");0===c.select("text#lblTxt").size()?c.append("svg:text").attr("dx",function(){var c=b.xScale(a.getAttribute("stop"))+8*p.length+5,d=b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5;c>b.gsvg.width&&(d=-8*p.length);return d}).attr("dy",10).attr("id","lblTxt").text(p):c.select("text#lblTxt").attr("dx",
...
return c}).attr("fill",function(a){return b.color(a,"")});b.svg.selectAll("g.probe").each(function(a){var c=d3.select(this),d=">";"-1"==a.getAttribute("strand")&&(d="<");var g="",e=c.select("rect").attr("width");if(7.5<=e&&15>=e)g=d;else if(15<e)for(e-=7.5;7.5<e;)g+=d,e-=7.5;c.select("text").text(g);if(2==b.density||3==b.density){var l=a.getAttribute("ID");0===c.select("text#lblTxt").size()?c.append("svg:text").attr("dx",function(){var c=b.xScale(a.getAttribute("stop"))+8*l.length+5,d=b.xScale(a.getAttribute("stop"))-
b.xScale(a.getAttribute("start"))+5;c>b.gsvg.width&&(d=-8*l.length);return d}).attr("dy",10).attr("id","lblTxt").text(l):c.select("text#lblTxt").attr("dx",function(){var c=b.xScale(a.getAttribute("stop"))+8*l.length+5,d=b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start"))+5;c>b.gsvg.width&&(d=-8*l.length);return d})}else c.selectAll("text#lblTxt").remove()});1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(b.trackYMax+1)):3==b.density&&b.svg.attr("height",15*
...
"#Level"+b.gsvg.levelNumber+b.trackClass));else{if(d){g=d.documentElement.getElementsByTagName("probe");for(var e=[],h=[],f=0,m=0;m<b.data.length;m++)"undefined"!==typeof b.data[m]&&(e[f]=b.data[m],h[b.data[m].getAttribute("ID")]=1,f++);for(m=0;m<g.length;m++)"undefined"!==typeof g[m]&&"undefined"===typeof h[g[m].getAttribute("ID")]&&(e[f]=g[m],f++);b.draw(e)}b.hideLoading()}})};b.draw=function(a){b.data=a;b.colorSelect=b.curColor;b.trackYMax=0;b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(var c=0;c<b.gsvg.width;c++)b.yMaxArr[c]=
0,b.yArr[0][c]=0;if("dabg"==b.colorSelect||"herit"==b.colorSelect){"dabg"==b.colorSelect?b.drawScaleLegend("0%","100%","of Samples DABG","#000000","#00FF00",0):"herit"==b.colorSelect&&b.drawScaleLegend("0","1.0","Probeset Heritability","#000000","#FF0000",0);b.svg.selectAll(".probe").remove();b.svg.selectAll(".tissueLbl").remove();b.tissueLen=b.tissues.length;for(var d=1,e=0;e<b.tissues.length;e++){var l=new String(b.tissues[e]);0<l.indexOf(";")&&(l=l.substr(0,l.indexOf(";")));0<l.indexOf(":")&&(l=
l.substr(0,l.indexOf(":")));b.trackYMax=0;b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(c=0;c<b.gsvg.width;c++)b.yMaxArr[c]=0,b.yArr[0][c]=0;c=l;"BrownAdipose"==c?c="Brown Adipose":"Brain"==c&&(c="Whole Brain");c=new String("Tissue: "+c);d++;b.svg.append("text").attr("class","tissueLbl "+l).attr("x",b.gsvg.width/2-c.length/2*7.5).attr("y",15*d).text(c);0<a.length&&(c=b.svg.selectAll(".probe."+l).data(a,function(a){return keyTissue(a,l)}).attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+
","+(b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c,a.getAttribute("ID").length)+15*d-10)+")"}),c.enter().append("g").attr("class","probe "+l).attr("transform",function(a,c){return"translate("+b.xScale(a.getAttribute("start"))+","+(b.calcY(parseInt(a.getAttribute("start"),10),parseInt(a.getAttribute("stop"),10),c,a.getAttribute("ID").length)+15*d-10)+")"}).append("rect").attr("height",10).attr("rx",1).attr("ry",1).attr("width",function(a){var c=1;1<b.xScale(a.getAttribute("stop"))-
b.xScale(a.getAttribute("start"))&&(c=b.xScale(a.getAttribute("stop"))-b.xScale(a.getAttribute("start")));return c}).attr("id",function(a){return a.getAttribute("ID")+l}).style("fill",function(a){return b.color(a,l)}).style("cursor","pointer").on("dblclick",b.zoomToFeature).on("mouseover",function(a){if(0==b.gsvg.isToolTip){overSelectable=1;$("#mouseHelp").html("<B>Double Click</B> to zoom in on this feature.");var c=d3.select(this);b.curTTColor=c.style("fill");0<c.style("opacity")&&(c.style("fill",
...
0))}),b.svg.selectAll("g.probe."+l).each(function(a){var c=d3.select(this),d=">";"-1"==a.getAttribute("strand")&&(d="<");var g=d,e=c.select("rect").attr("width");if(7.5>e)g="";else for(e-=7.5;8.5<e;)g+=d,e-=7.5;c.append("svg:text").attr("dx","1").attr("id","strand").attr("dy","10").style("pointer-events","none").style("fill","white").text(g);if(2==b.density||3==b.density){var h=a.getAttribute("ID");c.append("svg:text").attr("dx",function(){var c=b.xScale(a.getAttribute("stop"))+8*h.length+5,d=b.xScale(a.getAttribute("stop"))-
...
typeof d[c]&&""!=d[c]){var e=d[c].split(";"),l=a.append("tr").append("td"),h=e[0].substr(5);3<e.length&&0==e[3].indexOf("Default=")&&e[3].substr(8);if(0==e[1].toLowerCase().indexOf("select")){l.append("text").text(h+": ");h=e[1].split(":");e=e[2].split("}");var f=b.trackClass+"Dense"+b.level+"Select";"colorSelect"==h[1]&&(f=b.trackClass+b.level+"colorSelect");for(var m=l.append("select").attr("id",f).attr("name",h[1]),n=0;n<e.length;n++){var t=e[n].substr(1).split(":");2==t.length&&(l=m.append("option").attr("value",
t[1]).text(t[0]),"colorSelect"==h[1]&&t[1]==b.curColor?l.attr("selected","selected"):t[1]==b.density&&l.attr("selected","selected"))}d3.select("select#"+f).on("change",function(){"dabg"==$(this).val()||"herit"==$(this).val()?$("div#affyTissues"+b.level).show():$("div#affyTissues"+b.level).hide();b.updateSettingsFromUI();b.redraw()})}else if(0==e[1].toLowerCase().indexOf("cbx"))for(l=l.append("div").attr("id","affyTissues"+b.level).style("display","none"),l.append("text").text(h+": "),h=e[1].split(":"),
e=e[2].split("}"),n=0;n<e.length;n++)t=e[n].substr(1).split(":"),2==t.length&&(f=l.append("div").style("display","inline-block"),m=f.append("input").attr("type","checkbox").attr("id",t[1]+"CBX"+b.level).attr("name",h[1]).style("margin-left","5px"),f.append("text").text(t[0]),d3.select("input#"+t[1]+"CBX"+b.level).on("change",function(){b.updateSettingsFromUI();b.redraw()}))}"dabg"==b.curColor||"herit"==b.curColor?$("div#affyTissues"+b.level).show():$("div#affyTissues"+b.level).hide();for(d=0;d<b.tissues.length;d++)$("#"+
...
h.append("text").text(f+": ");3<e.length&&0==e[3].indexOf("Default=")&&e[3].substr(8);if(0==e[1].toLowerCase().indexOf("select")){var m=e[1].split(":");e=e[2].split("}");var n=f="";2<m.length&&(f=m[2]);3<m.length&&(n=m[3]);h=h.append("select").attr("id",b.trackClass+f+b.level+n).attr("name",m[1]);for(m=0;m<e.length;m++){var t=e[m].substr(1).split(":");if(2==t.length){var w=h.append("option").attr("value",t[1]).text(t[0]);("Dense"==f&&t[1]==b.density||""==f&&t[1]==b.include)&&w.attr("selected","selected")}}d3.select("#"+
...
"#Level"+b.gsvg.levelNumber+b.trackClass));else{d=g.documentElement.getElementsByTagName("Snp");for(var e=[],p=[],f=0,m=0;m<d.length;m++)"undefined"!==typeof d[m]&&(e[f]=d[m],p[keySNP(d[m])]=1,f++);for(m=0;m<b.data.length;m++)"undefined"!==typeof b.data[m]&&"undefined"===typeof p[keySNP(b.data[m])]&&(e[f]=b.data[m],f++);b.draw(e);b.hideLoading()}})};b.getDisplayedData=function(){var a=d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll(".snp");b.counts=[{value:0,perc:0,names:"SNP"},{value:0,
perc:0,names:"Insertion"},{value:0,perc:0,names:"Deletion"}];var c=[],e=0,l=0;a.each(function(a){var d=b.xScale(a.getAttribute("start")),g=b.xScale(a.getAttribute("stop"));if(0<=d&&d<=b.gsvg.width||0<=g&&g<=b.gsvg.width)d=0,"Insertion"==a.getAttribute("type")?d++:"Deletion"==a.getAttribute("type")&&(d+=2),b.counts[d].value++,c[e]=a,e++,l++});return c};b.draw=function(a){b.data=a;b.trackYMax=0;b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(var c=0;c<b.gsvg.width;c++)b.yMaxArr[c]=0,b.yArr[0][c]=0;c=b.strain;
...
c.trackClass),d3.select("#Level"+c.levelNumber+c.trackClass).attr("height",15),c.gsvg.addTrackErrorRemove(c.svg,"#Level"+c.gsvg.levelNumber+c.trackClass);else{if(g){d=g.documentElement.getElementsByTagName("QTL");for(var e=[],l=[],h=0,f=0;f<d.length;f++)"undefined"!==typeof d[f]&&(e[h]=d[f],l[d[f].getAttribute("ID")]=1,h++);for(f=0;f<c.data.length;f++)"undefined"!==typeof c.data[f]&&"undefined"===typeof l[c.data[f].getAttribute("ID")]&&(e[h]=c.data[f],h++);c.draw(e)}c.hideLoading()}})};c.calcY=function(b,
...
function TranscriptTrack(f,e,m,n){var c=Track(f,e,m,"Selected Gene Transcripts");c.createToolTip=function(b){var a=".";1==b.getAttribute("strand")?a="+":-1==b.getAttribute("strand")&&(a="-");a="ID: "+b.getAttribute("ID")+"<BR>Location: chr"+b.getAttribute("chromosome")+":"+numberWithCommas(b.getAttribute("start"))+"-"+numberWithCommas(b.getAttribute("stop"))+"<BR>Strand: "+a;-1==(new String(b.getAttribute("ID"))).indexOf("ENS")&&(b=getFirstChildByName(getFirstChildByName(b,"annotationList"),"annotation"),
...
"exonList"),"exon");for(var l=0;l<exList.length;l++){var h=parseInt(exList[l].getAttribute("start"),10),f=parseInt(exList[l].getAttribute("stop"),10);if(h<g&&g<f||f>d&&d>h){var m=0,n=0,t=0,w=0;h<g?(m=c.xScale(exList[l].getAttribute("start")),n=c.xScale(g)-c.xScale(exList[l].getAttribute("start")),t=c.xScale(g),w=c.xScale(exList[l].getAttribute("stop"))-c.xScale(g)):f>d&&(t=c.xScale(exList[l].getAttribute("start")),w=c.xScale(d)-c.xScale(exList[l].getAttribute("start")),m=c.xScale(d),n=c.xScale(exList[l].getAttribute("stop"))-
c.xScale(d));e.append("rect").attr("x",m).attr("y",2.5).attr("height",5).attr("width",n).attr("title",function(a){return exList[l].getAttribute("ID")}).attr("id",function(a){return"ExNC"+exList[l].getAttribute("ID")}).style("fill",c.color).style("cursor","pointer");e.append("rect").attr("x",t).attr("height",10).attr("width",w).attr("title",function(a){return exList[l].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[l].getAttribute("ID")}).style("fill",c.color).style("cursor","pointer")}else{m=
10;n=0;if(h<g&&f<g||f>d&&h>d)m=5,n=2.5;e.append("rect").attr("x",function(a){return c.xScale(exList[l].getAttribute("start"))}).attr("y",n).attr("height",m).attr("width",function(a){return c.xScale(exList[l].getAttribute("stop"))-c.xScale(exList[l].getAttribute("start"))}).attr("title",function(a){return exList[l].getAttribute("ID")}).attr("id",function(a){return"Ex"+exList[l].getAttribute("ID")}).style("fill",c.color).style("cursor","pointer")}if(0<l){e.append("line").attr("x1",function(a){return c.xScale(exList[l-
1].getAttribute("stop"))}).attr("x2",function(a){return c.xScale(exList[l].getAttribute("start"))}).attr("y1",5).attr("y2",5).attr("id",function(a){return"Int"+exList[l-1].getAttribute("ID")+"_"+exList[l].getAttribute("ID")}).attr("stroke",c.color).attr("stroke-width","2");h=">";"-1"==b.getAttribute("strand")&&(h="<");f=h;m=c.xScale(exList[l-1].getAttribute("stop"));n=c.xScale(exList[l].getAttribute("start"))-m;t=0;if(7>n)f="";else for(n-=7;8<n;)0==t?(f+=" ",t=1):(f+=h,t=0),n-=7;e.append("svg:text").attr("id",
function(a){return"IntTxt"+exList[l-1].getAttribute("ID")+"_"+exList[l].getAttribute("ID")}).attr("dx",m+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",c.color).style("font-size","16px").text(f)}}};c.redraw=function(){d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll("g.trx"+c.gsvg.levelNumber).each(function(b,a){var g=parseInt(b.getAttribute("start"),10),d=parseInt(b.getAttribute("stop"),10);"undefined"!==typeof b.getAttribute("cdsStart")&&"undefined"!==
typeof b.getAttribute("cdsStop")&&(g=parseInt(b.getAttribute("cdsStart"),10),d=parseInt(b.getAttribute("cdsStop"),10));exList=getAllChildrenByName(getFirstChildByName(b,"exonList"),"exon");for(var e=0;e<exList.length;e++){var l=parseInt(exList[e].getAttribute("start"),10),h=parseInt(exList[e].getAttribute("stop"),10);if(l<g&&g<h||h>d&&d>l){var f=l,m=g;h>d?(f=d,m=h,h=d):l=g;d3.select("#Level"+c.gsvg.levelNumber+c.trackClass+" g#tx"+b.getAttribute("ID")+" rect#ExNC"+exList[e].getAttribute("ID")).attr("x",
function(a){return c.xScale(f)}).attr("width",function(a){return c.xScale(m)-c.xScale(f)})}d3.select("#Level"+c.gsvg.levelNumber+c.trackClass+" g#tx"+b.getAttribute("ID")+" rect#Ex"+exList[e].getAttribute("ID")).attr("x",function(a){return c.xScale(l)}).attr("width",function(a){return c.xScale(h)-c.xScale(l)});if(0<e){var n=">";"-1"==b.getAttribute("strand")&&(n="<");var t=n,w=c.xScale(exList[e-1].getAttribute("stop")),u=c.xScale(exList[e].getAttribute("start")),v=u-w,y=0;if(7>v)t="";else for(v-=
7;8<v;)0==y?(t+=" ",y=1):(t+=n,y=0),v-=7;d3.select("#Level"+c.gsvg.levelNumber+c.trackClass+" g#tx"+b.getAttribute("ID")+" line#Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("x1",w).attr("x2",u);d3.select("#Level"+c.gsvg.levelNumber+c.trackClass+" g#tx"+b.getAttribute("ID")+" #IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID")).attr("dx",w+1).text(t)}}});c.redrawSelectedArea()};c.update=function(){var b=getAllChildrenByName(getFirstChildByName(c.gsvg.selectedData,
...
function HelicosTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#DD0000";f.updateLabel("Brain Helicos RNA Read Counts");f.redrawLegend();f.redraw();return f}function IlluminaSmallTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#99CC99";f.updateLabel("Brain Illumina Small RNA Read Counts");f.redrawLegend();f.redraw();f.redraw();return f}
function IlluminaTotalTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.updateLabel("BNLx/SHR Brain Illumina Total-RNA(rRNA depleted) Read Counts");f.redrawLegend();return f}function LiverIlluminaTotalPlusTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#abaecd";f.updateLabel("BNLx/SHR Liver + Strand Total-RNA Read Counts");f.redrawLegend();f.redraw();return f}
function LiverIlluminaTotalMinusTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#dbdefd";f.updateLabel("BNLx/SHR Liver - Strand Total-RNA Read Counts");f.redrawLegend();f.redraw();return f}function LiverIlluminaSmallTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#7b7e9d";f.updateLabel("Liver  Illumina Small RNA Read Counts");f.redrawLegend();f.redraw();return f}
function HeartIlluminaTotalPlusTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#CC6242";f.updateLabel("BNLx/SHR Heart + Strand Total-RNA Read Counts");f.redrawLegend();f.redraw();return f}function HeartIlluminaTotalMinusTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#FC9272";f.updateLabel("BNLx/SHR Heart - Strand Total-RNA Read Counts");f.redrawLegend();f.redraw();return f}
function HeartIlluminaSmallTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#9C3212";f.updateLabel("Heart Illumina Small RNA Read Counts");f.redrawLegend();f.redraw();return f}function BrainIlluminaTotalPlusTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#7EB5D6";f.updateLabel("Brain + Strand Total-RNA Read Counts");f.redrawLegend();f.redraw();return f}
function BrainIlluminaTotalMinusTrack(f,e,m,n){f=CountTrack(f,e,m,n);f.updateLabel("Brain - Strand Total-RNA Read Counts");f.redrawLegend();f.redraw();return f}
function StrainSpecificIlluminaTotalTrack(f,e,m,n){e=CountTrack(f,e,m,n);n=m.substr(m.indexOf("-")+1);var c=".";0<m.indexOf("Plus")?c="+":0<m.indexOf("Minus")&&(c="-");m=m.substr(0,m.indexOf("illumina"));"brain"===m?m="Whole Brain":"liver"===m&&(m="Liver");e.graphColorText=f.strainSpecificCountColors(n);e.updateLabel(n+" "+m+" "+c+" Strand Total-RNA Read Counts");e.redrawLegend();e.redraw();return e}
function IlluminaPolyATrack(f,e,m,n){f=CountTrack(f,e,m,n);f.graphColorText="#7EB5D6";f.updateLabel("Brain Illumina PolyA+ RNA Read Counts");f.redrawLegend();f.redraw();return f}
function SpliceJunctionTrack(f,e,m,n,c,b){f=GenericTranscriptTrack(f,e,m,n,c,b);f.dataFileName=m+".xml";f.xmlTag="Feature";f.xmlTagBlockElem="block";f.idPrefix="splcJnct";"liverspliceJnct"===m?f.idPrefix="lvrsplcJnct":"heartspliceJnct"===m&&(f.idPrefix="hrtsplcJnct");f.ttSVG=1;f.ttTrackList=[];f.ttTrackList.push(m);f.ttTrackList.push("ensemblcoding");f.ttTrackList.push("braincoding");f.ttTrackList.push("brainTotal");f.ttTrackList.push("liverTotal");f.ttTrackList.push("heartTotal");f.ttTrackList.push("mergedTotal");
f.ttTrackList.push("refSeq");f.ttTrackList.push("repeatMask");"splcJnct"===m?f.ttTrackList.push("illuminaPolyA"):"liverspliceJnct"===m?(f.ttTrackList.push("liverilluminaTotalPlus"),f.ttTrackList.push("liverilluminaTotalMinus")):"heartspliceJnct"===m?(f.ttTrackList.push("heartilluminaTotalPlus"),f.ttTrackList.push("heartilluminaTotalMinus")):"brainspliceJnct"===m&&(f.ttTrackList.push("brainilluminaTotalPlus"),f.ttTrackList.push("brainilluminaTotalMinus"));f.colorValueField="readCount";f.colorScale=
...
var c=getAllChildrenByName(getFirstChildByName(a,"blockList"),"block"),e=0,l=1,h="start",f="stop";-1==a.getAttribute("strand")&&(e=1,l=0,h="stop",f="start");b=b+"<BR><BR>Coverage Exon 1: "+numberWithCommas(c[e].getAttribute(h))+"-"+numberWithCommas(c[e].getAttribute(f))+" ("+(parseInt(c[e].getAttribute("stop"),10)-parseInt(c[e].getAttribute("start"),10))+" bp)";b=b+"<BR>Coverage Exon 2: "+numberWithCommas(c[l].getAttribute(h))+"-"+numberWithCommas(c[l].getAttribute(f))+" ("+(parseInt(c[l].getAttribute("stop"),
10)-parseInt(c[l].getAttribute("start"),10))+" bp)";return b=b+"<BR>Intron: "+numberWithCommas(parseInt(c[1].getAttribute("start"),10)-parseInt(c[0].getAttribute("stop"),10))+" bp<BR>"};f.drawScaleLegend("1","1000+","Read Counts","#E6E6E6","#000000",0);f.draw(e);return f}
function CustomCountTrack(f,e,m,n,c){var b=CountTrack(f,e,m,n);b.graphColorText="#4E85A6";b.updateControl=0;f="Custom Count Track";c=c.split(",");if(0<c.length)for(e=0;e<c.length;e++)0==e?b.dataFileName=c[e].substr(9):c[e].indexOf("Name=")&&(f=c[e].substr(5));b.updateFullData=function(a,c){if(b.updateControl==a){b.updateControl=a+1;var d=b.xScale.domain()[0],g=b.xScale.domain()[1],e=g-d;b.showLoading();b.bin=b.calculateBin(e);var h=dataPrefix+"tmpData/trackXML/"+b.gsvg.folderName+"/tmp/"+d+"_"+g+
".count."+b.trackClass+".xml",f=dataPrefix+"tmpData/trackUpload/"+b.trackClass.substr(6),m="bg",n="";0<b.bin&&(d-=2*b.bin,d-=d%(2*b.bin),g+=2*b.bin,g+=2*b.bin-g%(2*b.bin),h=dataPrefix+"tmpData/trackXML/"+b.gsvg.folderName+"/"+d+"_"+g+".bincount."+b.bin+"."+b.trackClass+".xml");-1<b.dataFileName.indexOf("http")&&(n=f=b.dataFileName,m="bw");d3.xml(h,function(e,l){if(e){if(0==a||1==c){var p="/"+pathPrefix;pathPrefix||(p="");$.ajax({url:p+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,
minCoord:d,maxCoord:g,myOrganism:organism,genomeVer:genomeVer,track:b.trackClass,bedFile:f,outFile:h,folder:b.gsvg.folderName,binSize:b.bin,type:m,web:n},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackCount")},error:function(a,b,c){}})}8>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1)},500):30>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1)},1E3):32>a?b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1)},
1E4):(d3.select("#Level"+b.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+b.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass),b.hideLoading(),b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},15E3))}else null==l?32<=a?(p=[],b.draw(p)):b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},5E3):(b.fullDataTimeOutHandle=0,b.loadedDataMin=
d,b.loadedDataMax=g,p=l.documentElement.getElementsByTagName("Count"),b.draw(p),b.updateControl=0)})}};b.updateLabel(f);b.redrawLegend();b.redraw();return b}
function CountTrack(f,e,m,n){var c=Track(f,e,m,"Generic Counts");c.loadedDataMin=c.xScale.domain()[0];c.loadedDataMax=c.xScale.domain()[1];c.dataFileName=c.trackClass;c.scaleMin=1;c.scaleMax=5E3;c.graphColorText="steelblue";c.colorScale=d3.scaleLinear().domain([c.scaleMin,c.scaleMax]).range(["#EEEEEE","#000000"]);c.ttSVG=1;c.data=e;c.density=n;c.prevDensity=n;c.displayBreakDown=null;f=c.gsvg.xScale.domain()[0];f=c.gsvg.xScale.domain()[1]-f;c.fullDataTimeOutHandle=0;c.ttTrackList=[];-1<m.indexOf("illuminaSmall")?
...
c.bin);c.xScale(b);c.prevMinCoord=b;c.prevMaxCoord=a;b=c.xScale.domain()[0];a=c.xScale.domain()[1];for(var g=[],d=0,e=0,l=0;l<c.data.length;l++)if("undefined"!==typeof c.data[l]){var h=parseInt(c.data[l].getAttribute("start"),10),f=parseInt(c.data[l].getAttribute("stop"),10),m=parseInt(c.data[l].getAttribute("count"),10);if(1!=c.density||1==c.density&&h!=f)if(l+1<c.data.length){if(f=parseInt(c.data[l+1].getAttribute("start"),10),h>=b&&h<=a||f>=b&&f<=a)g[d]=c.data[l],m>e&&(e=m),d++}else h>=b&&h<=a&&
(g[d]=c.data[l],m>e&&(e=m),d++)}1==c.density?(c.density!=c.prevDensity?(c.redrawLegend(),a=c.gsvg.xScale.domain()[1],c.prevDensity=c.density,c.svg.selectAll(".area").remove(),c.svg.selectAll("g.y").remove(),c.svg.selectAll(".grid").remove(),c.svg.selectAll(".leftLbl").remove(),b=c.svg.selectAll("."+c.trackClass).data(g,keyStart),b.enter().append("rect").attr("x",function(a){return c.xScale(a.getAttribute("start"))}).attr("y",15).attr("class",c.trackClass).attr("height",10).attr("width",function(a,
...
c.bin=c.calculateBin(e);e=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.gsvg.folderName+"/tmp/"+g+"_"+d+".count."+c.trackClass+".xml";0<c.bin&&(g-=2*c.bin,g-=g%(2*c.bin),d+=2*c.bin,d+=2*c.bin-d%(2*c.bin),e=dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+c.gsvg.folderName+"/tmp/"+g+"_"+d+".bincount."+c.bin+"."+c.trackClass+".xml");d3.xml(e,function(e,h){if(e){if(0==b||1==a){var l="/"+pathPrefix;pathPrefix||(l="");tmpPanel=panel;-1<c.trackClass.indexOf("-")&&(tmpPanel=c.trackClass.substr(c.trackClass.indexOf("-")+
1));$.ajax({url:l+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:g,maxCoord:d,panel:tmpPanel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:c.trackClass,folder:c.gsvg.folderName,binSize:c.bin},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackCount")},error:function(a,b,c){}})}8>b?c.fullDataTimeOutHandle=setTimeout(function(){c.updateFullData(b+1,0)},500):30>b?c.fullDataTimeOutHandle=setTimeout(function(){c.updateFullData(b+
1,0)},1E3):32>b?c.fullDataTimeOutHandle=setTimeout(function(){c.updateFullData(b+1,0)},1E4):(d3.select("#Level"+c.levelNumber+c.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+c.trackClass),d3.select("#Level"+c.levelNumber+c.trackClass).attr("height",15),c.gsvg.addTrackErrorRemove(c.svg,"#Level"+c.gsvg.levelNumber+c.trackClass),c.hideLoading(),c.fullDataTimeOutHandle=setTimeout(function(){c.updateFullData(b+1,1)},15E3))}else null==h?32<=b?(l=[],c.draw(l)):c.fullDataTimeOutHandle=
setTimeout(function(){c.updateFullData(b+1,0)},5E3):(c.fullDataTimeOutHandle=0,c.loadedDataMin=g,c.loadedDataMax=d,l=h.documentElement.getElementsByTagName("Count"),c.draw(l))})};c.updateCountScale=function(b,a){c.scaleMin=b;c.scaleMax=a;c.colorScale=d3.scaleLinear().domain([c.scaleMin,c.scaleMax]).range(["#EEEEEE","#000000"]);1==c.density&&(c.redrawLegend(),c.redraw())};c.setupToolTipSVG=function(b,a){var g=c.xScale.domain()[0],d=c.xScale.domain()[1],e=parseInt(b.getAttribute("start"),10),l=parseInt(b.getAttribute("stop"),
10);d=Math.floor((d-g)*a);20>d&&(d=20);g=e-d;d=l+d;1>g&&(g=1);"undefined"!==typeof c.ttSVGMinWidth&&d-g<c.ttSVGMinWidth&&(g=e-c.ttSVGMinWidth/2,d=l+c.ttSVGMinWidth/2);e=toolTipSVG("div#ttSVG",450,g,d,99,b.getAttribute("ID"),"transcript");e.addTrack(c.trackClass,3,"",c.data);for(l=0;l<c.ttTrackList.length;l++){var h=c.gsvg.getTrackData(c.ttTrackList[l]),f=[];if("undefined"!==typeof h&&0<h.length){for(var m=0,n=0;n<h.length;n++)if(g<=parseInt(h[n].getAttribute("start"),10)&&parseInt(h[n].getAttribute("start"),
10)<=d||parseInt(h[n].getAttribute("start"),10)<=g&&parseInt(h[n].getAttribute("stop"),10)>=g)f[m]=h[n],m++;0<f.length&&e.addTrack(c.ttTrackList[l],3,"DrawTrx",f)}}};c.draw=function(b){c.hideLoading();c.data=b;var a=c.gsvg.xScale.domain()[0],g=c.gsvg.xScale.domain()[1];c.xScale(a+c.bin);c.xScale(a);c.redrawLegend();for(var d=[],e=0,l=0,h=0;h<b.length;h++)if("undefined"!==typeof b[h]){var f=parseInt(b[h].getAttribute("start"),10),m=parseInt(b[h].getAttribute("stop"),10),n=parseInt(b[h].getAttribute("count"),
10);if(1!=c.density||1==c.density&&f!=m)if(h+1<b.length){if(m=parseInt(b[h+1].getAttribute("start"),10),f>=a&&f<=g||m>=a&&m<=g)d[e]=b[h],n>l&&(l=n),e++}else f>=a&&f<=g&&(d[e]=b[h],n>l&&(l=n),e++)}b=d;c.svg.selectAll("."+c.trackClass).remove();c.svg.select(".y.axis").remove();c.svg.select("g.grid").remove();c.svg.selectAll(".leftLbl").remove();c.svg.select(".area").remove();c.svg.selectAll(".area").remove();1==c.density?(g=c.gsvg.xScale.domain()[1],c.svg.selectAll("."+c.trackClass).data(b,keyStart).enter().append("rect").attr("x",
function(a){return c.xScale(a.getAttribute("start"))}).attr("y",15).attr("class",c.trackClass).attr("height",10).attr("width",function(a,b){return c.xScale(a.getAttribute("stop"))-c.xScale(a.getAttribute("start"))}).attr("fill",c.color).on("mouseover",function(a){0==c.gsvg.isToolTip&&(d3.select(this).style("fill","green"),tt.transition().duration(200).style("opacity",1),tt.html(c.createToolTip(a)).style("left",function(){return c.positionTTLeft(d3.event.pageX)}).style("top",function(){return c.positionTTTop(d3.event.pageY)}),
1==c.ttSVG&&c.setupToolTipSVG(a,.005))}).on("mouseout",function(a){d3.select(this).style("fill",c.color);tt.transition().delay(500).duration(200).style("opacity",0)}),c.svg.attr("height",30)):2==c.density&&(c.yScale.domain([0,l]),c.yAxis=d3.axisLeft(c.yScale).ticks(5),c.svg.select("g.grid").remove(),c.svg.select(".y.axis").remove(),c.svg.selectAll(".leftLbl").remove(),c.svg.append("g").attr("class","y axis").call(c.yAxis),c.svg.select("g.y").selectAll("text").each(function(a){var b=new String(a);
c.svg.append("svg:text").attr("class","leftLbl").attr("x",c.gsvg.width-(7.8*b.length+5)).attr("y",function(){return c.yScale(a)}).attr("dy","0.01em").style("font-weight","bold").text(numberWithCommas(a));d3.select(this).attr("x",function(){return 7.7*b.length+5}).attr("dy","0.05em")}),c.svg.append("g").attr("class","grid").call(c.yAxis.tickSize(-c.gsvg.width+10,0,0).tickFormat("")),c.svg.select(".area").remove(),c.area=d3.area().x(function(a){return c.xScale(a.getAttribute("start"))}).y0(140).y1(function(a,
b){return c.yScale(a.getAttribute("count"))}),c.svg.append("path").datum(b).attr("class","area").attr("stroke",c.graphColorText).attr("fill",c.graphColorText).attr("d",c.area),c.svg.attr("height",140));c.redrawSelectedArea()};c.redrawLegend=function(){if(2==c.density)d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll(".legend").remove();else if(1==c.density){var b=new String(c.label);b=c.gsvg.width/2+b.length/2*7.5-10;var a=new String("<"+c.scaleMin);c.drawScaleLegend(c.scaleMin,numberWithCommas(c.scaleMax)+
"+","Read Counts","#EEEEEE","#00000",15+7.6*a.length);c.svg.append("text").text("<"+c.scaleMin).attr("class","legend").attr("x",b).attr("y",12);c.svg.append("rect").attr("class","legend").attr("x",b+7.6*a.length+5).attr("y",0).attr("rx",2).attr("ry",2).attr("height",12).attr("width",15).attr("fill","#FFFFFF").attr("stroke","#CECECE")}};c.redrawSelectedArea=function(){if(1<c.density){var b=c.svg.attr("height");d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll(".selectedArea").remove();
if(-1<c.selectionStart&&-1<c.selectionEnd){var a=c.xScale(c.selectionStart),g=c.xScale(c.selectionEnd)-a;1>g&&(g=1);d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).append("rect").attr("class","selectedArea").attr("x",a).attr("y",0).attr("height",b).attr("width",g).attr("fill","#CECECE").attr("opacity",.3).attr("pointer-events","none")}}else d3.select("#Level"+c.gsvg.levelNumber+c.trackClass).selectAll(".selectedArea").remove()};c.savePrevious=function(){c.prevSetting={};c.prevSetting.density=
c.density;c.prevSetting.scaleMin=c.scaleMin;c.prevSetting.scaleMax=c.scaleMax};c.revertPrevious=function(){c.density=c.prevSetting.density;c.scaleMin=c.prevSetting.scaleMin;c.scaleMax=c.prevSetting.scaleMax};c.generateTrackSettingString=function(){return c.trackClass+","+c.density+","+c.include+";"};c.generateSettingsDiv=function(b){var a=trackInfo[c.trackClass];c.savePrevious();d3.select(b).select("table").select("tbody").html("");if(a&&a.Controls&&0<a.Controls.length){var g=(new String(a.Controls)).split(",");
b=d3.select(b).select("table").select("tbody");b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+a.Name);for(a=0;a<g.length;a++)if("undefined"!==typeof g[a]&&""!=g[a]){var d=g[a].split(";"),e=b.append("tr").append("td"),l=d[0].substr(5),h="";3<d.length&&0==d[3].indexOf("Default=")&&(h=d[3].substr(8));if(0==d[1].toLowerCase().indexOf("select")){e.append("text").text(l+": ");var f=d[1].split(":");d=d[2].split("}");l=c.trackClass+"Dense"+c.level+"Select";"colorSelect"==
f[1]&&(l=c.trackClass+c.level+"colorSelect");e=e.append("select").attr("id",l).attr("name",f[1]);for(f=0;f<d.length;f++){var m=d[f].substr(1).split(":");if(2==m.length){var n=e.append("option").attr("value",m[1]).text(m[0]);-1<l.indexOf("Dense")?m[1]==c.density&&n.attr("selected","selected"):m[1]==h&&n.attr("selected","selected")}}d3.select("select#"+l).on("change",function(){$(this).attr("id")==c.trackClass+"Dense"+c.level+"Select"&&1==$(this).val()?$("#scaleControl"+c.level).show():$("#scaleControl"+
c.level).hide();c.updateSettingsFromUI();c.redraw()})}else 0==d[1].toLowerCase().indexOf("slider")&&(h="none",1==c.density&&(h="inline-block"),e=e.append("div").attr("id","scaleControl"+c.level).style("display",h),e.append("text").text(l+": "),e.append("input").attr("type","text").attr("id","amount").attr("value",c.scaleMin+"-"+c.scaleMax).style("border",0).style("color","#f6931f").style("font-weight","bold").style("background-color","#EEEEEE"),f=d[1].split(":"),d=d[2].split("}"),e=e.append("div"),
e.append("text").text("Min:"),e.append("div").attr("id","min-"+f[1]).style("width","60%").style("display","inline-block").style("float","right"),e.append("br"),e.append("text").text("Max:"),e.append("div").attr("id","max-"+f[1]).style("width","60%").style("display","inline-block").style("float","right"),$("#min-"+f[1]).slider({min:1,max:1E3,step:1,value:c.scaleMin,slide:c.processSlider}),$("#max-"+f[1]).slider({min:1E3,max:2E4,step:100,value:c.scaleMax,slide:c.processSlider}),c.updateSettingsFromUI(),
...
a)});g.append("input").attr("type","button").attr("value","Apply").style("float","right").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");c.density==c.prevSetting.density&&c.scaleMin==c.prevSetting.scaleMin&&c.scaleMax==c.prevSetting.scaleMax||c.gsvg.setCurrentViewModified()});g.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left","5px").on("click",function(){c.revertPrevious();c.updateCountScale(c.scaleMin,
c.scaleMax);c.draw(c.data);$("#trackSettingDialog").fadeOut("fast")})}else b=d3.select(b).select("table").select("tbody"),b.append("tr").append("td").style("font-weight","bold").html("Track Settings: "+a.Name),b.append("tr").append("td").html("Sorry no settings for this track."),g=b.append("tr").append("td"),g.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){c.gsvg.setCurrentViewModified();$("#trackSettingDialog").fadeOut("fast")}),
g.append("input").attr("type","button").attr("value","Cancel").style("float","right").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast")})};c.processSlider=function(b,a){var g=$("#min-rangeslider").slider("value"),d=$("#max-rangeslider").slider("value");$("#amount").val(g+" - "+d);c.updateCountScale(g,d)};c.yScale=d3.scaleLinear().range([140,20]).domain([0,d3.max(e,function(b){return b.getAttribute("count")})]);c.area=d3.area().x(function(b){return c.xScale(b.getAttribute("start"))}).y0(140).y1(function(b){return b.getAttribute("count")});
c.yAxis=d3.axisLeft(c.yScale).ticks(5);c.redrawLegend();c.draw(e);return c}
function PolyATrack(f,e,m,n,c,b){var a=GenericTranscriptTrack(f,e,m,n,c,b);a.dataFileName="polyASite.xml";a.density=3;a.minFeatureWidth=1;a.colorBy="Color";a.ttSVG=1;a.ttTrackList=[];a.ttTrackList.push("ensemblcoding");a.ttTrackList.push("braincoding");a.ttTrackList.push("liverTotal");a.ttTrackList.push("heartTotal");a.ttTrackList.push("mergedTotal");a.ttTrackList.push("refSeq");a.ttTrackList.push("repeatMask");a.ttSVGMinWidth=200;a.createToolTip=function(a){var b=".";1==a.getAttribute("strand")?
...
g){if(c)if(3>b){var d=1E4;1==b&&(d=15E3);setTimeout(function(){a.updateData(b+1)},d)}else 3<=b&&(d3.select("#Level"+a.levelNumber+a.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+a.trackClass),d3.select("#Level"+a.levelNumber+a.trackClass).attr("height",15),a.gsvg.addTrackErrorRemove(a.svg,"#Level"+a.gsvg.levelNumber+a.trackClass));else{d=g.documentElement.getElementsByTagName(a.xmlTag);for(var e=[],f=[],p=0,m=0;m<d.length;m++)"undefined"!==typeof d[m]&&(e[p]=d[m],f[d[m].getAttribute("ID")]=
1,p++);for(m=0;m<a.data.length;m++)"undefined"!==typeof a.data[m]&&"undefined"===typeof f[a.data[m].getAttribute("ID")]&&(e[p]=a.data[m],p++);a.draw(e);a.hideLoading()}})};a.redrawLegend=function(){a.drawLegend([{color:"#FF8000",label:"+ Strand >>>"},{color:"#330570",label:"- Strand <<<"}])};a.updateFullData=void 0;a.draw(e);return a}
function CustomTranscriptTrack(f,e,m,n,c,b){var a=GenericTranscriptTrack(f,e,m,n,c,b);f=b.split(",");a.density=c;0<f.length&&(a.dataFileName=f[0].substr(9));a.colorBy=1<f.length?f[1]:"Score";2<f.length&&(a.minValue=f[2]);3<f.length&&(a.maxValue=f[3]);4<f.length&&(a.minColor=f[4]);5<f.length&&(a.maxColor=f[5]);a.colorValueField="score";a.minFeatureWidth=1;a.updateControl=0;"Score"==a.colorBy&&a.createColorScale();a.updateFullData=function(b,c){if(a.updateControl==b){a.updateControl=b+1;var d=a.xScale.domain()[0],
g=a.xScale.domain()[1],e=dataPrefix+"tmpData/trackXML/"+a.gsvg.folderName+"/"+a.dataFileName+".xml",f=dataPrefix+"tmpData/trackUpload/"+a.dataFileName,m="",n=new Date,t="bed";-1<a.dataFileName.indexOf("http")&&(m=a.dataFileName,f="tmpData/tmpDownload/"+n.getTime()+"_"+a.trackClass,e=dataPrefix+"tmpData/trackXML/"+a.gsvg.folderName+"/"+a.trackClass+".xml",t="bb");d3.xml(e,function(h,l){if(h){console.log(h);if(0==b||1==c){var p="/"+pathPrefix;pathPrefix||(p="");$.ajax({url:p+"generateTrackXML.jsp",
type:"GET",cache:!1,data:{chromosome:chr,minCoord:d,maxCoord:g,folder:a.gsvg.folderName,bedFile:f,outFile:e,track:a.trackClass,web:m,type:t},dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackCustomTranscript")},error:function(a,b,c){console.log(c)}})}3>b?(p=1E4,1==b&&(p=15E3),setTimeout(function(){a.updateFullData(b+1,0)},p)):(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+a.trackClass),d3.select("#Level"+
a.gsvg.levelNumber+a.trackClass).attr("height",15),a.gsvg.addTrackErrorRemove(a.svg,"#Level"+a.gsvg.levelNumber+a.trackClass),a.hideLoading())}else p=l.documentElement.getElementsByTagName(a.xmlTag),a.draw(p),a.hideLoading(),a.updateControl=0})}};a.updateSettingsFromUI=function(){0<$("#"+a.trackClass+"Dense"+a.level+"Select").length&&(a.density=$("#"+a.trackClass+"Dense"+a.level+"Select").val());0<$("#"+a.trackClass+a.level+"colorSelect").length&&(a.colorBy=$("#"+a.trackClass+a.level+"colorSelect").val());
...
": ");f=e[1].split(":");e=e[2].split("}");var m=a.trackClass+"Dense"+a.level+"Select";"colorSelect"==f[1]&&(m=a.trackClass+a.level+"colorSelect");h=h.append("select").attr("id",m).attr("name",f[1]);for(f=0;f<e.length;f++){var n=e[f].substr(1).split(":");if(2==n.length){var t=h.append("option").attr("value",n[1]).text(n[0]);(-1<m.indexOf("Dense")&&n[1]==a.density||-1<m.indexOf("colorSelect")&&n[1]==a.colorBy)&&t.attr("selected","selected")}}d3.select("select#"+m).on("change",function(){"Score"==$(this).val()?
$("div."+a.trackClass+"Scale"+a.level).show():"Color"==$(this).val()&&$("div."+a.trackClass+"Scale"+a.level).hide();a.updateSettingsFromUI();a.draw(a.data)})}else 0==e[1].toLowerCase().indexOf("txt")&&(h=0==$("#colorTrack"+a.level).size()?h.append("div").attr("class",a.trackClass+"Scale"+a.level).style("display","none"):d3.select("#"+a.trackClass+"Scale"+a.level),h.append("text").text(f+": "),f=e[1].split(":"),e=e[2].split("}"),e="Data",m="text",n=a.minValue,t=a.maxValue,"color"==f[1]&&(m=e="Color",
n=a.minColor,t=a.maxColor),h.append("input").attr("type",m).attr("id",a.trackClass+"min"+e+a.level).attr("class",f[1]).style("margin-left","5px").attr("value",n),h.append("text").text(" - "),h.append("input").attr("type",m).attr("id",a.trackClass+"max"+e+a.level).attr("class",f[1]).style("margin-left","5px").attr("value",t),"Color"==e&&(testIE||testSafari)?($("#"+a.trackClass+"min"+e+a.level).spectrum({change:function(b){a.updateSettingsFromUI();a.draw(a.data)}}),$("#"+a.trackClass+"max"+e+a.level).spectrum({change:function(b){a.updateSettingsFromUI();
...
function(b){return a.xScale(d[g].getAttribute("stop"))-a.xScale(d[g].getAttribute("start"))});if(0<g){e=">";"-1"==b.getAttribute("strand")&&(e="<");var f=e,p=a.xScale(d[g-1].getAttribute("stop"))-a.xScale(b.getAttribute("start")),m=a.xScale(d[g].getAttribute("start"))-a.xScale(b.getAttribute("start")),n=m-p,u=0;if(7>n)f="";else for(n-=7;8<n;)0==u?(f+=" ",u=1):(f+=e,u=0),n-=7;e=d[g-1].getAttribute("ID")+"_"+d[g].getAttribute("ID");null==d[g].getAttribute("ID")&&(e=b.getAttribute("ID")+"_"+(g-1)+"_"+
g);a.svg.select("g#"+a.idPrefix+"tx"+b.getAttribute("ID")+" line#"+a.idPrefix+"Int"+e).attr("x1",p).attr("x2",m);a.svg.select("g#"+a.idPrefix+"tx"+b.getAttribute("ID")+" #"+a.idPrefix+"IntTxt"+e).attr("dx",p+1).text(f)}}});1==a.density?a.svg.attr("height",30):2==a.density?a.svg.attr("height",15*(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+"trx"+a.gsvg.levelNumber).size()+1)):3==a.density&&a.svg.attr("height",15*(a.trackYMax+1));a.redrawSelectedArea()}};a.drawTrx=
...
"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(d=a.idPrefix+"Int"+b.getAttribute("ID")+"_"+(e-1)+"_"+e);return d});var m=">";"-1"==b.getAttribute("strand")&&(m="<");var n=m;f-=g;var t=0;if(7>f)n="";else for(f-=7;8<f;)0==t?(n+=" ",t=1):(n+=m,t=0),f-=7;d.append("svg:text").attr("id",function(b){var c=a.idPrefix+"IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(c=a.idPrefix+"IntTxt"+b.getAttribute("ID")+"_"+(e-1)+"_"+
...
function(b){return a.xScale(d[e].getAttribute("stop"))-a.xScale(d[e].getAttribute("start"))});if(0<e){g=">";"-1"==b.getAttribute("strand")&&(g="<");var f=g,p=a.xScale(d[e-1].getAttribute("stop"))-a.xScale(b.getAttribute("start")),m=a.xScale(d[e].getAttribute("start"))-a.xScale(b.getAttribute("start")),n=m-p,u=0;if(7>n)f="";else for(n-=7;8<n;)0==u?(f+=" ",u=1):(f+=g,u=0),n-=7;g=d[e-1].getAttribute("ID")+"_"+d[e].getAttribute("ID");null==d[e].getAttribute("ID")&&(g=b.getAttribute("ID")+"_"+(e-1)+"_"+
e);a.svg.select("g#"+a.idPrefix+"tx"+b.getAttribute("ID")+" line#"+a.idPrefix+"Int"+g).attr("x1",p).attr("x2",m);a.svg.select("g#"+a.idPrefix+"tx"+b.getAttribute("ID")+" #"+a.idPrefix+"IntTxt"+g).attr("dx",p+1).text(f)}}});1==a.density?a.svg.attr("height",30):2==a.density?a.svg.attr("height",15*(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).selectAll("g."+a.idPrefix+"trx"+a.gsvg.levelNumber).size()+1)):3==a.density&&a.svg.attr("height",15*(a.trackYMax+1));a.redrawSelectedArea()}};a.update=function(b){a.redraw()};
a.updateFullData=function(b,c){var d=a.xScale.domain()[0],e=a.xScale.domain()[1];a.showLoading();d3.xml(dataPrefix+"tmpData/browserCache/"+genomeVer+"/regionData/"+a.gsvg.folderName+"/"+a.dataFileName,function(g,f){if(g){if(0==b||1==c){var h="/"+pathPrefix;pathPrefix||(h="");$.ajax({url:h+"generateTrackXML.jsp",type:"GET",cache:!1,data:{chromosome:chr,minCoord:d,maxCoord:e,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:a.trackClass,folder:a.gsvg.folderName},
dataType:"json",success:function(a){ga&&ga("send","event","browser","generateTrackTranscript")},error:function(a,b,c){}})}3>b?(h=1E4,1==b&&(h=15E3),setTimeout(function(){a.updateFullData(b+1,0)},h)):(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+a.trackClass),d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).attr("height",15),a.gsvg.addTrackErrorRemove(a.svg,"#Level"+a.gsvg.levelNumber+a.trackClass),a.hideLoading())}else null==f?4<=
b?(h=[],a.draw(h),a.hideLoading()):setTimeout(function(){a.updateFullData(b+1,0)},5E3):(h=f.documentElement.getElementsByTagName(a.xmlTag),a.draw(h),a.hideLoading())})};a.redrawLegend=function(){var b=[];"Score"==a.colorBy?a.drawScaleLegend(a.minValue,a.maxValue+"+",a.legendLbl,a.minColor,a.maxColor,0):"Color"==a.colorBy&&(b[0]={color:"#FFFFFF",label:"User assigned color from track file."},a.drawLegend(b))};a.draw=function(b){a.data=b;a.prevDensity=a.density;a.trackYMax=0;a.yArr=[];a.yArr[0]=[];for(var c=
...
function loadTrackTable(){var f=svgList[0].xScale.domain()[0],e=svgList[0].xScale.domain()[1],m=chr+":"+minCoord+"-"+maxCoord+":"+reportSelectedTrack.trackClass;loadedTrackTable&&loadedTrackTable===m||(loadedTrackTable=m,m="",f={species:organism,minCoord:f,maxCoord:e,chromosome:chr,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,forwardPvalueCutoff:forwardPValueCutoff,folderName:regionfolderName,genomeVer:genomeVer,track:reportSelectedTrack.trackClass},-1<reportSelectedTrack.trackClass.indexOf("noncoding")?
(f.type="noncoding",f.source="ensembl",-1<reportSelectedTrack.trackClass.indexOf("brain")&&(f.source="brain"),m="web/GeneCentric/geneTable.jsp"):-1<reportSelectedTrack.trackClass.indexOf("coding")?(m="web/GeneCentric/geneTable.jsp",f.type="coding",f.source="ensembl",-1<reportSelectedTrack.trackClass.indexOf("brain")&&(f.source="brain")):"liverTotal"===reportSelectedTrack.trackClass||"heartTotal"===reportSelectedTrack.trackClass||"brainTotal"===reportSelectedTrack.trackClass||"mergedTotal"===reportSelectedTrack.trackClass?
(m="web/GeneCentric/geneTable.jsp",f.type="all",f.source="liver","heartTotal"===reportSelectedTrack.trackClass?f.source="heart":"brainTotal"===reportSelectedTrack.trackClass?f.source="brain":"mergedTotal"===reportSelectedTrack.trackClass&&(f.source="merged")):-1<reportSelectedTrack.trackClass.indexOf("smallnc")?(f.source="ensembl",-1<reportSelectedTrack.trackClass.indexOf("brain")?f.source="brain":-1<reportSelectedTrack.trackClass.indexOf("heart")&&(f.source="heart"),-1<reportSelectedTrack.trackClass.indexOf("liver")&&
(f.source="liver"),m="web/GeneCentric/smallGeneTable.jsp"):-1<(new String(reportSelectedTrack.trackClass)).indexOf("snp")||"qtl"===reportSelectedTrack.trackClass&&(m="web/GeneCentric/bqtlTable.jsp"),m&&loadDivWithPage("div#regionTable",m,!1,f,'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>'));ga&&ga("send","event","loadTrackTable",reportSelectedTrack.trackClass)}
...
function loadEQTLTableWParams(f,e,m,n){loadDivWithPage("div#regionEQTLTable","web/GeneCentric/regionEQTLTable.jsp",!1,{species:organism,minCoord:minCoord,maxCoord:maxCoord,chromosome:chr,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,pValueCutoff:n,tissues:m,chromosomes:e,levels:f,folderName:regionfolderName},'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>');ga&&ga("send","event","loadEQTLTableWParams","eqtlwparams")}
function loadDivWithPage(f,e,m,n,c){$(f).html(c);$.ajax({url:e,type:"GET",cache:!1,data:n,dataType:"html",success:function(b){$(f).html(b);m&&setTimeout(function(){$("html, body").animate({scrollTop:$(f).offset().top},200)},300)},error:function(b,a,c){$(f).html('<span style="color:#FF0000;">An error occurred generating this page.  Please try back later.</span>')}})}function trKey(f){var e="";f&&(e=f.trackClass);return e}
...
$(this).html(a+(b+1))});d3.select("table#trackListTbl"+e.level).select("tbody").selectAll("tr").attr("class",function(b,a){return 0===a%2?"even":"odd"});var c=e.findSelectedView();e.generatePreview(c)};e.generateSettingsString=function(f){var m="";d3.select("table#trackListTbl"+e.level).select("tbody").selectAll("tr").each(function(c){m=m+c.TrackClass+","+c.Settings+";"});return m};e.generateSettingStringFromUI=function(){return e.previewSVG.generateSettingsString()};e.getViewData=function(){var f=
...
for(h=0;h<c.length;h++){var a=c[h].split("=");if("TrackSettingList"===a[0]){a=a[1].split(";");b.TrackList=[];for(var g=0;g<a.length;g++)if(0<a[g].length){var d=a[g].substr(0,a[g].indexOf(",")),p=a[g].substr(a[g].indexOf(",")+1),l={};l.TrackClass=d;l.Settings=p;l.Order=g;b.TrackList.push(l)}}else 2>=a.length?b[a[0]]=a[1]:2<a.length&&(a=c[h].substr(0,c[h].indexOf("=")),g=c[h].substr(c[h].indexOf("=")+1),b[a]=g)}b.Source="local";if(3<c.length){for(var h=c=0;h<b.TrackList.length;h++){if(a=trackInfo[b.TrackList[h].TrackClass])b.TrackList[h].TrackID=
...
e.generateViewList();f=e.findDisplayedIndex(f);$("#viewSelect"+e.level).prop("selectedIndex",f)};e.findSelectedView=function(){for(var f=$("#viewSelect"+e.level).val(),n=NaN,c=0;c<e.viewList.length&&isNaN(n);c++)e.viewList[c].ViewID===f&&(n=e.viewList[c]);return n};e.findViewWithID=function(f){for(var m=NaN,c=0;c<e.viewList.length&&isNaN(m);c++)e.viewList[c].ViewID===f&&(m=e.viewList[c]);return m};e.findViewIndexByID=function(f){for(var m=NaN,c=0;c<e.viewList.length&&isNaN(m);c++)e.viewList[c].ViewID===
f&&(m=c);return m};e.findSelectedViewIndex=function(){for(var f=$("#viewSelect"+e.level).val(),n=NaN,c=0;c<e.viewList.length&&isNaN(n);c++)e.viewList[c].ViewID===f&&(n=c);return n};e.findDisplayedIndex=function(f){for(var m=NaN,c=0;c<e.filterList.length&&isNaN(m);c++)e.filterList[c].ViewID===f&&(m=c);return m};e.findTrackByClass=function(f,n){for(var c=e.findViewWithID(n).TrackList,b=NaN,a=0;a<c.length&&isNaN(b);a++)c[a].TrackClass===f&&(b=c[a]);return b};e.findTrack=function(f){for(var m=e.findSelectedView().TrackList,
c=NaN,b=0;b<m.length&&isNaN(c);b++)m[b].TrackID===f&&(c=m[b]);return c};e.findTrackIndex=function(f){var m=e.findSelectedView().TrackList,c=NaN;if("undefined"!==typeof m)for(var b=0;b<m.length&&isNaN(c);b++)m[b].TrackID===f&&(c=b);return c};e.findTrackIndexWithViewID=function(f,n){for(var c=e.findViewWithID(n).TrackList,b=NaN,a=0;a<c.length&&isNaN(b);a++)c[a].TrackID===f&&(b=a);return b};e.setupControls=function(){-1<f&&($(".control"+e.level+"#addTrack"+e.level).on("click",function(){if($(".trackLevel"+
...
e.level+" div#trackPreviewContent").html("");var m=(new String(organism)).toUpperCase();if("AA"===f.Organism||f.Organism===m){m=svgList[e.level].xScale.domain()[0];var c=svgList[e.level].xScale.domain()[1];e.previewSVG=toolTipSVG("div#trackPreviewOuter"+e.level+" div#trackPreviewContent",565,m,c,e.previewLevel,chr,svgList[e.level].type);e.previewSVG.folderName=svgList[e.level].folderName;f=e.generateSettingsString(f);loadStateFromString(f,"",e.previewLevel,e.previewSVG);e.previewSVG.updateData();
...
e.createRemoteCustomTrack=function(){$(".uploadStatus").html("Setting up track...");$(".uploadStatus").show();var f=$("#customFileURL"+e.level).val(),n="",c=new Date;$.cookie("JSESSIONID")&&(n=$.cookie("JSESSIONID"),n=n+"_"+c.getTime());e.saveTrack(n,f)};e.saveTrack=function(f,n){var c=$("#usrtrkNameTxt"+e.level).val(),b=$("#usrtrkDescTxt"+e.level).val(),a=$("#usrtrkOrgSelect"+e.level).val(),g=$("#usrtrkGenCatSelect"+e.level).val(),d=$("#usrtrkCatTxt"+e.level).val(),p="",l=$("#usrtrkFileTypeSelect"+
e.level).val(),h="";if("bed"===l||"bg"===l)h=$("input#customUploadFile"+e.level)[0].files[0].name;var m="Color";0<$("#usrtrkColorSelect"+e.level).length&&(m=$("#usrtrkColorSelect"+e.level).val());if("Score"===m){var r="000000",D="FFFFFF",t=1,w=1E3;testIE||testSafari?(r=$("#usrtrkColorMin"+e.level).spectrum("get").toHexString(),D=$("#usrtrkColorMax"+e.level).spectrum("get").toHexString()):(r=$("#usrtrkColorMin"+e.level).val(),D=$("#usrtrkColorMax"+e.level).val(),console.log(r),console.log(D));r=r.substr(1);
D=D.substr(1);t=$("#usrtrkScoreMinTxt"+e.level).val();w=$("#usrtrkScoreMaxTxt"+e.level).val()}"bed"===l?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Pack:3}{Full:2};Default=3,Name=Color by;Select:colorSelect::colorSelect;{Feature Defined:Color}{Score based scale:Score};Default="+m+",Name=Scale Range;Txt:tissuecbx;{min:1}{max:1000};Default="+t+":"+w+",Name=Color Range;Txt:color;{min:FFFFFF}{max:000000};Default="+r+":"+D:"bg"===l?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Full:2};Default=1,Name=Scale Range;Slider:rangeslider;{min:1-1000}{max:1000-20000};Default=1:5000":
"bw"===l?p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Full:2};Default=1,Name=Scale Range;Slider:rangeslider;{min:1-1000}{max:1000-20000};Default=1:5000":"bb"===l&&(p="Name=View Density;Select:trackSelect:Dense:Select;{Dense:1}{Pack:3}{Full:2};Default=3,Name=Color by;Select:colorSelect::colorSelect;{Feature Defined:Color}{Score based scale:Score};Default="+m+",Name=Scale Range;Txt:tissuecbx;{min:1}{max:1000};Default="+t+":"+w+",Name=Color Range;Txt:color;{min:FFFFFF}{max:000000};Default="+
r+":"+D);f="custom"+f;0<uid?(m="/"+pathPrefix,pathPrefix||(m=""),$.ajax({url:m+"addTrack.jsp",type:"GET",data:{trackClass:f,trackName:c,trackDesc:b,trackOrg:a,genericCategory:g,category:d,controls:p,location:n,type:l,file:h},dataType:"json",success:function(a){"true"===a.success?$(".uploadStatus").html("<B>Track Setup Successfully</B>"):$(".uploadStatus").html("<B>Track Setup Failed</B>")},error:function(a,b,c){console.log(c)},async:!1})):(m=new Date,c="TrackID="+f+"<->UserID=-999<->TrackClass="+
f+"<->Name="+c+"<->Description="+b+"<->Organism="+a.toUpperCase(),c=c+"<->Settings=<->Order=0<->GenericCategory="+g+"<->Category="+d+"<->Controls="+p+"<->SetupDate="+m+"<->Type="+l+"<->OriginalFile="+h+"<->Location="+n,c=c+"<->genomeVer="+genomeVer,c+="<;>",!0===isLocalStorage()?((g=localStorage.getItem("phenogenCustomTracks"))&&(c=g+c),localStorage.setItem("phenogenCustomTracks",c)):($.cookie("phenogenCustomTracks")&&(c=$.cookie("phenogenCustomTracks")+c),$.cookie("phenogenCustomTracks",c)),$(".uploadStatus").html("<B>Track Setup Successfully</B>"));
$("div#finished"+e.level).show();$("input#customUploadFile"+e.level).val("");$("input#customURL"+e.level).val("");$("input#usrtrkNameTxt"+e.level).val("");$("#usrtrkDescTxt"+e.level).val("");$("input#usrtrkOrgSelect"+e.level).val("Rn");$("input#usrtrkGenCatSelect"+e.level).val("Genome");$("input#usrtrkCatTxt"+e.level).val("");$("#usrtrkColorSelect"+e.level).val("Color");$("#customFileURL"+e.level).val("");e.getTrackData();ga&&ga("send","event","BrowserTracks","saveTrack")};e.readFile=function(f){var m=
new FileReader;m.readAsText(f,"UTF-8");m.onload=e.sendFile};e.sendFile=function(f){$.ajax({url:pathPrefix+"trackUpload.jsp",type:"POST",contentType:"text/plain",xhr:function(){var f=$.ajaxSettings.xhr();f.upload.addEventListener("progress",e.progressHandlingFunction,!1);return f},data:f.target.result,processData:!1,cache:!1,dataType:"json",success:function(f){$(".uploadStatus").html("<B>Upload Completed Successfully</b>");e.saveTrack(f.trackFile,f.trackFile);ga&&ga("send","event","BrowserTracks",
...
e=0,f=[],g=0,l=[];if("undefined"!==typeof d)for(var m=0;m<d.length;m++)for(var p=d[m].GeneList,n=0,y=0;y<p.length&&0===n;y++)p[y].ID===b.Gene.ID&&(0<p[y].V?(e++,f.push(d[m])):1<p[y].P&&(g++,l.push(d[m])),n=1);d3.select(this).append("td").html(b.Gene.geneSymbol);d3.select(this).append("td").html(b.Gene.ID);d3.select(this).append("td").html(function(b,d){if(0<g){var e='<span class="triggerpL" name="pl'+c+'">'+g+"</span><BR>";e=e+'<span id="pl'+c+'" style="display:none;" >';for(var f=0;f<l.length;f++){0<
...
that.selectedModule.TCList[c].Gene.geneSymbol}a.pathColor=function(b){return b&&b.data?b.data.name&&0==b.data.name.indexOf("ENS")?"#DFC184":b.data.name?a.colorGO(b.data.name):"#000000":b&&b.name&&0==b.name.indexOf("ENS")?"#DFC184":b&&b.name?a.colorGO(b.name):"#000000"};a.addLabels=function(b,c,d,e,f,g,m){g=a.radius/(m+1)*b.depth+a.radius/(m+1)-12;var h=a.radius/(m+1)*b.depth,l=(f-d)/360*Math.PI*h*2;m=Math.floor((f-d)/360*Math.PI*g*2/7)-1;g=Math.floor((g-h)/12);var p=b.data.name;0===b.data.name.indexOf("ENS")&&
"undefined"!==typeof a.geneList[p]&&(p=a.geneList[p],25<f-d&&(p=p+" ("+b.data.name+")"));var n=p.split(" ")[0].split("-")[0];if(n.length<=m||-1===b.data.name.indexOf("ENS")&&n.length/2<=m&&10<f-d){l=[];n=0;if(p.length>m)for(;0<p.length;){var q=p.indexOf(" ");if(-1===q)p.length>m?(l[n]=p.substr(0,m),n++,p=p.substr(m)):(l[n]=p,n++,p="");else{for(var r=0;q<m&&1>r;){var t=p.indexOf(" ",q+1);-1!==t&&t<=m?q=t:r=1}q>m?-1<p.indexOf("-")&&p.indexOf("-")<m?(q=p.indexOf("-"),l[n]=p.substr(0,q+1),n++,p=p.substr(q+
1)):(l[n]=p.substr(0,m),n++,p=p.substr(m)):(l[n]=p.substr(0,q),n++,p=p.substr(q+1))}}else l[n]=p;n=a.topG.append("text").style("fill-opacity",1).style("fill","#000").attr("pointer-events","none").attr("class","txt"+removeColon(b.data.id)).attr("text-anchor",function(a){return"start"}).attr("dy","1em").style("opacity",function(a){return 1}).append("textPath").attr("xlink:href",function(a){return"#ap"+c}).attr("startOffset",function(a){return 0===d&&360===f?90:5});for(b=0;b<l.length&&b<g;b++)b<g&&(b===
g-1&&l.length>g&&(l[b]+="...",l[b].length>m&&(l[b]=l[b].substr(0,m-4)+"...")),n.append("tspan").attr("x",0).attr("dy","1em").text(l[b]))}else if(12<=l&&0===b.data.name.indexOf("ENS"))for(n=a.topG.append("text").style("fill-opacity",1).style("fill","#000").attr("pointer-events","none").attr("class","txt"+removeColon(b.data.id)).attr("text-anchor",function(a){a="start";180<=f&&(a="end");return a}).attr("dy",".2em").attr("transform",function(a){a=e-2.5;var b=h;0===d&&360===f&&(b=20);180<=f&&(a=e+2.5);
return"rotate("+(a-90)+")translate("+b+")rotate("+(180<a?-180:0)+")"}).style("opacity",function(a){return 1}),p=b.data.name,0===b.data.name.indexOf("ENS")&&"undefined"!==typeof a.geneList[p]&&(p=a.geneList[p]),m=p.split(" "),b=0;b<m.length;b++)n.append("tspan").attr("x",0).attr("dy","1em").text(m[b])};a.redraw=function(){a.svg.selectAll("path").remove();a.svg.select("#center").remove();var b=a.partition(a.rootN).descendants().filter(a.filterNodes),c=a.displayDepth;a.arc=d3.arc().startAngle(function(a){return Math.max(0,
