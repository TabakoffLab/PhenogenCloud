...
q&&loadRegionWGCNA(),regionDetailLoaded[q]=m);"regionEQTLTable"===q&&tblFrom&&tblFrom.fnAdjustColumnSizing();gtag("event",q,{event_category:"loadRegionReport"})}
...
function loadEQTLTable(){var q=svgList[0].xScale.domain()[0],m=svgList[0].xScale.domain()[1];loadDivWithPage("div#regionEQTLTable","web/GeneCentric/regionEQTLTable.jsp",!1,{species:organism,minCoord:q,maxCoord:m,chromosome:chr,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,pValueCutoff:pValueCutoff,folderName:regionfolderName,genomeVer:genomeVer},'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>');gtag("event","eQTLTable",{event_category:"loadEQTLTable"})}
...
function loadEQTLTableWParams(q,m,r,u,d){loadDivWithPage("div#regionEQTLTable","web/GeneCentric/regionEQTLTable.jsp",!1,{species:organism,minCoord:minCoord,maxCoord:maxCoord,chromosome:chr,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,pValueCutoff:u,tissues:r,chromosomes:m,levels:q,folderName:regionfolderName,dataSource:d},'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>');gtag("event","eqtlwparams",{event_category:"loadEQTLTableWParams"})}
function loadDivWithPage(q,m,r,u,d){$(q).html(d);$.ajax({url:m,type:"GET",cache:!1,data:u,dataType:"html",success:function(b){$(q).html(b);r&&setTimeout(function(){$("html, body").animate({scrollTop:$(q).offset().top},200)},300)},error:function(b,a,e){$(q).html('<span style="color:#FF0000;">An error occurred generating this page.  Please try back later.</span>')}})}function trKey(q){var m="";q&&(m=q.trackClass);return m}
...
$("#"+g).is(":hidden")?($(this).addClass("less"),$("span#"+g).show()):($(this).removeClass("less"),$("span#"+g).hide());f.stopPropagation()});$("#mirTable .triggervL").on("click",function(f){var g=this.getAttribute("name");$("#"+g).is(":hidden")?($(this).addClass("less"),$("span#"+g).show()):($(this).removeClass("less"),$("span#"+g).hide());f.stopPropagation()});$("table#mirTable").DataTable({bPaginate:!1,aaSorting:[[4,"desc"]],sDom:'f<"rightTable"i><t>'})},createMirGeneTable:function(){$("div#waitMirGeneTable").hide();
...
$("#eqtlTable").DataTable({bPaginate:!1,aaSorting:[[3,"desc"]],sDom:"fi<t>"});$("div#waitEqtlTable").hide();d3.select("#circosModule").select("g#Link_"+h).select("path").style("stroke","#FFF450")},20)},error:function(a,e,c){}})};that.singleWGCNAImageMetaView=function(){var a=that.singleWGCNAImage();$(".goCTL").hide();$(".eqtlCtls").hide();$(".linkCTL").show();$(".mirCTL").hide();a.CorCutoff_min=.75;a.CorCutoff_max=1;a.type=that.viewType;a.truncated=0;a.filterOneDB=1;a.mirGeneCutoff=1;a.mirValid=0;
a.mirTargetID="";a.mirID="";a.miRcolor=d3.scaleOrdinal(d3.schemeCategory20b);a.sortMir=0;a.maxCorWidth=5;a.mlinkKey=function(e){return that.removeInvalidIDChars(e.MOD1)+"_"+that.removeInvalidIDChars(e.MOD2)};a.colorCircle=function(e){return e.COLOR};a.colorLink=function(e){var c=a.negColor;0<e.COR&&(c=a.posColor);return c};a.colorLinkHighlight=function(e){var c=a.negDarkColor;0<e.COR&&(c=a.posDarkColor);return c};a.drawLinks=function(e){a.updateColor("linkColor");a.svg.selectAll(".link").remove();
for(var c=a.metaData.LINKS,h=[],f=0,g=0;g<c.length;g++)(c[g].MOD1===e.MODULE||c[g].MOD2===e.MODULE)&&a.CorCutoff_min<=Math.abs(c[g].COR)&&Math.abs(c[g].COR)<=a.CorCutoff_max&&(h[f]=c[g],f++);c=h;1E4>c.length&&(e=a.topG.selectAll(".link").data(c,a.mlinkKey),e.enter().append("line").attr("class","link").attr("pointer-events","none").attr("id",function(l){return"ln_"+that.removeInvalidIDChars(l.MOD1)+that.removeInvalidIDChars(l.MOD2)}).attr("x1",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD1)).attr("cx")}).attr("y1",
function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD1)).attr("cy")}).attr("x2",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD2)).attr("cx")}).attr("y2",function(l){return d3.select("#mod_"+that.removeInvalidIDChars(l.MOD2)).attr("cy")}).attr("stroke",a.colorLink).attr("stroke-width",function(l){return Math.abs(l.COR)*a.maxCorWidth}).merge(e))};a.drawLinksBetween=function(e){a.updateColor("linkColor");for(var c={},h=0;h<e.length;h++)c[e[h]]=1;a.svg.selectAll(".link").remove();
e=a.metaData.LINKS;h=[];for(var f=0,g=0;g<e.length;g++)(0===a.showAllFromTarget&&"undefined"!==typeof c[e[g].MOD1]&&"undefined"!==typeof c[e[g].MOD2]||1===a.showAllFromTarget&&("undefined"!==typeof c[e[g].MOD1]||"undefined"!==typeof c[e[g].MOD2]))&&a.CorCutoff_min<=Math.abs(e[g].COR)&&Math.abs(e[g].COR)<=a.CorCutoff_max&&(h[f]=e[g],f++);e=h;1E4>e.length&&(c=a.topG.selectAll(".link").data(e,a.mlinkKey),c.enter().append("line").attr("class","link").attr("pointer-events","none").attr("id",function(l){return"ln_"+
