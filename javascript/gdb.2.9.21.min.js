...
var mmVer='Mouse(<span id="verSelect"></span>) Strain:C57BL/6J',rnVer='Rat(<span id="verSelect"></span>) HRDP version:<span id="hrdpSelect"></span> Strain:BN',siteVer="PhenoGen v3.9.5(5/16/2024)",trackBinCutoff=1E4,customTrackLevel=-1,mouseTTOver=0,ttHideHandle=0,tt=d3.select("body").append("div").attr("class","testToolTip").style("z-index",1001).style("opacity",0);
...
"dragzoom"===c.defaultMouseFunct&&!d3.event.altKey){var e=d3.mouse(c.vis.node());c.downZoomx=e[0];c.scaleSVG.append("rect").attr("class","zoomRect").attr("x",e[0]).attr("y",0).attr("height",c.scaleSVG.attr("height")).attr("width",1).attr("fill","#CECECE").attr("opacity",.3);c.scaleSVG.append("text").attr("id","zoomTextStart").attr("x",c.downZoomx).attr("y",15).text(numberWithCommas(Math.round(c.xScale.invert(c.downZoomx))));c.scaleSVG.append("text").attr("id","zoomTextEnd").attr("x",c.downZoomx).attr("y",
50).text(numberWithCommas(Math.round(c.xScale.invert(c.downZoomx))))}else 0==processAjax&&(c.prevMinCoord=c.xScale.domain()[0],c.prevMaxCoord=c.xScale.domain()[1],e=d3.mouse(c.vis.node()),c.downx=c.xScale.invert(e[0]),c.downscalex=c.xScale)};c.forceDrawAs=function(e){for(var h=0;h<c.trackList.length;h++)"undefined"!==typeof c.trackList[h]&&"undefined"!==typeof c.trackList[h].drawAs&&(c.trackList[h].drawAs=e,c.trackList[h].draw(c.trackList[h].data));c.forceDrawAsValue=e};c.resize=function(e){c.width=
e;c.xScale.range([0,c.width]);c.xAxis=d3.axisTop(c.xScale).ticks(6).tickSize(8).tickPadding(10);c.scaleSVG.attr("width",c.width);c.scaleSVG.select(".x.axis").call(c.xAxis);d3.select("#Level"+c.levelNumber).select(".axisLbl").attr("x",(c.width-2*c.margin)/2);c.topLevel.style("width",c.width+18+"px");for(var h=0;h<c.trackList.length;h++)"undefined"!==typeof c.trackList[h]&&"undefined"!==typeof c.trackList[h].redraw&&c.trackList[h].resize();c.selectSvg.width=e;c.selectSvg.draw()};c.updateCountScales=
function(e,h){for(var l=0;l<c.trackList.length;l++)"undefined"!==typeof c.trackList[l]&&"undefined"!==typeof c.trackList[l].updateCountScale&&c.trackList[l].updateCountScale(e,h)};c.resetDefaultMouse=function(e){var h="/web/images/icons/"+e+"_dark.png";d3.select("span#"+e+c.levelNumber+" img").attr("src",h);d3.select("span#"+e+c.levelNumber).style("background","#DCDCDC")};c.changeTrackCursor=function(e){for(var h=0;h<c.trackList.length;h++)"undefined"!==typeof c.trackList[h]&&"undefined"!==typeof c.trackList[h].svg&&
c.trackList[h].svg.style("cursor",e)};c.changeScaleCursor=function(e){"undefined"!==typeof c.scaleSVG&&c.scaleSVG.style("cursor",e)};c.updateTrackSelectedArea=function(e,h){c.selectionStart=e;c.selectionEnd=h;var l=c.xScale(e),n=c.xScale(h)-l;c.scaleSVG.selectAll("rect.selectedArea").remove();if((0<l||0<l+n)&&l<c.width)for(1>n&&(n=1),c.scaleSVG.append("rect").attr("class","selectedArea").attr("x",l).attr("y",15).attr("height",c.scaleSVG.attr("height")-15).attr("width",n).attr("fill","#CECECE").attr("opacity",
.3).attr("pointer-events","none"),l=0;l<c.trackList.length;l++)"undefined"!==typeof c.trackList[l]&&"undefined"!==typeof c.trackList[l].setSelectedArea&&c.trackList[l].setSelectedArea(e,h)};c.getAddMenus=function(){var e="/"+pathPrefix;""==pathPrefix&&(e="");$.ajax({url:e+"trackMenu.jsp",type:"GET",cache:!1,data:{level:c.levelNumber,organism},dataType:"html",success:function(h){$("#trackMenu"+c.levelNumber).remove();d3.select("div#trackMenu").append("div").attr("id","trackMenu"+c.levelNumber);$("#trackMenu"+
c.levelNumber).html(h)},error:function(h,l,n){$("#trackMenu"+c.levelNumber).remove();d3.select("div#trackMenu").append("div").attr("id","trackMenu"+c.levelNumber);$("#trackMenu"+c.levelNumber).append('<div class="viewsLevel'+c.levelNumber+'">An error occurred generating this menu.  Please try back later.</div>')},async:!1});$.ajax({url:e+"viewMenu.jsp",type:"GET",cache:!1,data:{level:c.levelNumber,organism},dataType:"html",success:function(h){$("#viewMenu"+c.levelNumber).remove();d3.select("div#viewMenu").append("div").attr("id",
"viewMenu"+c.levelNumber);$("#viewMenu"+c.levelNumber).html(h)},error:function(h,l,n){$("#viewMenu"+c.levelNumber).remove();d3.select("div#viewMenu").append("div").attr("id","viewMenu"+c.levelNumber);$("#viewMenu"+c.levelNumber).append('<div class="viewsLevel'+c.levelNumber+'">An error occurred generating this menu.  Please try back later.</div>')},async:!1})};c.zoomMenuChange=function(e){d3.select("#zoomSelectMenu"+c.levelNumber).selectAll("li").style("background","#FFFFFF");c.zoomFactor=parseFloat(d3.select(e).attr("value"));
...
localStorage.setItem(organism+"DefHRDPVer",$(this).val()):$.cookie(organism+"DefHRDPVer",$(this).val());changeHRDPVer($(this).val());setTimeout(function(){$("form#geneCentricForm").submit()},1500)}),"Rn"===organism?($("span#hrdpSelect").css("display","inline-block"),"rn7"===genomeVer?(console.log("append:v6"),h=e.append("option").attr("value","hrdp6").html("HRDP v6"),"hrdp6"===dataVer&&h.attr("selected","selected"),console.log("append:v7"),h=e.append("option").attr("value","hrdp7").html("HRDP v7"),
"hrdp7"===dataVer&&h.attr("selected","selected")):"rn6"===genomeVer?(console.log("append:v5"),e=e.append("option").attr("value","hrdp5").html("HRDP v5"),e.attr("selected","selected")):"rn5"===genomeVer?(console.log("append:v4"),e=e.append("option").attr("value","hrdp3").html("HRDP v4"),e.attr("selected","selected")):console.error("Unsupported genome version")):"Mm"===organism&&$("span#hrdpSelect").css("display","none"))},500);c.addTrack("genomeSeq",3,"both",0);c.selectSvg=selectionSVG("#Level"+c.levelNumber,
c.width,c.levelNumber,c);return c}
...
function loadEQTLTable(){var p=svgList[0].xScale.domain()[0],m=svgList[0].xScale.domain()[1];transcriptome=$("#transriptome").val();cisOnly=$("#cisTrans").val();pValueCutoff=$("#pvalueCutoffSelect2").val();p={species:organism,minCoord:p,maxCoord:m,chromosome:chr,rnaDatasetID,arrayTypeID,pValueCutoff,folderName:regionfolderName,genomeVer,transcriptome,cisOnly};"hrdp7"!=dataVer?loadDivWithPage("div#regionEQTLTable","web/GeneCentric/regionEQTLTableSeq.jsp",!1,p,'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>'):$("div#regionEQTLTable").css("font-size",
"20pt").css("color","#FF0000").html("Coming soon for HRDP v7.  HRDP v6(rn7) does have eQTLs if you would like to switch please change the drop down list at the top of the page.");gtag("event","eQTLTable",{event_category:"loadEQTLTable"})}
function loadRegionWGCNA(){$("div#geneWGCNA").html("");var p=svgList[0].xScale.domain()[0],m=svgList[0].xScale.domain()[1];p={region:chr+":"+p+"-"+m};"hrdp7"!=dataVer?loadDivWithPage("div#regionWGCNAEQTL","web/GeneCentric/wgcnaGene.jsp",!0,p,'<span style="text-align:center;width:100%;"><img src="web/images/ucsc-loading.gif"><BR>Loading...</span>'):$("div#regionWGCNAEQTL").css("font-size","20pt").css("color","#FF0000").html("Coming soon for HRDP v7.  HRDP v6(rn7) does have WGCNA modules if you would like to switch please change the drop down list at the top of the page.");
