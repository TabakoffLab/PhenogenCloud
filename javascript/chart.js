chart = function (params) {
    ...
    that.parseOptions = function (params) {
        ...
        if (params.allowResize === false) {
            ...
            if (that.curHeight == 0) {
                that.curWidth = Math.floor($(window).height() * .7);
            }
        }
        ...
        if (that.data && that.data.length > 0) {
            ...
            that.svg.selectAll(".dot")
                ...
                .attr("class", function (d) {
                    return "dot " + d.id.replace(/\./g, "_") + " " + d.strain;
                })
                .attr("r", 2.5)
                .attr("cx", function (d) {
                    return that.x(d.strain);
            ...
            that.geneScale = d3.scaleBand().domain(that.filteredGeneIDs.map(function (d) {
                return d.id;
            })).range([0, that.curHeight]);
    ...
    that.drawScatterLineLegend = function () {
        ...
        that.legend.append("rect")
            .attr("class", function (d) {
                return "legend box " + (d.replace(/\./g, "_"));
            })
            .attr("x", 0)
            .attr("width", 18)
            .attr("height", 18)
    ...
    that.drawHeatMap = function () {
        if (that.filteredGeneIDs && that.filteredStrains && that.filteredGeneIDs.length > 0 && that.filteredStrains.length > 0) {
            ...
            that.y = d3.scaleBand().domain(that.filteredGeneIDs.map(function (d) {
                return d.id;
            })).range([0, yRangeMax]);
            ...
            that.svg.selectAll(".box")
                ...
                .append("rect")
                .attr("x", function (d) {
                    return that.x(d.strain) + that.leftMarg;
                })
                .attr("y", function (d) {
                    return that.y(d.id) + that.topMarg;
                })
                ...
                .style("fill", function (d) {
                    return d3.rgb(that.color(d.val), 0, 0);
                })
                .on("mouseover", function (d) {
            ...
            if ((that.heritChartW == 0 && that.curWidth > 1200 && that.display.herit) || (that.heritChartW > 0 && that.curWidth < 1200 && that.display.herit)) {
                ...
                that.svg.selectAll(".box").each(function (data) {
                    d3.select(this).select("rect")
                        .attr("x", function (d) {
                            return that.x(data.strain) + that.leftMarg;
                        })
                        .attr("y", function (d) {
                            return that.y(data.id) + that.topMarg;
                        })
                        .attr("width", function (d) {
                            return itemW;
                        })
    ...
    that.drawHeritabiltiy = function (barMarg) {
        if (that.heritChartW > 0) {//draw to side of heatmap
            ...
            internalHChart.selectAll(".bar")
                ...
                .attr("y", function (d) {
                    return that.geneScale(d.id) + that.topMarg + barMarg + 10;
                })
                .attr("width", function (d) {
                    if (d && d.herit) {
            ...
            yRangeMax = that.curHeight - that.topMarg;
            itemH = yRangeMax / that.filteredGeneIDs.length;
            if (itemH < 10) {
                itemH = 10;
            } else if (itemH > 18) {
                itemH = 18;
            }
            minY = itemH * that.filteredGeneIDs.length + that.topMarg;
            if (yRangeMax < minY) {
                yRangeMax = minY;
            }
            ...
            that.heritAx = d3.axisTop(that.heritScale);
            that.yH = d3.scaleBand().domain(that.filteredGeneIDs.map(function (d) {
                return d.id;
            })).range([0, Math.floor(yRangeMax)]);
            ...
            that.hChart.selectAll(".bar")
                ...
                .attr("y", function (d) {
                    return that.yH(d.id) + that.topMarg + barMarg + 15;
                })
                .attr("width", function (d) {
                    if (d && d.herit) {
                        return that.heritScale(d.herit);
    ...
    that.redrawHeritability = function (barMarg) {
        if (that.heritChartW > 0 && that.display.herit) {
            ...
            that.svg.select("g#internalHChart").selectAll("rect.bar")
                .attr("y", function (d) {
                    return that.geneScale(d.id) + that.topMarg + barMarg;
                })
                .attr("height", function (d) {
                    return itemH - (2 * barMarg);
                });
