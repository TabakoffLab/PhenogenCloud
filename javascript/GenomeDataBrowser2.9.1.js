...
var siteVer="PhenoGen v3.7.3(8/21/2020)";
...
function GeneTrack(gsvg,data,trackClass,label,additionalOptions){
	...
	that.updateData=function(retry){
		...
		d3.xml(path,function (error,d){
			if(error){
				if(retry===0  && (that.trackClass.indexOf("heartTotal")===0 || that.trackClass.indexOf("liverTotal")===0 || that.trackClass.indexOf("brainTotal")===0 || that.trackClass==='mergedTotal' || that.trackClass.indexOf("smallnc")>-1)){
                	var tmpContext="/"+ pathPrefix;
					if(!pathPrefix){
						tmpContext="";
					}
        ...
        if(filterData.length>0){
			if(that.drawAs==="Gene" || that.trackClass.indexOf("smallnc")>-1){
				...
				gene.enter().append("g")
					...
					.attr("stroke",that.colorStroke)
					.attr("stroke-width","1")
					.attr("id",function(d){return that.cleanID(d.getAttribute("ID"));})
					.style("fill",that.color)
					.style("cursor", "pointer")
					.on("click", function(d){
	                                            setTimeout(function(){
