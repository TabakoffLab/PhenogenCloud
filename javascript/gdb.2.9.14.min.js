...
var mmVer='Mouse(<span id="verSelect"></span>) Strain:C57BL/6J',rnVer='Rat(<span id="verSelect"></span>) Strain:BN',siteVer="PhenoGen v3.7.10(6/23/2022)",trackBinCutoff=1E4,customTrackLevel=-1,mouseTTOver=0,ttHideHandle=0,tt=d3.select("body").append("div").attr("class","testToolTip").style("z-index",1001).style("opacity",0);tt.append("span").style("float","right").append("img").attr("src","web/images/icons/close.png").on("click",function(){tt.style("opacity",0)});
...
"tmpData/browserCache/"+genomeVer+"/regionData/"+c.folderName+"/"+e+".xml";var F="Brain ";"liverspliceJnct"==e?F="Liver ":"heartspliceJnct"==e&&(F="Heart ");d3.xml(q,function(D,E){if(D){console.log(D);if(0==m){var z="/"+pathPrefix;pathPrefix||(z="");$.ajax({url:z+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:w,maxCoord:A,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:e,folder:c.folderName},dataType:"json",
success:function(H){gtag("event","generateTrack"+e,{event_category:"browser"})},error:function(H,I,J){}})}60>m?(z=1500,1===m&&(z=2500),setTimeout(function(){c.addTrack(e,g,l,m+1)},z)):(d3.select("#Level"+c.levelNumber+e).select("#trkLbl").text("An errror occurred loading Track:"+e),d3.select("#Level"+c.levelNumber+e).attr("height",15),c.addTrackErrorRemove(d3.select("#Level"+c.levelNumber+e),"#Level"+c.levelNumber+e))}else if(null==E)if(11<=m){z=[];var C=SpliceJunctionTrack(c,z,e,F+"Splice Junctions",
...
b%3,m=a-q,g=[],g[0]=f.translateRegion(0,b-q,m,m+d),g[1]=f.translateRegion(1,b-q-1,m-1,m-1+d),g[2]=f.translateRegion(2,b-q-2,m-2,m-2+d),w=25,q=0;3>q;q++){var A=f.svg.selectAll(".aa"+q).data(g[q],keyID).attr("transform",function(B,G){return"translate("+(f.xScale(B.pos)-h)+","+w+")"}).each(function(B){d3.select(this).select("rect").attr("width",v);d<f.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",y).attr("font-size",c).text(function(G){return G.aa}):d3.select(this).select("text").attr("x",
y).attr("font-size",c):d3.select(this).select("text").remove()});m=A.enter().append("g").attr("class","aa"+q).attr("transform",function(B,G){return"translate("+(f.xScale(B.pos)-h)+","+w+")"});m.each(function(B){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",v).attr("stroke",f.colorAA).attr("fill",f.colorAA);d<f.dispCutoff&&d3.select(this).append("text").attr("x",y).attr("font-size",c).text(function(G){return G.aa})});A.exit().remove();w+=11}w="both"==f.strands?80:45;if("both"==
f.strands||"-"==f.strands)for(q=b%3,m=a-q,g=[],g[0]=f.translateRegion(0,b-q,m+d,m),g[1]=f.translateRegion(1,b-q-1,m-1+d,m-1),g[2]=f.translateRegion(2,b-q-2,m-2+d,m-2),q=0;3>q;q++)A=f.svg.selectAll(".aarev"+q).data(g[q],keyID).attr("transform",function(B,G){return"translate("+(f.xScale(B.pos)-h)+","+w+")"}).each(function(B){d3.select(this).select("rect").attr("width",v);d<f.dispCutoff?0==d3.select(this).select("text").size()?d3.select(this).append("text").attr("x",y).attr("font-size",c).text(function(G){return G.aa}):
d3.select(this).select("text").attr("x",y).attr("font-size",c):d3.select(this).select("text").remove()}),m=A.enter().append("g").attr("class","aarev"+q).attr("transform",function(B,G){return"translate("+f.xScale(B.pos)+","+w+")"}),m.each(function(B){d3.select(this).append("rect").attr("y",-10).attr("height",10).attr("width",v).attr("stroke",f.colorAA).attr("fill",f.colorAA);d<f.dispCutoff&&d3.select(this).append("text").attr("x",y).attr("font-size",c).text(function(G){return G.aa})}),A.exit().remove(),
...
15).text("<--"),f.svg.append("text").attr("class","dir").attr("x",f.gsvg.width-60).attr("y",15).text("<--"));if(d<f.dispCutoff&&1<b.length){for(var l=[],m=c;m<e;m++)l[m-c]=[],l[m-c].base=b.charAt(m),l[m-c].id=m,l[m-c].pos=m-c;m=f.textSize(d);var q=f.yPosition(d),t=27;1==f.includeAA&&"-"!=f.strands&&(t+=30);l=f.svg.selectAll(".base").data(l,keyID).attr("transform",function(A,B){return"translate("+A.pos*g+","+t+")"});b=l.enter().append("g").attr("class","base").attr("transform",function(A,B){return"translate("+
A.pos*g+","+t+")"});b.each(function(A){d3.select(this).append("text").text(function(B){return"-"!=f.strands?A.base:f.complement(A.base)});"both"==f.strands&&d3.select(this).append("text").attr("class","comp").attr("y",q).text(function(B){return f.complement(A.base)})});l.exit().remove();l.selectAll("text").attr("font-size",m)}else f.svg.append("g").attr("class","base").append("line").attr("stroke","#000000").attr("x1",0).attr("y1",57).attr("x2",f.gsvg.width).attr("y2",57);if(f.data&&1==f.includeAA){var v=
...
genomeVer||"small"===c&&-1<(new String(a.getAttribute("ID"))).indexOf("ENS")){console.log("totalType");h=a.getAttribute("ID");var g="";if(a.getAttribute("geneSymbol")&&0<a.getAttribute("geneSymbol").length)h=h+" ("+a.getAttribute("geneSymbol")+")";else{e=getAllChildrenByName(getFirstChildByName(a,"TranscriptList"),"Transcript");for(var l=0;l<e.length&&""===g;l++)if(""===g)for(var m=getAllChildrenByName(getFirstChildByName(e[l],"annotationList"),"annotation"),q=0;q<m.length&&0===g.length;q++)if("AKA"===
m[q].getAttribute("source")){var t=m[q].getAttribute("annot_value");t=(new String(t)).split(":");1<=t.length?(g=t[0],h=h+" ("+g+")"):2<=t.length&&(g=t[0],t=t[1],h=h+" ("+t+")")}}e=parseInt(a.getAttribute("start"),10);m=parseInt(a.getAttribute("stop"),10);a.getAttribute("extStart")&&a.getAttribute("extStop")&&(e=parseInt(a.getAttribute("extStart"),10),m=parseInt(a.getAttribute("extStop"),10));svgViewIDList[d]||(svgViewIDList[d]=b.gsvg.currentView?b.gsvg.currentView.ViewID:defaultView);l=GenomeSVG("div#selectedImage",
...
maxCoord:b.gsvg.xScale.domain()[1],panel:l,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:c,folder:b.gsvg.folderName},dataType:"json",success:function(m){gtag("event","generateTrackGene",{event_category:"browser"})},error:function(m,q,t){}})}5>d?(g=1E4,0===d&&(g=5E3),setTimeout(function(){b.updateDataVersion(a,d+1)},g)):(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+b.trackClass),d3.select("#Level"+
...
l;q=b.xScale(exList[g-1].getAttribute("stop"));t=b.xScale(exList[g].getAttribute("start"))-q;v=0;if(7>t)m="";else for(t-=7;8<t;)0==v?(m+=" ",v=1):(m+=l,v=0),t-=7;e.append("svg:text").attr("id",function(A){return"IntTxt"+exList[g-1].getAttribute("ID")+"_"+exList[g].getAttribute("ID")}).attr("dx",q+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",b.color).style("font-size","16px").text(m)}}if(2==b.density){var w=a.getAttribute("ID");"undefined"!==typeof a.getAttribute("geneSymbol")&&
...
a.parent);e.addTrack(b.trackClass,2,"DrawTrx",g);for(g=0;g<b.ttTrackList.length;g++)if(b.trackClass!==b.ttTrackList[g]){var l=b.gsvg.getTrackData(b.ttTrackList[g]),m=[];if("undefined"!==typeof l&&0<l.length){for(var q=0,t=0;t<l.length;t++)if(c<=parseInt(l[t].getAttribute("start"),10)&&parseInt(l[t].getAttribute("start"),10)<=h||c<=parseInt(l[t].getAttribute("stop"),10)&&parseInt(l[t].getAttribute("stop"),10)<=h||parseInt(l[t].getAttribute("start"),10)<=c&&parseInt(l[t].getAttribute("stop"),10)>=c)m[q]=
l[t],q++;0<m.length&&e.addTrack(b.ttTrackList[g],3,"DrawTrx",m)}}};b.draw=function(a){b.data=a;b.trackYMax=0;b.yMaxArr=[];b.yArr=[];b.yArr[0]=[];for(var d=0;d<b.gsvg.width;d++)b.yMaxArr[d]=0,b.yArr[0][d]=0;b.svg.selectAll(".gene").remove();d=b.drawAs;var c=b.xScale.domain()[0];if(b.xScale.domain()[1]-c<b.trxCutoff||$("#forceTrxCBX"+b.gsvg.levelNumber).is(":checked"))b.drawAs="Trx";c="Genes";"Trx"==b.drawAs&&(c="Transcripts");var h="Protein Coding",e=" / PolyA+";-1<b.trackClass.indexOf("noncoding")?
...
function(m){0===b.gsvg.isToolTip&&(overSelectable=1,$("#mouseHelp").html("<B>Click</B> to see additional details. <B>Double Click</B> to zoom in on this feature."),d3.select(this).style("fill","green"),tt.transition().duration(200).style("opacity",1).style("top",function(){return b.positionTTTop(d3.event.pageY)}).style("left",function(){return b.positionTTLeft(d3.event.pageX)}),tt.html(b.createToolTip(m)),m&&(b.triggerTableFilter(m),-1===b.trackClass.indexOf("smallnc")?"Trx"===b.drawAs?b.setupToolTipSVG(m.parent,
.05):b.setupToolTipSVG(m,.05):b.setupToolTipSVG(m,.05),tt.style("top",function(){return b.positionTTTop(d3.event.pageY)})))}).on("mouseout",function(m){$("#mouseHelp").html("Navigation Hints: Hold mouse over areas of the image for available actions.");"selected"!==d3.select(this).attr("class")&&d3.select(this).style("fill",b.color);tt.transition().delay(500).duration(200).style("opacity",0);b.clearTableFilter(m)}),a.exit().remove(),d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").each(function(m){if(m){var q=
d3.select(this);if("-1"==m.getAttribute("strand")||"1"==m.getAttribute("strand")){var t=">";"-1"==m.getAttribute("strand")&&(t="<");var v=t,y=q.select("rect").attr("width");if(8.5>y)v="";else for(y-=8.5,v=t;8.7<y;)v+=t,y-=8.5;q.append("svg:text").attr("dx","1").attr("dy","10").attr("id","strandTxt").style("pointer-events","none").text(v)}if(2===b.density){var w=m.getAttribute("ID");m.getAttribute("geneSymbol")&&0<m.getAttribute("geneSymbol").length&&(w=w+" ("+m.getAttribute("geneSymbol")+")");q.append("svg:text").attr("dx",
function(){var A=b.xScale(m.getAttribute("start"));return A=A<$(window).width()/2?b.xScale(m.getAttribute("stop"))-b.xScale(m.getAttribute("start"))+5:-9*w.length}).attr("dy",10).attr("id","lblTxt").text(w)}else q.select("text#lblTxt").remove()}}),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).selectAll("g.gene").size()+1)):3==b.density&&b.svg.attr("height",15*(b.trackYMax+1))):"Trx"==b.drawAs&&(b.drawnAs="Trx",b.svg.selectAll(".gene").each(function(){d3.select(this).remove()}),
b.svg.selectAll(".trx"+b.gsvg.levelNumber).remove(),a=b.svg.selectAll(".trx"+b.gsvg.levelNumber).data(c,key).attr("transform",function(m,q){return"translate("+b.xScale(m.getAttribute("start"))+","+b.calcY(parseInt(m.getAttribute("start"),10),parseInt(m.getAttribute("stop"),10),q)+")"}),a.enter().append("g").attr("class",function(m){m=new String(m.parent.getAttribute("ID"));return"trx"+b.gsvg.levelNumber+" gene"+m.replace(".","_")}).attr("transform",function(m,q){return"translate(0,"+b.calcY(parseInt(m.getAttribute("start"),
10),parseInt(m.getAttribute("stop"),10),q)+")"}).attr("id",function(m){var q="";1==b.gsvg.levelNumber&&(q="tx");99==b.gsvg.levelNumber&&(q="ttTx");return q+b.cleanID(m.getAttribute("ID"))+b.gsvg.levelNumber}).style("cursor","pointer").on("click",function(m){setTimeout(function(){if(0==b.gsvg.levelNumber){var q=(new String(m.parent.getAttribute("ID"))).replace(".","_");b.svg.selectAll("g.trx"+b.gsvg.levelNumber+".selected").selectAll("line").style("stroke",b.color);b.svg.selectAll("g.trx"+b.gsvg.levelNumber+
...
b.triggerTableFilter(m);if("undefined"!==typeof m){if(-1===b.trackClass.indexOf("smallnc"))b.setupToolTipSVG(m,.05);else{q=toolTipSVG("div#ttSVG",450,parseInt(m.getAttribute("start"),10)-10,parseInt(m.getAttribute("stop"),10)+10,99,b.getDisplayID(m.getAttribute("ID")),"");q.xMin=parseInt(m.getAttribute("start"),10)-20;q.xMax=parseInt(m.getAttribute("stop"),10)+20;var t=[];t[0]=m;q.addTrack(b.trackClass,2,"",t)}tt.style("top",function(){return b.positionTTTop(d3.event.pageY)})}}}).on("mouseout",function(m){if(-1==
(new String(d3.select(this).attr("class"))).indexOf("selected"))if(0==b.gsvg.levelNumber){var q=(new String(m.parent.getAttribute("ID"))).replace(".","_");b.svg.selectAll("g.gene"+q).selectAll("line").style("stroke",b.color);b.svg.selectAll("g.gene"+q).selectAll("rect").style("fill",b.color);b.svg.selectAll("g.gene"+q).selectAll("text").style("opacity","0.6").style("fill",b.color)}else d3.select(this).selectAll("line").style("stroke",b.color),d3.select(this).selectAll("rect").style("fill",b.color),
...
";"};b.redrawLegend();b.draw(n);return b}
...
0;q<exList.length;q++){var t=parseInt(exList[q].getAttribute("start"),10),v=parseInt(exList[q].getAttribute("stop"),10);if(t<g&&g<v||v>l&&l>t){var y=t,w=g;v>l?(y=l,w=v,v=l):t=g;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(h.getAttribute("ID"))+" rect#ExNC"+exList[q].getAttribute("ID")).attr("x",function(z){return b.xScale(y)}).attr("width",function(z){return b.xScale(w)-b.xScale(y)})}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(h.getAttribute("ID"))+
...
7;d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(h.getAttribute("ID"))+" #IntTxt"+exList[q-1].getAttribute("ID")+"_"+exList[q].getAttribute("ID")).attr("dx",A+1).text(F)}d3.select("#Level"+b.gsvg.levelNumber+b.trackClass).select("g#"+m+b.cleanID(h.getAttribute("ID"))+" line#Int"+exList[q-1].getAttribute("ID")+"_"+exList[q].getAttribute("ID")).attr("x1",A).attr("x2",B)}}}),1==b.density?b.svg.attr("height",30):2==b.density?b.svg.attr("height",15*(d3.select("#Level"+b.gsvg.levelNumber+
...
" ("+(parseInt(c[e].getAttribute("stop"),10)-parseInt(c[e].getAttribute("start"),10))+" bp)";return d=d+"<BR>Intron: "+numberWithCommas(parseInt(c[1].getAttribute("start"),10)-parseInt(c[0].getAttribute("stop"),10))+" bp<BR>"};p.drawScaleLegend("1","1000+","Read Counts","#E6E6E6","#000000",0);p.draw(n);return p}
...
10),m=parseInt(b.data[g].getAttribute("stop"),10),q=parseInt(b.data[g].getAttribute("count"),10);if(1!=b.density||1==b.density&&l!=m)if(g+1<b.data.length){if(m=parseInt(b.data[g+1].getAttribute("start"),10),l>=a&&l<=d||m>=a&&m<=d)c[h]=b.data[g],q>e&&(e=q),h++}else l>=a&&l<=d&&(c[h]=b.data[g],q>e&&(e=q),h++)}1==b.density?(b.density!=b.prevDensity?(b.redrawLegend(),d=b.gsvg.xScale.domain()[1],b.prevDensity=b.density,b.svg.selectAll(".area").remove(),b.svg.selectAll("g.y").remove(),b.svg.selectAll(".grid").remove(),
...
g);console.log(b.trackClass+":folder="+b.gsvg.folderName);d3.xml(g,function(l,m){if(l){console.log("error:"+b.bin+":"+g);if(0==a||1==d){var q="/"+pathPrefix;pathPrefix||(q="");tmpPanel=panel;-1<b.trackClass.indexOf("-")&&(tmpPanel=b.trackClass.substr(b.trackClass.indexOf("-")+1));$.ajax({url:q+"generateTrackXML.jsp",type:"GET",cache:!1,async:!0,data:{chromosome:chr,minCoord:c,maxCoord:h,panel:tmpPanel,rnaDatasetID:rnaDatasetID,arrayTypeID:arrayTypeID,myOrganism:organism,genomeVer:genomeVer,track:b.trackClass,
...
b.trackClass),d3.select("#Level"+b.levelNumber+b.trackClass).attr("height",15),b.gsvg.addTrackErrorRemove(b.svg,"#Level"+b.gsvg.levelNumber+b.trackClass),b.hideLoading(),b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,1)},15E3))}else console.log("update data"),null==m?(console.log("is null"),32<=a?(n=[],b.draw(n)):b.fullDataTimeOutHandle=setTimeout(function(){b.updateFullData(a+1,0)},5E3)):(console.log("not null is drawing"),b.fullDataTimeOutHandle=0,b.loadedDataMin=c,b.loadedDataMax=
h,setTimeout(function(){var t=m.documentElement.getElementsByTagName("Count");b.draw(t)},10))})};b.updateCountScale=function(a,d){b.scaleMin=a;b.scaleMax=d;b.colorScale=d3.scaleLinear().domain([b.scaleMin,b.scaleMax]).range(["#EEEEEE","#000000"]);1==b.density&&(b.redrawLegend(),b.redraw())};b.setupToolTipSVG=function(a,d){var c=b.xScale.domain()[0],h=b.xScale.domain()[1],e=parseInt(a.getAttribute("start"),10),g=parseInt(a.getAttribute("stop"),10);h=Math.floor((h-c)*d);20>h&&(h=20);c=e-h;h=g+h;1>c&&
...
10)<=c&&parseInt(l[t].getAttribute("stop"),10)>=c)m[q]=l[t],q++;0<m.length&&e.addTrack(b.ttTrackList[g],3,"DrawTrx",m)}}};b.draw=function(a){b.hideLoading();b.data=a;console.log("draw:"+b.trackClass+":"+a.length);var d=b.gsvg.xScale.domain()[0],c=b.gsvg.xScale.domain()[1];b.xScale(d+b.bin);b.xScale(d);b.redrawLegend();for(var h=[],e=0,g=0,l=0;l<b.data.length;l++)if("undefined"!==typeof b.data[l]){var m=parseInt(b.data[l].getAttribute("start"),10),q=parseInt(b.data[l].getAttribute("stop"),10),t=parseInt(b.data[l].getAttribute("count"),
10);if(1!=b.density||1==b.density&&m!=q)if(l+1<b.data.length){if(q=parseInt(a[l+1].getAttribute("start"),10),m>=d&&m<=c||q>=d&&q<=c)h[e]=b.data[l],t>g&&(g=t),e++}else m>=d&&m<=c&&(h[e]=b.data[l],t>g&&(g=t),e++)}console.log("newData:"+h.length);ndata=h;console.log("data:"+ndata.length);b.svg.select(".y.axis").remove();b.svg.select("g.grid").remove();b.svg.selectAll(".leftLbl").remove();b.svg.select(".area").remove();b.svg.selectAll(".area").remove();1==b.density?(console.log("draw:den1"),c=b.gsvg.xScale.domain()[1],
...
g=h[0].substr(5),l="";3<h.length&&0==h[3].indexOf("Default=")&&(l=h[3].substr(8));if(0==h[1].toLowerCase().indexOf("select")){e.append("text").text(g+": ");var m=h[1].split(":");h=h[2].split("}");g=b.trackClass+"Dense"+b.level+"Select";"colorSelect"==m[1]?g=b.trackClass+b.level+"colorSelect":"countSelect"==m[1]&&(g=b.trackClass+b.level+"countSelect");e=e.append("select").attr("id",g).attr("name",m[1]);for(m=0;m<h.length;m++){var q=h[m].substr(1).split(":");if(2==q.length){var t=e.append("option").attr("value",
...
l),e.append("text").text(g+": "),e.append("input").attr("type","text").attr("id","amount").attr("value",b.scaleMin+"-"+b.scaleMax).style("border",0).style("color","#f6931f").style("font-weight","bold").style("background-color","#EEEEEE"),m=h[1].split(":"),h=h[2].split("}"),e=e.append("div"),e.append("text").text("Min:"),e.append("div").attr("id","min-"+m[1]).style("width","60%").style("display","inline-block").style("float","right"),e.append("br"),e.append("text").text("Max:"),e.append("div").attr("id",
"max-"+m[1]).style("width","60%").style("display","inline-block").style("float","right"),$("#min-"+m[1]).slider({min:1,max:1E3,step:1,value:b.scaleMin,slide:b.processSlider}),$("#max-"+m[1]).slider({min:1E3,max:2E4,step:100,value:b.scaleMax,slide:b.processSlider}),b.updateSettingsFromUI(),b.redraw())}c=a.append("tr").append("td");c.append("input").attr("type","button").attr("value","Remove Track").style("float","left").style("margin-left","5px").on("click",function(){$("#trackSettingDialog").fadeOut("fast");
...
$("#amount").val(c+" - "+h);b.updateCountScale(c,h)};b.yScale=d3.scaleLinear().range([140,20]).domain([0,d3.max(n,function(a){return a.getAttribute("count")})]);b.area=d3.area().x(function(a){return b.xScale(a.getAttribute("start"))}).y0(140).y1(function(a){return a.getAttribute("count")});b.yAxis=d3.axisLeft(b.yScale).ticks(5);b.redrawLegend();b.draw(n);return b}
...
function CircRNATrack(p,n,r,u,f,b){var a=GenericTranscriptTrack(p,n,r,u,f,b);p=[];b&&(0<b.indexOf(",")?p=b.split(","):p[0]=b);a.density=f;0<p.length&&(a.dataFileName=p[0].substr(9));a.colorBy=1<p.length?p[1]:"Score";2<p.length&&(a.minValue=p[2]);3<p.length&&(a.maxValue=p[3]);4<p.length&&(a.minColor=p[4]);5<p.length&&(a.maxColor=p[5]);a.colorValueField="score";a.minFeatureWidth=1;a.updateControl=0;a.xmlTag="Gene";a.xmlTagBlockElem="Block";a.idPrefix="cirRNA";a.xCirLineWidth=4;a.xPadding=7;a.featureHeight=
...
d<=a.xScale.domain()[0]&&c>=a.xScale.domain()[1]){d=Math.round(a.xScale(d));0>d&&(d=0);c=Math.round(a.xScale(c));c>=a.gsvg.width&&(c=a.gsvg.width-1);h=d-a.xPadding;0>h&&(h=0);var e=c+a.xPadding;e>=a.gsvg.width&&(e=a.gsvg.width-1);for(var g=0,l=h;l<=e;l++)a.yMaxArr[l]>g&&(g=a.yMaxArr[l]);g++;var m=g;if(a.yArr.length<=g)for(a.yArr[m]=[],l=0;l<a.gsvg.width;l++)a.yArr[m][l]=0;l=g-a.scanBackYLines;1>l&&(l=1);m=0;for(var q=l;q<g&&0==m;q++){var t=0;for(l=h;l<=e&&0==t;l++)a.yArr[q][l]>t&&(t=1);0==t&&(g=q,
...
l=a.xScale(exList[e].getAttribute("start"))-a.xScale(d.getAttribute("start"));h.append("line").attr("x1",g).attr("x2",l).attr("y1",20).attr("y2",20).attr("stroke",a.color).attr("stroke-width","2").attr("id",function(v,y){var w=a.idPrefix+"Int"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(w=a.idPrefix+"Int"+v.getAttribute("ID")+"_"+(e-1)+"_"+e);return w});var m=">";"-"===d.getAttribute("strand")&&(m="<");var q=m;l-=g;var t=0;if(7>l)q="";else for(l-=
7;8<l;)0==t?(q+=" ",t=1):(q+=m,t=0),l-=7;h.append("svg:text").attr("id",function(v){var y=a.idPrefix+"IntTxt"+exList[e-1].getAttribute("ID")+"_"+exList[e].getAttribute("ID");null==exList[e].getAttribute("ID")&&(y=a.idPrefix+"IntTxt"+v.getAttribute("ID")+"_"+(e-1)+"_"+e);return y}).attr("y",15).attr("dx",g+1).attr("dy","11").style("pointer-events","none").style("opacity","0.5").style("fill",a.color).style("font-size","16px").text(q)}};a.calcXLine=function(d,c,h,e,g){h*=a.xCirLineWidth;return 0<d.length?
...
error:function(q,t,v){console.log(v)}})}3>d?(m=1E4,1==d&&(m=15E3),setTimeout(function(){a.updateFullData(d+1,0)},m)):(d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+a.trackClass),d3.select("#Level"+a.gsvg.levelNumber+a.trackClass).attr("height",15),a.gsvg.addTrackErrorRemove(a.svg,"#Level"+a.gsvg.levelNumber+a.trackClass),a.hideLoading())}else m=l.documentElement.getElementsByTagName(a.xmlTag),a.draw(m),a.hideLoading(),a.updateControl=
...
m).attr("name",l[1]);for(l=0;l<e.length;l++){var q=e[l].substr(1).split(":");if(2==q.length){var t=g.append("option").attr("value",q[1]).text(q[0]);(-1<m.indexOf("Dense")&&q[1]==a.density||-1<m.indexOf("colorSelect")&&q[1]==a.colorBy)&&t.attr("selected","selected")}}d3.select("select#"+m).on("change",function(){"Score"==$(this).val()?$("div."+a.trackClass+"Scale"+a.level).show():"Color"==$(this).val()&&$("div."+a.trackClass+"Scale"+a.level).hide();a.updateSettingsFromUI();a.draw(a.data)})}else 0==
...
label:"Predicted(Multi)"},{color:"#3282b8",label:"Predicted(Single)"},{color:"#5D576B",label:"Array"},{color:"#7A9D98",label:"Array/Predicted"}])};return a}window.GenomeSVG=GenomeSVG;var reportSelectedTrack=null,loadedTrackTable=null,regionDetailLoaded={};$(document).on("click","span.detailMenu",function(p){p=$(this).attr("name");var n=$("span.detailMenu.selected").attr("name");$("div#"+n).hide();$("span.detailMenu.selected").removeClass("selected");$("span[name='"+p+"']").addClass("selected");$("div#"+p).show();if("undefined"!==typeof svgList[0]){n=svgList[0].xScale.domain()[0];var r=svgList[0].xScale.domain()[1];loadRegionReport(p,chr,n,r)}});
...
function displayDetailedView(p){reportSelectedTrack=p;$("li.report").removeClass("selected");$("li."+p.trackClass).addClass("selected");p.displayBreakDown&&setTimeout(function(){$("div#trackGraph").html("");p.displayBreakDown("div#collapsableReport div#trackGraph")},50);var n=new String(p.trackClass);-1<n.indexOf("coding")||-1<n.indexOf("noncoding")||-1<n.indexOf("smallnc")||-1<n.indexOf("liverTotal")||-1<n.indexOf("heartTotal")||-1<n.indexOf("brainTotal")||-1<n.indexOf("mergedTotal")||-1<n.indexOf("kidneyTotal")||
