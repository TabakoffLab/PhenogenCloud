...
function WGCNABrowser(id, region, geneList, disptype, viewtype, tissue) {
    ...
    that.eQTLKey = function (d) {
        return "Link_" + d.Snp;
    };
    that.mirKey = function (d) {
        return d.ID;
    };
    that.modKey = function (d) {
        return d.MODULE;
    };
    ...
    that.setup = function () {
        ...
        gtag('event', 'setupWGCNA', {'event_category': 'setupWGCNA'});
    };
    ...
    that.requestModuleList = function () {
        ...
        $.ajax({
            ...
            async: true,
            data: {
                source: that.selSource,
                modFileType: that.viewtype,
                id: that.singleID,
                organism: organism,
                panel: that.panel,
                tissue: that.tissue,
            ...
            success: function (data2) {
                ...
                if (data2.length > 0 && !(data2.length === 1 && data2[0].ModuleID === "grey")) {
                    ...
                    that.displayMessage("There are no WGCNA Modules associated with this gene/region for the selected WGCNA dataset.");
                ...
                gtag('event', 'requestModuleList', {'event_category': 'requestModuleList'});
    ...
    that.createImageControls = function () {
        ...
        that.imageBar.append("span").attr("class", "saveImage control").style("display", "inline-block")
            ...
            .on("click", function () {
                ...
                if (that.singleImage) {
                    ...
                    gtag('event', 'saveWGCNAView', {'event_category': 'saveWGCNAView'});
                }
            })
        ...
        that.imageBar.append("span").attr("class", "reset control").style("display", "inline-block")
            ...
            .on("click", function () {
                ...
                gtag('event', 'resetWGCNAView', {'event_category': 'resetWGCNAView'});
            })
            .on("mouseover", function () {
        ...
        that.imageBar.append("span").attr("class", "zoomIn control")
            ...
            .on("click", function () {
                ...
                gtag('event', 'zoomInWGCNAView', {'event_category': 'zoomInWGCNAView'});
            })
            .on("mouseover", function () {
        ...
        that.imageBar.append("span").attr("class", "zoomOut control")
            ...
            .on("click", function () {
                ...
                gtag('event', 'zoomOutWGCNAView', {'event_category': 'zoomOutWGCNAView'});
            })
            .on("mouseover", function () {
    ...
    that.createViewControls = function () {
        ...
        that.viewBar.append("input").attr("type", "radio").attr("id", "wgcnaEQTLViewRB").attr("name", "wgcnaViewRB").attr("value", "eqtl").style("margin-left", "7px").style("margin-right", "3px").attr('disabled', function () {
            ...
            gtag('event', 'eQTLView', {'event_category': 'WGCNA'});
        });
        ...
        that.viewBar.append("br");
        that.viewBar.append("input").attr("type", "radio").attr("id", "wgcnaGOViewRB").attr("name", "wgcnaViewRB").attr("value", "go").style("margin-left", "7px").style("margin-right", "3px").attr('disabled', function () {
            if (that.wDSID > 10) {
                return true;
            } else {
                return false;
            ...
            .on("click", function () {
                ...
                gtag('event', 'goView', {'event_category': 'WGCNA'});
            });
        ...
        that.viewBar.append("input").attr("type", "radio").attr("id", "wgcnaMirViewRB")
            ...
            .on("click", function () {
                ...
                gtag('event', 'mirView', {'event_category': 'WGCNA'});
            });
        ...
        that.viewBar.append("br");
        that.viewBar.append("input").attr("type", "radio").attr("id", "wgcnaMetaViewRB").attr("name", "wgcnaViewRB").attr("value", "meta").style("margin-left", "7px").style("margin-right", "3px").attr('disabled', function () {
            if (that.wDSID > 10) {
                return true;
            } else {
                return false;
            ...
            .on("click", function () {
                ...
                gtag('event', 'metaView', {'event_category': 'WGCNA'});
            });
    ...
    that.createDataControls = function () {
        ...
        var sel = dataSel.append("select").attr("id", "wgcnaSourceSelect").style("margin-top", "5px").style("margin-bottom", "5px").on("change", function () {
            $("#singleWGCNASVG").hide();
            ...
            that.selLevel = $('#wgcnaLevelSelect').val();
            that.updateWDSID();
            if (that.selSource === "array") {
                $('span#spanVersion').hide();
                d3.select("#wgcnaTissueSelect").append("option").attr("value", "Heart").text("Heart");
                d3.select("#wgcnaTissueSelect").select("option[value=\"Kidney\"]").remove();
            } else if (that.selSource === "seq") {
                d3.select("#wgcnaTissueSelect").append("option").attr("value", "Heart").text("Heart");
                d3.select("#wgcnaTissueSelect").append("option").attr("value", "Kidney").text("Kidney");
                $('span#spanVersion').show();
                /*else {
                    that.wDSID = 14;
                    that.version = "6";
                    d3.select("#wgcnaTissueSelect").select("option[value=\"Brain\"]").attr("selected", "selected");
                    d3.select("#wgcnaVersionSelect").select("option[value=\"6\"]").attr("selected", "selected");
                }*/
                if (that.version == "5" || that.version == "2" || that.version == "1") {
        ...
        } else if (organism === "Rn" && genomeVer === "rn7") {
            that.sources = ["seq"];
            that.dispSources = ["RNA-Seq"];
        }
        for (var i = 0; i < that.sources.length; i++) {
            sel.append("option").attr("value", that.sources[i]).text(that.dispSources[i]);
        }

        dataSel.append("text").text("Tissue:").style("margin-left", "5px");
        var sel = dataSel.append("select").attr("id", "wgcnaTissueSelect").style("margin-top", "5px").style("margin-bottom", "5px").on("change", function () {
            $("#singleWGCNASVG").hide();
            d3.select("#multiWGCNASVG").remove();
            $("#circosModule").hide();
            $("#wgcnaModuleTable").hide();
        ...
        if (organism === "Rn" && genomeVer === "rn6") {
            ...
            that.tissues = ["Whole Brain", "Liver", "Heart", "Kidney"];
            that.dispTissues = ["Whole Brain", "Liver", "Heart", "Kidney"];
        }
        ...
        var sel = spanV.append("select").attr("id", "wgcnaVersionSelect").style("margin-top", "5px").style("margin-bottom", "5px").on("change", function () {
            that.selSource = $('#wgcnaSourceSelect').val();
            that.tissue = $('#wgcnaTissueSelect').val();
            that.version = $('#wgcnaVersionSelect').val();
            that.selLevel = $('#wgcnaLevelSelect').val();
        ...
        //////////////////////////////////////////////////////////////////////////
        //dataSel.append("br");
        var spanL = dataSel.append("span").attr("id", "spanLevel").style("display", function () {
            if (genomeVer === "rn7") {
                return "inline-block";
            } else {
                return "none";
        ...
        spanL.append("text").text("Level:").style("margin-left", "5px");
        var sel = spanL.append("select").attr("id", "wgcnaLevelSelect").style("margin-top", "5px").style("margin-bottom", "5px").on("change", function () {
            that.selSource = $('#wgcnaSourceSelect').val();
            that.tissue = $('#wgcnaTissueSelect').val();
            that.version = $('#wgcnaVersionSelect').val();
            that.selLevel = $('#wgcnaLevelSelect').val();
        ...
        sel.append("option").attr("value", "gene").text("Gene");
        sel.append("option").attr("value", "trx").text("Transcripts");
        dataSel.append("br");
        var spanV = dataSel.append("span").attr("id", "spanReturnType");
        spanV.append("text").text("Module selection by:");
        var sel = spanV.append("select").attr("id", "wgcnaModuleSelect").style("margin-top", "5px").style("margin-bottom", "5px").on("change", function () {
        ...
        /////////////////Temporary
        if (genomeVer === "rn7") {
            d3.select("#wgcnaModuleSelect").select("option[value=\"eQTL\"]").attr("disabled", "disabled");
        }
        //////////////////End Temp
        d3.select("#wgcnaModuleSelect").select("option[value=\"" + that.modSelect + "\"]").attr("selected", "selected");
        $('span#spanReturnType').show();
        sel = spanVC.append("select").attr("id", "wgcnaModuleEQTLCutoff").style("margin-top", "5px").style("margin-bottom", "5px").on("change", function () {
        ...
        sel = go.append("select").attr("id", "goDomainSelect").on("change", function () {
            ...
            gtag('event', 'changeDomain', {'event_category': 'WGCNA_go'});
        });
        ...
        $("#goSpinner").spinner({
            ...
            change: function (event, ui) {
                ...
                gtag('event', 'changeDepth', {'event_category': 'WGCNA_go'});
            },
        ...
        mir.append("input").attr("type", "checkbox").style("margin-right", "5px").attr("id", "mirValidatedCBX").on("click", function () {
            ...
            gtag('event', 'changeValidated', {'event_category': 'WGCNA_mir'});
        });
        ...
        mir.append("input").attr("type", "text").attr("id", "mirGeneFilterTxt").on("keyup", function (d) {
            ...
            gtag('event', 'geneFilter', {'event_category': 'WGCNA_mir'});
        });
        mir.append("br");
        ...
        mir.append("input").attr("type", "checkbox").attr("id", "mirSortCBX").style("margin-right", "5px").on("click", function () {
            ...
            gtag('event', 'mirSort', {'event_category': 'WGCNA_mir'});
        });
        ...
        mir.append("input").attr("type", "checkbox")
            ...
            .on("click", function () {
                ...
                gtag('event', 'showLinks', {'event_category': 'WGCNA_mir'});
            });
        ...
        mir.append("input").attr("type", "checkbox")
            ...
            .on("click", function () {
                ...
                gtag('event', 'linkColor', {'event_category': 'WGCNA_mir'});
            });
        ...
        $("#minMirGeneSlider").slider({
            ...
            slide: function (event, ui) {
                ...
                gtag('event', 'minMir', {'event_category': 'WGCNA_mir'});
            }
        });
        ...
        links.append("span").style("cursor", "pointer")
            .on("click", function () {
                ...
                $("#linkSliderMax").slider('value', 0.5);
                that.singleImage.CorCutoff_max = 1;
                $("span#maxLabel").html("0.5+");

                that.singleImage.draw();
                /*if(ga){
								ga('send','event','WGCNA_mir','resetCorrelation');
							}*/
                gtag('event', 'resetCorrelation', {'event_category': 'WGCNA_mir'});
            })
        ...
        $("#linkSliderMin").slider({
            ...
            slide: function (event, ui) {
                ...
                if (that.singleImage.CorCutoff_min <= 0.002) {
                    that.singleImage.CorCutoff_min = 0
                }
                that.singleImage.CorCutoff_min = that.singleImage.CorCutoff_min * 1;
                $("span#minLabel").html(ui.value);
                //that.singleImage.CorCutoff_max=ui.values[ 1 ];
                setTimeout(function () {
                    that.singleImage.filterLinks();
                    that.singleImage.draw();
                }, 10);
        ...
        $("#linkSliderMax").slider({
            ...
            slide: function (event, ui) {
                ...
                tmptext = ui.value;
                if (that.singleImage.CorCutoff_max >= 0.49) {
                    that.singleImage.CorCutoff_max = 1;
                    tmptext = "0.5+";
                }
                that.singleImage.CorCutoff_max = that.singleImage.CorCutoff_max * 1;
                $("span#maxLabel").html(tmptext);
                setTimeout(function () {
                    that.singleImage.filterLinks();
                    that.singleImage.draw();
                }, 10);
        ...
        chr.append("input").attr("type", "button").attr("value", "Apply Selections").on("click", function () {
            ...
            $.ajax({
                ...
                data: {
                    cutoffValue: cutoff,
                    organism: organism,
                    module: that.selectedModule.MOD_NAME,
                    modColor: that.selectedModule.ModRGB,
                    chrList: cList,
                    genomeVer: genomeVer,
                    tissue: that.tissue
                },
    ...
    that.multiWGCNAImage = function () {
        ...
        thatimg.sortTCLenDesc = function (a, b) {
            return b.TCList.length < a.TCList.length ? -1 : b.TCList.length > a.TCList.length ? 1 : 0;
        };
        thatimg.sortGeneInstanceDesc = function (a, b) {
            return b.GeneCount < a.GeneCount ? -1 : b.GeneCount > a.GeneCount ? 1 : thatimg.sortTCLenDesc(a, b);
        };
    ...
    that.multiWGCNAImageGeneView = function () {
        ...
        thatimg.draw = function () {
            ...
            var min = d3.min(data, function (d) {
                return d.TCList.length;
            });
            var max = d3.max(data, function (d) {
                return d.TCList.length;
            });

            thatimg.size = d3.scaleLinear().domain([min, max]).range([10, thatimg.maxWidth / 2]);
    ...
    that.createSingleWGCNAImage = function () {
        ...
        gtag('event', that.viewType, {'event_category': 'changedWGCNAView'});
    };
    ...
    that.singleWGCNAImage = function () {
        ...
        thatimg.geneKey = function (d) {
            return d.ID;
        };
        thatimg.linkKey = function (d) {
            return that.removeInvalidIDChars(d.TC1) + that.removeInvalidIDChars(d.TC2);
        };
        ...
        thatimg.setupLegend = function () {
            ...
            thatimg.legendG = thatimg.svg.append("g")
                ...
                .on("mouseover", function () {
                    ...
                    gtag('event', 'viewLegend', {'event_category': 'wgcna'});
                })
    ...
    that.singleWGCNAImageGeneView = function () {
        ...
        thatimg.sortMir = 0;
        thatimg.maxCorWidth = 15;


        thatimg.filterLinksGene = function (d) {

            var filterLinks = [];
            var count = 0;
            for (var i = 0; i < that.selectedModule.LinkList.length; i++) {
                if (that.selectedModule.LinkList[i].TC1 == d.ID || that.selectedModule.LinkList[i].TC2 == d.ID) {
                    filterLinks[count] = that.selectedModule.LinkList[i];
                    count++;

                }
            }
            that.selectedModule.filteredLinks = filterLinks;
        };
        thatimg.filterLinks = function () {
            console.log("filterLinks:" + thatimg.CorCutoff_min + "_" + thatimg.CorCutoff_max);
            var filterLinks = [];
            var count = 0;
            for (var i = 0; i < that.selectedModule.LinkList.length; i++) {
                if (that.selectedGene[that.selectedModule.LinkList[i].TC1] && that.selectedGene[that.selectedModule.LinkList[i].TC2]) {
                    console.log("found:");
                    var absVal = Math.abs(that.selectedModule.LinkList[i].Cor);
                    console.log("cor:" + absVal);
                    if (thatimg.CorCutoff_min <= absVal && absVal <= thatimg.CorCutoff_max) {
        ...
        thatimg.drawLinks = function (d) {
            ...
            if (that.selectedModule.filteredLinks.length < 10000) {
                console.log("filteredLen:" + that.selectedModule.filteredLinks.length);
                var lnk = thatimg.topG.selectAll(".link").data(that.selectedModule.filteredLinks, thatimg.linkKey);
                lnk.enter().append("line")
                    .attr("class", "link")
                    .attr("pointer-events", "none")
                    .attr("id", function (d) {
                        return "ln_" + that.removeInvalidIDChars(d.TC1) + that.removeInvalidIDChars(d.TC2);
                    })
                    .attr("x1", function (d) {
                        var n = d3.select("#tc_" + that.removeInvalidIDChars(d.TC1));
                        var x = 0;
                        if (n) {
                            x = n.attr("cx");
                        }
                    ...
                    .attr("x2", function (d) {
                        var n = d3.select("#tc_" + that.removeInvalidIDChars(d.TC2));
                        var x = 0;
                        if (n) {
                            x = n.attr("cx");
                        }
        ...
        thatimg.drawLinksBetween = function (list) {
            ...
            thatimg.svg.selectAll(".link").remove();
            var links = that.selectedModule.filteredLinks;
            var filterLinks = [];
            var count = 0;
        ...
        thatimg.draw = function () {
            if (!that.selectedModule.filteredLinks) {
                thatimg.filterLinks();
            }
            ...
            /*if (that.viewType === 'gene') {
                ...
                tc.enter().append("circle")
                    .attr("class", "geneTC")
                    .attr("id", function (d) {
                        return "tc_" + that.removeInvalidIDChars(d.ID);
                    })
            ...
            /*   var links = that.selectedModule.LinkList;
               ...
               if (thatimg.showLinks && links.length < 10000) {
                   ...
                   lnk.enter().append("line")
                       ...
                       .attr("x1", function (d) {
                           //console.log(d.TC1 + ":x1:");
                           //console.log(d3.select("#tc_" + that.removeInvalidIDChars(d.TC1)));
                           var x = d3.select("#tc_" + that.removeInvalidIDChars(d.TC1)).attr("cx");
                           return x;
                       })
                       .attr("y1", function (d) {
            ...
            tc.enter().append("g")
                ...
                .each(function (d, i) {
                    d3.select(this.parentNode).append("g").attr("class", "label")
                        .attr("id", function (d) {
                            return "txt_" + that.removeInvalidIDChars(d.ID);
                        })
            ...
            if (thatimg.showLinks && links.length < 10000) {
                ...
                lnk.enter().append("line")
                    ...
                    .attr("x2", function (d) {
                        //console.log(d.TC1 + ":" + d.TC2);
                        //console.log("#tc_" + that.removeInvalidIDChars(d.TC1));
                        //console.log(d3.select("#tc_" + that.removeInvalidIDChars(d.TC1)));
                        //console.log("#tc_" + that.removeInvalidIDChars(d.TC2));
                        var n = d3.select("#tc_" + that.removeInvalidIDChars(d.TC2));
                        //console.log(n);
                        var x = 0;
                        if (n[0]) {
        ...
        thatimg.drawMiR = function () {
            if (typeof thatimg.topG !== 'undefined' && typeof thatimg.mirData !== 'undefined') {
                ...
                thatimg.hierarch = d3.hierarchy({children: tmpData}).sum(function (d) {
                    return d.filterGLSize;
                });
                ...
                node.enter().append("g")
                    .attr("class", "node")
                    .attr("id", function (d) {
                        return that.removeInvalidIDChars(d.ID);
                    })
                    ...
                    if (d.depth > 0) {
                        ...
                        d3.select(this).append("circle")
                            ...
                            .style("fill", function (d) {
                                return thatimg.miRcolor(tmpD.ID);
                            });
                        var txt = tmpD.ID.substring(8);
                        if (txt.length < d.r / 3) {
                            d3.select(this).append("text")
        ...
        thatimg.mouseoverMir = function () {
            ...
            gtag('event', 'mouseOverMir', {'event_category': 'wgcna'});
        };
        ...
        thatimg.mouseoverGene = function () {
            ...
            gtag('event', 'mouseOverGene', {'event_category': 'wgcna'});
        };
    ...
    that.singleWGCNATableMirView = function () {
        ...
        thattbl.createMirTable = function () {
            ...
            var tracktbl = d3.select("table#mirTable")
                ...
                .enter().append("tr")
                .attr("id", function (d) {
                    return "mirTR" + that.removeInvalidIDChars(d.ID);
                })
                .attr("class", function (d, i) {
                    var ret = "odd";
                    if (i % 2 === 0) {
        ...
        thattbl.createMirGeneTable = function () {
            ...
            var tracktbl = d3.select("table#mirGeneTable").select("tbody").selectAll('tr').data(that.selectedModule.TCList, that.geneKey)
                .enter().append("tr")
                .attr("id", function (d) {
                    return "mirg" + that.removeInvalidIDChars(d.ID);
                })
                .attr("class", function (d, i) {
                    var ret = "odd";
                    if (i % 2 === 0) {
        ...
        thatimg.redraw = function () {
            ...
            thatimg.arc = d3.arc()
                ...
                .outerRadius(function (d) {
                    return Math.max(0, (d.depth - thatimg.rootN.depth) * (thatimg.radius / (tmpDisplayDepth + 1)) + (thatimg.radius / (tmpDisplayDepth + 1)));
                });
            ...
            if (thatimg.selectedNode !== thatimg.data.GOList[0] && thatimg.selectedNode !== thatimg.data.GOList[1] && thatimg.selectedNode !== thatimg.data.GOList[2]) {
                thatimg.svg.append("circle").attr("id", "center").attr("cx", thatimg.width / 2 + thatimg.padding).attr("cy", thatimg.height / 2 + thatimg.padding).attr("r", "20")
                    .attr("fill-rule", "evenodd")
                    .attr("fill", function () {
                        return thatimg.pathColor(thatimg.rootN.parent);
                    })
                    .attr("pointer-events", "all")
                    .on("click", function () {
            ...
            if (typeof thatimg.data.MOD_NAME === 'undefined' || (typeof that.selectedModule.MOD_NAME !== 'undefined' && replaceUnderscore(thatimg.data.MOD_NAME) !== replaceUnderscore(that.selectedModule.MOD_NAME))) {
                ...
                if (thatimg.goDomain !== thatimg.lastDrawn || typeof thatimg.selectedNode === 'undefined' || thatimg.selectedNode === null) {
                    ...
                    thatimg.rootN = d3.hierarchy(goRoot)
                        .sum(function (d) {
                            if (d && d.name && d.name.indexOf("ENS") > -1) {
                                return 1;
                            } else {
                                return d.uniqueGene;
                            }
                ...
                thatimg.arc = d3.arc()
                    ...
                    .outerRadius(function (d) {
                        return Math.max(0, d.depth * (thatimg.radius / (tmpDisplayDepth + 1)) + (thatimg.radius / (tmpDisplayDepth + 1)));
                    });
        ...
        thatimg.updateBreadcrumbs = function (nodeArray, percentageString) {
            ...
            d3.select("#trail")
                .attr("height", function () {
                    var tmpH = thatimg.b.runH + thatimg.b.h + 5;
                    if (tmpH < 68) {
                        tmpH = 68;
                    }
                    return tmpH;
    ...
    that.singleWGCNATableGoView = function () {
        ...
        tracktbl.enter().append("tr")
            ...
            .on("click", function (d) {
                ...
                gtag('event', 'wgcna', {'event_category': 'mouseClickGOTbl'});
            }).each(function (d, i) {
            var tmpI = i;
            var tmpD = d;
                ...
                success: function (data2) {
                    ...
                    d3.select("#circosModule").selectAll(".circosGene").style("cursor", "pointer")
                        .on("mouseover", function () {
                            ...
                            gtag('event', 'mouseOverEQTL', {'event_category': 'wgcna'});
                        })
                        .on("mouseout", function () {
    ...
    that.singleWGCNAImageMetaView = function () {
        ...
        thatimg.mlinkKey = function (d) {
            return that.removeInvalidIDChars(d.MOD1) + "_" + that.removeInvalidIDChars(d.MOD2);
        };
        ...
        thatimg.drawLinks = function (d) {
            ...
            if (links.length < 10000) {
                ...
                lnk.enter().append("line")
                    ...
                    .attr("pointer-events", "none")
                    .attr("id", function (d) {
                        return "ln_" + that.removeInvalidIDChars(d.MOD1) + that.removeInvalidIDChars(d.MOD2);
                    })
                    .attr("x1", function (d) {
        ...
        thatimg.draw = function () {
            ...
            setTimeout(function () {
                ...
                tc.enter().append("circle")
                    .attr("class", "mod")
                    .attr("id", function (d) {
                        return "mod_" + that.removeInvalidIDChars(d.MODULE);
                    })
                    .attr("cx", thatimg.nodeX)
                    .attr("cy", thatimg.nodeY)
                    .attr("r", 3)
                ...
                tc.enter().append("g")
                    ...
                    .attr("fill", thatimg.colorCircle)
                    .attr("class", function (d) {
                        return "mod " + that.removeInvalidIDChars(d.MODULE);
                    })
                    .attr("id", function (d) {
                        return "mod_" + that.removeInvalidIDChars(d.MODULE);
                    })
                    ...
                    .each(function (d, i) {
                        d3.select(this.parentNode).append("g").attr("class", "label")
                            .attr("id", function (d) {
                                return "txt_" + that.removeInvalidIDChars(d.MODULE);
                            })
                            .attr("transform", function () {
                                var x = thatimg.nodeXLbl(d, i);
                                var y = thatimg.nodeYLbl(d, i);
        ...
        thatimg.requestMeta = function () {
            $.ajax({
                ...
                data: {
                    source: that.selSource,
                    id: replaceUnderscore(that.selectedModule.MOD_NAME),
                    organism: organism,
                    panel: that.panel,
                    tissue: that.tissue,
                    region: that.region,
        ...
        thatimg.mouseoverModule = function () {
            ...
            gtag('event', 'mouseOverModule', {'event_category': 'wgcna'});
        };
